...
(function () {
  ...
  const g1m = [
    ...
    (r) => {
      ...
      return { q: T(`A technical drawing is made by rays that all make an angle of $${ang}^\\circ$ with the projection plane (not $90^\\circ$). Is this an orthogonal projection? Justify your answer.`, `Suatu lukisan teknikal dibuat menggunakan sinar yang semuanya membuat sudut $${ang}^\\circ$ dengan satah unjuran (bukan $90^\\circ$). Adakah ini unjuran ortogon? Berikan justifikasi.`), a: T(`No. An orthogonal projection requires the rays to be perpendicular ($90^\\circ$) to the plane; here they meet it at $${ang}^\\circ$, so it is an oblique projection.`, `Tidak. Unjuran ortogon memerlukan sinar berserenjang ($90^\\circ$) dengan satah; di sini sinar bertemu satah pada $${ang}^\\circ$, jadi ia adalah unjuran senget.`), w: W(ORTHW, `$${ang}^\\circ \\neq 90^\\circ$`, T('The rays are parallel but not perpendicular to the plane, so the projection is oblique, not orthogonal.', 'Sinar itu selari tetapi tidak berserenjang dengan satah, jadi unjuran itu senget, bukan ortogon.')), sp: 's' };
    },
  ...
  const g2m = [
    ...
    (r) => {
      ...
      return { q, a, w: W(SHAPEW, cmp, claim ? T('Both parts of the claim hold, so it is true.', 'Kedua-dua bahagian dakwaan itu benar, jadi ia betul.') : T('At least one part of the claim fails, so it is false.', 'Sekurang-kurangnya satu bahagian dakwaan itu gagal, jadi ia salah.')), sp: 'm' };
    },
