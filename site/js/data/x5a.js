...
(function () {
  ...
  const tfFrame = (bank) => (r) => {
    ...
    return { q: T(`True or False: ${stmt.en}`, `Betul atau Salah: ${stmt.ms}`), a: showTrue ? T('True', 'Betul') : T(`False — the correct statement is: ${f[0].en}`, `Salah — pernyataan yang betul ialah: ${f[0].ms}`), sp: 's' };
  };
  const spotFrame = (bank) => (r) => {
    const f = r.pick(bank);
    return { q: T(`A student claims: "${f[1].en}" Identify the error and give the correct statement.`, `Seorang murid mendakwa: "${f[1].ms}" Kenal pasti kesilapan itu dan berikan pernyataan yang betul.`), a: f[0], sp: 's' };
  };
  const mcqFrame = (bank, topicEn, topicMs) => (r) => {
    ...
    return { q: cat(T(`Which of the following statements about ${topicEn} is TRUE?<br>`, `Antara pernyataan berikut tentang ${topicMs}, yang manakah BETUL?<br>`), o.q), a: o.a, sp: 's' };
  };
  const pickTrueFrame = (bank, k) => (r) => {
    ...
    return { q: cat(T(`${only.en}?<br>`, `${only.ms}?<br>`), o.q), a: o.a, sp: 's' };
  };
  const dualSpotFrame = (bank) => (r) => {
    const [f1, f2] = r.sample(bank, 2);
    return { q: T(`A student made two claims: (a) "${f1[1].en}" (b) "${f2[1].en}" For each, identify the error and give the correct statement.`, `Seorang murid membuat dua dakwaan: (a) "${f1[1].ms}" (b) "${f2[1].ms}" Bagi setiap satu, kenal pasti kesilapan itu dan berikan pernyataan yang betul.`), a: PT([f1[0], f2[0]]), sp: 'm' };
  };
  ...
  const CONCEPT_DV = [
    [T('If $y$ varies directly as $x$, the graph of $y$ against $x$ is a straight line passing through the origin.', 'Jika $y$ berubah secara langsung dengan $x$, graf $y$ melawan $x$ ialah garis lurus yang melalui asalan.'), T('If $y$ varies directly as $x$, the graph of $y$ against $x$ is a straight line that does not pass through the origin.', 'Jika $y$ berubah secara langsung dengan $x$, graf $y$ melawan $x$ ialah garis lurus yang tidak melalui asalan.')],
    [T('In $y = kx$, the constant $k$ is also the gradient of the graph of $y$ against $x$.', 'Dalam $y = kx$, pemalar $k$ juga ialah kecerunan graf $y$ melawan $x$.'), T('In $y = kx$, the constant $k$ is the $y$-intercept of the graph of $y$ against $x$.', 'Dalam $y = kx$, pemalar $k$ ialah pintasan-$y$ graf $y$ melawan $x$.')],
    [T('If $y$ varies directly as $x$, doubling $x$ doubles $y$.', 'Jika $y$ berubah secara langsung dengan $x$, menggandakan dua $x$ akan menggandakan dua $y$.'), T('If $y$ varies directly as $x$, doubling $x$ leaves $y$ unchanged.', 'Jika $y$ berubah secara langsung dengan $x$, menggandakan dua $x$ tidak mengubah $y$.')],
    ...
    [T('When $y$ varies directly as $x$, the ratio $\\dfrac{y}{x}$ is constant.', 'Apabila $y$ berubah secara langsung dengan $x$, nisbah $\\dfrac{y}{x}$ adalah malar.'), T('When $y$ varies directly as $x$, the product $xy$ is constant.', 'Apabila $y$ berubah secara langsung dengan $x$, hasil darab $xy$ adalah malar.')],
  ...
  const dv11_backward = (r) => {
    ...
    return { q: T(`$y$ varies directly as ${dvXpEn(pw)}. Given $y = ${y1}$ when $x = ${x1}$, find the value of $x$ when $y = ${y2}$.`, `$y$ berubah secara langsung dengan ${dvXpMs(pw)}. Diberi $y = ${y1}$ apabila $x = ${x1}$, cari nilai $x$ apabila $y = ${y2}$.`), a: T(`$k = ${k}$; $x = ${x2}$`), sp: 's' };
  };
  const dv11_word = (r) => {
    const c = r.pick(DVCTX)(r);
    const x2 = r.int(2, 12);
    return { q: T(`${c.qEn} Write the equation connecting $${c.ys}$ and $${c.xs}$, then find $${c.ys}$ when $${c.xs} = ${x2}$.`, `${c.qMs} Tuliskan persamaan yang menghubungkan $${c.ys}$ dengan $${c.xs}$, kemudian cari $${c.ys}$ apabila $${c.xs} = ${x2}$.`), a: T(`$${c.ys} = ${c.k}${c.xs}$; $${c.ys} = ${c.k * x2}$`), sp: 's' };
  };
  const dv11_multiplier = (r) => {
    const pw = r.pick([DV[0], DV[1], DV[2]]);
    const factor = r.pick([2, 3]);
    const mult = Math.pow(factor, pw.kind === 'lin' ? 1 : pw.kind === 'sq' ? 2 : 3);
    return { q: T(`$y$ varies directly as ${dvXpEn(pw)}. If $x$ is multiplied by ${factor}, by what factor is $y$ multiplied?`, `$y$ berubah secara langsung dengan ${dvXpMs(pw)}. Jika $x$ didarab dengan ${factor}, $y$ didarab dengan faktor berapa?`), a: T(`$${mult}$`), sp: 'xs' };
  };
  const dv11_writeEq = (r) => {
    const pw = r.pick(DV);
    const subjEn = r.pick([['the perimeter $P$ of a square', 'its side $x$'], ['a quantity $y$', 'another quantity $x$']]);
    return { q: T(`Write, in the form $y = kx^{${pw.kind === 'sqrt' ? '1/2' : pw.kind === 'cbrt' ? '1/3' : pw.kind === 'sq' ? '2' : pw.kind === 'cb' ? '3' : '1'}}$, the statement: "$y$ ${DVPH[0].en(dvXpEn(pw))}, where $k$ is a constant."`, `Tulis, dalam bentuk $y = kx^{${pw.kind === 'sqrt' ? '1/2' : pw.kind === 'cbrt' ? '1/3' : pw.kind === 'sq' ? '2' : pw.kind === 'cb' ? '3' : '1'}}$, pernyataan: "$y$ ${DVPH[0].ms(dvXpMs(pw))}, dengan keadaan $k$ ialah pemalar."`), a: T(`$y = k${pw.tex('x')}$`), sp: 'xs' };
  };
  ...
  const dv11_mcqEq = (r) => {
    ...
    return { q: cat(T(`"$y$ ${DVPH[0].en(dvXpEn(pw))}, where $k$ is a nonzero constant." Which equation represents this?<br>`, `"$y$ ${DVPH[0].ms(dvXpMs(pw))}, dengan keadaan $k$ ialah pemalar bukan sifar." Persamaan manakah mewakili ini?<br>`), o.q), a: o.a, sp: 's' };
  };
  ...
  const dv11_findK_sq = (r) => {
    ...
    const y1 = k * dvVal(pw.kind, x1), y2 = k * dvVal(pw.kind, x2);
    return { q: T(`$y \\propto ${pw.tex('x')}$ and $y = ${y1}$ when $x = ${x1}$. Find $y$ when $x = ${x2}$.`, `$y \\propto ${pw.tex('x')}$ dan $y = ${y1}$ apabila $x = ${x1}$. Cari $y$ apabila $x = ${x2}$.`), a: T(`$k = ${k}$; $y = ${y2}$`), sp: 's' };
  };
  const dv11_table2gap = (r) => {
    ...
    return { q: T(`$y$ varies directly as ${dvXpEn(pw)}. Complete the table.<br>${tab}`, `$y$ berubah secara langsung dengan ${dvXpMs(pw)}. Lengkapkan jadual.<br>${tab}`), a: T(`$k = ${k}$; ${gaps.map((i) => `$x = ${xs[i]}$: $y = ${ys[i]}$`).join(', ')}`, `$k = ${k}$; ${gaps.map((i) => `$x = ${xs[i]}$: $y = ${ys[i]}$`).join(', ')}`), sp: 's' };
  };
  const dv11_pctChange = (r) => {
    ...
    return { q: T(`$y$ varies directly as ${dvXpEn(pw)}. If $x$ is increased by ${pct}%, find the percentage increase in $y$.`, `$y$ berubah secara langsung dengan ${dvXpMs(pw)}. Jika $x$ ditambah sebanyak ${pct}%, cari peratus pertambahan $y$.`), a: T(`$${change}\\%$`), w: T(`$y_{\\text{new}} = k(1.${pct < 100 ? String(pct).padStart(2, '0') : pct}x)^{${pwr}}$`), sp: 's' };
  };
  const dv11_reverseK = (r) => {
    ...
    return { q: T(`$y$ varies directly as ${dvXpEn(pw)}. When $x = ${x1}$, $y = ${frT(y1)}$. Find $y$ when $x = ${x2}$.`, `$y$ berubah secara langsung dengan ${dvXpMs(pw)}. Apabila $x = ${x1}$, $y = ${frT(y1)}$. Cari $y$ apabila $x = ${x2}$.`), a: T(`$k = ${frT(k)}$; $y = ${frT(y2)}$`), sp: 's' };
  };
  const dv11_testTable = (r) => {
    ...
    return { q: T(`The table shows values of $x$ and $y$.<br>${tab}<br>By finding $\\dfrac{y}{${pw.tex('x')}}$ for each pair, show that $y$ varies directly as ${dvXpEn(pw)}, and state the value of $k$.`, `Jadual menunjukkan nilai $x$ dan $y$.<br>${tab}<br>Dengan mencari $\\dfrac{y}{${pw.tex('x')}}$ bagi setiap pasangan, tunjukkan bahawa $y$ berubah secara langsung dengan ${dvXpMs(pw)}, dan nyatakan nilai $k$.`), a: T(`$\\dfrac{y}{${pw.tex('x')}} = ${k}$ for every pair (constant), so $k = ${k}$`, `$\\dfrac{y}{${pw.tex('x')}} = ${k}$ bagi setiap pasangan (malar), maka $k = ${k}$`), sp: 'm' };
  };
  const dv11_wordSq = (r) => {
    const shape = r.pick([T('square', 'segi empat sama'), T('circle-like tile pattern', 'corak jubin seakan-bulat')]);
    const k = r.int(2, 5);
    const x1 = r.int(2, 6), x2 = retry(() => { const v = r.int(2, 8); need(v !== x1); return v; });
    return { q: T(`The area, $A$ cm², of a ${shape.en} varies directly as the square of its side length, $x$ cm, with $A = ${k * x1 * x1}$ when $x = ${x1}$. Find $A$ when $x = ${x2}$.`, `Luas, $A$ cm², sesuatu ${shape.ms} berubah secara langsung dengan kuasa dua panjang sisinya, $x$ cm, dengan $A = ${k * x1 * x1}$ apabila $x = ${x1}$. Cari $A$ apabila $x = ${x2}$.`), a: T(`$k = ${k}$; $A = ${k * x2 * x2}\\ \\text{cm}^2$`), sp: 's' };
  };
  ...
  const dv11_determinePower = (r) => {
    ...
    return { q: T(`The table shows values of $x$ and $y$.<br>${tab}<br>By testing $\\dfrac{y}{x}$, $\\dfrac{y}{x^2}$ and $\\dfrac{y}{x^3}$, determine which one is constant, and write the relation between $y$ and $x$.`, `Jadual menunjukkan nilai $x$ dan $y$.<br>${tab}<br>Dengan menguji $\\dfrac{y}{x}$, $\\dfrac{y}{x^2}$ dan $\\dfrac{y}{x^3}$, tentukan yang manakah malar, dan tulis hubungan antara $y$ dengan $x$.`), a: T(`$\\dfrac{y}{${pwLabel}} = ${k}$ is constant, so $y = ${k}${pwLabel}$`, `$\\dfrac{y}{${pwLabel}} = ${k}$ malar, jadi $y = ${k}${pwLabel}$`), sp: 'm' };
  };
  ...
  const IVCTX = [
    (r) => { const w = r.int(4, 10), d = r.pick([6, 8, 9, 12, 18, 24]); return { qEn: `${w} workers can finish painting a school hall in ${d} days, all working at the same constant rate. The number of days needed, $D$, varies inversely as the number of workers, $w$.`, qMs: `${w} orang pekerja boleh menyiapkan pengecatan dewan sekolah dalam ${d} hari, semuanya bekerja pada kadar malar yang sama. Bilangan hari yang diperlukan, $D$, berubah secara songsang dengan bilangan pekerja, $w$.`, k: w * d, xs: 'w', ys: 'D' }; },
    (r) => { const spd = r.pick([40, 50, 60, 80]), t = r.pick([2, 3, 4, 6]); return { qEn: `A journey of ${spd * t} km is completed at a constant speed. The time taken, $t$ hours, varies inversely as the average speed, $v$ km/h.`, qMs: `Perjalanan sejauh ${spd * t} km dilalui pada kelajuan malar. Masa yang diambil, $t$ jam, berubah secara songsang dengan kelajuan purata, $v$ km/j.`, k: spd * t, xs: 'v', ys: 't' }; },
    (r) => { const total = r.pick([120, 180, 240, 360]), pp = r.pick([2, 3, 4, 6]); return { qEn: `A fixed prize of RM${total} is shared equally among $p$ winners. The amount, RM $A$, each winner receives varies inversely as $p$.`, qMs: `Hadiah tetap RM${total} dikongsi sama rata antara $p$ orang pemenang. Jumlah, RM $A$, yang diterima setiap pemenang berubah secara songsang dengan $p$.`, k: total, xs: 'p', ys: 'A' }; },
  ];
  const CONCEPT_IV = [
    [T('If $y$ varies inversely as $x$, the product $xy$ is constant.', 'Jika $y$ berubah secara songsang dengan $x$, hasil darab $xy$ adalah malar.'), T('If $y$ varies inversely as $x$, the ratio $\\dfrac{y}{x}$ is constant.', 'Jika $y$ berubah secara songsang dengan $x$, nisbah $\\dfrac{y}{x}$ adalah malar.')],
    [T('If $y$ varies inversely as $x$, doubling $x$ halves $y$.', 'Jika $y$ berubah secara songsang dengan $x$, menggandakan dua $x$ akan mengurangkan $y$ kepada separuh.'), T('If $y$ varies inversely as $x$, doubling $x$ also doubles $y$.', 'Jika $y$ berubah secara songsang dengan $x$, menggandakan dua $x$ turut menggandakan dua $y$.')],
    [T('For $y = \\dfrac{k}{x}$, $x$ cannot be $0$.', 'Bagi $y = \\dfrac{k}{x}$, $x$ tidak boleh menjadi $0$.'), T('For $y = \\dfrac{k}{x}$, $x$ can be $0$, giving $y = 0$.', 'Bagi $y = \\dfrac{k}{x}$, $x$ boleh menjadi $0$, memberi $y = 0$.')],
  ...
  const iv12_findK = (r) => {
    ...
    return { q: T(`$y$ ${ph.en(dvXpEn(pw))}. When $x = ${x1}$, $y = ${y1}$. Find (a) the equation connecting $y$ and $x$, (b) the value of $y$ when $x = ${x2}$.`, `$y$ ${ph.ms(dvXpMs(pw))}. Apabila $x = ${x1}$, $y = ${y1}$. Cari (a) persamaan yang menghubungkan $y$ dengan $x$, (b) nilai $y$ apabila $x = ${x2}$.`), a: T(`(a) $y = ${ivTex(pw).replace('k', k)}$ (b) $y = ${y2}$`), sp: 's' };
  };
  const iv12_backward = (r) => {
    const pw = r.pick([DV[0], DV[1]]);
    const { x1, x2, X1, X2, k } = ivPair(r, pw.kind);
    const y1 = k / X1, y2 = k / X2;
    return { q: T(`$y$ varies inversely as ${dvXpEn(pw)}. Given $y = ${y1}$ when $x = ${x1}$, find the value of $x$ when $y = ${y2}$.`, `$y$ berubah secara songsang dengan ${dvXpMs(pw)}. Diberi $y = ${y1}$ apabila $x = ${x1}$, cari nilai $x$ apabila $y = ${y2}$.`), a: T(`$k = ${k}$; $x = ${x2}$`), sp: 's' };
  };
  const iv12_word = (r) => {
    const c = r.pick(IVCTX)(r);
    const x2 = r.pick([2, 3, 4, 6].filter((v) => c.k % v === 0));
    need(x2);
    return { q: T(`${c.qEn} Write the equation connecting $${c.ys}$ and $${c.xs}$, then find $${c.ys}$ when $${c.xs} = ${x2}$.`, `${c.qMs} Tuliskan persamaan yang menghubungkan $${c.ys}$ dengan $${c.xs}$, kemudian cari $${c.ys}$ apabila $${c.xs} = ${x2}$.`), a: T(`$${c.ys} = \\dfrac{${c.k}}{${c.xs}}$; $${c.ys} = ${c.k / x2}$`), sp: 's' };
  };
  const iv12_multiplier = (r) => {
    const pw = r.pick([DV[0], DV[1], DV[2]]);
    const factor = r.pick([2, 3]);
    const pwr = pw.kind === 'lin' ? 1 : pw.kind === 'sq' ? 2 : 3;
    return { q: T(`$y$ varies inversely as ${dvXpEn(pw)}. If $x$ is multiplied by ${factor}, by what factor is $y$ multiplied?`, `$y$ berubah secara songsang dengan ${dvXpMs(pw)}. Jika $x$ didarab dengan ${factor}, $y$ didarab dengan faktor berapa?`), a: T(`$\\dfrac{1}{${Math.pow(factor, pwr)}}$`), sp: 'xs' };
  };
  const iv12_writeEq = (r) => {
    const pw = r.pick(DV);
    const expo = pw.kind === 'sqrt' ? '1/2' : pw.kind === 'cbrt' ? '1/3' : pw.kind === 'sq' ? '2' : pw.kind === 'cb' ? '3' : '1';
    return { q: T(`Write, in the form $y = kx^{-${expo}}$, the statement: "$y$ ${IVPH[0].en(dvXpEn(pw))}, where $k$ is a constant."`, `Tulis, dalam bentuk $y = kx^{-${expo}}$, pernyataan: "$y$ ${IVPH[0].ms(dvXpMs(pw))}, dengan keadaan $k$ ialah pemalar."`), a: T(`$y = ${ivTex(pw).replace('k', 'k')}$`), sp: 'xs' };
  };
  const iv12_table1gap = (r) => {
    ...
    const gap = r.int(1, 3);
    const tab = SPM.table([['$x$', ...xs.map(String)], ['$y$', ...ys.map((y, i) => (i === gap ? '' : String(y)))]]);
    return { q: T(`$y$ varies inversely as ${dvXpEn(pw)}. The table shows some values of $x$ and $y$.<br>${tab}<br>Find the value of $k$ and the missing value of $y$.`, `$y$ berubah secara songsang dengan ${dvXpMs(pw)}. Jadual menunjukkan beberapa nilai $x$ dan $y$.<br>${tab}<br>Cari nilai $k$ dan nilai $y$ yang tertinggal.`), a: T(`$k = ${k}$; $y = ${ys[gap]}$ when $x = ${xs[gap]}$`, `$k = ${k}$; $y = ${ys[gap]}$ apabila $x = ${xs[gap]}$`), sp: 's' };
  };
  const iv12_mcqEq = (r) => {
    const pw = r.pick(DV);
    ...
    return { q: cat(T(`"$y$ ${IVPH[0].en(dvXpEn(pw))}, where $k$ is a nonzero constant." Which equation represents this?<br>`, `"$y$ ${IVPH[0].ms(dvXpMs(pw))}, dengan keadaan $k$ ialah pemalar bukan sifar." Persamaan manakah mewakili ini?<br>`), o.q), a: o.a, sp: 's' };
  };
  ...
  const iv12_findK_sq = (r) => {
    ...
    const { x1, x2, X1, X2, k } = ivPair(r, pw.kind);
    const y1 = k / X1, y2 = k / X2;
    return { q: T(`$y \\propto \\dfrac{1}{${pw.tex('x')}}$ and $y = ${y1}$ when $x = ${x1}$. Find $y$ when $x = ${x2}$.`, `$y \\propto \\dfrac{1}{${pw.tex('x')}}$ dan $y = ${y1}$ apabila $x = ${x1}$. Cari $y$ apabila $x = ${x2}$.`), a: T(`$k = ${k}$; $y = ${y2}$`), sp: 's' };
  };
  ...
  const iv12_table2gap = (r) => {
    ...
    const gaps = r.sample([0, 1, 2, 3], 2).sort((a, b) => a - b);
    const tab = SPM.table([['$x$', ...xs.map(String)], ['$y$', ...ys.map((y, i) => (gaps.includes(i) ? '' : String(y)))]]);
    return { q: T(`$y$ varies inversely as ${dvXpEn(pw)}. Complete the table.<br>${tab}`, `$y$ berubah secara songsang dengan ${dvXpMs(pw)}. Lengkapkan jadual.<br>${tab}`), a: T(`$k = ${k}$; ${gaps.map((i) => `$x = ${xs[i]}$: $y = ${ys[i]}$`).join(', ')}`, `$k = ${k}$; ${gaps.map((i) => `$x = ${xs[i]}$: $y = ${ys[i]}$`).join(', ')}`), sp: 's' };
  };
  const iv12_testTable = (r) => {
    ...
    const ys = Xs.map((X) => k / X);
    const tab = SPM.table([['$x$', ...xs.map(String)], ['$y$', ...ys.map(String)]]);
    return { q: T(`The table shows values of $x$ and $y$.<br>${tab}<br>By finding $y \\times ${pw.tex('x')}$ for each pair, show that $y$ varies inversely as ${dvXpEn(pw)}, and state the value of $k$.`, `Jadual menunjukkan nilai $x$ dan $y$.<br>${tab}<br>Dengan mencari $y \\times ${pw.tex('x')}$ bagi setiap pasangan, tunjukkan bahawa $y$ berubah secara songsang dengan ${dvXpMs(pw)}, dan nyatakan nilai $k$.`), a: T(`$y \\times ${pw.tex('x')} = ${k}$ for every pair (constant), so $k = ${k}$`, `$y \\times ${pw.tex('x')} = ${k}$ bagi setiap pasangan (malar), maka $k = ${k}$`), sp: 'm' };
  };
  const iv12_pctChange = (r) => {
    ...
    const change = round((Math.pow(1 + pct / 100, -pwr) - 1) * 100, 2);
    return { q: T(`$y$ varies inversely as ${dvXpEn(pw)}. If $x$ is increased by ${pct}%, find the percentage decrease in $y$.`, `$y$ berubah secara songsang dengan ${dvXpMs(pw)}. Jika $x$ ditambah sebanyak ${pct}%, cari peratus pengurangan $y$.`), a: T(`$${-change}\\%$`), w: T(`$y_{\\text{new}} = k(1.${pct}x)^{-${pwr}}$`), sp: 's' };
  };
  const iv12_reverseK = (r) => {
    ...
    return { q: T(`$y$ varies inversely as ${dvXpEn(pw)}. When $x = ${x1}$, $y = ${frT(y1)}$. Find $y$ when $x = ${x2}$.`, `$y$ berubah secara songsang dengan ${dvXpMs(pw)}. Apabila $x = ${x1}$, $y = ${frT(y1)}$. Cari $y$ apabila $x = ${x2}$.`), a: T(`$k = ${frT(k)}$; $y = ${frT(y2)}$`), sp: 's' };
  };
  ...
  const iv12_gradientFromGraph = (r) => {
    ...
    return { q: T(`The graph of $y$ against $${inv}$ is a straight line through the origin passing through $\\left(${n(t1)}, ${y1}\\right)$, for a relation $y = k \\times ${inv}$.<br>${fig}<br>Find the gradient of the line (the value of $k$).`, `Graf $y$ melawan $${inv}$ ialah garis lurus melalui asalan dan melalui $\\left(${n(t1)}, ${y1}\\right)$, bagi hubungan $y = k \\times ${inv}$.<br>${fig}<br>Cari kecerunan garis itu (nilai $k$).`), a: T(`$k = ${k}$`), sp: 'm' };
  };
  const iv12_compareChange = (r) => {
    const which = r.chance();
    const factor = r.pick([2, 3]);
    const m1 = Math.round(100 / factor) / 100, m2 = Math.round(100 / (factor * factor)) / 100;
    return { q: T(`$P$ varies inversely as $x$, and $Q$ varies inversely as $x^2$. If $x$ is multiplied by ${factor}, which quantity decreases by a greater factor, $P$ or $Q$? Justify with the multiplying factors.`, `$P$ berubah secara songsang dengan $x$, dan $Q$ berubah secara songsang dengan $x^2$. Jika $x$ didarab dengan ${factor}, kuantiti manakah berkurang dengan faktor yang lebih besar, $P$ atau $Q$? Wajarkan dengan faktor pendarabnya.`), a: T(`$P$ is multiplied by $\\dfrac{1}{${factor}}$; $Q$ is multiplied by $\\dfrac{1}{${factor * factor}}$. $Q$ decreases by the greater factor.`, `$P$ didarab dengan $\\dfrac{1}{${factor}}$; $Q$ didarab dengan $\\dfrac{1}{${factor * factor}}$. $Q$ berkurang dengan faktor yang lebih besar.`), sp: 'm' };
  };
  const iv12_explainDomain = (r) => {
    return { q: T(`$y$ varies inversely as $x$. Explain why $x = 0$ cannot be substituted into the equation, even though $y = \\dfrac{k}{x}$ is defined for every other real value of $x$.`, `$y$ berubah secara songsang dengan $x$. Terangkan mengapa $x = 0$ tidak boleh digantikan ke dalam persamaan itu, walaupun $y = \\dfrac{k}{x}$ tertakrif bagi setiap nilai nyata $x$ yang lain.`), a: T(`Division by $0$ is not defined, so $\\dfrac{k}{0}$ has no value; $y$ has no value at $x = 0$.`, `Pembahagian dengan $0$ tidak tertakrif, jadi $\\dfrac{k}{0}$ tiada nilai; $y$ tiada nilai pada $x = 0$.`), sp: 'm' };
  };
  const iv12_construct = (r) => {
    const pw = r.pick([DV[0], DV[1]]);
    return { q: T(`Give an example of your own inverse-variation equation of the form $y = \\dfrac{k}{${pw.tex('x')}}$ with $k$ a positive integer, and state the value of $y$ it gives when $x = 2$.`, `Berikan contoh persamaan variasi songsang anda sendiri dalam bentuk $y = \\dfrac{k}{${pw.tex('x')}}$ dengan $k$ integer positif, dan nyatakan nilai $y$ yang diberikannya apabila $x = 2$.`), a: T(`Any $y = \\dfrac{k}{${pw.tex('x')}}$ with $k$ a positive integer, e.g. $k = 12$: $y = \\dfrac{12}{${dvVal(pw.kind, 2)}}$ when $x = 2$`, `Sebarang $y = \\dfrac{k}{${pw.tex('x')}}$ dengan $k$ integer positif, cth. $k = 12$: $y = \\dfrac{12}{${dvVal(pw.kind, 2)}}$ apabila $x = 2$`), sp: 'm' };
  };
  ...
  const CONCEPT_JV = [
    [T('For $z = kxy$, if $y$ is held constant, $z$ is directly proportional to $x$.', 'Bagi $z = kxy$, jika $y$ dipegang malar, $z$ berkadar langsung dengan $x$.'), T('For $z = kxy$, if $y$ is held constant, $z$ is inversely proportional to $x$.', 'Bagi $z = kxy$, jika $y$ dipegang malar, $z$ berkadar songsang dengan $x$.')],
    [T('To find $k$ in a joint/combined variation, one complete data row giving $x$, $y$ and $z$ together is needed.', 'Untuk mencari $k$ dalam variasi bergabung, satu baris data lengkap yang memberi $x$, $y$ dan $z$ bersama diperlukan.'), T('To find $k$ in a joint/combined variation, only the value of $x$ is needed.', 'Untuk mencari $k$ dalam variasi bergabung, hanya nilai $x$ sahaja diperlukan.')],
    [T('For $z \\propto \\dfrac{x^2}{y}$, doubling $x$ while $y$ is fixed makes $z$ four times as large.', 'Bagi $z \\propto \\dfrac{x^2}{y}$, menggandakan dua $x$ dengan $y$ tetap menjadikan $z$ empat kali ganda.'), T('For $z \\propto \\dfrac{x^2}{y}$, doubling $x$ while $y$ is fixed makes $z$ twice as large.', 'Bagi $z \\propto \\dfrac{x^2}{y}$, menggandakan dua $x$ dengan $y$ tetap menjadikan $z$ dua kali ganda.')],
    [T('For $z \\propto \\dfrac{x}{y}$, doubling $y$ while $x$ is fixed halves $z$.', 'Bagi $z \\propto \\dfrac{x}{y}$, menggandakan dua $y$ dengan $x$ tetap mengurangkan $z$ kepada separuh.'), T('For $z \\propto \\dfrac{x}{y}$, doubling $y$ while $x$ is fixed doubles $z$.', 'Bagi $z \\propto \\dfrac{x}{y}$, menggandakan dua $y$ dengan $x$ tetap menggandakan dua $z$.')],
    [T('For $z = kxy$, if both $x$ and $y$ are doubled, $z$ becomes four times as large.', 'Bagi $z = kxy$, jika $x$ dan $y$ kedua-duanya digandakan dua, $z$ menjadi empat kali ganda.'), T('For $z = kxy$, if both $x$ and $y$ are doubled, $z$ becomes twice as large.', 'Bagi $z = kxy$, jika $x$ dan $y$ kedua-duanya digandakan dua, $z$ menjadi dua kali ganda.')],
    [T('"$z$ varies directly as $x$ and inversely as $y$" is written $z = \\dfrac{kx}{y}$.', '"$z$ berubah secara langsung dengan $x$ dan secara songsang dengan $y$" ditulis $z = \\dfrac{kx}{y}$.'), T('"$z$ varies directly as $x$ and inversely as $y$" is written $z = k(x - y)$.', '"$z$ berubah secara langsung dengan $x$ dan secara songsang dengan $y$" ditulis $z = k(x - y)$.')],
    [T('When only $x$ changes in $z = \\dfrac{kx^2}{y}$, every other variable, including $y$, is assumed to stay constant.', 'Apabila hanya $x$ berubah dalam $z = \\dfrac{kx^2}{y}$, setiap pemboleh ubah lain, termasuk $y$, diandaikan kekal malar.'), T('When only $x$ changes in $z = \\dfrac{kx^2}{y}$, $y$ is assumed to change proportionally too.', 'Apabila hanya $x$ berubah dalam $z = \\dfrac{kx^2}{y}$, $y$ diandaikan turut berubah secara berkadar.')],
    [T('Using $k$ found from one relation in a different, unrelated relation gives a wrong answer.', 'Menggunakan $k$ yang diperoleh daripada satu hubungan dalam hubungan lain yang tidak berkaitan memberi jawapan yang salah.'), T('The value of $k$ is universal and can be reused across different variation relations.', 'Nilai $k$ adalah universal dan boleh digunakan semula merentasi hubungan variasi yang berbeza.')],
    [T('For $z \\propto xy^2$, tripling $y$ while $x$ is fixed makes $z$ nine times as large.', 'Bagi $z \\propto xy^2$, mengganda tigakan $y$ dengan $x$ tetap menjadikan $z$ sembilan kali ganda.'), T('For $z \\propto xy^2$, tripling $y$ while $x$ is fixed makes $z$ three times as large.', 'Bagi $z \\propto xy^2$, mengganda tigakan $y$ dengan $x$ tetap menjadikan $z$ tiga kali ganda.')],
    [T('In $z = \\dfrac{kx}{y}$, applying the inverse part as if it were direct (multiplying by $y$ instead of dividing) is a common error.', 'Dalam $z = \\dfrac{kx}{y}$, menggunakan bahagian songsang seolah-olah langsung (mendarab dengan $y$ dan bukan membahagi) ialah kesilapan biasa.'), T('In $z = \\dfrac{kx}{y}$, multiplying by $y$ or dividing by $y$ gives the same result.', 'Dalam $z = \\dfrac{kx}{y}$, mendarab dengan $y$ atau membahagi dengan $y$ memberi hasil yang sama.')],
  ];
  ...
  const jv13_findK = (r) => {
    ...
    return { q: T(`$z$ ${f.phEn}. $z = ${z1}$ when $x = ${p1.x}$ and $y = ${p1.y}$. Find (a) the equation connecting $z$, $x$ and $y$, (b) the value of $z$ when $x = ${p2.x}$ and $y = ${p2.y}$.`, `$z$ ${f.phMs}. $z = ${z1}$ apabila $x = ${p1.x}$ dan $y = ${p1.y}$. Cari (a) persamaan yang menghubungkan $z$, $x$ dan $y$, (b) nilai $z$ apabila $x = ${p2.x}$ dan $y = ${p2.y}$.`), a: T(`(a) $z = ${jvSym(f).replace('k', k)}$ (b) $z = ${z2}$`), sp: 's' };
  };
  const jv13_word = (r) => {
  ...
  const jv13_backward = (r) => {
    ...
    return { q: T(`$z$ ${f.phEn}. $k = ${k}$. Find $z$ when $x = ${p1.x}$ and $y = ${p1.y}$.`, `$z$ ${f.phMs}. $k = ${k}$. Cari $z$ apabila $x = ${p1.x}$ dan $y = ${p1.y}$.`), a: T(`$z = ${z1}$`), sp: 'xs' };
  };
  const jv13_writeEq = (r) => {
    const f = r.pick(JV);
    return { q: T(`Write, using a constant $k$, the equation for: "$z$ ${f.phEn}."`, `Tulis, menggunakan pemalar $k$, persamaan bagi: "$z$ ${f.phMs}."`), a: T(`$z = ${jvSym(f)}$`), sp: 'xs' };
  };
  const jv13_mcqEq = (r) => {
    const f = r.pick(JV);
    ...
    return { q: cat(T(`"$z$ ${f.phEn}, where $k$ is a nonzero constant." Which equation represents this?<br>`, `"$z$ ${f.phMs}, dengan keadaan $k$ ialah pemalar bukan sifar." Persamaan manakah mewakili ini?<br>`), o.q), a: o.a, sp: 's' };
  };
  ...
  const jv13_findK_hard = (r) => {
    ...
    const p1 = jvPoint(r)(f), p2 = retry(() => { const p = jvPoint(r)(f); need(p.x !== p1.x || p.y !== p1.y); return p; });
    const z1 = jvValK(f, k, p1.x, p1.y), z2 = jvValK(f, k, p2.x, p2.y);
    return { q: T(`$z$ ${f.phEn}. $z = ${z1}$ when $x = ${p1.x}$, $y = ${p1.y}$. Find $z$ when $x = ${p2.x}$, $y = ${p2.y}$.`, `$z$ ${f.phMs}. $z = ${z1}$ apabila $x = ${p1.x}$, $y = ${p1.y}$. Cari $z$ apabila $x = ${p2.x}$, $y = ${p2.y}$.`), a: T(`$k = ${k}$; $z = ${z2}$`), sp: 'm' };
  };
  const jv13_solveOther = (r) => {
    ...
    const z1 = jvValK(f, k, p1.x, p1.y);
    const askX = r.chance();
    return { q: T(`$z$ ${f.phEn}. $k = ${k}$. When $z = ${z1}$ and $${askX ? 'y' : 'x'} = ${askX ? p1.y : p1.x}$, find the value of $${askX ? 'x' : 'y'}$.`, `$z$ ${f.phMs}. $k = ${k}$. Apabila $z = ${z1}$ dan $${askX ? 'y' : 'x'} = ${askX ? p1.y : p1.x}$, cari nilai $${askX ? 'x' : 'y'}$.`), a: T(`$${askX ? 'x' : 'y'} = ${askX ? p1.x : p1.y}$`), sp: 'm' };
  };
  const jv13_table = (r) => {
    const f = r.pick([JV[0], JV[3]]);
    const k = r.int(2, 4);
    ...
    return { q: T(`$z$ ${f.phEn}. The table shows corresponding values.<br>${tab}<br>Find the value of $k$ and the missing value of $z$.`, `$z$ ${f.phMs}. Jadual menunjukkan nilai-nilai yang berkaitan.<br>${tab}<br>Cari nilai $k$ dan nilai $z$ yang tertinggal.`), a: T(`$k = ${k}$; $z = ${zs[gap]}$`), sp: 'm' };
  };
  const jv13_pctMultiplier = (r) => {
    ...
    return { q: T(`$z$ ${f.phEn}. If $x$ is multiplied by ${fx} and $y$ is multiplied by ${fy} at the same time, by what factor is $z$ multiplied?`, `$z$ ${f.phMs}. Jika $x$ didarab dengan ${fx} dan $y$ didarab dengan ${fy} pada masa yang sama, $z$ didarab dengan faktor berapa?`), a: T(`$${frT(factor)}$`), sp: 's' };
  };
  ...
  const CONCEPT_MAT = [
    ...
    [T('The element $a_{23}$ is in row 2, column 3.', 'Unsur $a_{23}$ berada pada baris 2, lajur 3.'), T('The element $a_{23}$ is in row 3, column 2.', 'Unsur $a_{23}$ berada pada baris 3, lajur 2.')],
    [T('A square matrix has the same number of rows and columns.', 'Matriks segi empat sama mempunyai bilangan baris dan lajur yang sama.'), T('A square matrix must have all its entries equal.', 'Matriks segi empat sama mesti mempunyai semua unsurnya sama.')],
    [T('A zero matrix has every entry equal to $0$.', 'Matriks sifar mempunyai setiap unsurnya bersamaan $0$.'), T('A zero matrix has a $0$ only in the top-left corner.', 'Matriks sifar hanya mempunyai $0$ di penjuru kiri atas sahaja.')],
    [T('An identity matrix is a square matrix with $1$s on the main diagonal and $0$s elsewhere.', 'Matriks identiti ialah matriks segi empat sama dengan $1$ pada pepenjuru utama dan $0$ di tempat lain.'), T('An identity matrix has $1$ in every entry.', 'Matriks identiti mempunyai $1$ pada setiap unsur.')],
    [T('A row matrix has exactly one row (order $1 \\times n$).', 'Matriks baris mempunyai tepat satu baris (peringkat $1 \\times n$).'), T('A row matrix has exactly one column (order $n \\times 1$).', 'Matriks baris mempunyai tepat satu lajur (peringkat $n \\times 1$).')],
    [T('A column matrix has exactly one column (order $n \\times 1$).', 'Matriks lajur mempunyai tepat satu lajur (peringkat $n \\times 1$).'), T('A column matrix has exactly one row (order $1 \\times n$).', 'Matriks lajur mempunyai tepat satu baris (peringkat $1 \\times n$).')],
    [T('Two matrices are equal only if they have the same order AND every corresponding element is equal.', 'Dua matriks adalah sama hanya jika kedua-duanya mempunyai peringkat yang sama DAN setiap unsur yang bersepadan adalah sama.'), T('Two matrices are equal as long as they have the same order, even if some elements differ.', 'Dua matriks adalah sama asalkan peringkatnya sama, walaupun sesetengah unsurnya berbeza.')],
    [T('Two matrices of different orders can never be equal.', 'Dua matriks yang berlainan peringkat tidak sekali-kali boleh sama.'), T('Two matrices of different orders can be equal if the sum of their entries is equal.', 'Dua matriks yang berlainan peringkat boleh sama jika jumlah unsurnya sama.')],
    [T('A $1 \\times 1$ matrix is simultaneously a row matrix, a column matrix and a square matrix.', 'Matriks $1 \\times 1$ adalah serentak matriks baris, matriks lajur dan matriks segi empat sama.'), T('A $1 \\times 1$ matrix cannot be called a square matrix.', 'Matriks $1 \\times 1$ tidak boleh dipanggil matriks segi empat sama.')],
    [T('The number of elements in an $m \\times n$ matrix is $mn$.', 'Bilangan unsur dalam matriks $m \\times n$ ialah $mn$.'), T('The number of elements in an $m \\times n$ matrix is $m + n$.', 'Bilangan unsur dalam matriks $m \\times n$ ialah $m + n$.')],
    [T('In $a_{ij}$, the row number $i$ is written before the column number $j$.', 'Dalam $a_{ij}$, nombor baris $i$ ditulis sebelum nombor lajur $j$.'), T('In $a_{ij}$, the column number $i$ is written before the row number $j$.', 'Dalam $a_{ij}$, nombor lajur $i$ ditulis sebelum nombor baris $j$.')],
  ];
  const MTYPES = [
  ...
  const mat21_orderElement = (r) => {
    ...
    if (style === 'order') return { q: T(`$A = ${M(A)}$. State the order of $A$ and the value of the element $a_{${i + 1}${j + 1}}$ (row ${i + 1}, column ${j + 1}).`, `$A = ${M(A)}$. Nyatakan peringkat $A$ dan nilai unsur $a_{${i + 1}${j + 1}}$ (baris ${i + 1}, lajur ${j + 1}).`), a: T(`Order $${rows} \\times ${cols}$; $a_{${i + 1}${j + 1}} = ${A[i][j]}$`), sp: 's' };
    return { q: T(`$A = ${M(A)}$. How many rows and columns does $A$ have, and how many elements does it have in total?`, `$A = ${M(A)}$. Berapakah bilangan baris dan lajur $A$, dan berapakah jumlah bilangan unsurnya?`), a: T(`${rows} row(s), ${cols} column(s); total $${rows * cols}$ elements`, `${rows} baris, ${cols} lajur; jumlah $${rows * cols}$ unsur`), sp: 's' };
  };
  const mat21_identifyType = (r) => {
    const key = r.pick(['row', 'col', 'sq', 'zero', 'id']);
    const A = mkType(r, key);
    const tname = MTYPES.find((t) => t.key === key);
    return { q: T(`State the type of matrix $${M(A)}$ (row, column, square, zero or identity).`, `Nyatakan jenis matriks $${M(A)}$ (baris, lajur, segi empat sama, sifar atau identiti).`), a: T(SPM.cap(tname.en), SPM.cap(tname.ms)), sp: 'xs' };
  };
  const mat21_mcqType = (r) => {
    ...
    const o = mc(r, right, wrong);
    return { q: cat(T(`$${M(A)}$ is a<br>`, `$${M(A)}$ ialah suatu<br>`), o.q), a: o.a, sp: 's' };
  };
  const mat21_equalOne = (r) => {
    ...
    Bnum[i][j] = c;
    return { q: T(`Given $${M(Asym)} = ${M(Bnum)}$, find the value of $x$.`, `Diberi $${M(Asym)} = ${M(Bnum)}$, cari nilai $x$.`), a: T(`$x = ${x}$`), sp: 's' };
  };
  ...
  const mat21_writeMatrix = (r) => {
    ...
    return { q: T(`${names[0]} and ${names[1]} each bought some ${colsTxt('en')}. ${names[0]} bought ${A[0].join(', ')} respectively; ${names[1]} bought ${A[1].join(', ')} respectively. Represent this information as a $2 \\times 3$ matrix, with rows ${rowsTxt('en')} and columns ${colsTxt('en')}.`, `${names[0]} dan ${names[1]} masing-masing membeli beberapa ${colsTxt('ms')}. ${names[0]} membeli ${A[0].join(', ')} masing-masing; ${names[1]} membeli ${A[1].join(', ')} masing-masing. Wakilkan maklumat ini sebagai matriks $2 \\times 3$, dengan baris ${rowsTxt('ms')} dan lajur ${colsTxt('ms')}.`), a: T(`$${M(A)}$`), sp: 's' };
  };
  const mat21_writeSpecial = (r) => {
    ...
    return { q: T(`Write down ${label.en}.`, `Tulis ${label.ms}.`), a: T(`$${M(A)}$`), sp: 'xs' };
  };
  ...
  const mat21_equalTwo = (r) => {
    ...
    return { q: T(`Given $${M(Asym)} = ${M(Bnum)}$, find the values of $x$ and $y$.`, `Diberi $${M(Asym)} = ${M(Bnum)}$, cari nilai $x$ dan $y$.`), a: T(`$x = ${x}$, $y = ${y}$`), sp: 's' };
  };
  ...
  const mat21_identityTest = (r) => {
    ...
    return { q: T(`True or False: $${M(A)}$ is the $${nn} \\times ${nn}$ identity matrix.`, `Betul atau Salah: $${M(A)}$ ialah matriks identiti $${nn} \\times ${nn}$.`), a: isId ? T('True', 'Betul') : T(`False — the identity matrix must have $1$s on the diagonal and $0$s elsewhere.`, `Salah — matriks identiti mesti mempunyai $1$ pada pepenjuru dan $0$ di tempat lain.`), sp: 's' };
  };
  ...
  const mat21_identityFromX = (r) => {
    ...
    return { q: T(`Given $${M(Asym)} = ${M(target)}$ (the $2 \\times 2$ identity matrix), find $x$ and $y$.`, `Diberi $${M(Asym)} = ${M(target)}$ (matriks identiti $2 \\times 2$), cari $x$ dan $y$.`), a: T(`$x = ${x}$, $y = ${y}$`), sp: 's' };
  };
  ...
  const CONCEPT_OP = [
    [T('To add or subtract two matrices, they must have the same order.', 'Untuk menambah atau menolak dua matriks, kedua-duanya mesti mempunyai peringkat yang sama.'), T('Any two matrices can be added, regardless of their order.', 'Sebarang dua matriks boleh ditambah, tanpa mengira peringkatnya.')],
    [T('In $A + B$, corresponding elements of $A$ and $B$ are added.', 'Dalam $A + B$, unsur yang bersepadan bagi $A$ dan $B$ ditambah.'), T('In $A + B$, every element of $A$ is added to every element of $B$.', 'Dalam $A + B$, setiap unsur $A$ ditambah dengan setiap unsur $B$.')],
    [T('$kA$ is found by multiplying every element of $A$ by $k$.', '$kA$ diperoleh dengan mendarab setiap unsur $A$ dengan $k$.'), T('$kA$ is found by adding $k$ to every element of $A$.', '$kA$ diperoleh dengan menambah $k$ pada setiap unsur $A$.')],
    [T('$A - B$ is the same as $A + (-1)B$.', '$A - B$ adalah sama seperti $A + (-1)B$.'), T('$A - B$ gives the same matrix as $B - A$.', '$A - B$ memberi matriks yang sama seperti $B - A$.')],
    [T('For $AB$ to be defined, the number of columns of $A$ must equal the number of rows of $B$.', 'Untuk $AB$ tertakrif, bilangan lajur $A$ mestilah sama dengan bilangan baris $B$.'), T('For $AB$ to be defined, $A$ and $B$ must have the same order.', 'Untuk $AB$ tertakrif, $A$ dan $B$ mestilah mempunyai peringkat yang sama.')],
    [T('If $A$ is $m \\times n$ and $B$ is $n \\times p$, then $AB$ has order $m \\times p$.', 'Jika $A$ berperingkat $m \\times n$ dan $B$ berperingkat $n \\times p$, maka $AB$ berperingkat $m \\times p$.'), T('If $A$ is $m \\times n$ and $B$ is $n \\times p$, then $AB$ has order $n \\times n$.', 'Jika $A$ berperingkat $m \\times n$ dan $B$ berperingkat $n \\times p$, maka $AB$ berperingkat $n \\times n$.')],
    [T('In general, $AB \\neq BA$ for matrices.', 'Secara amnya, $AB \\neq BA$ bagi matriks.'), T('Matrix multiplication is always commutative, so $AB = BA$.', 'Pendaraban matriks sentiasa kalis tukar tertib, jadi $AB = BA$.')],
  ...
  const mat22_addSub = (r) => {
    ...
    if (task === 'scalar') return { q: T(`$A = ${M(A)}$. Find $${k}A$.`, `$A = ${M(A)}$. Cari $${k}A$.`), a: T(`$${M(msc(k, A))}$`), sp: 's' };
    if (task === 'sum') return { q: T(`$A = ${M(A)}$ and $B = ${M(B)}$. Find $A ${op} B$.`, `$A = ${M(A)}$ dan $B = ${M(B)}$. Cari $A ${op} B$.`), a: T(`$${M(madd(A, B, op === '+' ? 1 : -1))}$`), sp: 's' };
    return { q: T(`$A = ${M(A)}$ and $B = ${M(B)}$. Find $A ${op} B$ and $${k}A$.`, `$A = ${M(A)}$ dan $B = ${M(B)}$. Cari $A ${op} B$ dan $${k}A$.`), a: T(`$${M(madd(A, B, op === '+' ? 1 : -1))}$; $${M(msc(k, A))}$`), sp: 's' };
  };
  const mat22_unknownSum = (r) => {
    ...
    return { q: T(`$A = ${M(A)}$ and $B = ${M(Bsym)}$. Given $A ${op ? '+' : '-'} B = ${M(Csum)}$, find the value of $x$.`, `$A = ${M(A)}$ dan $B = ${M(Bsym)}$. Diberi $A ${op ? '+' : '-'} B = ${M(Csum)}$, cari nilai $x$.`), a: T(`$x = ${x}$`), sp: 's' };
  };
  const mat22_mcqOp = (r) => {
    ...
    const o = mc(r, right, wrong);
    return { q: cat(T(`$A = ${M(A)}$ and $B = ${M(B)}$. What is $A + B$?<br>`, `$A = ${M(A)}$ dan $B = ${M(B)}$. Apakah $A + B$?<br>`), o.q), a: o.a, sp: 's' };
  };
  const mat22_associative = (r) => {
    ...
    return { q: T(`$A = ${M(A)}$, $B = ${M(B)}$, $C = ${M(C)}$. Find $(A + B) + C$ and $A + (B + C)$. What do you notice?`, `$A = ${M(A)}$, $B = ${M(B)}$, $C = ${M(C)}$. Cari $(A + B) + C$ dan $A + (B + C)$. Apakah yang anda perhatikan?`), a: T(`Both equal $${M(left)}$; they are the same (addition is associative)`, `Kedua-duanya bersamaan $${M(left)}$; kedua-duanya sama (penambahan bersifat kalis sekutuan)`), sp: 'm' };
  };
  ...
  const mat22_combined = (r) => {
    ...
    return { q: T(`$A = ${M(A)}$ and $B = ${M(B)}$. Find $${kA}A ${sign > 0 ? '+' : '-'} ${kB}B$.`, `$A = ${M(A)}$ dan $B = ${M(B)}$. Cari $${kA}A ${sign > 0 ? '+' : '-'} ${kB}B$.`), a: T(`$${M(madd(msc(kA, A), msc(kB, B), sign))}$`), sp: 'm' };
  };
  ...
  const mat22_mulRect = (r) => {
    ...
    return { q: T(`$A = ${M(A)}$ and $B = ${M(B)}$. State the order of $AB$, then find $AB$.`, `$A = ${M(A)}$ dan $B = ${M(B)}$. Nyatakan peringkat $AB$, kemudian cari $AB$.`), a: T(`Order $${A.length} \\times ${B[0].length}$; $AB = ${M(mmul(A, B))}$`), sp: 'm' };
  };
  const mat22_orderEffect = (r) => {
    const A = rmat(r, 2, 3, -4, 4), B = rmat(r, 3, 2, -4, 4);
    return { q: T(`$A = ${M(A)}$ and $B = ${M(B)}$. State the orders of $AB$ and $BA$, and find $AB$.`, `$A = ${M(A)}$ dan $B = ${M(B)}$. Nyatakan peringkat $AB$ dan $BA$, dan cari $AB$.`), a: T(`$AB$ is $${A.length} \\times ${B[0].length}$; $BA$ is $${B.length} \\times ${A[0].length}$; $AB = ${M(mmul(A, B))}$`, `$AB$ ialah $${A.length} \\times ${B[0].length}$; $BA$ ialah $${B.length} \\times ${A[0].length}$; $AB = ${M(mmul(A, B))}$`), sp: 'm' };
  };
  ...
  const CONCEPT_INV = [
    [T('For $M = \\begin{pmatrix} a & b \\\\ c & d \\end{pmatrix}$, $\\det M = ad - bc$.', 'Bagi $M = \\begin{pmatrix} a & b \\\\ c & d \\end{pmatrix}$, $\\det M = ad - bc$.'), T('For $M = \\begin{pmatrix} a & b \\\\ c & d \\end{pmatrix}$, $\\det M = ac - bd$.', 'Bagi $M = \\begin{pmatrix} a & b \\\\ c & d \\end{pmatrix}$, $\\det M = ac - bd$.')],
    [T('The inverse $M^{-1}$ exists only when $\\det M \\neq 0$.', 'Songsangan $M^{-1}$ wujud hanya apabila $\\det M \\neq 0$.'), T('The inverse $M^{-1}$ exists for every matrix $M$.', 'Songsangan $M^{-1}$ wujud bagi setiap matriks $M$.')],
    [T('To form $M^{-1}$, swap $a$ and $d$, and negate $b$ and $c$ (not swap $b$ and $c$).', 'Untuk membentuk $M^{-1}$, tukar kedudukan $a$ dan $d$, dan negatifkan $b$ dan $c$ (bukan tukar kedudukan $b$ dan $c$).'), T('To form $M^{-1}$, swap $b$ and $c$, and negate $a$ and $d$.', 'Untuk membentuk $M^{-1}$, tukar kedudukan $b$ dan $c$, dan negatifkan $a$ dan $d$.')],
    [T('$M^{-1} = \\dfrac{1}{\\det M}\\begin{pmatrix} d & -b \\\\ -c & a \\end{pmatrix}$.', '$M^{-1} = \\dfrac{1}{\\det M}\\begin{pmatrix} d & -b \\\\ -c & a \\end{pmatrix}$.'), T('$M^{-1} = \\det M \\begin{pmatrix} d & -b \\\\ -c & a \\end{pmatrix}$ (without dividing by $\\det M$).', '$M^{-1} = \\det M \\begin{pmatrix} d & -b \\\\ -c & a \\end{pmatrix}$ (tanpa membahagi dengan $\\det M$).')],
    [T('$MM^{-1} = I$, the identity matrix.', '$MM^{-1} = I$, matriks identiti.'), T('$MM^{-1} = M$.', '$MM^{-1} = M$.')],
    [T('To solve $AX = B$ for $X$, compute $X = A^{-1}B$ (order matters).', 'Untuk menyelesaikan $AX = B$ bagi $X$, hitung $X = A^{-1}B$ (tertib adalah penting).'), T('To solve $AX = B$ for $X$, compute $X = BA^{-1}$.', 'Untuk menyelesaikan $AX = B$ bagi $X$, hitung $X = BA^{-1}$.')],
    [T('If $\\det A = 0$, $A$ has no inverse, so $X = A^{-1}B$ cannot be used.', 'Jika $\\det A = 0$, $A$ tidak mempunyai songsangan, jadi $X = A^{-1}B$ tidak boleh digunakan.'), T('If $\\det A = 0$, $X = A^{-1}B$ still works, treating $\\dfrac{1}{0}$ as $0$.', 'Jika $\\det A = 0$, $X = A^{-1}B$ masih boleh digunakan, dengan menganggap $\\dfrac{1}{0}$ sebagai $0$.')],
    [T('A determinant of $0$ means the matrix has no inverse.', 'Penentu bersamaan $0$ bermaksud matriks itu tidak mempunyai songsangan.'), T('A determinant of $0$ means the matrix must be the zero matrix.', 'Penentu bersamaan $0$ bermaksud matriks itu mestilah matriks sifar.')],
    [T('When solving simultaneous equations by the matrix method, the equations are first written as a single matrix equation.', 'Apabila menyelesaikan persamaan serentak dengan kaedah matriks, persamaan itu mula-mula ditulis sebagai satu persamaan matriks.'), T('The matrix method finds the inverse of the coefficients without ever writing a matrix equation.', 'Kaedah matriks mencari songsangan pekali tanpa menulis sebarang persamaan matriks.')],
    [T('A negative determinant is possible; the inverse formula still applies, using $\\dfrac{1}{\\det M}$, which is negative.', 'Penentu negatif adalah mungkin; formula songsangan masih terpakai, menggunakan $\\dfrac{1}{\\det M}$, yang bernilai negatif.'), T('A negative determinant means the matrix has no inverse.', 'Penentu negatif bermaksud matriks itu tidak mempunyai songsangan.')],
    [T('For a matrix with a parameter, the value that makes $\\det = 0$ is exactly the value for which no inverse exists.', 'Bagi matriks dengan parameter, nilai yang menjadikan $\\det = 0$ adalah tepat nilai yang menyebabkan tiada songsangan wujud.'), T('For a matrix with a parameter, the determinant can never become $0$.', 'Bagi matriks dengan parameter, penentu tidak boleh menjadi $0$.')],
    [T('The order of $A$, $X$ and $B$ must be checked to be compatible before solving $AX = B$.', 'Peringkat $A$, $X$ dan $B$ mesti disemak supaya serasi sebelum menyelesaikan $AX = B$.'), T('The orders of $A$, $X$ and $B$ never need to be checked; any matrices can be multiplied.', 'Peringkat $A$, $X$ dan $B$ tidak perlu disemak; sebarang matriks boleh didarab.')],
  ];

  const mat23_detFind = (r) => {
    const A = rmat(r, 2, 2, -9, 9);
    return { q: T(`Find the determinant of $M = ${M(A)}$.`, `Cari penentu bagi $M = ${M(A)}$.`), a: T(`$\\det M = ${det(A)}$`), w: T(`$${A[0][0]}(${A[1][1]}) - ${A[0][1]}(${A[1][0]})$`), sp: 's' };
  };
  const mat23_detMCQ = (r) => {
    ...
    const o = mc(r, right, wrong);
    return { q: cat(T(`The determinant of $${M(A)}$ is<br>`, `Penentu bagi $${M(A)}$ ialah<br>`), o.q), a: o.a, sp: 's' };
  };
  ...
  const mat23_mcqInvError = (r) => {
    ...
    const o = mc(r, right, wrong);
    return { q: cat(T(`The inverse of $${M(A)}$ is<br>`, `Songsangan bagi $${M(A)}$ ialah<br>`), o.q), a: o.a, sp: 's' };
  };
  ...
  const mat23_invFraction = (r) => {
    ...
    return { q: T(`Find the inverse of $${M(A)}$, giving your answer with fractions where necessary.`, `Cari songsangan bagi $${M(A)}$, berikan jawapan anda dalam bentuk pecahan jika perlu.`), a: T(`$${fracM(invM(A))}$`), sp: 'm' };
  };
  const mat23_verifyFull = (r) => {
    const A = retry(() => { const a = rmat(r, 2, 2, -6, 6); need(Math.abs(det(a)) === 1 || Math.abs(det(a)) === 2); return a; });
    return { q: T(`Find $M^{-1}$ for $M = ${M(A)}$, then compute $MM^{-1}$ fully to verify it equals $I$.`, `Cari $M^{-1}$ bagi $M = ${M(A)}$, kemudian hitung $MM^{-1}$ sepenuhnya untuk mengesahkan ia bersamaan $I$.`), a: T(`$M^{-1} = ${invTex(A)}$; $MM^{-1} = ${M(idMat(2))} = I$`, `$M^{-1} = ${invTex(A)}$; $MM^{-1} = ${M(idMat(2))} = I$`), sp: 'm' };
  };
  const mat23_solveX = (r) => {
    const A = retry(() => { const a = rmat(r, 2, 2, 1, 5); need(det(a) === 1 || det(a) === -1); return a; });
    const X = rmat(r, 2, 1, -5, 6);
    const B = mmul(A, X);
    return { q: T(`$A = ${M(A)}$ and $AX = ${M(B)}$, where $X$ is a $2 \\times 1$ matrix. Find $X$.`, `$A = ${M(A)}$ dan $AX = ${M(B)}$, dengan $X$ ialah matriks $2 \\times 1$. Cari $X$.`), a: T(`$X = A^{-1}${M(B)} = ${M(X)}$`), sp: 'm' };
  };
  const mat23_paramSimple = (r) => {
    const a = r.int(1, 4), b = r.int(1, 4), c = r.int(1, 4);
    const x0 = r.nz(-6, 6);
    const D = a * x0 - b * c;
    return { q: T(`The matrix $${M([[a, b], [c, 'x']])}$ has a determinant of $${D}$. Find the value of $x$.`, `Matriks $${M([[a, b], [c, 'x']])}$ mempunyai penentu $${D}$. Cari nilai $x$.`), a: T(`$${a}x - ${b * c} = ${D}$, so $x = ${x0}$`, `$${a}x - ${b * c} = ${D}$, jadi $x = ${x0}$`), sp: 'm' };
  };
  ...
  const mat23_singularParam = (r) => {
    ...
    if (swap) {
      ...
      const xv = 2 * k;
      const A = [[a, k], [2 * a, 'x']];
      return { q: T(`The matrix $${M(A)}$ has no inverse. Find the value of $x$.`, `Matriks $${M(A)}$ tidak mempunyai songsangan. Cari nilai $x$.`), a: T(`$\\det = ${a}x - ${2 * a * k} = 0$, so $x = ${xv}$`, `$\\det = ${a}x - ${2 * a * k} = 0$, jadi $x = ${xv}$`), sp: 'm' };
    }
    const xv = r.int(1, 6);
    const k = 2 * xv;
    const A = [['x', a], [k, 2 * a]];
    return { q: T(`The matrix $${M(A)}$ has no inverse. Find the value of $x$.`, `Matriks $${M(A)}$ tidak mempunyai songsangan. Cari nilai $x$.`), a: T(`$\\det = ${2 * a}x - ${a * k} = 0$, so $x = ${xv}$`, `$\\det = ${2 * a}x - ${a * k} = 0$, jadi $x = ${xv}$`), sp: 'm' };
  };
  ...
  const mat23_fractionInverseSolve = (r) => {
    ...
    const B = mmul(A, X);
    return { q: T(`$A = ${M(A)}$ and $AX = ${M(B)}$. Find the matrix $X$, showing the fractional inverse clearly.`, `$A = ${M(A)}$ dan $AX = ${M(B)}$. Cari matriks $X$, dengan menunjukkan songsangan pecahan dengan jelas.`), a: T(`$A^{-1} = ${invTex(A)}$; $X = ${invTex(A)}${M(B)} = ${M(X)}$`), sp: 'l' };
  };
