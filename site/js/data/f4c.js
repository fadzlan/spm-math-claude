...
(function () {
  ...
  const g61e = [
    (r) => {
      ...
      return { q: T(`Does the point $(${P[0]}, ${P[1]})$ satisfy the inequality $x + y ${SYM[op]} ${c}$?`, `Adakah titik $(${P[0]}, ${P[1]})$ memenuhi ketaksamaan $x + y ${SYM[op]} ${c}$?`), a: T(`${sat(q, ...P) ? 'Yes' : 'No'}: $${P[0]} + ${P[1]} = ${P[0] + P[1]}$`, `${sat(q, ...P) ? 'Ya' : 'Tidak'}: $${P[0]} + ${P[1]} = ${P[0] + P[1]}$`), w: (() => {
        ...
        return W(`$x + y = ${P[0]} + ${P[1]} = ${s}$`, sat(q, ...P) ? T(`$${s} ${rel} ${c}$, so the point satisfies $x + y ${SYM[op]} ${c}$.`, `$${s} ${rel} ${c}$, maka titik itu memenuhi $x + y ${SYM[op]} ${c}$.`) : T(`$${s} ${rel} ${c}$, so $x + y ${SYM[op]} ${c}$ is false: the point does not satisfy it.`, `$${s} ${rel} ${c}$, maka $x + y ${SYM[op]} ${c}$ adalah palsu: titik itu tidak memenuhinya.`));
      })(), sp: 's' };
    },
  ];
  const g61m = [
  ...
  const g823e = [
    (r) => {
      ...
      return { q: T('Read the five-number summary from the boxplot (minimum, $Q_1$, median, $Q_3$, maximum).', 'Baca ringkasan lima nombor daripada plot kotak (minimum, $Q_1$, median, $Q_3$, maksimum).'), fig, a: T(`${q.min}, ${n(q.q1)}, ${n(q.med)}, ${n(q.q3)}, ${q.max}`), w: W(T('Left end of the whisker = minimum; left edge of the box = $Q_1$; line inside the box = median; right edge of the box = $Q_3$; right end of the whisker = maximum.', 'Hujung kiri misai = minimum; sisi kiri kotak = $Q_1$; garis dalam kotak = median; sisi kanan kotak = $Q_3$; hujung kanan misai = maksimum.'), `$${q.min},\\ ${n(q.q1)},\\ ${n(q.med)},\\ ${n(q.q3)},\\ ${q.max}$`), sp: 's' };
    },
  ];
  const g823m = [
    (r) => {
      ...
      const fig = boxFig([q], Math.floor(q.min / 10) * 10, Math.ceil(q.max / 10) * 10);
      return { q: T(`Construct a boxplot for the data $${list(v)}$. State the range and the interquartile range.`, `Bina plot kotak bagi data $${list(v)}$. Nyatakan julat dan julat antara kuartil.`), a: T(`${fig} Five-number summary: ${q.min}, ${n(q.q1)}, ${n(q.med)}, ${n(q.q3)}, ${q.max}; range ${q.max - q.min}; IQR ${n(q.q3 - q.q1)}`, `${fig} Ringkasan lima nombor: ${q.min}, ${n(q.q1)}, ${n(q.med)}, ${n(q.q3)}, ${q.max}; julat ${q.max - q.min}; JAK ${n(q.q3 - q.q1)}`), w: W(...quartLines(v), T(`Draw the box from $Q_1 = ${n(q.q1)}$ to $Q_3 = ${n(q.q3)}$ with a line at the median ${n(q.med)}, and whiskers to ${q.min} and ${q.max}.`, `Lukis kotak dari $Q_1 = ${n(q.q1)}$ hingga $Q_3 = ${n(q.q3)}$ dengan garis pada median ${n(q.med)}, dan misai hingga ${q.min} dan ${q.max}.`), rangeLine(v)), sp: 'l' };
    },
  ];
  const g823a = [
    (r) => {
