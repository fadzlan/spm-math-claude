...
(function () {
  ...
  const g102a = [
    (r) => {
      ...
      return { q: T(`The table shows Ali's income and expenses (RM) over three months. Find the surplus or deficit for each month and the cumulative balance after three months.<br>${SPM.table([['Income', ...inc], ['Expenses', ...exp]], { head: ['', 'Month 1', 'Month 2', 'Month 3'], rowHead: true })}`, `Jadual menunjukkan pendapatan dan perbelanjaan Ali (RM) selama tiga bulan. Cari lebihan atau defisit bagi setiap bulan dan baki terkumpul selepas tiga bulan.<br>${SPM.table([['Pendapatan', ...inc], ['Perbelanjaan', ...exp]], { head: ['', 'Bulan 1', 'Bulan 2', 'Bulan 3'], rowHead: true })}`), a: T(`${net.map((v, i) => `Month ${i + 1}: ${v >= 0 ? '+' : '-'}${Math.abs(v)}`).join('; ')}; cumulative ${cum[2] >= 0 ? '+' : '-'}RM${Math.abs(cum[2])}`, `${net.map((v, i) => `Bulan ${i + 1}: ${v >= 0 ? '+' : '-'}${Math.abs(v)}`).join('; ')}; terkumpul ${cum[2] >= 0 ? '+' : '-'}RM${Math.abs(cum[2])}`), sp: 'm' };
    },
  ];
  const g103Ee = [
    (r) => {
