...
(function () {
  ...
  const STM = [
    { s: T('17 is a prime number.', '17 ialah nombor perdana.'), v: true, neg: T('17 is not a prime number.', '17 bukan nombor perdana.') },
    { s: T('24 is a multiple of 5.', '24 ialah gandaan 5.'), v: false, neg: T('24 is not a multiple of 5.', '24 bukan gandaan 5.') },
    { s: T('A square has four equal sides.', 'Segi empat sama mempunyai empat sisi yang sama panjang.'), v: true, neg: T('A square does not have four equal sides.', 'Segi empat sama tidak mempunyai empat sisi yang sama panjang.') },
    { s: T('A pentagon has four sides.', 'Pentagon mempunyai empat sisi.'), v: false, neg: T('A pentagon does not have four sides.', 'Pentagon tidak mempunyai empat sisi.') },
    { s: T('The sum of the interior angles of a triangle is $180°$.', 'Hasil tambah sudut pedalaman sebuah segi tiga ialah $180°$.'), v: true, neg: T('The sum of the interior angles of a triangle is not $180°$.', 'Hasil tambah sudut pedalaman sebuah segi tiga bukan $180°$.') },
    { s: T('All prime numbers are odd.', 'Semua nombor perdana ialah nombor ganjil.'), v: false, neg: T('Not all prime numbers are odd.', 'Bukan semua nombor perdana ialah nombor ganjil.') },
    { s: T('A rhombus has all four sides equal.', 'Sebuah rombus mempunyai keempat-empat sisi yang sama panjang.'), v: true, neg: T('A rhombus does not have all four sides equal.', 'Sebuah rombus tidak mempunyai keempat-empat sisi yang sama panjang.') },
    { s: T('$\\sqrt{64} = 8$.', '$\\sqrt{64} = 8$.'), v: true, neg: T('$\\sqrt{64} \\neq 8$.', '$\\sqrt{64} \\neq 8$.') },
    { s: T('Every multiple of 4 is also a multiple of 8.', 'Setiap gandaan 4 juga ialah gandaan 8.'), v: false, neg: T('Not every multiple of 4 is a multiple of 8.', 'Bukan setiap gandaan 4 ialah gandaan 8.') },
    { s: T('The product of two negative numbers is positive.', 'Hasil darab dua nombor negatif ialah positif.'), v: true, neg: T('The product of two negative numbers is not positive.', 'Hasil darab dua nombor negatif bukan positif.') },
    { s: T('A right angle measures $90°$.', 'Sudut tegak bersudut $90°$.'), v: true, neg: T('A right angle does not measure $90°$.', 'Sudut tegak tidak bersudut $90°$.') },
    { s: T('The diagonals of a rectangle are equal in length.', 'Pepenjuru segi empat tepat sama panjang.'), v: true, neg: T('The diagonals of a rectangle are not equal in length.', 'Pepenjuru segi empat tepat tidak sama panjang.') },
    { s: T('$(-3)^2 = -9$.', '$(-3)^2 = -9$.'), v: false, neg: T('$(-3)^2 \\neq -9$.', '$(-3)^2 \\neq -9$.') },
    { s: T('$10\\%$ of $250$ is $25$.', '$10\\%$ daripada $250$ ialah $25$.'), v: true, neg: T('$10\\%$ of $250$ is not $25$.', '$10\\%$ daripada $250$ bukan $25$.') },
    { s: T('An obtuse angle is less than $90°$.', 'Sudut cakah kurang daripada $90°$.'), v: false, neg: T('An obtuse angle is not less than $90°$.', 'Sudut cakah tidak kurang daripada $90°$.') },
    { s: T('A kilometre is longer than a metre.', 'Satu kilometer lebih panjang daripada satu meter.'), v: true, neg: T('A kilometre is not longer than a metre.', 'Satu kilometer tidak lebih panjang daripada satu meter.') },
    { s: T('$2^{-1} = -2$.', '$2^{-1} = -2$.'), v: false, neg: T('$2^{-1} \\neq -2$.', '$2^{-1} \\neq -2$.') },
    { s: T('Every square is a rectangle.', 'Setiap segi empat sama ialah segi empat tepat.'), v: true, neg: T('Not every square is a rectangle.', 'Bukan setiap segi empat sama ialah segi empat tepat.') },
  ];
  const NON = [
  ...
  const g31e = [
    (r) => {
      ...
      return { q: T(`State which of the following is a statement, and give its truth value. (A) ${list[0][0].en} (B) ${list[1][0].en}`, `Nyatakan yang manakah antara berikut ialah pernyataan, dan berikan nilai kebenarannya. (A) ${list[0][0].ms} (B) ${list[1][0].ms}`), a: T(`(${list[0][1] ? 'A' : 'B'}); ${st.v ? 'true' : 'false'}`, `(${list[0][1] ? 'A' : 'B'}); ${st.v ? 'benar' : 'palsu'}`), sp: 's' };
    },
    (r) => {
      const st = r.pick(STM);
      return { q: T(`Write the negation of the statement: "${st.s.en}" State whether the negation is true or false.`, `Tulis penafian bagi pernyataan: "${st.s.ms}" Nyatakan sama ada penafian itu benar atau palsu.`), a: T(`${st.neg.en} ${st.v ? 'False' : 'True'}`, `${st.neg.ms} ${st.v ? 'Palsu' : 'Benar'}`), sp: 's' };
    },
    (r) => {
      const non = r.pick(NON);
      return { q: T(`Explain why the sentence "${non.s.en}" is not a statement.`, `Terangkan mengapa ayat "${non.s.ms}" bukan satu pernyataan.`), a: T(`It is ${non.kind.en}, so it cannot be judged true or false.`, `Ia ialah ${non.kind.ms}, jadi ia tidak boleh ditentukan benar atau palsu.`), sp: 's' };
    },
    (r) => {
      const non = r.pick(NON);
      ...
      need(opts.length >= 2);
      return { q: T(`"${non.s.en}" is an example of which type of sentence? ${opts.map((o, i) => `(${'ABCD'[i]}) ${o.en}`).join('  ')}`, `"${non.s.ms}" ialah contoh ayat jenis manakah? ${opts.map((o, i) => `(${'ABCD'[i]}) ${o.ms}`).join('  ')}`), a: T(`(${'ABCD'[opts.findIndex((o) => o.en === non.kind.en)]}) ${non.kind.en}`, `(${'ABCD'[opts.findIndex((o) => o.en === non.kind.en)]}) ${non.kind.ms}`), sp: 'xs' };
    },
    (r) => {
      const set = r.sample(STM.concat(NON.map((x) => ({ s: x.s, isStm: false }))), 4);
      const cnt = set.filter((x) => x.isStm !== false).length;
      return { q: T(`How many of the following sentences are statements? ${set.map((x, i) => `<br>(${'abcd'[i]}) ${x.s.en}`).join('')}`, `Berapakah antara ayat berikut yang merupakan pernyataan? ${set.map((x, i) => `<br>(${'abcd'[i]}) ${x.s.ms}`).join('')}`), a: T(`${cnt}`, `${cnt}`), sp: 's' };
    },
    (r) => {
      const st = r.pick(STM);
      return { q: T(`Is the sentence "${st.s.en}" a statement? If so, is it true or false?`, `Adakah ayat "${st.s.ms}" satu pernyataan? Jika ya, adakah ia benar atau palsu?`), a: T(`Yes, it is a statement; it is ${st.v ? 'true' : 'false'}.`, `Ya, ia satu pernyataan; ia ${st.v ? 'benar' : 'palsu'}.`), sp: 'xs' };
    },
  ];
  ...
  const g31m = [
    (r) => {
      ...
      return { q: T(`$D = ${roster(D)}$. Consider the statement: "${q.en(D, p)}" Write its negation and determine which of the two statements (the original or the negation) is true.`, `$D = ${roster(D)}$. Pertimbangkan pernyataan: "${q.ms(D, p)}" Tulis penafiannya dan tentukan yang manakah antara dua pernyataan itu (asal atau penafian) benar.`), a: T(`${q.negEn(D, p)} The negation is ${q.negTruth(D, p) ? 'true' : 'false'}; the original is ${q.truth(D, p) ? 'true' : 'false'}.`, `${q.negMs(D, p)} Penafian itu ${q.negTruth(D, p) ? 'benar' : 'palsu'}; pernyataan asal ${q.truth(D, p) ? 'benar' : 'palsu'}.`), sp: 'm' };
    },
    (r) => {
      const D = mkDomain(r), p = r.pick(PREDS), q = r.pick(QK);
      return { q: T(`$D = ${roster(D)}$. State whether the statement "${q.en(D, p)}" is true or false.`, `$D = ${roster(D)}$. Nyatakan sama ada pernyataan "${q.ms(D, p)}" benar atau palsu.`), a: T(`${q.truth(D, p) ? 'True' : 'False'}`, `${q.truth(D, p) ? 'Benar' : 'Palsu'}`), sp: 'xs' };
    },
    (r) => {
      const p = r.pick(STM), qq = r.pick(STM.filter((x) => x !== p));
      const and = r.chance();
      const v = and ? p.v && qq.v : p.v || qq.v;
      return { q: T(`Statement $p$: "${p.s.en}" Statement $q$: "${qq.s.en}" State whether "$p$ ${and ? 'and' : 'or'} $q$" is true or false, and explain using the truth values of $p$ and $q$.`, `Pernyataan $p$: "${p.s.ms}" Pernyataan $q$: "${qq.s.ms}" Nyatakan sama ada "$p$ ${and ? 'dan' : 'atau'} $q$" benar atau palsu, dan terangkan menggunakan nilai kebenaran $p$ dan $q$.`), a: T(`${v ? 'True' : 'False'} ($p$ is ${p.v ? 'true' : 'false'}, $q$ is ${qq.v ? 'true' : 'false'}, and "${and ? 'and' : 'or'}" needs ${and ? 'both' : 'at least one'} to be true)`, `${v ? 'Benar' : 'Palsu'} ($p$ ${p.v ? 'benar' : 'palsu'}, $q$ ${qq.v ? 'benar' : 'palsu'}, dan "${and ? 'dan' : 'atau'}" memerlukan ${and ? 'kedua-duanya' : 'sekurang-kurangnya satu'} benar)`), sp: 'm' };
    },
    (r) => {
      ...
      return { q: T(`Statement $p$: "${parts[0].s.en}" Statement $q$: "${parts[1].s.en}" Form the compound statements ${c1.en} ${c2.en} and state the truth value of each.`, `Pernyataan $p$: "${parts[0].s.ms}" Pernyataan $q$: "${parts[1].s.ms}" Bentukkan pernyataan majmuk ${c1.ms} ${c2.ms} dan nyatakan nilai kebenaran setiap satu.`), a: T(`(a) ${parts[0].v && parts[1].v ? 'True' : 'False'} (b) ${parts[0].v || parts[1].v ? 'True' : 'False'}`, `(a) ${parts[0].v && parts[1].v ? 'Benar' : 'Palsu'} (b) ${parts[0].v || parts[1].v ? 'Benar' : 'Palsu'}`), sp: 's' };
    },
  ];
  const g31a = [
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`Complete the truth table for $p$ and $q$, giving the truth value of ${cols.en[0]} and ${cols.en[1]} for all four combinations of truth values of $p$ and $q$.`, `Lengkapkan jadual kebenaran bagi $p$ dan $q$, dengan memberikan nilai kebenaran bagi ${cols.ms[0]} dan ${cols.ms[1]} untuk keempat-empat gabungan nilai kebenaran $p$ dan $q$.`), a: T(SPM.table(tv.map(([p, q], i) => [bm(p), bm(q), bm(rows[i][0]), bm(rows[i][1])]), { head: ['$p$', '$q$', cols.en[0], cols.en[1]] }), SPM.table(tv.map(([p, q], i) => [bmMs(p), bmMs(q), bmMs(rows[i][0]), bmMs(rows[i][1])]), { head: ['$p$', '$q$', cols.ms[0], cols.ms[1]] })), sp: 'm' };
    },
    (r) => {
      const D = mkDomain(r), p = r.pick(PREDS), q = r.pick(QK);
      const wrongKind = r.pick(QK.filter((x) => x.kind !== q.kind));
      const wrongNeg = wrongKind.negEn(D, p);
      return { q: T(`$D = ${roster(D)}$. A student writes the negation of "${q.en(D, p)}" as "${wrongNeg}". Is this correct? If not, write the correct negation.`, `$D = ${roster(D)}$. Seorang murid menulis penafian bagi "${q.ms(D, p)}" sebagai "${wrongKind.negMs(D, p)}". Adakah ini betul? Jika tidak, tulis penafian yang betul.`), a: T(`No, it is not correct. The correct negation is: "${q.negEn(D, p)}"`, `Tidak, ia tidak betul. Penafian yang betul ialah: "${q.negMs(D, p)}"`), sp: 'm' };
    },
    (r) => {
      const D = mkDomain(r), p = r.pick(PREDS), q = r.pick(QK);
      const distractors = r.sample(QK.filter((x) => x.kind !== q.kind), 2).map((x) => x.negEn(D, p));
      const otherPred = r.pick(PREDS.filter((x) => x !== p));
      const opts = r.shuffle([q.negEn(D, p), ...distractors, q.en(D, otherPred)]);
      const letters = 'ABCD';
      return { q: T(`$D = ${roster(D)}$. Which of the following is the correct negation of "${q.en(D, p)}"? ${opts.map((o, i) => `(${letters[i]}) ${o}`).join('  ')}`, `$D = ${roster(D)}$. Antara berikut, yang manakah penafian yang betul bagi "${q.ms(D, p)}"? ${opts.map((o, i) => `(${letters[i]}) ${o}`).join('  ')}`), a: T(`(${letters[opts.indexOf(q.negEn(D, p))]}) ${q.negEn(D, p)}`, `(${letters[opts.indexOf(q.negEn(D, p))]}) ${q.negMs(D, p)}`), sp: 's' };
    },
    (r) => {
      const D = mkDomain(r), p = r.pick(PREDS);
      const all = QK[0], some = QK[1], no = QK[2];
      return { q: T(`$D = ${roster(D)}$. (a) Is the statement "${all.en(D, p)}" true or false? (b) Is the statement "${some.en(D, p)}" true or false? (c) Explain why both can be evaluated from the same domain $D$.`, `$D = ${roster(D)}$. (a) Adakah pernyataan "${all.ms(D, p)}" benar atau palsu? (b) Adakah pernyataan "${some.ms(D, p)}" benar atau palsu? (c) Terangkan mengapa kedua-duanya boleh ditentukan daripada domain $D$ yang sama.`), a: T(`(a) ${all.truth(D, p) ? 'True' : 'False'} (b) ${some.truth(D, p) ? 'True' : 'False'} (c) Every element of $D$ is known, so each element can be checked against the property "${p.en}" directly.`, `(a) ${all.truth(D, p) ? 'Benar' : 'Palsu'} (b) ${some.truth(D, p) ? 'Benar' : 'Palsu'} (c) Setiap unsur $D$ diketahui, jadi setiap unsur boleh disemak terhadap sifat "${p.ms}" secara terus.`), sp: 'm' };
    },
  ];
  ...
  const ge32 = [
    ...
    (r) => {
      const c = r.pick(IMP);
      return { q: T(`For the implication "If ${c.p.en}, then ${c.q.en}," state (a) the hypothesis (premise), (b) the conclusion.`, `Bagi implikasi "Jika ${c.p.ms}, maka ${c.q.ms}," nyatakan (a) hipotesis (premis), (b) kesimpulan.`), a: T(`(a) ${c.p.en} (b) ${c.q.en}`, `(a) ${c.p.ms} (b) ${c.q.ms}`), sp: 's' };
    },
    (r) => {
      const c = r.pick(IMP);
      return { q: T(`Given $p$: "${c.p.en}" and $q$: "${c.q.en}," write the implication "if $p$ then $q$" as a full sentence.`, `Diberi $p$: "${c.p.ms}" dan $q$: "${c.q.ms}," tulis implikasi "jika $p$ maka $q$" sebagai satu ayat penuh.`), a: T(`If ${c.p.en}, then ${c.q.en}.`, `Jika ${c.p.ms}, maka ${c.q.ms}.`), sp: 's' };
    },
    (r) => {
      const c = r.pick(BIC);
      return { q: T(`Write the two implications represented by the biconditional statement "${c.p.en} if and only if ${c.q.en}."`, `Tulis dua implikasi yang diwakili oleh pernyataan dwibersyarat "${c.p.ms} jika dan hanya jika ${c.q.ms}."`), a: T(`If ${c.p.en}, then ${c.q.en}. If ${c.q.en}, then ${c.p.en}.`, `Jika ${c.p.ms}, maka ${c.q.ms}. Jika ${c.q.ms}, maka ${c.p.ms}.`), sp: 'm' };
    },
    (r) => {
      const c = r.pick(IMP);
      return { q: T(`Write the inverse of the implication "If ${c.p.en}, then ${c.q.en}."`, `Tulis songsang bagi implikasi "Jika ${c.p.ms}, maka ${c.q.ms}."`), a: T(`If it is not true that ${c.p.en}, then it is not true that ${c.q.en}.`, `Jika tidak benar bahawa ${c.p.ms}, maka tidak benar bahawa ${c.q.ms}.`), sp: 's' };
    },
    (r) => {
      const c = r.pick(IMP);
      return { q: T(`Write the contrapositive of the implication "If ${c.p.en}, then ${c.q.en}."`, `Tulis kontrapositif bagi implikasi "Jika ${c.p.ms}, maka ${c.q.ms}."`), a: T(`If it is not true that ${c.q.en}, then it is not true that ${c.p.en}.`, `Jika tidak benar bahawa ${c.q.ms}, maka tidak benar bahawa ${c.p.ms}.`), sp: 's' };
    },
    (r) => {
      const c = r.pick(IMP);
      const correct = T(`If ${c.q.en}, then ${c.p.en}.`, `Jika ${c.q.ms}, maka ${c.p.ms}.`);
      const distract = [
        T(`If it is not true that ${c.p.en}, then it is not true that ${c.q.en}.`, `Jika tidak benar bahawa ${c.p.ms}, maka tidak benar bahawa ${c.q.ms}.`),
        T(`If it is not true that ${c.q.en}, then it is not true that ${c.p.en}.`, `Jika tidak benar bahawa ${c.q.ms}, maka tidak benar bahawa ${c.p.ms}.`),
        T(`If ${c.p.en}, then ${c.q.en}.`, `Jika ${c.p.ms}, maka ${c.q.ms}.`),
      ];
      const opts = r.shuffle([correct, ...distract]);
      const letters = 'ABCD';
      return { q: T(`Which of the following is the converse of "If ${c.p.en}, then ${c.q.en}"? ${opts.map((o, i) => `(${letters[i]}) ${o.en}`).join('  ')}`, `Antara berikut, yang manakah akas bagi "Jika ${c.p.ms}, maka ${c.q.ms}"? ${opts.map((o, i) => `(${letters[i]}) ${o.ms}`).join('  ')}`), a: T(`(${letters[opts.indexOf(correct)]}) ${correct.en}`, `(${letters[opts.indexOf(correct)]}) ${correct.ms}`), sp: 's' };
    },
  ];
  const gm32 = [
    (r) => {
      const c = r.pick(IMP);
      return { q: T(`For the implication "If ${c.p.en}, then ${c.q.en}", write (a) the converse, (b) the inverse, (c) the contrapositive, and state whether each is true or false.`, `Bagi implikasi "Jika ${c.p.ms}, maka ${c.q.ms}", tulis (a) akas, (b) songsang, (c) kontrapositif, dan nyatakan sama ada setiap satu benar atau palsu.`), a: T(`(a) If ${c.q.en}, then ${c.p.en}: ${c.cvTrue ? 'true' : `false (${c.cvCex.en})`}. (b) If ${notEn(c.p.en)}, then ${notEn(c.q.en)}: ${c.cvTrue ? 'true' : `false (same counterexample)`}. (c) If ${notEn(c.q.en)}, then ${notEn(c.p.en)}: ${c.ivTrue ? 'true (equivalent to the original)' : `false (${c.ifCex.en})`}.`, `(a) Jika ${c.q.ms}, maka ${c.p.ms}: ${c.cvTrue ? 'benar' : `palsu (${c.cvCex.ms})`}. (b) Jika ${notMs(c.p.ms)}, maka ${notMs(c.q.ms)}: ${c.cvTrue ? 'benar' : `palsu (contoh bertentang yang sama)`}. (c) Jika ${notMs(c.q.ms)}, maka ${notMs(c.p.ms)}: ${c.ivTrue ? 'benar (setara dengan yang asal)' : `palsu (${c.ifCex.ms})`}.`), sp: 'l' };
    },
    (r) => {
      const c = r.pick(IMP_T);
      return { q: T(`Premise 1: If ${c.p.en}, then ${c.q.en}. Premise 2: ${c.p.en}, which is true. What can you validly conclude?`, `Premis 1: Jika ${c.p.ms}, maka ${c.q.ms}. Premis 2: ${c.p.ms}, iaitu benar. Apakah yang boleh anda simpulkan secara sah?`), a: T(`${c.q.en} (this follows because premise 2 confirms the hypothesis of premise 1)`, `${c.q.ms} (ini terhasil kerana premis 2 mengesahkan hipotesis premis 1)`), sp: 's' };
    },
    (r) => {
      const c = r.pick(IMP_T);
      return { q: T(`Premise 1: If ${c.p.en}, then ${c.q.en}. Premise 2: ${notEn(c.q.en)}. What can you validly conclude?`, `Premis 1: Jika ${c.p.ms}, maka ${c.q.ms}. Premis 2: ${notMs(c.q.ms)}. Apakah yang boleh anda simpulkan secara sah?`), a: T(`${notEn(c.p.en)} (since the conclusion fails, the hypothesis must also fail)`, `${notMs(c.p.ms)} (kerana kesimpulan itu gagal, hipotesis mesti juga gagal)`), sp: 's' };
    },
    (r) => {
      const c = r.pick(IMP_CVF);
      return { q: T(`Premise 1: If ${c.p.en}, then ${c.q.en}. Premise 2: ${c.q.en}, which is true. Can you validly conclude that "${c.p.en}"? Explain.`, `Premis 1: Jika ${c.p.ms}, maka ${c.q.ms}. Premis 2: ${c.q.ms}, iaitu benar. Bolehkah anda menyimpulkan secara sah bahawa "${c.p.ms}"? Terangkan.`), a: T(`No. Knowing that $q$ is true does not guarantee $p$ is true (e.g. ${c.cvCex.en}).`, `Tidak. Mengetahui $q$ benar tidak menjamin $p$ benar (cth. ${c.cvCex.ms}).`), sp: 'm' };
    },
    (r) => {
      const c = r.pick(IMP_T);
      return { q: T(`Premise 1: If ${c.p.en}, then ${c.q.en} (a true statement). Premise 2: ${c.p.en}. Conclusion: ${c.q.en}. Is this argument valid? Is it sound? Explain.`, `Premis 1: Jika ${c.p.ms}, maka ${c.q.ms} (satu pernyataan yang benar). Premis 2: ${c.p.ms}. Kesimpulan: ${c.q.ms}. Adakah hujah ini sah? Adakah ia munasabah (sound)? Terangkan.`), a: T('Valid: the conclusion follows the correct logical form. Sound: it is valid and premise 1 is true, so the conclusion is guaranteed true.', 'Sah: kesimpulan mengikut bentuk logik yang betul. Munasabah: ia sah dan premis 1 benar, jadi kesimpulan pasti benar.'), sp: 'm' };
    },
    (r) => {
      const c = r.pick(IMP_F);
      return { q: T(`Premise 1: If ${c.p.en}, then ${c.q.en}. Premise 2: ${c.p.en}. Conclusion: ${c.q.en}. Is this argument valid? Is it sound? Explain.`, `Premis 1: Jika ${c.p.ms}, maka ${c.q.ms}. Premis 2: ${c.p.ms}. Kesimpulan: ${c.q.ms}. Adakah hujah ini sah? Adakah ia munasabah (sound)? Terangkan.`), a: T(`Valid: the conclusion follows the correct logical form. Not sound: premise 1 is actually false (${c.ifCex.en}), so a sound conclusion is not guaranteed.`, `Sah: kesimpulan mengikut bentuk logik yang betul. Tidak munasabah: premis 1 sebenarnya palsu (${c.ifCex.ms}), jadi kesimpulan yang munasabah tidak terjamin.`), sp: 'm' };
    },
    (r) => {
      const c = r.pick(BIC);
      return { q: T(`State whether the biconditional "${c.p.en} if and only if ${c.q.en}" is true, by checking both directions.`, `Nyatakan sama ada dwibersyarat "${c.p.ms} jika dan hanya jika ${c.q.ms}" adalah benar, dengan menyemak kedua-dua arah.`), a: T(`True: "if ${c.p.en}, then ${c.q.en}" holds, and "if ${c.q.en}, then ${c.p.en}" also holds.`, `Benar: "jika ${c.p.ms}, maka ${c.q.ms}" adalah benar, dan "jika ${c.q.ms}, maka ${c.p.ms}" juga benar.`), sp: 'm' };
    },
    (r) => {
      const c = r.pick(REASON);
      return { q: T(`Is the following reasoning inductive or deductive? "${c.text.en}"`, `Adakah penaakulan berikut induktif atau deduktif? "${c.text.ms}"`), a: T(`${c.kind === 'deductive' ? 'Deductive' : 'Inductive'}: ${c.reason.en}`, `${c.kind === 'deductive' ? 'Deduktif' : 'Induktif'}: ${c.reason.ms}`), sp: 'm' };
    },
    (r) => {
      const c = r.pick(IMP_CVF);
      return { q: T(`Premise 1: If ${c.p.en}, then ${c.q.en}. Premise 2: ${notEn(c.p.en)}. Can you validly conclude that "${notEn(c.q.en)}"? Explain.`, `Premis 1: Jika ${c.p.ms}, maka ${c.q.ms}. Premis 2: ${notMs(c.p.ms)}. Bolehkah anda menyimpulkan secara sah bahawa "${notMs(c.q.ms)}"? Terangkan.`), a: T(`No. Knowing that $p$ is false does not guarantee $q$ is false (e.g. ${c.cvCex.en}, so $q$ can still be true).`, `Tidak. Mengetahui $p$ palsu tidak menjamin $q$ palsu (cth. ${c.cvCex.ms}, jadi $q$ masih boleh benar).`), sp: 'm' };
    },
  ];
  ...
  const ga32 = [
    (r) => {
      ...
      return { q: T(`Premise 1: If ${c.p.en}, then ${c.q.en}. Premise 2: ${pr[0]}. Conclusion: ${co[0]}. Is the argument valid? Explain.`, `Premis 1: Jika ${c.p.ms}, maka ${c.q.ms}. Premis 2: ${pr[1]}. Kesimpulan: ${co[1]}. Adakah hujah itu sah? Terangkan.`), a: k.valid ? T('Valid: whenever both premises are true, the conclusion must be true.', 'Sah: apabila kedua-dua premis benar, kesimpulan mesti benar.') : T(`Not valid: both premises can be true while the conclusion is false (${c.cvCex.en}).`, `Tidak sah: kedua-dua premis boleh benar sementara kesimpulan palsu (${c.cvCex.ms}).`), sp: 'm' };
    },
    (r) => {
      const c = r.pick(IMP_CVF);
      return { q: T(`The implication "If ${c.p.en}, then ${c.q.en}" is true. Its converse is "If ${c.q.en}, then ${c.p.en}." Show, using a counterexample, that the converse is false.`, `Implikasi "Jika ${c.p.ms}, maka ${c.q.ms}" adalah benar. Akasnya ialah "Jika ${c.q.ms}, maka ${c.p.ms}." Tunjukkan, menggunakan satu contoh bertentang, bahawa akas itu palsu.`), a: T(`${c.cvCex.en}, so the converse is false even though the original implication is true.`, `${c.cvCex.ms}, jadi akas itu palsu walaupun implikasi asal itu benar.`), sp: 'm' };
    },
    (r) => {
  ...
  const E3 = [
    ...
    ["A' \\cap B' \\cap C'", ['0']], ["(A \\cap B) \\cup (A \\cap C) \\cup (B \\cap C)", ['AB', 'AC', 'BC', 'ABC']],
  ];
  ...
  const g41e = [
    (r) => {
      const m = mk2(r);
      return { q: T(`$A = ${roster(m.A)}$ and $B = ${roster(m.B)}$. List the elements of $A \\cap B$ and state $n(A \\cap B)$.`, `$A = ${roster(m.A)}$ dan $B = ${roster(m.B)}$. Senaraikan unsur bagi $A \\cap B$ dan nyatakan $n(A \\cap B)$.`), a: T(`$A \\cap B = ${roster(m.elems.AB)}$; $n(A \\cap B) = ${m.elems.AB.length}$`), sp: 's' };
    },
    (r) => {
      const [P, Q] = r.pick(DISJ);
      return { q: T(`$\\xi = ${roster(UNI)}$. $P$ = the set of ${P.en}, $Q$ = the set of ${Q.en}. Find $P \\cap Q$. What can you say about $P$ and $Q$?`, `$\\xi = ${roster(UNI)}$. $P$ = set ${P.ms}, $Q$ = set ${Q.ms}. Cari $P \\cap Q$. Apakah yang boleh anda katakan tentang $P$ dan $Q$?`), a: T('$P \\cap Q = \\varnothing$: the sets are disjoint (no common elements).', '$P \\cap Q = \\varnothing$: set-set itu tidak bertindih (tiada unsur sepunya).'), sp: 's' };
    },
    (r) => {
      const { P, Q, sp, sq } = twoDesc(r);
      const x = r.pick(UNI);
      const inBoth = P.f(x) && Q.f(x);
      return { q: T(`$\\xi = ${roster(UNI)}$. $P$ = the set of ${P.en}, $Q$ = the set of ${Q.en}. Is $${x} \\in P \\cap Q$? Justify your answer.`, `$\\xi = ${roster(UNI)}$. $P$ = set ${P.ms}, $Q$ = set ${Q.ms}. Adakah $${x} \\in P \\cap Q$? Wajarkan jawapan anda.`), a: T(`${inBoth ? 'Yes' : 'No'}: $${x}$ is ${P.f(x) ? '' : 'not '}among the ${P.en} and is ${Q.f(x) ? '' : 'not '}among the ${Q.en}, so both conditions must hold to be in $P \\cap Q$.`, `${inBoth ? 'Ya' : 'Tidak'}: $${x}$ ${P.f(x) ? '' : 'bukan '}${P.ms} dan ${Q.f(x) ? '' : 'bukan '}${Q.ms}, dan kedua-dua syarat mesti dipenuhi untuk berada dalam $P \\cap Q$.`), sp: 's' };
    },
    (r) => {
      const { x, y, verbEn, verbMs } = pickCtx2(r);
      const opts = r.shuffle([['A \\cap B', true], ["A \\cup B", false], ["A'", false], ["A - B", false]]);
      return { q: T(`Let $A$ be the set of students who ${verbEn} ${x.en}, and $B$ be the set of students who ${verbEn} ${y.en}. Which set notation represents "students who ${verbEn} both ${x.en1 || x.en} and ${y.en1 || y.en}"? ${opts.map((o, i) => `(${'ABCD'[i]}) $${o[0]}$`).join('  ')}`, `Katakan $A$ ialah set murid yang ${verbMs} ${x.ms}, dan $B$ ialah set murid yang ${verbMs} ${y.ms}. Tatatanda set manakah mewakili "murid yang ${verbMs} kedua-dua ${x.ms} dan ${y.ms}"? ${opts.map((o, i) => `(${'ABCD'[i]}) $${o[0]}$`).join('  ')}`), a: T(`(${'ABCD'[opts.findIndex((o) => o[1])]}) $A \\cap B$`, `(${'ABCD'[opts.findIndex((o) => o[1])]}) $A \\cap B$`), sp: 's' };
    },
    (r) => {
      const m = mk2(r);
      const fig = vennElems(m, ['AB'], false);
      return { q: T('The Venn diagram shows sets $A$ and $B$. What does the shaded region represent? Write it in set notation and list its elements.', 'Gambar rajah Venn menunjukkan set $A$ dan $B$. Apakah yang diwakili oleh kawasan berlorek? Tulis dalam tatatanda set dan senaraikan unsurnya.'), fig, a: T(`$A \\cap B = ${roster(m.elems.AB)}$`), sp: 's' };
    },
  ];
  const g41m = [
    (r) => {
      ...
      return { q: T(`$n(A) = ${a}$, $n(B) = ${b}$ and $n(A \\cup B) = ${u}$. Find $n(A \\cap B)$.`, `$n(A) = ${a}$, $n(B) = ${b}$ dan $n(A \\cup B) = ${u}$. Cari $n(A \\cap B)$.`), a: T(`$n(A \\cap B) = ${a} + ${b} - ${u} = ${c}$`), sp: 's' };
    },
    (r) => {
      const m = mk2(r);
      const [name, keys] = r.pick(E2.filter((e) => ['A \\cap B', "A \\cap B'", "A' \\cap B"].includes(e[0])));
      const fig = vennElems(m, keys, false);
      return { q: T('The Venn diagram shows sets $A$ and $B$. Which set operation is represented by the shaded region? Write it using set notation and list its elements.', 'Gambar rajah Venn menunjukkan set $A$ dan $B$. Operasi set manakah yang diwakili oleh kawasan berlorek? Tulis menggunakan tatatanda set dan senaraikan unsurnya.'), fig, a: T(`$${name} = ${roster(regElems(m, keys))}$`), sp: 's' };
    },
    (r) => {
      const { x, y, verbEn, verbMs } = pickCtx2(r);
      const both = r.int(4, 12), onlyX = r.int(3, 12), onlyY = r.int(3, 12);
      const tot = both + onlyX + onlyY + r.int(0, 5);
      return { q: T(`In a group of ${tot} students, ${onlyX + both} ${verbEn} ${x.en}, ${onlyY + both} ${verbEn} ${y.en}, and ${both} ${verbEn} both. Find the number of students who ${verbEn} only ${x.en1 || x.en}.`, `Dalam sekumpulan ${tot} orang murid, ${onlyX + both} ${verbMs} ${x.ms}, ${onlyY + both} ${verbMs} ${y.ms}, dan ${both} ${verbMs} kedua-duanya. Cari bilangan murid yang ${verbMs} ${x.ms} sahaja.`), a: T(`${onlyX + both} - ${both} = ${onlyX}`), sp: 's' };
    },
    (r) => {
      const { P, Q, sp, sq } = twoDesc(r);
      return { q: T(`$\\xi = ${roster(UNI)}$. $P = \\{x : x \\text{ is one of the ${P.en}}, x \\in \\xi\\}$, $Q = \\{x : x \\text{ is one of the ${Q.en}}, x \\in \\xi\\}$. List the elements of $P \\cap Q$.`, `$\\xi = ${roster(UNI)}$. $P = \\{x : x \\text{ ialah salah satu ${P.ms}}, x \\in \\xi\\}$, $Q = \\{x : x \\text{ ialah salah satu ${Q.ms}}, x \\in \\xi\\}$. Senaraikan unsur bagi $P \\cap Q$.`), a: T(`$P \\cap Q = ${rst(inter(sp, sq))}$`), sp: 's' };
    },
    (r) => {
      const preds = r.sample(SETDESC, 3);
      const sets = preds.map((p) => UNI.filter(p.f));
      const pick2 = r.shuffle([0, 1, 2]).slice(0, 2);
      return { q: T(`$\\xi = ${roster(UNI)}$. $P$ = the set of ${preds[0].en}, $Q$ = the set of ${preds[1].en}, $R$ = the set of ${preds[2].en}. Find $${'PQR'[pick2[0]]} \\cap ${'PQR'[pick2[1]]}$.`, `$\\xi = ${roster(UNI)}$. $P$ = set ${preds[0].ms}, $Q$ = set ${preds[1].ms}, $R$ = set ${preds[2].ms}. Cari $${'PQR'[pick2[0]]} \\cap ${'PQR'[pick2[1]]}$.`), a: T(`$${rst(inter(sets[pick2[0]], sets[pick2[1]]))}$`), sp: 's' };
    },
  ];
  const g41a = [
    (r) => {
      ...
      return { q: T(`$\\xi = ${roster(UNI)}$, $P$ = the set of ${preds[0].en}, $Q$ = the set of ${preds[1].en}, $R$ = the set of ${preds[2].en}. Find (a) $P \\cap Q$, (b) $Q \\cap R$, (c) $P \\cap Q \\cap R$.`, `$\\xi = ${roster(UNI)}$, $P$ = set ${preds[0].ms}, $Q$ = set ${preds[1].ms}, $R$ = set ${preds[2].ms}. Cari (a) $P \\cap Q$, (b) $Q \\cap R$, (c) $P \\cap Q \\cap R$.`), a: T(`(a) $${rst(inter(sets[0], sets[1]))}$ (b) $${rst(inter(sets[1], sets[2]))}$ (c) $${rst(abc)}$`), sp: 'm' };
    },
    (r) => {
      const m = mk3(r);
    ...
    (r) => {
      ...
      need(a + b - neither <= tot && a + b - tot + neither >= 1);
    ...
    (r) => {
      ...
      return retry(() => {
        ...
        return { q: T(`A survey of ${tot} students asked about three activities: $A$ (${verbEn} ${x.en}), $B$ (${verbEn} ${y.en}) and $C$ (${verbEn} ${z.en}). ${m.A.length} are in $A$, ${m.B.length} in $B$ and ${m.C.length} in $C$; ${reg.AB + reg.ABC} are in both $A$ and $B$, ${reg.AC + reg.ABC} in both $A$ and $C$, ${reg.BC + reg.ABC} in both $B$ and $C$; ${reg['0']} are in none of the three. Find $n(A \\cap B \\cap C)$.`, `Satu tinjauan terhadap ${tot} orang murid bertanya tentang tiga aktiviti: $A$ (${verbMs} ${x.ms}), $B$ (${verbMs} ${y.ms}) dan $C$ (${verbMs} ${z.ms}). ${m.A.length} dalam $A$, ${m.B.length} dalam $B$ dan ${m.C.length} dalam $C$; ${reg.AB + reg.ABC} dalam kedua-dua $A$ dan $B$, ${reg.AC + reg.ABC} dalam kedua-dua $A$ dan $C$, ${reg.BC + reg.ABC} dalam kedua-dua $B$ dan $C$; ${reg['0']} tidak termasuk mana-mana satu. Cari $n(A \\cap B \\cap C)$.`), a: T(`$n(A \\cap B \\cap C) = ${reg.ABC}$ ${fig}`, `$n(A \\cap B \\cap C) = ${reg.ABC}$ ${fig}`), sp: 'xl' };
      });
    },
  ];
  ...
  const g42e = [
    (r) => {
      const m = mk2(r);
      return { q: T(`$A = ${roster(m.A)}$ and $B = ${roster(m.B)}$. List the elements of $A \\cup B$ and state $n(A \\cup B)$.`, `$A = ${roster(m.A)}$ dan $B = ${roster(m.B)}$. Senaraikan unsur bagi $A \\cup B$ dan nyatakan $n(A \\cup B)$.`), a: T(`$A \\cup B = ${roster(uni(m.A, m.B))}$; $n(A \\cup B) = ${uni(m.A, m.B).length}$`), sp: 's' };
    },
    (r) => {
      const { P, Q, sp, sq } = twoDesc(r);
      return { q: T(`$\\xi = ${roster(UNI)}$. $P$ = the set of ${P.en}, $Q$ = the set of ${Q.en}. List the elements of $P \\cup Q$.`, `$\\xi = ${roster(UNI)}$. $P$ = set ${P.ms}, $Q$ = set ${Q.ms}. Senaraikan unsur bagi $P \\cup Q$.`), a: T(`$P \\cup Q = ${roster(uni(sp, sq))}$`), sp: 's' };
    },
    (r) => {
      const { x, y, verbEn, verbMs } = pickCtx2(r);
      const opts = r.shuffle([['A \\cup B', true], ['A \\cap B', false], ["A'", false], ["A \\cap B'", false]]);
      return { q: T(`Let $A$ be the set of students who ${verbEn} ${x.en}, and $B$ be the set of students who ${verbEn} ${y.en}. Which set notation represents "students who ${verbEn} ${x.en1 || x.en}, ${y.en1 || y.en}, or both"? ${opts.map((o, i) => `(${'ABCD'[i]}) $${o[0]}$`).join('  ')}`, `Katakan $A$ ialah set murid yang ${verbMs} ${x.ms}, dan $B$ ialah set murid yang ${verbMs} ${y.ms}. Tatatanda set manakah mewakili "murid yang ${verbMs} ${x.ms}, ${y.ms}, atau kedua-duanya"? ${opts.map((o, i) => `(${'ABCD'[i]}) $${o[0]}$`).join('  ')}`), a: T(`(${'ABCD'[opts.findIndex((o) => o[1])]}) $A \\cup B$`, `(${'ABCD'[opts.findIndex((o) => o[1])]}) $A \\cup B$`), sp: 's' };
    },
    (r) => {
      const m = mk2(r);
      const fig = vennElems(m, ['A', 'B', 'AB'], false);
      return { q: T('The Venn diagram shows sets $A$ and $B$. What does the shaded region represent? Write it in set notation and list its elements.', 'Gambar rajah Venn menunjukkan set $A$ dan $B$. Apakah yang diwakili oleh kawasan berlorek? Tulis dalam tatatanda set dan senaraikan unsurnya.'), fig, a: T(`$A \\cup B = ${roster(uni(m.A, m.B))}$`), sp: 's' };
    },
    (r) => {
      const m = mk2(r);
      const combined = uni(m.A, m.B);
      const opts = r.shuffle([[combined, true], [m.elems.AB, false], [m.A, false]]);
      return { q: T(`$A = ${roster(m.A)}$ and $B = ${roster(m.B)}$. Which of these is $A \\cup B$? (A) $${roster(opts[0][0])}$ (B) $${roster(opts[1][0])}$ (C) $${roster(opts[2][0])}$`, `$A = ${roster(m.A)}$ dan $B = ${roster(m.B)}$. Yang manakah $A \\cup B$? (A) $${roster(opts[0][0])}$ (B) $${roster(opts[1][0])}$ (C) $${roster(opts[2][0])}$`), a: T(`(${'ABC'[opts.findIndex((o) => o[1])]}) $${roster(combined)}$`), sp: 's' };
    },
  ];
  const g42m = [
    (r) => {
    ...
    (r) => {
      ...
      const sets = preds.map((p) => UNI.filter(p.f));
      const pick2 = r.shuffle([0, 1, 2]).slice(0, 2);
      return { q: T(`$\\xi = ${roster(UNI)}$. $P$ = the set of ${preds[0].en}, $Q$ = the set of ${preds[1].en}, $R$ = the set of ${preds[2].en}. Find $${'PQR'[pick2[0]]} \\cup ${'PQR'[pick2[1]]}$.`, `$\\xi = ${roster(UNI)}$. $P$ = set ${preds[0].ms}, $Q$ = set ${preds[1].ms}, $R$ = set ${preds[2].ms}. Cari $${'PQR'[pick2[0]]} \\cup ${'PQR'[pick2[1]]}$.`), a: T(`$${rst(uni(sets[pick2[0]], sets[pick2[1]]))}$`), sp: 's' };
    },
  ];
  const g42a = [
    (r) => {
      const preds = r.sample(SETDESC, 3);
      const sets = preds.map((p) => UNI.filter(p.f));
      return { q: T(`$\\xi = ${roster(UNI)}$, $P$ = the set of ${preds[0].en}, $Q$ = the set of ${preds[1].en}, $R$ = the set of ${preds[2].en}. Find $n(P \\cup Q \\cup R)$.`, `$\\xi = ${roster(UNI)}$, $P$ = set ${preds[0].ms}, $Q$ = set ${preds[1].ms}, $R$ = set ${preds[2].ms}. Cari $n(P \\cup Q \\cup R)$.`), a: T(`${uni(uni(sets[0], sets[1]), sets[2]).length}`), w: T(`$P \\cup Q \\cup R = ${rst(uni(uni(sets[0], sets[1]), sets[2]))}$`, `$P \\cup Q \\cup R = ${rst(uni(uni(sets[0], sets[1]), sets[2]))}$`), sp: 'm' };
    },
    (r) => {
      const m = mk3(r);
    ...
    (r) => {
      ...
      return retry(() => {
        ...
        return { q: T(`In a group of ${tot} students, $A$ = students who ${verbEn} ${x.en} with $n(A) = ${a}$, and $B$ = students who ${verbEn} ${y.en} with $n(B) = ${b}$. $n(A \\cup B) = ${u}$. If $x$ students ${verbEn} both, form an equation in $x$ and solve it.`, `Dalam sekumpulan ${tot} orang murid, $A$ = murid yang ${verbMs} ${x.ms} dengan $n(A) = ${a}$, dan $B$ = murid yang ${verbMs} ${y.ms} dengan $n(B) = ${b}$. $n(A \\cup B) = ${u}$. Jika $x$ orang murid ${verbMs} kedua-duanya, bentukkan satu persamaan dalam $x$ dan selesaikannya.`), a: T(`$${a} + ${b} - x = ${u} \\Rightarrow x = ${a} + ${b} - ${u} = ${c}$`), sp: 'm' };
      });
    },
    (r) => {
      const { x, y, z, verbEn, verbMs } = pickCtx3(r);
      return retry(() => {
        ...
        return { q: T(`In a survey of ${tot} students about three activities $A$ (${verbEn} ${x.en}), $B$ (${verbEn} ${y.en}) and $C$ (${verbEn} ${z.en}), ${reg['0']} students take part in none of the three. Find $n(A \\cup B \\cup C)$.`, `Dalam satu tinjauan terhadap ${tot} orang murid tentang tiga aktiviti $A$ (${verbMs} ${x.ms}), $B$ (${verbMs} ${y.ms}) dan $C$ (${verbMs} ${z.ms}), ${reg['0']} orang murid tidak menyertai mana-mana satu. Cari $n(A \\cup B \\cup C)$.`), a: T(`$n(A \\cup B \\cup C) = n(\\xi) - n((A \\cup B \\cup C)') = ${tot} - ${reg['0']} = ${uABC}$`), sp: 'm' };
      });
    },
  ];
  ...
  const g43e = [
    (r) => {
      const m = mk2(r);
      return { q: T(`$\\xi = ${roster(m.U)}$ and $A = ${roster(m.A)}$. List the elements of $A'$.`, `$\\xi = ${roster(m.U)}$ dan $A = ${roster(m.A)}$. Senaraikan unsur bagi $A'$.`), a: T(`$A' = ${rst(diff(m.U, m.A))}$`), sp: 's' };
    },
    (r) => {
      const P = r.pick(SETDESC), sp = UNI.filter(P.f);
      return { q: T(`$\\xi = ${roster(UNI)}$. $P$ = the set of ${P.en}. List the elements of $P'$.`, `$\\xi = ${roster(UNI)}$. $P$ = set ${P.ms}. Senaraikan unsur bagi $P'$.`), a: T(`$P' = ${rst(diff(UNI, sp))}$`), sp: 's' };
    },
    (r) => {
      const m = mk2(r);
      const fig = vennElems(m, ['0'], false);
      return { q: T('In the Venn diagram, the shaded region is outside both circles. Write the set represented and list its elements.', 'Dalam gambar rajah Venn, kawasan berlorek berada di luar kedua-dua bulatan. Tulis set yang diwakili dan senaraikan unsurnya.'), fig, a: T(`$(A \\cup B)' = ${rst(m.elems['0'])}$`), sp: 's' };
    },
    (r) => {
      const { x, verbEn, verbMs } = pickCtx2(r);
      const opts = r.shuffle([["A'", true], ['A', false], ['A \\cap B', false]]);
      return { q: T(`$A$ is the set of students who ${verbEn} ${x.en}. Which set notation represents "students who do not ${verbEn} ${x.en1 || x.en}"? (A) $${opts[0][0]}$ (B) $${opts[1][0]}$ (C) $${opts[2][0]}$`, `$A$ ialah set murid yang ${verbMs} ${x.ms}. Tatatanda set manakah mewakili "murid yang tidak ${verbMs} ${x.ms}"? (A) $${opts[0][0]}$ (B) $${opts[1][0]}$ (C) $${opts[2][0]}$`), a: T(`(${'ABC'[opts.findIndex((o) => o[1])]}) $A'$`), sp: 's' };
    },
    (r) => {
      const m = mk2(r);
      const [name, keys] = r.pick(E2.filter((e) => e[0] === "A \\cap B'" || e[0] === "A' \\cap B"));
      const fig = vennElems(m, keys, false);
      return { q: T('The Venn diagram shows sets $A$ and $B$. Write an expression for the shaded region and list its elements.', 'Gambar rajah Venn menunjukkan set $A$ dan $B$. Tulis satu ungkapan bagi kawasan berlorek dan senaraikan unsurnya.'), fig, a: T(`$${name} = ${rst(regElems(m, keys))}$`), sp: 's' };
    },
    (r) => {
      const P = r.pick(SETDESC), sp = UNI.filter(P.f), x = r.pick(UNI);
      return { q: T(`$\\xi = ${roster(UNI)}$. $P$ = the set of ${P.en}. Is $${x} \\in P'$? Justify your answer.`, `$\\xi = ${roster(UNI)}$. $P$ = set ${P.ms}. Adakah $${x} \\in P'$? Wajarkan jawapan anda.`), a: T(`${P.f(x) ? 'No' : 'Yes'}: $${x}$ is ${P.f(x) ? '' : 'not '}among the ${P.en}, so $${x}$ is ${P.f(x) ? 'not ' : ''}in $P'$.`, `${P.f(x) ? 'Tidak' : 'Ya'}: $${x}$ ${P.f(x) ? '' : 'bukan '}${P.ms}, jadi $${x}$ ${P.f(x) ? 'tidak ' : ''}berada dalam $P'$.`), sp: 's' };
    },
    (r) => {
      const [P, Q] = r.pick(DISJ);
    ...
    (r) => {
      const m = mk2(r);
      const fig = S.venn1({ name: 'A', inA: m.A, out: m.elems['0'].concat(m.B).sort((a, b) => a - b), shade: 'out', xi: 'ξ' });
      return { q: T('The Venn diagram shows set $A$ inside the universal set $\\xi$. Write the set represented by the shaded region and list its elements.', 'Gambar rajah Venn menunjukkan set $A$ di dalam set semesta $\\xi$. Tulis set yang diwakili oleh kawasan berlorek dan senaraikan unsurnya.'), fig, a: T(`$A' = ${rst(m.elems['0'].concat(m.B).sort((a, b) => a - b))}$`), sp: 's' };
    },
    (r) => {
      const { x, y, verbEn, verbMs } = pickCtx2(r);
      const a = r.int(10, 25), tot = r.int(a + 8, a + 30);
      return { q: T(`In a group of ${tot} students, $A$ = students who ${verbEn} ${x.en}, with $n(A) = ${a}$. Find $n(A')$.`, `Dalam sekumpulan ${tot} orang murid, $A$ = murid yang ${verbMs} ${x.ms}, dengan $n(A) = ${a}$. Cari $n(A')$.`), a: T(`$n(A') = ${tot} - ${a} = ${tot - a}$`), sp: 's' };
    },
  ];
  const g43m = [
    (r) => {
  ...
  const ga43 = [
    ...
    (r) => {
      ...
      return retry(() => {
        ...
        const fig = vennCounts(reg, true);
        return { q: T(`A survey of ${tot} students asked about three activities: $A$ (${verbEn} ${x.en}), $B$ (${verbEn} ${y.en}) and $C$ (${verbEn} ${z.en}). ${m.A.length} are in $A$, ${m.B.length} in $B$ and ${m.C.length} in $C$; ${reg.AB + reg.ABC} are in both $A$ and $B$, ${reg.AC + reg.ABC} in both $A$ and $C$, ${reg.BC + reg.ABC} in both $B$ and $C$; ${reg.ABC} are in all three. Find (a) the number in exactly one activity, (b) the number in exactly two activities.`, `Satu tinjauan terhadap ${tot} orang murid bertanya tentang tiga aktiviti: $A$ (${verbMs} ${x.ms}), $B$ (${verbMs} ${y.ms}) dan $C$ (${verbMs} ${z.ms}). ${m.A.length} dalam $A$, ${m.B.length} dalam $B$ dan ${m.C.length} dalam $C$; ${reg.AB + reg.ABC} dalam kedua-dua $A$ dan $B$, ${reg.AC + reg.ABC} dalam kedua-dua $A$ dan $C$, ${reg.BC + reg.ABC} dalam kedua-dua $B$ dan $C$; ${reg.ABC} dalam ketiga-tiganya. Cari (a) bilangan yang menyertai tepat satu aktiviti, (b) bilangan yang menyertai tepat dua aktiviti.`), a: T(`(a) ${onlyOne} (b) ${exactlyTwo}`, `(a) ${onlyOne} (b) ${exactlyTwo}`), w: T(fig, fig), sp: 'l' };
      });
    },
    (r) => {
      const { x, y, verbEn, verbMs } = pickCtx2(r);
