...
(function () {
  ...
  const W = SPM.lines;
  const tf = (b) => (b ? 'true' : 'false'), tfm = (b) => (b ? 'benar' : 'palsu');
  const STM_DEF = T('A statement is a sentence that is either true or false, but not both.', 'Pernyataan ialah ayat yang sama ada benar atau palsu, tetapi bukan kedua-duanya.');
  ...
  const nonWhy = (x) => T(`"${x.s.en}" is ${x.kind.en} (${KIND_WHY[x.kind.en].en}), so it cannot be judged true or false.`, `"${x.s.ms}" ialah ${x.kind.ms} (${KIND_WHY[x.kind.en].ms}), jadi ia tidak boleh ditentukan benar atau palsu.`);
  ...
  const AND_R = T('"$p$ and $q$" is true only when both $p$ and $q$ are true.', '"$p$ dan $q$" benar hanya apabila kedua-dua $p$ dan $q$ benar.');
  const OR_R = T('"$p$ or $q$" is true when at least one of $p$ and $q$ is true.', '"$p$ atau $q$" benar apabila sekurang-kurangnya satu daripada $p$ dan $q$ benar.');
  const g31e = [
    (r) => {
      const st = r.pick(STM), non = r.pick(NON);
      const list = r.shuffle([[st.s, true], [non.s, false]]);
      return { q: T(`State which of the following is a statement, and give its truth value. (A) ${list[0][0].en} (B) ${list[1][0].en}`, `Nyatakan yang manakah antara berikut ialah pernyataan, dan berikan nilai kebenarannya. (A) ${list[0][0].ms} (B) ${list[1][0].ms}`), a: T(`(${list[0][1] ? 'A' : 'B'}); ${st.v ? 'true' : 'false'}`, `(${list[0][1] ? 'A' : 'B'}); ${st.v ? 'benar' : 'palsu'}`), w: W(nonWhy(non), stmWhy(st)), sp: 's' };
    },
    (r) => {
      const st = r.pick(STM);
      return { q: T(`Write the negation of the statement: "${st.s.en}" State whether the negation is true or false.`, `Tulis penafian bagi pernyataan: "${st.s.ms}" Nyatakan sama ada penafian itu benar atau palsu.`), a: T(`${st.neg.en} ${st.v ? 'False' : 'True'}`, `${st.neg.ms} ${st.v ? 'Palsu' : 'Benar'}`), w: W(stmWhy(st, T('The original statement', 'Pernyataan asal')), NEG_TV, T(`So the negation is ${tf(!st.v)}.`, `Maka penafian itu ${tfm(!st.v)}.`)), sp: 's' };
    },
    (r) => {
      const non = r.pick(NON);
      return { q: T(`Explain why the sentence "${non.s.en}" is not a statement.`, `Terangkan mengapa ayat "${non.s.ms}" bukan satu pernyataan.`), a: T(`It is ${non.kind.en}, so it cannot be judged true or false.`, `Ia ialah ${non.kind.ms}, jadi ia tidak boleh ditentukan benar atau palsu.`), w: W(STM_DEF, nonWhy(non)), sp: 's' };
    },
    (r) => {
      const non = r.pick(NON);
    ...
    (r) => {
      const st = r.pick(STM);
      return { q: T(`Is the sentence "${st.s.en}" a statement? If so, is it true or false?`, `Adakah ayat "${st.s.ms}" satu pernyataan? Jika ya, adakah ia benar atau palsu?`), a: T(`Yes, it is a statement; it is ${st.v ? 'true' : 'false'}.`, `Ya, ia satu pernyataan; ia ${st.v ? 'benar' : 'palsu'}.`), w: W(STM_DEF, stmWhy(st)), sp: 'xs' };
    },
  ];
  ...
  const qConc = (D, p, k) => {
    ...
    if (k === 'all') return no.length ? T(`$${no[0]}$ ${p.neg}, so "every element" is false.`, `$${no[0]}$ ${p.negms}, jadi "setiap unsur" adalah palsu.`) : T(`All ${D.length} elements have the property, so "every element" is true.`, `Kesemua ${D.length} unsur mempunyai sifat itu, jadi "setiap unsur" adalah benar.`);
    if (k === 'some') return yes.length ? T(`$${yes[0]}$ ${p.en}, so "at least one element" is true.`, `$${yes[0]}$ ${p.ms}, jadi "sekurang-kurangnya satu unsur" adalah benar.`) : T('No element has the property, so "at least one element" is false.', 'Tiada unsur yang mempunyai sifat itu, jadi "sekurang-kurangnya satu unsur" adalah palsu.');
    return yes.length ? T(`$${yes[0]}$ ${p.en}, so "no element" is false.`, `$${yes[0]}$ ${p.ms}, jadi "tiada unsur" adalah palsu.`) : T('No element has the property, so "no element" is true.', 'Tiada unsur yang mempunyai sifat itu, jadi "tiada unsur" adalah benar.');
  };
  ...
  const g31m = [
    (r) => {
      ...
      return { q: T(`$D = ${roster(D)}$. Consider the statement: "${q.en(D, p)}" Write its negation and determine which of the two statements (the original or the negation) is true.`, `$D = ${roster(D)}$. Pertimbangkan pernyataan: "${q.ms(D, p)}" Tulis penafiannya dan tentukan yang manakah antara dua pernyataan itu (asal atau penafian) benar.`), a: T(`${q.negEn(D, p)} The negation is ${q.negTruth(D, p) ? 'true' : 'false'}; the original is ${q.truth(D, p) ? 'true' : 'false'}.`, `${q.negMs(D, p)} Penafian itu ${q.negTruth(D, p) ? 'benar' : 'palsu'}; pernyataan asal ${q.truth(D, p) ? 'benar' : 'palsu'}.`), w: W(NEG_Q, qLine(D, p), qConc(D, p, q.kind), NEG_TV), sp: 'm' };
    },
    (r) => {
      const D = mkDomain(r), p = r.pick(PREDS), q = r.pick(QK);
      return { q: T(`$D = ${roster(D)}$. State whether the statement "${q.en(D, p)}" is true or false.`, `$D = ${roster(D)}$. Nyatakan sama ada pernyataan "${q.ms(D, p)}" benar atau palsu.`), a: T(`${q.truth(D, p) ? 'True' : 'False'}`, `${q.truth(D, p) ? 'Benar' : 'Palsu'}`), w: W(qLine(D, p), qConc(D, p, q.kind)), sp: 'xs' };
    },
    (r) => {
      const p = r.pick(STM), qq = r.pick(STM.filter((x) => x !== p));
      const and = r.chance();
      const v = and ? p.v && qq.v : p.v || qq.v;
      return { q: T(`Statement $p$: "${p.s.en}" Statement $q$: "${qq.s.en}" State whether "$p$ ${and ? 'and' : 'or'} $q$" is true or false, and explain using the truth values of $p$ and $q$.`, `Pernyataan $p$: "${p.s.ms}" Pernyataan $q$: "${qq.s.ms}" Nyatakan sama ada "$p$ ${and ? 'dan' : 'atau'} $q$" benar atau palsu, dan terangkan menggunakan nilai kebenaran $p$ dan $q$.`), a: T(`${v ? 'True' : 'False'} ($p$ is ${p.v ? 'true' : 'false'}, $q$ is ${qq.v ? 'true' : 'false'}, and "${and ? 'and' : 'or'}" needs ${and ? 'both' : 'at least one'} to be true)`, `${v ? 'Benar' : 'Palsu'} ($p$ ${p.v ? 'benar' : 'palsu'}, $q$ ${qq.v ? 'benar' : 'palsu'}, dan "${and ? 'dan' : 'atau'}" memerlukan ${and ? 'kedua-duanya' : 'sekurang-kurangnya satu'} benar)`), w: W(stmWhy(p, T('$p$')), stmWhy(qq, T('$q$')), and ? AND_R : OR_R, T(`So "$p$ ${and ? 'and' : 'or'} $q$" is ${tf(v)}.`, `Maka "$p$ ${and ? 'dan' : 'atau'} $q$" adalah ${tfm(v)}.`)), sp: 'm' };
    },
    (r) => {
      ...
      return { q: T(`Statement $p$: "${parts[0].s.en}" Statement $q$: "${parts[1].s.en}" Form the compound statements ${c1.en} ${c2.en} and state the truth value of each.`, `Pernyataan $p$: "${parts[0].s.ms}" Pernyataan $q$: "${parts[1].s.ms}" Bentukkan pernyataan majmuk ${c1.ms} ${c2.ms} dan nyatakan nilai kebenaran setiap satu.`), a: T(`(a) ${parts[0].v && parts[1].v ? 'True' : 'False'} (b) ${parts[0].v || parts[1].v ? 'True' : 'False'}`, `(a) ${parts[0].v && parts[1].v ? 'Benar' : 'Palsu'} (b) ${parts[0].v || parts[1].v ? 'Benar' : 'Palsu'}`), w: W(stmWhy(parts[0], T('$p$')), stmWhy(parts[1], T('$q$')), T(`(a) ${AND_R.en}`, `(a) ${AND_R.ms}`), T(`(b) ${OR_R.en}`, `(b) ${OR_R.ms}`)), sp: 's' };
    },
  ];
  const g31a = [
    (r) => {
      ...
      return c[2]
        ? { q: T('The compound statement "$p$ and $q$" is true. What can you say about the truth values of $p$ and $q$?', 'Pernyataan majmuk "$p$ dan $q$" adalah benar. Apakah yang boleh anda katakan tentang nilai kebenaran $p$ dan $q$?'), a: T('Both $p$ and $q$ must be true.', '$p$ dan $q$ mesti kedua-duanya benar.'), w: W(AND_R, T('If either $p$ or $q$ were false, "$p$ and $q$" would be false.', 'Jika salah satu daripada $p$ atau $q$ palsu, "$p$ dan $q$" akan palsu.')), sp: 's' }
        : { q: T('The compound statement "$p$ or $q$" is false. What can you say about the truth values of $p$ and $q$?', 'Pernyataan majmuk "$p$ atau $q$" adalah palsu. Apakah yang boleh anda katakan tentang nilai kebenaran $p$ dan $q$?'), a: T('Both $p$ and $q$ must be false.', '$p$ dan $q$ mesti kedua-duanya palsu.'), w: W(OR_R, T('If either $p$ or $q$ were true, "$p$ or $q$" would be true.', 'Jika salah satu daripada $p$ atau $q$ benar, "$p$ atau $q$" akan benar.')), sp: 's' };
    },
    (r) => {
      const cols = r.pick([
  ...
  const F_II = T('Form II: "if $p$, then $q$" and "$p$ is true" give the valid conclusion "$q$ is true".', 'Bentuk II: "jika $p$, maka $q$" dan "$p$ benar" memberikan kesimpulan sah "$q$ benar".');
  const F_III = T('Form III: "if $p$, then $q$" and "$\\sim q$ is true" give the valid conclusion "$\\sim p$ is true".', 'Bentuk III: "jika $p$, maka $q$" dan "$\\sim q$ benar" memberikan kesimpulan sah "$\\sim p$ benar".');
  const AFF_Q = T('Premise 2 confirms $q$ (the consequent), not $p$: this form is not valid, since $q$ can be true while $p$ is false.', 'Premis 2 mengesahkan $q$ (akibat), bukan $p$: bentuk ini tidak sah, kerana $q$ boleh benar sementara $p$ palsu.');
  const DEN_P = T('Premise 2 denies $p$ (the antecedent): this form is not valid, since $q$ can still be true when $p$ is false.', 'Premis 2 menafikan $p$ (antejadian): bentuk ini tidak sah, kerana $q$ masih boleh benar apabila $p$ palsu.');
  const cexLine = (x) => T(`Counterexample: ${x.en}.`, `Contoh bertentang: ${x.ms}.`);
  const SOUND = T('A valid argument is sound only if all its premises are true.', 'Hujah yang sah adalah munasabah hanya jika semua premisnya benar.');
  ...
  const ge32 = [
    ...
    (r) => {
      const c = r.pick(IMP);
      return { q: T(`Write the inverse of the implication "If ${c.p.en}, then ${c.q.en}."`, `Tulis songsang bagi implikasi "Jika ${c.p.ms}, maka ${c.q.ms}."`), a: T(`If it is not true that ${c.p.en}, then it is not true that ${c.q.en}.`, `Jika tidak benar bahawa ${c.p.ms}, maka tidak benar bahawa ${c.q.ms}.`), w: W(pqLine(c), R_INV), sp: 's' };
    },
    (r) => {
      const c = r.pick(IMP);
      return { q: T(`Write the contrapositive of the implication "If ${c.p.en}, then ${c.q.en}."`, `Tulis kontrapositif bagi implikasi "Jika ${c.p.ms}, maka ${c.q.ms}."`), a: T(`If it is not true that ${c.q.en}, then it is not true that ${c.p.en}.`, `Jika tidak benar bahawa ${c.q.ms}, maka tidak benar bahawa ${c.p.ms}.`), w: W(pqLine(c), R_CONTRA), sp: 's' };
    },
    (r) => {
      const c = r.pick(IMP);
  ...
  const gm32 = [
    (r) => {
      const c = r.pick(IMP);
      return { q: T(`For the implication "If ${c.p.en}, then ${c.q.en}", write (a) the converse, (b) the inverse, (c) the contrapositive, and state whether each is true or false.`, `Bagi implikasi "Jika ${c.p.ms}, maka ${c.q.ms}", tulis (a) akas, (b) songsang, (c) kontrapositif, dan nyatakan sama ada setiap satu benar atau palsu.`), a: T(`(a) If ${c.q.en}, then ${c.p.en}: ${c.cvTrue ? 'true' : `false (${c.cvCex.en})`}. (b) If ${notEn(c.p.en)}, then ${notEn(c.q.en)}: ${c.cvTrue ? 'true' : `false (same counterexample)`}. (c) If ${notEn(c.q.en)}, then ${notEn(c.p.en)}: ${c.ivTrue ? 'true (equivalent to the original)' : `false (${c.ifCex.en})`}.`, `(a) Jika ${c.q.ms}, maka ${c.p.ms}: ${c.cvTrue ? 'benar' : `palsu (${c.cvCex.ms})`}. (b) Jika ${notMs(c.p.ms)}, maka ${notMs(c.q.ms)}: ${c.cvTrue ? 'benar' : `palsu (contoh bertentang yang sama)`}. (c) Jika ${notMs(c.q.ms)}, maka ${notMs(c.p.ms)}: ${c.ivTrue ? 'benar (setara dengan yang asal)' : `palsu (${c.ifCex.ms})`}.`), w: W(pqLine(c), T(`(a) ${R_CONV.en} ${c.cvTrue ? 'It is true.' : `It is false: ${c.cvCex.en}.`}`, `(a) ${R_CONV.ms} ${c.cvTrue ? 'Ia benar.' : `Ia palsu: ${c.cvCex.ms}.`}`), T(`(b) ${R_INV.en} It always has the same truth value as the converse: ${tf(c.cvTrue)}.`, `(b) ${R_INV.ms} Ia sentiasa mempunyai nilai kebenaran yang sama dengan akas: ${tfm(c.cvTrue)}.`), T(`(c) ${R_CONTRA.en} It always has the same truth value as the original implication: ${tf(c.ivTrue)}.`, `(c) ${R_CONTRA.ms} Ia sentiasa mempunyai nilai kebenaran yang sama dengan implikasi asal: ${tfm(c.ivTrue)}.`)), sp: 'l' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`Premise 1: If ${c.p.en}, then ${c.q.en}. Premise 2: ${c.q.en}, which is true. Can you validly conclude that "${c.p.en}"? Explain.`, `Premis 1: Jika ${c.p.ms}, maka ${c.q.ms}. Premis 2: ${c.q.ms}, iaitu benar. Bolehkah anda menyimpulkan secara sah bahawa "${c.p.ms}"? Terangkan.`), a: T(`No. Knowing that $q$ is true does not guarantee $p$ is true (e.g. ${c.cvCex.en}).`, `Tidak. Mengetahui $q$ benar tidak menjamin $p$ benar (cth. ${c.cvCex.ms}).`), w: W(pqLine(c), AFF_Q, cexLine(c.cvCex)), sp: 'm' };
    },
    (r) => {
      const c = r.pick(IMP_T);
      return { q: T(`Premise 1: If ${c.p.en}, then ${c.q.en} (a true statement). Premise 2: ${c.p.en}. Conclusion: ${c.q.en}. Is this argument valid? Is it sound? Explain.`, `Premis 1: Jika ${c.p.ms}, maka ${c.q.ms} (satu pernyataan yang benar). Premis 2: ${c.p.ms}. Kesimpulan: ${c.q.ms}. Adakah hujah ini sah? Adakah ia munasabah (sound)? Terangkan.`), a: T('Valid: the conclusion follows the correct logical form. Sound: it is valid and premise 1 is true, so the conclusion is guaranteed true.', 'Sah: kesimpulan mengikut bentuk logik yang betul. Munasabah: ia sah dan premis 1 benar, jadi kesimpulan pasti benar.'), w: W(F_II, SOUND, T('Premise 1 is true and premise 2 is given as a fact, so the argument is valid and sound.', 'Premis 1 benar dan premis 2 diberi sebagai fakta, jadi hujah itu sah dan munasabah.')), sp: 'm' };
    },
    (r) => {
      const c = r.pick(IMP_F);
      return { q: T(`Premise 1: If ${c.p.en}, then ${c.q.en}. Premise 2: ${c.p.en}. Conclusion: ${c.q.en}. Is this argument valid? Is it sound? Explain.`, `Premis 1: Jika ${c.p.ms}, maka ${c.q.ms}. Premis 2: ${c.p.ms}. Kesimpulan: ${c.q.ms}. Adakah hujah ini sah? Adakah ia munasabah (sound)? Terangkan.`), a: T(`Valid: the conclusion follows the correct logical form. Not sound: premise 1 is actually false (${c.ifCex.en}), so a sound conclusion is not guaranteed.`, `Sah: kesimpulan mengikut bentuk logik yang betul. Tidak munasabah: premis 1 sebenarnya palsu (${c.ifCex.ms}), jadi kesimpulan yang munasabah tidak terjamin.`), w: W(F_II, T(`Premise 1 is false: ${c.ifCex.en}.`, `Premis 1 palsu: ${c.ifCex.ms}.`), SOUND), sp: 'm' };
    },
    (r) => {
      const c = r.pick(BIC);
      return { q: T(`State whether the biconditional "${c.p.en} if and only if ${c.q.en}" is true, by checking both directions.`, `Nyatakan sama ada dwibersyarat "${c.p.ms} jika dan hanya jika ${c.q.ms}" adalah benar, dengan menyemak kedua-dua arah.`), a: T(`True: "if ${c.p.en}, then ${c.q.en}" holds, and "if ${c.q.en}, then ${c.p.en}" also holds.`, `Benar: "jika ${c.p.ms}, maka ${c.q.ms}" adalah benar, dan "jika ${c.q.ms}, maka ${c.p.ms}" juga benar.`), w: W(pqLine(c), T('"$p$ if and only if $q$" is true only when both "if $p$, then $q$" and "if $q$, then $p$" are true.', '"$p$ jika dan hanya jika $q$" benar hanya apabila kedua-dua "jika $p$, maka $q$" dan "jika $q$, maka $p$" benar.'), T('Both implications are true here, so the biconditional is true.', 'Kedua-dua implikasi benar di sini, jadi dwibersyarat itu benar.')), sp: 'm' };
    },
    (r) => {
  ...
  const ga32 = [
    ...
    (r) => {
      ...
      return { q: T(`"${c.text.en}" Is this argument strong? Is its premise true? Is the argument cogent overall? Explain.`, `"${c.text.ms}" Adakah hujah ini kukuh? Adakah premisnya benar? Adakah hujah ini munasabah (cogent) secara keseluruhan? Terangkan.`), a: T(`Strong: ${c.strong ? 'yes' : 'no'}. Premise true: ${c.premiseTrue ? 'yes' : 'no'}. Cogent: ${cogent ? 'yes' : 'no'} — ${c.reason.en}`, `Kukuh: ${c.strong ? 'ya' : 'tidak'}. Premis benar: ${c.premiseTrue ? 'ya' : 'tidak'}. Munasabah (cogent): ${cogent ? 'ya' : 'tidak'} — ${c.reason.ms}`), w: W(T('An inductive argument is cogent only if it is strong and all its premises are true.', 'Hujah induktif adalah munasabah (cogent) hanya jika ia kukuh dan semua premisnya benar.'), T(`Strong: ${c.strong ? 'yes' : 'no'}; premise true: ${c.premiseTrue ? 'yes' : 'no'}; so cogent: ${cogent ? 'yes' : 'no'}.`, `Kukuh: ${c.strong ? 'ya' : 'tidak'}; premis benar: ${c.premiseTrue ? 'ya' : 'tidak'}; maka munasabah (cogent): ${cogent ? 'ya' : 'tidak'}.`)), sp: 'l' };
    },
    (r) => {
      const c = r.pick(CONJ);
