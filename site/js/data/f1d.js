...
(function () {
  ...
  const g113m = [
    (r) => {
      ...
      return { q: T(`Given $A = ${roster(A)}$, state whether each statement is true or false: ${four.map((s, i) => `(${'abcd'[i]}) $${s[0]}$`).join('  ')}`, `Diberi $A = ${roster(A)}$, nyatakan sama ada setiap pernyataan adalah benar atau palsu: ${four.map((s, i) => `(${'abcd'[i]}) $${s[0]}$`).join('  ')}`), a: T(four.map((s, i) => `(${'abcd'[i]}) ${s[1] ? 'True' : 'False'}`).join('  '), four.map((s, i) => `(${'abcd'[i]}) ${s[1] ? 'Benar' : 'Palsu'}`).join('  ')), w: W(...four.map((s, i) => T(`(${'abcd'[i]}) ${s[1] ? 'True' : 'False'} — ${s[2].en}`, `(${'abcd'[i]}) ${s[1] ? 'Benar' : 'Palsu'} — ${s[2].ms}`))), sp: 's' };
    },
    (r) => {
  ...
  const g113a = [
    ...
    (r) => {
      ...
      return { q: T('In the Venn diagram, $A \\subseteq B \\subseteq \\xi$. List the elements of $A$, $B$ and $B\'$, and state which of the following are true: (a) $A \\subseteq B$ (b) $B \\subseteq A$ (c) $A\' \\subseteq B\'$.', 'Dalam gambar rajah Venn, $A \\subseteq B \\subseteq \\xi$. Senaraikan unsur bagi $A$, $B$ dan $B\'$, dan nyatakan pernyataan yang benar: (a) $A \\subseteq B$ (b) $B \\subseteq A$ (c) $A\' \\subseteq B\'$.'), fig, a: T(`$A = ${roster(innerList)}$, $B = ${roster(outerList)}$, $B' = ${roster(out)}$. (a) True (b) False (c) False: $A'$ contains the elements of $B$ not in $A$, which are not in $B'$.`, `$A = ${roster(innerList)}$, $B = ${roster(outerList)}$, $B' = ${roster(out)}$. (a) Benar (b) Palsu (c) Palsu: $A'$ mengandungi unsur $B$ yang bukan dalam $A$, dan unsur ini tiada dalam $B'$.`), w: W(T(`Read the regions: inner circle $A = ${roster(innerList)}$; the whole big circle $B = ${roster(outerList)}$; outside $B$ gives $B' = ${roster(out)}$.`, `Baca kawasan: bulatan dalam $A = ${roster(innerList)}$; seluruh bulatan besar $B = ${roster(outerList)}$; di luar $B$ memberi $B' = ${roster(out)}$.`), T('(a) True: circle $A$ lies completely inside circle $B$.', '(a) Benar: bulatan $A$ terletak sepenuhnya di dalam bulatan $B$.'), T(`(b) False: $${mid[0]} \\in B$ but $${mid[0]} \\notin A$, so $B \\nsubseteq A$.`, `(b) Palsu: $${mid[0]} \\in B$ tetapi $${mid[0]} \\notin A$, jadi $B \\nsubseteq A$.`), T(`(c) False: $${mid[0]} \\in A'$ but $${mid[0]} \\in B$, so $${mid[0]} \\notin B'$.`, `(c) Palsu: $${mid[0]} \\in A'$ tetapi $${mid[0]} \\in B$, jadi $${mid[0]} \\notin B'$.`), T(`Taking complements reverses the direction: $A \\subseteq B$ gives $B' \\subseteq A'$.`, `Pelengkap membalikkan arah subset: $A \\subseteq B$ memberikan $B' \\subseteq A'$.`)), sp: 'm' };
    },
  ];
  ...
  const g132a = [
    (r) => {
      ...
      return { q: T(`A carpenter checks whether the corner of a roof frame with members ${list.join(' m, ')} m is a right angle. Does the converse of the Pythagoras theorem support this? Give the numerical evidence, and check the triangle inequality first.`, `Seorang tukang kayu menyemak sama ada bucu rangka bumbung dengan anggota ${list.join(' m, ')} m ialah sudut tegak. Adakah akas teorem Pythagoras menyokongnya? Berikan bukti berangka, dan semak ketaksamaan segi tiga dahulu.`), a: T(`${a} + ${b} = ${a + b} > ${c}, so a triangle exists. $${a}^2 + ${b}^2 = ${a * a + b * b}$ but $${c}^2 = ${c * c}$: not equal, so it is not a right angle.`, `${a} + ${b} = ${a + b} > ${c}, jadi segi tiga wujud. $${a}^2 + ${b}^2 = ${a * a + b * b}$ tetapi $${c}^2 = ${c * c}$: tidak sama, jadi ia bukan sudut tegak.`), w: W(T(`Triangle inequality first: $${a} + ${b} = ${a + b} > ${c}$, so the three members really do close up into a triangle.`, `Ketaksamaan segi tiga dahulu: $${a} + ${b} = ${a + b} > ${c}$, jadi ketiga-tiga anggota itu memang membentuk segi tiga.`), `$${a}^2 + ${b}^2 = ${a * a} + ${b * b} = ${a * a + b * b}$`, `$${c}^2 = ${c * c}$`, T('The two results are not equal, so the converse of the Pythagoras theorem does not apply: the corner is not a right angle.', 'Kedua-dua keputusan tidak sama, jadi akas teorem Pythagoras tidak terpakai: bucu itu bukan sudut tegak.')), sp: 'm' };
    },
    (r) => {
