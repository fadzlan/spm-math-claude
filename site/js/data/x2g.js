...
(function () {
  ...
  const ge122 = [
    ...
    (r) => {
      const t = r.pick(TF);
      return { q: T(`True or false? Give a reason.<br>"${t[0]}"`, `Betul atau salah? Berikan sebab.<br>"${t[1]}"`), a: T(`${t[2] ? 'True' : 'False'}. ${t[3]}`, `${t[2] ? 'Betul' : 'Salah'}. ${t[4]}`), w: W(T(t[2] ? 'Compare the statement with the definition of the measure.' : 'One counterexample is enough to show that a statement is false.', t[2] ? 'Bandingkan pernyataan itu dengan takrif ukuran berkenaan.' : 'Satu contoh penyangkal sudah cukup untuk menunjukkan sesuatu pernyataan itu salah.'), T(t[3], t[4]), T(t[2] ? 'So the statement is true.' : 'So the statement is false.', t[2] ? 'Maka pernyataan itu betul.' : 'Maka pernyataan itu salah.')), sp: 's' };
    },
    /* name the measure */
    (r) => {
  ...
  const ge123 = [
    ...
    (r) => {
      const t = r.pick(TF3);
      return { q: T(`True or false? Give a reason.<br>"${t[0]}"`, `Betul atau salah? Berikan sebab.<br>"${t[1]}"`), a: T(`${t[2] ? 'True' : 'False'}. ${t[3]}`, `${t[2] ? 'Betul' : 'Salah'}. ${t[4]}`), w: W(T(t[2] ? 'Check the statement against how a grouped frequency table is used.' : 'Check the statement against how a grouped frequency table is used — one counterexample makes it false.', t[2] ? 'Semak pernyataan itu dengan cara jadual kekerapan terkumpul digunakan.' : 'Semak pernyataan itu dengan cara jadual kekerapan terkumpul digunakan — satu contoh penyangkal menjadikannya salah.'), T(t[3], t[4]), T(t[2] ? 'So the statement is true.' : 'So the statement is false.', t[2] ? 'Maka pernyataan itu betul.' : 'Maka pernyataan itu salah.')), sp: 's' };
    },
  ...
  const ge124 = [
    ...
    (r) => {
      const t = r.pick(TF4);
      return { q: T(`True or false? Give a reason.<br>"${t[0]}"`, `Betul atau salah? Berikan sebab.<br>"${t[1]}"`), a: T(`${t[2] ? 'True' : 'False'}. ${t[3]}`, `${t[2] ? 'Betul' : 'Salah'}. ${t[4]}`), w: W(T(t[2] ? 'Check the statement against the meaning of range and of the measures of central tendency.' : 'Check the statement against the definitions — one counterexample is enough to make it false.', t[2] ? 'Semak pernyataan itu dengan maksud julat dan ukuran kecenderungan memusat.' : 'Semak pernyataan itu dengan takrifnya — satu contoh penyangkal sudah cukup untuk menjadikannya salah.'), T(t[3], t[4]), T(t[2] ? 'So the statement is true.' : 'So the statement is false.', t[2] ? 'Maka pernyataan itu betul.' : 'Maka pernyataan itu salah.')), sp: 's' };
    },
  ...
  const ge125 = [
    ...
    (r) => {
      ...
      return { q: T(`True or false? Give a reason.<br>"${t[0].split('|')[0]}"`, `Betul atau salah? Berikan sebab.<br>"${t[0].split('|')[1]}"`), a: T(`${t[1] ? 'True' : 'False'}. ${t[2].split('|')[0]}`, `${t[1] ? 'Betul' : 'Salah'}. ${t[2].split('|')[1]}`), w: W(T(t[1] ? 'Think about what happens to the total and to the number of values.' : 'Look for a case in which the statement fails — one counterexample is enough.', t[1] ? 'Fikirkan apa yang berlaku kepada jumlah dan bilangan nilai.' : 'Cari satu kes yang menyebabkan pernyataan itu gagal — satu contoh penyangkal sudah cukup.'), T(t[2].split('|')[0], t[2].split('|')[1]), T(t[1] ? 'So the statement is true.' : 'So the statement is false.', t[1] ? 'Maka pernyataan itu betul.' : 'Maka pernyataan itu salah.')), sp: 's' };
    },
