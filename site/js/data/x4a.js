...
(function () {
  ...
  const g11e5 = (r) => {
    ...
    return { q: T(`True or false: for $${Q(a, b, c)}$, ${label.en} is $${claim}$?`, `Betul atau salah: bagi $${Q(a, b, c)}$, ${label.ms} ialah $${claim}$?`), a: T(`False; ${label.en} is $${correct}$.`, `Salah; ${label.ms} ialah $${correct}$.`), w: coefLine(a, b, c), sp: 's' };
  };
  ...
  const g11m8 = (r) => {
    ...
    const s = kind ? s1 : s2;
    return { q: T(`True or false: "${s.en}" Give a reason.`, `Betul atau salah: "${s.ms}" Berikan sebab.`), a: kind ? T('True: a quadratic graph is symmetrical, so pairs of different $x$-values (equidistant from the axis of symmetry) share the same output.', 'Betul: graf kuadratik adalah simetri, jadi pasangan nilai $x$ berbeza (jarak sama dari paksi simetri) berkongsi output yang sama.') : T('False: two different inputs can give the same output (many-to-one), e.g. $f(x) = x^2$ has $f(-2) = f(2)$.', 'Salah: dua input berbeza boleh memberi output yang sama (banyak-kepada-satu), cth. $f(x) = x^2$ mempunyai $f(-2) = f(2)$.'), w: W(T('Take $f(x) = x^2$: $f(-2) = (-2)^2 = 4$ and $f(2) = 2^2 = 4$.', 'Ambil $f(x) = x^2$: $f(-2) = (-2)^2 = 4$ dan $f(2) = 2^2 = 4$.'), M2OW), sp: 's' };
  };
  ...
  const INTRO11TF = [
    (y, claim) => T(`True or false: for $${y}$, ${claim.en}?`, `Betul atau salah: bagi $${y}$, ${claim.ms}?`),
  ...
  const g11m20 = (r) => {
    ...
    return { q: s, a: T(`False; ${label.en} is $${correct}$.`, `Salah; ${label.ms} ialah $${correct}$.`), w: coefLine(a, b, c), sp: 's' };
  };
  const INTRO11CMP = [
  ...
  const TASKS_A = [
    ...
    ({ a1 }) => ({ en: `For $y = ${a1}x^2$, what must be true about the coefficient $a$ for the graph to open ${a1 > 0 ? 'upward' : 'downward'}?`, ms: `Bagi $y = ${a1}x^2$, apakah yang mesti betul tentang pekali $a$ supaya graf terbuka ${a1 > 0 ? 'ke atas' : 'ke bawah'}?`, aEn: `$a ${a1 > 0 ? '> 0' : '< 0'}$ (here $a = ${a1}$).`, aMs: `$a ${a1 > 0 ? '> 0' : '< 0'}$ (di sini $a = ${a1}$).`, w: T('A parabola opens upward when $a > 0$ and downward when $a < 0$.', 'Parabola terbuka ke atas apabila $a > 0$ dan ke bawah apabila $a < 0$.') }),
    ...
    ({ a1, a2, m1, m2 }) => ({ en: `True or false: the graph of $y = ${a1}x^2$ is wider than $y = ${a2}x^2$ when $|${a1}| ${m1 > m2 ? '>' : '<'} |${a2}|$?`, ms: `Betul atau salah: graf $y = ${a1}x^2$ lebih lebar daripada $y = ${a2}x^2$ apabila $|${a1}| ${m1 > m2 ? '>' : '<'} |${a2}|$?`, aEn: m1 > m2 ? `False; a larger $|a|$ gives a narrower (not wider) graph.` : `True; a smaller $|a|$ gives a wider graph.`, aMs: m1 > m2 ? `Salah; $|a|$ yang lebih besar memberikan graf yang lebih sempit (bukan lebih lebar).` : `Betul; $|a|$ yang lebih kecil memberikan graf yang lebih lebar.`, w: widthLine(a1, a2) }),
  ...
  const TASKS_B = [
    ...
    ({ a, b1, c }) => ({ en: `True or false: changing the value of $b$ in $y = ${Q(a, b1, c)}$ also changes the $y$-intercept?`, ms: `Betul atau salah: menukar nilai $b$ dalam $y = ${Q(a, b1, c)}$ turut menukar pintasan-$y$?`, aEn: `False; the $y$-intercept depends only on $c$ (here $c = ${c}$), not on $b$.`, aMs: `Salah; pintasan-$y$ hanya bergantung pada $c$ (di sini $c = ${c}$), bukan $b$.`, w: BYINTW(c) }),
  ...
  const INTRO_E5 = [
    ...
    (a1, a2) => T(`True or false: $y = ${a1}x^2$ and $y = ${a2}x^2$ open in opposite directions?`, `Betul atau salah: $y = ${a1}x^2$ dan $y = ${a2}x^2$ terbuka ke arah bertentangan?`),
  ];
  const g12e7 = (r) => {
  ...
  const INTRO_A8 = [
    (a, c1, c2) => T(`Three graphs share the equation form $y = ${a === 1 ? '' : a === -1 ? '-' : a}x^2 + c$. Two of them have $c = ${c1}$ and $c = ${c2}$. Without computing anything else, rank all statements true for every member of this family (shape, width, axis of symmetry) versus what varies.`, `Tiga graf berkongsi bentuk persamaan $y = ${a === 1 ? '' : a === -1 ? '-' : a}x^2 + c$. Dua daripadanya mempunyai $c = ${c1}$ dan $c = ${c2}$. Tanpa mengira apa-apa lagi, susun semua pernyataan yang betul bagi setiap ahli keluarga ini (bentuk, lebar, paksi simetri) berbanding apa yang berubah.`),
  ];
  const g12a8 = (r) => {
    const a = r.nz(-3, 3);
    const c1 = r.nz(-5, 5), c2 = r.nz(-5, 5);
  ...
  const g13e6 = (r) => {
    const p = r.nz(-5, 5), q = r.nz(-5, 5);
    need(p !== q);
    return { q: T(`True or false: the graph of $f(x) = ${fz(p)}${fz(q)}$ crosses the $x$-axis exactly twice?`, `Betul atau salah: graf $f(x) = ${fz(p)}${fz(q)}$ memotong paksi-$x$ tepat dua kali?`), a: T(`True; the two distinct roots $x = ${p}$ and $x = ${q}$ give two $x$-intercepts.`, `Betul; dua punca berbeza $x = ${p}$ dan $x = ${q}$ memberikan dua pintasan-$x$.`), w: W(...RL(p, q), T('Two different roots give two $x$-intercepts.', 'Dua punca berbeza memberi dua pintasan-$x$.')), sp: 's' };
  };
  ...
  const addW = (a, b, base) => {
    ...
    return [...cols.map((c, i) => {
      ...
      return T(`Column ${i + 1}${i ? '' : ' (units)'}: $${e}$, write $${c.digit}$${c.carryOut ? `, carry $${c.carryOut}$` : ''}`, `Tempat ke-${i + 1}${i ? '' : ' (sa)'}: $${e}$, tulis $${c.digit}$${c.carryOut ? `, bawa $${c.carryOut}$` : ''}`);
  ...
  const subW = (a, b, base) => {
    ...
    const lines = A.map((d, i) => {
      ...
      return T(`Column ${i + 1}${i ? '' : ' (units)'}: $${e}$${br ? ` (borrow $1$, worth $${base}$, from the next column)` : ''}`, `Tempat ke-${i + 1}${i ? '' : ' (sa)'}: $${e}$${br ? ` (pinjam $1$, bernilai $${base}$, daripada tempat seterusnya)` : ''}`);
    });
  ...
  const INTRO_ERR_E = [
    ...
    (v, b, wrong) => T(`True or false: $${nb(wrong, b)}$ equals $${v}$ in base ${b}?`, `Betul atau salah: $${nb(wrong, b)}$ bersamaan dengan $${v}$ dalam asas ${b}?`),
  ...
  const INTRO_CARRY_M = [
    (a, b, base) => T(`Add $${nb(a, base)}$ and $${nb(b, base)}$ in base ${base}, working column by column and stating the carry produced at the units column.`, `Tambahkan $${nb(a, base)}$ dan $${nb(b, base)}$ dalam asas ${base}, dengan bekerja tempat demi tempat dan menyatakan bawaan yang terhasil pada tempat sa.`),
    ...
    (a, b, base) => T(`In base ${base}, add $${nb(a, base)}$ and $${nb(b, base)}$. Begin at the units column and state what carries into the next column.`, `Dalam asas ${base}, tambahkan $${nb(a, base)}$ dan $${nb(b, base)}$. Mula pada tempat sa dan nyatakan apa yang dibawa ke tempat seterusnya.`),
    (a, b, base) => T(`Show the units-column working for $${nb(a, base)} + ${nb(b, base)}$ in base ${base}, including the carry.`, `Tunjukkan kerja tempat sa bagi $${nb(a, base)} + ${nb(b, base)}$ dalam asas ${base}, termasuk bawaan.`),
  ];
  const g21m14 = (r) => {
  ...
  const INTRO_ADD_A = [
    ...
    (expr, base) => T(`Present a column-by-column solution for $${expr}$ in base ${base}, annotating any carry or borrow.`, `Kemukakan penyelesaian tempat demi tempat bagi $${expr}$ dalam asas ${base}, dengan menandakan sebarang bawaan atau pinjaman.`),
  ];
  const g21a1 = (r) => {
    const base = r.pick([5, 6, 7, 8]);
  ...
  const INTRO_CARRY_A = [
    (a, b, base) => T(`Add $${nb(a, base)}$ and $${nb(b, base)}$ entirely within base ${base}, showing the carry produced at every column (there is more than one carry here).`, `Tambahkan $${nb(a, base)}$ dan $${nb(b, base)}$ sepenuhnya dalam asas ${base}, dengan menunjukkan bawaan yang terhasil pada setiap tempat (terdapat lebih daripada satu bawaan di sini).`),
    (a, b, base) => T(`Demonstrate, column by column with every carry justified, that $${nb(a, base)} + ${nb(b, base)}$ (base ${base}) gives the stated result.`, `Tunjukkan, tempat demi tempat dengan setiap bawaan diwajarkan, bahawa $${nb(a, base)} + ${nb(b, base)}$ (asas ${base}) memberikan hasil yang dinyatakan.`),
    (a, b, base) => T(`A junior student cannot see how carries chain across three columns in base ${base}. Using $${nb(a, base)} + ${nb(b, base)}$, explain fully.`, `Seorang pelajar junior tidak dapat melihat bagaimana bawaan berantai merentasi tiga tempat dalam asas ${base}. Menggunakan $${nb(a, base)} + ${nb(b, base)}$, terangkan sepenuhnya.`),
  ];
  const g21a9 = (r) => {
    const base = r.pick([5, 6, 7, 8]);
    ...
    const colNamesMs = ['tempat sa', 'tempat kedua (dari kanan)', 'tempat ketiga (dari kanan)'];
    ...
    return { q: s, a: T(`${traceEn}; result $= ${nb(result, base)}$.`, `${trace}; hasil $= ${nb(result, base)}$.`), w: W(T(`In base ${base}, when a column total reaches $${base}$ or more, write the remainder after dividing by $${base}$ and carry the quotient.`, `Dalam asas ${base}, apabila jumlah suatu tempat mencapai $${base}$ atau lebih, tulis baki selepas dibahagi dengan $${base}$ dan bawa hasil bahaginya.`), ...addW(a, b, base), T(`Check in base 10: $${fromBase(a, base)} + ${fromBase(b, base)} = ${fromBase(a, base) + fromBase(b, base)} = ${nb(result, base)}$`, `Semak dalam asas 10: $${fromBase(a, base)} + ${fromBase(b, base)} = ${fromBase(a, base) + fromBase(b, base)} = ${nb(result, base)}$`)), sp: 'l' };
  };
  const INTRO_CMPB_A = [
