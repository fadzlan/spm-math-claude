...
(function () {
  ...
  const g91e = [
    ...
    (r) => {
      ...
      return { q: T(`True or false: the two events described are independent. "${c[0].en}"`, `Betul atau salah: kedua-dua peristiwa yang diterangkan itu tak bersandar. "${c[0].ms}"`), a: c[1] ? T('True: the outcome of the first event does not affect the probabilities of the second.', 'Betul: keputusan peristiwa pertama tidak menjejaskan kebarangkalian peristiwa kedua.') : T('False: the composition available for the second event changes because of the first.', 'Salah: komposisi yang ada untuk peristiwa kedua berubah akibat peristiwa pertama.'),
  ...
  const g92a = [
    ...
    (r) => {
      const c = r.pick([
        [
          T('A student conjectures: "To find $P(A \\text{ and } B)$, always multiply $P(A)$ by $P(B)$." Give a counterexample and state the condition under which the conjecture is actually true.', 'Seorang murid membuat konjektur: "Untuk mencari $P(A \\text{ dan } B)$, sentiasa darabkan $P(A)$ dengan $P(B)$." Berikan satu contoh penyangkal dan nyatakan syarat di bawah konjektur itu sebenarnya betul.'),
          T('Counterexample: roll one die, $A = \\{1,2,3,4\\}$, $B = \\{3,4\\}$. $P(A) \\times P(B) = \\dfrac{2}{3} \\times \\dfrac{1}{3} = \\dfrac{2}{9}$, but $P(A \\cap B) = P(\\{3,4\\}) = \\dfrac{1}{3} \\neq \\dfrac{2}{9}$. The conjecture $P(A \\cap B) = P(A) \\times P(B)$ is true only when $A$ and $B$ are independent.', 'Contoh penyangkal: golekkan sebiji dadu, $A = \\{1,2,3,4\\}$, $B = \\{3,4\\}$: $P(A) \\times P(B) = \\dfrac{2}{3} \\times \\dfrac{1}{3} = \\dfrac{2}{9}$, tetapi $P(A \\cap B) = P(\\{3,4\\}) = \\dfrac{1}{3} \\neq \\dfrac{2}{9}$. Konjektur $P(A \\cap B) = P(A) \\times P(B)$ betul hanya apabila $A$ dan $B$ tak bersandar.'),
        ],
        [
          T('A student conjectures: "To find $P(A \\text{ or } B)$, always add $P(A)$ and $P(B)$." Give a counterexample and state the condition under which the conjecture is actually true.', 'Seorang murid membuat konjektur: "Untuk mencari $P(A \\text{ atau } B)$, sentiasa tambahkan $P(A)$ dengan $P(B)$." Berikan satu contoh penyangkal dan nyatakan syarat di bawah konjektur itu sebenarnya betul.'),
          T('Counterexample: roll one die, $A$: even $= \\{2,4,6\\}$, $B$: greater than 3 $= \\{4,5,6\\}$. $P(A) + P(B) = \\dfrac{1}{2} + \\dfrac{1}{2} = 1$, but $P(A \\cup B) = P(\\{2,4,5,6\\}) = \\dfrac{2}{3} \\neq 1$. The conjecture $P(A \\cup B) = P(A) + P(B)$ is true only when $A$ and $B$ are mutually exclusive.', 'Contoh penyangkal: golekkan sebiji dadu, $A$: genap $= \\{2,4,6\\}$, $B$: lebih besar daripada 3 $= \\{4,5,6\\}$. $P(A) + P(B) = \\dfrac{1}{2} + \\dfrac{1}{2} = 1$, tetapi $P(A \\cup B) = P(\\{2,4,5,6\\}) = \\dfrac{2}{3} \\neq 1$. Konjektur $P(A \\cup B) = P(A) + P(B)$ betul hanya apabila $A$ dan $B$ saling eksklusif.'),
        ],
      ]);
      return { q: c[0], a: c[1], w: W(T('$P(A \\cap B) = P(A) \\times P(B)$ is the condition for independence, and $P(A \\cup B) = P(A) + P(B)$ holds only when $P(A \\cap B) = 0$.', '$P(A \\cap B) = P(A) \\times P(B)$ ialah syarat ketidaksandaran, dan $P(A \\cup B) = P(A) + P(B)$ hanya betul apabila $P(A \\cap B) = 0$.'), T('So one pair of events that breaks the condition disproves "always".', 'Jadi satu pasangan peristiwa yang melanggar syarat itu menyangkal "sentiasa".')), sp: 'l' };
    },
