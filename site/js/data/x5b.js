...
(function () {
  ...
  const TERMS = [
    { term: T('premium', 'premium'), def: T('the amount the policyholder pays (usually every year) to keep the policy in force', 'jumlah yang dibayar oleh pemegang polisi (biasanya setiap tahun) untuk mengekalkan polisi berkuat kuasa') },
    { term: T('sum insured (face value)', 'jumlah diinsuranskan (nilai muka)'), def: T('the maximum amount the insurer will pay out under the policy', 'jumlah maksimum yang akan dibayar oleh penanggung insurans di bawah polisi itu') },
    { term: T('deductible (excess)', 'deduktibel (lebihan)'), def: T('the part of each claim that the policyholder must pay first, before the insurer pays the rest', 'bahagian setiap tuntutan yang mesti dibayar dahulu oleh pemegang polisi, sebelum penanggung insurans membayar selebihnya') },
    { term: T('co-insurance', 'ko-insurans'), def: T('a clause requiring the property to be insured for at least a stated percentage of its value, or the payout is reduced proportionally', 'klausa yang memerlukan harta diinsuranskan sekurang-kurangnya suatu peratusan tertentu daripada nilainya, jika tidak bayaran dikurangkan secara berkadar') },
    { term: T('indemnity principle', 'prinsip indemniti'), def: T('a payout must restore the policyholder to their financial position just before the loss, without creating a profit', 'bayaran mesti memulihkan kedudukan kewangan pemegang polisi seperti sebelum kerugian, tanpa mewujudkan keuntungan') },
    { term: T('risk pooling', 'pengumpulan risiko'), def: T('many policyholders pay premiums into a common fund, which is used to pay the claims of the few who suffer a loss', 'ramai pemegang polisi membayar premium ke dalam satu tabung bersama, yang digunakan untuk membayar tuntutan segelintir yang mengalami kerugian') },
    { term: T('policy limit', 'had polisi'), def: T('the maximum amount the insurer will pay for a claim or a type of claim', 'jumlah maksimum yang akan dibayar oleh penanggung insurans bagi sesuatu tuntutan atau jenis tuntutan') },
    { term: T('no-claim discount (NCD)', 'diskaun tanpa tuntutan (NCD)'), def: T('a reduction in premium given for each consecutive year that passes without the policyholder making a claim', 'pengurangan premium yang diberikan bagi setiap tahun berturut-turut yang berlalu tanpa pemegang polisi membuat tuntutan') },
    { term: T('beneficiary', 'benefisiari'), def: T('the person who receives the payout from a life insurance policy', 'orang yang menerima bayaran daripada sesuatu polisi insurans hayat') },
    { term: T('exclusion', 'pengecualian'), def: T('an event or a type of loss that the policy explicitly does not cover', 'peristiwa atau jenis kerugian yang secara jelas tidak dilindungi oleh polisi') },
  ];
  ...
  const g31e = [
    (r) => {
      ...
      return { q: T(w[0], w[1]), a: c.sub, sp: 's' };
    },
    (r) => {
      const c = r.pick(INS_SCEN);
      return { q: T(`Is ${c.sub.en} classified as life insurance or general insurance?`, `Adakah ${c.sub.ms} dikelaskan sebagai insurans hayat atau insurans am?`), a: c.type === 'life' ? T('Life insurance', 'Insurans hayat') : T('General insurance', 'Insurans am'), sp: 'xs' };
    },
    (r) => {
      ...
      return { q: T(w[0], w[1]), a: t.def, sp: 's' };
    },
    (r) => {
      ...
      const L = 'ABCD';
      return { q: T(`Which term matches this description? "${correct.def.en}"<br>${opts.map((o, i) => `(${L[i]}) ${o.term.en}`).join('  ')}`, `Istilah manakah yang sepadan dengan huraian ini? "${correct.def.ms}"<br>${opts.map((o, i) => `(${L[i]}) ${o.term.ms}`).join('  ')}`), a: T(`${L[ci]}: ${correct.term.en}`, `${L[ci]}: ${correct.term.ms}`), sp: 's' };
    },
    (r) => {
      const f = r.pick(PREMIUM_FACTORS);
      return { q: T(`Would ${f.en} usually increase or decrease an insurance premium? Give a reason.`, `Adakah ${f.ms} biasanya meningkatkan atau menurunkan premium insurans? Berikan sebab.`), a: T(`${f.dir === 'higher' ? 'Increase' : 'Decrease'}: ${f.reason.en}.`, `${f.dir === 'higher' ? 'Meningkatkan' : 'Menurunkan'}: ${f.reason.ms}.`), sp: 's' };
    },
    (r) => {
      const s = r.pick(INS_TF);
      return { q: T(`True or false? "${s.en}" Give a reason.`, `Betul atau salah? "${s.ms}" Berikan sebab.`), a: T(`${s.ok ? 'True' : 'False'}: ${s.why.en}.`, `${s.ok ? 'Betul' : 'Salah'}: ${s.why.ms}.`), sp: 's' };
    },
  ];
  const g31m = [
    (r) => {
    ...
    (r) => {
      ...
      const y = r.int(1, 5);
      return { q: T(`${lab.en}<br>${tab}<br>A policyholder has had ${y} consecutive claim-free year${y > 1 ? 's' : ''}. What NCD rate applies?`, `${lab.ms}<br>${tabMs}<br>Seorang pemegang polisi mempunyai ${y} tahun berturut-turut tanpa tuntutan. Apakah kadar NCD yang terpakai?`), a: T(`${rows[y]}%`), sp: 'm' };
    },
    (r) => {
      const rows = ncdLadder(r), ctx = r.pick(NCD_CTX);
      const tab = ncdTable(rows, 'en'), tabMs = ncdTable(rows, 'ms'), lab = ncdLabel(ctx);
  ...
  const TAX_TERMS = [
    { term: T('tax relief', 'pelepasan cukai'), def: T('an amount subtracted from taxable income (subject to a supplied cap) before chargeable income is worked out', 'jumlah yang ditolak daripada pendapatan bercukai (tertakluk kepada had yang diberikan) sebelum pendapatan bercukai dikira') },
    { term: T('tax rebate', 'rebat cukai'), def: T('an amount subtracted from the tax already calculated from the bands, not from income', 'jumlah yang ditolak daripada cukai yang telah dikira daripada jadual kadar, bukan daripada pendapatan') },
    { term: T('chargeable income', 'pendapatan bercukai'), def: T('taxable income after permitted exemptions and reliefs have been subtracted', 'pendapatan boleh cukai selepas pengecualian dan pelepasan yang dibenarkan ditolak') },
    { term: T('PCB (monthly tax deduction)', 'PCB (potongan cukai bulanan)'), def: T('tax already withheld from salary each month; it is compared with the final tax payable to find a refund or a balance due, and is neither a relief nor a rebate', 'cukai yang telah dipotong daripada gaji setiap bulan; ia dibandingkan dengan cukai muktamad yang perlu dibayar untuk mencari bayaran balik atau baki tertunggak, dan bukan pelepasan atau rebat') },
    { term: T('tax avoidance', 'pengelakan cukai secara sah'), def: T('legally arranging one\'s affairs to reduce tax, using reliefs and rules that are allowed', 'menyusun hal ehwal seseorang secara sah untuk mengurangkan cukai, menggunakan pelepasan dan peraturan yang dibenarkan') },
    { term: T('tax evasion', 'pengelakan cukai secara haram'), def: T('illegally hiding income or falsifying records to avoid paying tax that is legally due', 'menyembunyikan pendapatan atau memalsukan rekod secara menyalahi undang-undang untuk mengelak membayar cukai yang perlu dibayar mengikut undang-undang') },
  ];
  ...
  const g41e2 = [
    (r) => {
      ...
      return { q: T(w[0], w[1]), a: t.dir ? T('Direct tax', 'Cukai langsung') : T('Indirect tax', 'Cukai tidak langsung'), sp: 'xs' };
    },
    (r) => {
      const a = r.pick(DIRECT_TAXES), b = r.pick(INDIRECT_TAXES);
      const swap = r.chance();
      const pair = swap ? [b, a] : [a, b];
      return { q: T(`Which of these is a direct tax: ${pair[0].en} or ${pair[1].en}?`, `Yang manakah cukai langsung: ${pair[0].ms} atau ${pair[1].ms}?`), a: T(a.en, a.ms), sp: 'xs' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`A restaurant bill is RM${bill} before tax. SST of ${rate}% is charged. Find the SST and the total amount payable.`, `Bil sebuah restoran ialah RM${bill} sebelum cukai. SST sebanyak ${rate}% dikenakan. Cari SST dan jumlah yang perlu dibayar.`), a: T(`SST ${rm((bill * rate) / 100)}; total ${rm(bill * (1 + rate / 100))}`, `SST ${rm((bill * rate) / 100)}; jumlah ${rm(bill * (1 + rate / 100))}`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      const cc = thr;
      return { q: T(`A simplified road-tax schedule (for this question, use the schedule given) charges a flat RM${base} for vehicles up to ${n(thr)} cc. Find the road tax for a car with an engine capacity of ${n(cc)} cc.`, `Sebuah jadual cukai jalan ringkas (untuk soalan ini, gunakan jadual yang diberikan) mengenakan RM${base} rata bagi kenderaan sehingga ${n(thr)} cc. Cari cukai jalan bagi sebuah kereta yang berkuasa enjin ${n(cc)} cc.`), a: T(`RM${base}`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`A shopping list is shown; SST at ${rate}% applies only to the items marked "Yes".<br>${tab}<br>Find the total amount payable, including SST.`, `Senarai belian ditunjukkan; SST sebanyak ${rate}% hanya dikenakan ke atas barang yang ditandakan "Ya".<br>${tabMs}<br>Cari jumlah keseluruhan yang perlu dibayar, termasuk SST.`), a: T(rm(round(total, 2), 2)), sp: 's' };
    },
    (r) => {
      const t = r.pick(DIRECT_TAXES);
      return { q: T(`Name one direct tax other than ${t.en} that a Malaysian household might pay.`, `Namakan satu cukai langsung selain ${t.ms} yang mungkin dibayar oleh sebuah isi rumah di Malaysia.`), a: T(DIRECT_TAXES.filter((x) => x.en !== t.en).map((x) => x.en).join(' / '), DIRECT_TAXES.filter((x) => x.en !== t.en).map((x) => x.ms).join(' / ')), sp: 'xs' };
    },
  ];
  const g41m2 = [
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`${bandLabel.en}<br>${tab}<br>Find the tax payable on a chargeable income of ${rm(chargeable)}.`, `${bandLabel.ms}<br>${tabMs}<br>Cari cukai yang perlu dibayar bagi pendapatan bercukai ${rm(chargeable)}.`), a: T(`${rows.map((rw) => `${n(rw[0])} × ${rw[1]}%`).join(' + ')} = ${rm(round(tax, 2), 2)}`, `${rows.map((rw) => `${n(rw[0])} × ${rw[1]}%`).join(' + ')} = ${rm(round(tax, 2), 2)}`), sp: 'm' };
    },
    (r) => {
      ...
      return { q: T(`${roadLabel.en}<br>${roadTable(sch, 'en')}<br>Find the road tax for a ${veh.en} with an engine capacity of ${n(cc)} cc.`, `${roadLabel.ms}<br>${roadTable(sch, 'ms')}<br>Cari cukai jalan bagi sebuah ${veh.ms} yang berkuasa enjin ${n(cc)} cc.`), a: T(rm(round(tax, 2), 2)), sp: 'm' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`A shopping list is shown; SST at ${rate}% applies only to the items marked "Yes".<br>${tab}<br>Find (a) the total SST charged, (b) the total amount payable.`, `Senarai belian ditunjukkan; SST sebanyak ${rate}% hanya dikenakan ke atas barang yang ditandakan "Ya".<br>${tabMs}<br>Cari (a) jumlah SST yang dikenakan, (b) jumlah keseluruhan yang perlu dibayar.`), a: T(`(a) ${rm(round(sstOnly, 2), 2)} (b) ${rm(round(total, 2), 2)}`, `(a) ${rm(round(sstOnly, 2), 2)} (b) ${rm(round(total, 2), 2)}`), sp: 'm' };
    },
    (r) => {
      const sch = roadTaxSchedule(r);
    ...
    (r) => {
      ...
      const rows = rel.map((x, i) => [x.en, rm(x.cap), rm(claimed[i])]);
      const rowsMs = rel.map((x, i) => [x.ms, rm(x.cap), rm(claimed[i])]);
      return { q: T(`A taxpayer's claimed reliefs are shown, each capped as stated.<br>${SPM.table(rows, { head: ['Relief', 'Cap', 'Claimed'] })}<br>Which relief's claim exceeds its cap, and what amount is actually allowed for it?`, `Pelepasan yang dituntut oleh seorang pembayar cukai ditunjukkan, setiap satu dihadkan seperti dinyatakan.<br>${SPM.table(rowsMs, { head: ['Pelepasan', 'Had', 'Dituntut'] })}<br>Pelepasan manakah yang tuntutannya melebihi hadnya, dan berapakah jumlah yang sebenarnya dibenarkan baginya?`), a: T(`${rel[over].en}: allowed amount is ${rm(rel[over].cap)} (capped).`, `${rel[over].ms}: jumlah yang dibenarkan ialah ${rm(rel[over].cap)} (dihadkan).`), sp: 'm' };
    },
    (r) => {
      const bill = r.pick([150, 250, 400]), rate = r.pick([6, 8, 10]);
      return { q: T(`(a) Classify SST as a direct tax or an indirect tax. (b) A bill of ${rm(bill)} (before tax) is fully subject to SST at ${rate}%. Find the total amount payable.`, `(a) Kelaskan SST sebagai cukai langsung atau cukai tidak langsung. (b) Sebuah bil berjumlah ${rm(bill)} (sebelum cukai) dikenakan SST sepenuhnya pada ${rate}%. Cari jumlah yang perlu dibayar.`), a: T(`(a) Indirect tax (b) ${rm(bill * (1 + rate / 100))}`, `(a) Cukai tidak langsung (b) ${rm(bill * (1 + rate / 100))}`), sp: 'm' };
    },
  ];
  const g41a2 = [
    (r) => {
    ...
    (r) => {
      ...
      const tab = bandTableSimple(bands, 'en'), tabMs = bandTableSimple(bands, 'ms');
      return { q: T(`${bandLabel.en}<br>${tab}<br>Tax payable on a chargeable income of ${rm(chargeable)} is ${rm(payable, 2)}. Show this is consistent with the schedule, then find the chargeable income that gives a tax payable of exactly ${rm(round(baseTaxAtThreshold + ((excess / 2) * rate) / 100, 2), 2)}.`, `${bandLabel.ms}<br>${tabMs}<br>Cukai yang perlu dibayar bagi pendapatan bercukai ${rm(chargeable)} ialah ${rm(payable, 2)}. Tunjukkan ini konsisten dengan jadual, kemudian cari pendapatan bercukai yang memberikan cukai yang perlu dibayar tepat ${rm(round(baseTaxAtThreshold + ((excess / 2) * rate) / 100, 2), 2)}.`), a: T(`$${rm(baseTaxAtThreshold, 2)} + ${excess} \\times ${rate}\\% = ${rm(payable, 2)}$, consistent. Chargeable income for the smaller tax $= ${rm(thresholdLow + excess / 2)}$.`, `$${rm(baseTaxAtThreshold, 2)} + ${excess} \\times ${rate}\\% = ${rm(payable, 2)}$, konsisten. Pendapatan bercukai bagi cukai yang lebih kecil $= ${rm(thresholdLow + excess / 2)}$.`), sp: 'l' };
    },
    (r) => {
  ...
  const EVASION_SCEN = [
    { en: 'A shop owner keeps two sets of accounts, showing lower sales to the tax authority than actually made.', ms: 'Seorang pemilik kedai menyimpan dua set akaun, menunjukkan jualan yang lebih rendah kepada pihak berkuasa cukai berbanding jualan sebenar.', ok: true },
    { en: 'A freelancer does not declare cash payments received for work done.', ms: 'Seorang pekerja bebas tidak mengisytiharkan bayaran tunai yang diterima bagi kerja yang dilakukan.', ok: true },
    { en: 'A company creates fake invoices to claim expenses that were never actually incurred.', ms: 'Sebuah syarikat mencipta invois palsu untuk menuntut perbelanjaan yang sebenarnya tidak pernah dilakukan.', ok: true },
    { en: 'A taxpayer claims a relief that he is legitimately entitled to, keeping the receipts as proof.', ms: 'Seorang pembayar cukai menuntut pelepasan yang layak dituntutnya secara sah, dengan menyimpan resit sebagai bukti.', ok: false },
    { en: 'An employee accidentally forgets to declare a small amount of bank interest and corrects it as soon as she is reminded.', ms: 'Seorang pekerja terlupa mengisytiharkan sedikit faedah bank secara tidak sengaja dan membetulkannya sebaik sahaja diingatkan.', ok: false },
    { en: 'A business owner restructures a large purchase, within the rules, to make use of a legal tax relief.', ms: 'Seorang pemilik perniagaan menyusun semula satu pembelian besar, mengikut peraturan yang dibenarkan, untuk memanfaatkan pelepasan cukai yang sah.', ok: false },
    { en: 'A landlord under-reports rental income received in cash to reduce the tax owed.', ms: 'Seorang tuan rumah melapor kurang pendapatan sewa yang diterima secara tunai untuk mengurangkan cukai yang perlu dibayar.', ok: true },
    { en: 'An online seller does not declare any of the income earned from selling goods on a social media page.', ms: 'Seorang penjual dalam talian tidak mengisytiharkan langsung pendapatan yang diperoleh daripada menjual barangan di sebuah laman media sosial.', ok: true },
    { en: 'A worker who is paid in cash for odd jobs keeps no records at all and never files a tax return, hoping not to be noticed.', ms: 'Seorang pekerja yang dibayar secara tunai untuk kerja sambilan tidak menyimpan sebarang rekod dan tidak pernah memfailkan penyata cukai, berharap tidak disedari.', ok: true },
    { en: 'A company mixes the director\'s personal holiday expenses into the business accounts and claims them as business expenses to lower taxable profit.', ms: 'Sebuah syarikat mencampurkan perbelanjaan percutian peribadi pengarah ke dalam akaun perniagaan dan menuntutnya sebagai perbelanjaan perniagaan untuk mengurangkan keuntungan bercukai.', ok: true },
    { en: 'A taxpayer keeps proper receipts and claims exactly the education fees relief that the rules allow, no more and no less.', ms: 'Seorang pembayar cukai menyimpan resit yang lengkap dan menuntut tepat pelepasan yuran pendidikan yang dibenarkan oleh peraturan, tidak lebih dan tidak kurang.', ok: false },
    { en: 'A business delays a large purchase to the following tax year, within the rules, so that the relief is claimed when it is most useful.', ms: 'Sebuah perniagaan menangguhkan sesuatu pembelian besar ke tahun cukai berikutnya, mengikut peraturan yang dibenarkan, supaya pelepasan itu dituntut pada masa yang paling berfaedah.', ok: false },
    { en: 'A freelancer under-declares part of her income after being told by a friend that "small amounts are never checked".', ms: 'Seorang pekerja bebas melapor kurang sebahagian pendapatannya selepas diberitahu oleh seorang rakan bahawa "jumlah kecil tidak pernah disemak".', ok: true },
    { en: 'A taxpayer notices an error on last year\'s tax return that understated her income, and voluntarily informs the tax authority and pays the difference.', ms: 'Seorang pembayar cukai menyedari satu kesilapan pada penyata cukai tahun lepas yang menyatakan pendapatannya terlalu rendah, dan secara sukarela memaklumkan pihak berkuasa cukai serta membayar bakinya.', ok: false },
  ];
  const CONSEQ_BANK = [
  ...
  const DISP_TERMS = [
    { term: T('class boundary', 'sempadan kelas'), def: T('the value exactly halfway between the upper limit of one class and the lower limit of the next, used so histogram bars have no gaps', 'nilai yang tepat di antara had atas sesuatu kelas dengan had bawah kelas berikutnya, supaya palang histogram tiada jurang') },
    { term: T('class size (class width)', 'saiz kelas (lebar kelas)'), def: T('the difference between the upper and lower boundaries of a class', 'perbezaan antara sempadan atas dengan sempadan bawah sesuatu kelas') },
    ...
    { term: T('variance', 'varians'), def: T('the mean of the squared deviations from the mean; it equals the standard deviation squared', 'min sisihan kuasa dua daripada min; ia bersamaan dengan kuasa dua sisihan piawai') },
    { term: T('standard deviation', 'sisihan piawai'), def: T('a measure of dispersion equal to the square root of the variance; a smaller value means the data is more consistent', 'sukatan serakan yang bersamaan dengan punca kuasa dua varians; nilai yang lebih kecil bermaksud data lebih konsisten') },
    { term: T('modal class', 'kelas modal'), def: T('the class interval with the highest frequency', 'selang kelas yang mempunyai kekerapan tertinggi') },
    { term: T('estimated mean', 'min anggaran'), def: T('an approximate mean of grouped data, calculated from class midpoints since the exact original values are not known', 'min anggaran data terkumpul, dikira daripada titik tengah kelas kerana nilai asal yang tepat tidak diketahui') },
  ];
  ...
  const g71e2 = [
    (r) => {
      ...
      if (ask === 'bnd') return { q: T(`For the class interval ${cl[i][0]}–${cl[i][1]}, state the lower boundary and the upper boundary.`, `Bagi selang kelas ${cl[i][0]}–${cl[i][1]}, nyatakan sempadan bawah dan sempadan atas.`), a: T(`${n(lo)} and ${n(hi)}`, `${n(lo)} dan ${n(hi)}`), sp: 's' };
      if (ask === 'size') return { q: T(`Find the class size of the class interval ${cl[i][0]}–${cl[i][1]}.`, `Cari saiz kelas bagi selang kelas ${cl[i][0]}–${cl[i][1]}.`), a: T(n(w)), sp: 'xs' };
      if (ask === 'mid') return { q: T(`Find the midpoint of the class interval ${cl[i][0]}–${cl[i][1]}.`, `Cari titik tengah bagi selang kelas ${cl[i][0]}–${cl[i][1]}.`), a: T(n(mid(cl[i]))), sp: 'xs' };
      return { q: T(`For the class interval ${cl[i][0]}–${cl[i][1]}, state the lower and upper boundaries, the class size and the midpoint.`, `Bagi selang kelas ${cl[i][0]}–${cl[i][1]}, nyatakan sempadan bawah dan atas, saiz kelas dan titik tengah.`), a: T(`Boundaries ${n(lo)} and ${n(hi)}; class size ${w}; midpoint ${n(mid(cl[i]))}`, `Sempadan ${n(lo)} dan ${n(hi)}; saiz kelas ${w}; titik tengah ${n(mid(cl[i]))}`), sp: 's' };
    },
    (r) => {
      ...
      return { q: T(w[0], w[1]), a: T(`Mean $${f2(m)}$; s.d. $\\sqrt{\\dfrac{${sxx}}{${N}} - ${f2(m)}^2} = ${f2(Math.sqrt(v))}$`, `Min $${f2(m)}$; s.p. $\\sqrt{\\dfrac{${sxx}}{${N}} - ${f2(m)}^2} = ${f2(Math.sqrt(v))}$`), sp: 's' };
    },
    (r) => {
      ...
      return { q: T(`When calculating the estimated mean of grouped data using $\\bar{x} = \\dfrac{\\sum fx}{\\sum f}$, which value of $x$ should be used for each class?`, `Apabila mengira min anggaran data terkumpul menggunakan $\\bar{x} = \\dfrac{\\sum fx}{\\sum f}$, nilai $x$ manakah yang perlu digunakan bagi setiap kelas?`), a: T(opts[0].en, opts[0].ms), sp: 'xs' };
    },
    (r) => {
      ...
      return { q: T(`True or false? The variance of grouped data is given by ${stmt.en}.`, `Betul atau salah? Varians data terkumpul diberi oleh ${stmt.ms}.`), a: claim ? T('False: the terms are the other way round; $\\sigma^2 = \\dfrac{\\sum fx^2}{\\sum f} - \\bar{x}^2$.', 'Salah: sebutan itu tertukar; $\\sigma^2 = \\dfrac{\\sum fx^2}{\\sum f} - \\bar{x}^2$.') : T('True.', 'Betul.'), sp: 's' };
    },
    (r) => {
      const cl = cls(r.pick([0, 10]), 10, 4), fs = cl.map(() => r.int(2, 10));
      const st = gStats(cl, fs);
      const modal = fs.indexOf(Math.max(...fs));
      return { q: T(`The table shows a grouped frequency distribution.<br>${gt(cl, fs, ['Class', 'Frequency'])}<br>State the modal class.`, `Jadual menunjukkan taburan kekerapan terkumpul.<br>${gt(cl, fs, ['Kelas', 'Kekerapan'])}<br>Nyatakan kelas modal.`), a: T(`${cl[modal][0]}–${cl[modal][1]}`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      const stmts = [
        { en: 'Standard deviation can never be negative.', ms: 'Sisihan piawai tidak boleh menjadi negatif.', ok: true },
        { en: 'A larger standard deviation means the data is more spread out.', ms: 'Sisihan piawai yang lebih besar bermaksud data lebih terserak.', ok: true },
        { en: 'To find the estimated mean of grouped data, use the class limits instead of the midpoints.', ms: 'Untuk mencari min anggaran data terkumpul, gunakan had kelas dan bukannya titik tengah.', ok: false },
        { en: 'Variance is the square root of the standard deviation.', ms: 'Varians ialah punca kuasa dua sisihan piawai.', ok: false },
        { en: 'The modal class is the class with the highest frequency, not the class containing the mean.', ms: 'Kelas modal ialah kelas yang mempunyai kekerapan tertinggi, bukan kelas yang mengandungi min.', ok: true },
      ];
      const s = r.pick(stmts);
      return { q: T(`True or false? "${s.en}"`, `Betul atau salah? "${s.ms}"`), a: T(s.ok ? 'True' : 'False', s.ok ? 'Betul' : 'Salah'), sp: 'xs' };
    },
  ];
  const g71m2 = [
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`The table shows a grouped distribution with total frequency ${N}.<br>${gt(cl, shown, ['Class', 'Frequency'])}<br>Find the value of $p$.`, `Jadual menunjukkan taburan terkumpul dengan jumlah kekerapan ${N}.<br>${gt(cl, shown, ['Kelas', 'Kekerapan'])}<br>Cari nilai $p$.`), a: T(`$p = ${p}$`), w: T(`$${known.filter((_, i) => i !== 2).join(' + ')} + p = ${N}$`, `$${known.filter((_, i) => i !== 2).join(' + ')} + p = ${N}$`), sp: 'm' };
    },
    (r) => {
  ...
  const OGIVE_TERMS = [
    { term: T('ogive (cumulative frequency curve)', 'ogif (lengkung kekerapan longgokan)'), def: T('a curve plotted at the upper boundary of each class against its cumulative frequency, starting from 0 at the lower boundary of the first class', 'lengkung yang diplot pada sempadan atas setiap kelas melawan kekerapan longgokannya, bermula daripada 0 pada sempadan bawah kelas pertama') },
    { term: T('interquartile range (IQR)', 'julat antara kuartil (JAK)'), def: T('the difference $Q_3 - Q_1$, describing the spread of the middle half of the data', 'perbezaan $Q_3 - Q_1$, menghuraikan serakan separuh tengah data') },
    { term: T('median (grouped data)', 'median (data terkumpul)'), def: T('the value at position $\\frac{n}{2}$ of the ordered data, read from the ogive', 'nilai pada kedudukan $\\frac{n}{2}$ data yang tersusun, dibaca daripada ogif') },
    { term: T('first quartile $Q_1$', 'kuartil pertama $Q_1$'), def: T('the value at position $\\frac{n}{4}$ of the ordered data, read from the ogive', 'nilai pada kedudukan $\\frac{n}{4}$ data yang tersusun, dibaca daripada ogif') },
    { term: T('third quartile $Q_3$', 'kuartil ketiga $Q_3$'), def: T('the value at position $\\frac{3n}{4}$ of the ordered data, read from the ogive', 'nilai pada kedudukan $\\frac{3n}{4}$ data yang tersusun, dibaca daripada ogif') },
    { term: T('five-number summary', 'ringkasan lima nombor'), def: T('the minimum, $Q_1$, median, $Q_3$ and maximum of a data set, used to draw a box plot', 'minimum, $Q_1$, median, $Q_3$ dan maksimum sesuatu set data, digunakan untuk melukis plot kotak') },
  ];
  const g72e2 = [
    (r) => {
      const five = [r.int(5, 15), r.int(20, 30), r.int(35, 45), r.int(50, 60), r.int(65, 80)].sort((a, b) => a - b);
      const which = r.pick(['iqr', 'range', 'order']);
      if (which === 'iqr') return { q: T(`A box plot has $Q_1 = ${five[1]}$ and $Q_3 = ${five[3]}$. Find the interquartile range.`, `Satu plot kotak mempunyai $Q_1 = ${five[1]}$ dan $Q_3 = ${five[3]}$. Cari julat antara kuartil.`), a: T(n(five[3] - five[1])), sp: 'xs' };
      if (which === 'range') return { q: T(`A box plot has minimum ${five[0]} and maximum ${five[4]}. Find the range of the data.`, `Satu plot kotak mempunyai minimum ${five[0]} dan maksimum ${five[4]}. Cari julat data itu.`), a: T(n(five[4] - five[0])), sp: 'xs' };
      return { q: T(`True or false? In a box plot, the five values always satisfy minimum $\\le Q_1 \\le$ median $\\le Q_3 \\le$ maximum.`, `Betul atau salah? Dalam plot kotak, kelima-lima nilai sentiasa memenuhi minimum $\\le Q_1 \\le$ median $\\le Q_3 \\le$ maksimum.`), a: T('True.', 'Betul.'), sp: 'xs' };
    },
    (r) => {
      const axis = r.pick(['x', 'y']);
      if (axis === 'x') return { q: T('On an ogive, what is plotted on the horizontal axis?', 'Pada suatu ogif, apakah yang diplot pada paksi mendatar?'), a: T('The upper class boundaries.', 'Sempadan atas kelas.'), sp: 'xs' };
      return { q: T('On an ogive, what is plotted on the vertical axis?', 'Pada suatu ogif, apakah yang diplot pada paksi mencancang?'), a: T('The cumulative frequency.', 'Kekerapan longgokan.'), sp: 'xs' };
    },
    (r) => {
      const N = r.pick([30, 40, 50, 60, 80]);
      const cfAt = r.int(2, N - 2);
      const half = N / 2;
      return { q: T(`On an ogive with $n = ${N}$ items, a value $k$ has a cumulative frequency of ${cfAt} up to and including $k$. Is $k$ below, at, or above the median position?`, `Pada suatu ogif dengan $n = ${N}$ item, suatu nilai $k$ mempunyai kekerapan longgokan ${cfAt} sehingga dan termasuk $k$. Adakah $k$ di bawah, pada, atau di atas kedudukan median?`), a: T(cfAt < half ? 'Below the median position' : cfAt > half ? 'Above the median position' : 'At the median position', cfAt < half ? 'Di bawah kedudukan median' : cfAt > half ? 'Di atas kedudukan median' : 'Pada kedudukan median'), sp: 'xs' };
    },
    (r) => {
      const min_ = r.int(5, 20), max_ = min_ + r.int(40, 70), med = r.int(min_ + 10, max_ - 10);
      return { q: T(`A box plot shows minimum ${min_}, median ${med} and maximum ${max_}. Is the distribution's upper half or lower half wider (a first look at skewness)?`, `Satu plot kotak menunjukkan minimum ${min_}, median ${med} dan maksimum ${max_}. Adakah separuh atas atau separuh bawah taburan itu lebih lebar (tinjauan awal kepencongan)?`), a: T(max_ - med > med - min_ ? 'The upper half (median to maximum) is wider.' : 'The lower half (minimum to median) is wider.', max_ - med > med - min_ ? 'Separuh atas (median ke maksimum) lebih lebar.' : 'Separuh bawah (minimum ke median) lebih lebar.'), sp: 'xs' };
    },
    (r) => {
      const N = r.pick([30, 40, 50, 60, 80]);
      return { q: T(`Write the formula, in terms of $n$, for the position of the median item and the position of $Q_3$ in an ordered data set of $n = ${N}$ items.`, `Tulis formula, dalam sebutan $n$, bagi kedudukan item median dan kedudukan $Q_3$ dalam set data tersusun dengan $n = ${N}$ item.`), a: T(`Median: $\\dfrac{n}{2} = ${N / 2}$th item; $Q_3$: $\\dfrac{3n}{4} = ${(3 * N) / 4}$th item`, `Median: item ke-$\\dfrac{n}{2} = ${N / 2}$; $Q_3$: item ke-$\\dfrac{3n}{4} = ${(3 * N) / 4}$`), sp: 's' };
    },
    (r) => {
      const cl = cls(r.pick([10, 20]), 10, 5), fs = cl.map(() => r.int(2, 12));
      const cf = cfOf(fs);
      const ctx = r.pick(STAT_CTX);
      return { q: T(`The table shows ${ctx.en}.<br>${gt(cl, fs, ['Class', 'Frequency', 'Upper boundary', 'Cumulative frequency'], cl.map((c) => [c[1] + 0.5, '']))}<br>Complete the cumulative frequency column.`, `Jadual menunjukkan ${ctx.ms}.<br>${gt(cl, fs, ['Kelas', 'Kekerapan', 'Sempadan atas', 'Kekerapan longgokan'], cl.map((c) => [c[1] + 0.5, '']))}<br>Lengkapkan lajur kekerapan longgokan.`), a: T(cf.join(', ')), sp: 'l' };
    },
    (r) => {
      const N = r.pick([30, 40, 50, 60, 80]);
      ...
      return { q: T(`A grouped data set has $n = ${N}$ items. State the position (item number) of ${lab.en} in the ordered data.`, `Satu set data terkumpul mempunyai $n = ${N}$ item. Nyatakan kedudukan (nombor item) ${lab.ms} dalam data yang tersusun.`), a: T(`${n(pos)}th item`, `item ke-${n(pos)}`), sp: 'xs' };
    },
    (r) => {
  ...
  const g72m2 = [
    (r) => {
      ...
      const ctx = r.pick(STAT_CTX);
      return { q: T(`The ogive of ${ctx.en} (${N} items) is shown. Estimate the median and the interquartile range.`, `Ogif bagi ${ctx.ms} (${N} item) ditunjukkan. Anggarkan median dan julat antara kuartil.`), fig, a: T(`Median $\\approx ${f2(interp(cl, fs, N / 2))}$; $Q_1 \\approx ${f2(interp(cl, fs, N / 4))}$, $Q_3 \\approx ${f2(interp(cl, fs, (3 * N) / 4))}$; IQR $\\approx ${f2(interp(cl, fs, (3 * N) / 4) - interp(cl, fs, N / 4))}$`, `Median $\\approx ${f2(interp(cl, fs, N / 2))}$; $Q_1 \\approx ${f2(interp(cl, fs, N / 4))}$, $Q_3 \\approx ${f2(interp(cl, fs, (3 * N) / 4))}$; JAK $\\approx ${f2(interp(cl, fs, (3 * N) / 4) - interp(cl, fs, N / 4))}$`), sp: 'm' };
    },
    (r) => {
      const cl = cls(r.pick([10, 20]), 10, 5), fs = cl.map(() => r.int(3, 12));
      const cf = cfOf(fs), N = cf[4];
      ...
      const ctx = r.pick(STAT_CTX);
      return { q: T(`The ogive of ${ctx.en} (${N} items) is shown. Estimate the number of items with a value at most ${n(val)}.`, `Ogif bagi ${ctx.ms} (${N} item) ditunjukkan. Anggarkan bilangan item yang bernilai selebih-lebihnya ${n(val)}.`), fig, a: T(`$\\approx ${cf[k]}$`), sp: 'm' };
    },
    (r) => {
      const cl = cls(r.pick([10, 20]), 10, 5), fs = cl.map(() => r.int(3, 12));
      const cf = cfOf(fs), N = cf[4];
      ...
      const fig = ogiveFig(cl, cf);
      const ctx = r.pick(STAT_CTX);
      return { q: T(`The ogive of ${ctx.en} (${N} items) is shown; the extreme values are ${n(minV)} and ${n(maxV)}. Estimate the five-number summary and construct a box plot.`, `Ogif bagi ${ctx.ms} (${N} item) ditunjukkan; nilai ekstrem ialah ${n(minV)} dan ${n(maxV)}. Anggarkan ringkasan lima nombor dan bina plot kotak.`), fig, a: T(`Min ${f2(five[0])}, $Q_1 \\approx ${f2(five[1])}$, median $\\approx ${f2(five[2])}$, $Q_3 \\approx ${f2(five[3])}$, max ${f2(five[4])}.${boxFig(five, minV, maxV, 'en')}`, `Min ${f2(five[0])}, $Q_1 \\approx ${f2(five[1])}$, median $\\approx ${f2(five[2])}$, $Q_3 \\approx ${f2(five[3])}$, maks ${f2(five[4])}.${boxFig(five, minV, maxV, 'ms')}`), sp: 'xl' };
    },
    (r) => {
  ...
  const g72a2 = [
    ...
    (r) => {
      ...
      const ctx = r.pick(STAT_CTX);
      return { q: T(`The ogive of ${ctx.en} (${N} items, classes ${cl.map((c) => c[0] + '–' + c[1]).join(', ')}, frequencies ${fs.join(', ')}) is used to set a pass mark so that the top ${passPct}% of items pass. Estimate the pass mark.`, `Ogif bagi ${ctx.ms} (${N} item, kelas ${cl.map((c) => c[0] + '–' + c[1]).join(', ')}, kekerapan ${fs.join(', ')}) digunakan untuk menetapkan markah lulus supaya ${passPct}% teratas item lulus.`), a: T(`Pass mark $\\approx ${f2(passMark)}$ (the value with $${round(N * (1 - passPct / 100), 1)}$ items below it)`, `Markah lulus $\\approx ${f2(passMark)}$ (nilai dengan $${round(N * (1 - passPct / 100), 1)}$ item di bawahnya)`), sp: 'l' };
    },
    (r) => {
      const cl = cls(r.pick([10, 20]), 10, 5), fs = cl.map(() => r.int(3, 12));
      const cf = cfOf(fs), N = cf[4];
  ...
  const HIST_TERMS = [
    { term: T('histogram', 'histogram'), def: T('a bar chart for grouped data with class boundaries on the horizontal axis and frequency on the vertical axis, with no gaps between bars', 'carta bar bagi data terkumpul dengan sempadan kelas pada paksi mendatar dan kekerapan pada paksi mencancang, tanpa jurang antara bar') },
    { term: T('frequency polygon', 'poligon kekerapan'), def: T('a line graph formed by joining points plotted at each class midpoint against its frequency', 'graf garis yang dibentuk dengan menyambungkan titik yang diplot pada setiap titik tengah kelas melawan kekerapannya') },
    { term: T('contiguous bars', 'bar bersebelahan'), def: T('bars in a histogram that touch each other with no gaps, because the horizontal axis uses class boundaries', 'bar dalam histogram yang bersentuhan antara satu sama lain tanpa jurang, kerana paksi mendatar menggunakan sempadan kelas') },
    { term: T('class boundary', 'sempadan kelas'), def: T('the value exactly halfway between the upper limit of one class and the lower limit of the next, used on the horizontal axis of a histogram', 'nilai yang tepat di antara had atas sesuatu kelas dengan had bawah kelas berikutnya, digunakan pada paksi mendatar histogram') },
    { term: T('modal class (from a histogram)', 'kelas modal (daripada histogram)'), def: T('the class represented by the tallest bar in the histogram', 'kelas yang diwakili oleh bar tertinggi dalam histogram') },
    { term: T('skewed distribution', 'taburan condong'), def: T('a distribution whose histogram or polygon has a longer tail on one side, so it is not symmetric', 'taburan yang histogram atau poligonnya mempunyai ekor yang lebih panjang pada satu bahagian, jadi ia tidak simetri') },
  ...
  const g73e2 = [
    (r) => {
      ...
      return { q: T(`The histogram shows ${ctx.en}. Write the frequency table and the total number of items.`, `Histogram menunjukkan ${ctx.ms}. Tulis jadual kekerapan dan jumlah bilangan item.`), fig, a: T(`${cl.map((c, i) => `${c[0]}–${c[1]}: ${fs[i]}`).join('; ')}; total ${sum(fs)}`), sp: 'm' };
    },
    (r) => {
      const cl = cls(r.pick([10, 20]), 10, 5), fs = cl.map(() => r.int(2, 14));
      const i = r.int(0, 4);
      const fig = T(histFig(cl, fs, 'en'), histFig(cl, fs, 'ms'));
      return { q: T(`The histogram shows a grouped distribution. Read off the frequency of the class ${cl[i][0]}–${cl[i][1]}.`, `Histogram menunjukkan taburan terkumpul. Baca kekerapan kelas ${cl[i][0]}–${cl[i][1]}.`), fig, a: T(n(fs[i])), sp: 's' };
    },
    (r) => {
      const stmts = [
        { en: 'In a histogram, the bars are drawn with small gaps between them, like a bar chart of categories.', ms: 'Dalam histogram, bar dilukis dengan jurang kecil antaranya, seperti carta bar bagi kategori.', ok: false },
        { en: 'A frequency polygon is formed by joining points plotted at the class midpoints.', ms: 'Poligon kekerapan dibentuk dengan menyambungkan titik yang diplot pada titik tengah kelas.', ok: true },
        { en: 'The horizontal axis of a histogram uses the class boundaries, not the class limits.', ms: 'Paksi mendatar histogram menggunakan sempadan kelas, bukan had kelas.', ok: true },
        { en: 'The tallest bar of a histogram identifies the class with the lowest frequency.', ms: 'Bar tertinggi dalam histogram mengenal pasti kelas dengan kekerapan terendah.', ok: false },
      ];
      const s = r.pick(stmts);
      return { q: T(`True or false? "${s.en}"`, `Betul atau salah? "${s.ms}"`), a: T(s.ok ? 'True' : 'False', s.ok ? 'Betul' : 'Salah'), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The frequency polygon shows ${ctx.en}. At which class midpoint does the polygon reach its highest point?`, `Poligon kekerapan menunjukkan ${ctx.ms}. Pada titik tengah kelas manakah poligon itu mencapai titik tertingginya?`), fig, a: T(`$x = ${mid(peak)}$ (class ${peak[0]}–${peak[1]})`, `$x = ${mid(peak)}$ (kelas ${peak[0]}–${peak[1]})`), sp: 's' };
    },
    (r) => {
      const cl = cls(r.pick([0, 10]), r.pick([5, 10]), 4);
      const i = r.int(0, 3);
      return { q: T(`Give the class limits and the class boundaries of the class interval that has midpoint ${mid(cl[i])} and class width ${cl[0][1] - cl[0][0] + 1}.`, `Berikan had kelas dan sempadan kelas bagi selang kelas yang mempunyai titik tengah ${mid(cl[i])} dan lebar kelas ${cl[0][1] - cl[0][0] + 1}.`), a: T(`Limits ${cl[i][0]}–${cl[i][1]}; boundaries ${bnd(cl[i])[0]}–${bnd(cl[i])[1]}`, `Had ${cl[i][0]}–${cl[i][1]}; sempadan ${bnd(cl[i])[0]}–${bnd(cl[i])[1]}`), sp: 's' };
    },
    (r) => {
      const cl = cls(r.pick([0, 10]), 10, 4), fs = cl.map(() => r.int(2, 12));
      return { q: T(`A grouped distribution has classes ${cl.map((c) => `${c[0]}–${c[1]}`).join(', ')} with frequencies ${fs.join(', ')}. Which class has the lowest frequency (the shortest bar in a histogram of this data)?`, `Satu taburan terkumpul mempunyai kelas ${cl.map((c) => `${c[0]}–${c[1]}`).join(', ')} dengan kekerapan ${fs.join(', ')}. Kelas manakah mempunyai kekerapan terendah (bar terpendek dalam histogram data ini)?`), a: T(`${cl[fs.indexOf(Math.min(...fs))][0]}–${cl[fs.indexOf(Math.min(...fs))][1]}`), sp: 'xs' };
    },
  ];
  const g73m2 = [
    (r) => {
    ...
    (r) => {
      ...
      const ctx = r.pick(STAT_CTX);
      const fig = T(histFig(cl, fs, 'en'), histFig(cl, fs, 'ms'));
      return { q: T(`The histogram shows ${ctx.en}. Read off the frequencies and build the cumulative frequency table.`, `Histogram menunjukkan ${ctx.ms}. Baca kekerapan dan bina jadual kekerapan longgokan.`), fig, a: T(`Frequencies ${fs.join(', ')}; cumulative frequencies ${cf.join(', ')}`, `Kekerapan ${fs.join(', ')}; kekerapan longgokan ${cf.join(', ')}`), sp: 'l' };
    },
    (r) => {
    ...
    (r) => {
      ...
      const ctx = r.pick(STAT_CTX);
      const fig = T(polyFig(cl, fs, 'en'), polyFig(cl, fs, 'ms'));
      return { q: T(`The frequency polygon shows ${ctx.en}. Estimate the mean using the class midpoints shown by the polygon's vertices.`, `Poligon kekerapan menunjukkan ${ctx.ms}. Anggarkan min menggunakan titik tengah kelas yang ditunjukkan oleh bucu poligon.`), fig, a: T(f2(st.m)), sp: 'm' };
    },
    (r) => {
      const cl = cls(r.pick([10, 20]), 10, 5), fs = cl.map(() => r.int(2, 14));
      const w = cl[0][1] - cl[0][0] + 1;
  ...
  const g73a2 = [
    ...
    (r) => {
      ...
      const e = r.pick(ERR);
      const ctx = r.pick(STAT_CTX);
      return { q: T(`A student drew a histogram and frequency polygon for ${ctx.en} (classes ${cl.map((c) => `${c[0]}–${c[1]}`).join(', ')}, frequencies ${fs.join(', ')}), but ${e.en}. Explain the error and how to fix it.`, `Seorang murid melukis histogram dan poligon kekerapan bagi ${ctx.ms} (kelas ${cl.map((c) => `${c[0]}–${c[1]}`).join(', ')}, kekerapan ${fs.join(', ')}), tetapi ${e.ms}. Terangkan kesilapan itu dan cara membetulkannya.`), a: T(e.fix.en, e.fix.ms), sp: 'l' };
    },
    (r) => {
      const cl = cls(r.pick([10, 20]), 10, 5), fs = cl.map(() => r.int(2, 14));
      const ctx = r.pick(STAT_CTX);
  ...
  const COLLECT_CTX = [
    { en: 'measuring the height of every seedling in a nursery', ms: 'mengukur ketinggian setiap anak benih di sebuah tapak semaian', method: T('direct measurement', 'pengukuran langsung') },
    { en: 'finding out how many books each student borrowed last month', ms: 'mengetahui berapa buah buku yang dipinjam setiap murid bulan lepas', method: T('checking library/school records', 'menyemak rekod perpustakaan/sekolah') },
    { en: "finding out students' favourite subject", ms: 'mengetahui mata pelajaran kegemaran murid', method: T('a questionnaire/survey', 'soal selidik/tinjauan') },
    { en: 'finding out how many customers enter a shop each hour', ms: 'mengetahui berapa ramai pelanggan memasuki sebuah kedai setiap jam', method: T('observation and counting', 'pemerhatian dan mengira') },
  ];
  const PROJECT_TERMS = [
    { term: T('statistical question', 'soalan statistik'), def: T('a question that can be answered by collecting data that varies across a group, not a question with a single fixed or opinion-based answer', 'soalan yang boleh dijawab dengan mengumpul data yang berbeza merentas suatu kumpulan, bukan soalan yang mempunyai satu jawapan tetap atau berdasarkan pendapat') },
    { term: T('population (statistics)', 'populasi (statistik)'), def: T('the entire group being studied in an investigation', 'keseluruhan kumpulan yang dikaji dalam sesuatu inkuiri') },
    { term: T('sample', 'sampel'), def: T('a subset of the population that is actually collected or observed, used to draw conclusions about the whole population', 'sebahagian daripada populasi yang benar-benar dikumpul atau diperhatikan, digunakan untuk membuat kesimpulan tentang keseluruhan populasi') },
    { term: T('variable (statistics)', 'pemboleh ubah (statistik)'), def: T('the characteristic being measured or recorded for each member of the group, such as height or favourite sport', 'ciri yang diukur atau direkodkan bagi setiap ahli kumpulan, seperti ketinggian atau sukan kegemaran') },
    { term: T('sampling bias', 'bias pensampelan'), def: T('when a sample is chosen in a way that makes it unrepresentative of the population, leading to misleading conclusions', 'apabila sampel dipilih dengan cara yang menjadikannya tidak mewakili populasi, membawa kepada kesimpulan yang mengelirukan') },
    { term: T('random sample', 'sampel rawak'), def: T('a sample chosen so that every member of the population has a fair chance of being included, reducing the risk of bias', 'sampel yang dipilih supaya setiap ahli populasi mempunyai peluang yang saksama untuk disertakan, mengurangkan risiko bias') },
    { term: T('data collection method', 'kaedah pengumpulan data'), def: T('the way data is gathered, such as a survey, observation, measurement or existing records', 'cara data dikumpul, seperti tinjauan, pemerhatian, pengukuran atau rekod sedia ada') },
  ];
  const g74e2 = [
    (r) => {
      ...
      return { q: T(w[0], w[1]), a: T(`Variable: ${c.var.en}. Collection: ${c.method.en}.`, `Pemboleh ubah: ${c.var.ms}. Pengumpulan: ${c.method.ms}.`), sp: 's' };
    },
    (r) => {
      const ps = r.pick(POP_SAMPLE);
      return { q: T(`${ps.en} Identify the population and the sample in this study.`, `${ps.ms} Kenal pasti populasi dan sampel dalam kajian ini.`), a: T(`Population: ${ps.pop.en}. Sample: ${ps.sample.en}.`, `Populasi: ${ps.pop.ms}. Sampel: ${ps.sample.ms}.`), sp: 's' };
    },
    (r) => {
      const c = r.pick(STAT_Q);
      return { q: T(`For the variable "${c.var.en.replace(/\s*\([^)]*\)/, '')}" in the question "${c.en}", is the data categorical or numerical?`, `Bagi pemboleh ubah "${c.var.ms.replace(/\s*\([^)]*\)/, '')}" dalam soalan "${c.ms}", adakah data itu berkategori atau berangka?`), a: c.var, sp: 's' };
    },
    (r) => {
      const good = r.pick(STAT_Q), bad = r.pick(BAD_Q);
      const swap = r.chance();
      const pair = swap ? [bad, good] : [good, bad];
      return { q: T(`Which of these is a well-formed statistical question? (A) "${pair[0].en}" (B) "${pair[1].en}"`, `Yang manakah soalan statistik yang terbentuk dengan baik? (A) "${pair[0].ms}" (B) "${pair[1].ms}"`), a: T(pair[0] === good ? '(A)' : '(B)', pair[0] === good ? '(A)' : '(B)'), sp: 's' };
    },
    (r) => {
      const c = r.pick(COLLECT_CTX);
      return { q: T(`Which data collection method is most suitable for ${c.en}?`, `Kaedah pengumpulan data manakah yang paling sesuai untuk ${c.ms}?`), a: c.method, sp: 's' };
    },
    (r) => {
      const c = r.pick(COLLECT_CTX);
      return { q: T(`Explain briefly why ${c.method.en.toLowerCase()} is a suitable way of ${c.en}.`, `Terangkan secara ringkas mengapa ${c.method.ms.toLowerCase()} merupakan cara yang sesuai untuk ${c.ms}.`), a: T(`It directly gives the exact information needed without relying on estimation or opinion.`, `Ia secara langsung memberikan maklumat tepat yang diperlukan tanpa bergantung pada anggaran atau pendapat.`), sp: 's' };
    },
    (r) => {
      const c = r.pick(STAT_Q);
      ...
      const L = 'ABC';
      return { q: T(`Which of these is a feature of a good statistical question, such as "${c.en}"? ${feats.map((f, i) => `(${L[i]}) ${f.t.en}`).join(' ')}`, `Yang manakah ciri soalan statistik yang baik, seperti "${c.ms}"? ${feats.map((f, i) => `(${L[i]}) ${f.t.ms}`).join(' ')}`), a: T(`${L[ci]}: ${feats[ci].t.en}`, `${L[ci]}: ${feats[ci].t.ms}`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`Explain why "${bq.en}" is not a good statistical question, and suggest a better one on a similar topic.`, `Terangkan mengapa "${bq.ms}" bukan soalan statistik yang baik, dan cadangkan satu soalan yang lebih baik pada topik yang serupa.`), a: T(`Not good because ${bq.why.en}. A better version asks about a measurable variable across a group, e.g. "${better.en}"`, `Bukan baik kerana ${bq.why.ms}. Versi yang lebih baik bertanya tentang pemboleh ubah yang boleh diukur merentas suatu kumpulan, contohnya "${better.ms}"`), sp: 's' };
    },
    (r) => {
      const c = r.pick(STAT_Q);
      ...
      return { q: T(`Is the variable in "${c.en}" categorical, or numerical (discrete or continuous)?`, `Adakah pemboleh ubah dalam "${c.ms}" berkategori, atau berangka (diskret atau selanjar)?`), a: catType, sp: 'xs' };
    },
  ];
  const g74m2 = [
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`"${f.en}" Identify the sampling problem with this study, and suggest an improvement.`, `"${f.ms}" Kenal pasti masalah pensampelan kajian ini, dan cadangkan penambahbaikan.`), a: T(`Problem: ${f.flaw.en}. Improvement: ${f.fix.en}.`, `Masalah: ${f.flaw.ms}. Penambahbaikan: ${f.fix.ms}.`), sp: 'm' };
    },
    (r) => {
      const goal = r.pick([
        { en: 'show the overall shape of a single distribution (e.g. peaked, skewed)', ms: 'menunjukkan bentuk keseluruhan satu taburan (contohnya berpuncak, condong)', ans: T('a histogram or a frequency polygon', 'histogram atau poligon kekerapan') },
        { en: 'compare the spread and skewness of two data sets side by side, using their five-number summaries', ms: 'membandingkan serakan dan kepencongan dua set data secara bersebelahan, menggunakan ringkasan lima nombor mereka', ans: T('grouped box plots', 'plot kotak terkumpul') },
        { en: 'estimate the median, quartiles and percentiles of a data set', ms: 'menganggarkan median, kuartil dan persentil bagi satu set data', ans: T('an ogive (cumulative frequency curve)', 'ogif (lengkung kekerapan longgokan)') },
        { en: 'compare two distributions overlaid on the same axes', ms: 'membandingkan dua taburan yang dilapiskan pada paksi yang sama', ans: T('two frequency polygons on the same axes', 'dua poligon kekerapan pada paksi yang sama') },
      ]);
      return { q: T(`Which representation would be most suitable to ${goal.en}?`, `Perwakilan manakah yang paling sesuai untuk ${goal.ms}?`), a: goal.ans, sp: 'm' };
    },
    (r) => {
