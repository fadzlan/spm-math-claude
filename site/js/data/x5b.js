...
(function () {
  ...
  const TERMS = [
    ...
    { term: T('deductible (excess)', 'lebihan (lebihan)'), def: T('the part of each claim that the policyholder must pay first, before the insurer pays the rest', 'bahagian setiap tuntutan yang mesti dibayar dahulu oleh pemegang polisi, sebelum penanggung insurans membayar selebihnya'), ex: T(`e.g. deductible ${rm(500)}, loss ${rm(3000)}: the policyholder pays ${rm(500)}, the insurer pays ${rm(2500)}`, `cth. lebihan ${rm(500)}, kerugian ${rm(3000)}: pemegang polisi membayar ${rm(500)}, penanggung insurans membayar ${rm(2500)}`) },
  ...
  const PREMIUM_FACTORS = [
    ...
    { en: 'choosing a higher deductible', ms: 'memilih lebihan yang lebih tinggi', dir: 'lower', reason: T('the policyholder agrees to pay more of each claim, so the insurer expects to pay less', 'pemegang polisi bersetuju membayar lebih banyak bagi setiap tuntutan, jadi penanggung insurans menjangka membayar kurang') },
  ];
  const INS_TF = [
    ...
    { en: 'A deductible is paid by the policyholder, not the insurer.', ms: 'Lebihan dibayar oleh pemegang polisi, bukan penanggung insurans.', ok: true, why: T("the deductible is the policyholder's own share of a claim, paid before the insurer pays the rest", 'lebihan ialah bahagian pemegang polisi sendiri bagi sesuatu tuntutan, dibayar sebelum penanggung insurans membayar selebihnya'), key: T('Claim = deductible (policyholder) + payout (insurer).', 'Tuntutan = lebihan (pemegang polisi) + bayaran (penanggung insurans).') },
  ...
  const orderTxt = (order) =>
    order === 'before'
      ? T('the deductible is subtracted from the loss first, then the proportional-settlement ratio is applied', 'lebihan ditolak daripada kerugian dahulu, kemudian nisbah penyelesaian berkadar digunakan')
      : T('the proportional-settlement ratio is applied to the loss first, then the deductible is subtracted', 'nisbah penyelesaian berkadar digunakan pada kerugian dahulu, kemudian lebihan ditolak');
  ...
  const g32e = [
    ...
    (r) => {
      ...
      return { q: T(`A policy states that the deductible is subtracted from a covered loss before payment. A ${c.en} suffers a covered loss of ${rm(loss)}. The deductible is ${rm(ded)}. Find the payout.`, `Sebuah polisi menyatakan bahawa lebihan ditolak daripada kerugian yang dilindungi sebelum bayaran dibuat. Sebuah ${c.ms} mengalami kerugian yang dilindungi sebanyak ${rm(loss)}. Lebihannya ialah ${rm(ded)}. Cari bayaran itu.`), a: T(rm(loss - ded)), w: T(`$${loss} - ${ded}$`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      const c = r.pick(PROP_CTX);
      return { q: T(`A policy applies a deductible of ${rm(ded)} to every claim. A ${c.en} suffers a covered loss of ${rm(loss)}. Find the payout.`, `Sebuah polisi mengenakan lebihan ${rm(ded)} bagi setiap tuntutan. Sebuah ${c.ms} mengalami kerugian yang dilindungi sebanyak ${rm(loss)}. Cari bayaran itu.`), a: T('RM0 (the loss is below the deductible, so nothing is paid out).', 'RM0 (kerugian itu kurang daripada lebihan, jadi tiada bayaran dibuat).'), w: W(`$${loss} \\lt ${ded}$`, T(`The policyholder pays the whole loss of ${rm(loss)} as part of the deductible, so the payout is RM0.`, `Pemegang polisi membayar keseluruhan kerugian ${rm(loss)} sebagai sebahagian daripada lebihan, jadi bayarannya RM0.`)), sp: 's' };
    },
    (r) => {
      const prem = r.pick([400, 600, 800, 1000]), ded = r.pick([300, 500, 800]);
      const c = r.pick(PROP_CTX);
      return { q: T(`A ${c.en} owner pays an annual premium of ${rm(prem)}. During the year, a covered loss occurs and the owner also pays the deductible of ${rm(ded)}. Find the owner's total spending on the policy and the deductible that year.`, `Pemilik sebuah ${c.ms} membayar premium tahunan ${rm(prem)}. Sepanjang tahun itu, satu kerugian yang dilindungi berlaku dan pemilik turut membayar lebihan ${rm(ded)}. Cari jumlah perbelanjaan pemilik ke atas premium dan lebihan pada tahun itu.`), a: T(rm(prem + ded)), w: T(`$${prem} + ${ded}$`), sp: 's' };
    },
    (r) => {
      const rate = r.pick([4, 5, 6, 8]);
  ...
  const g32m = [
    ...
    (r) => {
      ...
      return { q: T(`A policy on a ${c.en} has a deductible of ${rm(ded)} and a claim limit of ${rm(limit)}: the deductible is subtracted first, then the result is capped at the limit. A covered loss of ${rm(loss)} occurs. Find the payout.`, `Sebuah polisi ke atas sebuah ${c.ms} mempunyai lebihan ${rm(ded)} dan had tuntutan ${rm(limit)}: lebihan ditolak dahulu, kemudian hasilnya dihadkan pada had itu. Kerugian yang dilindungi sebanyak ${rm(loss)} berlaku. Cari bayaran itu.`), a: T(rm(payout), rm(payout)), w: T(`$${loss} - ${ded} = ${loss - ded}$, capped at ${rm(limit)}`, `$${loss} - ${ded} = ${loss - ded}$, dihadkan pada ${rm(limit)}`), sp: 'm' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return {
        q: T(`${lab.en}<br>${ncdTable(rows, 'en')}<br>A motor policy has a base premium of ${rm(base)} and a deductible of ${rm(ded)} (subtracted from any covered loss). The policyholder has had ${y} consecutive claim-free years. Find (a) the premium payable after the NCD, (b) the payout for a covered loss of ${rm(loss)}.`, `${lab.ms}<br>${ncdTable(rows, 'ms')}<br>Sebuah polisi motor mempunyai premium asas ${rm(base)} dan lebihan ${rm(ded)} (ditolak daripada sebarang kerugian yang dilindungi). Pemegang polisi mempunyai ${y} tahun berturut-turut tanpa tuntutan. Cari (a) premium yang perlu dibayar selepas NCD, (b) bayaran bagi kerugian yang dilindungi sebanyak ${rm(loss)}.`),
    ...
    (r) => {
      ...
      return {
        q: T(
          ...
          `Sebuah ${c.ms} bernilai ${rm(V)} mesti diinsuranskan sekurang-kurangnya ${reqPct}% daripada nilainya (${rm(req)}) jika tidak bayaran dikurangkan secara berkadar (nisbah digunakan pada kerugian dahulu, kemudian lebihan ditolak). Sepanjang ${years} tahun, Polisi $A$ berkos ${rm(premA)} setahun dan menginsuranskan jumlah yang diperlukan sepenuhnya (${rm(insuredA)}); Polisi $B$ berkos ${rm(premB)} setahun dan hanya menginsuranskan ${rm(insuredB)}. Kedua-duanya mempunyai lebihan ${rm(ded)}. Dalam salah satu daripada ${years} tahun itu, kerugian yang dilindungi sebanyak ${rm(loss)} berlaku. Cari jumlah kos kepada pemegang polisi (premium sepanjang ${years} tahun + bahagian kerugian yang tidak dipampas) di bawah setiap polisi, dan nyatakan yang mana lebih murah secara keseluruhan.`
        ),
    ...
    (r) => {
      ...
      return {
        q: T(
          ...
          `Sebuah polisi mempunyai premium asas ${rm(base)}, lebihan ${rm(ded)} (ditolak daripada kerugian yang dilindungi, kemudian dihadkan pada had tuntutan yang dinyatakan) dan menggunakan penyelesaian berkadar (nisbah pada kerugian, kemudian lebihan ditolak) jika ${c.ms} kurang diinsuranskan berbanding ${reqPct}% nilainya yang diperlukan. Dengan menggunakan ${lab.ms}<br>${ncdTable(rows, 'ms')}<br>pemegang polisi mempunyai ${y} tahun berturut-turut tanpa tuntutan.<br>${partsMs}`
        ),
  ...
  const EV_RULE = T('Tax evasion = deliberately hiding income or falsifying records to avoid tax that is due (illegal).', 'Pengelakan cukai = sengaja menyembunyikan pendapatan atau memalsukan rekod untuk mengelak cukai yang perlu dibayar (menyalahi undang-undang).');
  const cap1 = (x) => x.charAt(0).toUpperCase() + x.slice(1);
  const evLine = (c, pre) => T(`${pre || ''}${pre ? c.why.en : cap1(c.why.en)} → ${c.ok ? 'evasion' : 'not evasion'}`, `${pre || ''}${pre ? c.why.ms : cap1(c.why.ms)} → ${c.ok ? 'pengelakan cukai' : 'bukan pengelakan cukai'}`);
  const CONSEQ_BANK = [
  ...
  const g4Ee2 = [
    (r) => {
      ...
      return { q: T(`Is this tax evasion, an honest error, or lawful tax planning? Explain. "${c.en}"`, `Adakah ini pengelakan cukai, kesilapan jujur, atau perancangan cukai yang sah? Terangkan. "${c.ms}"`), a: c.ok ? T('Tax evasion: it deliberately conceals or falsifies information to avoid paying tax that is legally due.', 'Pengelakan cukai: ia sengaja menyembunyikan atau memalsukan maklumat untuk mengelak membayar cukai yang perlu dibayar mengikut undang-undang.') : T('Not evasion: this is either an honest, corrected error or a lawful use of the rules, not deliberate concealment.', 'Bukan pengelakan cukai: ini sama ada kesilapan jujur yang dibetulkan atau penggunaan peraturan secara sah, bukan penyembunyian yang disengajakan.'), w: W(EV_RULE, evLine(c)), sp: 's' };
    },
    (r) => {
  ...
  const g4Em2 = [
    ...
    (r) => {
      ...
      return { q: T(`Classify each situation as tax evasion or not (with a one-phrase reason).<br>(a) "${scens[0].en}"<br>(b) "${scens[1].en}"<br>(c) "${scens[2].en}"`, `Kelaskan setiap situasi sebagai pengelakan cukai atau bukan (dengan sebab ringkas).<br>(a) "${scens[0].ms}"<br>(b) "${scens[1].ms}"<br>(c) "${scens[2].ms}"`), a: SPM.parts(scens.map((c) => (c.ok ? T('Evasion: deliberate concealment/falsification.', 'Pengelakan cukai: penyembunyian/pemalsuan yang disengajakan.') : T('Not evasion: honest error or lawful planning.', 'Bukan pengelakan cukai: kesilapan jujur atau perancangan yang sah.')))), w: W(EV_RULE, ...scens.map((c, i) => evLine(c, `(${'abc'[i]}) `))), sp: 'm' };
    },
    (r) => {
      const c = r.pick(EVASION_SCEN.filter((x) => x.ok));
  ...
  const g4Ea2 = [
    ...
    (r) => {
      const claim = r.pick([
        ...
        T('A friend says: "Tax evasion only hurts the government, not other people."', 'Seorang rakan berkata: "Pengelakan cukai hanya merugikan kerajaan, bukan orang lain."'),
      ]);
  ...
  const OGIVE_TERMS = [
    ...
    { term: T('five-number summary', 'ringkasan lima nombor'), def: T('the minimum, $Q_1$, median, $Q_3$ and maximum of a data set, used to draw a box plot', 'minimum, $Q_1$, median, $Q_3$ dan maksimum sesuatu set data, digunakan untuk melukis plot kotak'), ex: T('e.g. 5, 22, 30, 38, 60: box from 22 to 38 with a line at 30, extending to the minimum 5 and the maximum 60', 'cth. 5, 22, 30, 38, 60: kotak dari 22 hingga 38 dengan garis pada 30, dilanjutkan hingga nilai minimum 5 dan nilai maksimum 60') },
  ];
  const g72e2 = [
    (r) => {
