...
(function () {
  ...
  const CWHY = {
    one: T('Each input has exactly one output, and no two inputs share an output.', 'Setiap input mempunyai tepat satu output, dan tiada dua input berkongsi output.'),
    many: T('Each input has exactly one output, but two or more inputs share the same output.', 'Setiap input mempunyai tepat satu output, tetapi dua atau lebih input berkongsi output yang sama.'),
    none: T('At least one input is linked to more than one output.', 'Sekurang-kurangnya satu input dihubungkan dengan lebih daripada satu output.'),
  };
  const subLine = (R, v) => `$f(${n(v)}) = ${R.s(v)} = ${n(R.f(v))}$`;
  ...
  const WORDR = [
    ['add 5 to the input', 'tambah 5 kepada input', (v) => v + 5, 'x + 5', (v) => `${v} + 5`],
    ['subtract 3 from the input', 'tolak 3 daripada input', (v) => v - 3, 'x - 3', (v) => `${v} - 3`],
    ['multiply the input by 4', 'darab input dengan 4', (v) => 4 * v, '4x', (v) => `4 \\times ${v}`],
    ['multiply the input by 3 and then add 1', 'darab input dengan 3 kemudian tambah 1', (v) => 3 * v + 1, '3x + 1', (v) => `3 \\times ${v} + 1`],
    ['multiply the input by 2 and then subtract 5', 'darab input dengan 2 kemudian tolak 5', (v) => 2 * v - 5, '2x - 5', (v) => `2 \\times ${v} - 5`],
    ['square the input', 'kuasa duakan input', (v) => v * v, 'x^2', (v) => `${v}^2`],
    ['square the input and then add 2', 'kuasa duakan input kemudian tambah 2', (v) => v * v + 2, 'x^2 + 2', (v) => `${v}^2 + 2`],
    ['subtract the input from 10', 'tolak input daripada 10', (v) => 10 - v, '10 - x', (v) => `10 - ${v}`],
    ['double the input and then add 7', 'gandakan input kemudian tambah 7', (v) => 2 * v + 7, '2x + 7', (v) => `2 \\times ${v} + 7`],
    ['cube the input', 'kuasa tigakan input', (v) => v * v * v, 'x^3', (v) => `${v}^3`],
  ];
  const CLOZE = [
  ...
  const g81e = [
    ...
    (r) => {
      ...
      return { q: TT(r.pick(ph)), a: T(`$f(${v}) = ${n(R.f(v))}$`), w: W(T(`Substitute $x = ${v}$ into $y = ${Q}$.`, `Gantikan $x = ${v}$ ke dalam $y = ${Q}$.`), `$${R.s(v)} = ${n(R.f(v))}$`, `$f(${v}) = ${n(R.f(v))}$`), sp: 'xs' };
    },
    /* complete a table */
    (r) => {
      ...
      return { q: T(`${pe}<br>${blanks(xs, '$x$', '$y$')}`, `${pm}<br>${blanks(xs, '$x$', '$y$')}`), a: T(`$y = ${xs.map((x) => n(R.f(x))).join(',\\ ')}$`), w: W(T('Substitute each value of $x$ into the rule.', 'Gantikan setiap nilai $x$ ke dalam peraturan itu.'), `$${xs.map((x) => `${R.s(x)} = ${n(R.f(x))}`).join(',\\ ')}$`, `$y = ${xs.map((x) => n(R.f(x))).join(',\\ ')}$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      const inv = R.kind === 'ax' ? `$x = ${n(y)} \\div ${par(R.f(1))} = ${v}$` : R.kind === 'x+b' ? `$x = ${n(y)} - ${par(R.f(0))} = ${v}$` : `$x = ${n(R.f(1))} \\div ${par(y)} = ${v}$`;
      return { q: TT(r.pick(ph)), a: T(`$x = ${v}$`), w: W(T('Write the rule as an equation and work backwards.', 'Tulis peraturan itu sebagai persamaan dan kerja ke belakang.'), `$${R.t} = ${n(y)}$`, inv), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      const w = why(pairs, c);
      return { q: TT(r.pick(ph)), fig: arrowFig(pairs), a: T(c === 'none' ? `No, ${w.en}.` : `Yes, ${w.en}.`, c === 'none' ? `Tidak, ${w.ms}.` : `Ya, ${w.ms}.`), w: W(T('Count the arrows leaving each input: a function has exactly one arrow from every input.', 'Kira anak panah yang meninggalkan setiap input: fungsi mempunyai tepat satu anak panah daripada setiap input.'), T(cap(w.en) + '.', cap(w.ms) + '.'), T(c === 'none' ? 'So the relation is not a function.' : 'So the relation is a function.', c === 'none' ? 'Jadi hubungan itu bukan fungsi.' : 'Jadi hubungan itu ialah fungsi.')), sp: 's' };
    },
    ...
    (r) => {
      ...
      const [pe, pm] = r.pick(ph);
      return { q: T(pe, pm), a: T(`RM${p * k}`), w: W(T(`Substitute $x = ${k}$ into $C = ${p}x$.`, `Gantikan $x = ${k}$ ke dalam $C = ${p}x$.`), `$C = ${p} \\times ${k} = ${p * k}$`, T(`The cost is RM${p * k}.`, `Kosnya ialah RM${p * k}.`)), sp: 'xs' };
    },
    /* true / false */
    (r) => {
      const t = r.pick(TF);
    ...
    (r) => {
      ...
      const wy = why(pairs, c);
      return { q: TT(r.pick(ph)), a: c === 'none' ? T('No', 'Tidak') : T('Yes', 'Ya'), w: W(T('Look at the first number of each pair (the input) and check for a repeat.', 'Lihat nombor pertama setiap pasangan (input) dan semak sama ada ada yang berulang.'), T(cap(wy.en) + '.', cap(wy.ms) + '.'), T(c === 'none' ? 'So it is not a function: No.' : 'So it is a function: Yes.', c === 'none' ? 'Jadi ia bukan fungsi: Tidak.' : 'Jadi ia suatu fungsi: Ya.')), sp: 'xs' };
    },
    /* everyday relations */
    (r) => {
      ...
      const [pe, pm] = r.pick(ph);
      return { q: T(pe, pm), a: T(cap(CN[c].en), cap(CN[c].ms)), w: W(T('Ask: does every input have exactly one output, and can two inputs share an output?', 'Tanya: adakah setiap input mempunyai tepat satu output, dan bolehkah dua input berkongsi output?'), CWHY[c], T(cap(CN[c].en) + '.', cap(CN[c].ms) + '.')), sp: 's' };
    },
    /* vertical line test */
    (r) => {
      ...
      const [pe, pm] = r.pick(ph);
      return { q: T(pe, pm), fig: g.fig(), a: g.fn ? T('Yes: every vertical line cuts the graph at most once.', 'Ya: setiap garis mencancang memotong graf paling banyak sekali.') : T('No: some vertical line cuts the graph at two points (one input has two outputs).', 'Tidak: ada garis mencancang yang memotong graf pada dua titik (satu input mempunyai dua output).'), w: W(T('Slide a vertical line across the graph and count where it cuts.', 'Gerakkan garis mencancang merentasi graf dan kira di mana ia memotong.'), g.fn ? T('No vertical line cuts it more than once, so each input has exactly one output.', 'Tiada garis mencancang memotongnya lebih daripada sekali, jadi setiap input mempunyai tepat satu output.') : T('Some vertical line cuts it at two points, so one input would have two outputs.', 'Ada garis mencancang yang memotongnya pada dua titik, jadi satu input akan mempunyai dua output.'), T(g.fn ? 'It is a function.' : 'It is not a function.', g.fn ? 'Ia suatu fungsi.' : 'Ia bukan fungsi.')), sp: 's' };
    },
    /* cloze */
    (r) => {
      const c = r.pick(CLOZE);
      return { q: T(`Complete the sentence.<br>${c[0]}`, `Lengkapkan ayat berikut.<br>${c[1]}`), a: T(c[2], c[3]), w: W(T(`The completed sentence reads: ${fillBlanks(c[0], c[2])}`, `Ayat yang lengkap berbunyi: ${fillBlanks(c[1], c[3])}`)), sp: 'xs' };
    },
    /* word rule <-> notation */
    (r) => {
      const w = r.pick(WORDR), v = r.int(2, 6), k = r.chance();
      const wk = W(T(`In symbols the rule is $f(x) = ${w[3]}$.`, `Dalam simbol, peraturan itu ialah $f(x) = ${w[3]}$.`), `$f(${v}) = ${w[4](v)} = ${w[2](v)}$`);
      if (k) return { q: T(`A function machine follows the rule: ${w[0]}. Write the rule as $f(x) = \\ldots$ and find the output when the input is $${v}$.`, `Sebuah mesin fungsi mengikut peraturan: ${w[1]}. Tulis peraturan itu sebagai $f(x) = \\ldots$ dan cari output apabila input ialah $${v}$.`), a: T(`$f(x) = ${w[3]}$; $f(${v}) = ${w[2](v)}$`), w: wk, sp: 's' };
      return { q: T(`The function $f(x) = ${w[3]}$ can be described in words. Describe it, then find $f(${v})$.`, `Fungsi $f(x) = ${w[3]}$ boleh diterangkan dengan perkataan. Terangkan fungsi itu, kemudian cari $f(${v})$.`), a: T(`${cap(w[0])}; $f(${v}) = ${w[2](v)}$`, `${cap(w[1])}; $f(${v}) = ${w[2](v)}$`), w: W(T(`In words: ${w[0]}.`, `Dalam perkataan: ${w[1]}.`), `$f(${v}) = ${w[4](v)} = ${w[2](v)}$`), sp: 's' };
    },
    /* negative input */
    (r) => {
      ...
      return { q: TT(r.pick(ph)), a: T(`$${n(R.f(v))}$`), w: W(T(`Substitute $x = ${v}$, keeping the negative number in brackets.`, `Gantikan $x = ${v}$, kekalkan nombor negatif itu dalam kurungan.`), `$${R.s(v)} = ${n(R.f(v))}$`, R.sqr ? T('A negative number squared is positive.', 'Kuasa dua nombor negatif ialah positif.') : T('Keep track of the signs at each step.', 'Jaga tanda pada setiap langkah.')), sp: 'xs' };
    },
    /* input and output */
    (r) => {
      ...
      return { q: TT(r.pick(ph)), a: T(`Input $= ${v}$, output $= ${y}$; $(${v},\\ ${y})$`, `Input $= ${v}$, output $= ${y}$; $(${v},\\ ${y})$`), w: W(T('In $f(a) = b$ the number inside the brackets is the input and the value of $f(a)$ is the output.', 'Dalam $f(a) = b$, nombor dalam kurungan ialah input dan nilai $f(a)$ ialah output.'), T(`Check: $${R.s(v)} = ${n(y)}$.`, `Semak: $${R.s(v)} = ${n(y)}$.`), T(`Input $= ${v}$, output $= ${y}$, written $(${v},\\ ${y})$.`, `Input $= ${v}$, output $= ${y}$, ditulis $(${v},\\ ${y})$.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A function is defined by $f(x) = ${Rt(R)}$ for the inputs $${setTex(xs)}$. List the set of outputs.`, `Suatu fungsi ditakrifkan oleh $f(x) = ${Rt(R)}$ bagi input $${setTex(xs)}$. Senaraikan set output.`), a: T(`$${setTex(outs)}$`), w: W(T('Substitute each input in turn.', 'Gantikan setiap input satu demi satu.'), `$${xs.map((x) => `f(${x}) = ${R.s(x)} = ${n(R.f(x))}`).join(',\\ ')}$`, `$${setTex(outs)}$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The table shows the inputs and outputs of a function of the form $y = ${kind === 'ax' ? 'ax' : kind === 'x+b' ? 'x + b' : 'x^2'}$. Find the rule.<br>${valsTable(xs, xs.map((x) => R.f(x)))}`, `Jadual menunjukkan input dan output suatu fungsi berbentuk $y = ${kind === 'ax' ? 'ax' : kind === 'x+b' ? 'x + b' : 'x^2'}$. Cari peraturan fungsi itu.<br>${valsTable(xs, xs.map((x) => R.f(x)))}`), a: T(`$y = ${R.t}$`), w: kind === 'ax' ? W(T('Divide each output by its input; the answer is the same every time.', 'Bahagikan setiap output dengan inputnya; jawapannya sama setiap kali.'), `$${xs.map((x) => `${n(R.f(x))} \\div ${x} = ${n(R.f(1))}`).join(',\\ ')}$`, `$y = ${R.t}$`) : kind === 'x+b' ? W(T('Subtract each input from its output; the answer is the same every time.', 'Tolak setiap input daripada outputnya; jawapannya sama setiap kali.'), `$${xs.map((x) => `${n(R.f(x))} - ${x} = ${n(R.f(0))}`).join(',\\ ')}$`, `$y = ${R.t}$`) : W(T('Compare each output with the square of its input.', 'Bandingkan setiap output dengan kuasa dua inputnya.'), `$${xs.map((x) => `${x}^2 = ${x * x}`).join(',\\ ')}$`, `$y = ${R.t}$`), sp: 's' };
    },
  ];
  ...
  const g81m = [
    ...
    (r) => {
      ...
      const st = [T(`Substitute into $f(x) = ${Rt(R)}$, keeping negatives in brackets.`, `Gantikan ke dalam $f(x) = ${Rt(R)}$, kekalkan nombor negatif dalam kurungan.`), `$f(${v}) = ${R.s(v)} = ${n(R.f(v))}$`];
      if (two) st.push(`$f(${w}) = ${R.s(w)} = ${n(R.f(w))}$`);
      return { q: T(pe, pm), a: two ? T(`$f(${v}) = ${n(R.f(v))}$; $f(${w}) = ${n(R.f(w))}$`) : T(`$f(${v}) = ${n(R.f(v))}$`), w: W(...st), sp: 's' };
    },
    ...
    (r) => {
      ...
      const w = why(pairs, c);
      return { q: TT(r.pick(ph)), a: c === 'none' ? T(`Not a function: ${w.en}.`, `Bukan fungsi: ${w.ms}.`) : T(`It is a function: every input has exactly one output.`, `Ia ialah fungsi: setiap input mempunyai tepat satu output.`), w: W(T('Check the first number of each pair (the input) for a repeat.', 'Semak nombor pertama setiap pasangan (input) sama ada ada yang berulang.'), T(cap(w.en) + '.', cap(w.ms) + '.'), T(c === 'none' ? 'So it is not a function.' : 'So it is a function.', c === 'none' ? 'Jadi ia bukan fungsi.' : 'Jadi ia suatu fungsi.')), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: TT(r.pick(ph)), a: OUT(c, pairs), w: W(T(`List the outputs (second numbers): $${setTex(pairs.map((p) => p[1]))}$`, `Senaraikan output (nombor kedua): $${setTex(pairs.map((p) => p[1]))}$`), CWHY[c], T(cap(CN[c].en) + '.', cap(CN[c].ms) + '.')), sp: 's' };
    },
    /* table */
    (r) => {
      ...
      const [pe, pm] = r.pick(ph);
      return { q: T(`${pe}<br>${tblPairs(pp)}`, `${pm}<br>${tblPairs(pp)}`), a: OUT(c, pairs), w: W(T('Read the table as (input, output) pairs: check the top row for a repeated input, then the bottom row for a repeated output.', 'Baca jadual sebagai pasangan (input, output): semak baris atas untuk input berulang, kemudian baris bawah untuk output berulang.'), T(cap(why(pairs, c).en) + '.', cap(why(pairs, c).ms) + '.'), T(cap(CN[c].en) + '.', cap(CN[c].ms) + '.')), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: TT(r.pick(ph)), fig: arrowFig(pairs, [...new Set(pairs.map((p) => p[0]))], r.shuffle([...new Set(pairs.map((p) => p[1]))])), a: OUT(c, pairs), w: W(T('Count the arrows leaving each input, then the arrows arriving at each output.', 'Kira anak panah yang meninggalkan setiap input, kemudian anak panah yang sampai pada setiap output.'), T(cap(why(pairs, c).en) + '.', cap(why(pairs, c).ms) + '.'), T(cap(CN[c].en) + '.', cap(CN[c].ms) + '.')), sp: 's' };
    },
    ...
    (r) => {
      ...
      const head = T('Vertical line test: is it a function? Horizontal line test: is it one-to-one?', 'Ujian garis mencancang: adakah ia fungsi? Ujian garis mengufuk: adakah ia satu dengan satu?');
      const wk = !g.fn ? W(head, T('A vertical line meets the graph twice, so one input would have two outputs.', 'Garis mencancang memotong graf dua kali, jadi satu input akan mempunyai dua output.'), T('Not a function.', 'Bukan fungsi.'))
        : g.one ? W(head, T('Every vertical line meets it at most once, and so does every horizontal line.', 'Setiap garis mencancang memotongnya paling banyak sekali, begitu juga setiap garis mengufuk.'), T('A one-to-one function.', 'Fungsi satu dengan satu.'))
          : W(head, T('Every vertical line meets it at most once, but some horizontal line meets it twice.', 'Setiap garis mencancang memotongnya paling banyak sekali, tetapi ada garis mengufuk yang memotongnya dua kali.'), T('A many-to-one function.', 'Fungsi banyak dengan satu.'));
      return { q: T(pe, pm), fig: g.fig(), a, w: wk, sp: 's' };
    },
    /* find the rule from a table */
    (r) => {
      ...
      const b0 = R.f(0), a0 = R.f(1) - b0;
      const wk = kind === 'x2+b'
        ? W(T(`Subtract the square of each input from its output: $${n(R.f(1))} - 1^2 = ${n(b0)}$, so $b = ${n(b0)}$.`, `Tolak kuasa dua setiap input daripada outputnya: $${n(R.f(1))} - 1^2 = ${n(b0)}$, jadi $b = ${n(b0)}$.`), `$y = ${R.t}$`, `$x = 6$: $y = ${R.s(6)} = ${n(R.f(6))}$`)
        : W(T(`The outputs change by the same amount each step: $${n(R.f(2))} - ${par(R.f(1))} = ${n(a0)}$, so $a = ${n(a0)}$.`, `Output berubah dengan jumlah yang sama setiap langkah: $${n(R.f(2))} - ${par(R.f(1))} = ${n(a0)}$, jadi $a = ${n(a0)}$.`), T(`Then $b = ${n(R.f(1))} - ${par(a0)} = ${n(b0)}$, so $y = ${R.t}$.`, `Kemudian $b = ${n(R.f(1))} - ${par(a0)} = ${n(b0)}$, jadi $y = ${R.t}$.`), `$x = 6$: $y = ${R.s(6)} = ${n(R.f(6))}$`);
      return { q: T(`The table shows some inputs and outputs of a function of the form $${form}$. Find the rule and hence the output when $x = 6$.<br>${valsTable(xs, ys)}`, `Jadual menunjukkan beberapa input dan output bagi suatu fungsi berbentuk $${form}$. Cari peraturan itu dan seterusnya output apabila $x = 6$.<br>${valsTable(xs, ys)}`), a: T(`$y = ${R.t}$; when $x = 6$, $y = ${n(R.f(6))}$`), w: wk, sp: 's' };
    },
    ...
    (r) => {
      ...
      const b0 = R.f(0), a0 = R.f(1) - b0;
      const wk = R.kind === 'x2+b'
        ? W(`$${R.t} = ${n(y)}$`, `$x^2 = ${n(y)} - ${par(b0)} = ${n(y - b0)}$`, T(`$x = ${v}$ (the positive input is wanted here).`, `$x = ${v}$ (input positif yang dikehendaki di sini).`))
        : W(`$${R.t} = ${n(y)}$`, `$${n(a0)}x = ${n(y)} - ${par(b0)} = ${n(y - b0)}$`, `$x = ${n(y - b0)} \\div ${par(a0)} = ${v}$`);
      return { q: TT(r.pick(ph)), a: T(`$x = ${v}$`), w: wk, sp: 's' };
    },
    ...
    (r) => {
      ...
      return {
        ...
        a: ph ? T(`$y = ${y}$ (RM${y})`) : T(`$x = ${x}$`),
        w: ph ? W(T(`Substitute $x = ${x}$ into $y = ${lin(p, b)}$.`, `Gantikan $x = ${x}$ ke dalam $y = ${lin(p, b)}$.`), `$y = ${p} \\times ${x} + ${b} = ${y}$`, T(`The amount is RM${y}.`, `Jumlahnya ialah RM${y}.`))
          : W(`$${lin(p, b)} = ${y}$`, `$${p}x = ${y} - ${b} = ${p * x}$`, `$x = ${p * x} \\div ${p} = ${x}$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Each set of ordered pairs is written as (input, output). Which one is NOT a function?<br>${list}`, `Setiap set pasangan bertertib ditulis sebagai (input, output). Yang manakah BUKAN fungsi?<br>${list}`), a: T(`(${L[cor]}) $${pairTex(opts[2])}$`), w: W(T('A set of pairs is not a function when one input appears with two different outputs.', 'Set pasangan bukan fungsi apabila satu input muncul dengan dua output yang berbeza.'), T(`In (${L[cor]}), ${why(opts[2], 'none').en}.`, `Dalam (${L[cor]}), ${why(opts[2], 'none').ms}.`), T(`The other three have no repeated input, so the answer is (${L[cor]}).`, `Tiga yang lain tiada input berulang, jadi jawapannya ialah (${L[cor]}).`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The function $f(x) = ${Rt(R)}$ has the inputs $${setTex(dom)}$. Find the outputs and state whether $f$ is one-to-one or many-to-one.`, `Fungsi $f(x) = ${Rt(R)}$ mempunyai input $${setTex(dom)}$. Cari output dan nyatakan sama ada $f$ satu dengan satu atau banyak dengan satu.`), a: T(`Outputs: $${setTex(pairs.map((p) => p[1]))}$; ${CN[c].en}`, `Output: $${setTex(pairs.map((p) => p[1]))}$; ${CN[c].ms}`), w: W(T('Find the output for each input, then compare the outputs.', 'Cari output bagi setiap input, kemudian bandingkan outputnya.'), `$${dom.map((x) => `f(${x}) = ${n(R.f(x))}`).join(',\\ ')}$`, T(cap(why(pairs, c).en) + '.', cap(why(pairs, c).ms) + '.'), T(cap(CN[c].en) + '.', cap(CN[c].ms) + '.')), sp: 's' };
    },
    /* spot the error */
    (r) => {
      const s = r.pick(STUDENT)(r);
      return { q: T(s[0], s[1]), a: T(s[2], s[3]), fig: s[4], w: W(T('Test the claim: substitute the values, or compare it with the definition.', 'Uji dakwaan itu: gantikan nilai, atau bandingkan dengan takrif.'), T(s[2], s[3])), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The table shows some inputs and outputs of the function $f(x) = ${R.t}$. Find each missing value (?), from left to right.<br>${tb}`, `Jadual menunjukkan beberapa input dan output bagi fungsi $f(x) = ${R.t}$. Cari setiap nilai yang tidak diketahui (?), dari kiri ke kanan.<br>${tb}`), a: T(`Inputs: $${ai}$; outputs: $${ao}$`, `Input: $${ai}$; output: $${ao}$`), w: W(T('A missing input is found by working backwards from its output; a missing output is found by substituting its input.', 'Input yang tiada dicari dengan kerja ke belakang daripada outputnya; output yang tiada dicari dengan menggantikan inputnya.'), ...xs.map((x, i) => (i % 2 === 0 ? T(`Output $${n(R.f(x))}$ comes from the input $${x}$.`, `Output $${n(R.f(x))}$ datang daripada input $${x}$.`) : T(`Input $${x}$: $${R.s(x)} = ${n(R.f(x))}$`, `Input $${x}$: $${R.s(x)} = ${n(R.f(x))}$`)))), sp: 's' };
    },
  ];
  ...
  const g81a = [
    ...
    (r) => {
      ...
      return { q: TT(r.pick(ph)), a: T(`$x = ${jn('or')}$ (${ins.length} input${ins.length > 1 ? 's' : ''})`, `$x = ${jn('atau')}$ (${ins.length} input)`), w: W(T(`Set the output equal to $${n(y)}$ and solve.`, `Samakan output dengan $${n(y)}$ dan selesaikan.`), `$${R.t} = ${n(y)}$`, R.sqr ? T(`Both $${v}$ and $${-v}$ give this output, because squaring removes the sign.`, `Kedua-dua $${v}$ dan $${-v}$ memberikan output ini, kerana pengkuasaduaan menghapuskan tanda.`) : T('Different inputs give different outputs here, so there is only one input.', 'Input berbeza memberikan output berbeza di sini, jadi hanya ada satu input.'), T(`$x = ${jn('or')}$`, `$x = ${jn('atau')}$`)), sp: 's' };
    },
    /* two domains */
    (r) => {
      ...
      return { q: T(`Let $f(x) = ${Rt(R)}$. Compare the function on two sets of inputs: (a) $${setTex(D1)}$ and (b) $${setTex(D2)}$. For each, state whether $f$ is one-to-one or many-to-one and justify.`, `Katakan $f(x) = ${Rt(R)}$. Bandingkan fungsi itu pada dua set input: (a) $${setTex(D1)}$ dan (b) $${setTex(D2)}$. Bagi setiap set, nyatakan sama ada $f$ satu dengan satu atau banyak dengan satu dan beri justifikasi.`), w: W(T(`(a) Outputs: $${setTex(D1.map(R.f))}$`, `(a) Output: $${setTex(D1.map(R.f))}$`), CWHY[c1], T(`(b) Outputs: $${setTex(D2.map(R.f))}$`, `(b) Output: $${setTex(D2.map(R.f))}$`), CWHY[c2]), a: P([T(`${cap(CN[c1].en)}. Outputs $${setTex(D1.map(R.f))}$${c1 === 'many' ? ex(c1, D1).en : ''}.`, `${cap(CN[c1].ms)}. Output $${setTex(D1.map(R.f))}$${c1 === 'many' ? ex(c1, D1).ms : ''}.`), T(`${cap(CN[c2].en)}. Outputs $${setTex(D2.map(R.f))}$.`, `${cap(CN[c2].ms)}. Output $${setTex(D2.map(R.f))}$.`)]), sp: 'm' };
    },
    ...
    (r) => {
      ...
      need(one.length >= 3);
      return { q: T(`For $f(x) = ${Rt(R)}$, (a) write down three inputs for which $f$ is one-to-one, (b) write down three inputs for which $f$ is many-to-one. Give a reason for each.`, `Bagi $f(x) = ${Rt(R)}$, (a) tuliskan tiga input supaya $f$ ialah satu dengan satu, (b) tuliskan tiga input supaya $f$ ialah banyak dengan satu. Berikan sebab bagi setiap satu.`), w: W(T('(a) Pick inputs that are all positive: no two of them have the same square.', '(a) Pilih input yang semuanya positif: tiada dua daripadanya mempunyai kuasa dua yang sama.'), T(`$${setTex(one.slice(0, 3))}$ gives the outputs $${setTex(one.slice(0, 3).map(R.f))}$, all different.`, `$${setTex(one.slice(0, 3))}$ memberikan output $${setTex(one.slice(0, 3).map(R.f))}$, semuanya berbeza.`), T('(b) Include a positive input and its negative: they have the same square.', '(b) Masukkan input positif dan negatifnya: kedua-duanya mempunyai kuasa dua yang sama.'), T(`$${setTex([-m, m, p])}$: $f(${-m}) = f(${m}) = ${n(R.f(m))}$, so one output is shared.`, `$${setTex([-m, m, p])}$: $f(${-m}) = f(${m}) = ${n(R.f(m))}$, jadi satu output dikongsi.`)), a: P([T(`For example $${setTex(one.slice(0, 3))}$: the inputs are all positive, so their outputs $${setTex(one.slice(0, 3).map(R.f))}$ are different.`, `Contohnya $${setTex(one.slice(0, 3))}$: semua input positif, jadi outputnya $${setTex(one.slice(0, 3).map(R.f))}$ berbeza.`), T(`For example $${setTex([-m, m, p])}$: $f(${-m}) = f(${m}) = ${n(R.f(m))}$, so two inputs share one output.`, `Contohnya $${setTex([-m, m, p])}$: $f(${-m}) = f(${m}) = ${n(R.f(m))}$, jadi dua input berkongsi satu output.`)]), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The relation $${pairTex(base.concat([['k', d]]))}$ is written as (input, output), where $k \\in \\{1, 2, 3, 4, 5\\}$. Find the values of $k$ for which the relation is (a) not a function, (b) a one-to-one function, (c) a many-to-one function.`, `Hubungan $${pairTex(base.concat([['k', d]]))}$ ditulis sebagai (input, output), dengan $k \\in \\{1, 2, 3, 4, 5\\}$. Cari nilai $k$ supaya hubungan itu (a) bukan fungsi, (b) fungsi satu dengan satu, (c) fungsi banyak dengan satu.`), a: P([T(f(out.none), fm(out.none)), T(f(out.one), fm(out.one)), T(f(out.many), fm(out.many))]), w: W(T(`Add the pair $(k,\\ ${d})$ to $${pairTex(base)}$ and test each $k$ from $1$ to $5$.`, `Tambah pasangan $(k,\\ ${d})$ kepada $${pairTex(base)}$ dan uji setiap $k$ dari $1$ hingga $5$.`), T(`If $k$ is $1$, $2$ or $3$ the input is repeated: that breaks the rule unless the output $${d}$ is the one already paired with it.`, `Jika $k$ ialah $1$, $2$ atau $3$, input itu berulang: ini melanggar syarat melainkan output $${d}$ ialah output yang sudah dipadankan dengannya.`), T(`If $k = 4$ or $k = 5$ the input is new, so it is a function; it is one-to-one only when $${d}$ is not already an output.`, `Jika $k = 4$ atau $k = 5$, input itu baharu, jadi ia suatu fungsi; ia satu dengan satu hanya apabila $${d}$ belum menjadi output.`), T(`(a) ${f(out.none)}; (b) ${f(out.one)}; (c) ${f(out.many)}`, `(a) ${fm(out.none)}; (b) ${fm(out.one)}; (c) ${fm(out.many)}`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      const eq = poly([[a, 'x^2']]);
      const sqSub = (x) => `${a === 1 ? '' : a === -1 ? '-' : a}(${x})^2`;
      return { q: T(`The graph shows $f(x) = ${eq}$. The dashed horizontal line is drawn at $y = ${k}$. (a) Read the inputs $x$ that give the output $${k}$. (b) Use your answer to explain why $f$ is many-to-one.`, `Graf menunjukkan $f(x) = ${eq}$. Garis mengufuk putus-putus dilukis pada $y = ${k}$. (a) Baca input $x$ yang memberikan output $${k}$. (b) Gunakan jawapan anda untuk menerangkan mengapa $f$ ialah banyak dengan satu.`), fig, a: P([T(`$x = ${-m}$ and $x = ${m}$`), T(`Two different inputs, $${-m}$ and $${m}$, have the same output $${k}$.`, `Dua input berbeza, $${-m}$ dan $${m}$, mempunyai output yang sama, $${k}$.`)]), w: W(T(`(a) The line $y = ${k}$ cuts the curve at $x = ${-m}$ and $x = ${m}$.`, `(a) Garis $y = ${k}$ memotong lengkung pada $x = ${-m}$ dan $x = ${m}$.`), `$f(${-m}) = ${sqSub(-m)} = ${k}$`, `$f(${m}) = ${sqSub(m)} = ${k}$`, T(`(b) Two different inputs share the output $${k}$, so $f$ is many-to-one.`, `(b) Dua input berbeza berkongsi output $${k}$, jadi $f$ ialah banyak dengan satu.`)), sp: 'm' };
    },
    /* multi-part chain for a squared rule */
    (r) => {
      const R = mkRule(r, ['x2', 'ax2', 'x2+b', 'ax2+b']), m = r.int(2, 6);
      return { q: T(`A function is defined by $f(x) = ${R.t}$. (a) Find $f(${m})$. (b) Find $f(${-m})$. (c) What do (a) and (b) tell you about the type of function $f$ is?`, `Suatu fungsi ditakrifkan oleh $f(x) = ${R.t}$. (a) Cari $f(${m})$. (b) Cari $f(${-m})$. (c) Apakah yang (a) dan (b) beritahu tentang jenis fungsi $f$?`), w: W(`(a) $f(${m}) = ${R.s(m)} = ${n(R.f(m))}$`, `(b) $f(${-m}) = ${R.s(-m)} = ${n(R.f(-m))}$`, T('(c) The two inputs are different but the outputs are equal, so $f$ is many-to-one.', '(c) Dua input itu berbeza tetapi outputnya sama, jadi $f$ ialah banyak dengan satu.')), a: P([T(`$${n(R.f(m))}$`), T(`$${n(R.f(-m))}$`), T('The two different inputs have the same output, so $f$ is many-to-one (on a domain containing both).', 'Dua input berbeza itu mempunyai output yang sama, jadi $f$ ialah banyak dengan satu (pada domain yang mengandungi kedua-duanya).')]), sp: 'm' };
    },
    /* real-life functions with restricted inputs */
    (r) => {
      const m = r.int(2, 9), k = r.pick([12, 24, 36, 60, 120]);
      const C = [
        { t: 'A = x^2', f: (x) => x * x, s: (x) => `A = (${x})^2 = ${x * x}`, b: (y) => `x^2 = ${y}$, so $x = ${Math.round(Math.sqrt(y))}`, en: 'The area $A$ cm² of a square tile of side $x$ cm is given by $A = x^2$.', ms: 'Luas $A$ cm² sebuah jubin segi empat sama bersisi $x$ cm diberi oleh $A = x^2$.', ine: 'side', inm: 'sisi', v: m },
        { t: 'V = x^3', f: (x) => x * x * x, s: (x) => `V = (${x})^3 = ${x * x * x}`, b: (y) => `x^3 = ${y}$, so $x = ${Math.round(Math.cbrt(y))}`, en: 'The volume $V$ cm³ of a cube of edge $x$ cm is given by $V = x^3$.', ms: 'Isi padu $V$ cm³ sebuah kubus bersisi $x$ cm diberi oleh $V = x^3$.', ine: 'edge', inm: 'sisi', v: Math.min(m, 6) },
        { t: `t = ${dfr(k, 'v')}`, f: (x) => k / x, s: (x) => `t = ${dfr(k, x)} = ${n(k / x)}`, b: (y) => `${dfr(k, 'v')} = ${n(y)}$, so $v = ${k} \\div ${n(y)} = ${n(k / y)}`, en: `A journey of ${k} km is made at a constant speed of $v$ km/h. The time $t$ hours is given by $t = \\dfrac{${k}}{v}$.`, ms: `Suatu perjalanan sejauh ${k} km dibuat pada laju malar $v$ km/j. Masa $t$ jam diberi oleh $t = \\dfrac{${k}}{v}$.`, ine: 'speed', inm: 'laju', v: [2, 3, 4, 6].find((d) => k % d === 0) },
      ];
      const c = r.pick(C), v = c.v, out = c.f(v);
      need(isInt(out));
      return { q: T(`${c.en} (a) Find the output when the input is ${v}. (b) Find the input when the output is ${n(out)}. (c) Explain why a negative input is not sensible here, and state whether the function is one-to-one for the sensible inputs.`, `${c.ms} (a) Cari output apabila input ialah ${v}. (b) Cari input apabila output ialah ${n(out)}. (c) Terangkan mengapa input negatif tidak munasabah di sini, dan nyatakan sama ada fungsi itu satu dengan satu bagi input yang munasabah.`), w: W(`(a) $${c.s(v)}$`, `(b) $${c.b(out)}$`, T(`(c) A ${c.ine} cannot be negative; for positive inputs different inputs give different outputs, so the function is one-to-one there.`, `(c) ${cap(c.inm)} tidak boleh negatif; bagi input positif, input yang berbeza memberikan output yang berbeza, jadi fungsi itu satu dengan satu di situ.`)), a: P([T(`$${n(out)}$`), T(`$${v}$`), T(`A ${c.ine} cannot be negative; for positive inputs different inputs give different outputs, so it is one-to-one.`, `${cap(c.inm)} tidak boleh negatif; bagi input positif, input yang berbeza memberikan output yang berbeza, jadi ia satu dengan satu.`)]), sp: 'm' };
    },
    /* reciprocal function */
    (r) => {
      ...
      need(k % d === 0);
      const y = k / d;
      return { q: T(`The function $f$ is defined by $f(x) = \\dfrac{${k}}{x}$, $x \\ne 0$. (a) Find $f(${d})$ and $f(${-d})$. (b) Find the input for which $f(x) = ${y * 2}$. (c) Explain why $x = 0$ is excluded.`, `Fungsi $f$ ditakrifkan oleh $f(x) = \\dfrac{${k}}{x}$, $x \\ne 0$. (a) Cari $f(${d})$ dan $f(${-d})$. (b) Cari input yang menyebabkan $f(x) = ${y * 2}$. (c) Terangkan mengapa $x = 0$ dikecualikan.`), w: W(`(a) $f(${d}) = \\dfrac{${k}}{${d}} = ${y}$`, `$f(${-d}) = \\dfrac{${k}}{${-d}} = ${-y}$`, `(b) $\\dfrac{${k}}{x} = ${y * 2}$, $x = ${k} \\div ${y * 2} = ${k / (y * 2)}$`, T('(c) $x = 0$ needs division by $0$, which has no value, so $0$ is not an allowed input.', '(c) $x = 0$ memerlukan pembahagian dengan $0$, yang tiada nilai, jadi $0$ bukan input yang dibenarkan.')), a: P([T(`$f(${d}) = ${y}$; $f(${-d}) = ${-y}$`), T(`$x = ${k / (y * 2)}$`), T('$\\dfrac{' + k + '}{0}$ is not defined: division by zero has no value.', '$\\dfrac{' + k + '}{0}$ tidak ditakrifkan: pembahagian dengan sifar tiada nilai.')]), sp: 'm' };
    },
    /* find the constant */
    (r) => {
      ...
      need(isInt(fn) && nxt !== v);
      const wk = form === 'ax+b' ? W(`$a(${v}) ${sg(b)} = ${a * v + b}$`, `$${v}a = ${a * v + b} - ${par(b)} = ${a * v}$`, `$a = ${a * v} \\div ${v} = ${a}$`, `$f(${nxt}) = ${a}(${nxt}) ${sg(b)} = ${n(fn)}$`)
        : form === 'x2+c' ? W(`$(${v})^2 + c = ${v * v + b}$`, `$c = ${v * v + b} - ${v * v} = ${b}$`, `$f(${nxt}) = (${nxt})^2 ${sg(b)} = ${n(fn)}$`)
          : form === 'ax2' ? W(`$a(${v})^2 = ${a * v * v}$`, `$${v * v}a = ${a * v * v}$`, `$a = ${a * v * v} \\div ${v * v} = ${a}$`, `$f(${nxt}) = ${a}(${nxt})^2 = ${n(fn)}$`)
            : W(`$\\dfrac{k}{${v}} = ${a}$`, `$k = ${a} \\times ${v} = ${a * v}$`, `$f(${nxt}) = \\dfrac{${a * v}}{${nxt}} = ${n(fn)}$`);
      return { q: T(`Given that ${q[0]}, find the value of the constant, and hence find $f(${nxt})$.`, `Diberi ${q[1]}, cari nilai pemalar itu, dan seterusnya cari $f(${nxt})$.`), a: T(`${ans}; $f(${nxt}) = ${n(fn)}$`), w: wk, sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The diagram shows ${g.en}. A pupil says: ${claim[0]} Is the pupil correct? Explain.`, `Rajah menunjukkan ${g.ms}. Seorang murid berkata: ${claim[1]} Adakah murid itu betul? Terangkan.`), fig: g.fig(), a: T(claim[2], claim[3]), w: W(T('Use the two tests separately: the vertical line test decides whether it is a function; the horizontal line test decides one-to-one or many-to-one.', 'Gunakan dua ujian secara berasingan: ujian garis mencancang menentukan sama ada ia fungsi; ujian garis mengufuk menentukan satu dengan satu atau banyak dengan satu.'), T(claim[2], claim[3])), sp: 'm' };
    },
    /* restricting the domain */
    (r) => {
      const R = mkRule(r, ['x2', 'ax2', 'x2+b', 'ax2+b']), v = r.int(1, 5), y = R.f(v);
      return { q: T(`Let $f(x) = ${R.t}$. (a) Find all the inputs, when $x$ may be any number, for which $f(x) = ${y}$. (b) Find the input when the inputs are restricted to $x \\ge 0$. (c) Is $f$ one-to-one on $x \\ge 0$? Explain.`, `Katakan $f(x) = ${R.t}$. (a) Cari semua input, apabila $x$ boleh ialah sebarang nombor, yang menyebabkan $f(x) = ${y}$. (b) Cari input apabila input dihadkan kepada $x \\ge 0$. (c) Adakah $f$ satu dengan satu pada $x \\ge 0$? Terangkan.`), w: W(`(a) $${R.t} = ${n(y)}$`, T(`$x = ${v}$ or $x = ${-v}$, because $(${v})^2 = (${-v})^2$.`, `$x = ${v}$ atau $x = ${-v}$, kerana $(${v})^2 = (${-v})^2$.`), T(`(b) Only $x = ${v}$ satisfies $x \\ge 0$.`, `(b) Hanya $x = ${v}$ memenuhi $x \\ge 0$.`), T('(c) Yes: for $x \\ge 0$ different inputs have different squares, hence different outputs.', '(c) Ya: bagi $x \\ge 0$ input berbeza mempunyai kuasa dua berbeza, maka output berbeza.')), a: P([T(`$x = ${-v}$ or $x = ${v}$`, `$x = ${-v}$ atau $x = ${v}$`), T(`$x = ${v}$`), T('Yes: for $x \\ge 0$ different inputs have different squares, hence different outputs.', 'Ya: bagi $x \\ge 0$ input berbeza mempunyai kuasa dua berbeza, maka output berbeza.')]), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`On the inputs $${setTex(DOM5)}$, compare $f(x) = ${R1.t}$ and $g(x) = ${R2.t}$. Find the outputs of each and state which one is one-to-one and which is many-to-one.`, `Pada input $${setTex(DOM5)}$, bandingkan $f(x) = ${R1.t}$ dan $g(x) = ${R2.t}$. Cari output bagi setiap satu dan nyatakan yang mana satu ialah satu dengan satu dan yang mana satu banyak dengan satu.`), a: T(`$f$: $${setTex(DOM5.map(R1.f))}$, ${CN[c1].en}; $g$: $${setTex(DOM5.map(R2.f))}$, ${CN[c2].en}`, `$f$: $${setTex(DOM5.map(R1.f))}$, ${CN[c1].ms}; $g$: $${setTex(DOM5.map(R2.f))}$, ${CN[c2].ms}`), w: W(`$f$: $${setTex(DOM5.map(R1.f))}$`, CWHY[c1], `$g$: $${setTex(DOM5.map(R2.f))}$`, CWHY[c2], T(`So $f$ is ${CN[c1].en} and $g$ is ${CN[c2].en}.`, `Jadi $f$ ialah ${CN[c1].ms} dan $g$ ialah ${CN[c2].ms}.`)), sp: 'm' };
    },
  ];
  ...
  const blankFig = () => S.plane({ x: WX, y: WY, scale: 18, labelStep: 2 });
  const EQ = (F) => `y = ${F.t}`;
  const EQn = (F) => `y = ${F.t}${F.pole ? ',\\ x \\ne 0' : ''}`;
  const ptsText = (F, xs) => xs.map((x) => `(${n(x)},\\ ${ynum(F.f(x))})`).join(',\\ ');
  /** substituted form of y = a x^n at x, with brackets round the value */
  const fsub = (F, x) => {
    const xt = `(${n(x)})`, pw = F.n === 2 || F.n === -2 ? '^2' : F.n === 3 ? '^3' : '';
    if (F.pole) return `${F.a < 0 ? '-' : ''}\\dfrac{${Math.abs(F.a)}}{${xt}${pw}}`;
    return `${F.a === 1 ? '' : F.a === -1 ? '-' : `${n(F.a)} \\times `}${xt}${pw}`;
  };
  /** same, but with the unknown coefficient a left in */
  const fsubA = (F, x, sym) => {
    const xt = `(${n(x)})`, pw = Math.abs(F.n) === 2 ? '^2' : F.n === 3 ? '^3' : '', a = sym || 'a';
    return F.pole ? `\\dfrac{${a}}{${xt}${pw}}` : `${a} \\times ${xt}${pw}`;
  };
  /** the "substitute each x" lines of a table question */
  const tableSteps = (F, xs) => (F.n === 1 && F.a === 1
    ? [T('Here $y = x$, so each output is the same as its input.', 'Di sini $y = x$, jadi setiap output sama dengan inputnya.')]
    : [T('Substitute each value of $x$ into the rule.', 'Gantikan setiap nilai $x$ ke dalam peraturan itu.'), `$${xs.map((x) => `${fsub(F, x)} = ${ynum(F.f(x))}`).join(',\\ ')}$`]);
  const quads = (F) => {
    const q = new Set();
  ...
  const g82e = [
    ...
    (r) => {
      ...
      const [pe, pm] = r.pick(ph);
      return { q: T(`${pe}<br>${blanks(xs)}`, `${pm}<br>${blanks(xs)}`), a: T(`$y = ${listTex(xs, F.f)}$`), w: W(...tableSteps(F, xs), `$y = ${listTex(xs, F.f)}$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      const [pe, pm] = r.pick(ph);
      return { q: T(`${pe}<br>${blanks(xs)}`, `${pm}<br>${blanks(xs)}`), fig: blankFig(), a: T(`$${ptsText(F, xs)}$; the graph is ${F.shape[0]}.`, `$${ptsText(F, xs)}$; graf itu ialah ${F.shape[1]}.`), w: W(...tableSteps(F, xs), T(`Plot $${ptsText(F, xs)}$ and join them smoothly.`, `Plotkan $${ptsText(F, xs)}$ dan sambungkannya dengan licin.`), T(`The graph is ${F.shape[0]}.`, `Graf itu ialah ${F.shape[1]}.`)), sp: 'xl' };
    },
    ...
    (r) => {
      ...
      return { q: TT(r.pick(ph)), fig: gfig([{ F }], { pts: [[x, F.f(x), '', true]] }), a: T(`$y = ${n(F.f(x))}$`), w: W(T(`Go to $x = ${x}$ on the horizontal axis and read the height of the graph there.`, `Pergi ke $x = ${x}$ pada paksi mengufuk dan baca ketinggian graf di situ.`), T(`Check with the equation: $${fsub(F, x)} = ${n(F.f(x))}$.`, `Semak dengan persamaan: $${fsub(F, x)} = ${n(F.f(x))}$.`), `$y = ${n(F.f(x))}$`), sp: 'xs' };
    },
    /* read x from a graph */
    (r) => {
      _r = r;
      ...
      return { q: T(`The graph of $${EQ(F)}$ is shown. Find the value(s) of $x$ when $y = ${y}$.`, `Graf bagi $${EQ(F)}$ ditunjukkan. Cari nilai $x$ apabila $y = ${y}$.`), fig: gfig([{ F }]), a: T(`$x = ${xsA.join(' \\text{ or } ')}$`, `$x = ${xsA.join(' \\text{ atau } ')}$`), w: W(T(`Draw the horizontal line $y = ${n(y)}$ and see where it meets the graph.`, `Lukis garis mengufuk $y = ${n(y)}$ dan lihat di mana ia bertemu graf.`), `$${F.t} = ${n(y)}$`, F.n === 2 ? T(`Both $x = ${-x}$ and $x = ${x}$ give $y = ${n(y)}$, because squaring removes the sign.`, `Kedua-dua $x = ${-x}$ dan $x = ${x}$ memberikan $y = ${n(y)}$, kerana pengkuasaduaan menghapuskan tanda.`) : T('The graph meets the line only once here.', 'Graf bertemu garis itu sekali sahaja di sini.'), T(`$x = ${xsA.join(' \\text{ or } ')}$`, `$x = ${xsA.join(' \\text{ atau } ')}$`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`${pe}<br>${list(0)}`, `${pm}<br>${list(1)}`), a: T(`(${L[cor]}) ${SHAPE[F.n][0]}`, `(${L[cor]}) ${SHAPE[F.n][1]}`), w: W(T(`In $y = ax^n$ the shape is decided by $n$; here $n = ${F.n}$.`, `Dalam $y = ax^n$, bentuk ditentukan oleh $n$; di sini $n = ${F.n}$.`), T(`That gives ${SHAPE[F.n][0]}.`, `Itu memberikan ${SHAPE[F.n][1]}.`), T(`Answer: (${L[cor]}).`, `Jawapan: (${L[cor]}).`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      need(on || y !== y0);
      return { q: T(`Does the point $(${x},\\ ${y})$ lie on the graph of $${EQ(F)}$? Show your working.`, `Adakah titik $(${x},\\ ${y})$ terletak pada graf $${EQ(F)}$? Tunjukkan langkah kerja anda.`), a: T(`${on ? 'Yes' : 'No'}: when $x = ${x}$, $y = ${n(y0)}$${on ? '' : `, not $${y}$`}.`, `${on ? 'Ya' : 'Tidak'}: apabila $x = ${x}$, $y = ${n(y0)}$${on ? '' : `, bukan $${y}$`}.`), w: W(T(`Substitute $x = ${x}$ into $${EQ(F)}$.`, `Gantikan $x = ${x}$ ke dalam $${EQ(F)}$.`), `$y = ${fsub(F, x)} = ${n(y0)}$`, on ? T(`This matches the given $y$-coordinate $${n(y)}$, so the point lies on the graph.`, `Ini sepadan dengan koordinat-$y$ yang diberi, $${n(y)}$, jadi titik itu terletak pada graf.`) : T(`This is not $${n(y)}$, so the point does not lie on the graph.`, `Ini bukan $${n(y)}$, jadi titik itu tidak terletak pada graf.`)), sp: 's' };
    },
    /* symmetry pair */
    (r) => {
      _r = r;
      const F = rndFN(r, [2, 2, 3, 1]), x = r.int(1, 3);
      return { q: T(`For $${EQ(F)}$, find $y$ when (a) $x = ${x}$, (b) $x = ${-x}$. What do you notice?`, `Bagi $${EQ(F)}$, cari $y$ apabila (a) $x = ${x}$, (b) $x = ${-x}$. Apakah yang anda perhatikan?`), w: W(`(a) $${fsub(F, x)} = ${n(F.f(x))}$`, `(b) $${fsub(F, -x)} = ${n(F.f(-x))}$`, F.n === 2 ? T('The two outputs are equal, so the graph is symmetrical about the $y$-axis.', 'Dua output itu sama, jadi graf simetri pada paksi-$y$.') : T('The two outputs are negatives of each other, so the graph is symmetrical about the origin.', 'Dua output itu ialah negatif antara satu sama lain, jadi graf simetri pada asalan.')), a: P([T(`$${n(F.f(x))}$`), T(`$${n(F.f(-x))}$`), F.n === 2 ? T('The two values are equal, so the graph is symmetrical about the $y$-axis.', 'Dua nilai itu sama, jadi graf simetri pada paksi-$y$.') : T('The two values are negatives of each other, so the graph is symmetrical about the origin.', 'Dua nilai itu ialah negatif antara satu sama lain, jadi graf simetri pada asalan.')]), sp: 's' };
    },
    /* table -> equation MCQ */
    (r) => {
      _r = r;
    ...
    (r) => {
      ...
      const pos = F.a > 0;
      const ans = F.n === 2 ? (pos ? T('Positive: the curve opens upwards (all $y \\ge 0$).', 'Positif: lengkung terbuka ke atas (semua $y \\ge 0$).') : T('Negative: the curve opens downwards (all $y \\le 0$).', 'Negatif: lengkung terbuka ke bawah (semua $y \\le 0$).')) : (pos ? T('Positive: the line rises from left to right.', 'Positif: garis naik dari kiri ke kanan.') : T('Negative: the line falls from left to right.', 'Negatif: garis turun dari kiri ke kanan.'));
      return { q: TT(r.pick(ph)), fig: gfig([{ F }]), a: ans, w: W(F.n === 2 ? T('In $y = ax^2$ the value of $x^2$ is never negative, so the sign of $y$ follows the sign of $a$.', 'Dalam $y = ax^2$ nilai $x^2$ tidak pernah negatif, jadi tanda $y$ mengikut tanda $a$.') : T('In $y = ax$ a positive $a$ makes the line rise from left to right and a negative $a$ makes it fall.', 'Dalam $y = ax$, $a$ positif menjadikan garis naik dari kiri ke kanan dan $a$ negatif menjadikannya turun.'), ans), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: TT(r.pick(ph)), a: T(`$x = 0$: the rule needs division by $0$, which is not defined, so the graph has no point at $x = 0$.`, `$x = 0$: peraturan itu memerlukan pembahagian dengan $0$ yang tidak ditakrifkan, jadi graf tidak mempunyai titik pada $x = 0$.`), w: W(T('Try each value in the rule.', 'Cuba setiap nilai dalam peraturan itu.'), T(`$x = -2$ and $x = 2$ give ordinary values, but $${fsub(F, 0)}$ is not defined.`, `$x = -2$ dan $x = 2$ memberikan nilai biasa, tetapi $${fsub(F, 0)}$ tidak ditakrifkan.`), T('Division by $0$ has no value, so $x = 0$ cannot be used and the graph has no point there.', 'Pembahagian dengan $0$ tiada nilai, jadi $x = 0$ tidak boleh digunakan dan graf tiada titik di situ.')), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`${c.en(a)} Find the value of the output when $x = ${x}$.`, `${c.ms(a)} Cari nilai output apabila $x = ${x}$.`), a: T(`${n(F.f(x))}`), w: W(T(`Substitute $x = ${x}$ into the relation.`, `Gantikan $x = ${x}$ ke dalam hubungan itu.`), `$${fsub(F, x)} = ${n(F.f(x))}$`, T(`The output is ${n(F.f(x))}.`, `Outputnya ialah ${n(F.f(x))}.`)), sp: 'xs' };
    },
  ];
  ...
  const g82m = [
    ...
    (r) => {
      ...
      const [pe, pm] = r.pick(ph);
      return { q: T(`${pe}<br>${blanks(xs)}`, `${pm}<br>${blanks(xs)}`), a: T(`$y = ${listTex(xs, (x) => ynum(F.f(x)))}$`), w: W(...tableSteps(F, xs), `$y = ${listTex(xs, (x) => ynum(F.f(x)))}$`), sp: 's' };
    },
    /* compare two members */
    (r) => {
      _r = r;
      ...
      return { q: T(`(a) Complete the tables for $y = ${F.t}$ and $y = ${G.t}$.<br>${blanks(xs, '$x$', `$y = ${F.t}$`)}<br>${blanks(xs, '$x$', `$y = ${G.t}$`)}(b) Compare the two rows of $y$-values.`, `(a) Lengkapkan jadual bagi $y = ${F.t}$ dan $y = ${G.t}$.<br>${blanks(xs, '$x$', `$y = ${F.t}$`)}<br>${blanks(xs, '$x$', `$y = ${G.t}$`)}(b) Bandingkan dua baris nilai $y$.`), a: P([T(`$y = ${F.t}$: $${listTex(xs, F.f)}$; $y = ${G.t}$: $${listTex(xs, G.f)}$`), cmp]), w: W(T('Substitute each value of $x$ into both rules.', 'Gantikan setiap nilai $x$ ke dalam kedua-dua peraturan.'), `$y = ${F.t}$: $${xs.map((x) => `${fsub(F, x)} = ${n(F.f(x))}`).join(',\\ ')}$`, `$y = ${G.t}$: $${xs.map((x) => `${fsub(G, x)} = ${n(G.f(x))}`).join(',\\ ')}$`, cmp), sp: 'm' };
    },
    ...
    (r) => {
      ...
      need(Math.abs(y) <= 7.5);
      return { q: T(`The graph of $${EQ(F)}$ is drawn. Use the graph to estimate the value of $y$ when $x = ${n(x)}$.`, `Graf bagi $${EQ(F)}$ dilukis. Gunakan graf untuk menganggar nilai $y$ apabila $x = ${n(x)}$.`), fig: gfig([{ F }], { pts: [[x, y, '', true]] }), a: T(`$y \\approx ${ynum(y)}$ (accept a small reading error)`, `$y \\approx ${ynum(y)}$ (terima ralat bacaan yang kecil)`), w: W(T(`Find $x = ${n(x)}$ on the horizontal axis and read the height of the curve there.`, `Cari $x = ${n(x)}$ pada paksi mengufuk dan baca ketinggian lengkung di situ.`), T(`By calculation: $${fsub(F, x)} = ${ynum(y)}$.`, `Melalui pengiraan: $${fsub(F, x)} = ${ynum(y)}$.`), `$y \\approx ${ynum(y)}$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      const sh = r.shuffle(cands), L = 'ABCD', cor = sh.indexOf(F);
      return { q: T(`The graph passes through the point $(${pt[0]},\\ ${n(pt[1])})$. Which equation represents the graph?<br>${sh.map((c, j) => `(${L[j]}) $${EQ(c)}$`).join('&emsp;')}`, `Graf melalui titik $(${pt[0]},\\ ${n(pt[1])})$. Persamaan yang manakah mewakili graf itu?<br>${sh.map((c, j) => `(${L[j]}) $${EQ(c)}$`).join('&emsp;')}`), fig: gfig([{ F }], { pts: [[pt[0], pt[1], `(${pt[0]}, ${n(pt[1])})`]] }), a: T(`(${L[cor]}) $${EQ(F)}$`), w: W(T(`Substitute the point into each equation: $${fsub(F, pt[0])} = ${n(pt[1])}$, so $${EQ(F)}$ fits.`, `Gantikan titik itu ke dalam setiap persamaan: $${fsub(F, pt[0])} = ${n(pt[1])}$, jadi $${EQ(F)}$ sesuai.`), T(`Its shape also matches the diagram: ${F.shape[0]}.`, `Bentuknya juga sepadan dengan rajah: ${F.shape[1]}.`), T(`Answer: (${L[cor]}).`, `Jawapan: (${L[cor]}).`)), sp: 's' };
    },
    /* two curves: match */
    (r) => {
      _r = r;
      ...
      return { q: T(`The diagram shows the graphs of $${EQ(F)}$ and $${EQ(G)}$. Which curve, $A$ (solid) or $B$ (dashed), is the graph of $${EQ(F)}$? Give a reason.`, `Rajah menunjukkan graf bagi $${EQ(F)}$ dan $${EQ(G)}$. Lengkung yang manakah, $A$ (penuh) atau $B$ (putus-putus), ialah graf bagi $${EQ(F)}$? Berikan sebab.`), fig: gfig(items), a: T(`Curve $${first === F ? 'A' : 'B'}$: it is ${F.shape[0]}.`, `Lengkung $${first === F ? 'A' : 'B'}$: ia ialah ${F.shape[1]}.`), w: W(T(`$${EQ(F)}$ has shape: ${F.shape[0]}.`, `$${EQ(F)}$ mempunyai bentuk: ${F.shape[1]}.`), T(`$${EQ(G)}$ has shape: ${G.shape[0]}.`, `$${EQ(G)}$ mempunyai bentuk: ${G.shape[1]}.`), T(`Curve $${first === F ? 'A' : 'B'}$ has the first shape, so it is $${EQ(F)}$.`, `Lengkung $${first === F ? 'A' : 'B'}$ mempunyai bentuk pertama, jadi ia ialah $${EQ(F)}$.`)), sp: 's' };
    },
    /* quadrants */
    (r) => {
      const F = rndFN(r), qs = quads(F);
      return { q: T(`In which quadrants does the graph of $${EQn(F)}$ lie? Explain using the signs of $x$ and $y$.`, `Dalam kuadran yang manakah graf $${EQn(F)}$ terletak? Terangkan menggunakan tanda bagi $x$ dan $y$.`), a: T(`The ${QN(qs)} quadrant${qs.length > 1 ? 's' : ''}.`, `Kuadran ${QM(qs)}.`), w: W(T('Take one positive and one negative value of $x$ and find the sign of $y$ each time.', 'Ambil satu nilai $x$ positif dan satu negatif, kemudian cari tanda $y$ setiap kali.'), `$x = 1$: $y = ${ynum(F.f(1))}$; $x = -1$: $y = ${ynum(F.f(-1))}$`, T(`So the graph lies in the ${QN(qs)} quadrant${qs.length > 1 ? 's' : ''}.`, `Jadi graf terletak dalam kuadran ${QM(qs)}.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Use the equation $${EQn(F)}$ to find every value of $x$ for which $y = ${n(y)}$. How many values are there?`, `Gunakan persamaan $${EQn(F)}$ untuk mencari setiap nilai $x$ yang memberikan $y = ${n(y)}$. Berapakah bilangan nilai itu?`), a: T(`$x = ${sol.join(' \\text{ or } ')}$ (${sol.length} value${sol.length > 1 ? 's' : ''})`, `$x = ${sol.join(' \\text{ atau } ')}$ (${sol.length} nilai)`), w: W(`$${F.t} = ${n(y)}$`, F.n % 2 === 0 ? T('The power of $x$ is even, so $x$ and $-x$ give the same $y$: there are two solutions.', 'Kuasa $x$ ialah genap, jadi $x$ dan $-x$ memberikan $y$ yang sama: terdapat dua penyelesaian.') : T('The power of $x$ is odd, so only one value of $x$ gives this $y$.', 'Kuasa $x$ ialah ganjil, jadi hanya satu nilai $x$ memberikan $y$ ini.'), T(`$x = ${sol.join(' \\text{ or } ')}$`, `$x = ${sol.join(' \\text{ atau } ')}$`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph of $y = ${nm}$ passes through the point $(${x0},\\ ${n(F.f(x0))})$. Find (a) the value of $a$, (b) the value of $y$ when $x = ${x1}$.`, `Graf $y = ${nm}$ melalui titik $(${x0},\\ ${n(F.f(x0))})$. Cari (a) nilai $a$, (b) nilai $y$ apabila $x = ${x1}$.`), a: P([T(`$a = ${a}$`), T(`$y = ${ynum(F.f(x1))}$`)]), w: W(T(`(a) Substitute the point into $y = ${nm}$.`, `(a) Gantikan titik itu ke dalam $y = ${nm}$.`), `$${fsubA(F, x0)} = ${n(F.f(x0))}$`, `$a = ${a}$`, `(b) $x = ${x1}$: $y = ${fsub(F, x1)} = ${ynum(F.f(x1))}$`), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A pupil completed this table for $${EQn(F)}$ but made one mistake. Find the wrong value and write the correct one.<br>${tb}`, `Seorang murid melengkapkan jadual ini bagi $${EQn(F)}$ tetapi membuat satu kesilapan. Cari nilai yang salah dan tulis nilai yang betul.<br>${tb}`), a: T(`When $x = ${xs[i]}$, $y$ should be $${ynum(F.f(xs[i]))}$, not $${ynum(ys[i])}$.`, `Apabila $x = ${xs[i]}$, $y$ sepatutnya $${ynum(F.f(xs[i]))}$, bukan $${ynum(ys[i])}$.`), w: W(T(`Recompute each entry from $${EQn(F)}$.`, `Hitung semula setiap nilai daripada $${EQn(F)}$.`), `$x = ${xs[i]}$: $${fsub(F, xs[i])} = ${ynum(F.f(xs[i]))}$`, T(`The table shows $${ynum(ys[i])}$ there, so that entry is wrong; the sign was dropped.`, `Jadual menunjukkan $${ynum(ys[i])}$ di situ, jadi nilai itu salah; tandanya tertinggal.`)), sp: 's' };
    },
    /* trend in a table */
    (r) => {
      _r = r;
    ...
    (r) => {
      ...
      return { q: T(`${c.en(a)} (a) Complete the table of values for $x = ${xs.join(', ')}$. (b) Find the value of $x$ that gives the output $${ynum(y)}$.<br>${blanks(xs)}`, `${c.ms(a)} (a) Lengkapkan jadual nilai bagi $x = ${xs.join(', ')}$. (b) Cari nilai $x$ yang memberikan output $${ynum(y)}$.<br>${blanks(xs)}`), a: P([T(`$y = ${xs.map((v) => ynum(F.f(v))).join(',\\ ')}$`), T(`$x = ${x}$`)]), w: W(T('(a) Substitute each value into the relation.', '(a) Gantikan setiap nilai ke dalam hubungan itu.'), `$${xs.map((v) => `${fsub(F, v)} = ${ynum(F.f(v))}`).join(',\\ ')}$`, T(`(b) The output $${ynum(y)}$ appears when $x = ${x}$.`, `(b) Output $${ynum(y)}$ muncul apabila $x = ${x}$.`)), sp: 'm' };
    },
    /* wrong joining */
    (r) => {
      _r = r;
  ...
  const g82a = [
    ...
    (r) => {
      ...
      const [pe, pm] = r.pick(ph);
      return { q: T(`${pe}<br>${blanks(xs)}`, `${pm}<br>${blanks(xs)}`), w: W(T('(a) Divide by each value of $x$ in turn.', '(a) Bahagikan dengan setiap nilai $x$ satu demi satu.'), `$${xs.map((x) => `${fsub(F, x)} = ${ynum(F.f(x))}`).join(',\\ ')}$`, T('(b) As $x$ gets closer to $0$ the divisor gets smaller, so $y$ becomes larger and larger without limit.', '(b) Apabila $x$ menghampiri $0$, pembahagi menjadi semakin kecil, jadi $y$ menjadi semakin besar tanpa had.'), T('(c) $x = 0$ would need division by $0$, which has no value, so there is no point on the $y$-axis.', '(c) $x = 0$ memerlukan pembahagian dengan $0$ yang tiada nilai, jadi tiada titik pada paksi-$y$.')), a: P([T(`$y = ${xs.map((x) => ynum(F.f(x))).join(',\\ ')}$`), T('$y$ becomes larger and larger (without any limit).', '$y$ menjadi semakin besar (tanpa had).'), T('$x = 0$ would need division by $0$, so there is no point on the $y$-axis; the curve only gets closer to it.', '$x = 0$ memerlukan pembahagian dengan $0$, jadi tiada titik pada paksi-$y$; lengkung hanya menghampirinya.')]), sp: 'l' };
    },
    /* positive and negative a */
    (r) => {
      _r = r;
      const nn = r.pick(NS), a = r.pick(nn < 0 ? [2, 4] : [1, 2, 3]), F = FN(nn, a), G = FN(nn, -a);
      const xs = r.pick(nn < 0 ? [[1, 2, 4]] : [[1, 2, 3]]);
      return { q: T(`(a) Find the values of $y$ for $y = ${F.t}$ and $y = ${G.t}$ when $x = ${xs.join(', ')}$. (b) State the quadrants in which each graph lies. (c) Describe how the two graphs are related.`, `(a) Cari nilai $y$ bagi $y = ${F.t}$ dan $y = ${G.t}$ apabila $x = ${xs.join(', ')}$. (b) Nyatakan kuadran yang dilalui oleh setiap graf. (c) Huraikan hubungan antara dua graf itu.`), fig: gfig([{ F, lab: 'A' }, { F: G, lab: 'B', dash: true, side: nn % 2 === 0 ? 1 : 1 }]), w: W(T('(a) Substitute each value of $x$ into both rules.', '(a) Gantikan setiap nilai $x$ ke dalam kedua-dua peraturan.'), `$${F.t}$: $${xs.map((x) => `${fsub(F, x)} = ${ynum(F.f(x))}`).join(',\\ ')}$`, `$${G.t}$: $${xs.map((x) => `${fsub(G, x)} = ${ynum(G.f(x))}`).join(',\\ ')}$`, T(`(b) The signs of $x$ and $y$ put $${F.t}$ in the ${QN(quads(F))} quadrant${quads(F).length > 1 ? 's' : ''} and $${G.t}$ in the ${QN(quads(G))} quadrant${quads(G).length > 1 ? 's' : ''}.`, `(b) Tanda $x$ dan $y$ meletakkan $${F.t}$ dalam kuadran ${QM(quads(F))} dan $${G.t}$ dalam kuadran ${QM(quads(G))}.`), T('(c) Each $y$-value of one is the negative of the other, so the graphs are reflections of each other in the $x$-axis.', '(c) Setiap nilai $y$ bagi satu ialah negatif bagi yang lain, jadi graf-graf itu ialah pantulan antara satu sama lain pada paksi-$x$.')), a: P([T(`$${F.t}$: $${listTex(xs, (x) => ynum(F.f(x)))}$; $${G.t}$: $${listTex(xs, (x) => ynum(G.f(x)))}$`), T(`$${F.t}$: ${QN(quads(F))} quadrant${quads(F).length > 1 ? 's' : ''}; $${G.t}$: ${QN(quads(G))} quadrant${quads(G).length > 1 ? 's' : ''}.`, `$${F.t}$: kuadran ${QM(quads(F))}; $${G.t}$: kuadran ${QM(quads(G))}.`), T('Each $y$-value of one graph is the negative of the other: the graphs are reflections of each other in the $x$-axis.', 'Setiap nilai $y$ bagi satu graf ialah negatif bagi yang lain: graf-graf itu ialah pantulan antara satu sama lain pada paksi-$x$.')]), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph of a function of the form $y = ax^n$, where $n \\in \\{-2, -1, 1, 2, 3\\}$, passes through the points $(${x1},\\ ${n(F.f(x1))})$ and $(${x2},\\ ${n(F.f(x2))})$. Its shape is ${F.shape[0]}. Find the equation of the graph.`, `Graf suatu fungsi berbentuk $y = ax^n$, dengan $n \\in \\{-2, -1, 1, 2, 3\\}$, melalui titik $(${x1},\\ ${n(F.f(x1))})$ dan $(${x2},\\ ${n(F.f(x2))})$. Bentuknya ialah ${F.shape[1]}. Cari persamaan graf itu.`), fig: gfig([{ F }], { pts: [[x1, F.f(x1), `(${x1}, ${n(F.f(x1))})`], [x2, F.f(x2), '']].filter((p) => Math.abs(p[1]) <= 7.5 && Math.abs(p[0]) <= 5) }), a: T(`$${EQ(F)}$`), w: W(T(`The shape is ${F.shape[0]}, which fits $n = ${F.n}$.`, `Bentuknya ialah ${F.shape[1]}, yang sepadan dengan $n = ${F.n}$.`), T(`Substitute the first point: $${fsubA(F, x1)} = ${n(F.f(x1))}$, so $a = ${F.a}$.`, `Gantikan titik pertama: $${fsubA(F, x1)} = ${n(F.f(x1))}$, jadi $a = ${F.a}$.`), T(`Check the second point: $${fsub(F, x2)} = ${n(F.f(x2))}$.`, `Semak titik kedua: $${fsub(F, x2)} = ${n(F.f(x2))}$.`), `$${EQ(F)}$`), sp: 'm' };
    },
    /* claims to critique */
    (r) => {
      const c = r.pick(CLAIM)(r);
      return { q: T(`${c[0]} Is the pupil correct? Explain.`, `${c[1]} Adakah murid itu betul? Terangkan.`), a: T(c[2], c[3]), w: W(T('Test the claim against the rule and against the shape of the graph.', 'Uji dakwaan itu dengan peraturan dan dengan bentuk graf.'), T(c[2], c[3])), sp: 's' };
    },
    /* x^2 versus x^3 */
    (r) => {
      const xs = [0.5, 1, 2, 3];
      return { q: T(`(a) Complete the table for $y = x^2$ and $y = x^3$. (b) For which value of $x$ in the table are the two outputs equal? (c) Which output is greater when $0 < x < 1$, and which when $x > 1$?<br>${blanks(xs, '$x$', '$x^2$')}${blanks(xs, '$x$', '$x^3$')}`, `(a) Lengkapkan jadual bagi $y = x^2$ dan $y = x^3$. (b) Bagi nilai $x$ yang manakah dalam jadual kedua-dua output itu sama? (c) Output yang manakah lebih besar apabila $0 < x < 1$, dan yang manakah apabila $x > 1$?<br>${blanks(xs, '$x$', '$x^2$')}${blanks(xs, '$x$', '$x^3$')}`), a: P([T('$x^2$: $0.25,\\ 1,\\ 4,\\ 9$; $x^3$: $0.125,\\ 1,\\ 8,\\ 27$'), T('$x = 1$ (both are $1$).', '$x = 1$ (kedua-duanya $1$).'), T('For $0 < x < 1$, $x^2$ is greater; for $x > 1$, $x^3$ is greater.', 'Bagi $0 < x < 1$, $x^2$ lebih besar; bagi $x > 1$, $x^3$ lebih besar.')]), w: W('(a) $x^2$: $(0.5)^2 = 0.25,\\ (1)^2 = 1,\\ (2)^2 = 4,\\ (3)^2 = 9$', '$x^3$: $(0.5)^3 = 0.125,\\ (1)^3 = 1,\\ (2)^3 = 8,\\ (3)^3 = 27$', T('(b) The two rows agree only at $x = 1$, where both give $1$.', '(b) Dua baris itu sama hanya pada $x = 1$, dengan kedua-duanya memberikan $1$.'), T('(c) $x^3 = x^2 \\times x$: multiplying by $x < 1$ makes it smaller, so $x^2$ is greater; multiplying by $x > 1$ makes it larger, so $x^3$ is greater.', '(c) $x^3 = x^2 \\times x$: mendarab dengan $x < 1$ menjadikannya lebih kecil, jadi $x^2$ lebih besar; mendarab dengan $x > 1$ menjadikannya lebih besar, jadi $x^3$ lebih besar.')), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A horizontal line $y = ${n(c)}$ is drawn on the same axes as the graph of $${EQn(F)}$. How many points of intersection are there, and what are the $x$-coordinates?`, `Garis mengufuk $y = ${n(c)}$ dilukis pada paksi yang sama dengan graf $${EQn(F)}$. Berapakah bilangan titik persilangan, dan apakah koordinat-$x$nya?`), a: sol.length ? T(`${sol.length} point${sol.length > 1 ? 's' : ''}: $x = ${sol.join(' \\text{ and } ')}$`, `${sol.length} titik: $x = ${sol.join(' \\text{ dan } ')}$`) : T(`None: every $y$-value of the graph has the opposite sign to $${n(c)}$, so the line never meets the graph.`, `Tiada: setiap nilai $y$ pada graf mempunyai tanda yang bertentangan dengan $${n(c)}$, jadi garis itu tidak pernah bertemu graf.`), w: sol.length ? W(T(`At an intersection the two $y$-values are equal, so solve $${F.t} = ${n(c)}$.`, `Pada persilangan, dua nilai $y$ adalah sama, jadi selesaikan $${F.t} = ${n(c)}$.`), F.n % 2 === 0 ? T('The power of $x$ is even, so $x$ and $-x$ both fit.', 'Kuasa $x$ ialah genap, jadi $x$ dan $-x$ kedua-duanya sesuai.') : T('The power of $x$ is odd, so only one value of $x$ fits.', 'Kuasa $x$ ialah ganjil, jadi hanya satu nilai $x$ sesuai.'), T(`$x = ${sol.join(' \\text{ and } ')}$: ${sol.length} point${sol.length > 1 ? 's' : ''} of intersection.`, `$x = ${sol.join(' \\text{ dan } ')}$: ${sol.length} titik persilangan.`)) : W(T(`The power of $x$ is even, so every $y$-value of $${EQn(F)}$ has the same sign as $a$.`, `Kuasa $x$ ialah genap, jadi setiap nilai $y$ bagi $${EQn(F)}$ mempunyai tanda yang sama dengan $a$.`), T(`$${n(c)}$ has the opposite sign, so no point of the graph has $y = ${n(c)}$.`, `$${n(c)}$ mempunyai tanda yang bertentangan, jadi tiada titik pada graf yang mempunyai $y = ${n(c)}$.`), T('The line never meets the graph: no points of intersection.', 'Garis itu tidak pernah bertemu graf: tiada titik persilangan.')), sp: 's' };
    },
    ...
    (r) => {
      ...
      const ynm = c.n < 0 ? 'y' : 'y';
      const sh = c.n < 0 ? '' : '';
      return { q: T(`${c.ge(a, x0, k2)} (a) Find the constant. (b) Find the output when the input is ${x1}. (c) Find the input when the output is ${n(yOut)}. (d) Explain why the input cannot be negative here.`, `${c.gm(a, x0, k2)} (a) Cari pemalar itu. (b) Cari output apabila input ialah ${x1}. (c) Cari input apabila output ialah ${n(yOut)}. (d) Terangkan mengapa input tidak boleh negatif di sini.`), w: (() => { const sym = c.n < 0 ? 'k' : 'a'; return W(T(`(a) Substitute the given pair: $${fsubA(G, x0, sym)} = ${ynum(k2)}$, so $${sym} = ${n(k)}$.`, `(a) Gantikan pasangan yang diberi: $${fsubA(G, x0, sym)} = ${ynum(k2)}$, jadi $${sym} = ${n(k)}$.`), `(b) $x = ${x1}$: $${fsub(G, x1)} = ${ynum(y1)}$`, c.n < 0 ? T(`(c) $\\dfrac{${n(k)}}{x} = ${n(yOut)}$, so $x = ${n(k)} \\div ${n(yOut)} = ${n(xRes)}$.`, `(c) $\\dfrac{${n(k)}}{x} = ${n(yOut)}$, jadi $x = ${n(k)} \\div ${n(yOut)} = ${n(xRes)}$.`) : T(`(c) $${n(k)}x^2 = ${n(yOut)}$, so $x^2 = ${n(yOut / k)}$ and $x = ${n(xRes)}$.`, `(c) $${n(k)}x^2 = ${n(yOut)}$, jadi $x^2 = ${n(yOut / k)}$ dan $x = ${n(xRes)}$.`), T('(d) The input is a time, a speed or a number of people, which cannot be negative.', '(d) Input ialah masa, laju atau bilangan orang yang tidak boleh negatif.')); })(), a: P([T(`$${c.n < 0 ? 'k' : 'a'} = ${n(k)}$`), T(`$${ynum(y1)}$`), T(`$${n(xRes)}$`), T('The input is a time, speed or number of people, which cannot be negative.', 'Input ialah masa, laju atau bilangan orang yang tidak boleh negatif.')]), sp: 'l' };
    },
    ...
    (r) => {
      ...
      const L = 'ABCD';
      return { q: T(`Three of the four points below lie on the graph of $${EQ(F)}$. Which point does not? Give the correct $y$-coordinate.<br>${pts.map((p, i) => `(${L[i]}) $(${p[0]},\\ ${n(p[1])})$`).join('&emsp;')}`, `Tiga daripada empat titik di bawah terletak pada graf $${EQ(F)}$. Titik yang manakah tidak? Berikan koordinat-$y$ yang betul.<br>${pts.map((p, i) => `(${L[i]}) $(${p[0]},\\ ${n(p[1])})$`).join('&emsp;')}`), a: T(`(${L[bad]}) $(${pts[bad][0]},\\ ${n(pts[bad][1])})$; the correct $y$ is $${n(F.f(xs[bad]))}$.`, `(${L[bad]}) $(${pts[bad][0]},\\ ${n(pts[bad][1])})$; $y$ yang betul ialah $${n(F.f(xs[bad]))}$.`), w: W(T(`Substitute each $x$-coordinate into $${EQ(F)}$.`, `Gantikan setiap koordinat-$x$ ke dalam $${EQ(F)}$.`), `$${xs.map((x) => `${fsub(F, x)} = ${n(F.f(x))}`).join(',\\ ')}$`, T(`Only (${L[bad]}) disagrees with the table of values: the correct $y$ there is $${n(F.f(xs[bad]))}$.`, `Hanya (${L[bad]}) tidak sepadan: $y$ yang betul di situ ialah $${n(F.f(xs[bad]))}$.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Can the graph of $${EQn(F)}$ have a point with $y = ${cs[0]}$? Can it have a point with $y = ${cs[1]}$? Explain each answer.`, `Bolehkah graf $${EQn(F)}$ mempunyai titik dengan $y = ${cs[0]}$? Bolehkah ia mempunyai titik dengan $y = ${cs[1]}$? Terangkan setiap jawapan.`), a: P([why2(cs[0]), why2(cs[1])]), w: W(T(`The sign of $y$ in $${EQn(F)}$ is fixed by the sign of $a$ and by the power of $x$.`, `Tanda $y$ dalam $${EQn(F)}$ ditentukan oleh tanda $a$ dan oleh kuasa $x$.`), T(`$y = ${cs[0]}$: ${why2(cs[0]).en}`, `$y = ${cs[0]}$: ${why2(cs[0]).ms}`), T(`$y = ${cs[1]}$: ${why2(cs[1]).en}`, `$y = ${cs[1]}$: ${why2(cs[1]).ms}`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      need(Math.abs(y) <= 7.5);
      const lo = Math.floor(x), hi = lo + 1, avg0 = (F.f(lo) + F.f(hi)) / 2;
      return { w: W(T(`(a) Read the curve at $x = ${n(x)}$: about $${ynum(y)}$.`, `(a) Baca lengkung pada $x = ${n(x)}$: kira-kira $${ynum(y)}$.`), `(b) $${fsub(F, x)} = ${ynum(y)}$`, `(c) $\\dfrac{${ynum(F.f(lo))} + ${par(F.f(hi))}}{2} = ${ynum(avg0)}$`, T(`That is ${avg0 > y ? 'too large' : avg0 < y ? 'too small' : 'exact'}: the graph is curved, so it does not follow the straight line between the two points.`, `Itu ${avg0 > y ? 'terlalu besar' : avg0 < y ? 'terlalu kecil' : 'tepat'}: graf itu melengkung, jadi ia tidak mengikut garis lurus antara dua titik itu.`)), q: T(`The graph of $${EQ(F)}$ is drawn. (a) Estimate $y$ when $x = ${n(x)}$ from the graph. (b) Calculate the exact value. (c) The pupil estimated $y$ by taking the average of $y$ at $x = ${lo}$ and $x = ${hi}$. Is this estimate too large or too small? Explain briefly.`, `Graf $${EQ(F)}$ dilukis. (a) Anggarkan $y$ apabila $x = ${n(x)}$ daripada graf. (b) Hitung nilai tepat. (c) Murid itu menganggar $y$ dengan mengambil purata $y$ pada $x = ${lo}$ dan $x = ${hi}$. Adakah anggaran ini terlalu besar atau terlalu kecil? Terangkan secara ringkas.`), fig: gfig([{ F }], { pts: [[x, y, '', true]] }), a: (() => { const avg = (F.f(lo) + F.f(hi)) / 2; return P([T(`About $${ynum(y)}$`, `Kira-kira $${ynum(y)}$`), T(`$y = ${ynum(y)}$`), T(`The average is $${ynum(avg)}$, which is ${avg > y ? 'too large' : avg < y ? 'too small' : 'exact'}: the graph is curved, so it does not follow the straight line between the two points.`, `Puratanya ialah $${ynum(avg)}$, iaitu ${avg > y ? 'terlalu besar' : avg < y ? 'terlalu kecil' : 'tepat'}: graf itu melengkung, jadi ia tidak mengikut garis lurus antara dua titik itu.`)]); })(), sp: 'm' };
    },
  ];
  ...
  const g82e2 = [
    /* cloze */
    (r) => { const c = r.pick(CL82); return { q: T(`Fill in the blank(s).<br>${c[0]}`, `Isikan tempat kosong.<br>${c[1]}`), a: T(c[2], c[3]), w: W(T(`The completed sentence reads: ${fillBlanks(c[0], c[2])}`, `Ayat yang lengkap berbunyi: ${fillBlanks(c[1], c[3])}`)), sp: 'xs' }; },
    ...
    (r) => {
      ...
      return { q: T(pe, pm), a: ab ? (s > 0 ? T('Above the $x$-axis: $y > 0$.', 'Di atas paksi-$x$: $y > 0$.') : T('Below the $x$-axis: $y < 0$.', 'Di bawah paksi-$x$: $y < 0$.')) : T(cap(sgnWord(s).en), cap(sgnWord(s).ms)), w: W(T(`Here $a = ${F.a}$ (${F.a > 0 ? 'positive' : 'negative'}) and $x = ${x}$ (${x > 0 ? 'positive' : 'negative'}).`, `Di sini $a = ${F.a}$ (${F.a > 0 ? 'positif' : 'negatif'}) dan $x = ${x}$ (${x > 0 ? 'positif' : 'negatif'}).`), F.n === 2 ? T('Squaring makes $x^2$ positive, so $y$ takes the sign of $a$.', 'Pengkuasaduaan menjadikan $x^2$ positif, jadi $y$ mengambil tanda $a$.') : T(`The power of $x$ is odd, so ${F.n === 1 ? '$x$' : `$x^{${F.n}}$`} keeps the sign of $x$; the sign of $y$ is the sign of $a$ combined with the sign of $x$.`, `Kuasa $x$ ialah ganjil, jadi ${F.n === 1 ? '$x$' : `$x^{${F.n}}$`} mengekalkan tanda $x$; tanda $y$ ialah gabungan tanda $a$ dengan tanda $x$.`), T(`Check: $${fsub(F, x)} = ${ynum(F.f(x))}$, which is ${s > 0 ? 'positive' : 'negative'}.`, `Semak: $${fsub(F, x)} = ${ynum(F.f(x))}$, iaitu ${s > 0 ? 'positif' : 'negatif'}.`)), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph of $${EQ(F)}$ passes through the points $P$ and $Q$. Write down the coordinates of $P$ and $Q$.`, `Graf $${EQ(F)}$ melalui titik $P$ dan $Q$. Tuliskan koordinat $P$ dan $Q$.`), fig: gfig([{ F }], { pts: pts.map((p) => [p[0], p[1], p[2]]) }), a: T(`$P(${pts[0][0]},\\ ${n(pts[0][1])})$, $Q(${pts[1][0]},\\ ${n(pts[1][1])})$`), w: W(T('Read the $x$-coordinate of each marked point, then use the equation to confirm its $y$-coordinate.', 'Baca koordinat-$x$ bagi setiap titik yang ditanda, kemudian gunakan persamaan untuk mengesahkan koordinat-$y$nya.'), `$P$: $${fsub(F, pts[0][0])} = ${n(pts[0][1])}$`, `$Q$: $${fsub(F, pts[1][0])} = ${n(pts[1][1])}$`, `$P(${pts[0][0]},\\ ${n(pts[0][1])})$, $Q(${pts[1][0]},\\ ${n(pts[1][1])})$`), sp: 's' };
    },
    /* matching equations and shapes */
    (r) => {
      const ns3 = r.sample(NS, 3), Fs = ns3.map((k) => FN(k, k < 0 ? 4 : r.pick([1, 2, 3]))), sh = r.shuffle([0, 1, 2]);
      return { q: T(`Match each equation with the description of its graph.<br>${Fs.map((F, i) => `(${i + 1}) $${EQn(F)}$`).join('&emsp;')}<br>${sh.map((k, j) => `(${'ABC'[j]}) ${Fs[k].shape[0]}`).join('<br>')}`, `Padankan setiap persamaan dengan huraian grafnya.<br>${Fs.map((F, i) => `(${i + 1}) $${EQn(F)}$`).join('&emsp;')}<br>${sh.map((k, j) => `(${'ABC'[j]}) ${Fs[k].shape[1]}`).join('<br>')}`), a: T(Fs.map((F, i) => `(${i + 1}) - (${'ABC'[sh.indexOf(i)]})`).join(', ')), w: W(T('The shape of $y = ax^n$ depends only on $n$, so match by the power.', 'Bentuk $y = ax^n$ bergantung pada $n$ sahaja, jadi padankan mengikut kuasa.'), ...Fs.map((F, i) => T(`(${i + 1}) $${EQn(F)}$ has $n = ${F.n}$: ${F.shape[0]}.`, `(${i + 1}) $${EQn(F)}$ mempunyai $n = ${F.n}$: ${F.shape[1]}.`)), T(Fs.map((F, i) => `(${i + 1}) - (${'ABC'[sh.indexOf(i)]})`).join(', '))), sp: 's' };
    },
    /* y-intercept style: x = 0 */
    (r) => {
      const F = rndFN(r, [1, 2, 3], [1, 2, 3, -1, -2, -3]), G = rndFN(r, [-1, -2], [2, 4, 6]);
      return { q: T(`(a) Find $y$ when $x = 0$ for $${EQ(F)}$. (b) Can we find $y$ when $x = 0$ for $${EQn(G)}$? Explain.`, `(a) Cari $y$ apabila $x = 0$ bagi $${EQ(F)}$. (b) Bolehkah kita mencari $y$ apabila $x = 0$ bagi $${EQn(G)}$? Terangkan.`), a: P([T('$y = 0$: the graph passes through the origin.', '$y = 0$: graf melalui asalan.'), T('No: it needs division by $0$, which is not defined.', 'Tidak: ia memerlukan pembahagian dengan $0$ yang tidak ditakrifkan.')]), w: W(T(`(a) Substitute $x = 0$ into $${EQ(F)}$: $${fsub(F, 0)} = 0$, so the graph passes through the origin.`, `(a) Gantikan $x = 0$ ke dalam $${EQ(F)}$: $${fsub(F, 0)} = 0$, jadi graf melalui asalan.`), T(`(b) For $${EQ(G)}$, $x = 0$ gives $${fsub(G, 0)}$.`, `(b) Bagi $${EQ(G)}$, $x = 0$ memberikan $${fsub(G, 0)}$.`), T('Division by $0$ is not defined, so there is no value of $y$ at $x = 0$.', 'Pembahagian dengan $0$ tidak ditakrifkan, jadi tiada nilai $y$ pada $x = 0$.')), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Which of these graphs pass through the point $(${x},\\ ${n(F.f(x))})$?<br>${sh.map((c, j) => `(${L[j]}) $${EQ(c)}$`).join('&emsp;')}`, `Graf yang manakah melalui titik $(${x},\\ ${n(F.f(x))})$?<br>${sh.map((c, j) => `(${L[j]}) $${EQ(c)}$`).join('&emsp;')}`), a: T(hits.map((j) => `(${L[j]})`).join(', ')), w: W(T(`Substitute $x = ${x}$ into each equation and compare with $${n(F.f(x))}$.`, `Gantikan $x = ${x}$ ke dalam setiap persamaan dan bandingkan dengan $${n(F.f(x))}$.`), T(sh.map((c, j) => `(${L[j]}) $${fsub(c, x)} = ${ynum(c.f(x))}$`).join('; ')), T(`So ${hits.map((j) => `(${L[j]})`).join(', ')} pass through the point.`, `Jadi ${hits.map((j) => `(${L[j]})`).join(', ')} melalui titik itu.`)), sp: 's' };
    },
  ];
  const g82m2 = [
    ...
    (r) => {
      ...
      return { q: T(`Given that $y = ${nm}$ and $y = ${n(F.f(x0))}$ when $x = ${x0}$, find $a$ and complete the table.<br>${blanks(xs)}`, `Diberi $y = ${nm}$ dan $y = ${n(F.f(x0))}$ apabila $x = ${x0}$, cari $a$ dan lengkapkan jadual.<br>${blanks(xs)}`), a: T(`$a = ${F.a}$; $y = ${xs.map((x) => ynum(F.f(x))).join(',\\ ')}$`), w: W(T(`Substitute the given pair into $y = ${nm}$: $${fsubA(F, x0)} = ${n(F.f(x0))}$, so $a = ${F.a}$.`, `Gantikan pasangan yang diberi ke dalam $y = ${nm}$: $${fsubA(F, x0)} = ${n(F.f(x0))}$, jadi $a = ${F.a}$.`), ...tableSteps(F, xs), `$y = ${xs.map((x) => ynum(F.f(x))).join(',\\ ')}$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The diagram shows the graphs of $y = ${F.t}$ (solid, labelled $A$) and $y = ${G.t}$ (dashed, labelled $B$). At $x = ${x}$, which graph has the greater $y$-value? Give both values.`, `Rajah menunjukkan graf $y = ${F.t}$ (penuh, berlabel $A$) dan $y = ${G.t}$ (putus-putus, berlabel $B$). Pada $x = ${x}$, graf yang manakah mempunyai nilai $y$ yang lebih besar? Berikan kedua-dua nilai.`), fig: gfig([{ F, lab: 'A' }, { F: G, lab: 'B', dash: true }]), a: T(`$A$: $${n(F.f(x))}$; $B$: $${n(G.f(x))}$. Graph ${big === F ? 'A' : 'B'} is greater.`, `$A$: $${n(F.f(x))}$; $B$: $${n(G.f(x))}$. Graf ${big === F ? 'A' : 'B'} lebih besar.`), w: W(T(`Substitute $x = ${x}$ into each equation.`, `Gantikan $x = ${x}$ ke dalam setiap persamaan.`), `$A$: $${fsub(F, x)} = ${n(F.f(x))}$`, `$B$: $${fsub(G, x)} = ${n(G.f(x))}$`, T(`$${n(big.f(x))}$ is the larger value, so graph ${big === F ? 'A' : 'B'} is greater at $x = ${x}$.`, `$${n(big.f(x))}$ ialah nilai yang lebih besar, jadi graf ${big === F ? 'A' : 'B'} lebih besar pada $x = ${x}$.`)), sp: 's' };
    },
    /* decide function type from equation and graph shape */
    (r) => {
      const F = rndFN(r), one = F.n % 2 !== 0 && true, oneR = F.n === 1 || F.n === 3 || F.n === -1;
      return { q: T(`Consider the graph of $${EQn(F)}$ (${F.shape[0]}). Is it a function? If so, is it one-to-one or many-to-one? Use the vertical and horizontal line tests.`, `Pertimbangkan graf $${EQn(F)}$ (${F.shape[1]}). Adakah ia suatu fungsi? Jika ya, adakah ia satu dengan satu atau banyak dengan satu? Gunakan ujian garis mencancang dan mengufuk.`), a: oneR ? T('A function, and one-to-one: every vertical and every horizontal line meets the graph at most once.', 'Ia fungsi, dan satu dengan satu: setiap garis mencancang dan mengufuk memotong graf paling banyak sekali.') : T('A function, but many-to-one: a horizontal line can meet the graph at two points, e.g. inputs $x$ and $-x$ have the same output.', 'Ia fungsi, tetapi banyak dengan satu: garis mengufuk boleh memotong graf pada dua titik, contohnya input $x$ dan $-x$ mempunyai output yang sama.'), w: W(T('Every value of $x$ that is allowed gives exactly one $y$, so a vertical line meets the graph at most once: it is a function.', 'Setiap nilai $x$ yang dibenarkan memberikan tepat satu $y$, jadi garis mencancang memotong graf paling banyak sekali: ia suatu fungsi.'), oneR ? T(`The power of $x$ is odd, so different values of $x$ always give different values of $y$.`, `Kuasa $x$ ialah ganjil, jadi nilai $x$ yang berbeza sentiasa memberikan nilai $y$ yang berbeza.`) : T(`The power of $x$ is even, so $x$ and $-x$ give the same $y$ and a horizontal line can meet the graph twice.`, `Kuasa $x$ ialah genap, jadi $x$ dan $-x$ memberikan $y$ yang sama dan garis mengufuk boleh memotong graf dua kali.`), T(oneR ? 'A one-to-one function.' : 'A many-to-one function.', oneR ? 'Fungsi satu dengan satu.' : 'Fungsi banyak dengan satu.')), sp: 's' };
    },
  ...
  const g82a2 = [
    ...
    (r) => {
      ...
      const x = 2;
      return { q: T(`A graph has the form $y = ax^n$ with $n \\in \\{-2, -1, 1, 2, 3\\}$. ${props.map((p) => p.en).join(' ')} It passes through $(${x},\\ ${ynum(F.f(x))})$. Find the equation.`, `Suatu graf berbentuk $y = ax^n$ dengan $n \\in \\{-2, -1, 1, 2, 3\\}$. ${props.map((p) => p.ms).join(' ')} Ia melalui $(${x},\\ ${ynum(F.f(x))})$. Cari persamaannya.`), a: T(`$${EQ(F)}$`), w: W(T(`The listed properties fit only $n = ${F.n}$ (${F.shape[0]}).`, `Sifat yang disenaraikan hanya menepati $n = ${F.n}$ (${F.shape[1]}).`), T(`Substitute the point: $${fsubA(F, x)} = ${ynum(F.f(x))}$, so $a = ${F.a}$.`, `Gantikan titik itu: $${fsubA(F, x)} = ${ynum(F.f(x))}$, jadi $a = ${F.a}$.`), `$${EQ(F)}$`), sp: 'm' };
    },
    /* large x behaviour */
    (r) => {
      const F = FN(r.pick([-1, -2]), r.pick([1, 2, 5, 10])), xs = [10, 100, 1000];
      return { q: T(`(a) Complete the table for $y = ${F.t}$. (b) What happens to $y$ as $x$ becomes very large? (c) Will the graph ever touch the $x$-axis? Explain.<br>${blanks(xs)}`, `(a) Lengkapkan jadual bagi $y = ${F.t}$. (b) Apakah yang berlaku kepada $y$ apabila $x$ menjadi sangat besar? (c) Adakah graf itu akan menyentuh paksi-$x$? Terangkan.<br>${blanks(xs)}`), w: W(T('(a) Divide by each value of $x$.', '(a) Bahagikan dengan setiap nilai $x$.'), `$${xs.map((x) => `${fsub(F, x)} = ${n(F.f(x))}`).join(',\\ ')}$`, T('(b) As $x$ grows the divisor grows, so $y$ becomes smaller and closer to $0$.', '(b) Apabila $x$ bertambah, pembahagi bertambah, jadi $y$ menjadi semakin kecil dan menghampiri $0$.'), T(`(c) A fraction with a non-zero numerator is never exactly $0$, so the graph approaches the $x$-axis without touching it.`, `(c) Pecahan yang pengangkanya bukan sifar tidak pernah tepat $0$, jadi graf menghampiri paksi-$x$ tanpa menyentuhnya.`)), a: P([T(`$y = ${xs.map((x) => n(F.f(x))).join(',\\ ')}$`), T('$y$ becomes smaller and closer to $0$.', '$y$ menjadi semakin kecil dan menghampiri $0$.'), T(`No: $\\dfrac{${F.a}}{x${F.n === -2 ? '^2' : ''}}$ is never $0$ for any $x$.`, `Tidak: $\\dfrac{${F.a}}{x${F.n === -2 ? '^2' : ''}}$ tidak pernah $0$ bagi sebarang $x$.`)]), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph shows $s = ${a}t^2$, the distance $s$ metres fallen by a stone after $t$ seconds. (a) Use the equation to find $s$ when $t = ${t}$. (b) The stone has fallen ${n(a * 9)} m. How long has it been falling? (c) Explain why the graph is drawn only for $t \\ge 0$.`, `Graf menunjukkan $s = ${a}t^2$, jarak $s$ meter yang dijatuhi sebiji batu selepas $t$ saat. (a) Gunakan persamaan untuk mencari $s$ apabila $t = ${t}$. (b) Batu itu telah jatuh sejauh ${n(a * 9)} m. Berapa lamakah ia telah jatuh? (c) Terangkan mengapa graf hanya dilukis bagi $t \\ge 0$.`), fig, w: W(`(a) $s = ${a} \\times (${t})^2 = ${n(s)}$`, `(b) $${a}t^2 = ${n(a * 9)}$, $t^2 = 9$, $t = 3$`, T('(c) $t$ is a time measured from the moment the stone is released, and a time cannot be negative.', '(c) $t$ ialah masa yang diukur dari saat batu itu dilepaskan, dan masa tidak boleh negatif.')), a: P([T(`$s = ${n(s)}$ m`), T('$t = 3$ s', '$t = 3$ s'), T('Time cannot be negative.', 'Masa tidak boleh negatif.')]), sp: 'm' };
    },
  ];
  ...
  const g82cxE = [
    (r) => {
      ...
      const [pe, pm] = r.pick(ph);
      return { q: T(`${k.intro.en} ${pe}<br>${cxTable(k, xs, xs, true)}`, `${k.intro.ms} ${pm}<br>${cxTableM(k, xs, xs, true)}`), a: T(`${xs.map((x) => n(round(k.F.f(x), 2))).join(', ')}`), w: W(T(`Use the relation $${k.c.rel(k.a)}$ for each value in the top row.`, `Gunakan hubungan $${k.c.rel(k.a)}$ bagi setiap nilai dalam baris atas.`), `$${xs.map((x) => `${fsub(k.F, x)} = ${n(round(k.F.f(x), 2))}`).join(',\\ ')}$`), sp: 's' };
    },
    (r) => {
      ...
      const p2 = r.pick(ph);
      return { q: T(`${k.intro.en} ${p2[0]}`, `${k.intro.ms} ${p2[1]}`), a: T(`${n(round(k.F.f(x), 2))}`), w: W(T(`The relation is $${k.c.rel(k.a)}$. Substitute $x = ${n(x)}$.`, `Hubungan itu ialah $${k.c.rel(k.a)}$. Gantikan $x = ${n(x)}$.`), `$${fsub(k.F, x)} = ${n(round(k.F.f(x), 2))}$`), sp: 'xs' };
    },
    (r) => {
      const k = mkCtx(r), x = r.pick(k.xs);
      return { q: T(`${k.intro.en} Which graph describes the relation between the ${k.xn.en} and the ${k.yn.en}: a straight line through the origin, a U-shaped curve, an S-shaped curve or a curve with two branches?`, `${k.intro.ms} Graf yang manakah menggambarkan hubungan antara ${k.xn.ms} dengan ${k.yn.ms}: garis lurus melalui asalan, lengkung berbentuk U, lengkung berbentuk S atau lengkung dua cabang?`), a: T(cap(k.F.shape[0]) + (k.F.n < 0 ? ' (only $x > 0$ makes sense here)' : ''), cap(k.F.shape[1]) + (k.F.n < 0 ? ' (hanya $x > 0$ bermakna di sini)' : '')), w: W(T(`The relation is $${k.c.rel(k.a)}$, so the power of $x$ is ${k.F.n}.`, `Hubungan itu ialah $${k.c.rel(k.a)}$, jadi kuasa $x$ ialah ${k.F.n}.`), T(`A graph of that form is ${k.F.shape[0]}.`, `Graf berbentuk itu ialah ${k.F.shape[1]}.`)), sp: 's' };
    },
  ];
  const g82cxM = [
    (r) => {
      ...
      const [pe, pm] = r.pick(ph);
      const nn0 = k.F.n, aa0 = k.a;
      const solveLine = nn0 === 1 ? `$${n(aa0)}x = ${ynum(y)}$, so $x = ${ynum(y)} \\div ${n(aa0)} = ${n(x)}$`
        : nn0 === 2 ? `$${n(aa0)}x^2 = ${ynum(y)}$, so $x^2 = ${n(y / aa0)}$ and $x = ${n(x)}$`
          : nn0 === 3 ? `$${n(aa0)}x^3 = ${ynum(y)}$, so $x^3 = ${n(y / aa0)}$ and $x = ${n(x)}$`
            : nn0 === -1 ? `$\\dfrac{${n(aa0)}}{x} = ${ynum(y)}$, so $x = ${n(aa0)} \\div ${ynum(y)} = ${n(x)}$`
              : `$\\dfrac{${n(aa0)}}{x^2} = ${ynum(y)}$, so $x^2 = ${n(aa0 / y)}$ and $x = ${n(x)}$`;
      const solveLineM = solveLine.replace(/, so /g, ', jadi ').replace(/ and /g, ' dan ');
      return { q: T(`${k.intro.en} ${pe}`, `${k.intro.ms} ${pm}`), a: T(`${n(x)}`), w: W(T(`Set the relation $${k.c.rel(k.a)}$ equal to ${ynum(y)} and solve.`, `Samakan hubungan $${k.c.rel(k.a)}$ dengan ${ynum(y)} dan selesaikan.`), T(solveLine, solveLineM)), sp: 's' };
    },
    (r) => {
      const k = mkCtx(r), xs = k.xs.slice(0, 4), ys = xs.map(k.F.f);
      const up = ys[3] > ys[0];
      return { q: T(`${k.intro.en} (a) Complete the table. (b) State whether the ${k.yn.en} increases or decreases as the ${k.xn.en} increases. (c) Is the graph a straight line? Give a reason.<br>${cxTable(k, xs, xs, true)}`, `${k.intro.ms} (a) Lengkapkan jadual. (b) Nyatakan sama ada ${k.yn.ms} bertambah atau berkurang apabila ${k.xn.ms} bertambah. (c) Adakah graf itu garis lurus? Berikan sebab.<br>${cxTableM(k, xs, xs, true)}`), a: P([T(ys.map((v) => n(round(v, 2))).join(', ')), up ? T('It increases.', 'Ia bertambah.') : T('It decreases.', 'Ia berkurang.'), k.F.n === 1 ? T('Yes: the $y$-value increases by the same amount for equal steps in $x$ (a line through the origin).', 'Ya: nilai $y$ bertambah dengan jumlah yang sama bagi langkah $x$ yang sama (garis melalui asalan).') : T('No: equal steps in $x$ do not give equal changes in $y$, so it is a curve.', 'Tidak: langkah $x$ yang sama tidak memberikan perubahan $y$ yang sama, jadi ia ialah lengkung.')]), w: W(T(`(a) Use $${k.c.rel(k.a)}$ for each value of $x$.`, `(a) Gunakan $${k.c.rel(k.a)}$ bagi setiap nilai $x$.`), `$${xs.map((x) => `${fsub(k.F, x)} = ${n(round(k.F.f(x), 2))}`).join(',\\ ')}$`, T(`(b) The values ${up ? 'increase' : 'decrease'} as $x$ increases.`, `(b) Nilai itu ${up ? 'bertambah' : 'berkurang'} apabila $x$ bertambah.`), k.F.n === 1 ? T('(c) Yes: equal steps in $x$ give equal changes in $y$, so the points lie on a straight line through the origin.', '(c) Ya: langkah $x$ yang sama memberikan perubahan $y$ yang sama, jadi titik-titik itu terletak pada garis lurus melalui asalan.') : T('(c) No: equal steps in $x$ do not give equal changes in $y$, so the graph is a curve.', '(c) Tidak: langkah $x$ yang sama tidak memberikan perubahan $y$ yang sama, jadi graf itu ialah lengkung.')), sp: 'm' };
    },
    (r) => {
      const k = mkCtx(r), x = r.pick(k.xs), y = k.F.f(x);
      return { q: T(`${k.intro.en} A pupil says: "The ${k.yn.en} is ${ynum(y * 2)} when the ${k.xn.en} is ${n(x)}." Check this using the relation and give the correct value.`, `${k.intro.ms} Seorang murid berkata: "${cap(k.yn.ms)} ialah ${ynum(y * 2)} apabila ${k.xn.ms} ialah ${n(x)}." Semak kenyataan ini menggunakan hubungan itu dan berikan nilai yang betul.`), a: T(`Not correct: the ${k.yn.en} is ${ynum(y)}.`, `Tidak betul: ${k.yn.ms} ialah ${ynum(y)}.`), w: W(T(`Substitute $x = ${n(x)}$ into $${k.c.rel(k.a)}$.`, `Gantikan $x = ${n(x)}$ ke dalam $${k.c.rel(k.a)}$.`), `$${fsub(k.F, x)} = ${ynum(y)}$`, T(`The pupil gave ${ynum(y * 2)}, which is twice the correct value, so the claim is wrong.`, `Murid itu memberikan ${ynum(y * 2)}, iaitu dua kali nilai yang betul, jadi dakwaan itu salah.`)), sp: 's' };
    },
  ];
  const g82cxA = [
    (r) => {
      ...
      return { q: T(`The relation between the ${k.xn.en} ($x$) and the ${k.yn.en} ($y$) has the form $${nm}$. When $x = ${n(x0)}$, $y = ${ynum(y0)}$. Find $a$, and use it to find $y$ when $x = ${n(x1)}$.`, `Hubungan antara ${k.xn.ms} ($x$) dengan ${k.yn.ms} ($y$) berbentuk $${nm}$. Apabila $x = ${n(x0)}$, $y = ${ynum(y0)}$. Cari $a$, dan gunakannya untuk mencari $y$ apabila $x = ${n(x1)}$.`), a: T(`$a = ${n(k.a)}$; $y = ${ynum(y1)}$`), w: W(T(`Substitute $x = ${n(x0)}$ and $y = ${ynum(y0)}$ into $${nm}$.`, `Gantikan $x = ${n(x0)}$ dan $y = ${ynum(y0)}$ ke dalam $${nm}$.`), `$${fsubA(k.F, x0)} = ${ynum(y0)}$`, `$a = ${n(k.a)}$`, `$x = ${n(x1)}$: $y = ${fsub(k.F, x1)} = ${ynum(y1)}$`), sp: 's' };
    },
    (r) => {
      const k = mkCtx(r), lim = k.F.n < 0 ? 0 : 1;
      const x = r.pick(k.xs), y = k.F.f(x);
      return { q: T(`${k.intro.en} (a) Find the ${k.yn.en} when the ${k.xn.en} is ${n(x)}. (b) Explain why the ${k.xn.en} cannot be negative. (c) Would the relation still make sense when the ${k.xn.en} is $0$?${k.F.n < 0 ? ' Explain.' : ' Explain.'}`, `${k.intro.ms} (a) Cari ${k.yn.ms} apabila ${k.xn.ms} ialah ${n(x)}. (b) Terangkan mengapa ${k.xn.ms} tidak boleh negatif. (c) Adakah hubungan itu masih bermakna apabila ${k.xn.ms} ialah $0$? Terangkan.`), w: W(`(a) $${fsub(k.F, x)} = ${n(round(y, 2))}$`, T(`(b) The ${k.xn.en} is a quantity such as a time, a length, a speed or a count, which cannot be negative.`, `(b) ${cap(k.xn.ms)} ialah kuantiti seperti masa, panjang, laju atau bilangan, yang tidak boleh negatif.`), k.F.n < 0 ? T('(c) No: the relation would need division by $0$, which is not defined.', '(c) Tidak: hubungan itu memerlukan pembahagian dengan $0$ yang tidak ditakrifkan.') : T('(c) Yes: the value would be $0$, meaning nothing has been produced or travelled yet.', '(c) Ya: nilainya ialah $0$, bermakna belum ada apa-apa dihasilkan atau dilalui.')), a: P([T(`${n(round(y, 2))}`), T('A quantity such as time, distance, length, speed or number cannot be negative.', 'Kuantiti seperti masa, jarak, panjang, laju atau bilangan tidak boleh negatif.'), k.F.n < 0 ? T('No: it needs division by $0$ (and $0$ speed, workers, etc. would mean nothing happens).', 'Tidak: ia memerlukan pembahagian dengan $0$ (dan laju atau bilangan pekerja $0$ bermakna tiada apa-apa berlaku).') : T('Yes: the value is $0$, meaning there is no distance, cost, amount or volume yet.', 'Ya: nilainya $0$, bermakna belum ada jarak, kos, jumlah atau isi padu.')]), sp: 'm' };
    },
  ];
  ...
  const diffsOf = (vals) => vals.slice(1).map((v, i) => v - vals[i]);
  const diffLine = (S1) => T(`Changes from one point to the next: ${diffsOf(S1.vals).map(n).join(', ')} ${S1.c.yu[0]}.`, `Perubahan dari satu titik ke titik seterusnya: ${diffsOf(S1.vals).map(n).join(', ')} ${S1.c.yu[1]}.`);
  const endsLine = (S1) => { const L1 = S1.vals.length - 1; return T(`The graph goes from ${n(S1.vals[0])} ${S1.c.yu[0]} to ${n(S1.vals[L1])} ${S1.c.yu[0]}.`, `Graf berubah daripada ${n(S1.vals[0])} ${S1.c.yu[1]} kepada ${n(S1.vals[L1])} ${S1.c.yu[1]}.`); };
  ...
  const g83e = [
    ...
    (r) => {
      ...
      return { q: T(`The graph shows the ${S1.c.u[0]} over time. (a) What is the value at ${x} ${S1.c.xu[0]}? (b) Is it increasing, decreasing or constant?`, `Graf menunjukkan ${S1.c.u[1]} mengikut masa. (a) Berapakah nilainya pada ${x} ${S1.c.xu[1]}? (b) Adakah ia bertambah, berkurang atau malar?`), fig: mkFig(S1), a: T(`(a) ${n(S1.vals[x])} ${S1.c.yu[0]} (b) ${trendWord(S1.vals).en.replace(/ing$/, 'ing').replace('increasing', 'Increasing').replace('decreasing', 'Decreasing')}`, `(a) ${n(S1.vals[x])} ${S1.c.yu[1]} (b) ${cap(trendWord(S1.vals).ms)}`), w: W(T(`(a) Go to ${x} ${S1.c.xu[0]} on the horizontal axis and read the value above it: ${n(S1.vals[x])} ${S1.c.yu[0]}.`, `(a) Pergi ke ${x} ${S1.c.xu[1]} pada paksi mengufuk dan baca nilai di atasnya: ${n(S1.vals[x])} ${S1.c.yu[1]}.`), endsLine(S1), T(`(b) So it is ${trendWord(S1.vals).en}.`, `(b) Jadi ia ${trendWord(S1.vals).ms}.`)), sp: 's' };
    },
    /* read endpoints and change */
    (r) => {
      const S1 = mkSeries(r, { kind: r.pick(['inc', 'dec']) });
      const last = S1.vals.length - 1;
      return { q: T(`The graph shows the ${S1.c.u[0]} over ${last} ${S1.c.xu[0]}. Find the value at the start and at the end, and the total change.`, `Graf menunjukkan ${S1.c.u[1]} sepanjang ${last} ${S1.c.xu[1]}. Cari nilai pada permulaan dan pada akhir, serta jumlah perubahan.`), fig: mkFig(S1), a: T(`Start: ${n(S1.vals[0])} ${S1.c.yu[0]}; end: ${n(S1.vals[last])} ${S1.c.yu[0]}; change $= ${n(S1.vals[last] - S1.vals[0])}$ ${S1.c.yu[0]}`, `Permulaan: ${n(S1.vals[0])} ${S1.c.yu[1]}; akhir: ${n(S1.vals[last])} ${S1.c.yu[1]}; perubahan $= ${n(S1.vals[last] - S1.vals[0])}$ ${S1.c.yu[1]}`), w: W(T('Read the first point and the last point of the graph.', 'Baca titik pertama dan titik terakhir pada graf.'), T(`Start ${n(S1.vals[0])} ${S1.c.yu[0]}, end ${n(S1.vals[last])} ${S1.c.yu[0]}.`, `Permulaan ${n(S1.vals[0])} ${S1.c.yu[1]}, akhir ${n(S1.vals[last])} ${S1.c.yu[1]}.`), T(`Change $= ${n(S1.vals[last])} - ${par(S1.vals[0])} = ${n(S1.vals[last] - S1.vals[0])}$ ${S1.c.yu[0]}`, `Perubahan $= ${n(S1.vals[last])} - ${par(S1.vals[0])} = ${n(S1.vals[last] - S1.vals[0])}$ ${S1.c.yu[1]}`)), sp: 's' };
    },
    /* describe trend in words (MCQ style) */
    (r) => {
      const S1 = mkSeries(r, { kind: r.pick(['inc', 'dec']) });
      return { q: T(`Look at the graph of the ${S1.c.u[0]}. Describe the overall trend in one word: increasing, decreasing or constant.`, `Perhatikan graf ${S1.c.u[1]}. Huraikan aliran keseluruhan dalam satu perkataan: bertambah, berkurang atau malar.`), fig: mkFig(S1), a: T(cap(trendWord(S1.vals).en), cap(trendWord(S1.vals).ms)), w: W(endsLine(S1), T(`So the overall trend is ${trendWord(S1.vals).en}.`, `Jadi aliran keseluruhan ialah ${trendWord(S1.vals).ms}.`)), sp: 'xs' };
    },
    /* find the time for a given value */
    (r) => {
      const S1 = mkSeries(r, { kind: r.pick(['inc', 'dec']) });
      const i = r.int(1, S1.vals.length - 1);
      return { q: T(`The graph shows the ${S1.c.u[0]} over time. Find the time (in ${S1.c.xu[0]}) at which the ${S1.c.u[0]} first reaches ${n(S1.vals[i])} ${S1.c.yu[0]}.`, `Graf menunjukkan ${S1.c.u[1]} mengikut masa. Cari masa (dalam ${S1.c.xu[1]}) apabila ${S1.c.u[1]} mula-mula mencapai ${n(S1.vals[i])} ${S1.c.yu[1]}.`), fig: mkFig(S1), a: T(`${i} ${S1.c.xu[0]}`, `${i} ${S1.c.xu[1]}`), w: W(T(`Find ${n(S1.vals[i])} ${S1.c.yu[0]} on the vertical axis and move across to the graph.`, `Cari ${n(S1.vals[i])} ${S1.c.yu[1]} pada paksi mencancang dan bergerak mendatar ke graf.`), T(`Then read straight down to the horizontal axis: ${i} ${S1.c.xu[0]}.`, `Kemudian baca terus ke bawah pada paksi mengufuk: ${i} ${S1.c.xu[1]}.`)), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Study the graph of the ${S1.c.u[0]}. Does it change by equal amounts over equal time intervals?`, `Kaji graf ${S1.c.u[1]}. Adakah ia berubah dengan jumlah yang sama pada selang masa yang sama?`), fig: mkFig(S1), a: uni ? T('Yes, equal amounts each interval.', 'Ya, jumlah yang sama pada setiap selang.') : T('No, the amounts of change are different.', 'Tidak, jumlah perubahan adalah berbeza.'), w: W(diffLine(S1), uni ? T('They are all the same, so the graph changes by equal amounts over equal time intervals.', 'Semuanya sama, jadi graf berubah dengan jumlah yang sama pada selang masa yang sama.') : T('They are not all the same, so the amounts of change are different.', 'Semuanya tidak sama, jadi jumlah perubahan adalah berbeza.')), sp: 's' };
    },
  ];
  ...
  const g83m = [
    ...
    (r) => {
      ...
      return { q: T(`The graph shows the ${S1.c.u[0]}. Estimate the value at ${n(i + 0.5)} ${S1.c.xu[0]} by interpolating between the two nearest points.`, `Graf menunjukkan ${S1.c.u[1]}. Anggarkan nilai pada ${n(i + 0.5)} ${S1.c.xu[1]} dengan interpolasi antara dua titik terdekat.`), fig: mkFig(S1), a: T(`About ${n(est)} ${S1.c.yu[0]}`, `Kira-kira ${n(est)} ${S1.c.yu[1]}`), w: W(T(`At ${i} ${S1.c.xu[0]} the value is ${n(S1.vals[i])} ${S1.c.yu[0]}; at ${i + 1} it is ${n(S1.vals[i + 1])} ${S1.c.yu[0]}.`, `Pada ${i} ${S1.c.xu[1]} nilainya ialah ${n(S1.vals[i])} ${S1.c.yu[1]}; pada ${i + 1} ialah ${n(S1.vals[i + 1])} ${S1.c.yu[1]}.`), `$\\dfrac{${n(S1.vals[i])} + ${par(S1.vals[i + 1])}}{2} = ${n(est)}$`, T(`About ${n(est)} ${S1.c.yu[0]}.`, `Kira-kira ${n(est)} ${S1.c.yu[1]}.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph shows the ${S1.c.u[0]}. During which ${S1.c.xu[0].replace(/s$/, '')} interval did it change the fastest? Give the change in that interval.`, `Graf menunjukkan ${S1.c.u[1]}. Dalam selang ${S1.c.xu[1]} yang manakah ia berubah paling cepat? Berikan perubahan pada selang itu.`), fig: mkFig(S1), a: T(`Between ${S1.c.xu[0].replace(/s$/, '')} ${bi} and ${bi + 1}: a change of ${n(worst)} ${S1.c.yu[0]}`, `Antara ${S1.c.xu[1].replace(/s$/, '')} ${bi} dan ${bi + 1}: perubahan ${n(worst)} ${S1.c.yu[1]}`), w: W(T('The steepest part of the graph is where the change over one interval is greatest.', 'Bahagian graf yang paling curam ialah tempat perubahan dalam satu selang adalah paling besar.'), diffLine(S1), T(`The largest change in size is ${n(worst)} ${S1.c.yu[0]}, between ${bi} and ${bi + 1}.`, `Perubahan terbesar dari segi saiz ialah ${n(worst)} ${S1.c.yu[1]}, antara ${bi} dan ${bi + 1}.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The table shows the ${S1.c.u[0]} at equal time intervals.<br>${tb}Is the change uniform or non-uniform? Give a reason using the differences.`, `Jadual menunjukkan ${S1.c.u[1]} pada selang masa yang sama.<br>${tbM}Adakah perubahan itu seragam atau tidak seragam? Berikan sebab menggunakan beza antara nilai.`), a: uni ? T(`Uniform: the differences are all ${n(diffs[0])}.`, `Seragam: semua beza ialah ${n(diffs[0])}.`) : T(`Non-uniform: the differences are ${diffs.map(n).join(', ')}, which are not all equal.`, `Tidak seragam: beza-bezanya ialah ${diffs.map(n).join(', ')}, yang tidak semuanya sama.`), w: W(T('Subtract each value in the table from the next one.', 'Tolak setiap nilai dalam jadual daripada nilai berikutnya.'), T(`Differences: ${diffs.map(n).join(', ')} ${S1.c.yu[0]}.`, `Beza: ${diffs.map(n).join(', ')} ${S1.c.yu[1]}.`), uni ? T(`They are all ${n(diffs[0])}, so the change is uniform.`, `Semuanya ialah ${n(diffs[0])}, jadi perubahan itu seragam.`) : T('They are not all equal, so the change is non-uniform.', 'Semuanya tidak sama, jadi perubahan itu tidak seragam.')), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T('The diagram shows two straight-line graphs, $A$ and $B$. Find the coordinates of their point of intersection.', 'Rajah menunjukkan dua graf garis lurus, $A$ dan $B$. Cari koordinat titik persilangan mereka.'), fig, a: T(`$(${xi},\\ ${yi})$`), w: W(T('The point of intersection is the only point that lies on both lines.', 'Titik persilangan ialah satu-satunya titik yang terletak pada kedua-dua garis.'), T(`Read across and down from the marked point: $x = ${xi}$, $y = ${yi}$.`, `Baca ke paksi mengufuk dan mencancang dari titik yang ditanda: $x = ${xi}$, $y = ${yi}$.`), `$(${xi},\\ ${yi})$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graphs of $y_1 = ${lin(m1, c1)}$ (solid) and $y_2 = ${lin(m2, c2)}$ (dashed) are drawn. Use the graph to estimate the solution of $${lin(m1, c1)} = ${lin(m2, c2)}$.`, `Graf $y_1 = ${lin(m1, c1)}$ (penuh) dan $y_2 = ${lin(m2, c2)}$ (putus-putus) dilukis. Gunakan graf untuk menganggar penyelesaian $${lin(m1, c1)} = ${lin(m2, c2)}$.`), fig, a: T(`$x = ${xi}$ (at the intersection, both give $y = ${yi}$)`, `$x = ${xi}$ (pada persilangan, kedua-duanya memberikan $y = ${yi}$)`), w: W(T('The two expressions are equal exactly where the two graphs cross.', 'Dua ungkapan itu sama tepat di tempat dua graf itu bersilang.'), T(`The graphs cross at $(${xi},\\ ${yi})$.`, `Graf bersilang pada $(${xi},\\ ${yi})$.`), T(`Check: at $x = ${xi}$, $${lin(m1, c1)} = ${yi}$ and $${lin(m2, c2)} = ${yi}$.`, `Semak: pada $x = ${xi}$, $${lin(m1, c1)} = ${yi}$ dan $${lin(m2, c2)} = ${yi}$.`), `$x = ${xi}$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph shows the ${S1.c.u[0]} changing uniformly. Find the rate of change per ${S1.c.xu[0].replace(/s$/, '')}, with correct sign and units.`, `Graf menunjukkan ${S1.c.u[1]} berubah secara seragam. Cari kadar perubahan bagi setiap ${S1.c.xu[1].replace(/s$/, '')}, dengan tanda dan unit yang betul.`), fig: mkFig(S1), a: T(`${d > 0 ? '+' : ''}${n(d)} ${S1.c.yu[0]} per ${S1.c.xu[0].replace(/s$/, '')}`, `${d > 0 ? '+' : ''}${n(d)} ${S1.c.yu[1]} setiap ${S1.c.xu[1].replace(/s$/, '')}`), w: W(T('The change is the same over every equal interval, so use any one interval.', 'Perubahan adalah sama pada setiap selang yang sama, jadi gunakan mana-mana satu selang.'), `$${n(S1.vals[1])} - ${par(S1.vals[0])} = ${n(d)}$`, T(`Rate of change $= ${d > 0 ? '+' : ''}${n(d)}$ ${S1.c.yu[0]} per ${S1.c.xu[0].replace(/s$/, '')}${d < 0 ? ' (the minus sign shows a decrease)' : ''}.`, `Kadar perubahan $= ${d > 0 ? '+' : ''}${n(d)}$ ${S1.c.yu[1]} setiap ${S1.c.xu[1].replace(/s$/, '')}${d < 0 ? ' (tanda tolak menunjukkan pengurangan)' : ''}.`)), sp: 's' };
    },
  ];
  ...
  const g83a = [
    ...
    (r) => {
      ...
      return { q: T(`The diagram shows a curve $y = f(x)$ and a straight line $y = g(x)$ drawn on the same axes. Use the graph to find all the solutions of $f(x) = g(x)$ shown in this window.`, `Rajah menunjukkan lengkung $y = f(x)$ dan garis lurus $y = g(x)$ yang dilukis pada paksi yang sama. Gunakan graf untuk mencari semua penyelesaian $f(x) = g(x)$ yang ditunjukkan dalam tetingkap ini.`), fig, a: T(`$x = ${roots.join(' \\text{ and } ')}$`, `$x = ${roots.join(' \\text{ dan } ')}$`), w: W(T('$f(x) = g(x)$ exactly where the curve and the line cross, so look for the crossing points.', '$f(x) = g(x)$ tepat di tempat lengkung dan garis bersilang, jadi cari titik persilangan.'), T(`Reading the $x$-coordinates of those points from the graph gives $x = ${roots.join(' \\text{ and } ')}$.`, `Membaca koordinat-$x$ bagi titik-titik itu daripada graf memberikan $x = ${roots.join(' \\text{ dan } ')}$.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph compares the ${c.u[0]} in two separate cases, $P$ (solid) and $Q$ (dashed), over ${N - 1} ${c.xu[0]}. Compare how $P$ and $Q$ change, and state whether their values are ever equal.`, `Graf membandingkan ${c.u[1]} bagi dua kes yang berasingan, $P$ (penuh) dan $Q$ (putus-putus), sepanjang ${N - 1} ${c.xu[1]}. Bandingkan bagaimana $P$ dan $Q$ berubah, dan nyatakan sama ada nilai mereka pernah sama.`), fig, a: T(`$P$ changes at ${n(d1)} ${c.yu[0]} per interval, $Q$ at ${n(d2)} ${c.yu[0]} per interval (${d2 > d1 ? 'Q changes faster' : 'P changes faster'}). ${cross ? 'Their graphs cross, so their values are equal at some point.' : 'Their graphs do not cross in this range, so their values are never equal here.'}`, `$P$ berubah pada kadar ${n(d1)} ${c.yu[1]} setiap selang, $Q$ pada ${n(d2)} ${c.yu[1]} setiap selang (${d2 > d1 ? 'Q berubah lebih cepat' : 'P berubah lebih cepat'}). ${cross ? 'Graf mereka bersilang, jadi nilai mereka sama pada satu ketika.' : 'Graf mereka tidak bersilang dalam julat ini, jadi nilai mereka tidak pernah sama di sini.'}`), w: W(T(`$P$ changes by ${n(d1)} ${c.yu[0]} each interval; $Q$ changes by ${n(d2)} ${c.yu[0]} each interval.`, `$P$ berubah sebanyak ${n(d1)} ${c.yu[1]} setiap selang; $Q$ berubah sebanyak ${n(d2)} ${c.yu[1]} setiap selang.`), T(`${d2 > d1 ? '$Q$' : '$P$'} is the steeper graph, so it changes faster.`, `${d2 > d1 ? '$Q$' : '$P$'} ialah graf yang lebih curam, jadi ia berubah lebih cepat.`), T(`They start at ${n(v01)} and ${n(v02)} ${c.yu[0]}. ${cross ? 'The graphs cross, so their values are equal at some point.' : 'The graphs do not cross in this range, so their values are never equal here.'}`, `Ia bermula pada ${n(v01)} dan ${n(v02)} ${c.yu[1]}. ${cross ? 'Graf itu bersilang, jadi nilainya sama pada satu ketika.' : 'Graf itu tidak bersilang dalam julat ini, jadi nilainya tidak pernah sama di sini.'}`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graphs of the lines $A: y = ${lin(m1, c1)}$ and $B: y = ${lin(m2, c2)}$ are shown, without their point of intersection marked. Read the coordinates of the intersection point from the graph, then verify your answer by substitution.`, `Graf garis $A: y = ${lin(m1, c1)}$ dan $B: y = ${lin(m2, c2)}$ ditunjukkan, tanpa titik persilangan ditandakan. Baca koordinat titik persilangan daripada graf, kemudian sahkan jawapan anda dengan penggantian.`), fig, a: T(`$(${xi},\\ ${yi})$: substituting $x = ${xi}$ into both gives $y = ${yi}$.`, `$(${xi},\\ ${yi})$: menggantikan $x = ${xi}$ ke dalam kedua-dua persamaan memberikan $y = ${yi}$.`), w: W(T('Read where the two lines cross: follow the point down to the $x$-axis and across to the $y$-axis.', 'Baca di mana dua garis itu bersilang: ikut titik itu ke bawah pada paksi-$x$ dan mendatar ke paksi-$y$.'), `$(${xi},\\ ${yi})$`, T(`Check in $A$: $${lin(m1, c1)}$ at $x = ${xi}$ gives $y = ${yi}$.`, `Semak dalam $A$: $${lin(m1, c1)}$ pada $x = ${xi}$ memberikan $y = ${yi}$.`), T(`Check in $B$: $${lin(m2, c2)}$ at $x = ${xi}$ gives $y = ${yi}$.`, `Semak dalam $B$: $${lin(m2, c2)}$ pada $x = ${xi}$ memberikan $y = ${yi}$.`)), sp: 'm' };
    },
    /* limitation of domain */
    (r) => {
      const S1 = mkSeries(r, { kind: r.pick(['inc', 'dec']) });
      const last = S1.vals.length - 1;
      return { q: T(`The graph shows the ${S1.c.u[0]} recorded only between ${0} and ${last} ${S1.c.xu[0]}. Explain why it would not be reasonable to use this graph to state the value at $${last * 3}$ ${S1.c.xu[0]}.`, `Graf menunjukkan ${S1.c.u[1]} yang direkodkan hanya antara ${0} dan ${last} ${S1.c.xu[1]}. Terangkan mengapa tidak munasabah untuk menggunakan graf ini bagi menyatakan nilai pada $${last * 3}$ ${S1.c.xu[1]}.`), fig: mkFig(S1), w: W(T(`The recorded data cover only 0 to ${last} ${S1.c.xu[0]}.`, `Data yang direkodkan hanya meliputi 0 hingga ${last} ${S1.c.xu[1]}.`), T(`$${last * 3}$ is three times as far as the last reading, well outside that range.`, `$${last * 3}$ ialah tiga kali lebih jauh daripada bacaan terakhir, jauh di luar julat itu.`), T('Using the graph there assumes the trend continues unchanged, which the data cannot show, so the value would be unreliable.', 'Menggunakan graf di situ menganggap aliran itu berterusan tanpa berubah, yang tidak dapat ditunjukkan oleh data, jadi nilainya tidak boleh dipercayai.')), a: T(`$${last * 3}$ is far outside the range of the data shown (0 to ${last}); the trend may not continue in the same way, so extrapolating this far is unreliable.`, `$${last * 3}$ jauh di luar julat data yang ditunjukkan (0 hingga ${last}); aliran itu mungkin tidak berterusan dengan cara yang sama, jadi ekstrapolasi sejauh ini tidak boleh dipercayai.`), sp: 'm' };
    },
  ];
  ...
  const g83e2 = [
    ...
    (r) => {
      ...
      return { q: T(`Look at the graph of the ${S1.c.u[0]}. In how many of the intervals shown does the value increase?`, `Perhatikan graf ${S1.c.u[1]}. Dalam berapa banyak selang yang ditunjukkan nilainya bertambah?`), fig: mkFig(S1), a: T(`${ups}`), w: W(T('Compare each point with the one before it.', 'Bandingkan setiap titik dengan titik sebelumnya.'), diffLine(S1), T(`${ups} of these changes are positive, so the value increases in ${ups} interval${ups === 1 ? '' : 's'}.`, `${ups} daripada perubahan ini adalah positif, jadi nilainya bertambah dalam ${ups} selang.`)), sp: 's' };
    },
  ];
  const g83m2 = [
    ...
    (r) => {
      ...
      return { q: T(`Using the graph of the ${S1.c.u[0]}, find the greatest value shown and state when it occurs.`, `Menggunakan graf ${S1.c.u[1]}, cari nilai tertinggi yang ditunjukkan dan nyatakan bilakah ia berlaku.`), fig: mkFig(S1), a: T(`${n(mx)} ${S1.c.yu[0]}, at ${mi} ${S1.c.xu[0]}`, `${n(mx)} ${S1.c.yu[1]}, pada ${mi} ${S1.c.xu[1]}`), w: W(T('Look for the highest point on the graph and read both of its coordinates.', 'Cari titik tertinggi pada graf dan baca kedua-dua koordinatnya.'), T(`The values are ${S1.vals.map(n).join(', ')} ${S1.c.yu[0]}.`, `Nilainya ialah ${S1.vals.map(n).join(', ')} ${S1.c.yu[1]}.`), T(`The greatest is ${n(mx)} ${S1.c.yu[0]}, at ${mi} ${S1.c.xu[0]}.`, `Yang tertinggi ialah ${n(mx)} ${S1.c.yu[1]}, pada ${mi} ${S1.c.xu[1]}.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The table below shows the ${S1.c.u[0]}.<br>${tb}Plot this data as a graph, then state the total change over the period shown.`, `Jadual di bawah menunjukkan ${S1.c.u[1]}.<br>${tbM}Plotkan data ini sebagai graf, kemudian nyatakan jumlah perubahan sepanjang tempoh yang ditunjukkan.`), a: T(`Total change $= ${n(total)}$ ${S1.c.yu[0]}`, `Jumlah perubahan $= ${n(total)}$ ${S1.c.yu[1]}`), w: W(T('Plot each pair from the table and join the points.', 'Plotkan setiap pasangan daripada jadual dan sambungkan titik-titik itu.'), T(`The first value is ${n(S1.vals[0])} ${S1.c.yu[0]} and the last is ${n(S1.vals[S1.vals.length - 1])} ${S1.c.yu[0]}.`, `Nilai pertama ialah ${n(S1.vals[0])} ${S1.c.yu[1]} dan nilai terakhir ialah ${n(S1.vals[S1.vals.length - 1])} ${S1.c.yu[1]}.`), `$${n(S1.vals[S1.vals.length - 1])} - ${par(S1.vals[0])} = ${n(total)}$`), sp: 'm' };
    },
  ];
  const g83a2 = [
    ...
    (r) => {
      ...
      return { q: T(`The graph shows the ${S1.c.u[0]}. Describe, using the changing steepness between consecutive points, whether the rate of change is speeding up, slowing down, or irregular across the intervals shown.`, `Graf menunjukkan ${S1.c.u[1]}. Huraikan, menggunakan perubahan kecuraman antara titik berturutan, sama ada kadar perubahan semakin pantas, semakin perlahan, atau tidak tetap sepanjang selang yang ditunjukkan.`), fig: mkFig(S1), a: T(`Changes per interval: ${diffs.map(n).join(', ')} ${S1.c.yu[0]}. ${speeding ? 'The rate is generally speeding up.' : 'The rate is irregular (it does not consistently speed up or slow down).'}`, `Perubahan setiap selang: ${diffs.map(n).join(', ')} ${S1.c.yu[1]}. ${speeding ? 'Kadar itu secara umumnya semakin pantas.' : 'Kadar itu tidak tetap (ia tidak semakin pantas atau perlahan secara konsisten).'}`), w: W(T('The steepness between two points is the change over that interval, so list the changes.', 'Kecuraman antara dua titik ialah perubahan pada selang itu, jadi senaraikan perubahan itu.'), T(`Changes: ${diffs.map(n).join(', ')} ${S1.c.yu[0]}.`, `Perubahan: ${diffs.map(n).join(', ')} ${S1.c.yu[1]}.`), speeding ? T('Each change is at least as large as the one before, so the graph gets steeper: the rate is generally speeding up.', 'Setiap perubahan sekurang-kurangnya sebesar perubahan sebelumnya, jadi graf menjadi lebih curam: kadar itu secara umumnya semakin pantas.') : T('The changes get larger and smaller in turn, so the steepness is irregular.', 'Perubahan itu menjadi lebih besar dan lebih kecil secara berselang, jadi kecuraman itu tidak tetap.')), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The line $y = ${lin(m1, c1)}$ and the line $y = ${lin(m2, 0).replace(/\+ 0$/, '')} + k$ intersect at a point with $x = ${xi}$. Find $k$, then state the coordinates of the intersection.`, `Garis $y = ${lin(m1, c1)}$ dan garis $y = ${lin(m2, 0).replace(/\+ 0$/, '')} + k$ bersilang pada titik dengan $x = ${xi}$. Cari $k$, kemudian nyatakan koordinat titik persilangan.`), a: T(`$k = ${n(c2)}$; intersection at $(${xi},\\ ${yi})$`, `$k = ${n(c2)}$; persilangan pada $(${xi},\\ ${yi})$`), w: W(T(`At $x = ${xi}$ the first line gives $y = ${lin(m1, c1)} = ${yi}$.`, `Pada $x = ${xi}$ garis pertama memberikan $y = ${lin(m1, c1)} = ${yi}$.`), T(`The second line must give the same $y$ there: $${m2 === 1 ? '' : m2 === -1 ? '-' : m2}(${xi}) + k = ${yi}$.`, `Garis kedua mesti memberikan $y$ yang sama di situ: $${m2 === 1 ? '' : m2 === -1 ? '-' : m2}(${xi}) + k = ${yi}$.`), `$k = ${yi} - ${par(m2 * xi)} = ${n(c2)}$`, `$(${xi},\\ ${yi})$`), sp: 'm' };
    },
  ];
  ...
  const g91e = [
    ...
    (r) => {
      ...
      return { q: TT(r.pick(ph)), a: T(`${v} km/h`, `${v} km/j`), w: W(T('Speed $=$ distance $\\div$ time.', 'Laju $=$ jarak $\\div$ masa.'), `$${v * t} \\div ${t} = ${v}$`, T(`${v} km/h`, `${v} km/j`)), sp: 'xs' };
    },
    /* s = vt */
    (r) => {
      ...
      return { q: TT(r.pick(ph)), a: T(`${v * t} km`), w: W(T('Distance $=$ speed $\\times$ time.', 'Jarak $=$ laju $\\times$ masa.'), `$${v} \\times ${t} = ${v * t}$`, T(`${v * t} km`)), sp: 'xs' };
    },
    /* t = s/v */
    (r) => {
      const v = r.int(3, 12) * 10, t = r.int(2, 6);
      const c = speedContext(r);
      return { q: T(`A ${c.ve} travels ${v * t} km at a constant speed of ${v} km/h. Find the time taken.`, `Sebuah ${c.vm} bergerak sejauh ${v * t} km pada laju malar ${v} km/j. Cari masa yang diambil.`), a: T(`${t} h`, `${t} j`), w: W(T('Time $=$ distance $\\div$ speed.', 'Masa $=$ jarak $\\div$ laju.'), `$${v * t} \\div ${v} = ${t}$`, T(`${t} h`, `${t} j`)), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      const dir = r.chance();
      return dir
        ? { q: T(`Convert ${ms1} m/s to km/h.`, `Tukarkan ${ms1} m/s kepada km/j.`), a: T(`${n(kmh)} km/h`, `${n(kmh)} km/j`), w: W(T('$1$ m/s $= 3.6$ km/h, so multiply by $3.6$.', '$1$ m/s $= 3.6$ km/j, jadi darab dengan $3.6$.'), `$${ms1} \\times 3.6 = ${n(kmh)}$`, T(`${n(kmh)} km/h`, `${n(kmh)} km/j`)), sp: 'xs' }
        : { q: T(`Convert ${n(kmh)} km/h to m/s.`, `Tukarkan ${n(kmh)} km/j kepada m/s.`), a: T(`${ms1} m/s`), w: W(T('$1$ m/s $= 3.6$ km/h, so divide by $3.6$.', '$1$ m/s $= 3.6$ km/j, jadi bahagi dengan $3.6$.'), `$${n(kmh)} \\div 3.6 = ${ms1}$`, T(`${ms1} m/s`)), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Complete the sentence.<br>${cz[0]}`, `Lengkapkan ayat berikut.<br>${cz[1]}`), a: T(cz[2], cz[3]), w: W(T(`The completed sentence reads: ${fillBlanks(cz[0], cz[2])}`, `Ayat yang lengkap berbunyi: ${fillBlanks(cz[1], cz[3])}`)), sp: 'xs' };
    },
  ];
  ...
  const g91m = [
    ...
    (r) => {
      ...
      return { q: TT(r.pick(ph)), a: T(`${n(v * t)} km`), w: W(T(`Change the time to hours: ${hm(t)} $= ${n(t)}$ h.`, `Tukar masa kepada jam: ${hmMs(t)} $= ${n(t)}$ j.`), T('Distance $=$ speed $\\times$ time.', 'Jarak $=$ laju $\\times$ masa.'), `$${v} \\times ${n(t)} = ${n(v * t)}$`, T(`${n(v * t)} km`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      const c = speedContext(r);
      return { q: T(`A ${c.ve} travels ${n(d)} km at a constant speed of ${v} km/h. Find the time taken, giving your answer in hours and minutes.`, `Sebuah ${c.vm} bergerak sejauh ${n(d)} km pada laju malar ${v} km/j. Cari masa yang diambil, berikan jawapan anda dalam jam dan minit.`), a: T(hm(t), hmMs(t)), w: W(T('Time $=$ distance $\\div$ speed.', 'Masa $=$ jarak $\\div$ laju.'), `$${n(d)} \\div ${v} = ${n(t)}$`, T(`${n(t)} hours is ${hm(t)}.`, `${n(t)} jam ialah ${hmMs(t)}.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      const dir = r.chance();
      return dir
        ? { q: T(`A cyclist rides at ${kmh} km/h. Express this speed in m/s.`, `Seorang penunggang basikal menunggang pada ${kmh} km/j. Nyatakan laju ini dalam m/s.`), a: T(`${n(ms1)} m/s`), w: W(T('To change km/h to m/s, divide by $3.6$.', 'Untuk menukar km/j kepada m/s, bahagi dengan $3.6$.'), `$${kmh} \\div 3.6 ${apx(kmh / 3.6, 2)} ${n(ms1)}$`, T(`${n(ms1)} m/s`)), sp: 's' }
        : { q: T(`A cyclist rides at ${n(ms1)} m/s. Express this speed in km/h.`, `Seorang penunggang basikal menunggang pada ${n(ms1)} m/s. Nyatakan laju ini dalam km/j.`), a: T(`${kmh} km/h`, `${kmh} km/j`), w: W(T('To change m/s to km/h, multiply by $3.6$.', 'Untuk menukar m/s kepada km/j, darab dengan $3.6$.'), `$${n(ms1)} \\times 3.6 ${apx(ms1 * 3.6, 0)} ${kmh}$`, T(`${kmh} km/h`, `${kmh} km/j`)), sp: 's' };
    },
    /* arrival time */
    (r) => {
      ...
      return { q: T(`${name} leaves at ${fmtClock(start)} for a place ${n(d)} km away and drives at ${v} km/h. At what time does ${name} arrive?`, `${name} bertolak pada pukul ${fmtClock(start)} ke suatu tempat sejauh ${n(d)} km dan memandu pada ${v} km/j. Pukul berapakah ${name} tiba?`), a: T(fmtClock(end)), w: W(T('Time $=$ distance $\\div$ speed.', 'Masa $=$ jarak $\\div$ laju.'), T(`$${n(d)} \\div ${v} = ${n(t)}$ h, which is ${hm(t)}.`, `$${n(d)} \\div ${v} = ${n(t)}$ j, iaitu ${hmMs(t)}.`), T(`Leaving at ${fmtClock(start)} and adding ${hm(t)} gives ${fmtClock(end)}.`, `Bertolak pada ${fmtClock(start)} dan menambah ${hmMs(t)} memberikan ${fmtClock(end)}.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      const name = r.name();
      return { q: T(`${name} must arrive at a town ${n(d)} km away by ${fmtClock(arrive)}, driving at ${v} km/h. What is the latest time ${name} can leave?`, `${name} mesti tiba di sebuah bandar sejauh ${n(d)} km sebelum pukul ${fmtClock(arrive)}, memandu pada ${v} km/j. Pukul berapakah lewat-lewatnya ${name} boleh bertolak?`), a: T(fmtClock(start)), w: W(T('Time $=$ distance $\\div$ speed.', 'Masa $=$ jarak $\\div$ laju.'), T(`$${n(d)} \\div ${v} = ${n(t)}$ h, which is ${hm(t)}.`, `$${n(d)} \\div ${v} = ${n(t)}$ j, iaitu ${hmMs(t)}.`), T(`Count ${hm(t)} back from ${fmtClock(arrive)}: ${fmtClock(start)}.`, `Kira ${hmMs(t)} ke belakang dari ${fmtClock(arrive)}: ${fmtClock(start)}.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      const c = speedContext(r);
      return { q: T(`A ${c.ve} travels ${d} km at ${v1} km/h and then another ${d} km at ${v2} km/h. Find the average speed for the whole journey.`, `Sebuah ${c.vm} bergerak sejauh ${d} km pada ${v1} km/j dan kemudian ${d} km lagi pada ${v2} km/j. Cari purata laju bagi keseluruhan perjalanan.`), a: T(`${n(round(avg, 2))} km/h (not ${n((v1 + v2) / 2)})`, `${n(round(avg, 2))} km/j (bukan ${n((v1 + v2) / 2)})`), w: W(T(`First stage: $\\dfrac{${d}}{${v1}} = ${n(round(t1, 3))}$ h`, `Peringkat pertama: $\\dfrac{${d}}{${v1}} = ${n(round(t1, 3))}$ j`), T(`Second stage: $\\dfrac{${d}}{${v2}} = ${n(round(t2, 3))}$ h`, `Peringkat kedua: $\\dfrac{${d}}{${v2}} = ${n(round(t2, 3))}$ j`), T(`Average speed $= \\dfrac{${2 * d}}{${n(round(t1 + t2, 3))}} ${apx(avg, 2)} ${n(round(avg, 2))}$ km/h`, `Purata laju $= \\dfrac{${2 * d}}{${n(round(t1 + t2, 3))}} ${apx(avg, 2)} ${n(round(avg, 2))}$ km/j`), T(`More time is spent at the slower speed, so it is not $\\dfrac{${v1} + ${v2}}{2}$.`, `Lebih banyak masa dihabiskan pada laju yang lebih perlahan, jadi ia bukan $\\dfrac{${v1} + ${v2}}{2}$.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      const c = speedContext(r);
      return { q: T(`A ${c.ve} travels ${d1} km at ${v1} km/h, then ${d2} km at ${v2} km/h. Find (a) the total time taken, (b) the average speed for the journey.`, `Sebuah ${c.vm} bergerak sejauh ${d1} km pada ${v1} km/j, kemudian ${d2} km pada ${v2} km/j. Cari (a) jumlah masa yang diambil, (b) purata laju bagi perjalanan itu.`), a: T(`(a) ${hm(t1 + t2)} (b) ${n(round(avg, 2))} km/h`, `(a) ${hmMs(t1 + t2)} (b) ${n(round(avg, 2))} km/j`), w: W(T(`Stage 1: $\\dfrac{${d1}}{${v1}} = ${n(round(t1, 3))}$ h`, `Peringkat 1: $\\dfrac{${d1}}{${v1}} = ${n(round(t1, 3))}$ j`), T(`Stage 2: $\\dfrac{${d2}}{${v2}} = ${n(round(t2, 3))}$ h`, `Peringkat 2: $\\dfrac{${d2}}{${v2}} = ${n(round(t2, 3))}$ j`), T(`(a) Total time $= ${n(round(t1 + t2, 3))}$ h, i.e. ${hm(t1 + t2)}`, `(a) Jumlah masa $= ${n(round(t1 + t2, 3))}$ j, iaitu ${hmMs(t1 + t2)}`), T(`(b) Average speed $= \\dfrac{${d1 + d2}}{${n(round(t1 + t2, 3))}} ${apx(avg, 2)} ${n(round(avg, 2))}$ km/h`, `(b) Purata laju $= \\dfrac{${d1 + d2}}{${n(round(t1 + t2, 3))}} ${apx(avg, 2)} ${n(round(avg, 2))}$ km/j`)), sp: 'm' };
    },
    /* compare two speeds */
    (r) => {
      ...
      return { q: T(`A ${c1.ve} moves at ${v1} km/h and a ${c2.ve} moves at ${v2ms} m/s. Convert both speeds to the same unit and state which is faster.`, `Sebuah ${c1.vm} bergerak pada ${v1} km/j dan sebuah ${c2.vm} bergerak pada ${v2ms} m/s. Tukarkan kedua-dua laju kepada unit yang sama dan nyatakan yang mana lebih laju.`), a: T(`${v1} km/h $= ${n(v1ms)}$ m/s; ${v2ms} m/s $= ${n(v2kmh)}$ km/h. The ${v1ms > v2ms ? c1.ve : c2.ve} is faster.`, `${v1} km/j $= ${n(v1ms)}$ m/s; ${v2ms} m/s $= ${n(v2kmh)}$ km/j. ${cap(v1ms > v2ms ? c1.vm : c2.vm)} lebih laju.`), w: W(T('To change km/h to m/s divide by $3.6$; to change m/s to km/h multiply by $3.6$.', 'Untuk menukar km/j kepada m/s bahagi dengan $3.6$; untuk menukar m/s kepada km/j darab dengan $3.6$.'), `$${v1} \\div 3.6 = ${n(v1ms)}$`, `$${v2ms} \\times 3.6 = ${n(v2kmh)}$`, T(`In m/s: $${n(v1ms)}$ and $${v2ms}$, so the ${v1ms > v2ms ? c1.ve : c2.ve} is faster.`, `Dalam m/s: $${n(v1ms)}$ dan $${v2ms}$, jadi ${c1.vm === c2.vm ? '' : ''}${v1ms > v2ms ? c1.vm : c2.vm} lebih laju.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      const c = speedContext(r);
      return { q: T(`A ${c.ve} travels ${d} km at ${v} km/h, then stops for a rest of ${rest} minutes. Find the total time for the journey, including the rest.`, `Sebuah ${c.vm} bergerak sejauh ${d} km pada ${v} km/j, kemudian berhenti untuk berehat selama ${rest} minit. Cari jumlah masa bagi perjalanan itu, termasuk rehat.`), a: T(hm(tot), hmMs(tot)), w: W(T(`Moving time $= \\dfrac{${d}}{${v}} = ${n(move)}$ h $= ${n(move * 60)}$ min`, `Masa bergerak $= \\dfrac{${d}}{${v}} = ${n(move)}$ j $= ${n(move * 60)}$ min`), `$${n(move * 60)} + ${rest} = ${n(move * 60 + rest)}$`, T(`${n(move * 60 + rest)} minutes is ${hm(tot)}.`, `${n(move * 60 + rest)} minit ialah ${hmMs(tot)}.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T('The distance–time graph shows a journey. Use the graph to find the speed of the vehicle.', 'Graf jarak–masa menunjukkan suatu perjalanan. Gunakan graf itu untuk mencari laju kenderaan.'), fig: T(fig, figMs), a: T(`${v} km/h`, `${v} km/j`), w: W(T('The graph is a straight line through the origin, so the speed is uniform: read any point and divide distance by time.', 'Graf ialah garis lurus melalui asalan, jadi lajunya seragam: baca sebarang titik dan bahagikan jarak dengan masa.'), T(`At $4$ hours the distance is ${v * 4} km.`, `Pada $4$ jam jaraknya ialah ${v * 4} km.`), `$\\dfrac{${v * 4}}{4} = ${v}$`, T(`${v} km/h`, `${v} km/j`)), sp: 's' };
    },
  ];
  ...
  const g91a = [
    ...
    (r) => {
      ...
      const avg = (d1 + d2) / t;
      const c = speedContext(r);
      return { q: T(`A ${c.ve} travels ${d1} km at ${v1} km/h, rests for ${rest} minutes, then travels ${d2} km at ${v2} km/h. Find (a) the total time taken, (b) the average speed for the whole journey (including the rest), correct to 1 decimal place.`, `Sebuah ${c.vm} bergerak sejauh ${d1} km pada ${v1} km/j, berehat selama ${rest} minit, kemudian bergerak ${d2} km pada ${v2} km/j. Cari (a) jumlah masa yang diambil, (b) purata laju bagi keseluruhan perjalanan (termasuk rehat), betul kepada 1 tempat perpuluhan.`), a: T(`(a) ${n(round(t, 3))} h (b) ${n(round(avg, 1))} km/h`, `(a) ${n(round(t, 3))} j (b) ${n(round(avg, 1))} km/j`), w: W(T(`Stage 1: $\\dfrac{${d1}}{${v1}} = ${n(round(d1 / v1, 3))}$ h`, `Peringkat 1: $\\dfrac{${d1}}{${v1}} = ${n(round(d1 / v1, 3))}$ j`), T(`Rest: $\\dfrac{${rest}}{60} = ${n(round(rest / 60, 3))}$ h`, `Rehat: $\\dfrac{${rest}}{60} = ${n(round(rest / 60, 3))}$ j`), T(`Stage 2: $\\dfrac{${d2}}{${v2}} = ${n(round(d2 / v2, 3))}$ h`, `Peringkat 2: $\\dfrac{${d2}}{${v2}} = ${n(round(d2 / v2, 3))}$ j`), T(`(a) Total time $${apx(t, 3)} ${n(round(t, 3))}$ h`, `(a) Jumlah masa $${apx(t, 3)} ${n(round(t, 3))}$ j`), T(`(b) Average speed $= \\dfrac{${d1 + d2}}{${n(round(t, 3))}} ${apx(avg, 1)} ${n(round(avg, 1))}$ km/h`, `(b) Purata laju $= \\dfrac{${d1 + d2}}{${n(round(t, 3))}} ${apx(avg, 1)} ${n(round(avg, 1))}$ km/j`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      const avg = (2 * d) / (t1 + t2);
      const c = speedContext(r);
      return { q: T(`A ${c.ve} travels from town $X$ to town $Y$, a distance of ${d} km, at ${v1} km/h, and returns along the same road at ${v2} km/h. Find (a) the total time for the round trip, (b) the average speed for the whole round trip.`, `Sebuah ${c.vm} bergerak dari bandar $X$ ke bandar $Y$, sejauh ${d} km, pada ${v1} km/j, dan berpatah balik melalui jalan yang sama pada ${v2} km/j. Cari (a) jumlah masa bagi perjalanan pergi balik itu, (b) purata laju bagi keseluruhan perjalanan pergi balik.`), a: T(`(a) ${n(round(t1 + t2, 3))} h (b) ${n(round(avg, 2))} km/h`, `(a) ${n(round(t1 + t2, 3))} j (b) ${n(round(avg, 2))} km/j`), w: W(T(`Going: $\\dfrac{${d}}{${v1}} = ${n(round(t1, 3))}$ h`, `Pergi: $\\dfrac{${d}}{${v1}} = ${n(round(t1, 3))}$ j`), T(`Returning: $\\dfrac{${d}}{${v2}} = ${n(round(t2, 3))}$ h`, `Balik: $\\dfrac{${d}}{${v2}} = ${n(round(t2, 3))}$ j`), T(`(a) Total time $${apx(t1 + t2, 3)} ${n(round(t1 + t2, 3))}$ h`, `(a) Jumlah masa $${apx(t1 + t2, 3)} ${n(round(t1 + t2, 3))}$ j`), T(`(b) The whole trip is $${2 * d}$ km: average speed $= \\dfrac{${2 * d}}{${n(round(t1 + t2, 3))}} ${apx(avg, 2)} ${n(round(avg, 2))}$ km/h`, `(b) Keseluruhan perjalanan ialah $${2 * d}$ km: purata laju $= \\dfrac{${2 * d}}{${n(round(t1 + t2, 3))}} ${apx(avg, 2)} ${n(round(avg, 2))}$ km/j`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      const c = speedContext(r);
      return { q: T(`A ${c.ve} travels ${d1} km at ${v1} km/h, then ${d2} km more. If the average speed for the whole ${d1 + d2} km journey is ${avgTarget} km/h, find the speed for the second part of the journey, correct to 1 decimal place.`, `Sebuah ${c.vm} bergerak sejauh ${d1} km pada ${v1} km/j, kemudian ${d2} km lagi. Jika purata laju bagi keseluruhan perjalanan sejauh ${d1 + d2} km ialah ${avgTarget} km/j, cari laju bagi bahagian kedua perjalanan itu, betul kepada 1 tempat perpuluhan.`), a: T(`${n(round(v2, 1))} km/h`, `${n(round(v2, 1))} km/j`), w: W(T(`Total time allowed $= \\dfrac{${d1 + d2}}{${avgTarget}} ${apx(tTot, 3)} ${n(round(tTot, 3))}$ h`, `Jumlah masa yang dibenarkan $= \\dfrac{${d1 + d2}}{${avgTarget}} ${apx(tTot, 3)} ${n(round(tTot, 3))}$ j`), T(`Stage 1 takes $\\dfrac{${d1}}{${v1}} = ${n(round(t1, 3))}$ h`, `Peringkat 1 mengambil $\\dfrac{${d1}}{${v1}} = ${n(round(t1, 3))}$ j`), T(`Time left for stage 2 $= ${n(round(tTot, 3))} - ${n(round(t1, 3))} ${apx(t2, 3)} ${n(round(t2, 3))}$ h`, `Masa yang tinggal bagi peringkat 2 $= ${n(round(tTot, 3))} - ${n(round(t1, 3))} ${apx(t2, 3)} ${n(round(t2, 3))}$ j`), T(`Speed $= \\dfrac{${d2}}{${n(round(t2, 3))}} ${apx(v2, 1)} ${n(round(v2, 1))}$ km/h`, `Laju $= \\dfrac{${d2}}{${n(round(t2, 3))}} ${apx(v2, 1)} ${n(round(v2, 1))}$ km/j`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Two vehicles $P$ and $Q$ record the following distances.<br>${TB([['Time (h)', ...ts], ['$P$: distance (km)', ...uds], ['$Q$: distance (km)', ...nds]])}(a) Which vehicle moves at a uniform speed? Justify. (b) Find $P$'s speed. (c) Find $Q$'s average speed over the ${4} hours.`, `Dua kenderaan $P$ dan $Q$ merekodkan jarak berikut.<br>${TB([['Masa (j)', ...ts], ['$P$: jarak (km)', ...uds], ['$Q$: jarak (km)', ...nds]])}(a) Kenderaan yang manakah bergerak pada laju seragam? Justifikasikan. (b) Cari laju $P$. (c) Cari purata laju $Q$ sepanjang ${4} jam itu.`), a: P([T(`$P$: equal distances (${v} km) every hour.`, `$P$: jarak yang sama (${v} km) setiap jam.`), T(`${v} km/h`, `${v} km/j`), T(`${n(round(nds[4] / 4, 2))} km/h`, `${n(round(nds[4] / 4, 2))} km/j`)]), w: W(T(`(a) $P$ covers ${uds.slice(1).map((x, i) => x - uds[i]).join(', ')} km each hour; $Q$ covers ${nds.slice(1).map((x, i) => x - nds[i]).join(', ')} km.`, `(a) $P$ melalui ${uds.slice(1).map((x, i) => x - uds[i]).join(', ')} km setiap jam; $Q$ melalui ${nds.slice(1).map((x, i) => x - nds[i]).join(', ')} km.`), T(`Only $P$ covers equal distances in equal times, so only $P$ has a uniform speed.`, `Hanya $P$ melalui jarak yang sama dalam masa yang sama, jadi hanya $P$ mempunyai laju seragam.`), T(`(b) $P$: $\\dfrac{${uds[4]}}{4} = ${v}$ km/h`, `(b) $P$: $\\dfrac{${uds[4]}}{4} = ${v}$ km/j`), T(`(c) $Q$: $\\dfrac{${nds[4]}}{4} ${apx(nds[4] / 4, 2)} ${n(round(nds[4] / 4, 2))}$ km/h`, `(c) $Q$: $\\dfrac{${nds[4]}}{4} ${apx(nds[4] / 4, 2)} ${n(round(nds[4] / 4, 2))}$ km/j`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      const needV = d / avail;
      const c = speedContext(r);
      return { q: T(`A ${c.ve} plans to travel ${d} km at ${planV} km/h, but leaves ${delay} minutes late. To arrive at the planned time, find the speed it must now travel at, correct to 1 decimal place.`, `Sebuah ${c.vm} merancang untuk bergerak sejauh ${d} km pada ${planV} km/j, tetapi bertolak lewat ${delay} minit. Untuk tiba pada waktu yang dirancang, cari laju yang mesti digunakan sekarang, betul kepada 1 tempat perpuluhan.`), a: T(`${n(round(needV, 1))} km/h`, `${n(round(needV, 1))} km/j`), w: W(T(`Planned time $= \\dfrac{${d}}{${planV}} ${apx(planT, 3)} ${n(round(planT, 3))}$ h`, `Masa dirancang $= \\dfrac{${d}}{${planV}} ${apx(planT, 3)} ${n(round(planT, 3))}$ j`), T(`Leaving ${delay} minutes late leaves $${n(round(planT, 3))} - \\dfrac{${delay}}{60} ${apx(avail, 3)} ${n(round(avail, 3))}$ h`, `Bertolak lewat ${delay} minit meninggalkan $${n(round(planT, 3))} - \\dfrac{${delay}}{60} ${apx(avail, 3)} ${n(round(avail, 3))}$ j`), T(`Speed needed $= \\dfrac{${d}}{${n(round(avail, 3))}} ${apx(needV, 1)} ${n(round(needV, 1))}$ km/h`, `Laju yang diperlukan $= \\dfrac{${d}}{${n(round(avail, 3))}} ${apx(needV, 1)} ${n(round(needV, 1))}$ km/j`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A car travels at a constant speed of ${kmh} km/h. (a) Express the speed in m/s. (b) Find the distance, in metres, travelled in ${t_s} seconds.`, `Sebuah kereta bergerak pada laju malar ${kmh} km/j. (a) Nyatakan laju itu dalam m/s. (b) Cari jarak, dalam meter, yang dilalui dalam ${t_s} saat.`), a: T(`(a) ${n(ms1)} m/s (b) ${n(dist_m)} m`), w: W(T('(a) To change km/h to m/s, divide by $3.6$.', '(a) Untuk menukar km/j kepada m/s, bahagi dengan $3.6$.'), `$${kmh} \\div 3.6 = ${n(ms1)}$`, T(`(b) Distance $=$ speed $\\times$ time $= ${n(ms1)} \\times ${t_s} = ${n(dist_m)}$ m`, `(b) Jarak $=$ laju $\\times$ masa $= ${n(ms1)} \\times ${t_s} = ${n(dist_m)}$ m`)), sp: 'm' };
    },
  ];
  ...
  const g91a2 = [
    ...
    (r) => {
      ...
      const c = speedContext(r);
      return { q: T(`A ${c.ve} travels for ${hm(t1)} at ${v1} km/h, then continues at ${v2} km/h. If the average speed for the whole journey is ${avgTarget} km/h, find (a) the time taken for the second part, (b) the distance covered in the second part, both correct to 2 decimal places where needed.`, `Sebuah ${c.vm} bergerak selama ${hmMs(t1)} pada ${v1} km/j, kemudian meneruskan perjalanan pada ${v2} km/j. Jika purata laju bagi keseluruhan perjalanan ialah ${avgTarget} km/j, cari (a) masa yang diambil bagi bahagian kedua, (b) jarak yang dilalui pada bahagian kedua, kedua-duanya betul kepada 2 tempat perpuluhan jika perlu.`), a: T(`(a) ${n(round(t2, 2))} h (b) ${n(round(d2, 2))} km`, `(a) ${n(round(t2, 2))} j (b) ${n(round(d2, 2))} km`), w: W(T(`Stage 1: distance $= ${v1} \\times ${n(t1)} = ${n(d1)}$ km, time $= ${n(t1)}$ h.`, `Peringkat 1: jarak $= ${v1} \\times ${n(t1)} = ${n(d1)}$ km, masa $= ${n(t1)}$ j.`), T(`Let the second part take $t$ hours: $\\dfrac{${n(d1)} + ${v2}t}{${n(t1)} + t} = ${avgTarget}$.`, `Katakan bahagian kedua mengambil $t$ jam: $\\dfrac{${n(d1)} + ${v2}t}{${n(t1)} + t} = ${avgTarget}$.`), T(`(a) Solving gives $t ${apx(t2, 2)} ${n(round(t2, 2))}$ h.`, `(a) Penyelesaiannya memberikan $t ${apx(t2, 2)} ${n(round(t2, 2))}$ j.`), T(`(b) Distance $= ${v2} \\times ${n(round(t2, 2))} ${apx(d2, 2)} ${n(round(d2, 2))}$ km`, `(b) Jarak $= ${v2} \\times ${n(round(t2, 2))} ${apx(d2, 2)} ${n(round(d2, 2))}$ km`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A cyclist's journey is recorded in three 1-hour stages: ${v1} km/h, then ${v2} km/h, then ${v3} km/h. (a) Find the total distance travelled. (b) Find the average speed for the whole journey. (c) Is the average speed equal to the average of the three speeds? Check by calculation.`, `Perjalanan seorang penunggang basikal direkodkan dalam tiga peringkat 1 jam: ${v1} km/j, kemudian ${v2} km/j, kemudian ${v3} km/j. (a) Cari jumlah jarak yang dilalui. (b) Cari purata laju bagi keseluruhan perjalanan. (c) Adakah purata laju sama dengan purata tiga laju itu? Semak dengan pengiraan.`), a: P([T(`${totD} km`), T(`${n(totD / totT)} km/h`, `${n(totD / totT)} km/j`), T(`Yes here, because each stage takes the same time (1 hour); average of speeds $= ${n((v1 + v2 + v3) / 3)}$ km/h, which matches. (This only works because the times are equal.)`, `Ya di sini, kerana setiap peringkat mengambil masa yang sama (1 jam); purata laju $= ${n((v1 + v2 + v3) / 3)}$ km/j, yang sepadan. (Ini hanya berlaku kerana masa adalah sama.)`)]), w: W(T(`(a) Each stage lasts $1$ hour, so the distances are ${d1}, ${d2} and ${d3} km.`, `(a) Setiap peringkat berlangsung $1$ jam, jadi jaraknya ialah ${d1}, ${d2} dan ${d3} km.`), `$${d1} + ${d2} + ${d3} = ${totD}$`, T(`(b) Average speed $= \\dfrac{${totD}}{${totT}} = ${n(totD / totT)}$ km/h`, `(b) Purata laju $= \\dfrac{${totD}}{${totT}} = ${n(totD / totT)}$ km/j`), T(`(c) $\\dfrac{${v1} + ${v2} + ${v3}}{3} = ${n((v1 + v2 + v3) / 3)}$, which matches — but only because each stage takes the same time.`, `(c) $\\dfrac{${v1} + ${v2} + ${v3}}{3} = ${n((v1 + v2 + v3) / 3)}$, yang sepadan — tetapi hanya kerana setiap peringkat mengambil masa yang sama.`)), sp: 'l' };
    },
  ];
  ...
  const g91Ee = [
    ...
    (r) => {
      ...
      return { q: TT(r.pick(ph)), a: T(hm(t), hmMs(t)), w: W(T(`When $Q$ starts, $P$ is already ahead by $${v1} \\times ${n(head)} = ${n(gap)}$ km.`, `Apabila $Q$ bertolak, $P$ sudah mendahului sejauh $${v1} \\times ${n(head)} = ${n(gap)}$ km.`), T(`$Q$ closes the gap at $${v2} - ${v1} = ${v2 - v1}$ km/h.`, `$Q$ menutup jurang itu pada $${v2} - ${v1} = ${v2 - v1}$ km/j.`), `$\\dfrac{${n(gap)}}{${v2 - v1}} = ${n(round(t, 4))}$`, T(`${hm(t)} after $Q$ starts.`, `${hmMs(t)} selepas $Q$ bertolak.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A motorcyclist leaves at ${v1} km/h. ${hm(head)} later, a car leaves the same point along the same road at ${v2} km/h. Find how far from the starting point the car catches up with the motorcyclist.`, `Seorang penunggang motosikal bertolak pada ${v1} km/j. ${hmMs(head)} kemudian, sebuah kereta bertolak dari titik yang sama di jalan yang sama pada ${v2} km/j. Cari jarak dari titik permulaan di mana kereta itu memintas penunggang motosikal itu.`), a: T(`${n(d)} km`), w: W(T(`Gap when the car starts $= ${v1} \\times ${n(head)} = ${n(gap)}$ km.`, `Jurang apabila kereta bertolak $= ${v1} \\times ${n(head)} = ${n(gap)}$ km.`), T(`Closing speed $= ${v2} - ${v1} = ${v2 - v1}$ km/h, so the time is $\\dfrac{${n(gap)}}{${v2 - v1}} = ${n(round(t, 4))}$ h.`, `Laju penutupan $= ${v2} - ${v1} = ${v2 - v1}$ km/j, jadi masanya ialah $\\dfrac{${n(gap)}}{${v2 - v1}} = ${n(round(t, 4))}$ j.`), T(`Distance from the start $= ${v2} \\times ${n(round(t, 4))} = ${n(d)}$ km.`, `Jarak dari permulaan $= ${v2} \\times ${n(round(t, 4))} = ${n(d)}$ km.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      const c1 = speedContext(r), c2 = speedContext(r);
      return { q: T(`Two towns $A$ and $B$ are ${gap} km apart. A ${c1.ve} leaves $A$ towards $B$ at ${v1} km/h. ${hm(head)} later, a ${c2.ve} leaves $B$ towards $A$ at ${v2} km/h. Find the total time from when the ${c1.ve} left until they meet.`, `Dua buah bandar $A$ dan $B$ berjarak ${gap} km. Sebuah ${c1.vm} bertolak dari $A$ menuju $B$ pada ${v1} km/j. ${hmMs(head)} kemudian, sebuah ${c2.vm} bertolak dari $B$ menuju $A$ pada ${v2} km/j. Cari jumlah masa dari waktu ${c1.vm} itu bertolak sehingga mereka bertemu.`), a: T(hm(head + t), hmMs(head + t)), w: W(T(`Before the second vehicle starts, the first covers $${v1} \\times ${n(head)} = ${n(v1 * head)}$ km.`, `Sebelum kenderaan kedua bertolak, kenderaan pertama melalui $${v1} \\times ${n(head)} = ${n(v1 * head)}$ km.`), T(`Remaining gap $= ${gap} - ${n(v1 * head)} = ${n(remaining)}$ km.`, `Baki jurang $= ${gap} - ${n(v1 * head)} = ${n(remaining)}$ km.`), T(`They close it together at $${v1} + ${v2} = ${v1 + v2}$ km/h: $\\dfrac{${n(remaining)}}{${v1 + v2}} = ${n(round(t, 4))}$ h.`, `Mereka menutupnya bersama pada $${v1} + ${v2} = ${v1 + v2}$ km/j: $\\dfrac{${n(remaining)}}{${v1 + v2}} = ${n(round(t, 4))}$ j.`), T(`Total $= ${n(head)} + ${n(round(t, 4))} = ${n(round(head + t, 4))}$ h, i.e. ${hm(head + t)}.`, `Jumlah $= ${n(head)} + ${n(round(t, 4))} = ${n(round(head + t, 4))}$ j, iaitu ${hmMs(head + t)}.`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      const c = speedContext(r);
      return { q: T(`A ${c.ve} $P$ leaves at ${v1} km/h. ${hm(head)} later, ${c.ve} $Q$ leaves the same place along the same road and catches up with $P$ after ${hm(t)}. Find the speed of $Q$.`, `${cap(c.vm)} $P$ bertolak pada ${v1} km/j. ${hmMs(head)} kemudian, ${c.vm} $Q$ bertolak dari tempat yang sama di jalan yang sama dan memintas $P$ selepas ${hmMs(t)}. Cari laju $Q$.`), a: T(`${n(v2)} km/h`, `${n(v2)} km/j`), w: W(T(`Gap when $Q$ starts $= ${v1} \\times ${n(head)} = ${n(gap)}$ km.`, `Jurang apabila $Q$ bertolak $= ${v1} \\times ${n(head)} = ${n(gap)}$ km.`), T(`$Q$ closes it in ${hm(t)}, so the closing speed is $\\dfrac{${n(gap)}}{${n(t)}} = ${n(gap / t)}$ km/h.`, `$Q$ menutupnya dalam ${hmMs(t)}, jadi laju penutupannya ialah $\\dfrac{${n(gap)}}{${n(t)}} = ${n(gap / t)}$ km/j.`), T(`Speed of $Q = ${v1} + ${n(gap / t)} = ${n(v2)}$ km/h.`, `Laju $Q = ${v1} + ${n(gap / t)} = ${n(v2)}$ km/j.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      const c1 = speedContext(r), c2 = speedContext(r);
      return { q: T(`A ${c1.ve} and a ${c2.ve} start together from the same point, moving in the same direction, at ${v1} km/h and ${v2} km/h. How far apart are they after ${hm(t)}?`, `Sebuah ${c1.vm} dan sebuah ${c2.vm} bertolak bersama dari titik yang sama, bergerak dalam arah yang sama, pada ${v1} km/j dan ${v2} km/j. Berapa jauhkah jarak antara mereka selepas ${hmMs(t)}?`), a: T(`${n(gapAfter)} km`), w: W(T('They move in the same direction, so the gap between them grows at the difference of the speeds.', 'Mereka bergerak dalam arah yang sama, jadi jurang antara mereka bertambah pada beza laju.'), `$${v2} - ${v1} = ${v2 - v1}$`, `$${v2 - v1} \\times ${n(t)} = ${n(gapAfter)}$`, T(`They are ${n(gapAfter)} km apart.`, `Mereka berjarak ${n(gapAfter)} km.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      const c1 = speedContext(r), c2 = speedContext(r);
      return { q: T(`Two towns are ${gap} km apart. A ${c1.ve} leaves one town at ${v1} km/h and a ${c2.ve} leaves the other at the same time, travelling towards each other at ${v2} km/h. Find the time taken until they are ${remain} km apart.`, `Dua buah bandar berjarak ${gap} km. Sebuah ${c1.vm} bertolak dari satu bandar pada ${v1} km/j dan sebuah ${c2.vm} bertolak dari bandar yang satu lagi pada masa yang sama, bergerak menuju satu sama lain pada ${v2} km/j. Cari masa yang diambil sehingga mereka berjarak ${remain} km.`), a: T(hm(t), hmMs(t)), w: W(T(`Together they must cover $${gap} - ${remain} = ${gap - remain}$ km.`, `Bersama-sama mereka mesti melalui $${gap} - ${remain} = ${gap - remain}$ km.`), T(`Moving towards each other, they close the gap at $${v1} + ${v2} = ${v1 + v2}$ km/h.`, `Bergerak menuju satu sama lain, mereka menutup jurang pada $${v1} + ${v2} = ${v1 + v2}$ km/j.`), `$\\dfrac{${gap - remain}}{${v1 + v2}} = ${n(round(t, 4))}$`, T(`That is ${hm(t)}.`, `Itu ialah ${hmMs(t)}.`)), sp: 'm' };
    },
  ];
  ...
  const g92e = [
    ...
    (r) => {
      ...
      return { q: TT(r.pick(ph)), a: T(`${a} m/s²`), w: W(T('Acceleration $=$ (final speed $-$ initial speed) $\\div$ time; starting from rest means $u = 0$.', 'Pecutan $=$ (laju akhir $-$ laju awal) $\\div$ masa; bermula dari keadaan pegun bermakna $u = 0$.'), `$a = \\dfrac{${a * t} - 0}{${t}} = ${a}$`, T(`${a} m/s²`)), sp: 's' };
    },
    /* non-zero u, find a */
    (r) => {
      ...
      const v = u + a * t;
      const c = accCtx(r);
      return { q: T(`A ${c.ve} increases its speed from ${u} m/s to ${v} m/s in ${t} seconds. Find the acceleration.`, `Sebuah ${c.vm} meningkatkan lajunya daripada ${u} m/s kepada ${v} m/s dalam ${t} saat. Cari pecutannya.`), a: T(`${a} m/s²`), w: W(T('Acceleration $=$ (final speed $-$ initial speed) $\\div$ time.', 'Pecutan $=$ (laju akhir $-$ laju awal) $\\div$ masa.'), `$a = \\dfrac{${v} - ${u}}{${t}} = ${a}$`, T(`${a} m/s²`)), sp: 's' };
    },
    /* deceleration to rest */
    (r) => {
      const u = r.int(8, 30), t = r.int(2, 8);
      const c = accCtx(r);
      return { q: T(`A ${c.ve} travelling at ${u * t} m/s brakes uniformly and comes to rest in ${t} seconds. Find its deceleration.`, `Sebuah ${c.vm} yang bergerak pada ${u * t} m/s memberhentikan diri dengan brek secara seragam dalam ${t} saat. Cari nyahpecutannya.`), a: T(`Acceleration $= ${-u}$ m/s² (deceleration ${u} m/s²)`, `Pecutan $= ${-u}$ m/s² (nyahpecutan ${u} m/s²)`), w: W(T('Acceleration $=$ (final speed $-$ initial speed) $\\div$ time.', 'Pecutan $=$ (laju akhir $-$ laju awal) $\\div$ masa.'), `$\\dfrac{0 - ${u * t}}{${t}} = ${-u}$`, T(`The negative sign shows slowing down, so the deceleration is ${u} m/s².`, `Tanda negatif menunjukkan perlahan, jadi nyahpecutannya ialah ${u} m/s².`)), sp: 's' };
    },
    /* v = u + at */
    (r) => {
      const u = r.int(1, 10), a = r.int(1, 6), t = r.int(2, 8);
      const c = accCtx(r);
      return { q: T(`A ${c.ve} moves with an initial speed of ${u} m/s and a uniform acceleration of ${a} m/s². Find its speed after ${t} seconds.`, `Sebuah ${c.vm} bergerak dengan laju awal ${u} m/s dan pecutan seragam ${a} m/s². Cari lajunya selepas ${t} saat.`), a: T(`${u + a * t} m/s`), w: W(T('Use $v = u + at$.', 'Gunakan $v = u + at$.'), `$v = ${u} + ${a} \\times ${t} = ${u + a * t}$`, T(`${u + a * t} m/s`)), sp: 's' };
    },
    /* t = (v-u)/a */
    (r) => {
      const u = r.int(1, 10), a = r.int(1, 6), t = r.int(2, 8);
      const v = u + a * t;
      const c = accCtx(r);
      return { q: T(`A ${c.ve} accelerates uniformly at ${a} m/s² from ${u} m/s. How long does it take to reach ${v} m/s?`, `Sebuah ${c.vm} memecut secara seragam pada ${a} m/s² daripada ${u} m/s. Berapa lamakah masa yang diambil untuk mencapai ${v} m/s?`), a: T(`${t} s`), w: W(T('Rearranging $v = u + at$ gives $t = \\dfrac{v - u}{a}$.', 'Menyusun semula $v = u + at$ memberikan $t = \\dfrac{v - u}{a}$.'), `$t = \\dfrac{${v} - ${u}}{${a}} = ${t}$`, T(`${t} s`)), sp: 's' };
    },
    /* cloze: formula */
    (r) => {
      const cz = r.pick([
    ...
    (r) => {
      const up = r.chance();
      const c = accCtx(r);
      return { q: T(`A ${c.ve} is ${up ? 'speeding up' : 'slowing down'}. Is its acceleration positive or negative?`, `Sebuah ${c.vm} sedang ${up ? 'memecut' : 'perlahan'}. Adakah pecutannya positif atau negatif?`), a: up ? T('Positive', 'Positif') : T('Negative (a deceleration)', 'Negatif (nyahpecutan)'), w: W(T('Acceleration $=$ (final speed $-$ initial speed) $\\div$ time.', 'Pecutan $=$ (laju akhir $-$ laju awal) $\\div$ masa.'), up ? T('Speeding up means the final speed is greater, so the numerator is positive.', 'Memecut bermakna laju akhir lebih besar, jadi pengangkanya positif.') : T('Slowing down means the final speed is smaller, so the numerator is negative.', 'Semakin perlahan bermakna laju akhir lebih kecil, jadi pengangkanya negatif.'), up ? T('The acceleration is positive.', 'Pecutannya positif.') : T('The acceleration is negative: a deceleration.', 'Pecutannya negatif: nyahpecutan.')), sp: 'xs' };
    },
  ];
  ...
  const g92m = [
    ...
    (r) => {
      ...
      const u = v + dec * t;
      const c = accCtx(r);
      return { q: T(`A ${c.ve} travelling at ${u} m/s decelerates uniformly to ${v} m/s in ${t} seconds. Find its acceleration.`, `Sebuah ${c.vm} yang bergerak pada ${u} m/s menyahpecut secara seragam kepada ${v} m/s dalam ${t} saat. Cari pecutannya.`), a: T(`$-${dec}$ m/s² (a deceleration of ${dec} m/s²)`, `$-${dec}$ m/s² (nyahpecutan ${dec} m/s²)`), w: W(T('Acceleration $=$ (final speed $-$ initial speed) $\\div$ time; keep that order.', 'Pecutan $=$ (laju akhir $-$ laju awal) $\\div$ masa; kekalkan susunan itu.'), `$a = \\dfrac{${v} - ${u}}{${t}} = ${-dec}$`, T(`The minus sign shows a deceleration of ${dec} m/s².`, `Tanda tolak menunjukkan nyahpecutan ${dec} m/s².`)), sp: 's' };
    },
    /* find u given v, a, t */
    (r) => {
      ...
      need(u > 0);
      const c = accCtx(r);
      return { q: T(`A ${c.ve} accelerates uniformly at ${a} m/s² for ${t} seconds and reaches a speed of ${v} m/s. Find its initial speed.`, `Sebuah ${c.vm} memecut secara seragam pada ${a} m/s² selama ${t} saat dan mencapai laju ${v} m/s. Cari laju awalnya.`), a: T(`${u} m/s`), w: W(T('Rearranging $v = u + at$ gives $u = v - at$.', 'Menyusun semula $v = u + at$ memberikan $u = v - at$.'), `$u = ${v} - ${a} \\times ${t} = ${u}$`, T(`${u} m/s`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      const t = u / dec;
      const c = accCtx(r);
      return { q: T(`A ${c.ve} travelling at ${u} m/s decelerates uniformly at ${dec} m/s². How long does it take to stop?`, `Sebuah ${c.vm} yang bergerak pada ${u} m/s menyahpecut secara seragam pada ${dec} m/s². Berapa lamakah masa yang diambil untuk berhenti?`), a: T(`${t} s`), w: W(T('The final speed is $0$ and the acceleration is negative, since the vehicle is slowing down.', 'Laju akhir ialah $0$ dan pecutannya negatif, kerana kenderaan itu semakin perlahan.'), `$t = \\dfrac{0 - ${u}}{-${dec}} = ${t}$`, T(`${t} s`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      const c = accCtx(r);
      return { q: T(`A ${c.ve} increases its speed from ${u} km/h to ${v} km/h in ${t} seconds. Find the acceleration in m/s².`, `Sebuah ${c.vm} meningkatkan lajunya daripada ${u} km/j kepada ${v} km/j dalam ${t} saat. Cari pecutan dalam m/s².`), a: T(`${n(a)} m/s²`), w: W(T('Change both speeds to m/s first, by dividing by $3.6$.', 'Tukar kedua-dua laju kepada m/s dahulu, dengan membahagi dengan $3.6$.'), `$${u} \\div 3.6 = ${n(round(u / 3.6, 3))}$, $${v} \\div 3.6 = ${n(round(v / 3.6, 3))}$`, `$\\dfrac{${n(round(v / 3.6, 3))} - ${n(round(u / 3.6, 3))}}{${t}} ${apx((v - u) / 3.6 / t, 3)} ${n(a)}$`, T(`${n(a)} m/s²`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      const c = accCtx(r);
      return { q: T(`A ${c.ve} accelerates from rest at ${a1} m/s² for ${t1} seconds, then brakes to a stop in ${t2} seconds. Find (a) its maximum speed, (b) its deceleration.`, `Sebuah ${c.vm} memecut dari keadaan pegun pada ${a1} m/s² selama ${t1} saat dan kemudian memberhentikan diri dalam ${t2} saat. Cari (a) laju maksimumnya, (b) nyahpecutannya.`), a: T(`(a) ${v} m/s (b) ${n(a2)} m/s²`), w: W(`(a) $v = 0 + ${a1} \\times ${t1} = ${v}$`, T(`The maximum speed is ${v} m/s.`, `Laju maksimum ialah ${v} m/s.`), `(b) $\\dfrac{0 - ${v}}{${t2}} ${apx(-v / t2, 2)} ${n(-a2)}$`, T(`So the deceleration is ${n(a2)} m/s².`, `Jadi nyahpecutannya ialah ${n(a2)} m/s².`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A ${c1.ve} speeds up from ${u1} m/s to ${v1} m/s in ${t1} s. A ${c2.ve} speeds up from ${u2} m/s to ${v2} m/s in ${t2} s. Which one has the greater acceleration?`, `Sebuah ${c1.vm} meningkatkan laju daripada ${u1} m/s kepada ${v1} m/s dalam ${t1} s. Sebuah ${c2.vm} meningkatkan laju daripada ${u2} m/s kepada ${v2} m/s dalam ${t2} s. Yang manakah mempunyai pecutan yang lebih besar?`), a: T(`${c1.ve}: ${n(a1)} m/s²; ${c2.ve}: ${n(a2)} m/s². The ${a1 > a2 ? c1.ve : c2.ve} has the greater acceleration.`, `${cap(c1.vm)}: ${n(a1)} m/s²; ${cap(c2.vm)}: ${n(a2)} m/s². ${cap(a1 > a2 ? c1.vm : c2.vm)} mempunyai pecutan yang lebih besar.`), w: W(T('Acceleration $=$ change in speed $\\div$ time, for each vehicle.', 'Pecutan $=$ perubahan laju $\\div$ masa, bagi setiap kenderaan.'), `$\\dfrac{${v1} - ${u1}}{${t1}} ${apx((v1 - u1) / t1, 2)} ${n(a1)}$`, `$\\dfrac{${v2} - ${u2}}{${t2}} ${apx((v2 - u2) / t2, 2)} ${n(a2)}$`, T(`$${n(a1)}$ against $${n(a2)}$: the ${a1 > a2 ? c1.ve : c2.ve} has the greater acceleration.`, `$${n(a1)}$ berbanding $${n(a2)}$: ${a1 > a2 ? c1.vm : c2.vm} mempunyai pecutan yang lebih besar.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The table shows the speed of a van at different times.<br>${TB([['Time (s)', ...ts], ['Speed (m/s)', ...vs2]])}Find the acceleration of the van.`, `Jadual menunjukkan laju sebuah van pada masa yang berbeza.<br>${TB([['Masa (s)', ...ts], ['Laju (m/s)', ...vs2]])}Cari pecutan van itu.`), a: T(`${a} m/s²`), w: W(T('Use any two columns: change in speed $\\div$ change in time.', 'Gunakan mana-mana dua lajur: perubahan laju $\\div$ perubahan masa.'), `$\\dfrac{${vs2[1]} - ${vs2[0]}}{2 - 0} = ${a}$`, T(`The same value comes from every pair of columns, so the acceleration is ${a} m/s².`, `Nilai yang sama diperoleh daripada setiap pasangan lajur, jadi pecutannya ialah ${a} m/s².`)), sp: 's' };
    },
  ];
  ...
  const g92a = [
    ...
    (r) => {
      ...
      const c = accCtx(r);
      return { q: T(`A ${c.ve} increases its speed from ${u} km/h to ${v1} km/h in ${t1} seconds, then changes speed to ${v2} km/h over the next ${t2} seconds. Find, in m/s², (a) the acceleration in the first stage, (b) the acceleration in the second stage.`, `Sebuah ${c.vm} meningkatkan lajunya daripada ${u} km/j kepada ${v1} km/j dalam ${t1} saat, kemudian mengubah lajunya kepada ${v2} km/j dalam ${t2} saat berikutnya. Cari, dalam m/s², (a) pecutan pada peringkat pertama, (b) pecutan pada peringkat kedua.`), a: T(`(a) ${n(a1)} m/s² (b) ${n(a2)} m/s²`), w: W(T('Change every speed to m/s first, by dividing by $3.6$.', 'Tukar setiap laju kepada m/s dahulu, dengan membahagi dengan $3.6$.'), `$${u} \\div 3.6 \\approx ${n(round(u / 3.6, 3))}$, $${v1} \\div 3.6 \\approx ${n(round(v1 / 3.6, 3))}$, $${v2} \\div 3.6 \\approx ${n(round(v2 / 3.6, 3))}$`, T(`(a) $\\dfrac{${n(round(v1 / 3.6, 3))} - ${n(round(u / 3.6, 3))}}{${t1}} \\approx ${n(a1)}$ m/s²`, `(a) $\\dfrac{${n(round(v1 / 3.6, 3))} - ${n(round(u / 3.6, 3))}}{${t1}} \\approx ${n(a1)}$ m/s²`), T(`(b) $\\dfrac{${n(round(v2 / 3.6, 3))} - ${n(round(v1 / 3.6, 3))}}{${t2}} \\approx ${n(a2)}$ m/s²`, `(b) $\\dfrac{${n(round(v2 / 3.6, 3))} - ${n(round(v1 / 3.6, 3))}}{${t2}} \\approx ${n(a2)}$ m/s²`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      const a1 = round((v1 - u1) / t1, 2), a2 = round((v2 - u2) / t2, 2);
      const c1 = accCtx(r), c2 = accCtx(r);
      return { q: T(`A ${c1.ve} and a ${c2.ve} both increase their speed from ${u1} m/s to ${v1} m/s, but the ${c1.ve} takes ${t1} s while the ${c2.ve} takes ${t2} s. Without calculating, state which vehicle has the greater acceleration. Then verify by calculation.`, `Sebuah ${c1.vm} dan sebuah ${c2.vm} kedua-duanya meningkatkan laju daripada ${u1} m/s kepada ${v1} m/s, tetapi ${c1.vm} mengambil masa ${t1} s manakala ${c2.vm} mengambil masa ${t2} s. Tanpa mengira, nyatakan kenderaan yang manakah mempunyai pecutan yang lebih besar. Kemudian sahkan dengan pengiraan.`), w: W(T('Both have the same change in speed, so the one that takes less time must have the greater acceleration.', 'Kedua-duanya mempunyai perubahan laju yang sama, jadi yang mengambil masa lebih singkat mesti mempunyai pecutan yang lebih besar.'), `$\\dfrac{${v1} - ${u1}}{${t1}} ${apx((v1 - u1) / t1, 2)} ${n(a1)}$`, `$\\dfrac{${v2} - ${u2}}{${t2}} ${apx((v2 - u2) / t2, 2)} ${n(a2)}$`, T(`The ${t1 < t2 ? c1.ve : c2.ve} takes less time and has the larger value, $${n(Math.max(a1, a2))}$ m/s².`, `${cap(t1 < t2 ? c1.vm : c2.vm)} mengambil masa lebih singkat dan mempunyai nilai yang lebih besar, $${n(Math.max(a1, a2))}$ m/s².`)), a: T(`The ${t1 < t2 ? c1.ve : c2.ve} (same change in speed but less time means greater acceleration). ${c1.ve}: $${n(a1)}$ m/s²; ${c2.ve}: $${n(a2)}$ m/s².`, `${cap(t1 < t2 ? c1.vm : c2.vm)} (perubahan laju yang sama tetapi masa yang lebih singkat bermakna pecutan yang lebih besar). ${cap(c1.vm)}: $${n(a1)}$ m/s²; ${cap(c2.vm)}: $${n(a2)}$ m/s².`), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A car's speed decreases from ${u} m/s to ${v} m/s in ${t} s while moving in the same fixed direction. (a) Calculate the acceleration, keeping the correct sign. (b) Explain what the negative sign means physically.`, `Laju sebuah kereta berkurang daripada ${u} m/s kepada ${v} m/s dalam ${t} s semasa bergerak dalam arah tetap yang sama. (a) Kira pecutan, dengan mengekalkan tanda yang betul. (b) Terangkan maksud tanda negatif itu secara fizikal.`), w: W(T('Acceleration $=$ (final speed $-$ initial speed) $\\div$ time; keep that order.', 'Pecutan $=$ (laju akhir $-$ laju awal) $\\div$ masa; kekalkan susunan itu.'), `(a) $a = \\dfrac{${v} - ${u}}{${t}} ${apx((v - u) / t, 2)} ${n(a)}$`, T('(b) The minus sign shows that the speed is falling: the car is slowing down while still moving in the same direction.', '(b) Tanda tolak menunjukkan laju semakin berkurang: kereta itu semakin perlahan sementara masih bergerak dalam arah yang sama.')), a: P([T(`$a = \\dfrac{${v} - ${u}}{${t}} = ${n(a)}$ m/s²`), T('The negative sign shows the speed is decreasing (a deceleration), while the car still moves in the same direction.', 'Tanda negatif menunjukkan laju semakin berkurang (nyahpecutan), sementara kereta itu masih bergerak dalam arah yang sama.')]), sp: 'm' };
    },
  ...
  const g92e2 = [
    ...
    (r) => {
      ...
      const c = accCtx(r);
      return { q: T(`A ${c.ve} starts at ${u} m/s and accelerates at ${a} m/s² for ${t} s. What is its final speed?<br>${opts.map((o, j) => `(${L[j]}) ${n(o)} m/s`).join('&emsp;')}`, `Sebuah ${c.vm} bermula pada ${u} m/s dan memecut pada ${a} m/s² selama ${t} s. Apakah laju akhirnya?<br>${opts.map((o, j) => `(${L[j]}) ${n(o)} m/s`).join('&emsp;')}`), a: T(`(${L[cor]}) ${v} m/s`), w: W(T(`Use $v = u + at$ with $u = ${u}$, $a = ${a}$ and $t = ${t}$.`, `Gunakan $v = u + at$ dengan $u = ${u}$, $a = ${a}$ dan $t = ${t}$.`), `$v = ${u} + ${a} \\times ${t} = ${v}$`, T(`Answer: (${L[cor]}).`, `Jawapan: (${L[cor]}).`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      const c = accCtx(r);
      return { q: T(`A ${c.ve} travels at a constant ${v} m/s for ${t} seconds. Find its acceleration.`, `Sebuah ${c.vm} bergerak pada ${v} m/s yang malar selama ${t} saat. Cari pecutannya.`), a: T('$0$ m/s²', '$0$ m/s²'), w: W(T('Acceleration $=$ (final speed $-$ initial speed) $\\div$ time.', 'Pecutan $=$ (laju akhir $-$ laju awal) $\\div$ masa.'), `$\\dfrac{${v} - ${v}}{${t}} = 0$`, T('The speed does not change, so the acceleration is zero.', 'Laju tidak berubah, jadi pecutannya sifar.')), sp: 'xs' };
    },
  ];
  const g92m2 = [
    ...
    (r) => {
      ...
      const t = round((v - u) / a, 2);
      const c = accCtx(r);
      return { q: T(`A ${c.ve} moving at ${u} m/s accelerates uniformly at ${a} m/s². Find the time taken to reach ${v} m/s.`, `Sebuah ${c.vm} yang bergerak pada ${u} m/s memecut secara seragam pada ${a} m/s². Cari masa yang diambil untuk mencapai ${v} m/s.`), a: T(`${n(t)} s`), w: W(T('Rearranging $v = u + at$ gives $t = \\dfrac{v - u}{a}$.', 'Menyusun semula $v = u + at$ memberikan $t = \\dfrac{v - u}{a}$.'), `$t = \\dfrac{${v} - ${u}}{${a}} ${apx((v - u) / a, 2)} ${n(t)}$`, T(`${n(t)} s`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      const a = round((v - u) / t, 2);
      const c = accCtx(r);
      return { q: T(`A ${c.ve}'s speed changes from ${u} m/s to ${v} m/s in ${t} s. State its acceleration, using a negative value if it is a deceleration.`, `Laju sebuah ${c.vm} berubah daripada ${u} m/s kepada ${v} m/s dalam ${t} s. Nyatakan pecutannya, menggunakan nilai negatif jika ia nyahpecutan.`), a: T(`${n(a)} m/s²`), w: W(T('Acceleration $=$ (final speed $-$ initial speed) $\\div$ time.', 'Pecutan $=$ (laju akhir $-$ laju awal) $\\div$ masa.'), `$\\dfrac{${v} - ${u}}{${t}} ${apx((v - u) / t, 2)} ${n(a)}$`, T('The value is negative, which shows a deceleration.', 'Nilainya negatif, yang menunjukkan nyahpecutan.')), sp: 's' };
    },
    ...
    (r) => {
      ...
      const cc = accCtx(r);
      return { q: T(`A ${cc.ve} has $u = ${u}$ m/s, $a = ${a}$ m/s², $t = ${t}$ s and $v = ${v}$ m/s, connected by $v = u + at$. If the value of ${blankKey === 'v' ? '$v$' : blankKey === 'u' ? '$u$' : blankKey === 'a' ? '$a$' : '$t$'} is hidden, find it from the other three.`, `Sebuah ${cc.vm} mempunyai $u = ${u}$ m/s, $a = ${a}$ m/s², $t = ${t}$ s dan $v = ${v}$ m/s, dihubungkan oleh $v = u + at$. Jika nilai ${blankKey === 'v' ? '$v$' : blankKey === 'u' ? '$u$' : blankKey === 'a' ? '$a$' : '$t$'} disembunyikan, cari nilainya daripada tiga yang lain.`), a: T(`${n(parts2[blankKey])}${blankKey === 'v' || blankKey === 'u' ? ' m/s' : blankKey === 'a' ? ' m/s²' : ' s'}`), w: W(T('Rearrange $v = u + at$ to make the hidden quantity the subject.', 'Susun semula $v = u + at$ untuk menjadikan kuantiti yang tersembunyi sebagai perkara rumus.'), blankKey === 'v' ? `$v = ${u} + ${a} \\times ${t} = ${v}$` : blankKey === 'u' ? `$u = ${v} - ${a} \\times ${t} = ${u}$` : blankKey === 'a' ? `$a = \\dfrac{${v} - ${u}}{${t}} = ${a}$` : `$t = \\dfrac{${v} - ${u}}{${a}} = ${t}$`), sp: 's' };
    },
  ];
  const g92a2 = [
    ...
    (r) => {
      ...
      const c = accCtx(r);
      return { q: T(`A ${c.ve} moving at ${u} m/s decelerates at ${dec} m/s² for ${t1} s, then accelerates at ${acc2} m/s² for ${t2} s. Find its final speed.`, `Sebuah ${c.vm} yang bergerak pada ${u} m/s menyahpecut pada ${dec} m/s² selama ${t1} s, kemudian memecut pada ${acc2} m/s² selama ${t2} s. Cari laju akhirnya.`), a: T(`${n(vEnd)} m/s`), w: W(T('Use $v = u + at$ for each stage, with a negative $a$ while it is slowing down.', 'Gunakan $v = u + at$ bagi setiap peringkat, dengan $a$ negatif semasa ia semakin perlahan.'), `$${u} - ${dec} \\times ${t1} = ${vMid}$`, `$${vMid} + ${acc2} \\times ${t2} = ${n(vEnd)}$`, T(`${n(vEnd)} m/s`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      const c = accCtx(r);
      return { q: T(`A ${c.ve} starts at ${uKmh} km/h and accelerates uniformly at ${a} m/s² for ${t} s. Find its final speed in (a) m/s, (b) km/h.`, `Sebuah ${c.vm} bermula pada ${uKmh} km/j dan memecut secara seragam pada ${a} m/s² selama ${t} s. Cari laju akhirnya dalam (a) m/s, (b) km/j.`), a: T(`(a) ${n(vMs)} m/s (b) ${n(vKmh)} km/h`, `(a) ${n(vMs)} m/s (b) ${n(vKmh)} km/j`), w: W(T(`The acceleration is in m/s², so change the initial speed to m/s: $${uKmh} \\div 3.6 = ${n(uMs)}$.`, `Pecutan dalam m/s², jadi tukar laju awal kepada m/s: $${uKmh} \\div 3.6 = ${n(uMs)}$.`), `(a) $v = ${n(uMs)} + ${a} \\times ${t} = ${n(vMs)}$`, `(b) $${n(vMs)} \\times 3.6 ${apx(vMs * 3.6, 1)} ${n(vKmh)}$`), sp: 'l' };
    },
  ];
