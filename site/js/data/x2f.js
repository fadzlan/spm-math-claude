...
(function () {
  ...
  E112.push((r) => { // image of a point: many phrasings
    ...
    return { q: fill(r.pick(forms), { P: `${pt(P)}`, v: tv(v) }), a: T(`$${pt(Q)}$`), w: T(`Add the components: $${n(P[0])} + ${SPM.par(v[0])}$ and $${n(P[1])} + ${SPM.par(v[1])}$.`, `Tambah komponen: $${n(P[0])} + ${SPM.par(v[0])}$ dan $${n(P[1])} + ${SPM.par(v[1])}$.`), sp: 's' };
  });
  E112.push((r) => { // moves described in words -> new position
    ...
    return { q: fill(r.pick(forms), { P: pt(P), mv: move(v), Sa: cap(s.a), sa: s.a }), a: T(`$${pt(Q)}$`), sp: 's' };
  });
  E112.push((r) => { // words -> column vector, and vector -> words
    ...
    if (r.chance(0.55)) {
      ...
      return { q: fill(r.pick(forms), { mv: move(v) }), a: T(`$${tv(v)}$`), w: T('Horizontal movement on top (right is positive), vertical movement below (up is positive).', 'Pergerakan mengufuk di atas (kanan positif), pergerakan mencancang di bawah (atas positif).'), sp: 's' };
    }
    const forms = [
    ...
    return { q: fill(r.pick(forms), { v: tv(v) }), a: T(`Every point moves ${moveEN(v)}.`, `Setiap titik bergerak ${moveMS(v)}.`), sp: 's' };
  });
  E112.push((r) => { // vector from P -> P'
    ...
    return { q: fill(r.pick(forms), { P: pt(P), Q: pt(Q) }), a: T(`$${tv(v)}$`), w: T('Subtract: image minus object.', 'Tolak: imej tolak objek.'), sp: 's' };
  });
  E112.push((r) => { // object from image
    ...
    return { q: fill(r.pick(forms), { Q: pt(Q), v: tv(v) }), a: T(`$${pt(P)}$`), w: T('Work backwards: subtract the vector from the image.', 'Kerja ke belakang: tolak vektor daripada imej.'), sp: 's' };
  });
  E112.push((r) => { // figure: read the translation
    ...
    return { q: fill(r.pick(forms), { nn: noun(k), o: names.join(''), i: names.map((c) => c + "'").join('') }), fig: fig({ O, I, names }), a: T(`$${tv(v)}$`), w: T('Compare the position of one vertex and its image.', 'Bandingkan kedudukan satu bucu dan imejnya.'), sp: 's' };
  });
  E112.push((r) => { // figure: image of one vertex
    ...
    return { q: fill(r.pick(forms), { nn: noun(k), o: names.join(''), v: tv(v), p: names[j] + "'", x: names[j] }), fig: fig({ O, names }), a: T(`$${names[j]}${pt(O[j])} \\to ${names[j]}'${pt(I[j])}$`), sp: 's' };
  });
  E112.push((r) => { // true / false statements
    ...
    const b = r.pick(bank);
    return { q: T(`True or false? ${b[0]}`, `Benar atau palsu? ${b[1]}`), a: T(`${b[2] ? 'True' : 'False'}. ${b[3]}`, `${b[2] ? 'Benar' : 'Palsu'}. ${b[4]}`), sp: 's' };
  });
  E112.push((r) => { // MCQ: which vector
    ...
    return { q: T(`Which column vector represents the translation that maps $${pt(P)}$ onto $${pt(Q)}$?${m.opts.en}`, `Vektor lajur yang manakah mewakili translasi yang memetakan $${pt(P)}$ ke $${pt(Q)}$?${m.opts.ms}`), a: m.ans, w: T('Image minus object.', 'Imej tolak objek.'), sp: 's' };
  });
  E112.push((r) => { // fill in the blank
    ...
    return { q: T(`Fill in the blanks: the translation $${tv(v)}$ moves each point ${Math.abs(v[0])} unit(s) ______ and ${Math.abs(v[1])} unit(s) ______. (Choose from: to the left, to the right, up, down.)`, `Isi tempat kosong: translasi $${tv(v)}$ menggerakkan setiap titik ${Math.abs(v[0])} unit ______ dan ${Math.abs(v[1])} unit ______. (Pilih daripada: ke kiri, ke kanan, ke atas, ke bawah.)`), a: T(`${s1.en}; ${s2.en}`, `${s1.ms}; ${s2.ms}`), sp: 's' };
  });
  E112.push((r) => { // table completion
    ...
    return { q: T(`The table shows three points and their images under the translation $${tv(v)}$. Complete the missing ${hide === 'image' ? 'images' : 'objects'}.<br>${tbl}`, `Jadual menunjukkan tiga titik dan imejnya di bawah translasi $${tv(v)}$. Lengkapkan ${hide === 'image' ? 'imej' : 'objek'} yang tertinggal.<br>${tblM}`), a: T(ans), sp: 'm' };
  });
  E112.push((r) => { // grid counting: figure, units right/up
    ...
    return { q: T(`Look at the diagram. Which vertex of the image corresponds to $${names[0]}$, and how many units to the ${v[0] > 0 ? 'right' : 'left'} and how many units ${v[1] > 0 ? 'up' : 'down'} has the shape moved?`, `Lihat rajah. Bucu imej yang manakah sepadan dengan $${names[0]}$, dan berapa unit bentuk itu telah bergerak ke ${v[0] > 0 ? 'kanan' : 'kiri'} dan berapa unit ke ${v[1] > 0 ? 'atas' : 'bawah'}?`), fig: fig({ O, I, names }), a: T(`${names[0]}'; ${u(Math.abs(v[0]))} ${v[0] > 0 ? 'right' : 'left'} and ${u(Math.abs(v[1]))} ${v[1] > 0 ? 'up' : 'down'}`, `${names[0]}'; ${Math.abs(v[0])} unit ke ${v[0] > 0 ? 'kanan' : 'kiri'} dan ${Math.abs(v[1])} unit ke ${v[1] > 0 ? 'atas' : 'bawah'}`), sp: 's' };
  });
  ...
  M112.push((r) => { // shape image coordinates given vertex list
    ...
    return { q: fill(f, { nn: noun(k), Nn: cap(noun(k)), o: names.join(''), vl: vlist(names, O), v: tv(v) }), fig: fig({ O, names }), a: ansPts(names, I, "'"), sp: 'm' };
  });
  M112.push((r) => { // figure: find vector and state vertex
    ...
    return { q: fill(r.pick(forms), { nn: noun(k), Nn: cap(noun(k)), o: names.join(''), p: names[j] + "'", x: names[j] }), fig: fig({ O, I, names, noImgLab: false }), a: T(`(a) $${tv(v)}$ (b) $${names[j]}'${pt(I[j])}$`), sp: 'm' };
  });
  M112.push((r) => { // unknown components
    ...
    const L = (x) => x;
    if (kind === 'obj') return { q: T(`The translation $${tv(v)}$ maps $P(p,\\ ${b})$ onto $P'(${Q[0]},\\ q)$. Find $p$ and $q$.`, `Translasi $${tv(v)}$ memetakan $P(p,\\ ${b})$ ke $P'(${Q[0]},\\ q)$. Cari $p$ dan $q$.`), a: T(`$p = ${a},\\ q = ${Q[1]}$`), sp: 's' };
    if (kind === 'img') return { q: T(`A translation $\\begin{pmatrix} h \\\\ k \\end{pmatrix}$ maps $A${pt(P)}$ onto $A'${pt(Q)}$. Find $h$ and $k$.`, `Translasi $\\begin{pmatrix} h \\\\ k \\end{pmatrix}$ memetakan $A${pt(P)}$ ke $A'${pt(Q)}$. Cari $h$ dan $k$.`), a: T(`$h = ${v[0]},\\ k = ${v[1]}$`), sp: 's' };
    return { q: T(`The point $B(m,\\ n)$ is mapped onto $B'${pt(Q)}$ by the translation $${tv(v)}$. Find the values of $m$ and $n$.`, `Titik $B(m,\\ n)$ dipetakan ke $B'${pt(Q)}$ oleh translasi $${tv(v)}$. Cari nilai $m$ dan $n$.`), a: T(`$m = ${a},\\ n = ${b}$`), sp: 's' };
  });
  M112.push((r) => { // distance moved
    ...
    const f = r.pick(forms);
    return { q: fill(f, { v: tv(v), P: pt(P), Q: pt(Q) }), a: T(`$${tr[2]}$ units${f === forms[2] ? ' (metres)' : ''}`, `$${tr[2]}$ unit${f === forms[2] ? ' (meter)' : ''}`), w: T(`$\\sqrt{${tr[0]}^2 + ${tr[1]}^2} = ${tr[2]}$`), sp: 's' };
  });
  M112.push((r) => { // derive vector from one pair and apply to another
    ...
    return { q: fill(r.pick(forms), { P: pt(P), Q: pt(Q), R: pt(R) }), a: T(`$${pt(addv(R, v))}$`), w: T(`Translation vector $${tv(v)}$.`, `Vektor translasi $${tv(v)}$.`), sp: 'm' };
  });
  M112.push((r) => { // inverse translation
    ...
    return { q: fill(r.pick(forms), { v: tv(v), P: pt(P), Q: pt(Q) }), a: T(`$${tv([-v[0], -v[1]])}$`), w: T('Reverse both signs.', 'Songsangkan kedua-dua tanda.'), sp: 's' };
  });
  M112.push((r) => { // segment
    ...
    return { q: fill(r.pick(forms), { A: pt(A), B: pt(B), v: tv(v) }), a: T(`$A'${pt(addv(A, v))},\\ B'${pt(addv(B, v))}$`), w: T('$A\'B\'$ has the same length as $AB$ and is parallel to it.', '$A\'B\'$ mempunyai panjang yang sama dengan $AB$ dan selari dengannya.'), sp: 's' };
  });
  M112.push((r) => { // parallelogram 4th vertex
    ...
    const f = r.pick(forms);
    return { q: fill(f, { g: M(given), x: letters[which] }), a: T(`$${letters[which]}${pt(V[which])}$`), w: T(`Opposite sides are parallel and equal: the translation $${L3} \\to ${L2}$ is $${tv(tvv)}$, and it also maps ${letters[which]} onto ${L1}.`, `Sisi bertentangan adalah selari dan sama: translasi $${L3} \\to ${L2}$ ialah $${tv(tvv)}$, dan ia juga memetakan ${letters[which]} ke ${L1}.`), sp: 'm' };
  });
  M112.push((r) => { // spot the error
    ...
    need(!same(e.w, Q));
    return { q: T(`A student says the image of $P${pt(P)}$ under the translation $${tv(v)}$ is $${pt(e.w)}$. Is this correct? If not, state the mistake and give the correct image.`, `Seorang murid berkata imej $P${pt(P)}$ di bawah translasi $${tv(v)}$ ialah $${pt(e.w)}$. Adakah ini betul? Jika tidak, nyatakan kesilapan dan berikan imej yang betul.`), a: T(`Not correct: the student ${e.en}. The image is $${pt(Q)}$.`, `Tidak betul: murid itu ${e.ms}. Imej ialah $${pt(Q)}$.`), sp: 'm' };
  });
  M112.push((r) => { // quadrant / axis
    const v = vec(r, 6, false), P = [r.nz(-5, 5), r.nz(-5, 5)], Q = addv(P, v);
    const kind = r.pick(['axisx', 'axisy', 'origin']);
    if (kind === 'axisx') return { q: T(`Find the translation of the form $\\begin{pmatrix} 0 \\\\ k \\end{pmatrix}$ that moves $A${pt(P)}$ onto the $x$-axis.`, `Cari translasi berbentuk $\\begin{pmatrix} 0 \\\\ k \\end{pmatrix}$ yang menggerakkan $A${pt(P)}$ ke atas paksi-$x$.`), a: T(`$${tv([0, -P[1]])}$`), w: T('The $y$-coordinate must become 0.', 'Koordinat-$y$ mesti menjadi 0.'), sp: 's' };
    if (kind === 'axisy') return { q: T(`Find the translation of the form $\\begin{pmatrix} h \\\\ 0 \\end{pmatrix}$ that moves $A${pt(P)}$ onto the $y$-axis.`, `Cari translasi berbentuk $\\begin{pmatrix} h \\\\ 0 \\end{pmatrix}$ yang menggerakkan $A${pt(P)}$ ke atas paksi-$y$.`), a: T(`$${tv([-P[0], 0])}$`), w: T('The $x$-coordinate must become 0.', 'Koordinat-$x$ mesti menjadi 0.'), sp: 's' };
    return { q: T(`What translation moves the point $A${pt(P)}$ onto the origin?`, `Apakah translasi yang menggerakkan titik $A${pt(P)}$ ke asalan?`), a: T(`$${tv([-P[0], -P[1]])}$`), sp: 's' };
  });
  M112.push((r) => { // MCQ image
    const P = [r.int(-4, 4), r.int(-4, 4)], v = vec(r, 4, true), Q = addv(P, v);
    const W = [[P[0] - v[0], P[1] - v[1]], [P[0] + v[1], P[1] + v[0]], [P[0] + v[0], P[1] - v[1]], [P[0] - v[0], P[1] + v[1]]];
    const wr = W.filter((w, i) => !same(w, Q) && W.findIndex((x) => same(x, w)) === i).slice(0, 3);
    need(wr.length === 3);
    const m = mcq(r, T(`$${pt(Q)}$`), wr.map((w) => T(`$${pt(w)}$`)));
    return { q: T(`What is the image of $${pt(P)}$ under the translation $${tv(v)}$?${m.opts.en}`, `Apakah imej bagi $${pt(P)}$ di bawah translasi $${tv(v)}$?${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  ...
  A112.push((r) => { // recover the object from the image (figure with image only, or listed)
    ...
    return { q: fill(r.pick(forms), { nn: noun(k), Nn: cap(noun(k)), i: names.map((c) => c + "'").join(''), o: names.join(''), v: tv(v), il: M(plist(names, I, "'")) }), fig: asFig ? render({ items }) : undefined, a: ansPts(names, O), w: T('Subtract the vector from each image vertex.', 'Tolak vektor daripada setiap bucu imej.'), sp: 'm' };
  });
  A112.push((r) => { // two unknown vector components + unknown point
    ...
    return { q: T(`A translation $\\begin{pmatrix} h \\\\ k \\end{pmatrix}$ maps $A${pt(A)}$ onto $A'${pt(A1)}$ and $B(p,\\ q)$ onto $B'${pt(B1)}$. Find $h$, $k$, $p$ and $q$.`, `Translasi $\\begin{pmatrix} h \\\\ k \\end{pmatrix}$ memetakan $A${pt(A)}$ ke $A'${pt(A1)}$ dan $B(p,\\ q)$ ke $B'${pt(B1)}$. Cari $h$, $k$, $p$ dan $q$.`), a: T(`$h = ${v[0]},\\ k = ${v[1]},\\ p = ${B[0]},\\ q = ${B[1]}$`), w: T('First find $h$ and $k$ from $A$ and $A\'$.', 'Cari $h$ dan $k$ daripada $A$ dan $A\'$ dahulu.'), sp: 'm' };
  });
  A112.push((r) => { // three part chain
    ...
    const f = r.pick(forms);
    return { q: T(f[0], f[1]), a: T(f[2], f[3]), sp: 'l' };
  });
  A112.push((r) => { // explain bank
    ...
    const b = r.pick(bank);
    return { q: T(b[0], b[1]), a: T(b[2], b[3]), sp: 'm' };
  });
  A112.push((r) => { // equal and parallel, compare vectors
  ...
  A112.push((r) => { // t, 2t vector
    ...
    return { q: T(`The point $P${pt(P)}$ is mapped onto ${given} by the translation $\\begin{pmatrix} t \\\\ ${c === 1 ? '' : c === -1 ? '-' : c}t \\end{pmatrix}$. Find the value of $t$ and the missing coordinate.`, `Titik $P${pt(P)}$ dipetakan ke ${given} oleh translasi $\\begin{pmatrix} t \\\\ ${c === 1 ? '' : c === -1 ? '-' : c}t \\end{pmatrix}$. Cari nilai $t$ dan koordinat yang tidak diketahui.`), a: T(ansT), w: T('Use the known coordinate to form an equation for $t$.', 'Gunakan koordinat yang diketahui untuk membentuk persamaan bagi $t$.'), sp: 'm' };
  });
  ...
  A112.push((r) => { // preserved measures
    ...
    const f = r.pick(forms);
    return { q: T(f[0], f[1]), a: T(f[2], f[3]), sp: 'm' };
  });
  ...
  A112.push((r) => { // statements: which are true
    ...
    return { q: T(`Which of these statements about a translation are true?<br>${list}`, `Antara pernyataan berikut tentang translasi, yang manakah benar?<br>${listM}`), a: T(`${ok.split('').join(', ')} ${ok.length === 1 ? 'is' : 'are'} true; the others are false.`, `${ok.split('').join(', ')} benar; yang lain palsu.`), sp: 'm' };
  });
  ...
  E113.push((r) => { // image of a point in an axis
    ...
    return { q: fill(f, { P: pt(P), ax }), a: f === forms[2] ? T(`${which.en} changes sign; $${pt(Q)}$`, `${which.ms} bertukar tanda; $${pt(Q)}$`) : T(`$${pt(Q)}$`), sp: 's' };
  });
  E113.push((r) => { // shape in an axis, with figure
    ...
    const ax = l.kind === 'y' ? T('$x$-axis', 'paksi-$x$') : T('$y$-axis', 'paksi-$y$');
    return { q: fill(r.pick(forms), { nn: noun(k), o: names.join(''), i: names.map((c) => c + "'").join(''), ax }), fig: fig({ O, names }), a: ansPts(names, I, "'"), sp: 'm' };
  });
  E113.push((r) => { // point in x = k or y = k
    ...
    return { q: fill(r.pick(forms), { P: pt(P), e: l.eq }), fig: r.chance(0.4) ? fig({ O: [P], names: ['P'], mirror: l, mirrorLabel: l.eq.replace(' = ', '=') }) : undefined, a: T(`$${pt(Q)}$`), w: T(`Distance from the line is ${u(ldist(P, l))}; $P'$ is the same distance on the other side.`, `Jarak dari garis ialah ${n(ldist(P, l))} unit; $P'$ berada pada jarak yang sama di sebelah yang lain.`), sp: 's' };
  });
  E113.push((r) => { // distance to mirror line and image
    ...
    const f = r.pick(forms);
    return { q: fill([f[0], f[1]], { P: pt(P), d: n(d), e: l.eq }), a: T(f[2], f[3]), w: T('The mirror line is the perpendicular bisector of $PP\'$.', 'Garis cermin ialah pembahagi dua sama serenjang bagi $PP\'$.'), sp: 's' };
  });
  E113.push((r) => { // point on the mirror line is invariant
    ...
    return { q: fill(r.pick(forms), { P: pt(P), e: l.eq }), a: T(`$${pt(P)}$: it lies on the mirror line, so it does not move (an invariant point).`, `$${pt(P)}$: ia terletak pada garis cermin, jadi ia tidak bergerak (titik tak berubah).`), sp: 's' };
  });
  ...
  E113.push((r) => { // which axis
    ...
    return { q: fill(r.pick(forms), { P: pt(P), Q: pt(Q) }), a: T(l.kind === 'y' ? 'The $x$-axis (the $y$-coordinate changes sign)' : 'The $y$-axis (the $x$-coordinate changes sign)', l.kind === 'y' ? 'Paksi-$x$ (koordinat-$y$ bertukar tanda)' : 'Paksi-$y$ (koordinat-$x$ bertukar tanda)'), sp: 's' };
  });
  E113.push((r) => { // figure with mirror line drawn: read image vertex
    ...
    return { q: fill(r.pick(forms), { nn: noun(k), o: names.join(''), e: l.eq, p: names[j] + "'", x: names[j] }), fig: fig({ O, names, mirror: l, mirrorLabel: l.eq.replace(' = ', '=') }), a: T(`$${names[j]}'${pt(I[j])}$`), sp: 's' };
  });
  E113.push((r) => { // sentence completion
    ...
    const b = r.pick(bank);
    return { q: T(b[0], b[1]), a: T(b[2], b[3]), sp: 's' };
  });
  E113.push((r) => { // table
    const l = r.pick(AX());
    ...
    return { q: T(`Complete the table of images under a reflection in the ${ax.en}.<br>${SPM.table(rows, { head: ['Point', 'Object', 'Image'] })}`, `Lengkapkan jadual imej di bawah pantulan pada ${ax.ms}.<br>${SPM.table(rows, { head: ['Titik', 'Objek', 'Imej'] })}`), a: T(I.map((p, i) => `$${'ABC'[i]}'${pt(p)}$`).join(', ')), sp: 'm' };
  });
  E113.push((r) => { // letters/objects unchanged in a vertical mirror
    const sym = [['A'], ['H'], ['M'], ['O'], ['T'], ['U'], ['W'], ['X'], ['I'], ['V'], ['Y']], asym = ['F', 'G', 'J', 'L', 'P', 'R', 'S', 'Z', 'N', 'K', 'Q'];
    const c = r.pick(sym)[0], W = r.sample(asym, 3);
    const m = mcq(r, T(c, c), W.map((x) => T(x, x)));
    return { q: T(`A vertical mirror line is placed to the right of each capital letter. Which letter looks exactly the same as its image?${m.opts.en}`, `Satu garis cermin menegak diletakkan di sebelah kanan setiap huruf besar. Huruf yang manakah kelihatan sama seperti imejnya?${m.opts.ms}`), a: m.ans, w: T('The letter is symmetrical about a vertical line.', 'Huruf itu simetri pada garis menegak.'), sp: 's' };
  });
  E113.push((r) => { // MCQ image in an axis
    ...
    const ax = l.kind === 'y' ? T('$x$-axis', 'paksi-$x$') : T('$y$-axis', 'paksi-$y$');
    const m = mcq(r, T(`$${pt(Q)}$`), wr.map((w) => T(`$${pt(w)}$`)));
    return { q: T(`What is the image of $${pt(P)}$ under a reflection in the ${ax.en}?${m.opts.en}`, `Apakah imej bagi $${pt(P)}$ di bawah pantulan pada ${ax.ms}?${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  ...
  M113.push((r) => { // shape in x=k / y=k
    ...
    return { q: fill(r.pick(forms), { nn: noun(k), Nn: cap(noun(k)), o: names.join(''), i: names.map((c) => c + "'").join(''), vl: M(vlist(names, O)), e: l.eq }), fig: fig({ O, names }), a: ansPts(names, I, "'"), sp: 'm' };
  });
  M113.push((r) => { // mirror line from a point and its image
    ...
    return { q: fill(r.pick(forms), { P: pt(P), Q: pt(Q) }), a: T(`$${l.name ? (l.kind === 'x' ? 'x = 0' : 'y = 0') : l.eq}$`), w: T(`The mirror line passes through the midpoint of $PP'$ and is perpendicular to it: $${mid}$.`, `Garis cermin melalui titik tengah $PP'$ dan serenjang dengannya: $${mid}$.`), sp: 's' };
  });
  M113.push((r) => { // mirror line from a figure
    ...
    return { q: fill(r.pick(forms), { nn: noun(k), o: names.join(''), i: names.map((c) => c + "'").join('') }), fig: fig({ O, I, names }), a: T(`$${l.eq}$`), w: T('Find the midpoint of a point and its image.', 'Cari titik tengah suatu titik dan imejnya.'), sp: 's' };
  });
  M113.push((r) => { // diagonal reflection y=x / y=-x
    ...
    return { q: fill(r.pick(forms), { nn: noun(O.length), o: names.join(''), vl: M(vlist(names, O)), e: l.eq }), fig: fig({ O, names, mirror: l, mirrorLabel: l.eq.replace(' = ', '=') }), a: ansPts(names, I, "'"), w: l.kind === 'd1' ? T('In $y = x$ the coordinates swap.', 'Pada $y = x$ koordinat bertukar tempat.') : T('In $y = -x$ the coordinates swap and change sign.', 'Pada $y = -x$ koordinat bertukar tempat dan bertukar tanda.'), sp: 'm' };
  });
  ...
  M113.push((r) => { // invariant points among a list
    ...
    return { q: T(`The mirror line is $${l.eq}$. Which of the points $${listed}$ are invariant under the reflection?`, `Garis cermin ialah $${l.eq}$. Antara titik $${listed}$, yang manakah tak berubah di bawah pantulan itu?`), a: T(`${inv.join(' and ')}: they lie on the mirror line.`, `${inv.join(' dan ')}: ia terletak pada garis cermin.`), sp: 's' };
  });
  M113.push((r) => { // AA' distance
    ...
    return { q: fill(r.pick(forms), { P: pt(P), e: l.eq }), a: T(`$${pt(Q)}$; distance $= ${n(2 * d)}$ units`, `$${pt(Q)}$; jarak $= ${n(2 * d)}$ unit`), sp: 's' };
  });
  M113.push((r) => { // MCQ mirror line
    ...
    return { q: T(`Under a reflection, $P${pt(P)}$ is mapped onto $P'${pt(Q)}$. Which is the equation of the mirror line?${m.opts.en}`, `Di bawah satu pantulan, $P${pt(P)}$ dipetakan ke $P'${pt(Q)}$. Yang manakah persamaan garis cermin?${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  ...
  M113.push((r) => { // reconstruct object
    ...
    return { q: fill(r.pick(forms), { nn: noun(k), Nn: cap(noun(k)), i: names.map((c) => c + "'").join(''), o: names.join(''), il: M(plist(names, I, "'")), lt }), a: ansPts(names, O), w: T('A reflection is its own inverse: reflect the image in the same line.', 'Pantulan ialah songsangan bagi dirinya sendiri: pantulkan imej pada garis yang sama.'), sp: 'm' };
  });
  ...
  M113.push((r) => { // vertices on the mirror line
    ...
    return { q: T(`The diagram shows ${noun(k).en} $${names.join('')}$ and the mirror line $${l.eq}$. Write the coordinates of the image and state which vertices are invariant.`, `Rajah menunjukkan ${noun(k).ms} $${names.join('')}$ dan garis cermin $${l.eq}$. Tulis koordinat imej dan nyatakan bucu yang tak berubah.`), fig: fig({ O, names, mirror: l, mirrorLabel: l.eq.replace(' = ', '=') }), a: T(`$${plist(names, I, "'")}$; invariant: ${inv.join(', ')}`, `$${plist(names, I, "'")}$; tak berubah: ${inv.join(', ')}`), sp: 'm' };
  });
  M113.push((r) => { // real life distances
    ...
    const b = r.pick(bank);
    return { q: T(b[0], b[1]), a: T(b[2], b[3]), w: T('The image is as far behind the mirror as the object is in front.', 'Imej berada di belakang cermin sejauh objek berada di hadapannya.'), sp: 's' };
  });
  M113.push((r) => { // midpoint reasoning
    ...
    return { q: T(`A reflection maps $A${pt(P)}$ onto $A'${pt(Q)}$. (a) Find the midpoint of $AA'$. (b) Hence write the equation of the mirror line.`, `Satu pantulan memetakan $A${pt(P)}$ ke $A'${pt(Q)}$. (a) Cari titik tengah $AA'$. (b) Seterusnya, tulis persamaan garis cermin.`), a: T(`(a) $${pt(mid)}$ (b) $${l.eq}$`), sp: 'm' };
  });
  ...
  A113.push((r) => { // reconstruct object across a diagonal line (figure or list)
    ...
    return { q: fill(r.pick(forms), { nn: noun(k), Nn: cap(noun(k)), i: names.map((c) => c + "'").join(''), o: names.join(''), il: M(plist(names, I, "'")), e: l.eq }), fig: asFig ? render({ items }) : undefined, a: ansPts(names, O), w: T('Reflect the image in the same line (a reflection is its own inverse); check that each midpoint lies on the line.', 'Pantulkan imej pada garis yang sama (pantulan ialah songsangan bagi dirinya sendiri); semak bahawa setiap titik tengah terletak pada garis itu.'), sp: 'm' };
  });
  A113.push((r) => { // find mirror line from two pairs (axis-parallel or diagonal)
    ...
    return { q: fill(r.pick(forms), { P: pt(P), P1: pt(P1), Q: pt(Q), Q1: pt(Q1) }), a: T(`$${l.eq}$. The midpoints $${pt(mid)}$ and $${pt(mid2)}$ lie on this line, and each object–image segment is perpendicular to it.`, `$${l.eq}$. Titik tengah $${pt(mid)}$ dan $${pt(mid2)}$ terletak pada garis ini, dan setiap tembereng objek–imej serenjang dengannya.`), sp: 'l' };
  });
  A113.push((r) => { // mirror line not drawn: figure with object/image, diagonal
    ...
    const I = rpts(O, l), names = nm(r, k);
    need(inb(I, 8));
    return { q: T(`The diagram shows ${noun(k).en} $${names.join('')}$ and its image $${names.map((c) => c + "'").join('')}$ under a reflection. Find the equation of the mirror line and explain how you know.`, `Rajah menunjukkan ${noun(k).ms} $${names.join('')}$ dan imejnya $${names.map((c) => c + "'").join('')}$ di bawah satu pantulan. Cari persamaan garis cermin dan terangkan bagaimana anda tahu.`), fig: fig({ O, I, names }), a: T(`$${l.eq}$: it passes through the midpoints of $${names[0]}${names[0]}'$ and $${names[1]}${names[1]}'$ and is perpendicular to them.`, `$${l.eq}$: ia melalui titik tengah $${names[0]}${names[0]}'$ dan $${names[1]}${names[1]}'$ dan serenjang dengannya.`), sp: 'm' };
  });
  A113.push((r) => { // unknown k
    ...
    const f = r.pick(forms);
    return { q: T(f[0], f[1]), a: T(f[2], f[2].replace('units', 'unit')), sp: 'm' };
  });
  ...
  A113.push((r) => { // image in y = x + k / y = -x + k
    ...
    const dv = subv(Q, P);
    return { q: fill(r.pick(forms), { P: pt(P), e: l.eq }), a: T(`$P'${pt(Q)}$. The midpoint $${pt(mid)}$ satisfies $${l.eq}$; $PP'$ has direction $${tv(dv)}$, which is at right angles to the line.`, `$P'${pt(Q)}$. Titik tengah $${pt(mid)}$ memenuhi $${l.eq}$; $PP'$ mempunyai arah $${tv(dv)}$, yang serenjang dengan garis itu.`), sp: 'm' };
  });
  A113.push((r) => { // statements true/false selection
    ...
    const list = S4.map((s, i) => `<b>${'ABCD'[i]}.</b> ${s[0]}`).join('<br>'), listM = S4.map((s, i) => `<b>${'ABCD'[i]}.</b> ${s[1]}`).join('<br>');
    const ok = S4.map((s, i) => (s[2] ? 'ABCD'[i] : '')).join('');
    return { q: T(`Which of these statements about a reflection are true?<br>${list}`, `Antara pernyataan berikut tentang pantulan, yang manakah benar?<br>${listM}`), a: T(`${ok.split('').join(', ')} ${ok.length === 1 ? 'is' : 'are'} true; the others are false.`, `${ok.split('').join(', ')} benar; yang lain palsu.`), sp: 'm' };
  });
  A113.push((r) => { // isometry check with lengths
    ...
    const a = s2(O), b = s2(I);
    return { q: T(`Triangle $ABC$ with $A${pt(O[0])}$, $B${pt(O[1])}$, $C${pt(O[2])}$ is reflected in the line $${l.eq}$. Find $A'$, $B'$, $C'$, then show that $AB = A'B'$ by comparing $AB^2$ and $A'B'^2$.`, `Segi tiga $ABC$ dengan $A${pt(O[0])}$, $B${pt(O[1])}$, $C${pt(O[2])}$ dipantulkan pada garis $${l.eq}$. Cari $A'$, $B'$, $C'$, kemudian tunjukkan bahawa $AB = A'B'$ dengan membandingkan $AB^2$ dan $A'B'^2$.`), a: T(`$${plist(['A', 'B', 'C'], I, "'")}$; $AB^2 = ${a[0]}$ and $A'B'^2 = ${b[0]}$, so $AB = A'B'$.`, `$${plist(['A', 'B', 'C'], I, "'")}$; $AB^2 = ${a[0]}$ dan $A'B'^2 = ${b[0]}$, jadi $AB = A'B'$.`), sp: 'l' };
  });
  ...
  A113.push((r) => { // 3 candidate points mirror-check
    ...
    return { q: T(`The point $P${pt(P)}$ is reflected in the line $${l.eq}$. Which of the points $${list}$ is its image? Justify by checking the midpoint.`, `Titik $P${pt(P)}$ dipantulkan pada garis $${l.eq}$. Antara titik $${list}$, yang manakah imejnya? Berikan justifikasi dengan menyemak titik tengah.`), a: T(`${ok}$${pt(Q)}$: the midpoint of $P$ and this point lies on $${l.eq}$ and the segment is perpendicular to it.`.replace(/^(\w)\$/, '$1 $'), `${ok}$${pt(Q)}$: titik tengah $P$ dan titik ini terletak pada $${l.eq}$ dan tembereng itu serenjang dengannya.`.replace(/^(\w)\$/, '$1 $')), sp: 'm' };
  });
  ...
  E114.push((r) => { // point about the origin (quarter/half turn)
    ...
    return { q: fill(r.pick(forms), { P: pt(P), t }), fig: r.chance(0.5) ? fig({ O: [P], names: ['P'] }) : undefined, a: T(`$${pt(Q)}$`), sp: 's' };
  });
  E114.push((r) => { // point about a marked centre C
    ...
    const t = a === 180 ? T('$180^\\circ$', '$180^\\circ$') : turnL(a, d);
    return { q: fill(r.pick(forms), { P: pt(P), C: pt(C), t }), fig: fig({ O: [P], names: ['P'], centre: C }), a: T(`$${pt(Q)}$`), w: T('Compare the position of $P$ relative to $C$ before and after the turn.', 'Bandingkan kedudukan $P$ relatif kepada $C$ sebelum dan selepas putaran.'), sp: 's' };
  });
  E114.push((r) => { // half-turn: centre is the midpoint
    ...
    return { q: fill(r.pick(forms), { P: pt(P), C: pt(C) }), a: T(`$${pt(Q)}$`), w: T('$C$ is the midpoint of $PP\'$.', '$C$ ialah titik tengah $PP\'$.'), sp: 's' };
  });
  E114.push((r) => { // shape about the origin with figure
    ...
    return { q: fill(r.pick(forms), { nn: noun(s.k), Nn: cap(noun(s.k)), o: s.names.join(''), i: s.names.map((c) => c + "'").join(''), t }), fig: figR(s, { noImg: true }), a: ansPts(s.names, s.I, "'"), sp: 'm' };
  });
  E114.push((r) => { // describe: P -> P' about origin (P on axis)
    ...
    return { q: fill(r.pick(forms), { P: pt(P), Q: pt(Q) }), fig: fig({ O: [P], I: [Q], names: ['P'] }), a: a === 180 ? T('$180^\\circ$ (a half-turn) about $O$', '$180^\\circ$ (separuh pusingan) pada $O$') : T(`$${a}^\\circ$ ${d === 'cw' ? 'clockwise' : 'anticlockwise'} about $O$`, `$${a}^\\circ$ ${d === 'cw' ? 'ikut arah jam' : 'lawan arah jam'} pada $O$`), sp: 's' };
  });
  ...
  E114.push((r) => { // distances
    ...
    return { q: fill(forms[0], { P: pt(P), C: pt(C), r: String(tr[2]) }), a: T(`$CP' = ${tr[2]}$ units: every point stays at the same distance from the centre.`, `$CP' = ${tr[2]}$ unit: setiap titik kekal pada jarak yang sama dari pusat.`), sp: 's' };
  });
  E114.push((r) => { // centre is invariant
    ...
    const t = a === 180 ? T('$180^\\circ$', '$180^\\circ$') : turnL(a, d);
    return { q: fill(r.pick(forms), { C: pt(C), t }), a: T(`$${pt(C)}$: the centre of rotation does not move.`, `$${pt(C)}$: pusat putaran tidak bergerak.`), sp: 's' };
  });
  E114.push((r) => { // MCQ image
    ...
    need(wr.length === 3);
    const m = mcq(r, T(`$${pt(Q)}$`), wr.map((w) => T(`$${pt(w)}$`)));
    return { q: T(`What is the image of $${pt(P)}$ under a rotation of $90^\\circ$ ${d === 'cw' ? 'clockwise' : 'anticlockwise'} about the origin?${m.opts.en}`, `Apakah imej bagi $${pt(P)}$ di bawah putaran $90^\\circ$ ${d === 'cw' ? 'ikut arah jam' : 'lawan arah jam'} pada asalan?${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  ...
  M114.push((r) => { // shape about a vertex
    ...
    const t = s.a === 180 ? T('$180^\\circ$', '$180^\\circ$') : turnL(s.a, s.d);
    return { q: fill(r.pick(forms), { nn: noun(s.k), Nn: cap(noun(s.k)), o: s.names.join(''), i: s.names.map((c) => c + "'").join(''), c: cn, vl: M(vlist(s.names, s.O)), t }), fig: figR(s, { noImg: true, centre: undefined }), a: T(`$${plist(s.names, s.I, "'")}$; ${cn} is invariant`, `$${plist(s.names, s.I, "'")}$; ${cn} tak berubah`), sp: 'm' };
  });
  M114.push((r) => { // shape about marked centre
    ...
    const t = s.a === 180 ? T('$180^\\circ$', '$180^\\circ$') : turnL(s.a, s.d);
    return { q: fill(r.pick(forms), { nn: noun(s.k), o: s.names.join(''), C: pt(s.C), t }), fig: figR(s, { noImg: true }), a: ansPts(s.names, s.I, "'"), sp: 'm' };
  });
  M114.push((r) => { // describe fully, centre marked
    ...
    return { q: fill(r.pick(forms), { nn: noun(s.k), o: s.names.join(''), i: s.names.map((c) => c + "'").join(''), C: pt(s.C) }), fig: figR(s), a: rotFull(s.a, s.d, s.C), sp: 's' };
  });
  M114.push((r) => { // find the centre of a half-turn
    ...
    return { q: fill(r.pick(forms), { P: pt(P), Q: pt(Q) }), a: T(`$${pt(C)}$`), w: T('The centre is the midpoint of $AA\'$.', 'Pusat ialah titik tengah $AA\'$.'), sp: 's' };
  });
  ...
  M114.push((r) => { // reverse: find object
    ...
    const t = s.a === 180 ? T('$180^\\circ$', '$180^\\circ$') : turnL(s.a, s.d);
    return { q: fill(r.pick(forms), { nn: noun(s.k), Nn: cap(noun(s.k)), i: s.names.map((c) => c + "'").join(''), o: s.names.join(''), il: M(plist(s.names, s.I, "'")), t, c: centreT(s.C) }), a: ansPts(s.names, s.O), w: T('Rotate the image through the same angle in the opposite direction.', 'Putarkan imej melalui sudut yang sama pada arah yang bertentangan.'), sp: 'm' };
  });
  M114.push((r) => { // inverse rotation description
    ...
    return { q: fill(forms[0], { t, c: centreT(C) }), a: back, w: T('Same centre, same angle, opposite direction.', 'Pusat yang sama, sudut yang sama, arah bertentangan.'), sp: 's' };
  });
  M114.push((r) => { // CP = CP' distance
    ...
    need(inb([P, Q], 20));
    return { q: T(`$P${pt(P)}$ is rotated about $C${pt(C)}$ to $P'${pt(Q)}$. Show that $CP = CP'$ by calculating both distances.`, `$P${pt(P)}$ diputarkan pada $C${pt(C)}$ ke $P'${pt(Q)}$. Tunjukkan bahawa $CP = CP'$ dengan menghitung kedua-dua jarak.`), a: T(`$CP = \\sqrt{${Math.abs(P[0] - C[0])}^2 + ${Math.abs(P[1] - C[1])}^2} = ${tr[2]}$ and $CP' = \\sqrt{${Math.abs(Q[0] - C[0])}^2 + ${Math.abs(Q[1] - C[1])}^2} = ${tr[2]}$.`), sp: 'm' };
  });
  ...
  M114.push((r) => { // real-life coordinates
    ...
    const t = a === 180 ? T('$180^\\circ$', '$180^\\circ$') : turnL(a, d);
    const c = r.pick(ctx);
    return { q: fill([c.en, c.ms], { P: pt(P), t }), a: T(`$${pt(rotate(P, [0, 0], a, d))}$`), sp: 's' };
  });
  M114.push((r) => { // angle PCP'
    ...
    return { q: T(forms[0][0], forms[0][1]), fig: fig({ O: [P], I: [Q], names: ['P'], centre: C }), a: a === 180 ? T('$\\angle PCP\' = 180^\\circ$ (a half-turn)', '$\\angle PCP\' = 180^\\circ$ (separuh pusingan)') : T(`$\\angle PCP' = 90^\\circ$, ${d === 'cw' ? 'clockwise' : 'anticlockwise'}`, `$\\angle PCP' = 90^\\circ$, ${d === 'cw' ? 'ikut arah jam' : 'lawan arah jam'}`), sp: 's' };
  });
  ...
  M114.push((r) => { // offset from centre
    ...
    return { q: T(`The centre of rotation is $C${pt(C)}$. Point $P$ is ${dxE} and ${dyE}. Find the coordinates of the image of $P$ under a rotation of ${t.en} about $C$.`, `Pusat putaran ialah $C${pt(C)}$. Titik $P$ berada ${dxM} dan ${dyM}. Cari koordinat imej $P$ di bawah putaran ${t.ms} pada $C$.`), a: T(`$${pt(Q)}$`), w: T(`$P = ${pt(P)}$`), sp: 's' };
  });
  ...
  A114.push((r) => { // locate the centre from an object-image figure
    ...
    return { q: fill(r.pick(forms), { nn: noun(s.k), o: s.names.join(''), i: s.names.map((c) => c + "'").join('') }), fig: fig({ O: s.O, I: s.I, names: s.names }), a: rotFull(a, d, s.C), w: T('The centre is where the perpendicular bisectors of $AA\'$ and $BB\'$ meet.', 'Pusat ialah tempat pembahagi dua sama serenjang bagi $AA\'$ dan $BB\'$ bertemu.'), sp: 'l' };
  });
  A114.push((r) => { // recover the object; arbitrary centre
    ...
    return { q: fill(r.pick(forms), { nn: noun(s.k), Nn: cap(noun(s.k)), i: s.names.map((c) => c + "'").join(''), o: s.names.join(''), il: M(plist(s.names, s.I, "'")), C: pt(s.C), t }), fig: asFig ? render({ items }) : undefined, a: ansPts(s.names, s.O), w: T('Rotate each image vertex about $C$ through the same angle in the opposite direction.', 'Putarkan setiap bucu imej pada $C$ melalui sudut yang sama pada arah bertentangan.'), sp: 'l' };
  });
  ...
  A114.push((r) => { // solve for centre algebraically
    ...
    need(inb([P, Q], 9));
    const t = turnL(90, d);
    return { q: T(`Under a rotation of ${t.en} about an unknown centre $C(h,\\ k)$, $A${pt(P)}$ maps onto $A'${pt(Q)}$. Find $h$ and $k$.`, `Di bawah putaran ${t.ms} pada pusat yang tidak diketahui $C(h,\\ k)$, $A${pt(P)}$ dipetakan ke $A'${pt(Q)}$. Cari $h$ dan $k$.`), a: T(`$h = ${C[0]},\\ k = ${C[1]}$`), w: T('$CA$ and $CA\'$ are equal and perpendicular; test the candidate centre or set up equations from $CA = CA\'$ and the quarter-turn.', '$CA$ dan $CA\'$ sama panjang dan serenjang; uji pusat calon atau bentuk persamaan daripada $CA = CA\'$ dan suku pusingan.'), sp: 'l' };
  });
  ...
  A114.push((r) => { // true statements selection
    ...
    const list = S4.map((s, i) => `<b>${'ABCD'[i]}.</b> ${s[0]}`).join('<br>'), listM = S4.map((s, i) => `<b>${'ABCD'[i]}.</b> ${s[1]}`).join('<br>');
    const ok = S4.map((s, i) => (s[2] ? 'ABCD'[i] : '')).join('');
    return { q: T(`Which of these statements about rotations are true?<br>${list}`, `Antara pernyataan berikut tentang putaran, yang manakah benar?<br>${listM}`), a: T(`${ok.split('').join(', ')} ${ok.length === 1 ? 'is' : 'are'} true; the others are false.`, `${ok.split('').join(', ')} benar; yang lain palsu.`), sp: 'm' };
  });
  A114.push((r) => { // time-based rotation (multi-turn)
    ...
    return { q: T(`The tip of a windmill blade is at $P${pt(P)}$ (hub at the origin, units in metres). The blade turns $90^\\circ$ ${dirE} each second. (a) Through what total angle does it turn in ${t} second${t === 1 ? '' : 's'}? (b) Find the position of the tip after ${t} second${t === 1 ? '' : 's'}.`, `Hujung bilah kincir angin berada di $P${pt(P)}$ (hab di asalan, unit dalam meter). Bilah itu berputar $90^\\circ$ ${dirM} setiap saat. (a) Berapakah jumlah sudut ia berputar dalam ${t} saat? (b) Cari kedudukan hujung selepas ${t} saat.`), a: T(`(a) $${total}^\\circ$ ${dirE}${total > 360 ? ` (equivalent to $${eq === 0 ? 360 : eq}^\\circ$)` : ''} (b) $${pt(Q)}$`, `(a) $${total}^\\circ$ ${dirM}${total > 360 ? ` (setara dengan $${eq === 0 ? 360 : eq}^\\circ$)` : ''} (b) $${pt(Q)}$`), sp: 'l' };
  });
  ...
  E115.push((r) => { // order of a real-life object
    ...
    const f = r.pick(forms);
    return { q: fill(f, { x: T(o[0], o[1]), X: cap(T(o[0], o[1])) }), a: T(`${o[2]}`), w: o[2] === 1 ? T('It only fits onto itself after a full $360^\\circ$ turn, so its order is 1.', 'Ia hanya bertindih dengan dirinya selepas satu pusingan penuh $360^\\circ$, jadi peringkatnya ialah 1.') : undefined, sp: 'xs' };
  });
  E115.push((r) => { // order of a named shape
    ...
    return { q: fill(r.pick(forms), { x: shT(s) }), a: T(`${s[2]}`), sp: 'xs' };
  });
  E115.push((r) => { // figure: count the order
    ...
    return { q: fill(r.pick(forms), { n: T(kind[1], kind[2]) }), fig: symFig({ kind: kind[0], k }), a: T(`${k}`), w: T('The design fits onto itself once for each identical part.', 'Reka bentuk itu bertindih dengan dirinya sekali bagi setiap bahagian yang serupa.'), sp: 'xs' };
  });
  E115.push((r) => { // letters MCQ
    const c = r.pick(LET2), W = r.sample(LET1, 3);
    const m = mcq(r, T(c, c), W.map((x) => T(x, x)));
    return { q: T(`Which capital letter has rotational symmetry of order 2?${m.opts.en}`, `Huruf besar yang manakah mempunyai simetri putaran peringkat 2?${m.opts.ms}`), a: m.ans, w: T('It looks the same after a half-turn.', 'Ia kelihatan sama selepas separuh pusingan.'), sp: 's' };
  });
  E115.push((r) => { // letters: which has no rotational symmetry
    const c = r.pick(LET1), W = r.sample(LET2, 3);
    const m = mcq(r, T(c, c), W.map((x) => T(x, x)));
    return { q: T(`Which of these capital letters has no rotational symmetry (order 1)?${m.opts.en}`, `Antara huruf besar berikut, yang manakah tidak mempunyai simetri putaran (peringkat 1)?${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  E115.push((r) => { // order -> smallest angle (direct)
    ...
    return { q: fill(r.pick(forms), { k: String(k) }), a: T(`$${degTex(k)}$`), w: T(`$360^\\circ \\div ${k} = ${degTex(k)}$`), sp: 's' };
  });
  E115.push((r) => { // angle -> order
    ...
    return { q: fill(r.pick(forms), { a: degTex(k) }), a: T(`${k}`), w: T(`$360^\\circ \\div ${degTex(k)} = ${k}$`), sp: 's' };
  });
  ...
  E115.push((r) => { // classify shapes: which have order > 1
    ...
    return { q: T(`Which of these shapes have rotational symmetry of order greater than 1: ${list}?`, `Antara bentuk berikut, yang manakah mempunyai simetri putaran peringkat lebih daripada 1: ${listM}?`), a: T(yes.map((s) => s[0]).join('; '), yes.map((s) => s[1]).join('; ')), sp: 's' };
  });
  ...
  E115.push((r) => { // MCQ order of regular polygon
    ...
    return { q: T(`What is the order of rotational symmetry of a ${s[0]}?${m.opts.en}`, `Apakah peringkat simetri putaran bagi ${s[1]}?${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  ...
  M115.push((r) => { // regular polygon / order -> angle
    ...
    const f = r.pick(forms);
    return { q: fill(f, { k: String(k) }), a: f === forms[2] ? T(`$\\dfrac{1}{${k}}$ of a turn $= ${degTex(k)}$`) : T(`$${degTex(k)}$`), w: T(`Order $= ${k}$, so the angle is $360^\\circ \\div ${k}$.`, `Peringkat $= ${k}$, jadi sudutnya ialah $360^\\circ \\div ${k}$.`), sp: 's' };
  });
  M115.push((r) => { // angle -> order in a context
    ...
    const c = r.pick(ctx);
    return { q: fill([c[0] + ' What is the order of rotational symmetry?', c[1] + ' Apakah peringkat simetri putarannya?'], { a: degTex(k) }), a: T(`${k}`), w: T(`$360^\\circ \\div ${degTex(k)} = ${k}$`), sp: 's' };
  });
  M115.push((r) => { // smallest angle of an object
    ...
    const f = r.pick(forms);
    return { q: fill(f, { x: T(o[0], o[1]) }), a: T(`Order ${o[2]}, so $${degTex(o[2])}$`, `Peringkat ${o[2]}, jadi $${degTex(o[2])}$`), sp: 's' };
  });
  ...
  M115.push((r) => { // sectors figure: order & angle
    ...
    return { q: fill(r.pick(forms), { k: String(k) }), fig: symFig({ kind, k, seq }), a: T(`Order ${ord}; smallest angle $${degTex(ord)}$`, `Peringkat ${ord}; sudut terkecil $${degTex(ord)}$`), w: T('The order is the number of times the colouring pattern repeats.', 'Peringkat ialah bilangan kali corak warna itu berulang.'), sp: 'm' };
  });
  ...
  M115.push((r) => { // coordinates: order of a polygon
    ...
    return { q: fill(r.pick(forms), { nn: kind, o: names.join(''), vl: M(vlist(names, s.P)) }), fig: fig({ O: s.P, names }), a: T(`${s.ord}`), sp: 's' };
  });
  M115.push((r) => { // table of shapes
    ...
    return { q: T(`Complete the table.<br>${SPM.table(rows, { head: ['Shape', 'Order of rotational symmetry', 'Smallest angle of rotation'] })}`, `Lengkapkan jadual.<br>${SPM.table(rowsM, { head: ['Bentuk', 'Peringkat simetri putaran', 'Sudut putaran terkecil'] })}`), a: T(pool.map((s) => `${s[0]}: ${s[2]}, ${s[2] === 1 ? '360' : deg(s[2])}°`).join('; '), pool.map((s) => `${s[1]}: ${s[2]}, ${s[2] === 1 ? '360' : deg(s[2])}°`).join('; ')), sp: 'm' };
  });
  ...
  M115.push((r) => { // MCQ smallest angle
    ...
    return { q: T(`What is the smallest angle of rotational symmetry of ${shT(s).en}?${m.opts.en}`, `Apakah sudut simetri putaran terkecil bagi ${s[1]}?${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  ...
  M115.push((r) => { // petals angle between
    ...
    return { q: fill(c, { k: String(k) }), a: asks2 ? T(`$${degTex(k)}$; order ${k}`, `$${degTex(k)}$; peringkat ${k}`) : T(`$${degTex(k)}$`), sp: 's' };
  });
  M115.push((r) => { // lines and order
    ...
    return { q: fill(forms[0], { x: shT(s) }), a: T(`(a) ${s[3]} (b) ${s[2]}`), sp: 's' };
  });
  ...
  A115.push((r) => { // complete a partial design
    ...
    return { q: fill(r.pick(forms), { m: String(m), k: String(k) }), fig: symFig({ kind, k, only: m }), a: T(`${k - m} more blades; each is turned $${degTex(k)}$ from the previous one.`, `${k - m} bilah lagi; setiap bilah diputarkan $${degTex(k)}$ daripada yang sebelumnya.`), w: T(`Total ${k} parts, angle $360^\\circ \\div ${k}$.`, `Jumlah ${k} bahagian, sudut $360^\\circ \\div ${k}$.`), sp: 'm' };
  });
  ...
  A115.push((r) => { // statements selection
    ...
    const list = S4.map((s, i) => `<b>${'ABCD'[i]}.</b> ${s[0]}`).join('<br>'), listM = S4.map((s, i) => `<b>${'ABCD'[i]}.</b> ${s[1]}`).join('<br>');
    const ok = S4.map((s, i) => (s[2] ? 'ABCD'[i] : '')).join('');
    return { q: T(`Which of these statements are true?<br>${list}`, `Antara pernyataan berikut, yang manakah benar?<br>${listM}`), a: T(`${ok.split('').join(', ')} ${ok.length === 1 ? 'is' : 'are'} true; the others are false.`, `${ok.split('').join(', ')} benar; yang lain palsu.`), sp: 'm' };
  });
  ...
  E111.push((r) => { // classify a real-life scenario
    ...
    return { q: fill(r.pick(forms), { s: scenText(s) }), a: TY[s.t], sp: 'xs' };
  });
  E111.push((r) => { // is it an isometry (scenario with all 4 types)
    ...
    const iso = isoOf(s.t);
    return { q: fill(r.pick(forms), { s: scenText(s) }), a: iso ? T(`Yes: it is a ${TY[s.t].en}, which keeps size and shape.`, `Ya: ia ialah ${TY[s.t].ms}, yang mengekalkan saiz dan bentuk.`) : T('No: the size changes, so it is not an isometry (it is an enlargement or reduction).', 'Tidak: saiz berubah, jadi ia bukan isometri (ia ialah pembesaran atau pengecilan).'), sp: 's' };
  });
  E111.push((r) => { // MCQ classification
    ...
    return { q: T(`${s.en} This is an example of a:${m.opts.en}`, `${s.ms} Ini ialah contoh:${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  E111.push((r) => { // classify three at once
    ...
    return { q: T(`Match each situation with translation, reflection, rotation or enlargement.<br>${list}`, `Padankan setiap situasi dengan translasi, pantulan, putaran atau pembesaran.<br>${listM}`), a: T(three.map((s, i) => `(${['i', 'ii', 'iii'][i]}) ${TY[s.t].en}`).join('; '), three.map((s, i) => `(${['i', 'ii', 'iii'][i]}) ${TY[s.t].ms}`).join('; ')), sp: 'm' };
  });
  E111.push((r) => { // terms: object and image
    ...
    return { q: fill(r.pick(forms), { o: c.names.join(''), i: c.names.map((x) => x + "'").join('') }), fig: fig({ O: c.O, I: c.I, names: c.names }), a: T(`Object: $${c.names.join('')}$; image: $${c.names.map((x) => x + "'").join('')}$`, `Objek: $${c.names.join('')}$; imej: $${c.names.map((x) => x + "'").join('')}$`), sp: 's' };
  });
  E111.push((r) => { // corresponding vertex
    ...
    return { q: fill(r.pick(forms), { nn: noun(c.O.length), o: c.names.join(''), x: c.names[j] }), fig: fig({ O: c.O, I: c.I, names: c.names }), a: T(`$${c.names[j]}'$`), sp: 'xs' };
  });
  E111.push((r) => { // identify the transformation from a figure (type only)
    ...
    return { q: fill(r.pick(forms), { o: c.names.join(''), i: c.names.map((x) => x + "'").join('') }), fig: fig({ O: c.O, I: c.I, names: c.names }), a: TY[c.type], w: c.type === 'reflection' ? T('The image is a mirror image (the order of the letters is reversed).', 'Imej ialah imej cermin (susunan huruf disongsangkan).') : c.type === 'translation' ? T('The image is the same way round and every point moved the same way.', 'Imej menghadap arah yang sama dan setiap titik bergerak dengan cara yang sama.') : T('The image is turned but not flipped.', 'Imej diputarkan tetapi tidak dibalikkan.'), sp: 'xs' };
  });
  E111.push((r) => { // definitions MCQ
    ...
    return { q: T(`Which statement describes ${b[0]}?${m.opts.en}`, `Pernyataan yang manakah memerihalkan ${b[1]}?${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  E111.push((r) => { // preserved / not preserved
    ...
    return { q: fill(["Is the {p} of a shape preserved (unchanged) by a {t}?", "Adakah {p} suatu bentuk dikekalkan (tidak berubah) oleh {t}?"], { p: p[0], t: tn }), a: ans, sp: 's' };
  });
  E111.push((r) => { // T/F
    const bank = [
  ...
  E111.push((r) => { // given measure -> image measure
    ...
    const f = r.pick(forms);
    return { q: T(f[0], f[1]), a: T(f[2]), w: T('An isometry preserves lengths, angles and area.', 'Isometri mengekalkan panjang, sudut dan luas.'), sp: 's' };
  });
  E111.push((r) => { // which is an isometry: MCQ from 4 operations
    ...
    return { q: T(`Which of these operations is an isometry?${m.opts.en}`, `Antara operasi berikut, yang manakah isometri?${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  ...
  M111.push((r) => { // corresponding sides and angles
    ...
    const f = r.pick(forms);
    return { q: T(f[0], f[1]), fig: r.chance(0.5) ? fig({ O: c.O, I: c.I, names: nmC }) : undefined, a: T(`(a) $${nmC[i]}'${nmC[j]}'$ (b) $\\angle ${nmC[l]}'${nmC[i]}'${nmC[j]}'$; each is equal in size to the original.`, `(a) $${nmC[i]}'${nmC[j]}'$ (b) $\\angle ${nmC[l]}'${nmC[i]}'${nmC[j]}'$; setiap satunya sama saiz dengan yang asal.`), sp: 's' };
  });
  ...
  M111.push((r) => { // coordinates: identify the isometry
    ...
    return { q: fill(r.pick(forms), { o: names.join(''), i: names.map((x) => x + "'").join(''), ol: M(vlist(names, O)), il: M(plist(names, I, "'")) }), fig: r.chance(0.5) ? fig({ O, I, names }) : undefined, a: ans, sp: 'm' };
  });
  M111.push((r) => { // rule test with two points
    ...
    return { q: T(`A mapping sends every point $(x,\\ y)$ to $${rule.sym}$. Points $A${pt(A)}$ and $B${pt(B)}$ are mapped onto $A'$ and $B'$. Find $A'$ and $B'$, compare $AB^2$ with $A'B'^2$, and say whether the mapping is an isometry.`, `Satu pemetaan menghantar setiap titik $(x,\\ y)$ ke $${rule.sym}$. Titik $A${pt(A)}$ dan $B${pt(B)}$ dipetakan ke $A'$ dan $B'$. Cari $A'$ dan $B'$, bandingkan $AB^2$ dengan $A'B'^2$, dan nyatakan sama ada pemetaan itu isometri.`), a: T(`$A'${pt(A1)}$, $B'${pt(B1)}$; $AB^2 = ${d2(A, B)}$, $A'B'^2 = ${d2(A1, B1)}$. ${rule.iso ? `Equal: ${rule.w.en}, which is an isometry.` : `Not equal, so the mapping is not an isometry (${rule.w.en}).`}`, `$A'${pt(A1)}$, $B'${pt(B1)}$; $AB^2 = ${d2(A, B)}$, $A'B'^2 = ${d2(A1, B1)}$. ${rule.iso ? `Sama: ${rule.w.ms}, iaitu isometri.` : `Tidak sama, jadi pemetaan itu bukan isometri (${rule.w.ms}).`}`), sp: 'm' };
  });
  ...
  M111.push((r) => { // candidate triangles for the image
    ...
    return { q: T(`Triangle $PQR$ has sides ${a.join(', ')} cm. Which of these triangles could be the image of $PQR$ under an isometry?<br>${list}`, `Segi tiga $PQR$ mempunyai sisi ${a.join(', ')} cm. Antara segi tiga berikut, yang manakah boleh menjadi imej $PQR$ di bawah suatu isometri?<br>${list}`), a: T(`${ok}: it has the same three side lengths (in any order); the others have different side lengths.`, `${ok}: ia mempunyai tiga panjang sisi yang sama (dalam sebarang susunan); yang lain mempunyai panjang sisi yang berbeza.`), sp: 's' };
  });
  M111.push((r) => { // table of properties
    ...
    return { q: T(`Complete the table with "yes" if the property is preserved and "no" if it is not.<br>${SPM.table(rows, { head: ['Property', 'Translation', 'Reflection', 'Rotation'] })}`, `Lengkapkan jadual dengan "ya" jika sifat itu dikekalkan dan "tidak" jika tidak dikekalkan.<br>${SPM.table(rowsM, { head: ['Sifat', 'Translasi', 'Pantulan', 'Putaran'] })}`), a: T(ans, ansM), sp: 'm' };
  });
  M111.push((r) => { // reverse measures
    ...
    const f = r.pick(forms);
    return { q: T(f[0], f[1]), a: T(f[2]), w: T('Work backwards: an isometry preserves lengths, angles and area.', 'Kerja ke belakang: isometri mengekalkan panjang, sudut dan luas.'), sp: 's' };
  });
  ...
  M111.push((r) => { // properties that are preserved by all three
    ...
    return { q: T(`Which of these properties are preserved by every isometry (translation, reflection and rotation)? ${pick4.map((k) => PROP[k][0].en).join('; ')}.`, `Antara sifat berikut, yang manakah dikekalkan oleh setiap isometri (translasi, pantulan dan putaran)? ${pick4.map((k) => PROP[k][0].ms).join('; ')}.`), a: T(keep.map((k) => PROP[k][0].en).join('; '), keep.map((k) => PROP[k][0].ms).join('; ')), sp: 's' };
  });
  ...
  M111.push((r) => { // figure: describe (simple)
    ...
    return { q: fill(r.pick(forms), { o: names.join(''), i: names.map((x) => x + "'").join('') }), fig: fig({ O, I, names }), a: ans, sp: 's' };
  });
  ...
  A111.push((r) => { // solve for x with equal measures
    ...
    if (kind === 'len') {
      const L = a * x + b;
      need(L > 0 && dd !== 0);
      return { q: T(`A ${tn.en} maps $AB$ onto $A'B'$. $AB = (${a}x + ${b})$ cm and $A'B' = (${cc}x ${dd < 0 ? '-' : '+'} ${Math.abs(dd)})$ cm. Find $x$ and the length of $AB$.`, `${SPM.cap(tn.ms)} memetakan $AB$ ke $A'B'$. $AB = (${a}x + ${b})$ cm dan $A'B' = (${cc}x ${dd < 0 ? '-' : '+'} ${Math.abs(dd)})$ cm. Cari $x$ dan panjang $AB$.`), a: T(`${a}x + ${b} = ${cc}x ${dd < 0 ? '-' : '+'} ${Math.abs(dd)}$ gives $x = ${x}$; $AB = ${L}$ cm.`.replace(/^/, '$'), `${a}x + ${b} = ${cc}x ${dd < 0 ? '-' : '+'} ${Math.abs(dd)}$ memberi $x = ${x}$; $AB = ${L}$ cm.`.replace(/^/, '$')), w: T('Lengths are preserved, so the two expressions are equal.', 'Panjang dikekalkan, jadi kedua-dua ungkapan adalah sama.'), sp: 'm' };
    }
    const A0 = 10 * a + b * 2, Ang = a * x + b + 30;
    const ang1 = a * x + 20 + b, ang2 = cc * x + (a * x + 20 + b - cc * x);
    const d3 = ang1 - cc * x;
    need(ang1 < 170 && ang1 > 10 && Math.abs(d3) < 60 && d3 !== 0);
    return { q: T(`Under a ${tn.en}, $\\angle PQR = (${a}x + ${20 + b})^\\circ$ is mapped onto $\\angle P'Q'R' = (${cc}x ${d3 < 0 ? '-' : '+'} ${Math.abs(d3)})^\\circ$. Find the value of $x$ and the size of $\\angle PQR$.`, `Di bawah ${tn.ms}, $\\angle PQR = (${a}x + ${20 + b})^\\circ$ dipetakan ke $\\angle P'Q'R' = (${cc}x ${d3 < 0 ? '-' : '+'} ${Math.abs(d3)})^\\circ$. Cari nilai $x$ dan saiz $\\angle PQR$.`), a: T(`$x = ${x}$; $\\angle PQR = ${ang1}^\\circ$`), w: T('Angles are preserved, so the two expressions are equal.', 'Sudut dikekalkan, jadi kedua-dua ungkapan adalah sama.'), sp: 'm' };
  });
  ...
  A111.push((r) => { // multi-part with a reflection: images, lengths, what changes
    ...
    const ax = l.kind === 'y' ? T('$x$-axis', 'paksi-$x$') : T('$y$-axis', 'paksi-$y$');
    const d0 = d2(O[0], O[1]);
    return { q: T(`Triangle $${names.join('')}$ has vertices $${vlist(names, O)}$. (a) Find the image under a reflection in the ${ax.en}. (b) Show that $${names[0]}${names[1]}^2 = ${names[0]}'${names[1]}'^2$. (c) State one property that is preserved and one that is not.`, `Segi tiga $${names.join('')}$ mempunyai bucu $${vlist(names, O)}$. (a) Cari imej di bawah pantulan pada ${ax.ms}. (b) Tunjukkan bahawa $${names[0]}${names[1]}^2 = ${names[0]}'${names[1]}'^2$. (c) Nyatakan satu sifat yang dikekalkan dan satu yang tidak.`), a: T(`(a) $${plist(names, I, "'")}$ (b) both equal ${d2(O[0], O[1])} and ${d2(I[0], I[1])}: ${d2(O[0], O[1]) === d2(I[0], I[1]) ? 'equal' : 'differ'} (c) lengths are preserved; orientation is not (the image is a mirror image).`, `(a) $${plist(names, I, "'")}$ (b) ${d2(O[0], O[1])} dan ${d2(I[0], I[1])}: sama (c) panjang dikekalkan; orientasi tidak (imej ialah imej cermin).`), sp: 'l' };
  });
  ...
  E116.push((r) => { // real-life classification (three categories)
    ...
    return { q: fill(r.pick(forms), { x: T(it.en, it.ms) }), a: ans, sp: 's' };
  });
  ...
  E116.push((r) => { // grid candidate figure: which is congruent
    ...
    return { q: T('Which of the shapes $B$, $C$ and $D$ is congruent to shape $A$? Give a reason.', 'Antara bentuk $B$, $C$ dan $D$, yang manakah kongruen dengan bentuk $A$? Berikan sebab.'), fig: render({ items: cf.items, axes: false, win: cf.win }), a: T(`Shape $${ci}$: it has exactly the same size and shape as $A$ (it is only turned or flipped).`, `Bentuk $${ci}$: ia mempunyai saiz dan bentuk yang tepat sama seperti $A$ (ia hanya diputar atau dibalikkan).`), sp: 's' };
  });
  E116.push((r) => { // definition MCQ
    ...
    return { q: T(`${SPM.cap(b[0])} have:${m.opts.en}`, `${SPM.cap(b[1])} mempunyai:${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  E116.push((r) => { // isometry images are congruent
    ...
    return { q: T(...r.pick(forms)), a: T('They are congruent: a ' + tn.en + ' is an isometry, so lengths and angles are preserved.', 'Kedua-duanya kongruen: ' + tn.ms + ' ialah isometri, jadi panjang dan sudut dikekalkan.'), sp: 's' };
  });
  ...
  E116.push((r) => { // equal sides: congruent yes/no for two triangles
    ...
    return { q: T(...forms[0]), a: kind === 'same' ? T('Yes: the three side lengths are the same.', 'Ya: ketiga-tiga panjang sisi adalah sama.') : T('No: the side lengths are different.', 'Tidak: panjang sisi berbeza.'), sp: 's' };
  });
  ...
  E116.push((r) => { // MCQ pair
    ...
    return { q: kind === 'cong' ? T(`Which pair is congruent?${m.opts.en}`, `Pasangan yang manakah kongruen?${m.opts.ms}`) : T(`Which pair is similar but not congruent?${m.opts.en}`, `Pasangan yang manakah serupa tetapi tidak kongruen?${m.opts.ms}`), a: m.ans, sp: 's' };
  });
  ...
  M116.push((r) => { // triangles by sides: three kinds
    ...
    return { q: fill(r.pick(forms), { a: t.join(', '), b: b.join(', ') }), a: kind === 'cong' ? T(`Congruent: the sides (in order of size) are ${sortN(t).join(', ')} in both triangles.`, `Kongruen: sisi (mengikut susunan saiz) ialah ${sortN(t).join(', ')} bagi kedua-dua segi tiga.`) : kind === 'sim' ? T(`Similar but not congruent: the ratios ${ratios} are all equal to $${n(SPM.Fr.val(cmp.k))}$.`, `Serupa tetapi tidak kongruen: nisbah ${ratios} semuanya sama dengan $${n(SPM.Fr.val(cmp.k))}$.`) : T(`Neither: the ratios ${ratios} are not all equal.`, `Tidak kedua-duanya: nisbah ${ratios} tidak semuanya sama.`), sp: 's' };
  });
  ...
  M116.push((r) => { // similar: scale factor + missing length
    ...
    const f = r.pick(forms);
    return { q: T(f[0], f[1]), a: T(f[2], f[3]), w: T('Multiply every length by the same scale factor.', 'Darabkan setiap panjang dengan faktor skala yang sama.'), sp: 's' };
  });
  ...
  M116.push((r) => { // correspondence: triangle ABC ≅ PQR
    ...
    return { q: T(...forms[0]), a: T(`$${other[j]}${other[i2]}$ and $\\angle ${other[i2]}$`), w: T(`The vertices match in order: $${names.map((c, i) => c + ' \\leftrightarrow ' + other[i]).join(',\\ ')}$.`, `Bucu sepadan mengikut urutan: $${names.map((c, i) => c + ' \\leftrightarrow ' + other[i]).join(',\\ ')}$.`), sp: 's' };
  });
  M116.push((r) => { // statement evaluation with reason
    ...
    const b = r.pick(bank);
    return { q: T(`Is this statement true or false? Give a reason or a counterexample: "${b[0]}"`, `Adakah pernyataan ini benar atau palsu? Berikan sebab atau contoh yang menyangkal: "${b[1]}"`), a: T(`${b[2] ? 'True' : 'False'}. ${b[3]}`, `${b[2] ? 'Benar' : 'Palsu'}. ${b[4]}`), sp: 'm' };
  });
  ...
  A116.push((r) => { // congruent triangles: solve for x
    ...
    const kind = r.pick(['len', 'ang']);
    if (kind === 'len') return { q: T(`Triangle $ABC$ is congruent to triangle $PQR$ with $A \\leftrightarrow P$, $B \\leftrightarrow Q$, $C \\leftrightarrow R$. $AB = (${a}x + ${b})$ cm and $PQ = (${c}x ${d < 0 ? '-' : '+'} ${Math.abs(d)})$ cm. Find $x$ and $AB$.`, `Segi tiga $ABC$ kongruen dengan segi tiga $PQR$ dengan $A \\leftrightarrow P$, $B \\leftrightarrow Q$, $C \\leftrightarrow R$. $AB = (${a}x + ${b})$ cm dan $PQ = (${c}x ${d < 0 ? '-' : '+'} ${Math.abs(d)})$ cm. Cari $x$ dan $AB$.`), a: T(`$x = ${x}$; $AB = ${a * x + b}$ cm`), w: T('Corresponding sides of congruent triangles are equal.', 'Sisi sepadan bagi segi tiga kongruen adalah sama.'), sp: 'm' };
    const A1 = a * x + b + 20;
    const d3 = A1 - c * x;
    need(A1 < 160 && Math.abs(d3) < 70 && d3 !== 0);
    return { q: T(`Triangle $ABC$ is congruent to triangle $PQR$ with $A \\leftrightarrow P$, $B \\leftrightarrow Q$, $C \\leftrightarrow R$. $\\angle A = (${a}x + ${b + 20})^\\circ$ and $\\angle P = (${c}x ${d3 < 0 ? '-' : '+'} ${Math.abs(d3)})^\\circ$. Find $x$ and the size of $\\angle A$.`, `Segi tiga $ABC$ kongruen dengan segi tiga $PQR$ dengan $A \\leftrightarrow P$, $B \\leftrightarrow Q$, $C \\leftrightarrow R$. $\\angle A = (${a}x + ${b + 20})^\\circ$ dan $\\angle P = (${c}x ${d3 < 0 ? '-' : '+'} ${Math.abs(d3)})^\\circ$. Cari $x$ dan saiz $\\angle A$.`), a: T(`$x = ${x}$; $\\angle A = ${A1}^\\circ$`), w: T('Corresponding angles of congruent triangles are equal.', 'Sudut sepadan bagi segi tiga kongruen adalah sama.'), sp: 'm' };
  });
  ...
  A116.push((r) => { // angle-based similarity: find missing angles
    ...
    return { q: T(...forms[0]), a: T(`$\\angle C = ${a3}^\\circ$ and $\\angle R = ${a1}^\\circ$. Both triangles have angles $${a1}^\\circ, ${a2}^\\circ, ${a3}^\\circ$, so they are similar; without any side lengths we cannot tell whether they are congruent.`, `$\\angle C = ${a3}^\\circ$ dan $\\angle R = ${a1}^\\circ$. Kedua-dua segi tiga mempunyai sudut $${a1}^\\circ, ${a2}^\\circ, ${a3}^\\circ$, jadi ia serupa; tanpa sebarang panjang sisi kita tidak dapat menentukan sama ada ia kongruen.`), sp: 'm' };
  });
  ...
  A116.push((r) => { // scale drawing: real-life
    ...
    return { q: T(`A floor plan is drawn to a scale of 1 : ${k}. A room is ${real} m long in real life. Is the plan congruent or similar to the room? Find the length of the room on the plan in cm.`, `Pelan lantai dilukis pada skala 1 : ${k}. Sebuah bilik panjangnya ${real} m dalam kehidupan sebenar. Adakah pelan itu kongruen atau serupa dengan bilik itu? Cari panjang bilik itu pada pelan dalam cm.`), a: T(`Similar (same shape, different size). Length on plan $= ${real} \\times 100 \\div ${k} = ${n(round(real * 100 / k, 2))}$ cm.`, `Serupa (bentuk sama, saiz berbeza). Panjang pada pelan $= ${real} \\times 100 \\div ${k} = ${n(round(real * 100 / k, 2))}$ cm.`), sp: 'm' };
  });
  A116.push((r) => { // reasoning with congruent quadrilaterals
    ...
    return { q: T(forms[0][0], forms[0][1]), a: T(forms[0][2], forms[0][3]), sp: 'm' };
  });
