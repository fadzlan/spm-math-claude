...
(function () {
  ...
  E112.push((r) => { // true / false statements
    ...
    const b = r.pick(bank);
    return { q: T(`True or false? ${b[0]}`, `Betul atau salah? ${b[1]}`), a: T(`${b[2] ? 'True' : 'False'}. ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}. ${b[4]}`), w: W(T('A translation slides every point the same distance in the same direction; the top number of the vector is the horizontal move and the bottom number the vertical move.', 'Translasi menggelongsorkan setiap titik dengan jarak dan arah yang sama; nombor atas vektor ialah gerakan mengufuk dan nombor bawah gerakan mencancang.'), T(`${b[2] ? 'True' : 'False'}: ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}: ${b[4]}`)), sp: 's' };
  });
  ...
  E113.push((r) => { // T/F bank
    ...
    const b = r.pick(bank);
    return { q: T(`True or false? ${b[0]}`, `Betul atau salah? ${b[1]}`), a: T(`${b[2] ? 'True' : 'False'}. ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}. ${b[4]}`), w: W(T('A reflection flips the shape over the mirror line: it is an isometry, the mirror line is the perpendicular bisector of the segment joining a point to its image, and points on the mirror line do not move.', 'Pantulan membalikkan bentuk pada garis cermin: ia isometri, garis cermin ialah pembahagi dua sama serenjang bagi tembereng yang menyambung satu titik dengan imejnya, dan titik pada garis cermin tidak bergerak.'), T(`${b[2] ? 'True' : 'False'}: ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}: ${b[4]}`)), sp: 's' };
  });
  ...
  A113.push((r) => { // statements true/false selection
    ...
    const list = S4.map((s, i) => `<b>${'ABCD'[i]}.</b> ${s[0]}`).join('<br>'), listM = S4.map((s, i) => `<b>${'ABCD'[i]}.</b> ${s[1]}`).join('<br>');
    const ok = S4.map((s, i) => (s[2] ? 'ABCD'[i] : '')).join('');
    return { q: T(`Which of these statements about a reflection are true?<br>${list}`, `Antara pernyataan berikut tentang pantulan, yang manakah betul?<br>${listM}`), a: T(`${ok.split('').join(', ')} ${ok.length === 1 ? 'is' : 'are'} true; the others are false.`, `${ok.split('').join(', ')} betul; yang lain salah.`), w: W(T('A reflection is an isometry that flips the shape over the mirror line: lengths, angles and areas are unchanged, the orientation is reversed, every point of the mirror line is invariant, and reflecting twice in the same line undoes it.', 'Pantulan ialah isometri yang membalikkan bentuk pada garis cermin: panjang, sudut dan luas tidak berubah, orientasi disongsangkan, setiap titik pada garis cermin tak berubah, dan memantulkan dua kali pada garis yang sama membatalkannya.'), T(`Checking each statement against this: ${ok.split('').join(', ')} agree with it.`, `Menyemak setiap pernyataan dengan ini: ${ok.split('').join(', ')} menepatinya.`)), sp: 'm' };
  });
  ...
  E114.push((r) => { // TF bank
    ...
    const b = r.pick(bank);
    return { q: T(`True or false? ${b[0]}`, `Betul atau salah? ${b[1]}`), a: T(`${b[2] ? 'True' : 'False'}. ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}. ${b[4]}`), w: W(T('A rotation is fully described by its centre, angle and direction; it is an isometry, the centre is the only invariant point, and every point keeps its distance from the centre.', 'Putaran diperihalkan sepenuhnya oleh pusat, sudut dan arahnya; ia isometri, pusat ialah satu-satunya titik tak berubah, dan setiap titik mengekalkan jaraknya dari pusat.'), T(`${b[2] ? 'True' : 'False'}: ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}: ${b[4]}`)), sp: 's' };
  });
  ...
  E115.push((r) => { // T/F bank
    ...
    const b = r.pick(bank);
    return { q: T(`True or false? ${b[0]}`, `Betul atau salah? ${b[1]}`), a: T(`${b[2] ? 'True' : 'False'}. ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}. ${b[4]}`), w: W(T('The order is the number of positions in one full turn where the figure looks the same, and the smallest angle is $360^\\circ \\div \\text{order}$.', 'Peringkat ialah bilangan kedudukan dalam satu pusingan penuh yang menjadikan rajah kelihatan sama, dan sudut terkecil ialah $360^\\circ \\div \\text{peringkat}$.'), T(`${b[2] ? 'True' : 'False'}: ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}: ${b[4]}`)), sp: 's' };
  });
  E115.push((r) => { // fill in
    const bank = [
  ...
  E111.push((r) => { // T/F
    ...
    const b = r.pick(bank);
    return { q: T(`True or false? ${b[0]}`, `Betul atau salah? ${b[1]}`), a: T(`${b[2] ? 'True' : 'False'}. ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}. ${b[4]}`), w: W(T('An isometry keeps lengths, angles and area, so the image is congruent to the object; translation, reflection and rotation are isometries, an enlargement is not.', 'Isometri mengekalkan panjang, sudut dan luas, jadi imej kongruen dengan objek; translasi, pantulan dan putaran ialah isometri, pembesaran bukan.'), T(`${b[2] ? 'True' : 'False'}: ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}: ${b[4]}`)), sp: 's' };
  });
  ...
  E116.push((r) => { // T/F bank
    ...
    const b = r.pick(bank);
    return { q: T(`True or false? ${b[0]}`, `Betul atau salah? ${b[1]}`), a: T(`${b[2] ? 'True' : 'False'}. ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}. ${b[4]}`), w: W(T('Congruent $=$ same shape and same size; similar $=$ same shape with all lengths in the same ratio; every congruent pair is also similar, with scale factor $1$.', 'Kongruen $=$ bentuk dan saiz yang sama; serupa $=$ bentuk yang sama dengan semua panjang dalam nisbah yang sama; setiap pasangan kongruen juga serupa, dengan faktor skala $1$.'), T(`${b[2] ? 'True' : 'False'}: ${b[3]}`, `${b[2] ? 'Betul' : 'Salah'}: ${b[4]}`)), sp: 's' };
  });
