...
(function () {
  ...
  const g121e = [
    (r) => { const it = r.pick(SQ); return { q: T(`Is the following a statistical question? Answer Yes or No and give a reason.<br>"${it.q.en}"`, `Adakah yang berikut soalan statistik? Jawab Ya atau Tidak dan berikan sebab.<br>"${it.q.ms}"`), a: cat(YN(it.k === 'Y'), '. ', REASON[it.k]), sp: 's' }; },
    (r) => { const right = r.pick(SQY), w = r.sample(SQN, 3), o = mc(r, right.q, w.map((x) => x.q)); return { q: T(`Which one of these is a statistical question?<br>${o.en}`, `Yang manakah antara berikut ialah soalan statistik?<br>${o.ms}`), a: T(`(${o.letter}). ${REASON.Y.en}`, `(${o.letter}). ${REASON.Y.ms}`), sp: 's' }; },
    (r) => { const right = r.pick(SQN), w = r.sample(SQY, 3), o = mc(r, right.q, w.map((x) => x.q)); return { q: T(`Which one of these is NOT a statistical question?<br>${o.en}`, `Yang manakah antara berikut BUKAN soalan statistik?<br>${o.ms}`), a: T(`(${o.letter}). ${REASON[right.k].en}`, `(${o.letter}). ${REASON[right.k].ms}`), sp: 's' }; },
    (r) => { const v = r.pick(VARS); return { q: T(`Is the variable "${v.v.en}" categorical or numerical? If it is numerical, state whether it is discrete or continuous.`, `Adakah pemboleh ubah "${v.v.ms}" kategori atau berangka? Jika berangka, nyatakan sama ada ia diskret atau selanjar.`), a: TYPEL[v.t], sp: 's' }; },
    (r) => {
      ...
      return { q: T(`Which of these is ${nm.en}?<br>${o.en}`, `Yang manakah antara berikut ialah ${nm.ms}?<br>${o.ms}`), a: o.ans, sp: 's' };
    },
    (r) => { const nm = r.name(), a = r.pick(ACTS.map((l, i) => [i, r.pick(l)])); const o = mc(r, ST[a[0]], r.sample(ST.filter((_, i) => i !== a[0]), 3)); return { q: T(`${act(a[1], nm).en}. Which stage of a statistical inquiry is this?<br>${o.en}`, `${act(a[1], nm).ms}. Peringkat manakah dalam inkuiri statistik ini?<br>${o.ms}`), a: o.ans, sp: 's' }; },
    (r) => {
      const miss = r.int(1, 4), order = ST.filter((_, i) => i !== miss);
      return { q: T(`A student lists the stages of an inquiry in order: ${order.map((s) => s.en).join(' → ')}. One stage is missing. Which one?`, `Seorang murid menyenaraikan peringkat inkuiri mengikut turutan: ${order.map((s) => s.ms).join(' → ')}. Satu peringkat tertinggal. Yang manakah?`), a: ST[miss], sp: 's' };
    },
    (r) => { const it = r.pick(METH); return { q: T(`Which method of collecting data is most suitable to ${it.s.en}: a survey, observation, measurement or counting?`, `Kaedah pengumpulan data manakah yang paling sesuai untuk ${it.s.ms}: tinjauan, pemerhatian, pengukuran atau pengiraan?`), a: cat(METHN[it.m], '; ', METHX[it.m]), sp: 's' }; },
    (r) => { const it = r.pick(SQY); return { q: T(`State the variable in the statistical question "${it.q.en}" and whether it is categorical or numerical.`, `Nyatakan pemboleh ubah dalam soalan statistik "${it.q.ms}" dan sama ada ia kategori atau berangka.`), a: cat(it.v, '; ', TYPEL[it.t]), sp: 's' }; },
  ];
  const ASP = [
  ...
  const g121m = [
    (r) => {
      ...
      return { q: T(`The survey question "${it.q.en}" is not good. What is wrong with it?<br>${o.en}`, `Soalan tinjauan "${it.q.ms}" tidak baik. Apakah kesalahannya?<br>${o.ms}`), a: T(`(${o.letter}). ${BADK[it.k].en}`, `(${o.letter}). ${BADK[it.k].ms}`), sp: 's' };
    },
    (r) => { const it = r.pick(BAD); return { q: T(`Rewrite the survey question "${it.q.en}" so that it is a clear, neutral statistical question.`, `Tulis semula soalan tinjauan "${it.q.ms}" supaya menjadi soalan statistik yang jelas dan neutral.`), a: cat(T('Sample answer: ', 'Contoh jawapan: '), it.fix), sp: 'm' }; },
    (r) => { const it = r.pick(BAD); return { q: T(`A student plans to ask: "${it.q.en}" (a) State what is wrong with this question. (b) Write an improved question.`, `Seorang murid merancang untuk bertanya: "${it.q.ms}" (a) Nyatakan kesalahan soalan ini. (b) Tulis soalan yang lebih baik.`), a: SPM.parts([BADK[it.k], it.fix]) , sp: 'm' }; },
    (r) => {
      const it = r.pick(INQ), asp = r.sample(ASP, r.int(2, 3));
      return { q: T(`A student wants to answer the statistical question "${it.q.en}" For this inquiry, state ${asp.map((a) => a.s.en).join('; ')}.`, `Seorang murid ingin menjawab soalan statistik "${it.q.ms}" Bagi inkuiri ini, nyatakan ${asp.map((a) => a.s.ms).join('; ')}.`), a: SPM.parts(asp.map((a) => a.f(it))), sp: 'm' };
    },
    (r) => { const it = r.pick(FLAWQ); return { q: T(`Read the description and identify what is wrong in the inquiry, then suggest how to correct it.<br>${it.s.en}`, `Baca penerangan dan kenal pasti kesalahan dalam inkuiri itu, kemudian cadangkan cara membetulkannya.<br>${it.s.ms}`), a: it.f, sp: 'm' }; },
    (r) => { const c = r.pick(GEN_CTX); return { q: T(`Write a statistical question about ${c.c.en} for the students of your school. State the variable in your question.`, `Tulis satu soalan statistik tentang ${c.c.ms} untuk murid di sekolah anda. Nyatakan pemboleh ubah dalam soalan anda.`), a: cat(T('Sample answer: "', 'Contoh jawapan: "'), c.q, T('" Variable: ', '" Pemboleh ubah: '), c.v), sp: 'm' }; },
    (r) => {
      const v = r.sample(VARS, 3); need(new Set(v.map((x) => x.t)).size >= 2);
      return { q: T(`Classify each variable as categorical, numerical discrete or numerical continuous: (a) ${v[0].v.en}; (b) ${v[1].v.en}; (c) ${v[2].v.en}.`, `Kelaskan setiap pemboleh ubah sebagai kategori, berangka diskret atau berangka selanjar: (a) ${v[0].v.ms}; (b) ${v[1].v.ms}; (c) ${v[2].v.ms}.`), a: SPM.parts(v.map((x) => TYPEL[x.t])), sp: 'm' };
    },
    (r) => {
      ...
      return { q: T(`Which of these are statistical questions? ${all.map((x, i) => `(${LET[i]}) ${x.q.en}`).join(' ')}`, `Yang manakah antara berikut ialah soalan statistik? ${all.map((x, i) => `(${LET[i]}) ${x.q.ms}`).join(' ')}`), a: T(ids.join(', ')), sp: 's' };
    },
    (r) => {
      const [a, b] = r.sample(METH, 2); need(a.m !== b.m);
      return { q: T(`State the most suitable way of collecting data, and give a reason, to (a) ${a.s.en}; (b) ${b.s.en}.`, `Nyatakan cara pengumpulan data yang paling sesuai, dan berikan sebab, untuk (a) ${a.s.ms}; (b) ${b.s.ms}.`), a: SPM.parts([cat(METHN[a.m], ': ', METHX[a.m]), cat(METHN[b.m], ': ', METHX[b.m])]), sp: 'm' };
    },
  ];
  const POPS = [
  ...
  const multi = (r, tasks, lo, hi) => {
    ...
    return t.length === 1 ? { q: t[0][0], a: t[0][1] } : { q: SPM.parts(t.map((x) => x[0])), a: SPM.parts(t.map((x) => x[1])) };
  };
  ...
  const readTasks = (r, d) => {
    ...
    const dl = (f, l) => T(f, l);
    return [
      [B('Which category is the most popular? || Kategori manakah yang paling popular?'), catNm(d, mx)],
      [B('Which category is the least popular? || Kategori manakah yang paling kurang popular?'), catNm(d, mn)],
      [T(`How many students chose ${catNm(d, i).en}?`, `Berapakah murid yang memilih ${catNm(d, i).ms}?`), T(`${d.f[i]}`)],
      [B('How many students were surveyed altogether? || Berapakah jumlah murid yang ditinjau?'), T(`${tot}`)],
      [T(`How many more students chose ${catNm(d, mx).en} than ${catNm(d, mn).en}?`, `Berapa ramaikah murid yang memilih ${catNm(d, mx).ms} berbanding ${catNm(d, mn).ms}?`), T(`${d.f[mx] - d.f[mn]}`)],
      [T(`How many students chose ${catNm(d, i).en} or ${catNm(d, j).en}?`, `Berapakah murid yang memilih ${catNm(d, i).ms} atau ${catNm(d, j).ms}?`), T(`${d.f[i] + d.f[j]}`)],
      [T(`How many students did NOT choose ${catNm(d, i).en}?`, `Berapakah murid yang TIDAK memilih ${catNm(d, i).ms}?`), T(`${tot - d.f[i]}`)],
    ];
  };
  const barFig = (d, r, o) => {
  ...
  const g122e = [
    (r) => { const d = catData(r, r.int(3, 4), r.pick([20, 24, 30]), 2), o = rawOrder(r, d); return { q: T(`${d.name.en} of ${d.total} students: ${rawList(d, o, 'en')}. Complete a frequency table for the data.`, `${d.name.ms} bagi ${d.total} orang murid: ${rawList(d, o, 'ms')}. Lengkapkan jadual kekerapan bagi data itu.`), a: T(`${listL(d, d.f, 'en')}; total ${d.total}`, `${listL(d, d.f, 'ms')}; jumlah ${d.total}`), sp: 'm' }; },
    (r) => {
      ...
      return { q: T(`The tally chart shows the choices of ${d.total} students.<br>${rows('en')}<br>Complete the frequency column and find the total.`, `Carta tally menunjukkan pilihan ${d.total} orang murid.<br>${rows('ms')}<br>Lengkapkan lajur kekerapan dan cari jumlahnya.`), a: T(`${d.f.join(', ')}; total ${d.total}`, `${d.f.join(', ')}; jumlah ${d.total}`), sp: 's' };
    },
    (r) => {
      const d = numData(r, r.pick([16, 20, 24, 30]), 4);
      return { q: T(`${d.name.en} recorded for ${d.N} ${d.who.en}: ${d.raw.join(', ')}. Complete the frequency table.<br>${vtab(d.name, d.vals, d.f, 'en', d.f.map((_, i) => i))}`, `${d.name.ms} yang direkodkan bagi ${d.N} ${d.who.ms}: ${d.raw.join(', ')}. Lengkapkan jadual kekerapan.<br>${vtab(d.name, d.vals, d.f, 'ms', d.f.map((_, i) => i))}`), a: T(`${d.vals.map((v, i) => `${v}: ${d.f[i]}`).join('; ')}; total ${d.N}`, `${d.vals.map((v, i) => `${v}: ${d.f[i]}`).join('; ')}; jumlah ${d.N}`), sp: 's' };
    },
    (r) => { const d = catData(r, r.int(4, 6), r.pick([20, 30, 40, 60]), 2), m = multi(r, readTasks(r, d), 1, 3); return { q: cat(T('The bar chart shows the results of a survey. ', 'Carta palang menunjukkan keputusan satu tinjauan. '), m.q), fig: barFig(d, r, { title: r.chance() }), a: m.a, sp: 's' }; },
    (r) => {
      const d = catData(r, r.int(4, 5), r.pick([24, 30, 36, 40]), 3), m = multi(r, readTasks(r, d).concat([[B('What fraction of the students chose the most popular category? Give the answer in its simplest form. || Apakah pecahan murid yang memilih kategori paling popular? Beri jawapan dalam bentuk termudah.'), T(`$${SPM.Fr.tex(SPM.Fr.make(Math.max(...d.f), d.total))}$`)]]), 1, 3);
      const fig = dual((l) => pieSvg({ vals: d.f, labels: d.cats.map((c, i) => `${c[l]} (${d.f[i]})`), title: d.name[l] }));
      return { q: cat(T('The pie chart shows the number of students who chose each category in a survey. ', 'Carta pai menunjukkan bilangan murid yang memilih setiap kategori dalam satu tinjauan. '), m.q), fig, a: m.a, sp: 's' };
    },
    (r) => {
      ...
      const m = multi(r, [[B('How many students are there altogether? || Berapakah jumlah murid?'), T(`${d.N}`)], [B('Which value occurs most often? || Nilai yang manakah paling kerap berlaku?'), T(`${d.vals[mx]}`)], [T(`How many students have exactly ${t}?`, `Berapakah murid yang mempunyai tepat ${t}?`), T(`${d.f[d.vals.indexOf(t)]}`)], [T(`How many students have more than ${t}?`, `Berapakah murid yang mempunyai lebih daripada ${t}?`), T(`${sum(d.f.filter((_, i) => d.vals[i] > t))}`)], [T(`How many students have at most ${t}?`, `Berapakah murid yang mempunyai selebih-lebihnya ${t}?`), T(`${sum(d.f.filter((_, i) => d.vals[i] <= t))}`)]], 2, 3);
      return { q: cat(T(`The dot plot shows the "${d.name.en}" of ${d.N} ${d.who.en}. `, `Plot titik menunjukkan "${d.name.ms}" bagi ${d.N} ${d.who.ms}. `), m.q), fig: dual((l) => dotSvg(d.raw, lo, hi, { xl: d.name[l] })), a: m.a, sp: 's' };
    },
    (r) => {
      const d = stemData(r, r.int(11, 16)), fm = sfmt(d.c.dv), s = d.sorted, t = s[r.int(4, s.length - 4)], k = r.pick(s);
      const m = multi(r, [[B('State the highest value. || Nyatakan nilai tertinggi.'), T(fm(s[s.length - 1]) + '')], [B('State the lowest value. || Nyatakan nilai terendah.'), T(fm(s[0]) + '')], [B('How many values are in the plot? || Berapakah bilangan nilai dalam plot itu?'), T(`${s.length}`)], [T(`How many values are greater than ${fm(t)}?`, `Berapakah nilai yang lebih besar daripada ${fm(t)}?`), T(`${s.filter((v) => v > t).length}`)], [T(`What value is shown by the stem ${Math.floor(k / 10)} and the leaf ${k % 10}?`, `Apakah nilai yang ditunjukkan oleh batang ${Math.floor(k / 10)} dan daun ${k % 10}?`), T(fm(k) + '')], [B('Find the difference between the highest and the lowest values. || Cari beza antara nilai tertinggi dengan nilai terendah.'), T(n(s[s.length - 1] / d.c.dv - s[0] / d.c.dv))]], 2, 3);
      const key = s[Math.floor(s.length / 2)];
      return { q: T(`The stem-and-leaf plot shows the ${d.c.w.en} of some people.<br>${stemHtml(s, 'en', fm, d.c.u.split('|')[0], { key })}<br>${m.q.en}`, `Plot batang dan daun menunjukkan ${d.c.w.ms} bagi beberapa orang.<br>${stemHtml(s, 'ms', fm, d.c.u.split('|')[1] || d.c.u, { key })}<br>${m.q.ms}`), a: m.a, sp: 's' };
    },
    (r) => {
      const d = catData(r, r.int(4, 5), r.pick([20, 30, 40, 60]), 3), i = r.int(0, d.k - 1), tot = sum(d.f);
      return { q: T(`${intro(d, tot).en}<br>${ftab(d, 'en', { blank: [i], total: tot })}<br>Find the missing frequency${r.chance() ? ' and state which category is the most popular' : ''}.`, `${intro(d, tot).ms}<br>${ftab(d, 'ms', { blank: [i], total: tot })}<br>Cari kekerapan yang tertinggal${'' }.`), a: T(`$${tot} - (${d.f.filter((_, j) => j !== i).join(' + ')}) = ${d.f[i]}$; ${catNm(d, imax(d.f)).en} is the most popular`, `$${tot} - (${d.f.filter((_, j) => j !== i).join(' + ')}) = ${d.f[i]}$; ${catNm(d, imax(d.f)).ms} paling popular`), sp: 's' };
    },
    (r) => {
      ...
      const m = multi(r, [[T(`How many students chose ${cats[i].en}?`, `Berapakah murid yang memilih ${cats[i].ms}?`), T(`${cnt[i]}`)], [B('How many students are shown altogether? || Berapakah jumlah murid yang ditunjukkan?'), T(`${sum(cnt)}`)], [B('Which category is the most popular, and by how many students does it lead the least popular? || Kategori manakah yang paling popular, dan berapa muridkah ia mendahului yang paling kurang popular?'), T(`${cats[mx].en}; ${Math.max(...cnt) - Math.min(...cnt)}`, `${cats[mx].ms}; ${Math.max(...cnt) - Math.min(...cnt)}`)]], 1, 3);
      return { q: cat(T(`(Prerequisite skill: reading a pictograph.) The pictograph shows "${c.name.en}". `, `(Kemahiran prasyarat: membaca piktograf.) Piktograf menunjukkan "${c.name.ms}". `), m.q), fig: dual((l) => picSvg(cats.map((x) => x[l]), cnt, per, Ln(l, 'students', 'murid'), l)), a: m.a, sp: 's' };
    },
    (r) => {
      ...
      return r.chance() ? { q: T(`In a pie chart representing ${d.total} students, the sector for ${catNm(d, i).en} has an angle of $${a}^\\circ$. How many students does this sector represent?`, `Dalam sebuah carta pai yang mewakili ${d.total} orang murid, sektor bagi ${catNm(d, i).ms} mempunyai sudut $${a}^\\circ$. Berapakah murid yang diwakili oleh sektor ini?`), a: T(`$\\dfrac{${a}}{360} \\times ${d.total} = ${d.f[i]}$`), sp: 's' }
        : { q: T(`${d.f[i]} out of ${d.total} students chose ${catNm(d, i).en}. Find the angle of its sector in a pie chart.`, `${d.f[i]} daripada ${d.total} orang murid memilih ${catNm(d, i).ms}. Cari sudut sektornya dalam carta pai.`), a: T(`$\\dfrac{${d.f[i]}}{${d.total}} \\times 360^\\circ = ${a}^\\circ$`), sp: 's' };
    },
  ];
  const g122m = [
    (r) => {
  ...
  const g123e = [
    (r) => { const p = r.pick(PURP); return { q: T(`Which representation is most suitable to ${p.p.en}: a bar chart, a pie chart, a line graph, a histogram or a dot plot?`, `Perwakilan manakah yang paling sesuai untuk ${p.p.ms}: carta palang, carta pai, graf garis, histogram atau plot titik?`), a: cat(REPN[p.k], ': ', REPX[p.k]), sp: 's' }; },
    (r) => { const p = r.pick(PURP), o = mc(r, REPN[p.k], r.sample(REPN.filter((_, i) => i !== p.k), 3)); return { q: T(`To ${p.p.en}, which representation should be used?<br>${o.en}`, `Untuk ${p.p.ms}, perwakilan manakah yang patut digunakan?<br>${o.ms}`), a: cat(o.ans, '. ', REPX[p.k]), sp: 's' }; },
    (r) => { const h = r.pick(HS); return { q: T(`True or false? "${h.s.en}"`, `Benar atau palsu? "${h.s.ms}"`), a: cat(h.t ? T('True. ', 'Benar. ') : T('False. ', 'Palsu. '), h.x), sp: 's' }; },
    (r) => {
      const [a, b] = r.sample(HS, 2), ok = (h) => (h.t ? 'True' : 'False');
      need(a.t !== b.t);
      const o = mc(r, a.t ? a.s : b.s, [a.t ? b.s : a.s]);
      return { q: T(`Which of these statements is correct?<br>${o.en}`, `Pernyataan manakah yang betul?<br>${o.ms}`), a: cat(o.ans, '. ', a.t ? a.x : b.x), sp: 's' };
    },
    (r) => {
      ...
      return { q: T(`The pie chart shows the results of a survey of ${d.total} students. How many students chose ${catNm(d, i).en}?`, `Carta pai menunjukkan keputusan tinjauan terhadap ${d.total} orang murid. Berapakah murid yang memilih ${catNm(d, i).ms}?`), fig, a: T(`$${n(p)}\\% \\times ${d.total} = ${d.f[i]}$`), sp: 's' };
    },
    (r) => {
      const d = numData(r, r.pick([20, 24, 30]), 4), v = r.pick(d.vals), i = d.vals.indexOf(v);
      need(d.f.filter((x) => x === Math.max(...d.f)).length === 1);
      const m = multi(r, [[T(`State the frequency for ${v}.`, `Nyatakan kekerapan bagi ${v}.`), T(`${d.f[i]}`)], [B('For which value is the frequency the highest? || Bagi nilai manakah kekerapan paling tinggi?'), T(`${d.vals[d.f.indexOf(Math.max(...d.f))]}`)], [B('Find the total frequency. || Cari jumlah kekerapan.'), T(`${d.N}`)], [B('How many points are plotted on the graph, including the two on the horizontal axis? || Berapakah bilangan titik yang diplot pada graf itu, termasuk dua titik pada paksi mengufuk?'), T(`${d.vals.length + 2}`)]], 2, 3);
      return { q: cat(T(`The frequency polygon shows the "${d.name.en}" of ${d.N} ${d.who.en}. `, `Poligon kekerapan menunjukkan "${d.name.ms}" bagi ${d.N} ${d.who.ms}. `), m.q), fig: dual((l) => numFig(d, l, { poly: true })), a: m.a, sp: 's' };
    },
    (r) => {
      const d = numData(r, r.pick([20, 24, 30]), 4), v = r.pick(d.vals), i = d.vals.indexOf(v);
      return r.chance() ? { q: T(`${vtab(d.name, d.vals, d.f, 'en')}<br>To draw a frequency polygon for this table, what are the coordinates of the point for ${v}?`, `${vtab(d.name, d.vals, d.f, 'ms')}<br>Untuk melukis poligon kekerapan bagi jadual ini, apakah koordinat titik bagi ${v}?`), a: T(`$(${v}, ${d.f[i]})$`), sp: 's' }
        : { q: T(`${vtab(d.name, d.vals, d.f, 'en')}<br>A frequency polygon is drawn from this table, joined to the horizontal axis at both ends. State the coordinates of the two end points.`, `${vtab(d.name, d.vals, d.f, 'ms')}<br>Sebuah poligon kekerapan dilukis daripada jadual ini dan disambung ke paksi mengufuk pada kedua-dua hujung. Nyatakan koordinat dua titik hujung.`), a: T(`$(${d.vals[0] - 1}, 0)$ and $(${d.vals[d.vals.length - 1] + 1}, 0)$`), sp: 's' };
    },
    (r) => {
      const ps = r.sample(PURP, 3); need(new Set(ps.map((x) => x.k)).size === 3);
      const opts = r.shuffle([0, 1, 2]);
      return { q: T(`Match each purpose with the best representation from: bar chart, pie chart, line graph, histogram, dot plot. (a) ${ps[0].p.en}. (b) ${ps[1].p.en}. (c) ${ps[2].p.en}.`, `Padankan setiap tujuan dengan perwakilan terbaik daripada: carta palang, carta pai, graf garis, histogram, plot titik. (a) ${ps[0].p.ms}. (b) ${ps[1].p.ms}. (c) ${ps[2].p.ms}.`), a: SPM.parts(ps.map((x) => REPN[x.k])), sp: 'm' };
    },
    (r) => {
  ...
  const g123m = [
    ...
    (r) => {
      ...
      const m = multi(r, [[B('How many students are in each class? || Berapakah bilangan murid dalam setiap kelas?'), T(`A: ${d1.N}; B: ${N2}`)], [T(`Which class has more students with ${v}? By how many?`, `Kelas manakah yang mempunyai lebih ramai murid dengan ${v}? Berapa ramai?`), d1.f[i] === f2[i] ? T('Equal', 'Sama') : T(`Class ${d1.f[i] > f2[i] ? 'A' : 'B'}: ${Math.abs(d1.f[i] - f2[i])}`, `Kelas ${d1.f[i] > f2[i] ? 'A' : 'B'}: ${Math.abs(d1.f[i] - f2[i])}`)], [B('For which value do the two classes differ most? || Bagi nilai manakah kedua-dua kelas paling berbeza?'), T(`${d1.vals[d1.f.map((x, q) => Math.abs(x - f2[q])).reduce((b, x, q, a) => (x > a[b] ? q : b), 0)]}`)]], 2, 3);
      return { q: cat(T(`The graph compares "${d1.name.en}" in two classes. `, `Graf membandingkan "${d1.name.ms}" dalam dua kelas. `), m.q), fig, a: m.a, sp: 'm' };
    },
    (r) => {
  ...
  const g124e = [
    (r) => {
      ...
      const m = multi(r, [[B('State the highest value and when it occurred. || Nyatakan nilai tertinggi dan bila ia berlaku.'), T(`${v[mx]}, ${xa.at('en', mx)}`, `${v[mx]}, ${xa.at('ms', mx)}`)], [B('State the lowest value and when it occurred. || Nyatakan nilai terendah dan bila ia berlaku.'), T(`${v[mn]}, ${xa.at('en', mn)}`, `${v[mn]}, ${xa.at('ms', mn)}`)], [B('Find the total of all the values shown. || Cari jumlah semua nilai yang ditunjukkan.'), T(`${sum(v)}`)], [T(`How many times was the value greater than ${t}?`, `Berapa kalikah nilai lebih besar daripada ${t}?`), T(`${v.filter((x) => x > t).length}`)], [T(`Find the change in the value from ${xa.f('en', i)} to ${xa.f('en', len - 1)}.`, `Cari perubahan nilai dari ${xa.f('ms', i)} hingga ${xa.f('ms', len - 1)}.`), i === len - 1 ? null : T(`${v[len - 1] - v[i] >= 0 ? '+' : '-'}${Math.abs(v[len - 1] - v[i])}`)]].filter((x) => x[1]), 2, 3);
      return { q: cat(T(`The line graph shows the ${ts.w.en}. `, `Graf garis menunjukkan ${ts.w.ms}. `), m.q), fig: lineFig(v, ts, xa), a: m.a, sp: 's' };
    },
    (r) => {
      ...
      const m = multi(r, [[T(`In which category was the number highest for ${sn[1].en}?`, `Dalam kategori manakah bilangan paling tinggi bagi ${sn[1].ms}?`), catNm(d, mxB)], [T(`Find the total number of students for ${sn[0].en}.`, `Cari jumlah murid bagi ${sn[0].ms}.`), T(`${d.total}`)], [T(`For ${catNm(d, i).en}, which group is larger, and by how many?`, `Bagi ${catNm(d, i).ms}, kumpulan manakah yang lebih besar, dan lebih berapa?`), T(`${d.f[i] > f2[i] ? sn[0].en : sn[1].en}, ${Math.abs(d.f[i] - f2[i])}`, `${d.f[i] > f2[i] ? sn[0].ms : sn[1].ms}, ${Math.abs(d.f[i] - f2[i])}`)], [B('How many students are shown altogether? || Berapakah jumlah semua murid yang ditunjukkan?'), T(`${d.total + sum(f2)}`)]], 2, 3);
      return { q: cat(T(`The double bar chart compares two groups: ${sn[0].en} and ${sn[1].en}. `, `Carta palang berganda membandingkan dua kumpulan: ${sn[0].ms} dan ${sn[1].ms}. `), m.q), fig, a: m.a, sp: 's' };
    },
    (r) => {
      const d = catData(r, 4, r.pick([20, 25, 40, 50]), 2), i = r.int(0, 3), mx = imax(d.f), tw = d.f.map((x) => (x * 100) / d.total);
      const m = multi(r, [[T(`What percentage of the students chose ${catNm(d, i).en}?`, `Berapakah peratusan murid yang memilih ${catNm(d, i).ms}?`), T(`${n(tw[i])}%`)], [B('Did more than half of the students choose the most popular category? || Adakah lebih separuh murid memilih kategori paling popular?'), YN(d.f[mx] * 2 > d.total)], [B('What percentage of the students did not choose the most popular category? || Berapakah peratusan murid yang tidak memilih kategori paling popular?'), T(`${n(100 - tw[mx])}%`)]], 1, 2);
      return { q: cat(intro(d, d.total), T(' ', ' '), m.q), fig: barFig(d, r), a: m.a, sp: 's' };
    },
    (r) => {
      ...
      return { q: T(`The graph shows the ${ts.w.en}. Which statement best describes the trend?<br>${o.en}`, `Graf menunjukkan ${ts.w.ms}. Pernyataan manakah yang paling baik menerangkan aliran itu?<br>${o.ms}`), fig: lineFig(v, ts, xa), a: o.ans, sp: 's' };
    },
    (r) => {
      ...
      return { q: T(`The graph shows the ${ts.w.en}. Is the statement "${c.q.en}" supported by the graph? Give a reason.`, `Graf menunjukkan ${ts.w.ms}. Adakah pernyataan "${c.q.ms}" disokong oleh graf? Berikan sebab.`), fig: lineFig(v, ts, xa), a: c.a, sp: 's' };
    },
    (r) => {
      const d = numData(r, r.pick([20, 25, 40, 50]), 4), mx = d.f.indexOf(Math.max(...d.f)), t = r.pick(d.vals.slice(1, -1));
      need(d.f.filter((x) => x === Math.max(...d.f)).length === 1);
      const ge = sum(d.f.filter((_, i) => d.vals[i] >= t)), m = multi(r, [[B('Which value is the most common? || Nilai yang manakah paling biasa?'), T(`${d.vals[mx]}`)], [T(`How many have ${t} or more?`, `Berapakah yang mempunyai ${t} atau lebih?`), T(`${ge}`)], [T(`What percentage have fewer than ${t}?`, `Berapakah peratusan yang mempunyai kurang daripada ${t}?`), T(`${n((d.N - ge) * 100 / d.N)}%`)]], 2, 3);
      return { q: cat(T(`The histogram shows the "${d.name.en}" of ${d.N} ${d.who.en}. `, `Histogram menunjukkan "${d.name.ms}" bagi ${d.N} ${d.who.ms}. `), m.q), fig: dual((l) => numFig(d, l)), a: m.a, sp: 's' };
    },
  ];
  ...
  const g124m = [
    ...
    (r) => {
      ...
      const m = multi(r, [[T(`Which was higher ${xa.at('en', i)}, and by how much?`, `Yang manakah lebih tinggi ${xa.at('ms', i)}, dan lebih berapa?`), va[i] === vb[i] ? B('Equal || Sama') : T(`${(va[i] > vb[i] ? pr[0] : pr[1]).en}, ${Math.abs(va[i] - vb[i])}`, `${(va[i] > vb[i] ? pr[0] : pr[1]).ms}, ${Math.abs(va[i] - vb[i])}`)], [T(`In how many of the ${len} periods was ${pr[0].en} higher than ${pr[1].en}?`, `Dalam berapa daripada ${len} tempoh ${pr[0].ms} lebih tinggi daripada ${pr[1].ms}?`), T(`${na}`)], [B('When was the difference between the two greatest, and what was it? || Bilakah beza antara keduanya paling besar, dan berapakah bezanya?'), T(`${xa.f('en', bi)}, ${dm[bi]}`, `${xa.f('ms', bi)}, ${dm[bi]}`)], [B('Which has the larger total for all the periods shown? || Yang manakah mempunyai jumlah yang lebih besar bagi semua tempoh yang ditunjukkan?'), sum(va) === sum(vb) ? B('Equal || Sama') : T(`${(sum(va) > sum(vb) ? pr[0] : pr[1]).en} (${Math.max(sum(va), sum(vb))} against ${Math.min(sum(va), sum(vb))})`, `${(sum(va) > sum(vb) ? pr[0] : pr[1]).ms} (${Math.max(sum(va), sum(vb))} berbanding ${Math.min(sum(va), sum(vb))})`)]], 2, 3);
      return { q: cat(T(`The line graph compares the ${ts.w.en} for ${pr[0].en} (solid line) and ${pr[1].en} (dashed line). `, `Graf garis membandingkan ${ts.w.ms} bagi ${pr[0].ms} (garis penuh) dan ${pr[1].ms} (garis putus-putus). `), m.q), fig, a: m.a, sp: 'm' };
    },
    (r) => {
      ...
      const m = multi(r, [[T(`How many times as many students chose ${cats[0].en} as ${cats[1].en}?`, `Berapa kalikah ganda murid yang memilih ${cats[0].ms} berbanding ${cats[1].ms}?`), T(`${k}`)], [T(`What fraction of the students chose ${cats[0].en}? Give your answer in its simplest form.`, `Apakah pecahan murid yang memilih ${cats[0].ms}? Beri jawapan dalam bentuk termudah.`), T(`$${SPM.Fr.tex(SPM.Fr.make(fa, d.total))}$`)], [T(`How many more students chose ${cats[0].en} than ${cats[1].en}?`, `Berapa ramaikah murid yang memilih ${cats[0].ms} berbanding ${cats[1].ms}?`), T(`${fa - fb}`)], [B('How many students were surveyed altogether? || Berapakah jumlah murid yang ditinjau?'), T(`${d.total}`)]], 2, 3);
      return { q: cat(T('The bar chart shows the results of a survey of students. ', 'Carta palang menunjukkan keputusan satu tinjauan terhadap murid. '), m.q), fig: barFig(d, r), a: m.a, sp: 'm' };
    },
    (r) => {
      ...
      return { q: T(`The graph shows the ${ts.w.en}. For each statement, say whether it is ${CLSN.en}. (a) ${pick[0].q.en} (b) ${pick[1].q.en} (c) ${pick[2].q.en}`, `Graf menunjukkan ${ts.w.ms}. Bagi setiap pernyataan, nyatakan sama ada ia ${CLSN.ms}. (a) ${pick[0].q.ms} (b) ${pick[1].q.ms} (c) ${pick[2].q.ms}`), fig: lineFig(v, ts, xa), a: SPM.parts(pick.map((x) => x.a)), sp: 'l' };
    },
    (r) => {
      ...
      return { q: T(`The graph shows the ${ts.w.en}. Which statement can NOT be concluded from the graph?<br>${o.en}`, `Graf menunjukkan ${ts.w.ms}. Pernyataan manakah yang TIDAK boleh disimpulkan daripada graf?<br>${o.ms}`), fig: lineFig(v, ts, xa), a: T(`(${o.letter}). ${bad.a.en}`, `(${o.letter}). ${bad.a.ms}`), sp: 's' };
    },
    (r) => {
  ...
  const g125m = [
    ...
    (r) => {
      ...
      return { q: T(`Sales of RM${a} thousand and RM${a * k} thousand are shown by two ${nm.en} whose ${side.en} are in the ratio 1 : ${k}. (a) Find the ratio of their areas. (b) By how many times does the display exaggerate the second value? (c) Suggest a fairer display.`, `Jualan RM${a} ribu dan RM${a * k} ribu ditunjukkan oleh dua ${nm.ms} yang ${side.ms}nya dalam nisbah 1 : ${k}. (a) Cari nisbah luas kedua-duanya. (b) Berapa kalikah paparan itu membesar-besarkan nilai kedua? (c) Cadangkan paparan yang lebih adil.`), fig: T(sqFig(a, k, shape, 'en'), sqFig(a, k, shape, 'ms')), a: SPM.parts([T(`$1 : ${k * k}$`), T(`It looks ${k * k} times as large, but the value is only ${k} times as large, so the display exaggerates it ${k} times.`, `Ia kelihatan ${k * k} kali ganda, tetapi nilainya hanya ${k} kali ganda, jadi paparan itu membesar-besarkannya ${k} kali ganda.`), B('Use bars of equal width whose heights are proportional to the values. || Gunakan palang berlebar sama dengan tinggi berkadaran dengan nilai.')]), sp: 'm' };
    },
    (r) => {
