...
(function () {
  ...
  const g123e = [
    ...
    (r) => { const h = r.pick(HS); return { q: T(`True or false? "${h.s.en}"`, `Benar atau palsu? "${h.s.ms}"`), a: cat(h.t ? T('True. ', 'Benar. ') : T('False. ', 'Palsu. '), h.x), w: W(B('Test the statement against what each kind of display is for and how it is drawn. || Uji pernyataan itu berdasarkan tujuan setiap jenis paparan dan cara ia dilukis.'), cat(h.t ? T('True: ', 'Benar: ') : T('False: ', 'Palsu: '), h.x)), sp: 's' }; },
    (r) => {
  ...
  const g125e = [
    ...
    (r) => { const h = r.pick(ET); return { q: T(`True or false? "${h.s.en}"`, `Benar atau palsu? "${h.s.ms}"`), a: cat(h.t ? T('True. ', 'Benar. ') : T('False. ', 'Palsu. '), h.x), w: W(B('Ask whether the practice leaves the reader correctly informed about all the data. || Tanya sama ada amalan itu memberikan pembaca maklumat yang betul tentang keseluruhan data.'), cat(h.t ? T('True: ', 'Benar: ') : T('False: ', 'Palsu: '), h.x)), sp: 's' }; },
