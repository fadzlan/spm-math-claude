...
(function () {
  ...
  const g31e = [
    (r) => {
      ...
      return { q: T(`Which of these is a statement? (A) ${list[0][0].en} (B) ${list[1][0].en} State its truth value.`, `Antara yang berikut, yang manakah pernyataan? (A) ${list[0][0].ms} (B) ${list[1][0].ms} Nyatakan nilai kebenarannya.`), a: T(`(${list[0][1] ? 'A' : 'B'}); ${st.v ? 'true' : 'false'}`, `(${list[0][1] ? 'A' : 'B'}); ${st.v ? 'betul' : 'salah'}`), w: W(T(`A statement is a sentence that is either true or false – not a question, command or exclamation: (${list[0][1] ? 'A' : 'B'})`, `Pernyataan ialah ayat yang sama ada betul atau salah – bukan soalan, arahan atau seruan: (${list[0][1] ? 'A' : 'B'})`), T(`${st.why.en} So it is ${st.v ? 'true' : 'false'}.`, `${st.why.ms} Maka ia ${st.v ? 'betul' : 'salah'}.`)), sp: 's' };
    },
    (r) => {
      const st = r.pick(STM);
      return { q: T(`Write the negation of the statement: "${st.s.en}" Is the negation true or false?`, `Tulis penafian bagi pernyataan: "${st.s.ms}" Adakah penafian itu betul atau salah?`), a: T(`${st.neg.en} ${st.v ? 'False' : 'True'}`, `${st.neg.ms} ${st.v ? 'Salah' : 'Betul'}`), w: W(T('Negation: insert "not" – it reverses the truth value.', 'Penafian: masukkan "bukan"/"tidak" – nilai kebenaran bertukar.'), T(`${st.why.en} The statement is ${st.v ? 'true' : 'false'}, so its negation is ${st.v ? 'false' : 'true'}.`, `${st.why.ms} Pernyataan itu ${st.v ? 'betul' : 'salah'}, maka penafiannya ${st.v ? 'salah' : 'betul'}.`)), sp: 's' };
    },
  ];
  const g31m = [
    (r) => {
      ...
      return { q: T(`Set $S = \\{${set.join(', ')}\\}$. Write the negation of "All elements of $S$ are ${k === 3 ? 'odd' : 'greater than 3'}" and determine which of the two statements is true.`, `Set $S = \\{${set.join(', ')}\\}$. Tulis penafian bagi "Semua unsur $S$ ialah ${k === 3 ? 'ganjil' : 'lebih besar daripada 3'}" dan tentukan yang manakah antara dua pernyataan itu betul.`), a: k === 3 ? T('"At least one element of $S$ is not odd (is even)." The negation is true (e.g. 6); the original is false.', '"Sekurang-kurangnya satu unsur $S$ bukan ganjil (genap)." Penafian itu betul (cth. 6); pernyataan asal salah.') : T('"At least one element of $S$ is not greater than 3." The negation is true (2 is not greater than 3); the original is false.', '"Sekurang-kurangnya satu unsur $S$ tidak lebih besar daripada 3." Penafian itu betul (2 tidak lebih besar daripada 3); pernyataan asal salah.'), w: W(T('The negation of "All … are …" is "At least one … is not …" (not "All … are not …").', 'Penafian bagi "Semua … ialah …" ialah "Sekurang-kurangnya satu … bukan …" (bukan "Semua … bukan …").'), k === 3 ? T('$6 \\in S$ is even, so "all are odd" is false and its negation is true.', '$6 \\in S$ ialah genap, maka "semua ganjil" salah dan penafiannya betul.') : T('$2 \\in S$ and $2 < 3$, so "all are greater than 3" is false and its negation is true.', '$2 \\in S$ dan $2 < 3$, maka "semua lebih besar daripada 3" salah dan penafiannya betul.')), sp: 's' };
    },
    (r) => {
      const p = r.pick(STM), q = r.pick(STM.filter((x) => x !== p));
      const and = r.chance();
      const v = and ? p.v && q.v : p.v || q.v;
      return { q: T(`Statement $p$: "${p.s.en}" Statement $q$: "${q.s.en}" State whether "$p$ ${and ? 'and' : 'or'} $q$" is true or false.`, `Pernyataan $p$: "${p.s.ms}" Pernyataan $q$: "${q.s.ms}" Nyatakan sama ada "$p$ ${and ? 'dan' : 'atau'} $q$" betul atau salah.`), a: T(`${v ? 'True' : 'False'} ($p$ is ${p.v ? 'true' : 'false'}, $q$ is ${q.v ? 'true' : 'false'})`, `${v ? 'Betul' : 'Salah'} ($p$ ${p.v ? 'betul' : 'salah'}, $q$ ${q.v ? 'betul' : 'salah'})`), w: W(T(`$p$: ${p.why.en} ($p$ is ${p.v ? 'true' : 'false'})`, `$p$: ${p.why.ms} ($p$ ${p.v ? 'betul' : 'salah'})`), T(`$q$: ${q.why.en} ($q$ is ${q.v ? 'true' : 'false'})`, `$q$: ${q.why.ms} ($q$ ${q.v ? 'betul' : 'salah'})`), and ? T(`"$p$ and $q$" is true only when both are true: ${v ? 'true' : 'false'}`, `"$p$ dan $q$" betul hanya apabila kedua-duanya betul: ${v ? 'betul' : 'salah'}`) : T(`"$p$ or $q$" is true when at least one is true: ${v ? 'true' : 'false'}`, `"$p$ atau $q$" betul apabila sekurang-kurangnya satu betul: ${v ? 'betul' : 'salah'}`)), sp: 's' };
    },
  ];
  const g31a = [
    (r) => {
      const c = r.pick([['p and q', 'p dan q', true, 'both $p$ and $q$ are true', '$p$ dan $q$ kedua-duanya betul'], ['p or q', 'p atau q', false, 'both $p$ and $q$ are false', '$p$ dan $q$ kedua-duanya salah']]);
      return c[2]
        ? { q: T('The compound statement "$p$ and $q$" is true. What can you say about the truth values of $p$ and $q$?', 'Pernyataan majmuk "$p$ dan $q$" adalah betul. Apakah yang boleh anda katakan tentang nilai kebenaran $p$ dan $q$?'), a: T('Both $p$ and $q$ must be true.', '$p$ dan $q$ mesti kedua-duanya betul.'), w: T('"$p$ and $q$" is true only in the row where $p$ is true and $q$ is true; if either were false, "$p$ and $q$" would be false.', '"$p$ dan $q$" betul hanya dalam baris dengan $p$ betul dan $q$ betul; jika salah satu salah, "$p$ dan $q$" salah.'), sp: 's' }
        : { q: T('The compound statement "$p$ or $q$" is false. What can you say about the truth values of $p$ and $q$?', 'Pernyataan majmuk "$p$ atau $q$" adalah salah. Apakah yang boleh anda katakan tentang nilai kebenaran $p$ dan $q$?'), a: T('Both $p$ and $q$ must be false.', '$p$ dan $q$ mesti kedua-duanya salah.'), w: T('"$p$ or $q$" is true as soon as one of $p$, $q$ is true, so it is false only when both are false.', '"$p$ atau $q$" betul jika salah satu daripada $p$, $q$ betul, maka ia salah hanya apabila kedua-duanya salah.'), sp: 's' };
    },
    (r) => ({ q: T('Complete the truth table for $p$ and $q$ giving the truth value of $p \\wedge q$ and $p \\vee q$ for all four combinations of truth values of $p$ and $q$.', 'Lengkapkan jadual kebenaran bagi $p$ dan $q$ dengan memberikan nilai kebenaran bagi $p \\wedge q$ dan $p \\vee q$ untuk keempat-empat gabungan nilai kebenaran $p$ dan $q$.'), a: T(SPM.table([['T', 'T', 'T', 'T'], ['T', 'F', 'F', 'T'], ['F', 'T', 'F', 'T'], ['F', 'F', 'F', 'F']], { head: ['$p$', '$q$', '$p \\wedge q$', '$p \\vee q$'] }), SPM.table([['B', 'B', 'B', 'B'], ['B', 'P', 'P', 'B'], ['P', 'B', 'P', 'B'], ['P', 'P', 'P', 'P']], { head: ['$p$', '$q$', '$p \\wedge q$', '$p \\vee q$'] })), w: W(T('$p \\wedge q$ ($p$ and $q$) is true only when both $p$ and $q$ are true.', '$p \\wedge q$ ($p$ dan $q$) betul hanya apabila $p$ dan $q$ kedua-duanya betul.'), T('$p \\vee q$ ($p$ or $q$) is false only when both $p$ and $q$ are false.', '$p \\vee q$ ($p$ atau $q$) salah hanya apabila $p$ dan $q$ kedua-duanya salah.')), sp: 'm' }),
  ];
  const IMP = [
  ...
  const g32m = [
    (r) => {
      const c = r.pick(IMP);
      return { q: T(`For the implication "If ${c.p.en}, then ${c.q.en}", write (a) the converse, (b) the inverse, (c) the contrapositive, and state whether each is true or false.`, `Bagi implikasi "Jika ${c.p.ms}, maka ${c.q.ms}", tulis (a) akas, (b) songsang, (c) kontrapositif, dan nyatakan sama ada setiap satu betul atau salah.`), a: T(`(a) If ${c.q.en}, then ${c.p.en}: false (${c.cex.en}). (b) If not (${c.p.en}), then not (${c.q.en}): false (same counterexample). (c) If not (${c.q.en}), then not (${c.p.en}): true (equivalent to the original).`, `(a) Jika ${c.q.ms}, maka ${c.p.ms}: salah (${c.cex.ms}). (b) Jika bukan (${c.p.ms}), maka bukan (${c.q.ms}): salah (contoh penyangkal yang sama). (c) Jika bukan (${c.q.ms}), maka bukan (${c.p.ms}): betul (setara dengan yang asal).`), w: W(T('Converse: if $q$, then $p$. Inverse: if $\\sim p$, then $\\sim q$. Contrapositive: if $\\sim q$, then $\\sim p$.', 'Akas: jika $q$, maka $p$. Songsangan: jika $\\sim p$, maka $\\sim q$. Kontrapositif: jika $\\sim q$, maka $\\sim p$.'), T(`The original implication is true, and the contrapositive always has the same truth value, so (c) is true.`, `Implikasi asal adalah betul, dan kontrapositif sentiasa mempunyai nilai kebenaran yang sama, maka (c) betul.`), T(`Counterexample to the converse: ${c.cex.en}; the inverse is equivalent to the converse, so it is false too.`, `Contoh penyangkal bagi akas: ${c.cex.ms}; songsangan setara dengan akas, maka ia juga salah.`)), sp: 'l' };
    },
  ];
  const notEn = (x) => `it is not true that ${x}`;
  const notMs = (x) => `tidak betul bahawa ${x}`;
  const KINDS = [
    { valid: true, prem: (c) => [c.p.en, c.p.ms], concl: (c) => [c.q.en, c.q.ms], how: T('Form: if $p$, then $q$; $p$ is true; therefore $q$ is true. This is a valid form.', 'Bentuk: jika $p$, maka $q$; $p$ betul; maka $q$ betul. Ini bentuk hujah yang sah.') },
  ...
  const g32a = [
    (r) => {
      ...
      return { q: T(`Premise 1: If ${c.p.en}, then ${c.q.en}. Premise 2: ${pr[0]}. Conclusion: ${co[0]}. Is the argument valid? Explain.`, `Premis 1: Jika ${c.p.ms}, maka ${c.q.ms}. Premis 2: ${pr[1]}. Kesimpulan: ${co[1]}. Adakah hujah itu sah? Terangkan.`), a: k.valid ? T('Valid: whenever both premises are true, the conclusion must be true.', 'Sah: apabila kedua-dua premis betul, kesimpulan mesti betul.') : T(`Not valid: both premises can be true while the conclusion is false (${c.cex.en}).`, `Tidak sah: kedua-dua premis boleh betul sementara kesimpulan salah (${c.cex.ms}).`), w: W(k.how, ...(k.valid ? [] : [T(`Counterexample: ${c.cex.en}.`, `Contoh penyangkal: ${c.cex.ms}.`)])), sp: 'm' };
    },
  ];
