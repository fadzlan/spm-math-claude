...
(function () {
  ...
  const g81e = [
    ...
    (r) => {
      ...
      const f = r.pick(forms);
      return { q: T(f.en, f.ms), a: T('An assumption is a statement taken as fixed/true to simplify the model; a variable is a quantity that is allowed to change and is represented by a symbol.', 'Andaian ialah pernyataan yang dianggap tetap/benar untuk memudahkan model; pemboleh ubah ialah kuantiti yang boleh berubah dan diwakili oleh suatu simbol.'), w: W(T('Example (taxi fare): "the rate per kilometre stays the same" is an assumption;', 'Contoh (tambang teksi): "kadar bagi setiap kilometer kekal sama" ialah andaian;'), T('the distance travelled, $d$, and the fare, $F$, are variables because their values change.', 'jarak yang dilalui, $d$, dan tambang, $F$, ialah pemboleh ubah kerana nilainya berubah.')), sp: 's' };
    },
  ...
  const g82m = [
    ...
    (r) => {
      ...
      return { q: T(`True or False: for the model $${modelStr(ctx, m, c)}$, doubling $${ctx.sym}$ always doubles $${ctx.qsym}$. Justify your answer using $${ctx.sym} = ${x}$ and $${ctx.sym} = ${2 * x}$.`, `Benar atau Palsu: bagi model $${modelStr(ctx, m, c)}$, menggandakan $${ctx.sym}$ akan sentiasa menggandakan $${ctx.qsym}$. Justifikasikan jawapan anda menggunakan $${ctx.sym} = ${x}$ dan $${ctx.sym} = ${2 * x}$.`), a: T(`False — at $${ctx.sym} = ${x}$, $${ctx.qsym} = ${y1}$, but at $${ctx.sym} = ${2 * x}$, $${ctx.qsym} = ${y2} \\ne ${2 * y1}$. Doubling only doubles $${ctx.qsym}$ when $c = 0$.`, `Palsu — pada $${ctx.sym} = ${x}$, $${ctx.qsym} = ${y1}$, tetapi pada $${ctx.sym} = ${2 * x}$, $${ctx.qsym} = ${y2} \\ne ${2 * y1}$. Penggandaan hanya menggandakan $${ctx.qsym}$ apabila $c = 0$.`), w: W(subW(ctx.qsym, m, c, x), subW(ctx.qsym, m, c, 2 * x), T(`$2 \\times ${y1} = ${2 * y1} \\ne ${y2}$, so the statement is false.`, `$2 \\times ${y1} = ${2 * y1} \\ne ${y2}$, jadi pernyataan itu palsu.`), T(`Only the $${m}${ctx.sym}$ part doubles; the fixed value ${c} does not.`, `Hanya bahagian $${m}${ctx.sym}$ yang berganda; nilai tetap ${c} tidak berganda.`)), sp: 'm' };
    },
    // explain a domain restriction
    (r) => {
      const ctx = r.pick(LINCTX);
      const m = r.int(...ctx.m), c = r.int(...ctx.c);
      return { q: T(`For the model $${modelStr(ctx, m, c)}$ of the ${ctx.noun.en}, explain why $${ctx.sym}$ cannot realistically be negative, and why the model may stop being realistic for very large values of $${ctx.sym}$.`, `Bagi model $${modelStr(ctx, m, c)}$ bagi ${ctx.noun.ms}, terangkan mengapa $${ctx.sym}$ tidak boleh menjadi negatif secara realistik, dan mengapa model itu mungkin tidak lagi realistik bagi nilai $${ctx.sym}$ yang sangat besar.`), a: T(`$${ctx.sym}$ represents ${ctx.unit.en}, which cannot be negative; for very large $${ctx.sym}$, real limits (capacity, practical bounds) usually stop the cost/quantity increasing at exactly the same constant rate forever.`, `$${ctx.sym}$ mewakili ${ctx.unit.ms}, yang tidak boleh negatif; bagi $${ctx.sym}$ yang sangat besar, had sebenar (kapasiti, batasan praktikal) biasanya menghalang kos/kuantiti daripada terus bertambah pada kadar malar yang sama selama-lamanya.`), w: W(T(`$${ctx.sym}$ is ${ctx.unit.en}, which is never below $0$, so the domain is $${ctx.sym} \\ge 0$.`, `$${ctx.sym}$ ialah ${ctx.unit.ms}, yang tidak pernah kurang daripada $0$, jadi domainnya ialah $${ctx.sym} \\ge 0$.`), T(`The model assumes the rate ${m} per unit never changes; for very large $${ctx.sym}$ this assumption usually fails.`, `Model itu menganggap kadar ${m} bagi setiap unit tidak pernah berubah; bagi $${ctx.sym}$ yang sangat besar andaian ini biasanya tidak lagi benar.`)), sp: 's' };
    },
  ...
  const g84m = [
    ...
    (r) => {
      ...
      const bb = a0 * b * b;
      return { q: T(`True or False: for ${s.d.en} changing by ${rate}% each period, the value after 2 periods can be found by applying ${2 * rate}% once to the starting value ${a0}. Justify your answer.`, `Benar atau Palsu: bagi ${s.d.ms} yang berubah sebanyak ${rate}% setiap tempoh, nilai selepas 2 tempoh boleh dicari dengan mengenakan ${2 * rate}% sekali sahaja kepada nilai permulaan ${a0}. Justifikasikan jawapan anda.`), a: T(`False — the correct value is $${a0}(${n(b)})^2 ${eqR(bb, 1)} ${n(correctVal)}$, not $${n(wrongVal)}$; percentage change compounds (multiplies) each period, it does not simply add up.`, `Palsu — nilai yang betul ialah $${a0}(${n(b)})^2 ${eqR(bb, 1)} ${n(correctVal)}$, bukan $${n(wrongVal)}$; perubahan peratus berkompaun (mendarab) setiap tempoh, ia tidak semudah dijumlahkan.`), w: W(`$${a0}(${n(b)})^2 = ${a0} \\times ${n(b * b)} ${eqR(bb, 1)} ${n(correctVal)}$`, T(`Applying ${2 * rate}% once: $${a0} \\times ${n(1 + (s.dir === 1 ? 2 * rate : -2 * rate) / 100)} = ${n(wrongVal)}$`, `Mengenakan ${2 * rate}% sekali: $${a0} \\times ${n(1 + (s.dir === 1 ? 2 * rate : -2 * rate) / 100)} = ${n(wrongVal)}$`), T(`The second ${rate}% is taken on the new value, not on ${a0}, so the results differ: False.`, `${rate}% yang kedua dikira atas nilai baharu, bukan atas ${a0}, jadi hasilnya berbeza: Palsu.`)), sp: 'm' };
    },
  ...
  const g86e = [
    ...
    (r) => {
      ...
      if (isBox) {
        ...
        need(V1 !== V2);
        line = { en: `Two candidate cuts are tried for an open box (side ${s} cm sheet of ${mat.en}, volume $V = x(${s} - 2x)^2$): $x = ${x1}$ cm and $x = ${x2}$ cm. Which gives the greater volume?`, ms: `Dua potongan calon dicuba bagi sebuah kotak terbuka (kepingan ${mat.ms} bersisi ${s} cm, isipadu $V = x(${s} - 2x)^2$): $x = ${x1}$ cm dan $x = ${x2}$ cm. Yang manakah memberikan isipadu lebih besar?`, aEn: `$x = ${x1}$: $V = ${V1}$ cm³; $x = ${x2}$: $V = ${V2}$ cm³; $x = ${V1 > V2 ? x1 : x2}$ gives the greater volume.`, aMs: `$x = ${x1}$: $V = ${V1}$ cm³; $x = ${x2}$: $V = ${V2}$ cm³; $x = ${V1 > V2 ? x1 : x2}$ memberikan isipadu lebih besar.`, w: W(boxW(s, x1), boxW(s, x2), T(`$${Math.max(V1, V2)} > ${Math.min(V1, V2)}$, so $x = ${V1 > V2 ? x1 : x2}$ cm.`, `$${Math.max(V1, V2)} > ${Math.min(V1, V2)}$, jadi $x = ${V1 > V2 ? x1 : x2}$ cm.`)) };
      } else {
        const K = r.pick([100, 200, 300]);
        const [r1, r2] = r.sample([2, 3, 4, 5], 2);
        const S1 = round(2 * 3.142 * r1 * r1 + (2 * K) / r1, 1), S2 = round(2 * 3.142 * r2 * r2 + (2 * K) / r2, 1);
        need(S1 !== S2);
        line = { en: `Two candidate radii are tried for a closed cylinder of fixed volume ${K} cm³ ($S = 2\\pi r^2 + \\dfrac{2(${K})}{r}$): $r = ${r1}$ cm and $r = ${r2}$ cm. Which gives the smaller surface area?`, ms: `Dua jejari calon dicuba bagi sebuah silinder tertutup berisipadu tetap ${K} cm³ ($S = 2\\pi r^2 + \\dfrac{2(${K})}{r}$): $r = ${r1}$ cm dan $r = ${r2}$ cm. Yang manakah memberikan luas permukaan lebih kecil?`, aEn: `$r = ${r1}$: $S \\approx ${n(S1)}$ cm²; $r = ${r2}$: $S \\approx ${n(S2)}$ cm²; $r = ${S1 < S2 ? r1 : r2}$ gives the smaller area.`, aMs: `$r = ${r1}$: $S \\approx ${n(S1)}$ cm²; $r = ${r2}$: $S \\approx ${n(S2)}$ cm²; $r = ${S1 < S2 ? r1 : r2}$ memberikan luas lebih kecil.`, w: W(cylW(K, r1), cylW(K, r2), T(`$${n(Math.min(S1, S2))} < ${n(Math.max(S1, S2))}$, so $r = ${S1 < S2 ? r1 : r2}$ cm.`, `$${n(Math.min(S1, S2))} < ${n(Math.max(S1, S2))}$, jadi $r = ${S1 < S2 ? r1 : r2}$ cm.`)) };
      }
    ...
    (r) => {
      ...
      if (isBox) {
        ...
        w = W(T(`Compare the volumes ${vols.join(', ')}: the largest is ${best}, at $x = ${cands[vols.indexOf(best)]}$.`, `Bandingkan isipadu ${vols.join(', ')}: yang terbesar ialah ${best}, pada $x = ${cands[vols.indexOf(best)]}$.`));
      } else {
        const can = r.pick(CYLCTX);
        const K = r.pick([100, 200, 300]);
        const cands = [2, 3, 4];
