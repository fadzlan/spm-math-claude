...
(function () {
  ...
  const STAGE_DESC = [
    ...
    T('Deciding what will be taken as fixed or true, and naming the quantities that will change', 'Menentukan apa yang dianggap tetap atau betul, dan menamakan kuantiti yang akan berubah'),
  ...
  const CYC_LINE = T('Cycle: ' + CYCLE.map((c, k) => `(${k + 1}) ${c.en}`).join(' → '), 'Kitaran: ' + CYCLE.map((c, k) => `(${k + 1}) ${c.ms}`).join(' → '));
  const stageW = (i) => T(`This describes stage ${i + 1} of 6: "${CYCLE[i].en}".`, `Ini menerangkan peringkat ${i + 1} daripada 6: "${CYCLE[i].ms}".`);
  const twoStageW = (i, j) => T(`"${STAGE_DESC[i].en}" is stage ${i + 1}; "${STAGE_DESC[j].en}" is stage ${j + 1}.`, `"${STAGE_DESC[i].ms}" ialah peringkat ${i + 1}; "${STAGE_DESC[j].ms}" ialah peringkat ${j + 1}.`);
  /** why each family suits a situation (in words) */
  const FAM_WHY = {
    linear: T('the quantity changes by the same amount for each equal step, which gives a straight line', 'kuantiti berubah dengan jumlah yang sama bagi setiap langkah yang sama, yang memberikan garis lurus'),
    quadratic: T('the quantity rises to a maximum and then falls (or falls to a minimum and then rises), which is the shape of a parabola', 'kuantiti meningkat ke nilai maksimum kemudian menurun (atau menurun ke nilai minimum kemudian meningkat), iaitu bentuk parabola'),
    exponential: T('the quantity changes by the same percentage for each equal step, so it is multiplied by the same factor each time', 'kuantiti berubah dengan peratusan yang sama bagi setiap langkah yang sama, jadi ia didarab dengan faktor yang sama setiap kali'),
  };
  /** "=" when a/b is exact to 3 d.p., else "\approx" */
  const eqAp = (a, b) => (Math.abs(round(a / b, 3) - a / b) < 1e-9 ? '=' : '\\approx');
  ...
  const g81e = [
    ...
    (r) => {
      const i = r.int(0, 5);
      return { q: T(`Which stage of the mathematical modelling cycle involves the following: "${STAGE_DESC[i].en}"?`, `Peringkat manakah dalam kitaran pemodelan matematik yang melibatkan perkara berikut: "${STAGE_DESC[i].ms}"?`), a: CYCLE[i], w: W(stageW(i)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Which stage of the mathematical modelling cycle involves the following: "${STAGE_DESC[i].en}"?<br>${line('en')}`, `Peringkat manakah dalam kitaran pemodelan matematik yang melibatkan perkara berikut: "${STAGE_DESC[i].ms}"?<br>${line('ms')}`), a: T(`${c}) ${CYCLE[i].en}`, `${c}) ${CYCLE[i].ms}`), w: W(stageW(i)), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      return { q: T(`In the mathematical modelling cycle, which happens first: "${STAGE_DESC[p].en}", or "${STAGE_DESC[q].en}"?`, `Dalam kitaran pemodelan matematik, yang manakah berlaku dahulu: "${STAGE_DESC[p].ms}", atau "${STAGE_DESC[q].ms}"?`), a: CYCLE[first], w: W(twoStageW(p, q), T(`Stage ${first + 1} comes first: "${CYCLE[first].en}".`, `Peringkat ${first + 1} berlaku dahulu: "${CYCLE[first].ms}".`)), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      const f = r.pick(forms);
      return { q: T(f.en, f.ms), a: T('An assumption is a statement taken as fixed/true to simplify the model; a variable is a quantity that is allowed to change and is represented by a symbol.', 'Andaian ialah pernyataan yang dianggap tetap/betul untuk memudahkan model; pemboleh ubah ialah kuantiti yang boleh berubah dan diwakili oleh suatu simbol.'), w: W(T('Example (taxi fare): "the rate per kilometre stays the same" is an assumption;', 'Contoh (tambang teksi): "kadar bagi setiap kilometer kekal sama" ialah andaian;'), T('the distance travelled, $d$, and the fare, $F$, are variables because their values change.', 'jarak yang dilalui, $d$, dan tambang, $F$, ialah pemboleh ubah kerana nilainya berubah.')), sp: 's' };
    },
    // identify the variable and what it depends on, from a scenario
    (r) => {
      const s = r.pick(SCEN);
      return { q: T(`In modelling ${s.d.en}, name the variable that changes, and state what it depends on.`, `Dalam pemodelan ${s.d.ms}, namakan pemboleh ubah yang berubah, dan nyatakan bergantung kepada apa.`), a: T(`${SPM.cap(s.v.en)} depends on ${s.x.en}.`, `${SPM.cap(s.v.ms)} bergantung kepada ${s.x.ms}.`), w: W(T(`The quantity we want to predict is ${s.v.en}.`, `Kuantiti yang ingin diramal ialah ${s.v.ms}.`), T(`Its value changes when ${s.x.en} changes, so it depends on ${s.x.en}.`, `Nilainya berubah apabila ${s.x.ms} berubah, jadi ia bergantung kepada ${s.x.ms}.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The stages of the mathematical modelling cycle are listed below, in order, with one stage missing.<br>${line('en')}<br>Name the missing stage, and state its position (before which listed stage, or last).`, `Peringkat kitaran pemodelan matematik disenaraikan di bawah, mengikut urutan, dengan satu peringkat tertinggal.<br>${line('ms')}<br>Namakan peringkat yang tertinggal, dan nyatakan kedudukannya (sebelum peringkat mana yang disenaraikan, atau di akhir).`), a: T(`${CYCLE[miss].en}; ${posEn}`, `${CYCLE[miss].ms}; ${posMs}`), w: W(CYC_LINE, T(`Stage ${miss + 1} is not in the list: "${CYCLE[miss].en}".`, `Peringkat ${miss + 1} tiada dalam senarai: "${CYCLE[miss].ms}".`)), sp: 's' };
    },
    // true/false with justification, ordering of two stages
    (r) => {
      const [i, j] = r.distinct(2, 0, 5);
      const claimTrue = i < j;
      return { q: T(`True or False: in the mathematical modelling cycle, "${STAGE_DESC[i].en}" happens before "${STAGE_DESC[j].en}". Justify your answer.`, `Betul atau Salah: dalam kitaran pemodelan matematik, "${STAGE_DESC[i].ms}" berlaku sebelum "${STAGE_DESC[j].ms}". Justifikasikan jawapan anda.`), a: T(`${claimTrue ? 'True' : 'False'} — the correct order has "${CYCLE[Math.min(i, j)].en}" before "${CYCLE[Math.max(i, j)].en}".`, `${claimTrue ? 'Betul' : 'Salah'} — susunan yang betul meletakkan "${CYCLE[Math.min(i, j)].ms}" sebelum "${CYCLE[Math.max(i, j)].ms}".`), w: W(twoStageW(i, j), T(`Stage ${i + 1} ${claimTrue ? 'comes before' : 'comes after'} stage ${j + 1}, so the statement is ${claimTrue ? 'true' : 'false'}.`, `Peringkat ${i + 1} berlaku ${claimTrue ? 'sebelum' : 'selepas'} peringkat ${j + 1}, jadi pernyataan itu ${claimTrue ? 'betul' : 'salah'}.`)), sp: 's' };
    },
  ];
  ...
  const g81m = [
    ...
    (r) => {
      ...
      return { q: T(`In modelling ${s.d.en}, which of the following is an assumption rather than a variable?<br>${opts.join('<br>')}`, `Dalam pemodelan ${s.d.ms}, yang manakah merupakan andaian dan bukan pemboleh ubah?<br>${optsMs.join('<br>')}`), a: T(`${c}) ${s.as.en}`, `${c}) ${s.as.ms}`), w: W(T(`${SPM.cap(s.v.en)} and ${s.x.en} are quantities that change, so they are variables.`, `${SPM.cap(s.v.ms)} dan ${s.x.ms} ialah kuantiti yang berubah, jadi ia pemboleh ubah.`), T(`"${SPM.cap(s.as.en)}" is a condition taken as true to simplify the model, so it is the assumption.`, `"${SPM.cap(s.as.ms)}" ialah syarat yang dianggap betul untuk memudahkan model, jadi ia ialah andaian.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Solving a model for the number of ${it.en} needed gives two mathematically valid roots: $n = ${forms[0]}$ and $n = ${forms[1]}$. Which value is rejected, and at which stage of the modelling cycle does this rejection happen?`, `Menyelesaikan suatu model bagi bilangan ${it.ms} yang diperlukan memberikan dua punca yang sah secara matematik: $n = ${forms[0]}$ dan $n = ${forms[1]}$. Nilai manakah yang ditolak, dan pada peringkat manakah dalam kitaran pemodelan penolakan ini berlaku?`), a: T(`$n = ${bad}$ is rejected (the number of ${it.en} cannot be negative); this happens at the "${CYCLE[4].en}" stage.`, `$n = ${bad}$ ditolak (bilangan ${it.ms} tidak boleh negatif); ini berlaku pada peringkat "${CYCLE[4].ms}".`), w: W(T(`$n$ counts ${it.en}, so it must be a positive whole number: reject $n = ${bad}$, accept $n = ${good}$.`, `$n$ membilang ${it.ms}, jadi ia mesti nombor bulat positif: tolak $n = ${bad}$, terima $n = ${good}$.`), T(`Checking a solution against the real situation is stage 5: "${CYCLE[4].en}".`, `Menyemak penyelesaian berdasarkan situasi sebenar ialah peringkat 5: "${CYCLE[4].ms}".`)), sp: 's' };
    },
    // limitation of long-range prediction, tied to family
    (r) => {
      const s = r.pick(SCEN);
      const t = r.int(2, 5);
      return { q: T(`A model for ${s.d.en} is built using data collected over the first ${t} periods. Someone wants to use it to predict far beyond that range. State one reason this long-range prediction might not be reliable.`, `Suatu model bagi ${s.d.ms} dibina menggunakan data yang dikumpul sepanjang ${t} tempoh pertama. Seseorang ingin menggunakannya untuk meramal jauh melebihi julat itu. Nyatakan satu sebab ramalan jarak jauh ini mungkin tidak boleh dipercayai.`), a: LIMIT_REASON[s.fam], w: W(T(`This situation is usually modelled by ${famArt(s.fam)} ${FAM_NAME[s.fam].en} model, built from only ${t} periods of data.`, `Situasi ini biasanya dimodelkan oleh model ${FAM_NAME[s.fam].ms}, yang dibina daripada data ${t} tempoh sahaja.`), T('Predicting far outside the data (extrapolation) assumes the same pattern continues, which real limits may break.', 'Meramal jauh di luar julat data (ekstrapolasi) menganggap pola yang sama berterusan, sedangkan had sebenar mungkin mengubahnya.')), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Match each action (1-${idx.length}) to its stage of the modelling cycle (${opts.map((_, i) => LET[i]).join(', ')}).<br>${dLine('en')}<br>${oLine('en')}`, `Padankan setiap tindakan (1-${idx.length}) dengan peringkat kitaran pemodelan (${opts.map((_, i) => LET[i]).join(', ')}).<br>${dLine('ms')}<br>${oLine('ms')}`), a: T(ans.map((c, i) => `${i + 1}-${c}`).join(', ')), w: W(...shownDesc.map((k, i) => T(`${i + 1}: stage ${k + 1}, "${CYCLE[k].en}" → ${ans[i]}`, `${i + 1}: peringkat ${k + 1}, "${CYCLE[k].ms}" → ${ans[i]}`))), sp: 's' };
    },
  ];
  ...
  const g81a = [
    ...
    (r) => {
      ...
      return { q: T(`Consider modelling ${s.d.en}.<br>${SPM.parts(parts).en}`, `Pertimbangkan pemodelan ${s.d.ms}.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(T(`(a) The quantity to predict, ${s.v.en}, changes as ${s.x.en} changes.`, `(a) Kuantiti yang diramal, ${s.v.ms}, berubah apabila ${s.x.ms} berubah.`), T(`(b) An assumption simplifies the real situation: ${s.as.en}.`, `(b) Andaian memudahkan situasi sebenar: ${s.as.ms}.`), T(`(c) Here ${FAM_WHY[s.fam].en}; model: $${FAM_EQ[s.fam]}$.`, `(c) Di sini ${FAM_WHY[s.fam].ms}; model: $${FAM_EQ[s.fam]}$.`)), sp: 'l' };
    },
    // critique a validation that only checks the data used to build the model
    (r) => {
      const s = r.pick(SCEN);
      const t = r.int(4, 8);
      return { q: T(`A student built a model for ${s.d.en} using ${t} data points, then checked that the model fits those same ${t} points well, and concluded "the model is fully validated." Explain what is wrong with this reasoning, and suggest a better way to validate the model.`, `Seorang pelajar membina model bagi ${s.d.ms} menggunakan ${t} titik data, kemudian menyemak bahawa model itu sepadan dengan ${t} titik data yang sama, lalu membuat kesimpulan "model ini telah disahkan sepenuhnya." Terangkan apa yang salah dengan penaakulan ini, dan cadangkan cara yang lebih baik untuk mengesahkan model itu.`), a: T('Fitting the data used to build the model does not prove the model is valid — it will usually fit that data well by construction. A better check is to compare its prediction against a data point that was NOT used to build the model.', 'Kesesuaian dengan data yang digunakan untuk membina model tidak membuktikan model itu sah — ia biasanya akan sepadan dengan baik atas dasar pembinaannya. Semakan yang lebih baik ialah membandingkan ramalannya dengan satu titik data yang TIDAK digunakan untuk membina model itu.'), w: W(T(`The model was chosen to fit those ${t} points, so a good fit on them is expected and proves little.`, `Model itu dipilih supaya sepadan dengan ${t} titik tersebut, jadi kesesuaian yang baik memang dijangka dan tidak membuktikan banyak.`), T('Validate with new data: predict a value that was not used, then compare it with the actual value.', 'Sahkan dengan data baharu: ramal satu nilai yang tidak digunakan, kemudian bandingkan dengan nilai sebenar.')), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A linear model $C = ${m}n + ${c}$ predicted $C = ${pred}$ when $n = ${x}$, but the actual value observed was $C = ${actual}$. Suggest ONE parameter of the model (the rate $m$ or the fixed value $c$) that could be refined, and explain, in terms of this data point, how you would adjust it.`, `Model linear $C = ${m}n + ${c}$ meramalkan $C = ${pred}$ apabila $n = ${x}$, tetapi nilai sebenar yang diperhatikan ialah $C = ${actual}$. Cadangkan SATU parameter model (kadar $m$ atau nilai tetap $c$) yang boleh diperhalusi, dan terangkan, berdasarkan titik data ini, bagaimana anda akan melaraskannya.`), a: T(`The actual value is ${dir > 0 ? 'higher' : 'lower'} than predicted by ${delta}, so the ${which === 'rate' ? `rate $m$ could be ${dir > 0 ? 'increased' : 'decreased'} slightly (each extra unit of $n$ costs a little ${dir > 0 ? 'more' : 'less'})` : `fixed value $c$ could be adjusted by about ${dir > 0 ? '+' : '-'}${delta} to shift every prediction`}; either change would bring the prediction closer to $C = ${actual}$.`, `Nilai sebenar adalah ${dir > 0 ? 'lebih tinggi' : 'lebih rendah'} daripada yang diramal sebanyak ${delta}, jadi ${which === 'rate' ? `kadar $m$ boleh ${dir > 0 ? 'ditambah' : 'dikurangkan'} sedikit (setiap tambahan unit $n$ berkos sedikit ${dir > 0 ? 'lebih' : 'kurang'})` : `nilai tetap $c$ boleh dilaraskan sebanyak lebih kurang ${dir > 0 ? '+' : '-'}${delta} untuk menganjak setiap ramalan`}; kedua-dua perubahan itu akan mendekatkan ramalan kepada $C = ${actual}$.`), w: W(`$C = ${m}(${x}) + ${c} = ${pred}$`, T(`Actual $-$ predicted $= ${actual} - ${pred} = ${actual - pred}$`, `Sebenar $-$ ramalan $= ${actual} - ${pred} = ${actual - pred}$`), which === 'rate' ? T(`Keeping $c = ${c}$: $m = \\dfrac{${actual} - ${c}}{${x}} = ${Fr.tex(Fr.make(actual - c, x))}$ would fit this point.`, `Kekalkan $c = ${c}$: $m = \\dfrac{${actual} - ${c}}{${x}} = ${Fr.tex(Fr.make(actual - c, x))}$ akan sepadan dengan titik ini.`) : T(`Keeping $m = ${m}$: $c = ${actual} - ${m}(${x}) = ${actual - m * x}$ would fit this point.`, `Kekalkan $m = ${m}$: $c = ${actual} - ${m}(${x}) = ${actual - m * x}$ akan sepadan dengan titik ini.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A short report on modelling ${s.d.en} lists these stages, in order, but one stage of the cycle is missing entirely: ${rep}. Name the missing stage, and write one sentence that could fill that gap for this situation.`, `Satu laporan ringkas tentang pemodelan ${s.d.ms} menyenaraikan peringkat berikut, mengikut urutan, tetapi satu peringkat kitaran hilang sepenuhnya: ${repMs}. Namakan peringkat yang hilang, dan tulis satu ayat yang boleh mengisi jurang itu bagi situasi ini.`), a: T(`Missing stage: "${CYCLE[miss].en}". Example sentence: ${miss === 2 ? `"We model ${s.v.en} as a function of ${s.x.en} using ${famArt(s.fam)} ${FAM_NAME[s.fam].en} model."` : miss === 3 ? `"Substituting the given values into the model gives the numerical answer."` : miss === 4 ? `"The answer is checked against the context and, where possible, against real data."` : `"The model, the answer and its limitations are written up clearly so that others can check them."`}`, `Peringkat yang hilang: "${CYCLE[miss].ms}". Contoh ayat: ${miss === 2 ? `"Kami memodelkan ${s.v.ms} sebagai fungsi ${s.x.ms} menggunakan model ${FAM_NAME[s.fam].ms}."` : miss === 3 ? `"Menggantikan nilai yang diberi ke dalam model memberikan jawapan berangka."` : miss === 4 ? `"Jawapan disemak berdasarkan konteks dan, jika boleh, berdasarkan data sebenar."` : `"Model, jawapan dan batasannya ditulis dengan jelas supaya orang lain dapat menyemaknya."`}`), w: W(CYC_LINE, T(`The list goes from "${CYCLE[miss - 1].en}" ${miss === 5 ? 'and then stops' : `straight to "${CYCLE[miss + 1].en}"`}, so stage ${miss + 1} is missing.`, `Senarai itu bergerak daripada "${CYCLE[miss - 1].ms}" ${miss === 5 ? 'dan kemudian berhenti' : `terus ke "${CYCLE[miss + 1].ms}"`}, jadi peringkat ${miss + 1} hilang.`)), sp: 'm' };
    },
  ];
  ...
  /** "$Q = m(x) + c = y$" substitution line */
  const subW = (q, m, c, x) => `$${q} = ${m}(${x}) ${c < 0 ? '-' : '+'} ${Math.abs(c)} = ${n(m * x + c)}$`;
  /** "=" if v is exact to dp decimal places, else "\approx" */
  const eqR = (v, dp) => (Math.abs(round(v, dp) - v) < 1e-9 ? '=' : '\\approx');
  /** \dfrac{num}{den} = value (exact) or \approx value (2 d.p.) */
  /** "$kx = r \\Rightarrow x = r/k$" (just "$x = r$" when k = 1) */
  const solveX = (k, rhs, v) => (k === 1 ? `$${v || 'x'} = ${n(rhs)}$` : `$${lin(k, 0, v || 'x')} = ${n(rhs)} \\Rightarrow ${v || 'x'} = ${n(rhs / k)}$`);
  const divT = (num, den) => `\\dfrac{${num}}{${den}} ${eqR(num / den, 2)} ${n(round(num / den, 2))}`;
  ...
  const g82e = [
    ...
    (r) => {
      ...
      if (interpretOnly) {
        return { q: T(`A model for the ${ctx.noun.en} is $${eq}$, where $${ctx.sym}$ is ${ctx.unit.en}. State what the values ${m} and ${c} represent.`, `Suatu model bagi ${ctx.noun.ms} ialah $${eq}$, dengan $${ctx.sym}$ ialah ${ctx.unit.ms}. Nyatakan apa yang diwakili oleh nilai ${m} dan ${c}.`), a: T(`${m} is the rate of change per unit of ${ctx.sym}; ${c} is the fixed/initial value (value when ${ctx.sym} = 0).`, `${m} ialah kadar perubahan bagi setiap unit ${ctx.sym}; ${c} ialah nilai tetap/awal (nilai apabila ${ctx.sym} = 0).`), w: W(T(`${m} is the coefficient of $${ctx.sym}$ (the gradient): $${ctx.qsym}$ increases by ${m} for each extra unit of $${ctx.sym}$.`, `${m} ialah pekali $${ctx.sym}$ (kecerunan): $${ctx.qsym}$ bertambah sebanyak ${m} bagi setiap tambahan unit $${ctx.sym}$.`), T(`${c} is the constant term: $${ctx.qsym} = ${m}(0) + ${c} = ${c}$ when $${ctx.sym} = 0$.`, `${c} ialah sebutan pemalar: $${ctx.qsym} = ${m}(0) + ${c} = ${c}$ apabila $${ctx.sym} = 0$.`)), sp: 's' };
      }
      return { q: T(`A model for the ${ctx.noun.en} is $${eq}$, where $${ctx.sym}$ is ${ctx.unit.en}. Find the ${ctx.noun.en} when $${ctx.sym} = ${x}$.`, `Suatu model bagi ${ctx.noun.ms} ialah $${eq}$, dengan $${ctx.sym}$ ialah ${ctx.unit.ms}. Cari ${ctx.noun.ms} apabila $${ctx.sym} = ${x}$.`), a: T(fmtV(ctx, y)), w: W(subW(ctx.qsym, m, c, x), fmtV(ctx, y)), sp: 'xs' };
    },
    // construct the model from a worded description (no equation given)
    (r) => {
      const ctx = r.pick(LINCTX);
      const m = r.int(...ctx.m), c = r.int(...ctx.c);
      return { q: T(`A ${ctx.noun.en} starts at ${fmtV(ctx, c)} and increases by ${fmtV(ctx, m)} for every extra unit of $${ctx.sym}$ (${ctx.unit.en}). Write a linear model $${ctx.qsym} = m${ctx.sym} + c$ for this.`, `${SPM.cap(ctx.noun.ms)} bermula pada ${fmtV(ctx, c)} dan bertambah sebanyak ${fmtV(ctx, m)} bagi setiap tambahan unit $${ctx.sym}$ (${ctx.unit.ms}). Tulis model linear $${ctx.qsym} = m${ctx.sym} + c$ bagi keadaan ini.`), a: T(`$${modelStr(ctx, m, c)}$`), w: W(T(`Starting (fixed) value: $c = ${c}$; increase per unit of $${ctx.sym}$: $m = ${m}$`, `Nilai permulaan (tetap): $c = ${c}$; pertambahan bagi setiap unit $${ctx.sym}$: $m = ${m}$`), `$${modelStr(ctx, m, c)}$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The table shows the ${ctx.noun.en} for a few values of $${ctx.sym}$ (${ctx.unit.en}).<br>${tab}<br>State the values of $m$ and $c$ in the linear model $${ctx.qsym} = m${ctx.sym} + c$.`, `Jadual menunjukkan ${ctx.noun.ms} bagi beberapa nilai $${ctx.sym}$ (${ctx.unit.ms}).<br>${tab}<br>Nyatakan nilai $m$ dan $c$ dalam model linear $${ctx.qsym} = m${ctx.sym} + c$.`), a: T(`$m = ${m}$, $c = ${c}$`), w: W(T(`At $${ctx.sym} = 0$, $${ctx.qsym} = ${c}$, so $c = ${c}$.`, `Pada $${ctx.sym} = 0$, $${ctx.qsym} = ${c}$, jadi $c = ${c}$.`), T(`Each increase of 1 in $${ctx.sym}$ adds $${m + c} - ${c} = ${m}$, so $m = ${m}$.`, `Setiap pertambahan 1 dalam $${ctx.sym}$ menambah $${m + c} - ${c} = ${m}$, jadi $m = ${m}$.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`In the model $${modelStr(ctx, m, c)}$ for the ${ctx.noun.en}, what does the value ${val} represent?<br>${line('en')}`, `Dalam model $${modelStr(ctx, m, c)}$ bagi ${ctx.noun.ms}, apakah yang diwakili oleh nilai ${val}?<br>${line('ms')}`), a: T(`${c2}) ${correct.en}`, `${c2}) ${correct.ms}`), w: W(askC ? T(`${c} is the constant term: $${ctx.qsym} = ${m}(0) + ${c} = ${c}$ when $${ctx.sym} = 0$.`, `${c} ialah sebutan pemalar: $${ctx.qsym} = ${m}(0) + ${c} = ${c}$ apabila $${ctx.sym} = 0$.`) : T(`${m} is the coefficient of $${ctx.sym}$: each extra unit of $${ctx.sym}$ adds ${m} to $${ctx.qsym}$.`, `${m} ialah pekali $${ctx.sym}$: setiap tambahan unit $${ctx.sym}$ menambah ${m} kepada $${ctx.qsym}$.`)), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph shows a linear model for the ${ctx.noun.en} against $${ctx.sym}$ (${ctx.unit.en}). State the gradient and the $${ctx.qsym}$-intercept, and hence write the equation of the model.`, `Graf menunjukkan model linear bagi ${ctx.noun.ms} berbanding $${ctx.sym}$ (${ctx.unit.ms}). Nyatakan kecerunan dan pintasan-$${ctx.qsym}$, dan seterusnya tulis persamaan model itu.`), a: T(`Gradient $= ${m}$, intercept $= ${c}$; $${modelStr(ctx, m, c)}$`, `Kecerunan $= ${m}$, pintasan $= ${c}$; $${modelStr(ctx, m, c)}$`), w: W(T(`The line meets the vertical axis at $(0, ${c})$, so $c = ${c}$.`, `Garis itu bertemu paksi mencancang pada $(0, ${c})$, jadi $c = ${c}$.`), `$m = \\dfrac{${m * xmax + c} - ${c}}{${xmax} - 0} = \\dfrac{${m * xmax}}{${xmax}} = ${m}$`, `$${modelStr(ctx, m, c)}$`), sp: 's', fig };
    },
  ];
  ...
  const g82m = [
    ...
    (r) => {
      ...
      return { q: T(`The ${ctx.noun.en} follows the model $${modelStr(ctx, m, c)}$. If the budget is ${rm(budget)}, find the greatest whole value of $${ctx.sym}$ (${ctx.unit.en}) that can be afforded.`, `${SPM.cap(ctx.noun.ms)} mengikut model $${modelStr(ctx, m, c)}$. Jika bajet ialah ${rm(budget)}, cari nilai bulat terbesar $${ctx.sym}$ (${ctx.unit.ms}) yang mampu dibiayai.`), a: T(`$${ctx.sym} \\le ${divT(budget - c, m)}$, so the greatest whole value is $${ctx.sym} = ${feasible}$.`, `$${ctx.sym} \\le ${divT(budget - c, m)}$, jadi nilai bulat terbesar ialah $${ctx.sym} = ${feasible}$.`), w: W(`$${lin(m, c, ctx.sym)} \\le ${budget}$`, `$${lin(m, 0, ctx.sym)} \\le ${budget} - ${c} = ${budget - c}$`, `$${ctx.sym} \\le ${divT(budget - c, m)}$`, T(`Round down (rounding up would go over the budget): $${ctx.sym} = ${feasible}$`, `Bundarkan ke bawah (membundarkan ke atas akan melebihi bajet): $${ctx.sym} = ${feasible}$`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`True or False: for the model $${modelStr(ctx, m, c)}$, doubling $${ctx.sym}$ always doubles $${ctx.qsym}$. Justify your answer using $${ctx.sym} = ${x}$ and $${ctx.sym} = ${2 * x}$.`, `Betul atau Salah: bagi model $${modelStr(ctx, m, c)}$, menggandakan $${ctx.sym}$ akan sentiasa menggandakan $${ctx.qsym}$. Justifikasikan jawapan anda menggunakan $${ctx.sym} = ${x}$ dan $${ctx.sym} = ${2 * x}$.`), a: T(`False — at $${ctx.sym} = ${x}$, $${ctx.qsym} = ${y1}$, but at $${ctx.sym} = ${2 * x}$, $${ctx.qsym} = ${y2} \\ne ${2 * y1}$. Doubling only doubles $${ctx.qsym}$ when $c = 0$.`, `Salah — pada $${ctx.sym} = ${x}$, $${ctx.qsym} = ${y1}$, tetapi pada $${ctx.sym} = ${2 * x}$, $${ctx.qsym} = ${y2} \\ne ${2 * y1}$. Penggandaan hanya menggandakan $${ctx.qsym}$ apabila $c = 0$.`), w: W(subW(ctx.qsym, m, c, x), subW(ctx.qsym, m, c, 2 * x), T(`$2 \\times ${y1} = ${2 * y1} \\ne ${y2}$, so the statement is false.`, `$2 \\times ${y1} = ${2 * y1} \\ne ${y2}$, jadi pernyataan itu salah.`), T(`Only the $${m}${ctx.sym}$ part doubles; the fixed value ${c} does not.`, `Hanya bahagian $${m}${ctx.sym}$ yang berganda; nilai tetap ${c} tidak berganda.`)), sp: 'm' };
    },
    // explain a domain restriction
    (r) => {
      const ctx = r.pick(LINCTX);
      const m = r.int(...ctx.m), c = r.int(...ctx.c);
      return { q: T(`For the model $${modelStr(ctx, m, c)}$ of the ${ctx.noun.en}, explain why $${ctx.sym}$ cannot realistically be negative, and why the model may stop being realistic for very large values of $${ctx.sym}$.`, `Bagi model $${modelStr(ctx, m, c)}$ bagi ${ctx.noun.ms}, terangkan mengapa $${ctx.sym}$ tidak boleh menjadi negatif secara realistik, dan mengapa model itu mungkin tidak lagi realistik bagi nilai $${ctx.sym}$ yang sangat besar.`), a: T(`$${ctx.sym}$ represents ${ctx.unit.en}, which cannot be negative; for very large $${ctx.sym}$, real limits (capacity, practical bounds) usually stop the cost/quantity increasing at exactly the same constant rate forever.`, `$${ctx.sym}$ mewakili ${ctx.unit.ms}, yang tidak boleh negatif; bagi $${ctx.sym}$ yang sangat besar, had sebenar (kapasiti, batasan praktikal) biasanya menghalang kos/kuantiti daripada terus bertambah pada kadar malar yang sama selama-lamanya.`), w: W(T(`$${ctx.sym}$ is ${ctx.unit.en}, which is never below $0$, so the domain is $${ctx.sym} \\ge 0$.`, `$${ctx.sym}$ ialah ${ctx.unit.ms}, yang tidak pernah kurang daripada $0$, jadi domainnya ialah $${ctx.sym} \\ge 0$.`), T(`The model assumes the rate ${m} per unit never changes; for very large $${ctx.sym}$ this assumption usually fails.`, `Model itu menganggap kadar ${m} bagi setiap unit tidak pernah berubah; bagi $${ctx.sym}$ yang sangat besar andaian ini biasanya tidak lagi betul.`)), sp: 's' };
    },
  ...
  const g82a = [
    ...
    (r) => {
      ...
      // A: higher fixed charge, lower rate; B: lower fixed charge, higher rate (cB < cA always)
      need(mB > mA);
      ...
      const ansParts = [T(`$C_A = ${lin(mA, cA, 'x')}$, $C_B = ${lin(mB, cB, 'x')}$`), T(`$${x0}$`), T(`For $x > ${x0}$, provider A is cheaper (smaller rate $${mA} < ${mB}$); however real providers often cap usage or change rates, so the model may only be realistic within a limited range of $x$.`, `Bagi $x > ${x0}$, pembekal A lebih murah (kadar lebih kecil $${mA} < ${mB}$); namun pembekal sebenar selalunya mengehadkan penggunaan atau menukar kadar, jadi model itu mungkin hanya realistik dalam julat $x$ yang terhad.`)];
      return { q: T(`Provider A charges ${rm(cA)} plus ${rm(mA)} per unit of $x$ for the ${ctxA.noun.en}-style service; Provider B charges ${rm(cB)} plus ${rm(mB)} per unit of $x$.<br>${SPM.parts(parts).en}`, `Pembekal A mengenakan ${rm(cA)} ditambah ${rm(mA)} bagi setiap unit $x$ untuk perkhidmatan seperti ${ctxA.noun.ms}; Pembekal B mengenakan ${rm(cB)} ditambah ${rm(mB)} bagi setiap unit $x$.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(`(a) $C_A = ${lin(mA, cA, 'x')}$, $C_B = ${lin(mB, cB, 'x')}$`, `(b) $${lin(mA, cA, 'x')} = ${lin(mB, cB, 'x')}$`, solveX(mB - mA, cA - cB), T(`(c) Beyond $x = ${x0}$ the smaller rate wins: A (${mA} < ${mB}). Check $x = ${x0 + 1}$: A $= ${mA * (x0 + 1) + cA}$, B $= ${mB * (x0 + 1) + cB}$.`, `(c) Selepas $x = ${x0}$ kadar yang lebih kecil lebih murah: A (${mA} < ${mB}). Semak $x = ${x0 + 1}$: A $= ${mA * (x0 + 1) + cA}$, B $= ${mB * (x0 + 1) + cB}$.`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A linear model $${modelStr(ctx, m, c)}$ for the ${ctx.noun.en} was built from data with $${ctx.sym}$ between ${lo} and ${hi}.<br>${SPM.parts(parts).en}`, `Model linear $${modelStr(ctx, m, c)}$ bagi ${ctx.noun.ms} dibina daripada data dengan $${ctx.sym}$ antara ${lo} dan ${hi}.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(T(`(a) ${subW(ctx.qsym, m, c, xin)}; $${lo} \\le ${xin} \\le ${hi}$, inside the data range: interpolation.`, `(a) ${subW(ctx.qsym, m, c, xin)}; $${lo} \\le ${xin} \\le ${hi}$, dalam julat data: interpolasi.`), T(`(b) ${subW(ctx.qsym, m, c, xout)}; $${xout} > ${hi}$, outside the data range: extrapolation.`, `(b) ${subW(ctx.qsym, m, c, xout)}; $${xout} > ${hi}$, di luar julat data: ekstrapolasi.`)), sp: 'l' };
    },
    // construct your own example
    (r) => {
      const m = r.int(2, 8), c = r.pick([10, 15, 20, 25, 30]);
      return { q: T(`Describe a real-life situation that could be modelled by $y = ${m}x + ${c}$, stating clearly what $x$ and $y$ represent (with units) and one assumption your model relies on.`, `Terangkan satu situasi kehidupan sebenar yang boleh dimodelkan oleh $y = ${m}x + ${c}$, dengan menyatakan dengan jelas apa yang diwakili oleh $x$ dan $y$ (berserta unit) dan satu andaian yang menjadi asas model anda.`), a: T(`Any correct context works, for example: $y$ = total cost (RM), $x$ = number of items bought, ${c} = fixed/delivery fee, ${m} = price per item; assumption: the price per item does not change.`, `Sebarang konteks yang betul boleh diterima, contohnya: $y$ = jumlah kos (RM), $x$ = bilangan item dibeli, ${c} = yuran tetap/penghantaran, ${m} = harga bagi setiap item; andaian: harga bagi setiap item tidak berubah.`), w: W(T(`${c} is the value of $y$ when $x = 0$ (a fixed amount); ${m} is the increase in $y$ for each extra unit of $x$ (a rate).`, `${c} ialah nilai $y$ apabila $x = 0$ (jumlah tetap); ${m} ialah pertambahan $y$ bagi setiap tambahan unit $x$ (kadar).`), T(`Check the example: $x = 3$ gives $y = ${m}(3) + ${c} = ${3 * m + c}$.`, `Semak contoh itu: $x = 3$ memberi $y = ${m}(3) + ${c} = ${3 * m + c}$.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      const ansParts = [T(`${yAlow < yBlow ? 'Plan A' : 'Plan B'} is cheaper by ${rm(Math.abs(yAlow - yBlow))}.`, `${yAlow < yBlow ? 'Pelan A' : 'Pelan B'} lebih murah sebanyak ${rm(Math.abs(yAlow - yBlow))}.`), T(`${yAhigh < yBhigh ? 'Plan A' : 'Plan B'} is cheaper by ${rm(Math.abs(yAhigh - yBhigh))}.`, `${yAhigh < yBhigh ? 'Pelan A' : 'Pelan B'} lebih murah sebanyak ${rm(Math.abs(yAhigh - yBhigh))}.`), T(`A low-usage customer (below $x = ${x0}$) should choose Plan B (lower fixed cost); a high-usage customer (above $x = ${x0}$) should choose Plan A (lower rate) — the two plans cross at $x = ${x0}$.`, `Pelanggan penggunaan rendah (bawah $x = ${x0}$) patut memilih Pelan B (kos tetap lebih rendah); pelanggan penggunaan tinggi (atas $x = ${x0}$) patut memilih Pelan A (kadar lebih rendah) — kedua-dua pelan bersilang pada $x = ${x0}$.`)];
      return { q: T(`Plan A costs $C = ${lin(mA, cA, 'x')}$ and Plan B costs $C = ${lin(mB, cB, 'x')}$, for the same service as ${ctxA.noun.en}.<br>${SPM.parts(parts).en}`, `Pelan A berkos $C = ${lin(mA, cA, 'x')}$ dan Pelan B berkos $C = ${lin(mB, cB, 'x')}$, bagi perkhidmatan yang sama seperti ${ctxA.noun.ms}.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(`(a) A: ${subW('C', mA, cA, lowX)}, B: ${subW('C', mB, cB, lowX)}`, `(b) A: ${subW('C', mA, cA, highX)}, B: ${subW('C', mB, cB, highX)}`, `(c) $${lin(mA, cA, 'x')} = ${lin(mB, cB, 'x')} \\Rightarrow x = \\dfrac{${cA - cB}}{${mB - mA}} = ${x0}$`, T(`Below $x = ${x0}$ Plan B (smaller fixed cost) is cheaper; above it Plan A (smaller rate) is cheaper.`, `Di bawah $x = ${x0}$ Pelan B (kos tetap lebih kecil) lebih murah; di atasnya Pelan A (kadar lebih kecil) lebih murah.`)), sp: 'l' };
    },
  ];
  ...
  /** (x - v) with correct sign, plain x when v = 0 */
  const fz = (v) => (v === 0 ? 'x' : `(${lin(1, -v)})`);
  /** bracket a negative number */
  const pn = (v) => (v < 0 ? `(${n(v)})` : n(v));
  const ZS = SPM.zeroSteps;
  const orT = (a, b) => T(`${a} or ${b}`, `${a} atau ${b}`);
  /** first/second differences of a list, as maths text */
  const diffList = (ys) => ys.slice(1).map((y, i) => y - ys[i]);
  const diffW = (ys) => diffList(ys).map((d, i) => `$${pn(ys[i + 1])} - ${pn(ys[i])} = ${d}$`).join(', ');
  const projSub = (v, t) => `$h = ${v}(${t}) - 5(${t})^2 = ${n(v * t)} - ${n(5 * t * t)} = ${n(v * t - 5 * t * t)}$`;
  ...
  const g83e = [
    ...
    (r) => {
      ...
      return { q: T(`The height of a ${item.en} thrown upwards is $h = ${v}t - 5t^2$ metres after $t$ seconds. Find its height after ${t} second${t > 1 ? 's' : ''}.`, `Ketinggian sebiji ${item.ms} yang dilontar ke atas ialah $h = ${v}t - 5t^2$ meter selepas $t$ saat. Cari ketinggiannya selepas ${t} saat.`), a: T(`${v * t - 5 * t * t} m`), w: W(projSub(v, t), `${v * t - 5 * t * t} m`), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A ${item.en} has a perimeter of ${P} m. Its width is $x$ m, so its area is modelled by $A = x(${half} - x)$. Find the area when $x = ${x}$.`, `Sebuah ${item.ms} mempunyai perimeter ${P} m. Lebarnya ialah $x$ m, jadi luasnya dimodelkan oleh $A = x(${half} - x)$. Cari luas apabila $x = ${x}$.`), a: T(`${x * (half - x)} m²`), w: W(`$A = ${x}(${half} - ${x}) = ${x}(${half - x}) = ${x * (half - x)}$`, T(`(Half the perimeter is $${P} \\div 2 = ${half}$, so the length is $${half} - x$.)`, `(Separuh perimeter ialah $${P} \\div 2 = ${half}$, jadi panjangnya $${half} - x$.)`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A stall's profit (RM) from selling $n$ ${item.en} is modelled by $P = -(n - ${a})(n - ${b})$. Find the profit when $n = ${nMid}$.`, `Keuntungan (RM) sebuah gerai daripada menjual $n$ ${item.ms} dimodelkan oleh $P = -(n - ${a})(n - ${b})$. Cari keuntungan apabila $n = ${nMid}$.`), a: T(`RM${-(nMid - a) * (nMid - b)}`), w: W(`$P = -(${nMid} - ${a})(${nMid} - ${b}) = -(${nMid - a})(${nMid - b}) = ${-(nMid - a) * (nMid - b)}$`), sp: 's' };
    },
    // read off a, b, c from the expanded form
    (r) => {
      const a = r.int(1, 5) * r.pick([1, -1]), b = r.nz(-8, 8), c = r.int(-6, 10);
      need(a !== 0);
      return { q: T(`A quadratic model is $y = ${q2(a, b, c)}$. State the values of $a$, $b$ and $c$ when this is written as $y = ax^2 + bx + c$.`, `Suatu model kuadratik ialah $y = ${q2(a, b, c)}$. Nyatakan nilai $a$, $b$ dan $c$ apabila ini ditulis sebagai $y = ax^2 + bx + c$.`), a: T(`$a = ${a}$, $b = ${b}$, $c = ${c}$`), w: W(T(`Coefficient of $x^2$: $a = ${a}$; coefficient of $x$: $b = ${b}$; constant term: $c = ${c}$`, `Pekali $x^2$: $a = ${a}$; pekali $x$: $b = ${b}$; sebutan pemalar: $c = ${c}$`), T('Take each coefficient together with its sign.', 'Ambil setiap pekali bersama tandanya.')), sp: 's' };
    },
    ...
    (r) => {
      ...
      const v = r.pick([10, 15, 20, 25, 30, 35]);
      const root = v / 5;
      return { q: T(`The height of a ${item.en} factorises as $h = 5t(${root} - t)$. State the two values of $t$ for which $h = 0$.`, `Ketinggian sebuah ${item.ms} difaktorkan sebagai $h = 5t(${root} - t)$. Nyatakan dua nilai $t$ apabila $h = 0$.`), a: T(`$t = 0$ and $t = ${root}$`, `$t = 0$ dan $t = ${root}$`), w: W(`$5t(${root} - t) = 0$`, orT(`$5t = 0$`, `$${root} - t = 0$`), orT(`$t = 0$`, `$t = ${root}$`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Solving a model for the time a ${item.en} reaches a certain height gives $t = ${pair[0]}$ or $t = ${pair[1]}$. Which value is the valid answer, and why?`, `Menyelesaikan suatu model bagi masa sebuah ${item.ms} mencapai ketinggian tertentu memberikan $t = ${pair[0]}$ atau $t = ${pair[1]}$. Nilai manakah jawapan yang sah, dan mengapa?`), a: T(`$t = ${good}$ s; time cannot be negative, so $t = ${bad}$ is rejected.`, `$t = ${good}$ s; masa tidak boleh negatif, jadi $t = ${bad}$ ditolak.`), w: W(T(`$t$ is the time after the throw, so $t \\ge 0$: reject $t = ${bad}$ and keep $t = ${good}$.`, `$t$ ialah masa selepas lontaran, jadi $t \\ge 0$: tolak $t = ${bad}$ dan terima $t = ${good}$.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph shows $y = ${fz(rt1)}${fz(rt2)}$. State the values of $x$ for which $y = 0$ (the $x$-intercepts).`, `Graf menunjukkan $y = ${fz(rt1)}${fz(rt2)}$. Nyatakan nilai $x$ apabila $y = 0$ (pintasan-$x$).`), a: T(`$x = ${rt1}$ and $x = ${rt2}$`, `$x = ${rt1}$ dan $x = ${rt2}$`), w: W(...ZS(1, -rt1, 1, -rt2), T('These are where the curve crosses the $x$-axis.', 'Ini ialah titik lengkung itu memotong paksi-$x$.')), sp: 's', fig };
    },
    ...
    (r) => {
      ...
      return { q: T(`A stall's profit (RM) from selling $n$ ${item.en} is $P = -(n - ${a})(n - ${b})$. Show that the profit is RM0 when $n = ${which}$.`, `Keuntungan (RM) sebuah gerai daripada menjual $n$ ${item.ms} ialah $P = -(n - ${a})(n - ${b})$. Tunjukkan bahawa keuntungannya RM0 apabila $n = ${which}$.`), a: T(`$P = -(${which} - ${a})(${which} - ${b}) = ${which === a ? `-(0)(${a - b})` : `-(${b - a})(0)`} = 0$`, `$P = -(${which} - ${a})(${which} - ${b}) = ${which === a ? `-(0)(${a - b})` : `-(${b - a})(0)`} = 0$`), w: W(T(`At $n = ${which}$ the factor $(n - ${which})$ is $0$, and any product with a factor $0$ is $0$.`, `Pada $n = ${which}$ faktor $(n - ${which})$ ialah $0$, dan sebarang hasil darab dengan faktor $0$ ialah $0$.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      need(h1 !== h2);
      return { q: T(`For a ${item.en} with height $h = ${v}t - 5t^2$, is the height greater at $t = ${t1}$ s or at $t = ${t2}$ s?`, `Bagi sebuah ${item.ms} dengan ketinggian $h = ${v}t - 5t^2$, ketinggiannya lebih besar pada $t = ${t1}$ s atau pada $t = ${t2}$ s?`), a: T(`At $t = ${t1}$ s, $h = ${h1}$ m; at $t = ${t2}$ s, $h = ${h2}$ m; height is greater at $t = ${h1 > h2 ? t1 : t2}$ s.`, `Pada $t = ${t1}$ s, $h = ${h1}$ m; pada $t = ${t2}$ s, $h = ${h2}$ m; ketinggian lebih besar pada $t = ${h1 > h2 ? t1 : t2}$ s.`), w: W(projSub(v, t1), projSub(v, t2), T(`$${Math.max(h1, h2)} > ${Math.min(h1, h2)}$, so it is higher at $t = ${h1 > h2 ? t1 : t2}$ s.`, `$${Math.max(h1, h2)} > ${Math.min(h1, h2)}$, jadi ia lebih tinggi pada $t = ${h1 > h2 ? t1 : t2}$ s.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A ${item.en} has perimeter ${P} m and width $x$ m, modelled by $A = x(${half} - x)$. Which value of $x$ below is NOT physically valid?<br>${line}`, `Sebuah ${item.ms} mempunyai perimeter ${P} m dan lebar $x$ m, dimodelkan oleh $A = x(${half} - x)$. Nilai $x$ manakah di bawah yang TIDAK sah secara fizikal?<br>${line}`), a: T(`${c}) $x = ${invalid}$ (width must satisfy $0 < x < ${half}$)`, `${c}) $x = ${invalid}$ (lebar mesti memenuhi $0 < x < ${half}$)`), w: W(T(`Width $x > 0$ and length $${half} - x > 0$, so $0 < x < ${half}$.`, `Lebar $x > 0$ dan panjang $${half} - x > 0$, jadi $0 < x < ${half}$.`), invalid < 0 ? T(`$x = ${invalid}$ is a negative width.`, `$x = ${invalid}$ ialah lebar negatif.`) : T(`$x = ${invalid}$ gives length $${half} - ${invalid} = ${half - invalid} < 0$.`, `$x = ${invalid}$ memberi panjang $${half} - ${invalid} = ${half - invalid} < 0$.`)), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      const line = opts.map((o, k) => `${LET[k]}) $${o}$`).join('<br>');
      return { q: T(`Which factorised expression is equivalent to $${q2(1, bExp, cExp)}$?<br>${line}`, `Ungkapan bentuk faktor manakah yang setara dengan $${q2(1, bExp, cExp)}$?<br>${line}`), a: T(`${c}) $${correct}$`), w: W(T(`Two numbers with product $${cExp}$ and sum $${bExp}$: $-${p}$ and $-${q}$`, `Dua nombor dengan hasil darab $${cExp}$ dan hasil tambah $${bExp}$: $-${p}$ dan $-${q}$`), `$(x - ${p})(x - ${q}) = x^2 - ${p}x - ${q}x + ${cExp} = ${q2(1, bExp, cExp)}$`), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      const zw = W(orT(`$n - ${a} = 0$`, `$n - ${b} = 0$`), orT(`$n = ${a}$`, `$n = ${b}$`), T(`Only $n = ${correct}$ appears in the options.`, `Hanya $n = ${correct}$ terdapat dalam pilihan.`));
      return { q: T(`A stall's profit from selling $n$ ${item.en} is $P = -(n - ${a})(n - ${b})$. Which value of $n$ below gives exactly zero profit?<br>${line}`, `Keuntungan sebuah gerai daripada menjual $n$ ${item.ms} ialah $P = -(n - ${a})(n - ${b})$. Nilai $n$ manakah di bawah memberikan keuntungan sifar dengan tepat?<br>${line}`), a: T(`${c}) $n = ${correct}$`), w: zw, sp: 'xs' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A stall's profit from selling $n$ ${item.en} is $P = ${q2v('n', -1, bExp, cExp)}$. State the coefficient of $n^2$, the coefficient of $n$, and the constant term.`, `Keuntungan sebuah gerai daripada menjual $n$ ${item.ms} ialah $P = ${q2v('n', -1, bExp, cExp)}$. Nyatakan pekali $n^2$, pekali $n$, dan sebutan pemalar.`), a: T(`Coefficient of $n^2$ is $-1$; coefficient of $n$ is ${bExp}; constant term is ${cExp}.`, `Pekali $n^2$ ialah $-1$; pekali $n$ ialah ${bExp}; sebutan pemalar ialah ${cExp}.`), w: W(T(`Compare with $P = an^2 + bn + c$: $a = -1$, $b = ${bExp}$, $c = ${cExp}$`, `Bandingkan dengan $P = an^2 + bn + c$: $a = -1$, $b = ${bExp}$, $c = ${cExp}$`), T(`$-n^2$ means the coefficient is $-1$.`, `$-n^2$ bermaksud pekalinya $-1$.`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph shows $y = ${fz(rt1)}${fz(rt2)}$. State the value of $y$ when $x = ${xm}$ (marked on the graph).`, `Graf menunjukkan $y = ${fz(rt1)}${fz(rt2)}$. Nyatakan nilai $y$ apabila $x = ${xm}$ (ditandakan pada graf).`), a: T(`$y = ${f(xm)}$`), w: W(`$y = (${xm} - ${pn(rt1)})(${xm} - ${rt2}) = (${xm - rt1})(${xm - rt2}) = ${f(xm)}$`), sp: 'xs', fig };
    },
  ];
  ...
  const g83m = [
    ...
    (r) => {
      ...
      const v = 5 * root;
      return { q: T(`The height of a ${item.en} is $h = ${v}t - 5t^2$ metres after $t$ seconds. Find the time when it returns to the ground ($h = 0$).`, `Ketinggian sebuah ${item.ms} ialah $h = ${v}t - 5t^2$ meter selepas $t$ saat. Cari masa apabila ia kembali ke tanah ($h = 0$).`), a: T(`$5t(${root} - t) = 0$; $t = ${root}$ s (reject $t = 0$)`, `$5t(${root} - t) = 0$; $t = ${root}$ s (tolak $t = 0$)`), w: W(`$${v}t - 5t^2 = 0$`, `$5t(${root} - t) = 0$`, orT(`$t = 0$`, `$t = ${root}$`), T(`$t = 0$ is the moment it is thrown, so it lands at $t = ${root}$ s.`, `$t = 0$ ialah saat ia dilontar, jadi ia mendarat pada $t = ${root}$ s.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      const a = r.int(3, 10), gap = r.int(15, 35), b = a + gap;
      const nMid = r.int(a + 1, b - 1);
      return { q: T(`A stall makes no profit selling ${a} or ${b} ${item.en} in a day, and makes a profit for any number of sales strictly between these. Write the profit model $P$ (RM) in factorised form in terms of $n$, taking the coefficient of $n^2$ as $-1$, and find the profit when $n = ${nMid}$.`, `Sebuah gerai tidak memperoleh keuntungan apabila menjual ${a} atau ${b} ${item.ms} sehari, dan memperoleh keuntungan bagi sebarang bilangan jualan yang berada ketat antara nilai ini. Tulis model keuntungan $P$ (RM) dalam bentuk faktor dari segi $n$, dengan mengambil pekali $n^2$ sebagai $-1$, dan cari keuntungan apabila $n = ${nMid}$.`), a: T(`$P = -(n - ${a})(n - ${b})$; at $n = ${nMid}$, $P = $ RM${-(nMid - a) * (nMid - b)}`, `$P = -(n - ${a})(n - ${b})$; pada $n = ${nMid}$, $P = $ RM${-(nMid - a) * (nMid - b)}`), w: W(T(`Zero profit at $n = ${a}$ and $n = ${b}$, so the factors are $(n - ${a})$ and $(n - ${b})$.`, `Keuntungan sifar pada $n = ${a}$ dan $n = ${b}$, jadi faktornya ialah $(n - ${a})$ dan $(n - ${b})$.`), T(`Profit is positive between them, so the parabola opens downwards: $P = -(n - ${a})(n - ${b})$`, `Keuntungan positif di antaranya, jadi parabola terbuka ke bawah: $P = -(n - ${a})(n - ${b})$`), `$P = -(${nMid} - ${a})(${nMid} - ${b}) = -(${nMid - a})(${nMid - b}) = ${-(nMid - a) * (nMid - b)}$`), sp: 'm' };
    },
    ...
    (r) => {
      ...
      need(y0 >= 0 && y1 >= 0 && y2 >= 0);
      return { q: T(`True or False: for the quadratic model $y = ${q2(a, b, c)}$, equal increases in $x$ always give equal increases in $y$. Justify using $x = ${x0}, ${x0 + 1}, ${x0 + 2}$.`, `Betul atau Salah: bagi model kuadratik $y = ${q2(a, b, c)}$, pertambahan sama dalam $x$ sentiasa memberikan pertambahan sama dalam $y$. Justifikasikan menggunakan $x = ${x0}, ${x0 + 1}, ${x0 + 2}$.`), a: T(`False — the increases are ${y1 - y0} then ${y2 - y1}, which are not equal (only the second differences of a quadratic are constant, not the first).`, `Salah — pertambahannya ialah ${y1 - y0} kemudian ${y2 - y1}, yang tidak sama (hanya perbezaan kedua bagi kuadratik yang malar, bukan perbezaan pertama).`), w: W(`$x = ${x0}, ${x0 + 1}, ${x0 + 2} \\Rightarrow y = ${y0}, ${y1}, ${y2}$`, T(`Increases: $${y1} - ${y0} = ${y1 - y0}$ and $${y2} - ${y1} = ${y2 - y1}$ – not equal, so False.`, `Pertambahan: $${y1} - ${y0} = ${y1 - y0}$ dan $${y2} - ${y1} = ${y2 - y1}$ – tidak sama, jadi Salah.`), T(`They differ by $2a = ${2 * a}$ each step.`, `Bezanya ialah $2a = ${2 * a}$ bagi setiap langkah.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph shows the height of a ${item.en}, $h = ${v}t - 5t^2$. Using the graph, estimate the interval of time during which the height is at least ${thr} m.`, `Graf menunjukkan ketinggian sebuah ${item.ms}, $h = ${v}t - 5t^2$. Menggunakan graf, anggarkan selang masa apabila ketinggian sekurang-kurangnya ${thr} m.`), a: T(`Approximately $${n(round(t1, 1))} \\le t \\le ${n(round(t2, 1))}$ (read from where the curve meets $h = ${thr}$).`, `Lebih kurang $${n(round(t1, 1))} \\le t \\le ${n(round(t2, 1))}$ (dibaca dari titik lengkung bertemu $h = ${thr}$).`), w: W(T(`Draw the line $h = ${thr}$; it meets the curve at $t \\approx ${n(round(t1, 1))}$ and $t \\approx ${n(round(t2, 1))}$.`, `Lukis garis $h = ${thr}$; ia bertemu lengkung pada $t \\approx ${n(round(t1, 1))}$ dan $t \\approx ${n(round(t2, 1))}$.`), T(`Check by symmetry: the two times add up to $${root}$ (the curve is symmetric about $t = ${n(root / 2)}$).`, `Semak dengan simetri: hasil tambah dua masa itu ialah $${root}$ (lengkung simetri pada $t = ${n(root / 2)}$).`), T(`The curve is above the line between these times.`, `Lengkung berada di atas garis itu di antara dua masa ini.`)), sp: 'm', fig };
    },
  ...
  const g83a = [
    ...
    (r) => {
      ...
      return { q: T(`A ${item.en} has perimeter ${P} m.<br>${SPM.parts(parts).en}`, `Sebuah ${item.ms} mempunyai perimeter ${P} m.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(T(`(a) Width + length $= ${P} \\div 2 = ${P / 2}$, so length $= ${P / 2} - x$ and $A = x(${P / 2} - x)$`, `(a) Lebar + panjang $= ${P} \\div 2 = ${P / 2}$, jadi panjang $= ${P / 2} - x$ dan $A = x(${P / 2} - x)$`), `(b) $x(${P / 2} - x) = ${w * l} \\Rightarrow x^2 - ${P / 2}x + ${w * l} = 0$`, ...ZS(1, -w, 1, -l), T(`(c) $x = ${w}$ gives length ${l}; $x = ${l}$ gives length ${w}: the same ${w} m by ${l} m rectangle.`, `(c) $x = ${w}$ memberi panjang ${l}; $x = ${l}$ memberi panjang ${w}: segi empat tepat ${w} m kali ${l} m yang sama.`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A ${item.en} has perimeter ${P} m and width $x$ m.<br>${SPM.parts(parts).en}`, `Sebuah ${item.ms} mempunyai perimeter ${P} m dan lebar $x$ m.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(T(`(a) Width $x > 0$ and length $${half} - x > 0$, so $0 < x < ${half}$`, `(a) Lebar $x > 0$ dan panjang $${half} - x > 0$, jadi $0 < x < ${half}$`), T(`(b) Midway between the roots: $x = \\dfrac{0 + ${half}}{2} = ${xOpt}$`, `(b) Di pertengahan antara punca: $x = \\dfrac{0 + ${half}}{2} = ${xOpt}$`), `$A = ${xOpt}(${half} - ${xOpt}) = ${maxA}$`), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A stall's profit from selling $n$ ${item.en} breaks even at $n = ${a}$ and $n = ${b}$.<br>${SPM.parts(parts).en}`, `Keuntungan sebuah gerai daripada menjual $n$ ${item.ms} pulang modal pada $n = ${a}$ dan $n = ${b}$.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(`(a) $P = -(n - ${a})(n - ${b}) = -(${q2v('n', 1, -(a + b), a * b)}) = ${q2v('n', -1, a + b, -a * b)}$`, `(b) $${q2v('n', -1, a + b, -a * b)} = ${target}$`, `$${q2v('n', 1, -(a + b), a * b + target)} = 0$`, ...ZS(1, -n1, 1, -n2).map((l) => T(l.en.replace(/x/g, 'n'), l.ms.replace(/x/g, 'n'))), T(`(c) Between the roots the two factors have opposite signs, so $P > 0$ only for $${a} < n < ${b}$.`, `(c) Di antara punca, kedua-dua faktor berlainan tanda, jadi $P > 0$ hanya bagi $${a} < n < ${b}$.`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Data on ${item.en}: ${xs.map((x, i) => `(${x}, ${ys[i]})`).join(', ')}. A quadratic model and an exponential model are both proposed. Decide which family fits, using differences and ratios, then use your chosen model to predict the value at $x = ${xPred}$, stating whether this is interpolation or extrapolation.`, `Data tentang ${item.ms}: ${xs.map((x, i) => `(${x}, ${ys[i]})`).join(', ')}. Model kuadratik dan model eksponen kedua-duanya dicadangkan. Tentukan keluarga manakah yang sepadan, menggunakan perbezaan dan nisbah, kemudian gunakan model pilihan anda untuk meramal nilai pada $x = ${xPred}$, dengan menyatakan sama ada ini interpolasi atau ekstrapolasi.`), a: T(`First differences ${diffs.join(', ')} are not constant, but second differences are constant (${2 * a}), so it is quadratic, not exponential (ratios ${ratios.join(', ')} are not constant either). Predicted value at $x = ${xPred}$ is $y = ${yPred}$; this is extrapolation, since $x = ${xPred}$ is beyond the data range $[0, 3]$.`, `Perbezaan pertama ${diffs.join(', ')} tidak malar, tetapi perbezaan kedua malar (${2 * a}), jadi ia kuadratik, bukan eksponen (nisbah ${ratios.join(', ')} juga tidak malar). Nilai ramalan pada $x = ${xPred}$ ialah $y = ${yPred}$; ini ialah ekstrapolasi, kerana $x = ${xPred}$ berada di luar julat data $[0, 3]$.`), w: (() => { const d3 = diffs[2] + 2 * a, d4 = d3 + 2 * a, y4 = ys[3] + d3; return W(T(`First differences: ${diffs.join(', ')}; second differences: ${diffList(diffs).join(', ')} (constant).`, `Perbezaan pertama: ${diffs.join(', ')}; perbezaan kedua: ${diffList(diffs).join(', ')} (malar).`), T(`Ratios: ${ratios.join(', ')} – not constant, so not exponential.`, `Nisbah: ${ratios.join(', ')} – tidak malar, jadi bukan eksponen.`), T(`Continue the pattern: first differences ${d3}, ${d4}, so $y(4) = ${ys[3]} + ${d3} = ${y4}$ and $y(5) = ${y4} + ${d4} = ${y4 + d4}$.`, `Teruskan pola: perbezaan pertama ${d3}, ${d4}, jadi $y(4) = ${ys[3]} + ${d3} = ${y4}$ dan $y(5) = ${y4} + ${d4} = ${y4 + d4}$.`), T(`$x = ${xPred} > 3$ is outside the data: extrapolation.`, `$x = ${xPred} > 3$ berada di luar data: ekstrapolasi.`)); })(), sp: 'm' };
    },
    // construct your own example
    (r) => {
      const a = r.pick([1, 2]), rt1 = 0, rt2 = r.int(4, 10);
      const yEg = `${a === 1 ? '' : a}x(${rt2} - x)`;
      return { q: T(`Describe a real-life situation that could be modelled by $y = ${a}x(${rt2} - x)$ for $0 \\le x \\le ${rt2}$, stating clearly what $x$ and $y$ represent (with units) and why the domain is restricted to $0 \\le x \\le ${rt2}$.`, `Terangkan satu situasi kehidupan sebenar yang boleh dimodelkan oleh $y = ${a}x(${rt2} - x)$ bagi $0 \\le x \\le ${rt2}$, dengan menyatakan dengan jelas apa yang diwakili oleh $x$ dan $y$ (berserta unit) dan mengapa domainnya terhad kepada $0 \\le x \\le ${rt2}$.`), a: T(`Any correct context works, for example: $x$ = width (m) of a rectangular plot with perimeter ${2 * rt2} m, ${a === 1 ? '$y$ = area (m²)' : `$y$ = cost (RM) of turfing the plot at RM${a} per m²`}; the domain is restricted because both the width $x$ and the length $(${rt2} - x)$ must stay positive.`, `Sebarang konteks yang betul boleh diterima, contohnya: $x$ = lebar (m) sebidang tanah segi empat tepat berperimeter ${2 * rt2} m, ${a === 1 ? '$y$ = luas (m²)' : `$y$ = kos (RM) menanam rumput pada tanah itu dengan RM${a} bagi setiap m²`}; domain terhad kerana lebar $x$ dan panjang $(${rt2} - x)$ mestilah kekal positif.`), w: W(T(`$y = ${yEg}$ is $0$ at $x = 0$ and $x = ${rt2}$, and positive between them.`, `$y = ${yEg}$ bernilai $0$ pada $x = 0$ dan $x = ${rt2}$, dan positif di antaranya.`), T(`$x$ and $${rt2} - x$ can be the two sides of a rectangle with half-perimeter ${rt2}${a === 1 ? '' : `, and ${a} a rate per m²`}; outside $0 \\le x \\le ${rt2}$ one side would be negative.`, `$x$ dan $${rt2} - x$ boleh menjadi dua sisi segi empat tepat dengan separuh perimeter ${rt2}${a === 1 ? '' : `, dan ${a} ialah kadar bagi setiap m²`}; di luar $0 \\le x \\le ${rt2}$ satu sisi akan menjadi negatif.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      if (achievable) {
        ...
        const rt = Math.sqrt(disc);
        // the widths must come out exactly (solved by factorisation), otherwise the part cannot be answered
        need(Number.isInteger(rt));
      ...
      const partB = achievable ? [`(b) $x(${half} - x) = ${target} \\Rightarrow ${q2(1, -half, target)} = 0$`, ...ZS(1, -(half / 2 - Math.sqrt((half / 2) * (half / 2) - target)), 1, -(half / 2 + Math.sqrt((half / 2) * (half / 2) - target))), T(`Both widths lie in $0 < x < ${half}$.`, `Kedua-dua lebar berada dalam $0 < x < ${half}$.`)] : [T(`(b) $${target} > ${maxA}$, the greatest possible area, so it cannot be reached.`, `(b) $${target} > ${maxA}$, iaitu luas terbesar yang mungkin, jadi ia tidak boleh dicapai.`)];
      const ansParts = [T(`Maximum area $= ${maxA}$ m², at $x = ${half / 2}$ m.`, `Luas maksimum $= ${maxA}$ m², pada $x = ${half / 2}$ m.`), secondAns];
      return { q: T(`A ${item.en} has perimeter ${P} m and width $x$ m, so $A = x(${half} - x)$.<br>${SPM.parts(parts).en}`, `Sebuah ${item.ms} mempunyai perimeter ${P} m dan lebar $x$ m, jadi $A = x(${half} - x)$.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(T(`(a) Midway between the roots: $x = \\dfrac{0 + ${half}}{2} = ${half / 2}$, $A = ${half / 2}(${half / 2}) = ${maxA}$`, `(a) Di pertengahan antara punca: $x = \\dfrac{0 + ${half}}{2} = ${half / 2}$, $A = ${half / 2}(${half / 2}) = ${maxA}$`), ...partB), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The height of a ${item.en} thrown upwards is $h = ${v}t - 5t^2$ metres after $t$ seconds.<br>${SPM.parts(parts).en}`, `Ketinggian sebuah ${item.ms} yang dilontar ke atas ialah $h = ${v}t - 5t^2$ meter selepas $t$ saat.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(`(a) $${v}t - 5t^2 = 5t(${root} - t) = 0$`, orT(`$t = 0$`, `$t = ${root}$`), T(`(b) Midway between the roots: $t = \\dfrac{0 + ${root}}{2} = ${tMax}$`, `(b) Di pertengahan antara punca: $t = \\dfrac{0 + ${root}}{2} = ${tMax}$`), projSub(v, tMax), T(`(c) It is in the air between the roots: $0 \\le t \\le ${root}$`, `(c) Ia berada di udara di antara punca: $0 \\le t \\le ${root}$`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A ${item.en} has perimeter ${P} m and width $x$ m, so $A = x(${half} - x)$.<br>${SPM.parts(parts).en}`, `Sebuah ${item.ms} mempunyai perimeter ${P} m dan lebar $x$ m, jadi $A = x(${half} - x)$.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(`(a) $A = ${xin}(${half} - ${xin}) = ${xin}(${half - xin}) = ${xin * (half - xin)}$`, T(`(b) Length $= ${half} - ${xout} = ${half - xout} < 0$: no rectangle has a negative side, so $x = ${xout}$ is outside the domain $0 < x < ${half}$.`, `(b) Panjang $= ${half} - ${xout} = ${half - xout} < 0$: tiada segi empat tepat dengan sisi negatif, jadi $x = ${xout}$ di luar domain $0 < x < ${half}$.`)), sp: 'l' };
    },
    // justify choosing a quadratic model from a qualitative description (no table)
    (r) => {
      const item = r.pick(PROFIT_ITEM);
      return { q: T(`A stall owner says: "As I sell more ${item.en}, my profit rises at first, but if I try to sell far more than my usual stock (by discounting heavily), my profit falls again." Explain why a quadratic model is more appropriate here than a linear model, and sketch what a table of second differences would show if the data were collected.`, `Pemilik gerai berkata: "Apabila saya menjual lebih banyak ${item.ms}, keuntungan saya meningkat pada mulanya, tetapi jika saya cuba menjual jauh lebih banyak daripada stok biasa (dengan memberi diskaun besar-besaran), keuntungan saya jatuh semula." Terangkan mengapa model kuadratik lebih sesuai di sini berbanding model linear, dan lakarkan apa yang akan ditunjukkan oleh jadual perbezaan kedua jika data itu dikumpul.`), a: T(`A linear model can only keep rising or keep falling, but here profit rises then falls, which a quadratic (one turning point) can capture; if data were collected at equal intervals, the second differences of profit would be constant and negative (profit curves downward like an upside-down parabola).`, `Model linear hanya boleh terus meningkat atau terus menurun, tetapi di sini keuntungan meningkat kemudian menurun, yang boleh digambarkan oleh model kuadratik (satu titik pusingan); jika data dikumpul pada selang sama, perbezaan kedua keuntungan akan malar dan negatif (keuntungan melengkung ke bawah seperti parabola terbalik).`), w: W(T('A linear model has a constant rate, so it only rises or only falls: it has no turning point.', 'Model linear mempunyai kadar malar, jadi ia hanya meningkat atau hanya menurun: tiada titik pusingan.'), T('Profit rises then falls, so it has a maximum point: $P = an^2 + bn + c$ with $a < 0$.', 'Keuntungan meningkat kemudian menurun, jadi ia mempunyai titik maksimum: $P = an^2 + bn + c$ dengan $a < 0$.'), T('Second differences $= 2a$: constant and negative.', 'Perbezaan kedua $= 2a$: malar dan negatif.')), sp: 'm' };
    },
    ...
    (r) => {
      ...
      const tMax = root / 2, hMax = v * tMax - 5 * tMax * tMax;
      const shorter = measuredRoot < root;
      const why = shorter
        ? T(`Air resistance (ignored by the model) would slow the ${item.en} and shorten its flight`, `Rintangan udara (diabaikan oleh model) akan memperlahankan ${item.ms} dan memendekkan penerbangannya`)
        : T(`The model assumes it is launched from ground level at exactly ${v} m/s; if it was released from above the ground (e.g. from a hand) or a little faster, it stays in the air longer`, `Model itu mengandaikan ia dilancarkan dari aras tanah pada tepat ${v} m/s; jika ia dilepaskan dari atas tanah (contohnya dari tangan) atau sedikit lebih laju, ia berada di udara lebih lama`);
      const ansParts = [T(`$t = ${tMax}$ s; greatest height $= ${hMax}$ m.`, `$t = ${tMax}$ s; ketinggian terbesar $= ${hMax}$ m.`), T(`${why.en}; the model ${shorter ? 'over-estimated' : 'under-estimated'} the time of flight ($${root}$ s predicted vs $${measuredRoot}$ s actual).`, `${why.ms}; model itu ${shorter ? 'melebih anggaran' : 'mengurang anggaran'} masa penerbangan ($${root}$ s diramal berbanding $${measuredRoot}$ s sebenar).`)];
      return { q: T(`A model for the height of a ${item.en} is $h = ${v}t - 5t^2$ metres after $t$ seconds.<br>${SPM.parts(parts).en}`, `Suatu model bagi ketinggian sebuah ${item.ms} ialah $h = ${v}t - 5t^2$ meter selepas $t$ saat.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(T(`(a) Midway between the roots: $t = \\dfrac{0 + ${root}}{2} = ${n(tMax)}$`, `(a) Di pertengahan antara punca: $t = \\dfrac{0 + ${root}}{2} = ${n(tMax)}$`), projSub(v, tMax), T(`(b) Predicted ${root} s, actual ${measuredRoot} s: the model ${shorter ? 'over-estimated' : 'under-estimated'} the flight time by ${Math.abs(root - measuredRoot)} s.`, `(b) Ramalan ${root} s, sebenar ${measuredRoot} s: model itu ${shorter ? 'melebih anggaran' : 'mengurang anggaran'} masa penerbangan sebanyak ${Math.abs(root - measuredRoot)} s.`)), sp: 'l' };
    },
  ];
  ...
  /** "$y = a(b)^{t} = value$" (rounded to dp, "\approx" when rounded) */
  const expSub = (a0, b, t, dp) => { const v = a0 * Math.pow(b, t); return `$y = ${a0}(${n(b)})^{${t}} ${eqR(v, dp)} ${n(round(v, dp))}$`; };
  /** "$b = 1 ± rate/100 = b$" */
  const bLine = (rate, up) => `$b = 1 ${up ? '+' : '-'} \\dfrac{${rate}}{100} = ${n(up ? 1 + rate / 100 : 1 - rate / 100)}$`;
  /** trial table x = 1..t for y = a0 b^x */
  const trialW = (a0, b, t) => Array.from({ length: t }, (_, i) => `$x = ${i + 1}$: ${expSub(a0, b, i + 1, 1)}`);
  ...
  const g84e = [
    ...
    (r) => {
      ...
      const b = bPct(rate, s);
      const t = r.int(2, 4);
      return { q: T(`For ${s.d.en}, the model is $y = ${a0}(${n(b)})^x$, starting at ${a0} and changing by ${rate}% each period. Find the value when $x = ${t}$.`, `Bagi ${s.d.ms}, modelnya ialah $y = ${a0}(${n(b)})^x$, bermula pada ${a0} dan berubah sebanyak ${rate}% setiap tempoh. Cari nilainya apabila $x = ${t}$.`), a: T(`${n(round(a0 * Math.pow(b, t), 2))}`), w: W(bLine(rate, s.dir === 1), expSub(a0, b, t, 2)), sp: 's' };
    },
    ...
    (r) => {
      ...
      const t = r.int(2, 4);
      return { q: T(`For ${s.d.en}, the model is $y = ${a0}(${b})^x$. Find the value when $x = ${t}$.`, `Bagi ${s.d.ms}, modelnya ialah $y = ${a0}(${b})^x$. Cari nilainya apabila $x = ${t}$.`), a: T(`${a0 * Math.pow(b, t)}`), w: W(`$y = ${a0}(${b})^{${t}} = ${a0} \\times ${Math.pow(b, t)} = ${a0 * Math.pow(b, t)}$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      const b = bPct(rate, s);
      return { q: T(`For ${s.d.en}, the starting value is ${a0} and it ${s.dir === 1 ? 'increases' : 'decreases'} by ${rate}% each period. Write the model $y = ab^x$.`, `Bagi ${s.d.ms}, nilai permulaan ialah ${a0} dan ia ${s.dir === 1 ? 'bertambah' : 'berkurang'} sebanyak ${rate}% setiap tempoh. Tulis model $y = ab^x$.`), a: T(`$y = ${a0}(${n(b)})^x$`), w: W(T(`Starting value: $a = ${a0}$`, `Nilai permulaan: $a = ${a0}$`), bLine(rate, s.dir === 1), `$y = ${a0}(${n(b)})^x$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The table shows values for ${s.d.en} that follow an exponential model.<br>${tab}<br>State the values of $a$ and $b$ in the model $y = ab^x$.`, `Jadual menunjukkan nilai bagi ${s.d.ms} yang mengikut model eksponen.<br>${tab}<br>Nyatakan nilai $a$ dan $b$ dalam model $y = ab^x$.`), a: T(`$a = ${a0}$, $b = ${b}$`), w: W(T(`At $x = 0$: $y = ab^0 = a$, so $a = ${a0}$.`, `Pada $x = 0$: $y = ab^0 = a$, jadi $a = ${a0}$.`), `$b = \\dfrac{${a0 * b}}{${a0}} = ${b}$`), sp: 's' };
    },
    ...
    (r) => {
      ...
      const c = LET[opts.indexOf(correct)];
      const line = (lang) => opts.map((o, k) => `${LET[k]}) ${o[lang]}`).join('<br>');
      return { q: T(`For the model $y = a(${n(b)})^x$ with $a > 0$, what happens to $y$ as $x$ increases?<br>${line('en')}`, `Bagi model $y = a(${n(b)})^x$ dengan $a > 0$, apakah yang berlaku kepada $y$ apabila $x$ bertambah?<br>${line('ms')}`), a: T(`${c}) ${correct.en}`, `${c}) ${correct.ms}`), w: W(T(`$b = ${n(b)} ${growth ? '> 1' : '< 1'}$: each step multiplies $y$ by ${n(b)}, so $y$ ${growth ? 'keeps increasing, faster and faster' : 'keeps shrinking but stays positive'}.`, `$b = ${n(b)} ${growth ? '> 1' : '< 1'}$: setiap langkah mendarab $y$ dengan ${n(b)}, jadi $y$ ${growth ? 'terus bertambah dengan semakin cepat' : 'terus mengecil tetapi kekal positif'}.`)), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      return { q: T(`The graph shows an exponential model $y = ${a0}(${b})^x$ for ${s.d.en}. Read off the value of $y$ when $x = ${xm}$.`, `Graf menunjukkan model eksponen $y = ${a0}(${b})^x$ bagi ${s.d.ms}. Baca nilai $y$ apabila $x = ${xm}$.`), a: T(`$y = ${a0 * Math.pow(b, xm)}$`), w: W(`$y = ${a0}(${b})^{${xm}} = ${a0 * Math.pow(b, xm)}$`, T('This agrees with the plotted point.', 'Ini sepadan dengan titik yang diplot.')), sp: 'xs', fig };
    },
    ...
    (r) => {
      ...
      return { q: T(`True or False: in the model $y = a(${n(b)})^x$ with $a > 0$, the value is ${claimGrowth ? 'increasing' : 'decreasing'} as $x$ increases.`, `Betul atau Salah: dalam model $y = a(${n(b)})^x$ dengan $a > 0$, nilainya ${claimGrowth ? 'bertambah' : 'berkurang'} apabila $x$ bertambah.`), a: T(`${isTrue ? 'True' : 'False'} — since $b ${b > 1 ? '> 1' : '< 1'}$, the model represents ${b > 1 ? 'growth (increasing)' : 'decay (decreasing)'}.`, `${isTrue ? 'Betul' : 'Salah'} — kerana $b ${b > 1 ? '> 1' : '< 1'}$, model ini mewakili ${b > 1 ? 'pertumbuhan (bertambah)' : 'penyusutan (berkurang)'}.`), w: W(T(`$b = ${n(b)} ${b > 1 ? '> 1' : '< 1'}$, so $y$ is ${b > 1 ? 'increasing' : 'decreasing'}; the statement says ${claimGrowth ? 'increasing' : 'decreasing'}, so it is ${isTrue ? 'true' : 'false'}.`, `$b = ${n(b)} ${b > 1 ? '> 1' : '< 1'}$, jadi $y$ ${b > 1 ? 'bertambah' : 'berkurang'}; pernyataan itu menyebut ${claimGrowth ? 'bertambah' : 'berkurang'}, jadi ia ${isTrue ? 'betul' : 'salah'}.`)), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      const rate = r.pick([5, 10, 15, 20]);
      const b = bPct(rate, s);
      return { q: T(`For ${s.d.en}, the model is $y = ${a0}(${n(b)})^x$. State the value when $x = 0$, and explain what this represents.`, `Bagi ${s.d.ms}, modelnya ialah $y = ${a0}(${n(b)})^x$. Nyatakan nilai apabila $x = 0$, dan terangkan apa yang diwakilinya.`), a: T(`$y = ${a0}$ when $x = 0$; this represents the starting/initial value.`, `$y = ${a0}$ apabila $x = 0$; ini mewakili nilai permulaan/awal.`), w: W(`$y = ${a0}(${n(b)})^0 = ${a0}(1) = ${a0}$`, T('Any non-zero number to the power 0 is 1, so $a$ is the starting value.', 'Sebarang nombor bukan sifar kuasa 0 ialah 1, jadi $a$ ialah nilai permulaan.')), sp: 's' };
    },
  ];
  ...
  const g84m = [
    ...
    (r) => {
      ...
      return { q: T(`For ${s.d.en}, the model is $y = ${a0}(${n(b)})^x$. By trying values of $x$, find the smallest whole number of periods for the value to at least double.`, `Bagi ${s.d.ms}, modelnya ialah $y = ${a0}(${n(b)})^x$. Dengan mencuba nilai $x$, cari bilangan tempoh bulat terkecil supaya nilainya sekurang-kurangnya berganda dua.`), a: T(`$x = ${t}$ (value $\\approx ${n(round(v, 1))}$)`, `$x = ${t}$ (nilai $\\approx ${n(round(v, 1))}$)`), w: W(T(`Target: $2 \\times ${a0} = ${target}$`, `Sasaran: $2 \\times ${a0} = ${target}$`), ...trialW(a0, b, t), T(`The first value $\\ge ${target}$ is at $x = ${t}$.`, `Nilai pertama $\\ge ${target}$ ialah pada $x = ${t}$.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Model 1 (for ${sg.d.en}) is $y = ${a0}(${n(1 + rg / 100)})^x$; Model 2 (for ${sd.d.en}) is $y = ${a0}(${n(1 - rd / 100)})^x$. Which gives the larger value at $x = ${t}$, and by how much?`, `Model 1 (bagi ${sg.d.ms}) ialah $y = ${a0}(${n(1 + rg / 100)})^x$; Model 2 (bagi ${sd.d.ms}) ialah $y = ${a0}(${n(1 - rd / 100)})^x$. Model manakah memberikan nilai yang lebih besar pada $x = ${t}$, dan berapakah bezanya?`), a: T(`Model 1: ${n(vg)}, Model 2: ${n(vd)}; Model 1 is larger by ${n(round(vg - vd, 1))}.`, `Model 1: ${n(vg)}, Model 2: ${n(vd)}; Model 1 lebih besar sebanyak ${n(round(vg - vd, 1))}.`), w: W(T(`Model 1: ${expSub(a0, 1 + rg / 100, t, 1)}`, `Model 1: ${expSub(a0, 1 + rg / 100, t, 1)}`), T(`Model 2: ${expSub(a0, 1 - rd / 100, t, 1)}`, `Model 2: ${expSub(a0, 1 - rd / 100, t, 1)}`), T(`Difference $= ${n(vg)} - ${n(vd)} = ${n(round(vg - vd, 1))}$`, `Beza $= ${n(vg)} - ${n(vd)} = ${n(round(vg - vd, 1))}$`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      const bb = a0 * b * b;
      return { q: T(`True or False: for ${s.d.en} changing by ${rate}% each period, the value after 2 periods can be found by applying ${2 * rate}% once to the starting value ${a0}. Justify your answer.`, `Betul atau Salah: bagi ${s.d.ms} yang berubah sebanyak ${rate}% setiap tempoh, nilai selepas 2 tempoh boleh dicari dengan mengenakan ${2 * rate}% sekali sahaja kepada nilai permulaan ${a0}. Justifikasikan jawapan anda.`), a: T(`False — the correct value is $${a0}(${n(b)})^2 ${eqR(bb, 1)} ${n(correctVal)}$, not $${n(wrongVal)}$; percentage change compounds (multiplies) each period, it does not simply add up.`, `Salah — nilai yang betul ialah $${a0}(${n(b)})^2 ${eqR(bb, 1)} ${n(correctVal)}$, bukan $${n(wrongVal)}$; perubahan peratus berkompaun (mendarab) setiap tempoh, ia tidak semudah dijumlahkan.`), w: W(`$${a0}(${n(b)})^2 = ${a0} \\times ${n(b * b)} ${eqR(bb, 1)} ${n(correctVal)}$`, T(`Applying ${2 * rate}% once: $${a0} \\times ${n(1 + (s.dir === 1 ? 2 * rate : -2 * rate) / 100)} = ${n(wrongVal)}$`, `Mengenakan ${2 * rate}% sekali: $${a0} \\times ${n(1 + (s.dir === 1 ? 2 * rate : -2 * rate) / 100)} = ${n(wrongVal)}$`), T(`The second ${rate}% is taken on the new value, not on ${a0}, so the results differ: False.`, `${rate}% yang kedua dikira atas nilai baharu, bukan atas ${a0}, jadi hasilnya berbeza: Salah.`)), sp: 'm' };
    },
    // explain the risk of long-range extrapolation (reuses the 8.1 reasoning bank)
    (r) => {
      const s = r.pick(EXPCTX);
      const t = r.int(2, 5);
      return { q: T(`A model for ${s.d.en} is built from data over the first ${t} periods. Explain why using this exponential model to predict very far into the future is risky.`, `Suatu model bagi ${s.d.ms} dibina daripada data sepanjang ${t} tempoh pertama. Terangkan mengapa menggunakan model eksponen ini untuk meramal jauh ke hadapan adalah berisiko.`), a: LIMIT_REASON.exponential, w: W(T('An exponential model multiplies by the same factor every period, so it changes faster and faster without any limit.', 'Model eksponen mendarab dengan faktor yang sama setiap tempoh, jadi ia berubah semakin cepat tanpa sebarang had.'), T(`It was fitted to only ${t} periods; predicting far beyond them (extrapolation) assumes the rate never changes.`, `Ia dipadankan dengan ${t} tempoh sahaja; meramal jauh melebihinya (ekstrapolasi) menganggap kadarnya tidak pernah berubah.`)), sp: 's' };
    },
  ...
  const g84a = [
    ...
    (r) => {
      ...
      return { q: T(`For ${s.d.en}, the model is $y = ${a0}(${n(b)})^x$.<br>${SPM.parts(parts).en}`, `Bagi ${s.d.ms}, modelnya ialah $y = ${a0}(${n(b)})^x$.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(T(`(a) Target: $2 \\times ${a0} = ${target}$`, `(a) Sasaran: $2 \\times ${a0} = ${target}$`), ...trialW(a0, b, t), T(`The first value $\\ge ${target}$ is at $x = ${t}$.`, `Nilai pertama $\\ge ${target}$ ialah pada $x = ${t}$.`), T(`(b) The model keeps the same ${rate}% rate forever; real growth usually slows down.`, `(b) Model itu mengekalkan kadar ${rate}% yang sama selama-lamanya; pertumbuhan sebenar biasanya menjadi perlahan.`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`RM${p0} is deposited in a savings account earning ${rate}% interest per year.<br>${SPM.parts(parts).en}`, `RM${p0} disimpan dalam sebuah akaun simpanan yang memperoleh faedah ${rate}% setahun.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(`(a) ${expSub(p0, 1 + rate / 100, t, 2)}`, `(b) $y = ${p0} + ${n(p0 * rate / 100)}(${t}) = ${n(round(simple, 2))}$`, T(`Difference $= ${n(compound)} - ${n(round(simple, 2))} = ${n(round(compound - simple, 2))}$`, `Beza $= ${n(compound)} - ${n(round(simple, 2))} = ${n(round(compound - simple, 2))}$`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`For ${s.d.en}:<br>${SPM.parts(parts).en}`, `Bagi ${s.d.ms}:<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(`(a) $b = \\dfrac{${n(a1)}}{${a0}} = ${n(b)}$, $a = ${a0}$: $y = ${a0}(${n(b)})^x$`, `(b) ${expSub(a0, b, tPred, 1)}`, T(`(c) The model assumes the same percentage change in every period: ${s.as.en}.`, `(c) Model itu menganggap perubahan peratus yang sama dalam setiap tempoh: ${s.as.ms}.`)), sp: 'l' };
    },
  ];
  ...
  /** what each report section does (for worked solutions) */
  const SEC_ROLE = [
    T('states the question being investigated', 'menyatakan persoalan yang disiasat'),
    T('names what is taken as fixed and the quantities that change', 'menamakan perkara yang dianggap tetap dan kuantiti yang berubah'),
    T('gives the equation and why that family fits the data', 'memberi persamaan dan sebab keluarga itu sesuai dengan data'),
    T('shows the substitution and working that produce the numbers', 'menunjukkan penggantian dan kerja yang menghasilkan nombor'),
    T('explains what the numbers mean in context, with units', 'menerangkan maksud nombor dalam konteks, dengan unit'),
    T('answers the question and says where the model may fail', 'menjawab persoalan dan menyatakan bila model mungkin gagal'),
  ];
  const roleW = (k) => T(`The "${REPLAB[k].en}" section ${SEC_ROLE[k].en}.`, `Bahagian "${REPLAB[k].ms}" ${SEC_ROLE[k].ms}.`);
  ...
  const g85e = [
    ...
    (r) => {
      ...
      return { q: T(`A short modelling report on ${s.d.en} reads:<br>${lines('en')}<br>Which section states the ${REPLAB[ask].en.toLowerCase()}?`, `Sebuah laporan pemodelan ringkas tentang ${s.d.ms} berbunyi:<br>${lines('ms')}<br>Bahagian manakah yang menyatakan ${REPLAB[ask].ms.toLowerCase()}?`), a: REPLAB[ask], w: W(roleW(ask), T(`Here: "${repSent(s, ask).en}"`, `Di sini: "${repSent(s, ask).ms}"`)), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Match each sentence (1-${idx.length}) of this modelling report on ${s.d.en} to its section.<br>${dLine('en')}<br>${oLine('en')}`, `Padankan setiap ayat (1-${idx.length}) laporan pemodelan tentang ${s.d.ms} ini dengan bahagiannya.<br>${dLine('ms')}<br>${oLine('ms')}`), a: T(ans.map((c, i) => `${i + 1}-${c}`).join(', ')), w: W(...shown.map((k, i) => T(`${i + 1}: this sentence ${SEC_ROLE[k].en} → "${REPLAB[k].en}" (${ans[i]})`, `${i + 1}: ayat ini ${SEC_ROLE[k].ms} → "${REPLAB[k].ms}" (${ans[i]})`))), sp: 's' };
    },
    // list N of the 6 components (N varies)
    (r) => {
      const nAsk = r.int(3, 6);
      return { q: T(`List ${nAsk === 6 ? 'the six' : nAsk} component${nAsk > 1 ? 's' : ''} that a good mathematical modelling report should contain.`, `Senaraikan ${nAsk === 6 ? 'keenam-enam' : nAsk} komponen yang perlu ada dalam laporan pemodelan matematik yang baik.`), a: T(REPLAB.slice(0, nAsk).map((x) => x.en).join('; '), REPLAB.slice(0, nAsk).map((x) => x.ms).join('; ')), w: W(T(`A report follows the modelling cycle: ${REPLAB.map((x) => x.en).join(' → ')}.`, `Laporan mengikut kitaran pemodelan: ${REPLAB.map((x) => x.ms).join(' → ')}.`), ...(nAsk < 6 ? [T(`Any ${nAsk} of these are acceptable.`, `Mana-mana ${nAsk} daripadanya boleh diterima.`)] : [])), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`In a modelling report on ${s.d.en}, which section would contain the sentence: "${repSent(s, k).en}"?<br>${line('en')}`, `Dalam laporan pemodelan tentang ${s.d.ms}, bahagian manakah yang mengandungi ayat: "${repSent(s, k).ms}"?<br>${line('ms')}`), a: T(`${c}) ${REPLAB[k].en}`, `${c}) ${REPLAB[k].ms}`), w: W(T(`The sentence ${SEC_ROLE[k].en}, so it belongs to "${REPLAB[k].en}".`, `Ayat itu ${SEC_ROLE[k].ms}, jadi ia tergolong dalam "${REPLAB[k].ms}".`)), sp: 'xs' };
    },
  ];
  ...
  const g85m = [
    ...
    (r) => {
      ...
      need(actual > 0);
      return { q: T(`A report on ${s.d.en} predicted ${pred} but the actual observed value was ${actual}. Write one sentence for the report's conclusion that explains this discrepancy, referring to the assumption "${s.as.en}".`, `Laporan tentang ${s.d.ms} meramalkan ${pred} tetapi nilai sebenar yang diperhatikan ialah ${actual}. Tulis satu ayat bagi kesimpulan laporan itu yang menerangkan percanggahan ini, dengan merujuk kepada andaian "${s.as.ms}".`), a: T(`The discrepancy may arise because the assumption "${s.as.en}" does not hold exactly in practice, so the model's prediction differs somewhat from the actual value.`, `Percanggahan ini mungkin timbul kerana andaian "${s.as.ms}" tidak berlaku dengan tepat dalam praktiknya, jadi ramalan model itu berbeza sedikit daripada nilai sebenar.`), w: W(T(`Difference $= ${actual} - ${pred} = ${actual - pred}$, about ${n(round((Math.abs(dev) / pred) * 100, 1))}% of the prediction.`, `Beza $= ${actual} - ${pred} = ${actual - pred}$, lebih kurang ${n(round((Math.abs(dev) / pred) * 100, 1))}% daripada ramalan.`), T('A prediction is only as good as the assumptions behind it; if an assumption is only roughly true, the prediction is only roughly right.', 'Ramalan hanya sebaik andaian di sebaliknya; jika sesuatu andaian hanya lebih kurang betul, ramalan itu juga hanya lebih kurang tepat.')), sp: 'm' };
    },
  ...
  const g85a = [
    ...
    (r) => {
      const s = r.pick(SCEN);
      return { q: T(`Write a short report (3-4 sentences) on modelling ${s.d.en}. Your report must state: the model family used and why, one assumption, and a conclusion that includes one limitation.`, `Tulis satu laporan ringkas (3-4 ayat) tentang pemodelan ${s.d.ms}. Laporan anda mesti menyatakan: keluarga model yang digunakan dan sebabnya, satu andaian, dan kesimpulan yang merangkumi satu had.`), a: T(`Acceptable answers combine: model = ${FAM_NAME[s.fam].en} ($${FAM_EQ[s.fam]}$) because ${FAM_TEST[s.fam].en}; assumption = ${s.as.en}; limitation = ${LIMIT_REASON[s.fam].en}`, `Jawapan yang boleh diterima menggabungkan: model = ${FAM_NAME[s.fam].ms} ($${FAM_EQ[s.fam]}$) kerana ${FAM_TEST[s.fam].ms}; andaian = ${s.as.ms}; had = ${LIMIT_REASON[s.fam].ms}`), w: W(T(`Model: ${FAM_NAME[s.fam].en}, because ${FAM_WHY[s.fam].en}.`, `Model: ${FAM_NAME[s.fam].ms}, kerana ${FAM_WHY[s.fam].ms}.`), T(`Assumption: ${s.as.en}.`, `Andaian: ${s.as.ms}.`), T(`Conclusion with a limitation: ${LIMIT_REASON[s.fam].en}.`, `Kesimpulan dengan had: ${LIMIT_REASON[s.fam].ms}.`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`A report on ${s.d.en} has two issues.<br>${SPM.parts(parts).en}`, `Laporan tentang ${s.d.ms} mempunyai dua isu.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(T(`(a) "${val}" of what? Every quantity in a report needs its unit (e.g. RM, m, people).`, `(a) "${val}" apa? Setiap kuantiti dalam laporan memerlukan unitnya (contohnya RM, m, orang).`), T('(b) A model fitted to limited data cannot prove anything "always"; the conclusion must state its range and limitations.', '(b) Model yang dipadankan dengan data terhad tidak boleh membuktikan sesuatu "sentiasa" berlaku; kesimpulan mesti menyatakan julat dan hadnya.')), sp: 'l' };
    },
  ...
  /** worked lines for the open box V = x(s - 2x)^2 and the closed cylinder S = 2πr² + 2K/r (π ≈ 3.142) */
  const boxW = (s, x) => `$V = ${n(x)}(${s} - 2(${n(x)}))^2 = ${n(x)}(${n(s - 2 * x)})^2 = ${n(x * (s - 2 * x) * (s - 2 * x))}$`;
  const cylW = (K, r) => `$S = 2(3.142)(${r})^2 + \\dfrac{${2 * K}}{${r}} \\approx ${n(round(2 * 3.142 * r * r, 3))} + ${n(round((2 * K) / r, 2))} \\approx ${n(round(2 * 3.142 * r * r + (2 * K) / r, 1))}$`;
  const objW = (a, b, [x, y]) => `$(${x}, ${y})$: $P = ${a}(${x}) + ${b}(${y}) = ${a * x + b * y}$`;
  const JOINTCTX = [
  ...
  const g86e = [
    ...
    (r) => {
      ...
      return { q: T(`A square sheet of ${mat.en} of side ${s} cm has a square of side $x$ cm cut from each corner, and the sides folded up to form an open box of volume $V = x(${s} - 2x)^2$. Find $V$ when $x = ${x}$. ${TAG_MENS.en}`, `Sekeping kepingan ${mat.ms} berbentuk segi empat sama bersisi ${s} cm mempunyai segi empat sama bersisi $x$ cm dipotong dari setiap sudut, dan sisinya dilipat ke atas untuk membentuk sebuah kotak terbuka berisi padu $V = x(${s} - 2x)^2$. Cari $V$ apabila $x = ${x}$. ${TAG_MENS.ms}`), a: T(`${V} cm³`), w: W(boxW(s, x), `${V} cm³`), sp: 's' };
    },
    ...
    (r) => {
      ...
      return { q: T(`${SPM.cap(can.en)}, a closed cylinder, must hold a fixed volume of ${K} cm³; its radius is $r$ cm and its surface area is $S = 2\\pi r^2 + \\dfrac{2(${K})}{r}$. Using $\\pi \\approx 3.142$, find $S$ when $r = ${rr}$.`, `${SPM.cap(can.ms)}, sebuah silinder tertutup, mesti memuatkan isi padu tetap ${K} cm³; jejarinya ialah $r$ cm dan luas permukaannya ialah $S = 2\\pi r^2 + \\dfrac{2(${K})}{r}$. Dengan menggunakan $\\pi \\approx 3.142$, cari $S$ apabila $r = ${rr}$.`), a: T(`$S \\approx ${n(S)}$ cm²`), w: W(cylW(K, rr)), sp: 's' };
    },
    ...
    (r) => {
      ...
      if (isBox) {
        ...
        need(V1 !== V2);
        line = { en: `Two candidate cuts are tried for an open box (side ${s} cm sheet of ${mat.en}, volume $V = x(${s} - 2x)^2$): $x = ${x1}$ cm and $x = ${x2}$ cm. Which gives the greater volume?`, ms: `Dua potongan calon dicuba bagi sebuah kotak terbuka (kepingan ${mat.ms} bersisi ${s} cm, isi padu $V = x(${s} - 2x)^2$): $x = ${x1}$ cm dan $x = ${x2}$ cm. Yang manakah memberikan isi padu lebih besar?`, aEn: `$x = ${x1}$: $V = ${V1}$ cm³; $x = ${x2}$: $V = ${V2}$ cm³; $x = ${V1 > V2 ? x1 : x2}$ gives the greater volume.`, aMs: `$x = ${x1}$: $V = ${V1}$ cm³; $x = ${x2}$: $V = ${V2}$ cm³; $x = ${V1 > V2 ? x1 : x2}$ memberikan isi padu lebih besar.`, w: W(boxW(s, x1), boxW(s, x2), T(`$${Math.max(V1, V2)} > ${Math.min(V1, V2)}$, so $x = ${V1 > V2 ? x1 : x2}$ cm.`, `$${Math.max(V1, V2)} > ${Math.min(V1, V2)}$, jadi $x = ${V1 > V2 ? x1 : x2}$ cm.`)) };
      } else {
        const K = r.pick([100, 200, 300]);
        const [r1, r2] = r.sample([2, 3, 4, 5], 2);
        const S1 = round(2 * 3.142 * r1 * r1 + (2 * K) / r1, 1), S2 = round(2 * 3.142 * r2 * r2 + (2 * K) / r2, 1);
        need(S1 !== S2);
        line = { en: `Two candidate radii are tried for a closed cylinder of fixed volume ${K} cm³ ($S = 2\\pi r^2 + \\dfrac{2(${K})}{r}$): $r = ${r1}$ cm and $r = ${r2}$ cm. Which gives the smaller surface area?`, ms: `Dua jejari calon dicuba bagi sebuah silinder tertutup berisi padu tetap ${K} cm³ ($S = 2\\pi r^2 + \\dfrac{2(${K})}{r}$): $r = ${r1}$ cm dan $r = ${r2}$ cm. Yang manakah memberikan luas permukaan lebih kecil?`, aEn: `$r = ${r1}$: $S \\approx ${n(S1)}$ cm²; $r = ${r2}$: $S \\approx ${n(S2)}$ cm²; $r = ${S1 < S2 ? r1 : r2}$ gives the smaller area.`, aMs: `$r = ${r1}$: $S \\approx ${n(S1)}$ cm²; $r = ${r2}$: $S \\approx ${n(S2)}$ cm²; $r = ${S1 < S2 ? r1 : r2}$ memberikan luas lebih kecil.`, w: W(cylW(K, r1), cylW(K, r2), T(`$${n(Math.min(S1, S2))} < ${n(Math.max(S1, S2))}$, so $r = ${S1 < S2 ? r1 : r2}$ cm.`, `$${n(Math.min(S1, S2))} < ${n(Math.max(S1, S2))}$, jadi $r = ${S1 < S2 ? r1 : r2}$ cm.`)) };
      }
      return { q: T(line.en, line.ms), a: T(line.aEn, line.aMs), w: line.w, sp: 's' };
    },
    ...
    (r) => {
      ...
      const c = LET[opts.indexOf(correct)];
      const line = (lang) => opts.map((o, k) => `${LET[k]}) ${o[lang]}`).join('<br>');
      return { q: T(`For $y = k${direct ? 'x' : '/x'}$ with $k > 0$ and $x > 0$, what happens to $y$ as $x$ increases? ${TAG_VAR.en}<br>${line('en')}`, `Bagi $y = k${direct ? 'x' : '/x'}$ dengan $k > 0$ dan $x > 0$, apakah yang berlaku kepada $y$ apabila $x$ bertambah? ${TAG_VAR.ms}<br>${line('ms')}`), a: T(`${c}) ${correct.en}`, `${c}) ${correct.ms}`), w: W(direct ? T('$y = kx$: doubling $x$ doubles $y$, so $y$ increases.', '$y = kx$: menggandakan $x$ menggandakan $y$, jadi $y$ bertambah.') : T('$y = \\dfrac{k}{x}$: doubling $x$ halves $y$, so $y$ decreases.', '$y = \\dfrac{k}{x}$: menggandakan $x$ menjadikan $y$ separuh, jadi $y$ berkurang.')), sp: 'xs' };
    },
    ...
    (r) => {
      ...
      if (isBox) {
        ...
        q = T(`For an open box made from ${mat.en} of side ${s} cm ($V = x(${s}-2x)^2$), the table shows tested volumes.<br>${tab}<br>Which tested value of $x$ gives the greatest volume?`, `Bagi sebuah kotak terbuka daripada ${mat.ms} bersisi ${s} cm ($V = x(${s}-2x)^2$), jadual menunjukkan isi padu yang diuji.<br>${tab}<br>Nilai $x$ yang diuji manakah memberikan isi padu terbesar?`);
        a = T(`$x = ${cands[vols.indexOf(best)]}$ (volume ${best} cm³)`, `$x = ${cands[vols.indexOf(best)]}$ (isi padu ${best} cm³)`);
        w = W(T(`Compare the volumes ${vols.join(', ')}: the largest is ${best}, at $x = ${cands[vols.indexOf(best)]}$.`, `Bandingkan isi padu ${vols.join(', ')}: yang terbesar ialah ${best}, pada $x = ${cands[vols.indexOf(best)]}$.`));
      } else {
        const can = r.pick(CYLCTX);
        const K = r.pick([100, 200, 300]);
        const cands = [2, 3, 4];
  ...
  const g86m = [
    ...
    (r) => {
      ...
      return { q: T(`A square sheet of ${mat.en} of side ${s} cm is folded into an open box of volume $V = x(${s} - 2x)^2$, where $x$ cm is cut from each corner. Complete the table for $x = ${cands.join(', ')}$, and state the greatest volume among these tested values.<br>${tab}<br>${TAG_MENS.en}`, `Sekeping kepingan ${mat.ms} berbentuk segi empat sama bersisi ${s} cm dilipat menjadi kotak terbuka berisi padu $V = x(${s} - 2x)^2$, dengan $x$ cm dipotong dari setiap sudut. Lengkapkan jadual bagi $x = ${cands.join(', ')}$, dan nyatakan isi padu terbesar antara nilai yang diuji ini.<br>${tab}<br>${TAG_MENS.ms}`), a: T(`Volumes: ${vols.join(', ')} cm³; the greatest tested volume is ${best} cm³, at $x = ${bestX}$ (best among the tested values, not a proven maximum).`, `Isi padu: ${vols.join(', ')} cm³; isi padu terbesar yang diuji ialah ${best} cm³, pada $x = ${bestX}$ (terbaik antara nilai yang diuji, bukan maksimum yang terbukti).`), w: W(...cands.map((x) => boxW(s, x)), T(`Greatest tested volume: ${best} cm³ at $x = ${bestX}$.`, `Isi padu terbesar yang diuji: ${best} cm³ pada $x = ${bestX}$.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      need(vals.filter((v) => v === maxV).length === 1);
      const bestV = verts[vals.indexOf(maxV)];
      return { q: T(`A feasible region has vertices at $(${verts[0].join(', ')})$, $(${verts[1].join(', ')})$ and $(${verts[2].join(', ')})$. The objective function for ${obj.en} is $P = ${a}x + ${b}y$. Find $P$ at each vertex, and state which vertex gives the greatest value. ${TAG_INEQ.en}`, `Suatu rantau tersaur mempunyai bucu pada $(${verts[0].join(', ')})$, $(${verts[1].join(', ')})$ dan $(${verts[2].join(', ')})$. Fungsi objektif bagi ${obj.ms} ialah $P = ${a}x + ${b}y$. Cari $P$ pada setiap bucu, dan nyatakan bucu manakah memberikan nilai terbesar. ${TAG_INEQ.ms}`), a: T(`$P$-values: ${vals.join(', ')}; greatest at $(${bestV.join(', ')})$, $P = ${maxV}$.`, `Nilai $P$: ${vals.join(', ')}; terbesar pada $(${bestV.join(', ')})$, $P = ${maxV}$.`), w: W(...verts.map((v) => objW(a, b, v)), T(`The greatest value is ${maxV}, at $(${bestV.join(', ')})$.`, `Nilai terbesar ialah ${maxV}, pada $(${bestV.join(', ')})$.`)), sp: 'm' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Three routes between two towns are proposed: ${line('en')}. Which route is shortest?`, `Tiga laluan antara dua bandar dicadangkan: ${line('ms')}. Laluan manakah paling pendek?`), a: T(`Route ${best} (${Math.min(...totals)} km).`, `Laluan ${best} (${Math.min(...totals)} km).`), w: W(T(`Compare ${totals.join(', ')} km: the smallest is ${Math.min(...totals)} km, Route ${best}.`, `Bandingkan ${totals.join(', ')} km: yang terkecil ialah ${Math.min(...totals)} km, Laluan ${best}.`)), sp: 'xs' };
    },
  ];
  ...
  const g86a = [
    ...
    (r) => {
      ...
      const best = Math.max(...vols);
      const bestX = cands[vols.indexOf(best)];
      const parts = [T(`Write the volume model $V(x)$, and state the domain of $x$ for it to be physically meaningful.`, `Tulis model isi padu $V(x)$, dan nyatakan domain $x$ supaya ia bermakna secara fizikal.`), T(`Complete a table of $V$ for $x = ${cands.join(', ')}$, and state which tested value gives the greatest volume.`, `Lengkapkan jadual $V$ bagi $x = ${cands.join(', ')}$, dan nyatakan nilai yang diuji manakah memberikan isi padu terbesar.`), T(`Explain why this is only the best value among those tested, not a proven overall maximum, and suggest how the search could be refined.`, `Terangkan mengapa ini hanya nilai terbaik antara yang diuji, bukan maksimum keseluruhan yang terbukti, dan cadangkan bagaimana carian itu boleh diperhalusi.`)];
      const ansParts = [T(`$V(x) = x(${s} - 2x)^2$; domain $0 < x < ${s / 2}$.`, `$V(x) = x(${s} - 2x)^2$; domain $0 < x < ${s / 2}$.`), T(`Volumes: ${vols.join(', ')} cm³; greatest tested at $x = ${bestX}$, $V = ${best}$ cm³.`, `Isi padu: ${vols.join(', ')} cm³; terbesar yang diuji pada $x = ${bestX}$, $V = ${best}$ cm³.`), T(`Only whole-number values of $x$ were tested, so a value between these (e.g. using steps of 0.5 cm) might give a larger volume; without calculus, only a finer step-by-step search (not an exact proof) can improve on this.`, `Hanya nilai $x$ bernombor bulat diuji, jadi suatu nilai antaranya (contohnya menggunakan langkah 0.5 cm) mungkin memberikan isi padu yang lebih besar; tanpa kalkulus, hanya carian langkah demi langkah yang lebih halus (bukan bukti tepat) dapat menambah baik ini.`)];
      return { q: T(`A square sheet of ${mat.en} of side ${s} cm is used to build an open box by cutting a square of side $x$ cm from each corner and folding up the sides. ${TAG_MENS.en}<br>${SPM.parts(parts).en}`, `Sekeping kepingan ${mat.ms} berbentuk segi empat sama bersisi ${s} cm digunakan untuk membina kotak terbuka dengan memotong segi empat sama bersisi $x$ cm dari setiap sudut dan melipat ke atas sisi-sisinya. ${TAG_MENS.ms}<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(T(`(a) $x > 0$ and the base side $${s} - 2x > 0$, so $0 < x < ${s / 2}$`, `(a) $x > 0$ dan sisi tapak $${s} - 2x > 0$, jadi $0 < x < ${s / 2}$`), ...cands.map((x) => `(b) ${boxW(s, x)}`), T(`Greatest tested: $x = ${bestX}$, $V = ${best}$`, `Terbesar yang diuji: $x = ${bestX}$, $V = ${best}$`), T(`(c) Refine with half-steps near $x = ${bestX}$: ${[bestX - 0.5, bestX + 0.5].filter((x) => x > 0 && x < s / 2).map((x) => boxW(s, x)).join(', ')}`, `(c) Perhalusi dengan langkah separuh berhampiran $x = ${bestX}$: ${[bestX - 0.5, bestX + 0.5].filter((x) => x > 0 && x < s / 2).map((x) => boxW(s, x)).join(', ')}`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`${SPM.cap(can.en)}, a closed cylinder, must hold a fixed volume of ${K} cm³.<br>${SPM.parts(parts).en}`, `${SPM.cap(can.ms)}, sebuah silinder tertutup, mesti memuatkan isi padu tetap ${K} cm³.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(`(a) $\\pi r^2 h = ${K} \\Rightarrow h = \\dfrac{${K}}{\\pi r^2}$`, `$S = 2\\pi r^2 + 2\\pi r h = 2\\pi r^2 + 2\\pi r \\left(\\dfrac{${K}}{\\pi r^2}\\right) = 2\\pi r^2 + \\dfrac{2(${K})}{r}$`, ...cands.map((rr) => `(b) ${cylW(K, rr)}`), T(`Smallest tested: $r = ${bestR}$ cm, $S \\approx ${n(best)}$ cm²`, `Terkecil yang diuji: $r = ${bestR}$ cm, $S \\approx ${n(best)}$ cm²`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Weekly sales of ${item.en} at $x = ${xs.join(', ')}$ (weeks) were actually ${actuals.join(', ')}. Model A predicted ${predsA.join(', ')}; Model B predicted ${predsB.join(', ')}.<br>${SPM.parts(parts).en}`, `Jualan mingguan ${item.ms} pada $x = ${xs.join(', ')}$ (minggu) sebenarnya ialah ${actuals.join(', ')}. Model A meramalkan ${predsA.join(', ')}; Model B meramalkan ${predsB.join(', ')}.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(`(a) A: $${actuals.map((v, i) => Math.abs(v - predsA[i])).join(' + ')} = ${resA}$`, `B: $${actuals.map((v, i) => Math.abs(v - predsB[i])).join(' + ')} = ${resB}$`, T(`(b) $${Math.min(resA, resB)} < ${Math.max(resA, resB)}$, so Model ${resA < resB ? 'A' : 'B'} fits these ${xs.length} points better – but only these points were checked.`, `(b) $${Math.min(resA, resB)} < ${Math.max(resA, resB)}$, jadi Model ${resA < resB ? 'A' : 'B'} lebih sepadan dengan ${xs.length} titik ini – tetapi hanya titik ini yang disemak.`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`Route A from ${p.en} to ${q.en} is ${distA} km with no toll. Route B is ${distB} km.<br>${SPM.parts(parts).en}`, `Laluan A dari ${p.ms} ke ${q.ms} ialah ${distA} km tanpa tol. Laluan B ialah ${distB} km.<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(`(a) A: $${distA} \\times ${rate} = ${n(round(distA * rate, 2))}$; B: $${distB} \\times ${rate} = ${n(round(distB * rate, 2))}$`, `(b) A: $${n(costA)}$; B: $${n(round(distB * rate, 2))} + ${tollB} = ${n(costB)}$`, T(`$${n(Math.min(costA, costB))} < ${n(Math.max(costA, costB))}$, so Route ${costA < costB ? 'A' : 'B'} is cheaper.`, `$${n(Math.min(costA, costB))} < ${n(Math.max(costA, costB))}$, jadi Laluan ${costA < costB ? 'A' : 'B'} lebih murah.`)), sp: 'l' };
    },
    ...
    (r) => {
      ...
      return { q: T(`${TAG_VAR.en}<br>${SPM.parts(parts).en}`, `${TAG_VAR.ms}<br>${SPM.parts(parts).ms}`), a: SPM.parts(ansParts), w: W(direct ? `(a) $k = \\dfrac{y}{x} = \\dfrac{${y1}}{${x1}} = ${k}$, $y = ${k}x$` : `(a) $k = xy = ${x1} \\times ${y1} = ${k}$, $y = \\dfrac{${k}}{x}$`, direct ? `(b) $y = ${k}(${x2}) = ${y2}$` : `(b) $y = \\dfrac{${k}}{${x2}} = ${y2}$`), sp: 'l' };
    },
  ];
