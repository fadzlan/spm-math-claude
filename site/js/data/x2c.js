...
(function () {
  ...
  const TF41 = [
    ['All the sides of a regular polygon are equal, and all its angles are equal.', 'Semua sisi sebuah poligon sekata adalah sama panjang dan semua sudutnya sama besar.', true, 'This is the definition of a regular polygon.', 'Ini ialah takrifan poligon sekata.'],
    ['A rhombus is a regular polygon.', 'Sebuah rombus ialah poligon sekata.', false, 'Its sides are equal but its angles are not all equal (unless it is a square).', 'Sisinya sama panjang tetapi sudutnya tidak semua sama (kecuali segi empat sama).'],
    ['A rectangle is a regular polygon.', 'Sebuah segi empat tepat ialah poligon sekata.', false, 'Its angles are equal but its sides are not all equal (unless it is a square).', 'Sudutnya sama tetapi sisinya tidak semua sama (kecuali segi empat sama).'],
    ['The exterior angle of a regular polygon becomes smaller when the number of sides increases.', 'Sudut peluaran poligon sekata menjadi lebih kecil apabila bilangan sisi bertambah.', true, 'Exterior angle $= 360^\\circ \\div n$, which decreases as $n$ increases.', 'Sudut peluaran $= 360^\\circ \\div n$, yang berkurang apabila $n$ bertambah.'],
    ['Each interior angle of a regular polygon can be $180^\\circ$.', 'Setiap sudut pedalaman poligon sekata boleh bersaiz $180^\\circ$.', false, 'The exterior angle would be $0^\\circ$, but it is $360^\\circ \\div n > 0^\\circ$.', 'Sudut peluaran akan menjadi $0^\\circ$, tetapi ia ialah $360^\\circ \\div n > 0^\\circ$.'],
    ['A regular polygon with $n$ sides has $n$ axes of symmetry.', 'Poligon sekata dengan $n$ sisi mempunyai $n$ paksi simetri.', true, 'Each axis passes through the centre, and there are $n$ of them.', 'Setiap paksi melalui pusat, dan terdapat $n$ paksi.'],
    ['Each interior angle of a regular hexagon is $60^\\circ$.', 'Setiap sudut pedalaman heksagon sekata ialah $60^\\circ$.', false, '$60^\\circ$ is the exterior angle; the interior angle is $180^\\circ - 60^\\circ = 120^\\circ$.', '$60^\\circ$ ialah sudut peluaran; sudut pedalaman ialah $180^\\circ - 60^\\circ = 120^\\circ$.'],
    ['At any vertex of a regular polygon, the interior angle and the exterior angle add up to $180^\\circ$.', 'Pada mana-mana bucu poligon sekata, sudut pedalaman dan sudut peluaran berjumlah $180^\\circ$.', true, 'They lie on a straight line.', 'Kedua-duanya terletak pada satu garis lurus.'],
    ['A polygon whose sides are all equal must be a regular polygon.', 'Poligon yang semua sisinya sama panjang mestilah poligon sekata.', false, 'A rhombus has four equal sides but its angles are not all equal.', 'Rombus mempunyai empat sisi sama panjang tetapi sudutnya tidak semua sama.'],
    ['A regular polygon has an exterior angle of $45^\\circ$ if it has 8 sides.', 'Poligon sekata mempunyai sudut peluaran $45^\\circ$ jika ia mempunyai 8 sisi.', true, '$360^\\circ \\div 8 = 45^\\circ$.', '$360^\\circ \\div 8 = 45^\\circ$.'],
    ['The sum of the exterior angles of a regular pentagon is smaller than that of a regular decagon.', 'Hasil tambah sudut peluaran pentagon sekata lebih kecil daripada dekagon sekata.', false, 'Both sums equal $360^\\circ$.', 'Kedua-dua hasil tambah ialah $360^\\circ$.'],
    ['An equilateral triangle is a regular polygon.', 'Segi tiga sama sisi ialah poligon sekata.', true, 'Its three sides are equal and its three angles are each $60^\\circ$.', 'Ketiga-tiga sisinya sama panjang dan setiap sudutnya $60^\\circ$.'],
  ];
  ...
  const g41e = [
    (r) => {
      ...
      return { q, a: v === 2 ? T(`$${k}$ ; $${D(e)}$`) : T(`$${D(e)}$`), w: T(`$360^\\circ \\div ${k}$`), sp: 's' };
    },
    (r) => {
      ...
      return { q, a: T(`$${D(itr(k))}$`), w: T(`$180^\\circ - ${D(e)}$`), sp: 's' };
    },
    (r) => {
      ...
      return { q, a: v === 1 ? T(`${k} sides; ${pn(k)[0]}`, `${k} sisi; ${pn(k)[1]} sekata`) : T(`${k}`), sp: v === 2 ? 'm' : 's' };
    },
    (r) => {
      const k = r.pick([5, 6, 8, 9, 10, 12]), v = r.int(0, 2);
      return v === 0 ? { q: T(`How many sides has a ${pn(k)[0]}?`, `Berapakah bilangan sisi bagi ${pn(k)[1]}?`), a: T(`${k}`), sp: 'xs' }
        : v === 1 ? { q: T(`What is the name of a polygon with ${k} sides? How many vertices does it have?`, `Apakah nama poligon yang mempunyai ${k} sisi? Berapakah bilangan bucunya?`), a: T(`${cap(pn(k)[0])}; ${k} vertices`, `${cap(pn(k)[1])}; ${k} bucu`), sp: 's' }
        : { q: T(`A regular ${pn(k)[0]} has $n$ sides. State the value of $n$ and the number of exterior angles (one at each vertex).`, `${cap(pn(k)[1])} sekata mempunyai $n$ sisi. Nyatakan nilai $n$ dan bilangan sudut peluaran (satu pada setiap bucu).`), a: T(`$n = ${k}$; ${k} exterior angles`, `$n = ${k}$; ${k} sudut peluaran`), sp: 's' };
    },
    (r) => {
      const [se, sm, tv, re, rm2] = r.pick(TF41);
      return { q: T(`True or false? "${se}" Give a reason.`, `Benar atau palsu? "${sm}" Berikan satu sebab.`), a: T(`${tv ? 'True' : 'False'}. ${re}`, `${tv ? 'Benar' : 'Palsu'}. ${rm2}`), sp: 's' };
    },
    (r) => {
      const k = r.pick([5, 6, 8, 9, 10, 12]), i = r.int(1, k - 1);
      return { q: nts(T(`The diagram shows a regular polygon with one side extended. Find $x$.`, `Rajah menunjukkan sebuah poligon sekata dengan satu sisi dipanjangkan. Cari $x$.`)), fig: extFig(k, i, 'x'), a: T(`$x = ${D(ext(k))}$`), w: T(`$360^\\circ \\div ${k}$ (${k} sides, counted from the diagram)`), sp: 's' };
    },
    (r) => {
      const k = r.pick([5, 6, 8, 9, 10, 12]), i = r.int(0, k - 1);
      return { q: T(`The diagram shows a regular ${pn(k)[0]}. Find the value of $x$.`, `Rajah menunjukkan ${pn(k)[1]} sekata. Cari nilai $x$.`), fig: polyFig(regPts(k), { angles: { [i]: 'x' } }), a: T(`$x = ${D(itr(k))}$`), w: T(`$180^\\circ - 360^\\circ \\div ${k}$`), sp: 's' };
    },
    (r) => {
      ...
      return { q: T(`The table gives the angles of some regular polygons. Fill in each "?".<br>${SPM.table(rows, { head })}`, `Jadual memberi sudut beberapa poligon sekata. Isikan setiap "?".<br>${SPM.table(rows, { head: headM })}`), a: T(asW.join(' ; ')), sp: 'm' };
    },
    (r) => {
      const k = r.pick([5, 6, 8, 9, 10, 12]), [a, am] = rna(k), m = mcq(r, ext(k), [itr(k), 180 / k, 360 / (k + 1), 360 - itr(k), ext(k) * 2], (v) => `$${D(v)}$`);
      return { q: T(`What is the size of each exterior angle of ${a}?${m.text}`, `Berapakah saiz setiap sudut peluaran bagi ${am}?${m.text}`), a: T(m.ans), sp: 's' };
    },
    (r) => {
      const k = r.pick(NC), e = ext(k), v = r.int(0, 2);
      return [
        { q: T(`At a vertex of a regular polygon the exterior angle is $${D(e)}$. Find the interior angle at that vertex.`, `Pada satu bucu poligon sekata, sudut peluaran ialah $${D(e)}$. Cari sudut pedalaman pada bucu itu.`), a: T(`$${D(itr(k))}$`), w: T(`$180^\\circ - ${D(e)}$`) },
        { q: T(`At a vertex of a regular polygon the interior angle is $${D(itr(k))}$. Find the exterior angle at that vertex.`, `Pada satu bucu poligon sekata, sudut pedalaman ialah $${D(itr(k))}$. Cari sudut peluaran pada bucu itu.`), a: T(`$${D(e)}$`), w: T(`$180^\\circ - ${D(itr(k))}$`) },
        { q: T(`Complete: interior angle $+$ exterior angle $= \\underline{\\qquad}$. Hence find the interior angle when the exterior angle is $${D(e)}$.`, `Lengkapkan: sudut pedalaman $+$ sudut peluaran $= \\underline{\\qquad}$. Seterusnya cari sudut pedalaman apabila sudut peluaran ialah $${D(e)}$.`), a: T(`$180^\\circ$ ; $${D(itr(k))}$`) },
    ...
    (r) => {
      ...
      return { q: T(`Each side of ${a} is ${s} cm long. Find its perimeter.`, `Setiap sisi ${am} panjangnya ${s} cm. Cari perimeternya.`), a: T(`${k * s} cm`), w: T(`$${k} \\times ${s}$`), sp: 's' };
    },
    (r) => {
      ...
      return { q: T(`Arrange these regular polygons in ${asc ? 'ascending' : 'descending'} order of the size of one interior angle: ${list(0)}.`, `Susun poligon sekata ini mengikut tertib ${asc ? 'menaik' : 'menurun'} saiz satu sudut pedalaman: ${list(1)}.`), a: T(ans(0), ans(1)), sp: 's' };
    },
    (r) => {
  ...
  const g41m = [
    ...
    (r) => {
      ...
      return { q: T(`${who[0]} says that ${W[0]}. Find the mistake and give the correct answer.`, `${who[0]} berkata ${W[1]}. Cari kesilapan itu dan berikan jawapan yang betul.`), a: T(W[2], W[3]), sp: 's' };
    },
    (r) => {
      ...
      return { q: src, a: ok ? T(`Yes. $n = 360 \\div ${n(e)} = ${360 / e}$ sides.`, `Ya. $n = 360 \\div ${n(e)} = ${360 / e}$ sisi.`) : T(`No. $360 \\div ${n(e)} = ${n(round(360 / e, 2))}\\ldots$ is not a whole number, so the number of sides would not be an integer.`, `Tidak. $360 \\div ${n(e)} = ${n(round(360 / e, 2))}\\ldots$ bukan nombor bulat, maka bilangan sisi bukan integer.`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`Complete the table for three regular polygons.<br>${SPM.table(rows, { head: ['$n$', 'Exterior angle', 'Interior angle'] })}`, `Lengkapkan jadual bagi tiga poligon sekata.<br>${SPM.table(rows, { head: ['$n$', 'Sudut peluaran', 'Sudut pedalaman'] })}`), a: T(ans), sp: 'm' };
    },
    (r) => {
      const k = r.pick([5, 6, 8, 9, 10, 12, 15, 18, 20]), I = itr(k), m = mcq(r, k, [k + 2, k - 1, Math.round(I / 10) * 1 + 1, 2 * k, k + 4], (v) => `${v}`);
      return { q: T(`Each interior angle of a regular polygon is $${D(I)}$. How many sides does it have?${m.text}`, `Setiap sudut pedalaman poligon sekata ialah $${D(I)}$. Berapakah bilangan sisinya?${m.text}`), a: T(m.ans), w: T(`Exterior angle $= ${D(ext(k))}$, so $n = 360 \\div ${n(ext(k))}$`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      const k = r.pick([5, 6, 8, 9, 10, 12]), i = r.int(1, k - 1);
      return { q: nts(T(`The diagram shows part of a regular polygon. One side is extended and the exterior angle is $${D(ext(k))}$. How many sides does the polygon have?`, `Rajah menunjukkan sebahagian poligon sekata. Satu sisi dipanjangkan dan sudut peluarannya ialah $${D(ext(k))}$. Berapakah bilangan sisi poligon itu?`)), fig: extFig(k, i, `${n(ext(k))}°`, true), a: T(`${k}`), w: T(`$360 \\div ${n(ext(k))}$`), sp: 's' };
    },
    (r) => {
      const [ce, cm, k] = r.pick(CTX41), s = r.int(20, 60), v = r.int(0, 1);
      return { q: T(`${ce} Each side is ${s} cm long. Find (a) the interior angle, (b) the perimeter.`, `${cm} Setiap sisinya panjangnya ${s} cm. Cari (a) sudut pedalaman, (b) perimeter.`), a: T(`(a) $${D(itr(k))}$ (b) ${k * s} cm`, `(a) $${D(itr(k))}$ (b) ${k * s} cm`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(q[0], q[1]), a: T(q[2]), sp: 's' };
    },
    (r) => {
  ...
  const g41a = [
    ...
    (r) => {
      ...
      need(b >= 1 && b <= 30 && Math.abs(d) >= 1 && Math.abs(d) <= 60 && a1 !== c);
      const dd = d > 0 ? `+ ${d}` : `- ${-d}`;
    ...
    (r) => {
      ...
      return { q: T(M[0] + txt, M[1] + txt), a: T(`${'ABCD'[idx]}. ${fo(M[2][0])}`), sp: 's' };
    },
    (r) => {
      ...
      return { q: T(W[0], W[1]), a: T(W[2], W[3]), sp: 'm' };
    },
    (r) => {
      const [k1, k2] = r.pick([[5, 4], [6, 4], [6, 5], [8, 4], [5, 5], [6, 6], [8, 6], [10, 5], [12, 4], [8, 8], [10, 6], [9, 5]]);
      const x = 360 - itr(k1) - itr(k2), v = r.int(0, 1);
  ...
  const g41be = [
    ...
    (r) => {
      const [se, sm, tv, re, rm2] = r.pick([
        ...
        ['The centre angle of a regular polygon is the same as its interior angle.', 'Sudut pusat poligon sekata sama dengan sudut pedalamannya.', false, 'The centre angle is $360^\\circ \\div n$, which equals the exterior angle, not the interior angle.', 'Sudut pusat ialah $360^\\circ \\div n$, sama dengan sudut peluaran, bukan sudut pedalaman.'],
        ['Paper folding a round piece of paper in half three times gives eight equal sectors.', 'Melipat kertas bulat separuh sebanyak tiga kali menghasilkan lapan sektor yang sama.', true, '$2 \\times 2 \\times 2 = 8$ sectors of $45^\\circ$ each, which give the vertices of a regular octagon.', '$2 \\times 2 \\times 2 = 8$ sektor, masing-masing $45^\\circ$, yang memberi bucu oktagon sekata.'],
      ]);
      return { q: T(`True or false? "${se}" Give a reason.`, `Benar atau palsu? "${sm}" Berikan satu sebab.`), a: T(`${tv ? 'True' : 'False'}. ${re}`, `${tv ? 'Benar' : 'Palsu'}. ${rm2}`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`Fill in the blank: ${blank[0]}`, `Isi tempat kosong: ${blank[1]}`), a: T(blank[2]), sp: 'xs' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(Q[0], Q[1]), a: T(Q[2]), sp: 'xs' };
    },
  ];
  ...
  const g41bm = [
    ...
    (r) => {
      ...
      return { q: T(`Complete the missing step ${pos + 1} in the construction of ${d[0]}.<br>${li.map((x) => x[0]).join('<br>')}`, `Lengkapkan langkah ${pos + 1} yang tertinggal dalam pembinaan ${d[1]}.<br>${li.map((x) => x[1]).join('<br>')}`), a: T(st[pos][0], st[pos][1]), sp: 'm' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(rot[0], rot[1]), a: T(rot[2], rot[3]), sp: v === 3 ? 'm' : 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(Q[0], Q[1]), a: T(Q[2], Q[3]), sp: 'm' };
    },
    (r) => {
  ...
  const g42e = [
    ...
    (r) => {
      ...
      return { q: T(`Complete the table.<br>${SPM.table(rows2, { head: ['Number of sides', 'Sum of interior angles'] })}`, `Lengkapkan jadual.<br>${SPM.table(rows2, { head: ['Bilangan sisi', 'Hasil tambah sudut pedalaman'] })}`), a: T(ks.map((k, i) => (i % 2 ? `$${D(ssum(k))}$` : `${k}`)).join(' ; ')), sp: 'm' };
    },
    (r) => {
      const [tf] = [r.pick([
    ...
    (r) => {
      ...
      return { q: nts(T(`The diagram shows a ${pn(k)[0]}. Find the value of $x$.`, `Rajah menunjukkan sebuah ${pn(k)[1]}. Cari nilai $x$.`)), fig: irrFig(r, angs, labels), a: T(`$x = ${D(angs[i])}$`), w: T(`$${ssum(k)} - (${known.join(' + ')})$`), sp: 's' };
    },
    (r) => {
      ...
      return { q: T(`A ${pn(k)[0]} has ${k - 1} exterior angles of ${ok.map((a) => a + '^\\circ').map((a) => `$${a}$`).join(', ')}. Find the ${k}th exterior angle.`, `Sebuah ${pn(k)[1]} mempunyai ${k - 1} sudut peluaran ${ok.map((a) => `$${a}^\\circ$`).join(', ')}. Cari sudut peluaran ke-${k}.`), a: T(`$${D(x)}$`), w: T(`$360^\\circ - (${ok.join(' + ')})^\\circ$`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: nts(T(`The diagram shows a ${pn(k)[0]} with all its exterior angles marked except $x$. Find $x$.`, `Rajah menunjukkan sebuah ${pn(k)[1]} dengan semua sudut peluarannya ditanda kecuali $x$. Cari $x$.`)), fig: irrExtFig(r, angs, labels), a: T(`$x = ${D(ext2[i])}$`), sp: 's' };
    },
    (r) => {
  ...
  const g42m = [
    (r) => {
      ...
      return { q: nts(T(`The diagram shows a ${pn(k)[0]} in which two of the angles are both equal to $x$. Find $x$.`, `Rajah menunjukkan sebuah ${pn(k)[1]} yang dua sudutnya sama dengan $x$. Cari $x$.`)), fig: irrFig(r, all, labels), a: T(`$x = ${D(x)}$`), w: T(`$2x = ${ssum(k)} - (${a2.join(' + ')}) = ${2 * x}$`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`${c1} Its interior angles are ${known.map((a) => `$${a}^\\circ$`).join(', ')} and one more angle. Find the missing angle.`, `${c2} Sudut pedalamannya ialah ${known.map((a) => `$${a}^\\circ$`).join(', ')} dan satu lagi sudut. Cari sudut yang tidak diketahui.`), a: T(`$${D(angs[0])}$`), w: T(`$(${k} - 2) \\times 180^\\circ - (${known.join(' + ')})^\\circ$`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`The interior angles of a ${pn(k)[0]} are $${terms.map((t) => `(${t})^\\circ`).join(', ')}$. Find $x$ and the largest angle.`, `Sudut pedalaman sebuah ${pn(k)[1]} ialah $${terms.map((t) => `(${t})^\\circ`).join(', ')}$. Cari $x$ dan sudut terbesar.`), a: T(`$x = ${x}$ ; $${D(x + d * (k - 1))}$`), w: T(`$${k}x + ${d * (k * (k - 1)) / 2} = ${ssum(k)}$`), sp: 'm' };
    },
    (r) => {
    ...
    (r) => {
      ...
      const [a, am] = pna(k);
      return { q: v === 0 ? T(`What is the sum of the interior angles of ${a}?${txt}`, `Berapakah hasil tambah sudut pedalaman ${am}?${txt}`) : T(`The interior angles of a polygon with ${k} sides add up to which of the following?${txt}`, `Sudut pedalaman sebuah poligon dengan ${k} sisi berjumlah yang manakah berikut?${txt}`), a: T(`${'ABCD'[idx]}. $${D(s)}$`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`The interior angles of ${nm} are $${shown.join(', ')}$. Find $x$ and the largest interior angle.`, `Sudut pedalaman ${nmm} ialah $${shown.join(', ')}$. Cari $x$ dan sudut pedalaman terbesar.`), a: T(`$x = ${xv}$ ; $${D(Math.max(...use.map((f) => f[1] * xv + f[2])))}$`), w: T(`${cs}x${cn ? ' + ' + cn : ''} = ${ssum(k)}`.replace(/^/, '$') + '$'), sp: 'm' };
    },
    (r) => {
      ...
      if (isPar) {
        ...
        need(A > 30 && A < 150);
        return { q: T(`In the parallelogram $ABCD$, $\\angle A = (${p}x + ${q})^\\circ$ and $\\angle B = ${D(B)}$. Find $x$.`, `Dalam segi empat selari $ABCD$, $\\angle A = (${p}x + ${q})^\\circ$ dan $\\angle B = ${D(B)}$. Cari $x$.`), a: T(`$x = ${x}$`), w: T(`$\\angle A + \\angle B = 180^\\circ$`), sp: 's' };
      }
  ...
  const g42a = [
    ...
    (r) => {
      ...
      return { q: T(`Which statement is correct?<br>${o.map((x, i) => `${'ABCD'[i]}. ${x[0]}`).join('<br>')}`, `Pernyataan manakah yang betul?<br>${o.map((x, i) => `${'ABCD'[i]}. ${x[1]}`).join('<br>')}`), a: T(`${'ABCD'[idx]}. ${good[0]}`, `${'ABCD'[idx]}. ${good[1]}`), sp: 's' };
    },
  ];
  /*END42*/
  ...
  const g51e = [
    ...
    (r) => {
      const t = r.int(0, 6);
      const Q = [
        ['A line segment joining two points on a circle is called a ________.', 'Tembereng garis yang menyambung dua titik pada bulatan dipanggil ________.', 'chord', 'perentas'],
        ['A chord that passes through the centre of a circle is called a ________.', 'Perentas yang melalui pusat bulatan dipanggil ________.', 'diameter', 'diameter'],
        ['The distance from the centre of a circle to any point on the circle is the ________.', 'Jarak dari pusat bulatan ke mana-mana titik pada bulatan ialah ________.', 'radius', 'jejari'],
        ['The distance round a circle is called its ________.', 'Jarak mengelilingi sebuah bulatan dipanggil ________.', 'circumference', 'lilitan'],
        ['A part of the circumference of a circle is called an ________.', 'Sebahagian daripada lilitan bulatan dipanggil ________.', 'arc', 'lengkok'],
        ['The region bounded by two radii and an arc is called a ________.', 'Kawasan yang dibatasi oleh dua jejari dan satu lengkok dipanggil ________.', 'sector', 'sektor'],
        ['The region bounded by a chord and an arc is called a ________.', 'Kawasan yang dibatasi oleh satu perentas dan satu lengkok dipanggil ________.', 'segment', 'tembereng'],
      ][t];
      return { q: T(`Fill in the blank: ${Q[0]}`, `Isi tempat kosong: ${Q[1]}`), a: T(Q[2], Q[3]), sp: 'xs' };
    },
    (r) => {
      const [se, sm, tv, re, rm2] = r.pick([
      ...
      return { q: T(`True or false? "${se}" Explain.`, `Benar atau palsu? "${sm}" Terangkan.`), a: T(`${tv ? 'True' : 'False'}. ${re}`, `${tv ? 'Benar' : 'Palsu'}. ${rm2}`), sp: 's' };
    },
    (r) => {
      const c = r.pick(CIRC_CTX), d = 2 * r.int(c[3], c[4]), v = r.int(0, 2);
      return { q: [T(`To draw a circle for ${c[0]} with a diameter of ${d} ${c[2]}, to what length should the compasses be set?`, `Untuk melukis bulatan bagi ${c[1]} dengan diameter ${d} ${c[2]}, kepada panjang berapakah jangka lukis perlu ditetapkan?`), T(`A circle with diameter ${d} ${c[2]} is to be drawn with compasses. What radius should be set?`, `Sebuah bulatan dengan diameter ${d} ${c[2]} hendak dilukis dengan jangka lukis. Apakah jejari yang perlu ditetapkan?`), T(`Aina sets her compasses to ${d / 2} ${c[2]} to draw a circle. What is the diameter of her circle?`, `Aina menetapkan jangka lukisnya pada ${d / 2} ${c[2]} untuk melukis bulatan. Berapakah diameter bulatannya?`)][v], a: T(v === 2 ? `${d} ${c[2]}` : `${d / 2} ${c[2]}`), sp: 'xs' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(Q[0], Q[1]), a: T(Q[2], Q[3]), sp: 's' };
    },
  ];
  const CSTEP = {
  ...
  const g51m = [
    (r) => {
      ...
      return { q: [T(`The steps for constructing ${CNAME[key][0]} are in the wrong order. Write the letters in the correct order.${sh.en}`, `Langkah untuk membina ${CNAME[key][1]} disusun dalam urutan yang salah. Tulis huruf mengikut urutan yang betul.${sh.ms}`), T(`Arrange the steps to construct ${CNAME[key][0]}.${sh.en}`, `Susun langkah untuk membina ${CNAME[key][1]}.${sh.ms}`), T(`Put these instructions for ${CNAME[key][0]} in order.${sh.en}`, `Susun arahan ini bagi ${CNAME[key][1]} mengikut turutan.${sh.ms}`)][v], a: T(sh.order), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`Complete the table of radii and diameters of some circles (all in cm).<br>${SPM.table(cells, { head: ['Radius $r$', 'Diameter $d$'] })}`, `Lengkapkan jadual jejari dan diameter beberapa bulatan (semuanya dalam cm).<br>${SPM.table(cells, { head: ['Jejari $r$', 'Diameter $d$'] })}`), a: T(asw.join(' ; ')), sp: 'm' };
    },
    (r) => {
      const v = r.int(0, 3);
      const Q = [
  ...
  const g52e = [
    ...
    (r) => {
      ...
      return { q, a: T(task === 0 ? `${d} cm (equal chords are equidistant from the centre)` : `${2 * h} cm (chords equidistant from the centre are equal)`, task === 0 ? `${d} cm (perentas sama panjang sama jarak dari pusat)` : `${2 * h} cm (perentas sama jarak dari pusat adalah sama panjang)`), sp: 's' };
    },
  ];
  const LSTEP = {
  ...
  const g52m = [
    ...
    (r) => {
      ...
      return { q: qs[st], a: task === 0 ? T(`${2 * R} cm`) : T(`No. The longest is ${2 * R} cm (the diameter, through the centre).`, `Tidak. Yang terpanjang ialah ${2 * R} cm (diameter, melalui pusat).`), w: task === 0 ? T(`$R^2 = ${h}^2 + ${d}^2$; diameter $= 2R$`) : undefined, sp: 'm' };
    },
    (r) => {
  ...
  const g53e = [
    ...
    (r) => {
      ...
      return { q: T(`${q[0]} ${p.note[0]}`, `${q[1]} ${p.note[1]}`), a: T(`$${p.s}$`), sp: 's' };
    },
    (r) => {
      const v = r.int(0, 7);
      const Q = [
      ...
      return { q: T(`Fill in the blank: ${Q[0]}`, `Isi tempat kosong: ${Q[1]}`), a: T(Q[2]), sp: 'xs' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`Complete the table for three circles. ${PNOTE[kind][0]}<br>${SPM.table(rows, { head })}`, `Lengkapkan jadual bagi tiga bulatan. ${PNOTE[kind][1]}<br>${SPM.table(rows, { head: headM })}`), a: T(asw.join(' ; ')), sp: 'm' };
    },
    (r) => {
      const [se, sm, tv, re, rm2] = r.pick([
  ...
  const g53m = [
    ...
    (r) => {
      ...
      return { q: T(`${cap(c[0])} forms a sector of radius ${n(rr)} ${u} and angle $${th}^\\circ$. Find ${task === 0 ? 'the length of its curved edge' : 'its area'}. ${p.note[0]}`, `${cap(c[1])} membentuk sektor berjejari ${n(rr)} ${u} dan bersudut $${th}^\\circ$. Cari ${task === 0 ? 'panjang tepi melengkungnya' : 'luasnya'}. ${p.note[1]}`), a: T(`$${p.s}$`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(Q[0], Q[1]), a: T(Q[2], Q[3]), sp: v === 2 ? 'm' : 's' };
    },
    (r) => {
