...
(function () {
  ...
  const e11 = [
    ...
    (r) => { const s = stmts(r, 1)[0]; return { q: T(`True or false? ${s.t.en}.`, `Benar atau palsu? ${s.t.ms}.`), a: tfAns(s), w: W(s.w), sp: 'xs' }; },
    (r) => { const s = stmts(r, 1)[0]; return { q: T(`Is this statement true? "${s.t.en}." Answer Yes or No.`, `Adakah pernyataan ini benar? "${s.t.ms}." Jawab Ya atau Tidak.`), a: T(s.ok ? 'Yes' : 'No', s.ok ? 'Ya' : 'Tidak'), w: W(s.w), sp: 'xs' }; },
  ...
  const m11 = [
    ...
    (r) => {
      const ss = stmts(r, 3);
      return { q: T(`State whether each statement is true or false.<br>${ss.map((s, i) => `(${'abc'[i]}) ${s.t.en}`).join('<br>')}`, `Nyatakan sama ada setiap pernyataan itu benar atau palsu.<br>${ss.map((s, i) => `(${'abc'[i]}) ${s.t.ms}`).join('<br>')}`), a: T(ss.map((s, i) => `(${'abc'[i]}) ${s.ok ? 'True' : 'False'}`).join('; '), ss.map((s, i) => `(${'abc'[i]}) ${s.ok ? 'Benar' : 'Palsu'}`).join('; ')), w: W(...ss.map((s, i) => pre(`(${'abc'[i]})`, s.w))), sp: 'm' };
    },
  ...
  const m12 = [
    ...
    (r) => {
      ...
      need(ev(plain) !== ev(br));
      return { q: T(`Insert one pair of brackets in the left side so that the equation is true: $${tx(plain)} = ${ev(br)}$`, `Masukkan satu pasang kurungan pada sebelah kiri supaya persamaan itu benar: $${tx(plain)} = ${ev(br)}$`), a: T(`$${tx(br)} = ${ev(br)}$`), w: W(T(`Without brackets the $\\times$ is done first and gives $${ev(plain)}$, which is wrong.`, `Tanpa kurungan, $\\times$ dilakukan dahulu dan memberikan $${ev(plain)}$, iaitu salah.`), T(`Brackets round ${kind[0] === '*' ? 'the second pair' : 'the first pair'} change the order: $${tx(br)}$`, `Kurungan pada ${kind[0] === '*' ? 'pasangan kedua' : 'pasangan pertama'} mengubah tertib itu: $${tx(br)}$`), ...evalLines(br)), sp: 's' };
    },
    // find the operation
    (r) => {
  ...
  const a12 = [
    ...
    (r) => {
      ...
      const d = a * y + b - c * y;
      need(y !== 0 && d !== b);
      return { q: T(`By trying integers from $-6$ to $6$, find the value of $\\square$ that makes this true: $${a} \\times \\square + ${pn(b)} = ${c} \\times \\square + ${pn(d)}$`, `Dengan mencuba integer dari $-6$ hingga $6$, cari nilai $\\square$ yang menjadikan pernyataan ini benar: $${a} \\times \\square + ${pn(b)} = ${c} \\times \\square + ${pn(d)}$`), a: T(`$${y}$`), w: W(T(`Try $\\square = ${y}$.`, `Cuba $\\square = ${y}$.`), T(`Left: $${a} \\times ${pn(y)} + ${pn(b)} = ${a * y} + ${pn(b)} = ${a * y + b}$`, `Kiri: $${a} \\times ${pn(y)} + ${pn(b)} = ${a * y} + ${pn(b)} = ${a * y + b}$`), T(`Right: $${c} \\times ${pn(y)} + ${pn(d)} = ${c * y} + ${pn(d)} = ${c * y + d}$`, `Kanan: $${c} \\times ${pn(y)} + ${pn(d)} = ${c * y} + ${pn(d)} = ${c * y + d}$`), T(`Both sides are $${a * y + b}$, so $\\square = ${y}$.`, `Kedua-dua belah ialah $${a * y + b}$, jadi $\\square = ${y}$.`)), sp: 'm' };
    },
    (r) => {
      const a = r.nz(-9, 9), b = r.nz(-9, 9);
      const c = r.pick([
  ...
  const a12b = [
    (r) => {
      ...
      return { q: c[0], a: c[1], w: W(c[1], T('A general statement must hold for every case, so one case that fails disproves it.', 'Sesuatu pernyataan umum mesti benar bagi setiap kes, jadi satu kes yang gagal sudah menyangkalnya.')), sp: 'm' };
    },
    (r) => {
  ...
  const e13b = [
    ...
    (r) => {
      const ss = stf(r, 3);
      return { q: T(`Write True or False for each statement.<br>${ss.map((s, i) => `(${'abc'[i]}) ${s.t.en}`).join('<br>')}`, `Tulis Benar atau Palsu bagi setiap pernyataan.<br>${ss.map((s, i) => `(${'abc'[i]}) ${s.t.ms}`).join('<br>')}`), a: T(ss.map((s, i) => `(${'abc'[i]}) ${s.ok ? 'True' : 'False'}`).join(' '), ss.map((s, i) => `(${'abc'[i]}) ${s.ok ? 'Benar' : 'Palsu'}`).join(' ')), w: W(...ss.map((s, i) => pre(`(${'abc'[i]})`, s.w))), sp: 's' };
    },
    (r) => { const s = stf(r, 1)[0]; return { q: T(`True or false? ${s.t.en}.`, `Benar atau palsu? ${s.t.ms}.`), a: tfAns(s), w: W(s.w), sp: 'xs' }; },
  ...
  const e14b = [
    (r) => { const ss = std(r, 3); return { q: T(`Write True or False for each statement.<br>${ss.map((s2, i) => `(${'abc'[i]}) ${s2.t.en}`).join('<br>')}`, `Tulis Benar atau Palsu bagi setiap pernyataan.<br>${ss.map((s2, i) => `(${'abc'[i]}) ${s2.t.ms}`).join('<br>')}`), a: T(ss.map((s2, i) => `(${'abc'[i]}) ${s2.ok ? 'True' : 'False'}`).join(' '), ss.map((s2, i) => `(${'abc'[i]}) ${s2.ok ? 'Benar' : 'Palsu'}`).join(' ')), w: W(...ss.map((s2, i) => pre(`(${'abc'[i]})`, s2.w))), sp: 's' }; },
    (r) => { const s2 = std(r, 1)[0]; return { q: T(`Is this statement true? ${s2.t.en}. Answer Yes or No.`, `Adakah pernyataan ini benar? ${s2.t.ms}. Jawab Ya atau Tidak.`), a: T(s2.ok ? 'Yes' : 'No', s2.ok ? 'Ya' : 'Tidak'), w: W(s2.w), sp: 'xs' }; },
    // double / half / tenth
    (r) => {
      const a = dr(r, 1, 1, 9);
      const c = r.pick([
  ...
  const e15 = [
    ...
    (r) => { const c = r.pick(STR); return { q: T(`True or false? ${c[0].en} Give a reason.`, `Benar atau palsu? ${c[0].ms} Berikan sebab.`), a: T(`${c[1] ? 'True' : 'False'}. ${c[2].en}`, `${c[1] ? 'Benar' : 'Palsu'}. ${c[2].ms}`), w: W(T('A rational number is any number that can be written as $\\dfrac{a}{b}$ with $a$, $b$ integers and $b \\neq 0$.', 'Nombor nisbah ialah sebarang nombor yang boleh ditulis sebagai $\\dfrac{a}{b}$ dengan $a$, $b$ integer dan $b \\neq 0$.'), c[2]), sp: 's' }; },
    (r) => { const ss = r.sample(STR, 3); return { q: T(`Which of these statements are true?<br>${ss.map((x, i) => `(${'abc'[i]}) ${x[0].en}`).join('<br>')}`, `Antara pernyataan berikut, yang manakah benar?<br>${ss.map((x, i) => `(${'abc'[i]}) ${x[0].ms}`).join('<br>')}`), a: T(ss.map((x, i) => `(${'abc'[i]}) ${x[1] ? 'true' : 'false'}`).join(', '), ss.map((x, i) => `(${'abc'[i]}) ${x[1] ? 'benar' : 'palsu'}`).join(', ')), w: W(...ss.map((x, i) => pre(`(${'abc'[i]})`, x[2]))), sp: 's' }; },
    // number line reading
    (r) => {
  ...
  const e15E = [
    ...
    (r) => { const c = r.pick(STE); return { q: T(`True or false? ${c[0].en} Give a reason.`, `Benar atau palsu? ${c[0].ms} Berikan sebab.`), a: T(`${c[1] ? 'True' : 'False'}. ${c[2].en}`, `${c[1] ? 'Benar' : 'Palsu'}. ${c[2].ms}`), w: W(c[2]), sp: 's' }; },
    (r) => { const ss = r.sample(STE, 3); return { q: T(`Which of these statements are true?<br>${ss.map((x, i) => `(${'abc'[i]}) ${x[0].en}`).join('<br>')}`, `Antara pernyataan berikut, yang manakah benar?<br>${ss.map((x, i) => `(${'abc'[i]}) ${x[0].ms}`).join('<br>')}`), a: T(ss.map((x, i) => `(${'abc'[i]}) ${x[1] ? 'true' : 'false'}`).join(', '), ss.map((x, i) => `(${'abc'[i]}) ${x[1] ? 'benar' : 'palsu'}`).join(', ')), w: W(...ss.map((x, i) => pre(`(${'abc'[i]})`, x[2]))), sp: 's' }; },
