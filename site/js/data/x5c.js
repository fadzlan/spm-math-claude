...
(function () {
  ...
  /* =============================================================== 5.1 Congruency */
  const COND = [
    { en: 'three pairs of corresponding sides equal', ms: 'tiga pasang sisi sepadan yang sama panjang', rule: 'SSS', ok: true, how: T('SSS (side, side, side): three pairs of equal sides fix the triangle.', 'SSS (sisi, sisi, sisi): tiga pasang sisi yang sama menetapkan segi tiga itu.') },
    { en: 'two pairs of corresponding sides equal and the included angle between them equal', ms: 'dua pasang sisi sepadan yang sama panjang dan sudut yang diapit antaranya sama', rule: 'SAS', ok: true, how: T('SAS (side, angle, side): the equal angle lies between the two pairs of equal sides.', 'SAS (sisi, sudut, sisi): sudut yang sama diapit oleh dua pasang sisi yang sama.') },
    { en: 'two pairs of corresponding angles equal and the side between them equal', ms: 'dua pasang sudut sepadan yang sama dan sisi yang diapit antaranya sama', rule: 'ASA', ok: true, how: T('ASA (angle, side, angle): the equal side lies between the two pairs of equal angles.', 'ASA (sudut, sisi, sudut): sisi yang sama diapit oleh dua pasang sudut yang sama.') },
    { en: 'two pairs of corresponding angles equal and a corresponding side that is not between them equal', ms: 'dua pasang sudut sepadan yang sama dan satu sisi sepadan yang tidak diapit antaranya, sama', rule: 'AAS', ok: true, how: T('AAS (angle, angle, side): two pairs of equal angles and an equal side that is not between them.', 'AAS (sudut, sudut, sisi): dua pasang sudut yang sama dan satu sisi yang sama yang tidak diapit olehnya.') },
    { en: "the hypotenuse and one other side of two right-angled triangles equal", ms: 'hipotenus dan satu sisi lain bagi dua segi tiga bersudut tegak, sama', rule: 'RHS', ok: true, how: T('RHS (right angle, hypotenuse, side): for right-angled triangles, equal hypotenuses and one other pair of equal sides.', 'RHS (sudut tegak, hipotenus, sisi): bagi segi tiga bersudut tegak, hipotenus yang sama dan sepasang sisi lain yang sama.') },
    { en: 'three pairs of corresponding angles equal, with no side given', ms: 'tiga pasang sudut sepadan yang sama, tanpa sebarang sisi diberi', rule: 'AAA', ok: false, how: T('AAA: equal angles fix only the shape, not the size (the triangles are similar); not sufficient.', 'AAA: sudut yang sama hanya menetapkan bentuk, bukan saiz (segi tiga itu serupa); tidak mencukupi.') },
    { en: 'two pairs of corresponding sides equal and a non-included angle equal', ms: 'dua pasang sisi sepadan yang sama dan satu sudut yang tidak diapit, sama', rule: 'SSA', ok: false, how: T('SSA: the angle is not between the two sides, so the third side can swing to two positions (two different triangles); not sufficient.', 'SSA: sudut itu tidak diapit oleh dua sisi itu, jadi sisi ketiga boleh berayun ke dua kedudukan (dua segi tiga berbeza); tidak mencukupi.') },
    { en: 'only one pair of corresponding sides equal', ms: 'hanya sepasang sisi sepadan yang sama', rule: '—', ok: false, how: T('One pair of sides: infinitely many different triangles share one side; not sufficient.', 'Sepasang sisi: tak terhingga banyak segi tiga berbeza berkongsi satu sisi; tidak mencukupi.') },
    { en: 'two pairs of corresponding angles equal, with no side given', ms: 'dua pasang sudut sepadan yang sama, tanpa sebarang sisi diberi', rule: '—', ok: false, how: T('Two pairs of angles and no side: the triangles are similar but can differ in size; not sufficient.', 'Dua pasang sudut tanpa sisi: segi tiga itu serupa tetapi saiznya boleh berbeza; tidak mencukupi.') },
    { en: 'equal areas', ms: 'luas yang sama', rule: '—', ok: false, how: T('Equal areas: e.g. right-angled triangles with legs 2 cm, 6 cm and 3 cm, 4 cm both have area 6 cm² but are different; not sufficient.', 'Luas yang sama: cth. segi tiga bersudut tegak dengan kaki 2 cm, 6 cm dan 3 cm, 4 cm kedua-duanya berluas 6 cm² tetapi berbeza; tidak mencukupi.') },
    { en: 'equal perimeters', ms: 'perimeter yang sama', rule: '—', ok: false, how: T('Equal perimeters: e.g. sides 3, 4, 5 and 4, 4, 4 both have perimeter 12 but are different; not sufficient.', 'Perimeter yang sama: cth. sisi 3, 4, 5 dan 4, 4, 4 kedua-duanya berperimeter 12 tetapi berbeza; tidak mencukupi.') },
  ];
  const HOW = (rule) => COND.find((c) => c.rule === rule).how;
  ...
  const g51e = [
    (r) => {
      ...
      const srt = (t) => [...t].sort((u, v) => u - v).join(', ');
      return { q, a: T(`${x} and ${y}: all three pairs of corresponding sides are equal (SSS).`, `${x} dan ${y}: ketiga-tiga pasang sisi sepadan adalah sama (SSS).`), w: W(T(`Sides in order: ${x}: ${srt(a)}; ${y}: ${srt(b)}; ${z}: ${srt(c)}`, `Sisi mengikut tertib: ${x}: ${srt(a)}; ${y}: ${srt(b)}; ${z}: ${srt(c)}`), T(`${x} and ${y} have the same three lengths but ${z} does not: SSS.`, `${x} dan ${y} mempunyai tiga panjang yang sama tetapi ${z} tidak: SSS.`)), sp: 's' };
    },
    (r) => {
      ...
      return { q: T(`The diagram shows two triangles. Are they congruent? Give a reason. ${NTS.en}`, `Rajah menunjukkan dua segi tiga. Adakah kedua-duanya kongruen? Berikan sebab. ${NTS.ms}`), fig, a: T('Yes: all three pairs of corresponding sides are equal (SSS).', 'Ya: ketiga-tiga pasang sisi sepadan adalah sama (SSS).'), w: W(T(`Sides in order: ${a.join(', ')} cm and ${[...b].sort((u, v) => u - v).join(', ')} cm, the same three lengths.`, `Sisi mengikut tertib: ${a.join(', ')} cm dan ${[...b].sort((u, v) => u - v).join(', ')} cm, tiga panjang yang sama.`), HOW('SSS')), sp: 's' };
    },
    (r) => {
      const c = r.pick(OKC);
      const f = r.pick([
      ...
      return { q: T(f.en.replace('{c}', c.en), f.ms.replace('{c}', c.ms)), a: T(`$${c.rule}$`), w: W(c.how), sp: 'xs' };
    },
    (r) => {
      ...
      const f = r.pick(forms);
      return { q: T(f.en.replace('{list}', list), f.ms.replace('{list}', listms)), a: T(`(${letters[opts.indexOf(ok)]}) — ${ok.rule}`), w: W(ok.how, T('None of the other options fixes one unique triangle.', 'Tiada pilihan lain yang menetapkan satu segi tiga yang unik.')), sp: 's' };
    },
    (r) => {
      const st = r.pick([
        [T('Congruent figures have exactly the same shape and the same size.', 'Rajah kongruen mempunyai bentuk yang sama dan saiz yang sama.'), true, T('This is the definition of congruence.', 'Ini ialah takrif kekongruenan.')],
        [T('Two figures can be congruent even if one is larger than the other, as long as the shape is the same.', 'Dua rajah boleh kongruen walaupun satu lebih besar daripada yang lain, asalkan bentuknya sama.'), false, T('Same shape but different size means similar, not congruent.', 'Bentuk sama tetapi saiz berbeza bermakna serupa, bukan kongruen.')],
        [T('If two triangles are similar, they must also be congruent.', 'Jika dua segi tiga serupa, kedua-duanya juga mesti kongruen.'), false, T('Similar triangles can have different sizes (scale factor not 1).', 'Segi tiga serupa boleh berlainan saiz (faktor skala bukan 1).')],
        [T('SSS means that three pairs of corresponding sides being equal is enough to prove two triangles congruent.', 'SSS bermaksud tiga pasang sisi sepadan yang sama sudah mencukupi untuk membuktikan dua segi tiga kongruen.'), true, HOW('SSS')],
        [T('Knowing that three pairs of corresponding angles are equal (AAA) is enough to prove two triangles congruent.', 'Mengetahui tiga pasang sudut sepadan yang sama (AAA) sudah mencukupi untuk membuktikan dua segi tiga kongruen.'), false, HOW('AAA')],
        [T('RHS can only be used for right-angled triangles.', 'RHS hanya boleh digunakan untuk segi tiga bersudut tegak.'), true, T('R stands for the right angle and H for the hypotenuse, which only right-angled triangles have.', 'R mewakili sudut tegak dan H mewakili hipotenus, yang hanya ada pada segi tiga bersudut tegak.')],
        [T('If $\\triangle ABC \\equiv \\triangle PQR$, then $AB = PQ$.', 'Jika $\\triangle ABC \\equiv \\triangle PQR$, maka $AB = PQ$.'), true, T('The vertices match in order ($A \\leftrightarrow P$, $B \\leftrightarrow Q$), so $AB$ corresponds to $PQ$.', 'Bucu sepadan mengikut susunan ($A \\leftrightarrow P$, $B \\leftrightarrow Q$), jadi $AB$ sepadan dengan $PQ$.')],
        [T('If $\\triangle ABC \\equiv \\triangle PQR$, then $AB$ must equal $QR$.', 'Jika $\\triangle ABC \\equiv \\triangle PQR$, maka $AB$ mesti sama dengan $QR$.'), false, T('The vertices match in order, so $AB$ corresponds to $PQ$, not $QR$.', 'Bucu sepadan mengikut susunan, jadi $AB$ sepadan dengan $PQ$, bukan $QR$.')],
        [T('A right-angled triangle can be proved congruent to another just by matching their hypotenuses, with no other information.', 'Sebuah segi tiga bersudut tegak boleh dibuktikan kongruen dengan yang lain hanya dengan memadankan hipotenusnya sahaja, tanpa maklumat lain.'), false, T('RHS also needs one other pair of equal sides; many right-angled triangles share the same hypotenuse.', 'RHS juga memerlukan sepasang sisi lain yang sama; banyak segi tiga bersudut tegak berkongsi hipotenus yang sama.')],
        [T('Corresponding sides and corresponding angles of two congruent triangles are always equal.', 'Sisi sepadan dan sudut sepadan bagi dua segi tiga yang kongruen adalah sentiasa sama.'), true, T('Congruent triangles fit exactly on top of each other, so every corresponding part matches.', 'Segi tiga kongruen bertindih dengan tepat, jadi setiap bahagian sepadan adalah sama.')],
    ...
    (r) => {
      ...
      if (giveSide) {
        return { q: T(`$\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${q}${s}$ and $${x}${y} = ${val}$ cm. Find $${p}${q}$.`, `$\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${q}${s}$ dan $${x}${y} = ${val}$ cm. Cari $${p}${q}$.`), a: T(`$${val}$ cm`), w: W(T(`Vertices match in order: ${x} ↔ ${p}, ${y} ↔ ${q}, so $${p}${q} = ${x}${y} = ${val}$ cm`, `Bucu sepadan mengikut susunan: ${x} ↔ ${p}, ${y} ↔ ${q}, jadi $${p}${q} = ${x}${y} = ${val}$ cm`)), sp: 'xs' };
      }
      return { q: T(`$\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${q}${s}$ and $\\angle ${y} = ${val + 30}^\\circ$. Find $\\angle ${q}$.`, `$\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${q}${s}$ dan $\\angle ${y} = ${val + 30}^\\circ$. Cari $\\angle ${q}$.`), a: T(`$${val + 30}^\\circ$`), w: W(T(`The second vertices correspond (${y} ↔ ${q}), so $\\angle ${q} = \\angle ${y} = ${val + 30}^\\circ$`, `Bucu kedua sepadan (${y} ↔ ${q}), jadi $\\angle ${q} = \\angle ${y} = ${val + 30}^\\circ$`)), sp: 'xs' };
    },
    (r) => {
      const t = r.pick(RT3);
      const [x, y, z, p, q, s] = letPick(r);
      const fig = [triFig([x, y, z], { AC: t[2] + ' cm', AB: t[0] + ' cm' }, { right: 'B' }), triFig([p, q, s], { AC: t[2] + ' cm', AB: t[0] + ' cm' }, { right: 'B' })];
      return { q: T(`Both triangles are right-angled at ${y} and ${q}. The hypotenuses $${x}${z} = ${p}${s} = ${t[2]}$ cm and $${x}${y} = ${p}${q} = ${t[0]}$ cm. Name the rule that proves they are congruent. ${NTS.en}`, `Kedua-dua segi tiga bersudut tegak di ${y} dan ${q}. Hipotenus $${x}${z} = ${p}${s} = ${t[2]}$ cm dan $${x}${y} = ${p}${q} = ${t[0]}$ cm. Namakan syarat yang membuktikan kedua-duanya kongruen. ${NTS.ms}`), fig, a: T('$RHS$'), w: W(T(`Right angles at ${y} and ${q}, equal hypotenuses $${x}${z} = ${p}${s}$ and one more pair of equal sides $${x}${y} = ${p}${q}$.`, `Sudut tegak di ${y} dan ${q}, hipotenus yang sama $${x}${z} = ${p}${s}$ dan sepasang lagi sisi yang sama $${x}${y} = ${p}${q}$.`), HOW('RHS')), sp: 'xs' };
    },
  ];
  const g51m = [
    (r) => {
      ...
      return { q, a: c.ok ? T(`Yes: ${c.rule}.`, `Ya: ${c.rule}.`) : T(`No: this does not guarantee congruence (e.g. two triangles of different sizes, or two different triangles, can share this data).`, `Tidak: ini tidak menjamin kekongruenan (cth. dua segi tiga berlainan saiz, atau dua segi tiga berbeza, boleh mempunyai data yang sama ini).`), w: W(c.how), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q, a: T(`RHS: the hypotenuse and one leg are equal ($${t[2]}$ m and $${t[0]}$ m), with the right angle at ${y} and ${qq}. So $\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${qq}${s}$, and $\\angle ${z} = \\angle ${s} = ${ang}^\\circ$.`, `RHS: hipotenus dan satu kaki adalah sama ($${t[2]}$ m dan $${t[0]}$ m), dengan sudut tegak di ${y} dan ${qq}. Jadi $\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${qq}${s}$, dan $\\angle ${z} = \\angle ${s} = ${ang}^\\circ$.`), w: W(T(`Right angles at ${y} and ${qq}; hypotenuses $${x}${z} = ${p}${s} = ${t[2]}$ m; legs $${x}${y} = ${p}${qq} = ${t[0]}$ m: RHS.`, `Sudut tegak di ${y} dan ${qq}; hipotenus $${x}${z} = ${p}${s} = ${t[2]}$ m; kaki $${x}${y} = ${p}${qq} = ${t[0]}$ m: RHS.`), T(`${z} ↔ ${s}, so $\\angle ${z} = \\angle ${s} = ${ang}^\\circ$.`, `${z} ↔ ${s}, jadi $\\angle ${z} = \\angle ${s} = ${ang}^\\circ$.`)), sp: 'm' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q, a: ans, w: W(HOW(wrong), wrong === 'AAA' ? T('Correct conclusion: the triangles are similar.', 'Kesimpulan yang betul: segi tiga itu serupa.') : T('Only the valid tests (SSS, SAS, ASA, AAS, RHS) prove congruence.', 'Hanya ujian yang sah (SSS, SAS, ASA, AAS, RHS) membuktikan kekongruenan.')), sp: 'm' };
    },
    (r) => {
      const rule = r.pick(['SAS', 'ASA', 'AAS']);
      const [x, y, z, p, q, s] = letPick(r);
      const missing = rule === 'SAS' ? T(`the included angle $\\angle ${y}$ (and its equal $\\angle ${q}$)`, `sudut yang diapit $\\angle ${y}$ (dan sudut sepadannya $\\angle ${q}$)`) : rule === 'ASA' ? T(`the side $${x}${z}$ between the two known angles (and its equal $${p}${s}$)`, `sisi $${x}${z}$ di antara dua sudut yang diketahui (dan sisi sepadannya $${p}${s}$)`) : T(`a corresponding side not between the two known angles, e.g. $${y}${z}$ (and its equal $${q}${s}$)`, `sisi sepadan yang tidak diapit oleh dua sudut yang diketahui, cth. $${y}${z}$ (dan sisi sepadannya $${q}${s}$)`);
      const wk = rule === 'SAS' ? T(`The two given sides $${x}${y}$ and $${y}${z}$ meet at ${y}, so the included angle is $\\angle ${y}$.`, `Dua sisi yang diberi $${x}${y}$ dan $${y}${z}$ bertemu di ${y}, jadi sudut yang diapit ialah $\\angle ${y}$.`) : rule === 'ASA' ? T(`The known angles are at ${x} and ${z}; the side joining them is $${x}${z}$.`, `Sudut yang diketahui berada di ${x} dan ${z}; sisi yang menyambungkannya ialah $${x}${z}$.`) : T(`The known angles are at ${x} and ${y}; $${x}${y}$ joins them, so AAS needs $${y}${z}$ or $${x}${z}$ instead.`, `Sudut yang diketahui berada di ${x} dan ${y}; $${x}${y}$ menyambungkannya, jadi AAS memerlukan $${y}${z}$ atau $${x}${z}$ sebaliknya.`);
      ...
      return { q: T(`Triangles ${x}${y}${z} and ${p}${q}${s} are given with ${given.en}. What additional single piece of information is needed to prove congruency by ${rule}?`, `Segi tiga ${x}${y}${z} dan ${p}${q}${s} diberi dengan ${given.ms}. Apakah satu maklumat tambahan yang diperlukan untuk membuktikan kekongruenan melalui ${rule}?`), a: missing, w: W(HOW(rule), wk), sp: 's' };
    },
    (r) => {
      const [x, y, z, p, q, s] = letPick(r);
      const cor = r.chance();
      ...
      return { q: T(`In $\\triangle ${x}${y}${z}$ and $\\triangle ${p}${q}${s}$, $${x}${y} = ${p}${q}$, $${y}${z} = ${q}${s}$ and $\\angle ${y} = \\angle ${q}$ (SAS). Which of these correspondence statements is/are correct: ${stmts[0].en} or ${stmts[1].en}? Explain using the matching order of vertices.`, `Dalam $\\triangle ${x}${y}${z}$ dan $\\triangle ${p}${q}${s}$, $${x}${y} = ${p}${q}$, $${y}${z} = ${q}${s}$ dan $\\angle ${y} = \\angle ${q}$ (SAS). Yang manakah antara pernyataan sepadan ini betul: ${stmts[0].ms} atau ${stmts[1].ms}? Terangkan menggunakan susunan bucu yang sepadan.`), a: cor ? T(`Both are correct: each lists the vertices in matching order ($${x} \\leftrightarrow ${p}$, $${y} \\leftrightarrow ${q}$, $${z} \\leftrightarrow ${s}$), since that is how the equal sides and angle were paired.`, `Kedua-duanya betul: setiap satu menyenaraikan bucu mengikut susunan sepadan ($${x} \\leftrightarrow ${p}$, $${y} \\leftrightarrow ${q}$, $${z} \\leftrightarrow ${s}$), kerana begitulah sisi dan sudut yang sama itu dipasangkan.`) : T(`Neither is correct: the vertices must match in order ($${x} \\leftrightarrow ${p}$, $${y} \\leftrightarrow ${q}$, $${z} \\leftrightarrow ${s}$), so the correct statement is $\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${q}${s}$.`, `Kedua-duanya tidak betul: bucu mesti sepadan mengikut susunan ($${x} \\leftrightarrow ${p}$, $${y} \\leftrightarrow ${q}$, $${z} \\leftrightarrow ${s}$), jadi pernyataan yang betul ialah $\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${q}${s}$.`), w: W(T(`From $${x}${y} = ${p}${q}$, $\\angle ${y} = \\angle ${q}$, $${y}${z} = ${q}${s}$: $${x} \\leftrightarrow ${p}$, $${y} \\leftrightarrow ${q}$, $${z} \\leftrightarrow ${s}$`, `Daripada $${x}${y} = ${p}${q}$, $\\angle ${y} = \\angle ${q}$, $${y}${z} = ${q}${s}$: $${x} \\leftrightarrow ${p}$, $${y} \\leftrightarrow ${q}$, $${z} \\leftrightarrow ${s}$`), cor ? T('Check each statement letter by letter: both keep every pair in the same position.', 'Semak setiap pernyataan huruf demi huruf: kedua-duanya mengekalkan setiap pasangan pada kedudukan yang sama.') : T(`Check letter by letter: the first pairs ${x} with ${q}, the second pairs ${y} with ${s}; both break the matching.`, `Semak huruf demi huruf: yang pertama memasangkan ${x} dengan ${q}, yang kedua memasangkan ${y} dengan ${s}; kedua-duanya melanggar padanan.`)), sp: 'm' };
    },
    (r) => {
      const [x, y, z, p, q, s] = letPick(r);
      const t1 = r.int(2, 4), t2 = r.int(1, 3);
      return { q: T(`In the diagram, $${x}${y}$ is marked with ${t1} tick${t1 > 1 ? 's' : ''} and $${p}${q}$ with ${t1} tick${t1 > 1 ? 's' : ''} (so $${x}${y} = ${p}${q}$); $${y}${z}$ is marked with ${t2} tick${t2 > 1 ? 's' : ''} and $${q}${s}$ with ${t2} tick${t2 > 1 ? 's' : ''} (so $${y}${z} = ${q}${s}$); and the angle between them, $\\angle ${y}$ and $\\angle ${q}$, are marked equal with an arc. ${NTS.en} Name the congruency rule shown, and write the full congruency statement.`, `Dalam rajah itu, $${x}${y}$ ditanda dengan ${t1} tanda dan $${p}${q}$ dengan ${t1} tanda (jadi $${x}${y} = ${p}${q}$); $${y}${z}$ ditanda dengan ${t2} tanda dan $${q}${s}$ dengan ${t2} tanda (jadi $${y}${z} = ${q}${s}$); dan sudut di antaranya, $\\angle ${y}$ dan $\\angle ${q}$, ditanda sama dengan lengkok. ${NTS.ms} Namakan syarat kekongruenan yang ditunjukkan, dan tulis pernyataan kekongruenan penuh.`), a: T(`SAS; $\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${q}${s}$`, `SAS; $\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${q}${s}$`), w: W(T(`Two pairs of equal sides with the equal angle between them (at ${y} and ${q}): SAS.`, `Dua pasang sisi yang sama dengan sudut yang sama diapit di antaranya (di ${y} dan ${q}): SAS.`), T(`Write the vertices in matching order: ${x} ↔ ${p}, ${y} ↔ ${q}, ${z} ↔ ${s}.`, `Tulis bucu mengikut susunan sepadan: ${x} ↔ ${p}, ${y} ↔ ${q}, ${z} ↔ ${s}.`)), sp: 's' };
    },
    (r) => {
      const t = r.pick(RT3), h = t[0];
      const [x, y, z, p, q, s] = letPick(r);
      return { q: T(`A symmetrical roof truss is built from two right-angled triangles $${x}${y}${z}$ and $${p}${q}${s}$ meeting at the ridge, both with the right angle at ${y} and ${q}. The sloping beams (hypotenuses) $${x}${z} = ${p}${s} = ${t[2]}$ m are equal and the vertical struts $${x}${y} = ${p}${q} = ${h}$ m are equal. State the rule that proves the two triangles are congruent, and explain why this guarantees the base half-widths $${y}${z}$ and $${q}${s}$ are also equal.`, `Sebuah kekuda bumbung simetri dibina daripada dua segi tiga bersudut tegak $${x}${y}${z}$ dan $${p}${q}${s}$ yang bertemu di rabung, kedua-duanya bersudut tegak di ${y} dan ${q}. Rasuk condong (hipotenus) $${x}${z} = ${p}${s} = ${t[2]}$ m adalah sama dan tunjang menegak $${x}${y} = ${p}${q} = ${h}$ m adalah sama. Nyatakan syarat yang membuktikan kedua-dua segi tiga itu kongruen, dan terangkan mengapa ini menjamin separuh lebar tapak $${y}${z}$ dan $${q}${s}$ juga sama.`), a: T(`RHS: the hypotenuse and one leg of each right-angled triangle are equal. So $\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${q}${s}$, and corresponding sides give $${y}${z} = ${q}${s}$.`, `RHS: hipotenus dan satu kaki bagi setiap segi tiga bersudut tegak adalah sama. Jadi $\\triangle ${x}${y}${z} \\equiv \\triangle ${p}${q}${s}$, dan sisi sepadan memberikan $${y}${z} = ${q}${s}$.`), w: W(T(`Right angles at ${y} and ${q}; hypotenuses $${x}${z} = ${p}${s} = ${t[2]}$ m; struts $${x}${y} = ${p}${q} = ${h}$ m: RHS.`, `Sudut tegak di ${y} dan ${q}; hipotenus $${x}${z} = ${p}${s} = ${t[2]}$ m; tunjang $${x}${y} = ${p}${q} = ${h}$ m: RHS.`), T(`Check (Pythagoras): $${y}${z} = ${q}${s} = \\sqrt{${t[2]}^2 - ${h}^2} = ${t[1]}$ m`, `Semakan (Pythagoras): $${y}${z} = ${q}${s} = \\sqrt{${t[2]}^2 - ${h}^2} = ${t[1]}$ m`)), sp: 'm' };
    },
  ];
  const g51a = [
    (r) => {
      ...
      return { q: T(`$ABCD$ is ${shape.en}. Show that triangles $${shape.tri[0]}$ and $${shape.tri[1]}$ are congruent, and deduce a property of the diagonal(s).`, `$ABCD$ ialah ${shape.ms}. Tunjukkan bahawa segi tiga $${shape.tri[0]}$ dan $${shape.tri[1]}$ adalah kongruen, dan deduksikan satu sifat pepenjuru itu.`), a: T(`${shape.reason.en}: ${shape.rule}. So $\\triangle ${shape.tri[0]} \\equiv \\triangle ${shape.tri[1]}$, and ${shape.ded.en}.`, `${shape.reason.ms}: ${shape.rule}. Jadi $\\triangle ${shape.tri[0]} \\equiv \\triangle ${shape.tri[1]}$, dan ${shape.ded.ms}.`), w: W(shape.reason, HOW(shape.rule), T(`Corresponding parts of $\\triangle ${shape.tri[0]}$ and $\\triangle ${shape.tri[1]}$ are equal: ${shape.ded.en}.`, `Bahagian sepadan $\\triangle ${shape.tri[0]}$ dan $\\triangle ${shape.tri[1]}$ adalah sama: ${shape.ded.ms}.`)), sp: 'l' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`Write down one complete, valid set of side lengths and/or angles for $\\triangle ${x}${y}${z}$ and $\\triangle ${p}${q}${s}$ that would prove them congruent by ${rule}, but would NOT prove them congruent by any other single rule from {SSS, SAS, ASA, AAS, RHS}. Explain your choice.`, `Tuliskan satu set panjang sisi dan/atau sudut yang lengkap dan sah bagi $\\triangle ${x}${y}${z}$ dan $\\triangle ${p}${q}${s}$ yang akan membuktikan kedua-duanya kongruen melalui ${rule}, tetapi TIDAK membuktikan kongruen melalui mana-mana satu syarat lain daripada {SSS, SAS, ASA, AAS, RHS}. Terangkan pilihan anda.`), a: ans, w: W(HOW(rule), T('Give exactly that data and nothing more, so no other rule can be used.', 'Berikan data itu sahaja dan tidak lebih, supaya tiada syarat lain boleh digunakan.')), sp: 'l' };
    },
  ];
  ...
  const kP = (kt) => (kt[0] === '-' ? `(${kt})` : kt);
  /** "(C0 + k × (dx), C1 + k × (dy)) = (x', y')" (or "(k × x, k × y) = …" for centre the origin); kt = TeX of k */
  const enlLine = (P, C, k, kt) => {
    const I = enl(P, C, k);
    kt = kP(kt || n(k));
    if (C[0] === 0 && C[1] === 0) return `(${kt} \\times ${par(P[0])}, ${kt} \\times ${par(P[1])}) = ${pt(I)}`;
    return `(${n(C[0])} + ${kt} \\times ${par(P[0] - C[0])}, ${n(C[1])} + ${kt} \\times ${par(P[1] - C[1])}) = ${pt(I)}`;
  };
  const ENLF = "$P' = C + k(P - C)$";
  /** which side of C is P' on: compare P - C with P' - C */
  const sideW = (P, C, I) => {
    const d = [P[0] - C[0], P[1] - C[1]], e = [I[0] - C[0], I[1] - C[1]], same = d[0] * e[0] + d[1] * e[1] > 0;
    return [`$P - C = ${pt(d)}$, $P' - C = ${pt(e)}$`, same ? T("$P' - C$ points the same way as $P - C$: same side of $C$, so $k > 0$", "$P' - C$ sehala dengan $P - C$: sebelah $C$ yang sama, jadi $k > 0$") : T("$P' - C$ points the opposite way to $P - C$: opposite side of $C$, so $k < 0$", "$P' - C$ berlawanan arah dengan $P - C$: sebelah $C$ yang bertentangan, jadi $k < 0$")];
  };
  const kindW = (k) => W(T(`$|k| = ${n(Math.abs(k))}$ ${Math.abs(k) > 1 ? 'is greater than 1: the image is larger (enlargement)' : 'is less than 1: the image is smaller (reduction)'}`, `$|k| = ${n(Math.abs(k))}$ ${Math.abs(k) > 1 ? 'lebih besar daripada 1: imej lebih besar (pembesaran)' : 'kurang daripada 1: imej lebih kecil (pengecilan)'}`), T(`$k$ is ${k > 0 ? 'positive: same side of the centre' : 'negative: opposite side of the centre'}`, `$k$ ${k > 0 ? 'positif: sebelah pusat yang sama' : 'negatif: sebelah pusat yang bertentangan'}`));
  ...
  const g52e = [
    (r) => ({ q: T(`Complete the sentence: an enlargement is a transformation in which every point $P$ maps to the point ____, for a fixed centre $C$ and scale factor $k$.`, `Lengkapkan ayat: pembesaran ialah satu transformasi di mana setiap titik $P$ dipetakan kepada titik ____, bagi satu pusat tetap $C$ dan faktor skala $k$.`), a: T(`$C + k(P - C)$`), w: W(T('$P - C$ is the vector from the centre to $P$; the enlargement multiplies it by $k$ and starts again from $C$.', '$P - C$ ialah vektor dari pusat ke $P$; pembesaran mendarabnya dengan $k$ dan bermula semula dari $C$.')), sp: 'xs' }),
    (r) => {
    ...
    (r) => {
      ...
      return { q, a: T(`$${pt(enl(P, [0, 0], kVal(f)))}$`), w: W(T('Centre the origin: multiply each coordinate by $k$', 'Pusat pada asalan: darabkan setiap koordinat dengan $k$'), `$${enlLine(P, [0, 0], kVal(f))}$`), sp: 's' };
    },
    (r) => {
      ...
      return { q, a: T(`$${pt(enl(P, C, kVal(f)))}$`), w: W(ENLF, `$P' = ${enlLine(P, C, kVal(f))}$`), sp: 's' };
    },
    (r) => {
      const obj = r.int(3, 9), f = r.pick(KSET.slice(0, 2));
      const ctx = r.pick(ECTX);
      return { q: T(`A length of $${obj}$ cm on ${ctx.en} is enlarged with scale factor ${n(kVal(f))}. Find the corresponding length on the enlarged copy.`, `Panjang $${obj}$ cm pada ${ctx.ms} dibesarkan dengan faktor skala ${n(kVal(f))}. Cari panjang yang sepadan pada salinan yang dibesarkan.`), a: T(`$${n(obj * kVal(f))}$ cm`), w: W(T(`Image length $= k \\times$ object length $= ${n(kVal(f))} \\times ${obj} = ${n(obj * kVal(f))}$`, `Panjang imej $= k \\times$ panjang objek $= ${n(kVal(f))} \\times ${obj} = ${n(obj * kVal(f))}$`)), sp: 'xs' };
    },
    (r) => {
      const img = r.int(6, 18), f = r.pick(KSET.slice(0, 2));
      need(img % f.num === 0);
      const ctx = r.pick(ECTX);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} is an enlargement, scale factor ${n(kVal(f))}, of a smaller original. A length on the enlarged copy is $${img}$ cm. Find the corresponding length on the original.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} ialah pembesaran, faktor skala ${n(kVal(f))}, daripada satu asal yang lebih kecil. Satu panjang pada salinan yang dibesarkan ialah $${img}$ cm. Cari panjang yang sepadan pada yang asal.`), a: T(`$${n(img / kVal(f))}$ cm`), w: W(T(`Object length $= \\dfrac{\\text{image length}}{k} = \\dfrac{${img}}{${n(kVal(f))}} = ${n(img / kVal(f))}$`, `Panjang objek $= \\dfrac{\\text{panjang imej}}{k} = \\dfrac{${img}}{${n(kVal(f))}} = ${n(img / kVal(f))}$`)), sp: 'xs' };
    },
    (r) => {
    ...
    (r) => {
      const f = r.pick(KSET);
      return { q: T(`An enlargement has scale factor $${kTexOf(f)}$. State the ratio of the area of the image to the area of the object.`, `Satu pembesaran mempunyai faktor skala $${kTexOf(f)}$. Nyatakan nisbah luas imej kepada luas objek.`), a: T(`$${Fr.tex(areaFrOf(f))} : 1$`), w: W(T(`Areas are multiplied by $k^2 = \\left(${kTexOf(f)}\\right)^2 = ${Fr.tex(areaFrOf(f))}$`, `Luas didarab dengan $k^2 = \\left(${kTexOf(f)}\\right)^2 = ${Fr.tex(areaFrOf(f))}$`)), sp: 'xs' };
    },
    (r) => {
      const obj = r.int(3, 9), f = r.pick(KSET.slice(0, 2)), ctx = r.pick(ECTX);
      return { q: T(`On ${ctx.en}, a length of $${obj}$ cm is scaled by a factor of $${n(kVal(f))}$. Is this an enlargement or a reduction? Find the new length.`, `Pada ${ctx.ms}, satu panjang $${obj}$ cm diskalakan dengan faktor $${n(kVal(f))}$. Adakah ini pembesaran atau pengecilan? Cari panjang baharu.`), a: T(`${kVal(f) > 1 ? 'Enlargement' : 'Reduction'}; $${n(obj * kVal(f))}$ cm`, `${kVal(f) > 1 ? 'Pembesaran' : 'Pengecilan'}; $${n(obj * kVal(f))}$ cm`), w: W(T(`$k = ${n(kVal(f))} ${kVal(f) > 1 ? '> 1$: enlargement' : '< 1$: reduction'}`, `$k = ${n(kVal(f))} ${kVal(f) > 1 ? '> 1$: pembesaran' : '< 1$: pengecilan'}`), `$${obj} \\times ${n(kVal(f))} = ${n(obj * kVal(f))}$`), sp: 'xs' };
    },
    (r) => {
      const A = r.pick([4, 5, 6, 8, 10]), f = r.pick(KSET.slice(0, 2));
      return { q: T(`An object has area $${A}$ cm$^2$. Find the area of its image under an enlargement with scale factor ${n(kVal(f))}.`, `Sebuah objek mempunyai luas $${A}$ cm$^2$. Cari luas imejnya di bawah pembesaran dengan faktor skala ${n(kVal(f))}.`), a: T(`$${n(A * kVal(f) * kVal(f))}$ cm$^2$`), w: W(T(`Image area $= k^2 \\times$ object area $= ${n(kVal(f))}^2 \\times ${A} = ${n(A * kVal(f) * kVal(f))}$`, `Luas imej $= k^2 \\times$ luas objek $= ${n(kVal(f))}^2 \\times ${A} = ${n(A * kVal(f) * kVal(f))}$`)), sp: 's' };
    },
    (r) => {
      const A = r.pick([4, 5, 6, 8, 10]), f = r.pick(KSET.slice(0, 2)), ctx = r.pick(ECTX);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} has area $${A}$ cm$^2$. It is enlarged with scale factor ${n(kVal(f))}. Find the new area.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} mempunyai luas $${A}$ cm$^2$. Ia dibesarkan dengan faktor skala ${n(kVal(f))}. Cari luas baharu.`), a: T(`$${n(A * kVal(f) * kVal(f))}$ cm$^2$`), w: W(T(`New area $= k^2 \\times$ original area $= ${n(kVal(f))}^2 \\times ${A} = ${n(A * kVal(f) * kVal(f))}$`, `Luas baharu $= k^2 \\times$ luas asal $= ${n(kVal(f))}^2 \\times ${A} = ${n(A * kVal(f) * kVal(f))}$`)), sp: 's' };
    },
    (r) => {
      const den = r.pick([2, 3, 4, 5]), obj = den * r.int(2, 5), k = r.pick([2, 3]);
      return { q: T(`A side of the object is $${obj}$ cm. Under an enlargement with scale factor ${k}, find the length of the corresponding side of the image, and hence find the scale factor written as image length $\\div$ object length.`, `Satu sisi objek ialah $${obj}$ cm. Di bawah pembesaran dengan faktor skala ${k}, cari panjang sisi imej yang sepadan, dan seterusnya cari faktor skala yang ditulis sebagai panjang imej $\\div$ panjang objek.`), a: T(`Image side $${obj * k}$ cm; $${obj * k} \\div ${obj} = ${k}$`, `Sisi imej $${obj * k}$ cm; $${obj * k} \\div ${obj} = ${k}$`), w: W(`$${obj} \\times ${k} = ${obj * k}$`, `$${obj * k} \\div ${obj} = ${k}$`), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      const list = opts.map((o, i) => `(${letters[i]}) ${o.en}`).join(', ');
      const listms = opts.map((o, i) => `(${letters[i]}) ${o.ms}`).join(', ');
      return { q: T(`A transformation maps every point $P$ to $C + ${n(kVal(f))}(P - C)$ for a fixed centre $C$. Which type of transformation is this? ${list}`, `Satu transformasi memetakan setiap titik $P$ kepada $C + ${n(kVal(f))}(P - C)$ bagi satu pusat tetap $C$. Transformasi jenis manakah ini? ${listms}`), a: T(`(${letters[opts.indexOf(correct)]}) ${correct.en}`, `(${letters[opts.indexOf(correct)]}) ${correct.ms}`), w: W(T(`Each point moves along the line from $C$ and its distance from $C$ is multiplied by ${n(kVal(f))}: that is an enlargement.`, `Setiap titik bergerak di sepanjang garis dari $C$ dan jaraknya dari $C$ didarab dengan ${n(kVal(f))}: itu ialah pembesaran.`)), sp: 'xs' };
    },
    (r) => {
      const f = r.pick(KSET), ctx = r.pick(ECTX);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} is enlarged with scale factor $${kTexOf(f)}$. State the ratio of the area of the enlarged copy to the area of the original.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} dibesarkan dengan faktor skala $${kTexOf(f)}$. Nyatakan nisbah luas salinan yang dibesarkan kepada luas yang asal.`), a: T(`$${Fr.tex(areaFrOf(f))} : 1$`), w: W(T(`Area ratio $= k^2 = \\left(${kTexOf(f)}\\right)^2 = ${Fr.tex(areaFrOf(f))}$`, `Nisbah luas $= k^2 = \\left(${kTexOf(f)}\\right)^2 = ${Fr.tex(areaFrOf(f))}$`)), sp: 'xs' };
    },
    (r) => {
      const item = r.pick(SPM.bank.items), width = r.int(3, 6), f = r.pick(KSET.slice(0, 2));
      return { q: T(`A logo of width $${width}$ cm is printed on a batch of ${item.en} at $${n(kVal(f))}$ times its original size. Is this printing an enlargement or a reduction, and what is the new width?`, `Sebuah logo berlebar $${width}$ cm dicetak pada sekumpulan ${item.ms} dengan $${n(kVal(f))}$ kali saiz asalnya. Adakah percetakan ini pembesaran atau pengecilan, dan apakah lebar baharunya?`), a: T(`${kVal(f) > 1 ? 'Enlargement' : 'Reduction'}; new width $${n(width * kVal(f))}$ cm`, `${kVal(f) > 1 ? 'Pembesaran' : 'Pengecilan'}; lebar baharu $${n(width * kVal(f))}$ cm`), w: W(T(`$k = ${n(kVal(f))} ${kVal(f) > 1 ? '> 1$: enlargement' : '< 1$: reduction'}`, `$k = ${n(kVal(f))} ${kVal(f) > 1 ? '> 1$: pembesaran' : '< 1$: pengecilan'}`), `$${width} \\times ${n(kVal(f))} = ${n(width * kVal(f))}$`), sp: 'xs' };
    },
  ];
  const g52m = [
    (r) => {
      ...
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} of area $${A}$ cm$^2$ undergoes a transformation with scale factor $${kTexOf(f, neg ? -1 : 1)}$ about a fixed centre. Find the area of the resulting copy, and state whether it is an enlargement or a reduction.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} berluas $${A}$ cm$^2$ menjalani satu transformasi dengan faktor skala $${kTexOf(f, neg ? -1 : 1)}$ pada satu pusat tetap. Cari luas salinan yang terhasil, dan nyatakan sama ada ia pembesaran atau pengecilan.`), a: T(`Area $= ${A} \\times \\left(${kTexOf(f, neg ? -1 : 1)}\\right)^2 = ${n(A * k * k)}$ cm$^2$; ${Math.abs(k) > 1 ? 'enlargement' : 'reduction'}`, `Luas $= ${A} \\times \\left(${kTexOf(f, neg ? -1 : 1)}\\right)^2 = ${n(A * k * k)}$ cm$^2$; ${Math.abs(k) > 1 ? 'pembesaran' : 'pengecilan'}`), w: W(T(`Area is multiplied by $k^2$ (always positive): $\\left(${kTexOf(f, neg ? -1 : 1)}\\right)^2 = ${Fr.tex(areaFrOf(f))}$`, `Luas didarab dengan $k^2$ (sentiasa positif): $\\left(${kTexOf(f, neg ? -1 : 1)}\\right)^2 = ${Fr.tex(areaFrOf(f))}$`), `$${A} \\times ${Fr.tex(areaFrOf(f))} = ${n(A * k * k)}$`, T(`$|k| ${Math.abs(k) > 1 ? '> 1$: enlargement' : '< 1$: reduction'}`, `$|k| ${Math.abs(k) > 1 ? '> 1$: pembesaran' : '< 1$: pengecilan'}`)), sp: 's' };
    },
    (r) => {
      ...
      return { q, a: T(`$${pt(I)}$`), w: W(ENLF, `$P' = ${enlLine(P, C, k, kTexOf(f, neg ? -1 : 1))}$`), sp: 's' };
    },
    (r) => {
      ...
      return { q: T(`Triangle $${x}${y}${z}$ has $${x} = ${pt(P)}$, $${y} = ${pt(Q)}$, $${z} = ${pt(R)}$. Find the coordinates of its image under an enlargement with centre ${cen.en} and scale factor $${n(kVal(f))}$.`, `Segi tiga $${x}${y}${z}$ mempunyai $${x} = ${pt(P)}$, $${y} = ${pt(Q)}$, $${z} = ${pt(R)}$. Cari koordinat imejnya di bawah pembesaran berpusat ${cen.ms} dan faktor skala $${n(kVal(f))}$.`), a: T(`$${x}'${pt(I[0])}, ${y}'${pt(I[1])}, ${z}'${pt(I[2])}$`), w: W(C[0] === 0 && C[1] === 0 ? T('Centre the origin: multiply each coordinate by $k$', 'Pusat pada asalan: darabkan setiap koordinat dengan $k$') : ENLF, ...[P, Q, R].map((p, i) => `$${[x, y, z][i]}' = ${enlLine(p, C, kVal(f))}$`)), sp: 'm' };
    },
    (r) => {
      const orig = r.pick([2, 3, 4, 5]), factor = r.pick([4, 9, 16, 25]);
      const ctx = r.pick(ECTX);
      return { q: T(`The area of ${ctx.en} is enlarged by a factor of $${factor}$. Find the scale factor of the (linear) enlargement, and the new length of a side that was originally $${orig}$ cm.`, `Luas ${ctx.ms} dibesarkan dengan faktor $${factor}$. Cari faktor skala pembesaran (linear) itu, dan panjang baharu bagi satu sisi yang asalnya $${orig}$ cm.`), a: T(`Scale factor $${Math.sqrt(factor)}$; new length $${orig * Math.sqrt(factor)}$ cm`, `Faktor skala $${Math.sqrt(factor)}$; panjang baharu $${orig * Math.sqrt(factor)}$ cm`), w: W(T(`Areas are multiplied by $k^2$: $k^2 = ${factor}$, $k = \\sqrt{${factor}} = ${Math.sqrt(factor)}$`, `Luas didarab dengan $k^2$: $k^2 = ${factor}$, $k = \\sqrt{${factor}} = ${Math.sqrt(factor)}$`), `$${orig} \\times ${Math.sqrt(factor)} = ${orig * Math.sqrt(factor)}$`), sp: 'm' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`A student says: "An object has area $${obj}$ cm$^2$. Under an enlargement with scale factor $${n(kVal(f))}$, the image area is $${n(wrongArea)}$ cm$^2$." Explain the student's error and give the correct image area.`, `Seorang murid berkata: "Sebuah objek berluas $${obj}$ cm$^2$. Di bawah pembesaran dengan faktor skala $${n(kVal(f))}$, luas imej ialah $${n(wrongArea)}$ cm$^2$." Terangkan kesilapan murid itu dan berikan luas imej yang betul.`), a: T(`The student multiplied the area by $k$ instead of $k^2$. Correct image area $= ${obj} \\times ${n(kVal(f))}^2 = ${n(rightArea)}$ cm$^2$.`, `Murid itu mendarab luas dengan $k$ dan bukannya $k^2$. Luas imej yang betul $= ${obj} \\times ${n(kVal(f))}^2 = ${n(rightArea)}$ cm$^2$.`), w: W(T('Lengths are multiplied by $k$, but areas by $k^2$ (both length and width are multiplied by $k$).', 'Panjang didarab dengan $k$, tetapi luas dengan $k^2$ (panjang dan lebar kedua-duanya didarab dengan $k$).'), `$${obj} \\times ${n(kVal(f))}^2 = ${obj} \\times ${n(kVal(f) * kVal(f))} = ${n(rightArea)}$`), sp: 'm' };
    },
    (r) => {
      const st = r.pick([
        [T('An enlargement with scale factor $k = 1$ maps every point to itself.', 'Pembesaran dengan faktor skala $k = 1$ memetakan setiap titik kepada dirinya sendiri.'), true, T('$C + 1(P - C) = P$, so every point stays where it is.', '$C + 1(P - C) = P$, jadi setiap titik kekal di tempatnya.')],
        [T('An enlargement with $0 < k < 1$ is called a reduction, and the image is smaller than the object.', 'Pembesaran dengan $0 < k < 1$ dipanggil pengecilan, dan imej lebih kecil daripada objek.'), true, T('Every length is multiplied by a number less than 1, so it becomes shorter.', 'Setiap panjang didarab dengan nombor kurang daripada 1, jadi ia menjadi lebih pendek.')],
        [T('The image under an enlargement is never similar to the object.', 'Imej di bawah pembesaran tidak pernah serupa dengan objek.'), false, T('An enlargement keeps all angles and multiplies all lengths by the same $k$, so the image is always similar to the object.', 'Pembesaran mengekalkan semua sudut dan mendarab semua panjang dengan $k$ yang sama, jadi imej sentiasa serupa dengan objek.')],
        [T('If the scale factor is negative, the image is on the opposite side of the centre from the object.', 'Jika faktor skala negatif, imej berada di sebelah pusat yang bertentangan dengan objek.'), true, T("$P' - C = k(P - C)$ with $k < 0$ points the opposite way from $C$.", "$P' - C = k(P - C)$ dengan $k < 0$ menghala ke arah bertentangan dari $C$.")],
    ...
    (r) => {
      ...
      const letters = 'ABCD';
      return { q: T(`An enlargement has centre $C = ${pt(C)}$ and scale factor $${n(kVal(f))}$. Which of the following is the correct image of $P = ${pt(P)}$? ${opts.map((o, i) => `(${letters[i]}) $${o}$`).join(', ')}`, `Satu pembesaran berpusat $C = ${pt(C)}$ dan berfaktor skala $${n(kVal(f))}$. Yang manakah imej yang betul bagi $P = ${pt(P)}$? ${opts.map((o, i) => `(${letters[i]}) $${o}$`).join(', ')}`), a: T(`(${letters[opts.indexOf(pt(correct))]}) $${pt(correct)}$`), w: W(ENLF, `$P' = ${enlLine(P, C, kVal(f))}$`, T('The wrong options come from adding $k(P - C)$ to $P$ instead of $C$, using $\\dfrac{1}{k}$, or ignoring the centre.', 'Pilihan yang salah terhasil daripada menambah $k(P - C)$ kepada $P$ dan bukan $C$, menggunakan $\\dfrac{1}{k}$, atau mengabaikan pusat.')), sp: 's' };
    },
    (r) => {
      const L = r.pick([4, 5, 6]), Wd = r.pick([3, 4, 5]), f = r.pick(KSET.slice(0, 2)), ctx = r.pick(ECTX);
      need(L !== Wd);
      const per = 2 * (L + Wd), newPer = per * kVal(f);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} is rectangular, $${L}$ cm by $${Wd}$ cm. It is enlarged with scale factor ${n(kVal(f))}. Find the perimeter of the original and of the enlarged copy.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} berbentuk segi empat tepat, $${L}$ cm dengan $${Wd}$ cm. Ia dibesarkan dengan faktor skala ${n(kVal(f))}. Cari perimeter yang asal dan salinan yang dibesarkan.`), a: T(`Original perimeter $${per}$ cm; enlarged perimeter $${n(newPer)}$ cm`, `Perimeter asal $${per}$ cm; perimeter yang dibesarkan $${n(newPer)}$ cm`), w: W(`$2(${L} + ${Wd}) = ${per}$`, T(`Perimeter is a length, so it is multiplied by $k$: $${per} \\times ${n(kVal(f))} = ${n(newPer)}$`, `Perimeter ialah panjang, jadi ia didarab dengan $k$: $${per} \\times ${n(kVal(f))} = ${n(newPer)}$`)), sp: 's' };
    },
    (r) => {
  ...
  const g52a = [
    ...
    (r) => {
      ...
      return { q: T(`Shape $X$ has area $${A1}$ cm$^2$. It is enlarged with scale factor ${k1} to give shape $Y$, and shape $Y$ is then enlarged with scale factor ${k2} to give shape $Z$. Find the area of $Y$, the area of $Z$, and the single scale factor that would enlarge $X$ directly to $Z$.`, `Bentuk $X$ mempunyai luas $${A1}$ cm$^2$. Ia dibesarkan dengan faktor skala ${k1} untuk menghasilkan bentuk $Y$, dan bentuk $Y$ kemudian dibesarkan dengan faktor skala ${k2} untuk menghasilkan bentuk $Z$. Cari luas $Y$, luas $Z$, dan faktor skala tunggal yang akan membesarkan $X$ terus kepada $Z$.`), a: T(`Area of $Y = ${A2}$ cm$^2$; area of $Z = ${A3}$ cm$^2$; overall scale factor $= ${k1} \\times ${k2} = ${overallK}$ (check: $${A1} \\times ${overallK}^2 = ${A3}$)`, `Luas $Y = ${A2}$ cm$^2$; luas $Z = ${A3}$ cm$^2$; faktor skala keseluruhan $= ${k1} \\times ${k2} = ${overallK}$ (semak: $${A1} \\times ${overallK}^2 = ${A3}$)`), w: W(T(`$Y$: $${A1} \\times ${k1}^2 = ${A2}$`, `$Y$: $${A1} \\times ${k1}^2 = ${A2}$`), T(`$Z$: $${A2} \\times ${k2}^2 = ${A3}$`, `$Z$: $${A2} \\times ${k2}^2 = ${A3}$`), T(`Lengths are multiplied by ${k1} and then by ${k2}: $${k1} \\times ${k2} = ${overallK}$`, `Panjang didarab dengan ${k1} dan kemudian dengan ${k2}: $${k1} \\times ${k2} = ${overallK}$`)), sp: 'm' };
    },
    (r) => {
      const f = r.pick(KSET), ctx = r.pick(ECTX);
  ...
  const ROTR = { 90: '(-y, x)', 180: '(-x, -y)', 270: '(y, -x)' };
  const rotRule = (t) => ROTR[((((t.dir === 'cw' ? -t.deg : t.deg) % 360) + 360) % 360)];
  const isO = (C) => !C || (C[0] === 0 && C[1] === 0);
  /** coordinate rule "(x, y) \to (…)" of t, or null (rotation/enlargement about a centre other than the origin) */
  const ruleOf = (t) => {
    if (t.kind === 'transl') return `(x, y) \\to (${poly([[1, 'x'], [t.v[0], '']])}, ${poly([[1, 'y'], [t.v[1], '']])})`;
    if (t.kind === 'reflX') return '(x, y) \\to (x, -y)';
    if (t.kind === 'reflY') return '(x, y) \\to (-x, y)';
    if (t.kind === 'reflV') return `(x, y) \\to (${poly([[-1, 'x'], [2 * t.k, '']])}, y)`;
    if (t.kind === 'reflH') return `(x, y) \\to (x, ${poly([[-1, 'y'], [2 * t.k, '']])})`;
    if (!isO(t.C)) return null;
    if (t.kind === 'rot') return `(x, y) \\to ${rotRule(t)}`;
    return `(x, y) \\to (${n(t.k)}x, ${n(t.k)}y)`;
  };
  /** one worked line: t applied to P (lab = optional prefix such as "$T_1$: ") */
  const stepW = (t, P, lab) => {
    const I = t.apply(P);
    lab = lab || '';
    if (t.kind === 'transl') return T(`${lab}$${pt(P)} \\to (${n(P[0])} + ${par(t.v[0])}, ${n(P[1])} + ${par(t.v[1])}) = ${pt(I)}$`);
    if (t.kind === 'enl') return T(`${lab}$${pt(P)} \\to ${enlLine(P, t.C, t.k)}$`);
    const rl = ruleOf(t);
    if (rl) return T(`${lab}$${rl}$: $${pt(P)} \\to ${pt(I)}$`);
    const d = [P[0] - t.C[0], P[1] - t.C[1]], d2 = [I[0] - t.C[0], I[1] - t.C[1]];
    return T(`${lab}relative to the centre $${pt(t.C)}$: $${pt(d)} \\to ${pt(d2)}$ by $(x, y) \\to ${rotRule(t)}$, so the image is $${pt(I)}$`, `${lab}relatif kepada pusat $${pt(t.C)}$: $${pt(d)} \\to ${pt(d2)}$ oleh $(x, y) \\to ${rotRule(t)}$, jadi imejnya ialah $${pt(I)}$`);
  };
  /** chain line "P → T1(P) → T2(T1(P))" */
  const chainW = (lab, P, ts) => { let Q = P; const s = [pt(P)]; for (const t of ts) { Q = t.apply(Q); s.push(pt(Q)); } return T(`${lab}$${s.join(' \\to ')}$`); };
  const rulesW = (T1, T2) => (ruleOf(T1) && ruleOf(T2) ? T(`$T_1$: $${ruleOf(T1)}$; $T_2$: $${ruleOf(T2)}$`) : null);
  const invW = (t) => (t.kind === 'transl' ? T('A translation moves every point by the same non-zero vector, so no point stays fixed.', 'Translasi menggerakkan setiap titik dengan vektor bukan sifar yang sama, jadi tiada titik yang kekal.') : t.kind.startsWith('refl') ? T('Points on the mirror line are their own images; every other point moves to the other side.', 'Titik pada garis cermin ialah imejnya sendiri; titik lain bergerak ke sebelah yang lain.') : t.kind === 'rot' ? T('Every point turns about the centre, so only the centre itself stays fixed.', 'Setiap titik berputar pada pusat, jadi hanya pusat itu sendiri yang kekal.') : T('$C + k(P - C) = P$ only when $P = C$ (since $k \\ne 1$), so only the centre stays fixed.', '$C + k(P - C) = P$ hanya apabila $P = C$ (kerana $k \\ne 1$), jadi hanya pusat yang kekal.'));
  const KINDW = { transl: T('Moving every point by the same vector is a translation.', 'Menggerakkan setiap titik dengan vektor yang sama ialah translasi.'), refl: T('Mirroring every point in a line is a reflection.', 'Memantulkan setiap titik pada satu garis ialah pantulan.'), rot: T('Turning every point through an angle about a centre is a rotation.', 'Memutarkan setiap titik melalui satu sudut pada satu pusat ialah putaran.'), enl: T('Multiplying distances from a centre by a scale factor is an enlargement.', 'Mendarab jarak dari satu pusat dengan faktor skala ialah pembesaran.') };
  const trKindW = (t) => KINDW[t.kind.startsWith('refl') ? 'refl' : t.kind];
  const CTX53 = [
  ...
  const g53e = [
    (r) => {
      ...
      return { q: T(`Name the type of isometric or size transformation described: "${T1.en}".`, `Namakan jenis transformasi isometri atau saiz yang diterangkan: "${T1.ms}".`), a: kindName, w: W(trKindW(T1)), sp: 'xs' };
    },
    (r) => {
      ...
      return { q: T(`A shape undergoes ${T1.en}. Does the size (area) of the shape change or stay the same under this transformation?`, `Sebuah bentuk mengalami ${T1.ms}. Adakah saiz (luas) bentuk itu berubah atau kekal sama di bawah transformasi ini?`), a: preserves, w: W(T1.kind === 'enl' ? T(`An enlargement multiplies lengths by $k$ and areas by $k^2 = ${n(T1.k * T1.k)}$.`, `Pembesaran mendarab panjang dengan $k$ dan luas dengan $k^2 = ${n(T1.k * T1.k)}$.`) : T('Translations, reflections and rotations are isometries: every length, and so the area, is unchanged.', 'Translasi, pantulan dan putaran ialah isometri: setiap panjang, dan oleh itu luas, tidak berubah.')), sp: 'xs' };
    },
    (r) => {
      ...
      return { q, a: T(`After $T_1$: $${pt(I1)}$; final image $${pt(I2)}$`, `Selepas $T_1$: $${pt(I1)}$; imej akhir $${pt(I2)}$`), w: W(stepW(T1, P, '$T_1$: '), stepW(T2, I1, '$T_2$: ')), sp: 's' };
    },
    (r) => {
      const P = [r.int(-4, 4), r.int(-4, 4)];
      const T1 = randTr(r, { lim: 4 }), T2 = randTr(r, { lim: 4 });
      const I1 = T1.apply(P), I2 = T2.apply(I1);
      return { q: T(`Define $T_1T_2(P)$ to mean "apply $T_1$ first, then apply $T_2$". Given $T_1$: ${T1.en}, and $T_2$: ${T2.en}, find $T_1T_2(P)$ for $P = ${pt(P)}$.`, `Takrifkan $T_1T_2(P)$ bermaksud "kenakan $T_1$ dahulu, kemudian kenakan $T_2$". Diberi $T_1$: ${T1.ms}, dan $T_2$: ${T2.ms}, cari $T_1T_2(P)$ bagi $P = ${pt(P)}$.`), a: T(`$${pt(I2)}$`), w: W(stepW(T1, P, '$T_1$: '), stepW(T2, I1, '$T_2$: ')), sp: 's' };
    },
    (r) => {
      const v1 = [r.nz(-4, 4), r.nz(-4, 4)], v2 = [r.nz(-4, 4), r.nz(-4, 4)];
      const P = [r.int(-3, 3), r.int(-3, 3)];
      const I2 = [P[0] + v1[0] + v2[0], P[1] + v1[1] + v2[1]];
      return { q: T(`$P = ${pt(P)}$ is mapped by translation $\\begin{pmatrix} ${v1[0]} \\\\ ${v1[1]} \\end{pmatrix}$ followed by translation $\\begin{pmatrix} ${v2[0]} \\\\ ${v2[1]} \\end{pmatrix}$. Find the final image, and the single translation that has the same effect.`, `$P = ${pt(P)}$ dipetakan oleh translasi $\\begin{pmatrix} ${v1[0]} \\\\ ${v1[1]} \\end{pmatrix}$ diikuti oleh translasi $\\begin{pmatrix} ${v2[0]} \\\\ ${v2[1]} \\end{pmatrix}$. Cari imej akhir, dan translasi tunggal yang memberi kesan yang sama.`), a: T(`$${pt(I2)}$; $\\begin{pmatrix} ${v1[0] + v2[0]} \\\\ ${v1[1] + v2[1]} \\end{pmatrix}$`), w: W(stepW(mkTransl(v1), P), stepW(mkTransl(v2), [P[0] + v1[0], P[1] + v1[1]]), T(`Single translation: add the vectors, $${tv(v1)} + ${tv(v2)} = ${tv([v1[0] + v2[0], v1[1] + v2[1]])}$`, `Translasi tunggal: tambah vektor, $${tv(v1)} + ${tv(v2)} = ${tv([v1[0] + v2[0], v1[1] + v2[1]])}$`)), sp: 's' };
    },
    (r) => {
      const t = randTr(r, { lim: 3, enl: true, rotC: [0, 0] });
      return { q: T(`State all the invariant point(s) (points that map to themselves) under ${t.en}.`, `Nyatakan semua titik-titik tak varian (titik yang dipetakan kepada dirinya sendiri) di bawah ${t.ms}.`), a: invPts(t), w: W(invW(t)), sp: 'xs' };
    },
    (r) => {
      ...
      return { q: T(`Let $T_1$: ${T1.en}, and $T_2$: ${T2.en}. For $P = ${pt(P)}$, find the image under $T_1$ followed by $T_2$, and the image under $T_2$ followed by $T_1$. Are the two results the same for this point?`, `Katakan $T_1$: ${T1.ms}, dan $T_2$: ${T2.ms}. Bagi $P = ${pt(P)}$, cari imej di bawah $T_1$ diikuti $T_2$, dan imej di bawah $T_2$ diikuti $T_1$. Adakah kedua-dua hasil ini sama bagi titik ini?`), a: T(`$T_1$ then $T_2$: $${pt(AB)}$; $T_2$ then $T_1$: $${pt(BA)}$. ${same ? 'Same for this point.' : 'Not the same.'}`, `$T_1$ kemudian $T_2$: $${pt(AB)}$; $T_2$ kemudian $T_1$: $${pt(BA)}$. ${same ? 'Sama bagi titik ini.' : 'Tidak sama.'}`), w: W(stepW(T1, P, '$T_1$: '), stepW(T2, T1.apply(P), '$T_2$: '), stepW(T2, P, '$T_2$: '), stepW(T1, T2.apply(P), '$T_1$: ')), sp: 's' };
    },
    (r) => {
      const P = [r.int(-4, 4), r.int(-4, 4)];
      ...
      return { q: T(`$P = ${pt(P)}$ is mapped to $P' = ${pt(I)}$ by a single isometric transformation. Given that the transformation is ${t.en}, name its type.`, `$P = ${pt(P)}$ dipetakan kepada $P' = ${pt(I)}$ oleh satu transformasi isometri tunggal. Diberi bahawa transformasi itu ialah ${t.ms}, namakan jenisnya.`), a: correctName, w: W(trKindW(t)), sp: 'xs' };
    },
  ];
  const g53m = [
    (r) => {
      ...
      return { q: T(sub(f.en, 'en'), sub(f.ms, 'ms')), a: T(`After $T_1$: $${pt(I1)}$; final image $${pt(I2)}$`, `Selepas $T_1$: $${pt(I1)}$; imej akhir $${pt(I2)}$`), w: W(stepW(T1, P, '$T_1$: '), stepW(T2, I1, '$T_2$: ')), sp: 's' };
    },
    (r) => {
      const C = [0, 0];
    ...
    (r) => {
      ...
      return { q: T(`$P = ${pt(P)}$ is rotated $${d1}^\\circ$ ${dir === 'cw' ? 'clockwise' : 'anticlockwise'} about the origin, and the image is then rotated $${d2}^\\circ$ ${dir === 'cw' ? 'clockwise' : 'anticlockwise'} about the origin. Find the final image, and the single rotation about the origin that has the same effect.`, `$P = ${pt(P)}$ diputarkan $${d1}^\\circ$ ${dir === 'cw' ? 'ikut arah jam' : 'lawan arah jam'} pada asalan, dan imejnya kemudian diputarkan $${d2}^\\circ$ ${dir === 'cw' ? 'ikut arah jam' : 'lawan arah jam'} pada asalan. Cari imej akhir, dan putaran tunggal pada asalan yang memberi kesan yang sama.`), a: T(`$${pt(I2)}$; a rotation of ${totalTxt.en}`, `$${pt(I2)}$; putaran ${totalTxt.ms}`), w: W(stepW(T1, P, '$T_1$: '), stepW(T2, I1, '$T_2$: '), T(`Same centre and direction, so the angles add: $${d1}^\\circ + ${d2}^\\circ = ${d1 + d2}^\\circ$${d1 + d2 === 360 ? ', a full turn' : ''}`, `Pusat dan arah yang sama, jadi sudut ditambah: $${d1}^\\circ + ${d2}^\\circ = ${d1 + d2}^\\circ$${d1 + d2 === 360 ? ', satu pusingan lengkap' : ''}`)), sp: 'm' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`A student describes a transformation only as "${incomplete.en}". Explain why this description is incomplete, and give the full, correct description if the actual transformation is ${t.en}.`, `Seorang murid menerangkan satu transformasi hanya sebagai "${incomplete.ms}". Terangkan mengapa penerangan ini tidak lengkap, dan berikan penerangan penuh yang betul jika transformasi sebenar ialah ${t.ms}.`), a: T(`A full description must also state ${missing.en}. Full description: ${t.en}.`, `Penerangan penuh mesti turut menyatakan ${missing.ms}. Penerangan penuh: ${t.ms}.`), w: W(T(`A ${incomplete.en} is only fully described when ${missing.en} ${kind.startsWith('refl') ? 'is' : 'are'} given; otherwise many different ${incomplete.en}s fit the description.`, `Satu ${incomplete.ms} hanya diterangkan dengan lengkap apabila ${missing.ms} diberi; jika tidak, banyak ${incomplete.ms} yang berbeza sesuai dengan penerangan itu.`)), sp: 'm' };
    },
    (r) => {
      const t = randTr(r, { lim: 3, line: true, enl: true, rotC: [r.int(-2, 2), r.int(-2, 2)] });
      return { q: T(`State all the invariant point(s) under ${t.en}.`, `Nyatakan semua titik tak varian di bawah ${t.ms}.`), a: invPts(t), w: W(invW(t)), sp: 's' };
    },
    (r) => {
      const P = [r.nz(-3, 3), r.nz(-3, 3)];
  ...
  const g53a = [
    ...
    (r) => {
      ...
      return { q: T(`Points $${x} = ${pt(P)}$ and $${y} = ${pt(Q)}$ are mapped to $${x}' = ${pt(I1P)}$ and $${y}' = ${pt(I1Q)}$ by transformation $T_1$, and then to $${x}'' = ${pt(I2P)}$ and $${y}'' = ${pt(I2Q)}$ by transformation $T_2$. Give the full description (type and all parameters) of $T_1$ and of $T_2$.`, `Titik $${x} = ${pt(P)}$ dan $${y} = ${pt(Q)}$ dipetakan kepada $${x}' = ${pt(I1P)}$ dan $${y}' = ${pt(I1Q)}$ oleh transformasi $T_1$, dan kemudian kepada $${x}'' = ${pt(I2P)}$ dan $${y}'' = ${pt(I2Q)}$ oleh transformasi $T_2$. Berikan penerangan penuh (jenis dan semua parameter) bagi $T_1$ dan bagi $T_2$.`), a: T(`$T_1$: ${T1.en}. $T_2$: ${T2.en}.`, `$T_1$: ${T1.ms}. $T_2$: ${T2.ms}.`), w: W(T('Compare each point with its image (change in coordinates, fixed lines, distances from a centre), then check the description on both points:', 'Bandingkan setiap titik dengan imejnya (perubahan koordinat, garis tetap, jarak dari pusat), kemudian semak penerangan itu pada kedua-dua titik:'), stepW(T1, P, `$T_1$, $${x}$: `), stepW(T1, Q, `$T_1$, $${y}$: `), stepW(T2, I1P, `$T_2$, $${x}'$: `), stepW(T2, I1Q, `$T_2$, $${y}'$: `)), sp: 'l' };
    },
    (r) => {
      const a = r.int(-3, 3), b = retry(() => { const v = r.int(-3, 3); need(v !== a); return v; });
      const P = [r.nz(-4, 4), r.nz(-4, 4)];
      ...
      return { q: T(`$P = ${pt(P)}$ is reflected in the line $x = ${a}$, and the image is then reflected in the line $y = ${b}$. Find the final image. Show that this combined transformation is equivalent to a single rotation, and find its centre and angle.`, `$P = ${pt(P)}$ dipantulkan pada garis $x = ${a}$, dan imejnya kemudian dipantulkan pada garis $y = ${b}$. Cari imej akhir. Tunjukkan bahawa gabungan transformasi ini setara dengan satu putaran tunggal, dan cari pusat serta sudutnya.`), a: T(`$${pt(I2)}$; equivalent to a rotation of $180^\\circ$ about $${pt([a, b])}$ (the intersection of the two mirror lines). Check: $${pt(I2)} = ${pt(check)}$.`, `$${pt(I2)}$; setara dengan putaran $180^\\circ$ pada $${pt([a, b])}$ (persilangan kedua-dua garis cermin). Semak: $${pt(I2)} = ${pt(check)}$.`), w: W(stepW(T1, P, '$x = ' + a + '$: '), stepW(T2, I1, '$y = ' + b + '$: '), T(`Combined: $(x, y) \\to (${poly([[-1, 'x'], [2 * a, '']])}, ${poly([[-1, 'y'], [2 * b, '']])})$, which is a $180^\\circ$ rotation about $${pt([a, b])}$, where the perpendicular mirror lines meet`, `Gabungan: $(x, y) \\to (${poly([[-1, 'x'], [2 * a, '']])}, ${poly([[-1, 'y'], [2 * b, '']])})$, iaitu putaran $180^\\circ$ pada $${pt([a, b])}$, tempat garis cermin yang berserenjang bersilang`)), sp: 'l' };
    },
    (r) => {
    ...
    (r) => {
      ...
      const AB = T2.apply(T1.apply(P)), BA = T1.apply(T2.apply(P));
      const same = AB[0] === BA[0] && AB[1] === BA[1];
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} starts at $P = ${pt(P)}$. It undergoes $T_1$: ${T1.en}, then $T_2$: ${T2.en}. (a) Find its final position. (b) State one invariant point of $T_2$, if any. (c) If the two steps had been applied in the opposite order ($T_2$ then $T_1$), would the final position be the same? Justify using calculation.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} bermula di $P = ${pt(P)}$. Ia mengalami $T_1$: ${T1.ms}, kemudian $T_2$: ${T2.ms}. (a) Cari kedudukan akhirnya. (b) Nyatakan satu titik tak varian $T_2$, jika ada. (c) Jika kedua-dua langkah itu dikenakan dalam tertib bertentangan ($T_2$ kemudian $T_1$), adakah kedudukan akhir akan sama? Wajarkan dengan pengiraan.`), a: T(`(a) $${pt(I2)}$ (b) ${invPts(T2).en} (c) $T_2$ then $T_1$ gives $${pt(BA)}$, ${same ? 'the same as (a) for this point' : 'different from (a), so the order matters here'}.`, `(a) $${pt(I2)}$ (b) ${invPts(T2).ms} (c) $T_2$ kemudian $T_1$ memberi $${pt(BA)}$, ${same ? 'sama seperti (a) bagi titik ini' : 'berbeza daripada (a), jadi tertib adalah penting di sini'}.`), w: W(stepW(T1, P, '(a) $T_1$: '), stepW(T2, I1, '(a) $T_2$: '), T(`(b) ${invW(T2).en}`, `(b) ${invW(T2).ms}`), stepW(T2, P, '(c) $T_2$: '), stepW(T1, T2.apply(P), '(c) $T_1$: ')), sp: 'l' };
    },
    (r) => {
      const a = r.int(-3, 3), b = retry(() => { const v = r.int(-3, 3); need(v !== a); return v; });
  ...
  const intLine = (p) => T(`${POLYNM[p].en[0].toUpperCase()}${POLYNM[p].en.slice(1)}: $180^\\circ - \\dfrac{360^\\circ}{${p}} = ${n(intAngle(p))}^\\circ$`, `${POLYNM[p].ms[0].toUpperCase()}${POLYNM[p].ms.slice(1)}: $180^\\circ - \\dfrac{360^\\circ}{${p}} = ${n(intAngle(p))}^\\circ$`);
  const intLines = (combo) => Object.keys(countBy(combo)).map((k) => intLine(+k));
  /** worked lines for the missing regular polygon whose interior angle is `remain` degrees */
  const missW = (remain, targetN) => [`$180^\\circ - \\dfrac{360^\\circ}{n} = ${n(remain)}^\\circ$`, `$\\dfrac{360^\\circ}{n} = ${n(180 - remain)}^\\circ$`, `$n = \\dfrac{360}{${n(180 - remain)}} = ${targetN}$`];
  ...
  const g54e = [
    (r) => {
      ...
      return { q: T(`Can identical ${POLYNM[p].en}s alone tessellate ${ctx.en}? Use the interior angle to explain.`, `Bolehkah ${POLYNM[p].ms} yang serupa sahaja bertesselasi pada ${ctx.ms}? Gunakan sudut pedalaman untuk menerangkan.`), a: ok ? T(`Yes: interior angle $${n(a)}^\\circ$, and $360 \\div ${n(a)} = ${360 / a}$, a whole number, so ${360 / a} tiles fit exactly round a point.`, `Ya: sudut pedalaman $${n(a)}^\\circ$, dan $360 \\div ${n(a)} = ${360 / a}$, nombor bulat, jadi ${360 / a} jubin muat tepat di sekeliling satu titik.`) : T(`No: interior angle $${n(a)}^\\circ$, and $360 \\div ${n(a)} = ${n(round(360 / a, 2))}$ is not a whole number, so gaps or overlaps remain.`, `Tidak: sudut pedalaman $${n(a)}^\\circ$, dan $360 \\div ${n(a)} = ${n(round(360 / a, 2))}$ bukan nombor bulat, jadi jurang atau pertindihan tetap ada.`), w: W(intLine(p), `$360 \\div ${n(a)} ${eq2(360 / a)}$`, T('A shape tessellates on its own only if its interior angle divides $360^\\circ$ exactly.', 'Suatu bentuk bertesselasi dengan sendirinya hanya jika sudut pedalamannya membahagi $360^\\circ$ dengan tepat.')), sp: 's' };
    },
    (r) => {
      const valid = r.chance(0.6);
      const combo = valid ? r.pick(VALID_COMBOS) : randInvalidCombo(r);
      const sum = combo.reduce((s, p) => s + intAngle(p), 0);
      return { q: T(`At a point, the following shapes are placed edge to edge: ${comboListTxt(combo, 'en')}. Do their interior angles add up to $360^\\circ$, so that they fit exactly with no gaps or overlaps?`, `Pada satu titik, bentuk berikut diletakkan bertemu tepi: ${comboListTxt(combo, 'ms')}. Adakah sudut pedalamannya berjumlah $360^\\circ$, supaya ia muat tepat tanpa jurang atau pertindihan?`), a: T(`$${comboAngles(combo)} = ${n(sum)}^\\circ$, ${sum === 360 ? 'which is exactly $360^\\circ$: yes, they fit.' : `which is ${sum < 360 ? 'less than' : 'more than'} $360^\\circ$: no, ${sum < 360 ? 'a gap' : 'an overlap'} remains.`}`, `$${comboAngles(combo)} = ${n(sum)}^\\circ$, ${sum === 360 ? 'iaitu tepat $360^\\circ$: ya, ia muat.' : `iaitu ${sum < 360 ? 'kurang daripada' : 'lebih daripada'} $360^\\circ$: tidak, ${sum < 360 ? 'jurang' : 'pertindihan'} tetap ada.`}`), w: W(...intLines(combo), `$${comboAngles(combo)} = ${n(sum)}^\\circ$`), sp: 's' };
    },
    (r) => {
      const st = r.pick([
        [T('A tessellation must cover the plane with no gaps and no overlaps.', 'Satu tesselasi mesti melitupi satah tanpa jurang dan tanpa pertindihan.'), true, T('This is the definition of a tessellation.', 'Ini ialah takrif tesselasi.')],
        [T('A tessellation is built by repeating a fundamental (basic) tile using translations, rotations and/or reflections.', 'Satu tesselasi dibina dengan mengulang satu jubin asas (fundamental) menggunakan translasi, putaran dan/atau pantulan.'), true, T('These isometries give congruent copies of the tile.', 'Isometri ini memberikan salinan jubin yang kongruen.')],
        [T('Enlargement can be used to generate the repeated copies in a tessellation.', 'Pembesaran boleh digunakan untuk menjana salinan berulang dalam satu tesselasi.'), false, T('An enlargement changes the size, so the copies would not be congruent to the tile.', 'Pembesaran mengubah saiz, jadi salinan itu tidak kongruen dengan jubin.')],
        [T('A regular pentagon on its own can tessellate the plane.', 'Sebuah pentagon sekata dengan sendirinya boleh bertesselasi pada satah.'), false, '$180^\\circ - \\dfrac{360^\\circ}{5} = 108^\\circ$, $360 \\div 108 \\approx 3.33$'],
        [T('A regular hexagon on its own can tessellate the plane.', 'Sebuah heksagon sekata dengan sendirinya boleh bertesselasi pada satah.'), true, '$180^\\circ - \\dfrac{360^\\circ}{6} = 120^\\circ$, $360 \\div 120 = 3$'],
    ...
    (r) => {
      ...
      return { q: T(`Which one of the following regular shapes CANNOT tessellate the plane on its own? ${list}`, `Bentuk sekata manakah antara berikut TIDAK BOLEH bertesselasi pada satah dengan sendirinya? ${listms}`), a: T(`(${letters[badIdx]}) ${POLYNM[badN].en}`, `(${letters[badIdx]}) ${POLYNM[badN].ms}`), w: W(T('Interior angles $60^\\circ$, $90^\\circ$, $120^\\circ$ divide $360^\\circ$ exactly ($6$, $4$, $3$ copies).', 'Sudut pedalaman $60^\\circ$, $90^\\circ$, $120^\\circ$ membahagi $360^\\circ$ dengan tepat ($6$, $4$, $3$ salinan).'), intLine(badN), `$360 \\div ${n(intAngle(badN))} ${eq2(360 / intAngle(badN))}$`), sp: 's' };
    },
    (r) => {
      const item = r.pick([
        { pat: T('a row of identical bricks, each shifted to the right of the previous one by the same distance', 'satu baris bata yang serupa, setiap satu dianjak ke kanan daripada yang sebelumnya dengan jarak yang sama'), ans: T('translation', 'translasi'), why: T('Each copy is the previous one slid by the same distance in the same direction, without turning or flipping.', 'Setiap salinan ialah yang sebelumnya digelongsor dengan jarak yang sama pada arah yang sama, tanpa diputar atau dibalikkan.') },
        { pat: T('a strip of identical footprints, each a mirror image of the one before it, alternating left and right', 'satu jalur tapak kaki yang serupa, setiap satu imej cermin bagi yang sebelumnya, berselang-seli kiri dan kanan'), ans: T('reflection', 'pantulan'), why: T('Each copy is flipped (a mirror image) of the one before it.', 'Setiap salinan ialah imej cermin (dibalikkan) bagi yang sebelumnya.') },
        { pat: T('a pinwheel pattern where each identical blade is turned by the same angle about the centre', 'satu corak kincir angin di mana setiap bilah yang serupa diputar dengan sudut yang sama pada pusat'), ans: T('rotation', 'putaran'), why: T('Each copy is turned through the same angle about a fixed centre.', 'Setiap salinan diputar melalui sudut yang sama pada satu pusat tetap.') },
      ]);
      return { q: T(`In ${item.pat.en}, which single isometric transformation relates each tile to the next?`, `Dalam ${item.pat.ms}, transformasi isometri tunggal manakah yang mengaitkan setiap jubin dengan yang seterusnya?`), a: item.ans, w: W(item.why), sp: 's' };
    },
  ];
  const g54m = [
    (r) => {
      ...
      return { q: T(`At a single point, regular polygons meet edge to edge with no gaps or overlaps: ${comboListTxt(combo, 'en')}. Verify this using the interior angles, and state how many different types of regular polygon are used.`, `Pada satu titik, poligon sekata bertemu bertepi tanpa jurang atau pertindihan: ${comboListTxt(combo, 'ms')}. Sahkan ini menggunakan sudut pedalaman, dan nyatakan berapa jenis poligon sekata yang berbeza digunakan.`), a: T(`$${comboAngles(combo)} = 360^\\circ$, confirmed; ${keys.length} type${keys.length > 1 ? 's' : ''} of polygon.`, `$${comboAngles(combo)} = 360^\\circ$, disahkan; ${keys.length} jenis poligon.`), w: W(...intLines(combo), `$${comboAngles(combo)} = 360^\\circ$`), sp: 's' };
    },
    (r) => {
  ...
  const FNM = { sin: T('sine', 'sinus'), cos: T('cosine', 'kosinus'), tan: T('tangent', 'tangen') };
  const quadW = (t) => { const q = quadOf(t), lo = 90 * (q - 1); return T(`$${lo}^\\circ < ${t}^\\circ < ${lo + 90}^\\circ$, so quadrant ${QROMAN[q]}`, `$${lo}^\\circ < ${t}^\\circ < ${lo + 90}^\\circ$, jadi sukuan ${QROMAN[q]}`); };
  ...
  const refW = (t) => T(`Reference angle $= ${refTex(t)}$`, `Sudut rujukan $= ${refTex(t)}$`);
  /** angle in quadrant q with reference angle a, as TeX */
  const fromRef = (a, q) => (q === 1 ? `${a}^\\circ` : q === 2 ? `180^\\circ - ${a}^\\circ = ${180 - a}^\\circ` : q === 3 ? `180^\\circ + ${a}^\\circ = ${180 + a}^\\circ` : `360^\\circ - ${a}^\\circ = ${360 - a}^\\circ`);
  const signLine = (fn, q) => { const s = SGN[fn][q - 1] > 0; return T(`By ASTC, ${FNM[fn].en} is ${s ? 'positive' : 'negative'} in quadrant ${QROMAN[q]}`, `Mengikut ASTC, ${FNM[fn].ms} ${s ? 'positif' : 'negatif'} dalam sukuan ${QROMAN[q]}`); };
  /** worked lines for the exact value of fn t (t not quadrantal) */
  const exW = (fn, t) => { const q = quadOf(t), a = refAngle(t); return [quadW(t), refW(t), signLine(fn, q), `$\\${fn}\\,${t}^\\circ = ${SGN[fn][q - 1] < 0 ? '-' : ''}\\${fn}\\,${a}^\\circ = ${exactTex(fn, t)}$`]; };
  /** worked lines for solving fn(theta) = ±table value, 0..360 */
  const solW = (fn, ref, sign, a1, a2) => {
    const table = fn === 'sin' ? SIN30 : fn === 'cos' ? COS30 : TAN30;
    return [T(`Reference angle: $\\${fn}\\,${ref}^\\circ = ${table[IDX30[ref]]}$, so it is $${ref}^\\circ$`, `Sudut rujukan: $\\${fn}\\,${ref}^\\circ = ${table[IDX30[ref]]}$, jadi ia ialah $${ref}^\\circ$`), T(`${FNM[fn].en[0].toUpperCase()}${FNM[fn].en.slice(1)} is ${sign > 0 ? 'positive' : 'negative'} in quadrants ${QROMAN[quadOf(a1)]} and ${QROMAN[quadOf(a2)]}`, `${FNM[fn].ms[0].toUpperCase()}${FNM[fn].ms.slice(1)} ${sign > 0 ? 'positif' : 'negatif'} dalam sukuan ${QROMAN[quadOf(a1)]} dan ${QROMAN[quadOf(a2)]}`), `$\\theta = ${fromRef(ref, quadOf(a1))}$, $\\theta = ${fromRef(ref, quadOf(a2))}$`];
  };
  const UPT = { 0: [1, 0], 90: [0, 1], 180: [-1, 0], 270: [0, -1], 360: [1, 0] };
  const tpW = (x, y, h) => [`$r = \\sqrt{(${x})^2 + (${y})^2} = \\sqrt{${h * h}} = ${h}$`, T('$\\sin\\theta = \\dfrac{y}{r}$, $\\cos\\theta = \\dfrac{x}{r}$, $\\tan\\theta = \\dfrac{y}{x}$', '$\\sin\\theta = \\dfrac{y}{r}$, $\\cos\\theta = \\dfrac{x}{r}$, $\\tan\\theta = \\dfrac{y}{x}$')];
  const ASTCW = T('ASTC: quadrant I all positive, II sine only, III tangent only, IV cosine only.', 'ASTC: sukuan I semua positif, II sinus sahaja, III tangen sahaja, IV kosinus sahaja.');
  ...
  const g61e = [
    (r) => {
      ...
      const f = r.pick(forms);
      return { q: T(f.en.replace('{t}', String(t)), f.ms.replace('{t}', String(t))), a: T(`Quadrant ${QROMAN[quadOf(t)]}; reference angle $${refAngle(t)}^\\circ$`, `Sukuan ${QROMAN[quadOf(t)]}; sudut rujukan $${refAngle(t)}^\\circ$`), w: W(quadW(t), refW(t)), sp: 's' };
    },
    (r) => {
      ...
      const ang = +c[0].match(/(\d+)\^/)[1], U = UPT[ang];
      const w = W(T(`On the unit circle, $${ang}^\\circ$ gives the point $(x, y) = ${pt(U)}$; $\\sin\\theta = y$, $\\cos\\theta = x$, $\\tan\\theta = \\dfrac{y}{x}$`, `Pada bulatan unit, $${ang}^\\circ$ memberikan titik $(x, y) = ${pt(U)}$; $\\sin\\theta = y$, $\\cos\\theta = x$, $\\tan\\theta = \\dfrac{y}{x}$`), c[1] === null ? T(`$${c[0]} = \\dfrac{${U[1]}}{${U[0]}}$: division by zero, so not defined`, `$${c[0]} = \\dfrac{${U[1]}}{${U[0]}}$: pembahagian dengan sifar, jadi tidak ditakrifkan`) : `$${c[0]} = ${c[1]}$`);
      return { q: T(`Write down the value of $${c[0]}$.`, `Tuliskan nilai $${c[0]}$.`), a: c[1] === null ? T('not defined', 'tidak ditakrifkan') : T(`$${c[1]}$`), w, sp: 'xs' };
    },
    (r) => {
      const fn = r.pick(['sin', 'cos', 'tan']), q = r.int(1, 4);
      const sgn = SGN[fn][q - 1];
      return { q: T(`Is $\\${fn}\\,\\theta$ positive or negative when $\\theta$ is in quadrant ${QROMAN[q]}?`, `Adakah $\\${fn}\\,\\theta$ positif atau negatif apabila $\\theta$ berada dalam sukuan ${QROMAN[q]}?`), a: sgn > 0 ? T('Positive', 'Positif') : T('Negative', 'Negatif'), w: W(ASTCW, signLine(fn, q)), sp: 'xs' };
    },
    (r) => {
      const q = r.int(1, 4);
      return { q: T(`Using the ASTC rule, which trigonometric ratio(s) are positive when $\\theta$ is in quadrant ${QROMAN[q]}?`, `Menggunakan peraturan ASTC, nisbah trigonometri manakah yang positif apabila $\\theta$ berada dalam sukuan ${QROMAN[q]}?`), a: ASTC[q], w: W(ASTCW), sp: 'xs' };
    },
    (r) => {
      const q = r.int(2, 4), ref = r.pick([30, 45, 60]);
      const t = q === 2 ? 180 - ref : q === 3 ? 180 + ref : 360 - ref;
      return { q: T(`An angle $\\theta$ in quadrant ${QROMAN[q]} has a reference angle of $${ref}^\\circ$. Find $\\theta$.`, `Satu sudut $\\theta$ dalam sukuan ${QROMAN[q]} mempunyai sudut rujukan $${ref}^\\circ$. Cari $\\theta$.`), a: T(`$${t}^\\circ$`), w: W(T(`In quadrant ${QROMAN[q]}: $\\theta = ${fromRef(ref, q)}$`, `Dalam sukuan ${QROMAN[q]}: $\\theta = ${fromRef(ref, q)}$`)), sp: 's' };
    },
    (r) => {
      const st = r.pick([
        [T('The reference angle is always measured from the $x$-axis to the terminal arm of $\\theta$.', 'Sudut rujukan sentiasa diukur daripada paksi-$x$ ke lengan penamat $\\theta$.'), true, T('The reference angle is the acute angle between the terminal arm and the $x$-axis.', 'Sudut rujukan ialah sudut tirus antara lengan penamat dengan paksi-$x$.')],
        [T('$\\tan 90^\\circ = 0$.', '$\\tan 90^\\circ = 0$.'), false, T('$\\tan 90^\\circ = \\dfrac{\\sin 90^\\circ}{\\cos 90^\\circ} = \\dfrac{1}{0}$: not defined.', '$\\tan 90^\\circ = \\dfrac{\\sin 90^\\circ}{\\cos 90^\\circ} = \\dfrac{1}{0}$: tidak ditakrifkan.')],
        [T('$\\tan 90^\\circ$ is not defined.', '$\\tan 90^\\circ$ tidak ditakrifkan.'), true, T('$\\tan 90^\\circ = \\dfrac{\\sin 90^\\circ}{\\cos 90^\\circ} = \\dfrac{1}{0}$: division by zero.', '$\\tan 90^\\circ = \\dfrac{\\sin 90^\\circ}{\\cos 90^\\circ} = \\dfrac{1}{0}$: pembahagian dengan sifar.')],
        [T('In quadrant III, both $\\sin\\theta$ and $\\cos\\theta$ are negative.', 'Dalam sukuan III, kedua-dua $\\sin\\theta$ dan $\\cos\\theta$ adalah negatif.'), true, T('In quadrant III, $x < 0$ and $y < 0$; only tangent is positive (ASTC).', 'Dalam sukuan III, $x < 0$ dan $y < 0$; hanya tangen positif (ASTC).')],
        [T('In quadrant IV, $\\cos\\theta$ is negative.', 'Dalam sukuan IV, $\\cos\\theta$ adalah negatif.'), false, T('In quadrant IV, $x > 0$, so $\\cos\\theta$ is positive (the C in ASTC).', 'Dalam sukuan IV, $x > 0$, jadi $\\cos\\theta$ positif (C dalam ASTC).')],
        [T('ASTC stands for the ratios that are positive in quadrants I, II, III, IV respectively: All, Sine, Tangent, Cosine.', 'ASTC mewakili nisbah yang positif dalam sukuan I, II, III, IV masing-masing: All (semua), Sine (sinus), Tangent (tangen), Cosine (kosinus).'), true, ASTCW],
        [T('The reference angle of $200^\\circ$ is $200^\\circ - 180^\\circ = 20^\\circ$.', 'Sudut rujukan bagi $200^\\circ$ ialah $200^\\circ - 180^\\circ = 20^\\circ$.'), true, T('$200^\\circ$ is in quadrant III, where the reference angle is $\\theta - 180^\\circ$.', '$200^\\circ$ berada dalam sukuan III, dengan sudut rujukan $\\theta - 180^\\circ$.')],
        [T('The reference angle of $200^\\circ$ is $360^\\circ - 200^\\circ = 160^\\circ$.', 'Sudut rujukan bagi $200^\\circ$ ialah $360^\\circ - 200^\\circ = 160^\\circ$.'), false, T('$200^\\circ$ is in quadrant III, so the reference angle is $200^\\circ - 180^\\circ = 20^\\circ$; a reference angle is never more than $90^\\circ$.', '$200^\\circ$ berada dalam sukuan III, jadi sudut rujukan ialah $200^\\circ - 180^\\circ = 20^\\circ$; sudut rujukan tidak pernah melebihi $90^\\circ$.')],
        [T('The reference angle of $320^\\circ$ is $360^\\circ - 320^\\circ = 40^\\circ$.', 'Sudut rujukan bagi $320^\\circ$ ialah $360^\\circ - 320^\\circ = 40^\\circ$.'), true, T('$320^\\circ$ is in quadrant IV, where the reference angle is $360^\\circ - \\theta$.', '$320^\\circ$ berada dalam sukuan IV, dengan sudut rujukan $360^\\circ - \\theta$.')],
        [T('An angle of $95^\\circ$ lies in quadrant I.', 'Satu sudut $95^\\circ$ berada dalam sukuan I.'), false, T('$90^\\circ < 95^\\circ < 180^\\circ$, so quadrant II.', '$90^\\circ < 95^\\circ < 180^\\circ$, jadi sukuan II.')],
        [T('An angle of $95^\\circ$ lies in quadrant II.', 'Satu sudut $95^\\circ$ berada dalam sukuan II.'), true, T('$90^\\circ < 95^\\circ < 180^\\circ$, so quadrant II.', '$90^\\circ < 95^\\circ < 180^\\circ$, jadi sukuan II.')],
        [T('The reference angle can never be greater than $90^\\circ$.', 'Sudut rujukan tidak boleh melebihi $90^\\circ$.'), true, T('It is the acute angle to the $x$-axis, so it is between $0^\\circ$ and $90^\\circ$.', 'Ia ialah sudut tirus ke paksi-$x$, jadi ia antara $0^\\circ$ dan $90^\\circ$.')],
        [T('$\\cos\\theta$ is positive in both quadrant I and quadrant IV.', '$\\cos\\theta$ adalah positif dalam kedua-dua sukuan I dan sukuan IV.'), true, T('$\\cos\\theta$ has the sign of $x$, and $x > 0$ in quadrants I and IV.', '$\\cos\\theta$ mempunyai tanda $x$, dan $x > 0$ dalam sukuan I dan IV.')],
        [T('$\\sin\\theta$ is positive in both quadrant I and quadrant III.', '$\\sin\\theta$ adalah positif dalam kedua-dua sukuan I dan sukuan III.'), false, T('$\\sin\\theta$ has the sign of $y$, and $y < 0$ in quadrant III.', '$\\sin\\theta$ mempunyai tanda $y$, dan $y < 0$ dalam sukuan III.')],
    ...
    (r) => {
      ...
      return { q: T(`If $\\sin\\theta ${signs[0] > 0 ? '> 0' : '< 0'}$ and $\\cos\\theta ${signs[1] > 0 ? '> 0' : '< 0'}$, in which quadrant does $\\theta$ lie?`, `Jika $\\sin\\theta ${signs[0] > 0 ? '> 0' : '< 0'}$ dan $\\cos\\theta ${signs[1] > 0 ? '> 0' : '< 0'}$, dalam sukuan manakah $\\theta$ berada?`), a: T(`Quadrant ${signs[2]}`, `Sukuan ${signs[2]}`), w: W(T(`$\\sin\\theta$ has the sign of $y$ and $\\cos\\theta$ the sign of $x$: $y ${signs[0] > 0 ? '> 0' : '< 0'}$, $x ${signs[1] > 0 ? '> 0' : '< 0'}$, which is quadrant ${signs[2]}`, `$\\sin\\theta$ mempunyai tanda $y$ dan $\\cos\\theta$ tanda $x$: $y ${signs[0] > 0 ? '> 0' : '< 0'}$, $x ${signs[1] > 0 ? '> 0' : '< 0'}$, iaitu sukuan ${signs[2]}`)), sp: 's' };
    },
    (r) => {
      const angs = r.sample([50, 130, 220, 310], 4);
      const quads = angs.map(quadOf);
      return { q: T(`Match each angle to the quadrant it lies in: angles $${angs.map((a) => a + '^\\circ').join(', ')}$; quadrants I, II, III, IV.`, `Padankan setiap sudut dengan sukuan tempat ia berada: sudut $${angs.map((a) => a + '^\\circ').join(', ')}$; sukuan I, II, III, IV.`), a: T(angs.map((a, i) => `$${a}^\\circ \\to$ ${QROMAN[quads[i]]}`).join(', ')), w: W(...angs.map(quadW)), sp: 's' };
    },
    (r) => {
      const sx = r.sign(), sy = r.sign();
      const q = sx > 0 && sy > 0 ? 1 : sx < 0 && sy > 0 ? 2 : sx < 0 && sy < 0 ? 3 : 4;
      const x = sx * r.int(2, 8), y = sy * r.int(2, 8);
      return { q: T(`The terminal arm of angle $\\theta$ (measured from the positive $x$-axis) passes through the point $(${x}, ${y})$. State the quadrant in which $\\theta$ lies.`, `Lengan penamat sudut $\\theta$ (diukur daripada paksi-$x$ positif) melalui titik $(${x}, ${y})$. Nyatakan sukuan tempat $\\theta$ berada.`), a: T(`Quadrant ${QROMAN[q]}`, `Sukuan ${QROMAN[q]}`), w: W(T(`$x = ${x} ${x > 0 ? '> 0' : '< 0'}$ and $y = ${y} ${y > 0 ? '> 0' : '< 0'}$: quadrant ${QROMAN[q]}`, `$x = ${x} ${x > 0 ? '> 0' : '< 0'}$ dan $y = ${y} ${y > 0 ? '> 0' : '< 0'}$: sukuan ${QROMAN[q]}`)), sp: 'xs' };
    },
    (r) => {
      const ctx = r.pick(ANGCTX), ang = r.pick([120, 200, 250, 320, 60, 160]);
      const fn = r.pick(['sin', 'cos']);
      const sgn = SGN[fn][quadOf(ang) - 1];
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} turns through an angle of $${ang}^\\circ$ from its starting position. Is $\\${fn}\\,${ang}^\\circ$ positive or negative?`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} berputar melalui sudut $${ang}^\\circ$ daripada kedudukan permulaannya. Adakah $\\${fn}\\,${ang}^\\circ$ positif atau negatif?`), a: sgn > 0 ? T('Positive', 'Positif') : T('Negative', 'Negatif'), w: W(quadW(ang), signLine(fn, quadOf(ang))), sp: 'xs' };
    },
    (r) => {
      const ctx = r.pick(ANGCTX), ang = r.pick([25, 100, 200, 250, 300, 330, 145, 80, 170, 190, 350]);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} turns through an angle of $${ang}^\\circ$ from its starting position. In which quadrant does this angle lie?`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} berputar melalui sudut $${ang}^\\circ$ daripada kedudukan permulaannya. Dalam sukuan manakah sudut ini berada?`), a: T(`Quadrant ${QROMAN[quadOf(ang)]}`, `Sukuan ${QROMAN[quadOf(ang)]}`), w: W(quadW(ang)), sp: 'xs' };
    },
    (r) => {
      const ctx = r.pick(ANGCTX), q = r.int(1, 4);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} has turned so that it is now in quadrant ${QROMAN[q]}. Using ASTC, which trigonometric ratio(s) are positive at this position?`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} telah berputar supaya ia kini berada dalam sukuan ${QROMAN[q]}. Menggunakan ASTC, nisbah trigonometri manakah yang positif pada kedudukan ini?`), a: ASTC[q], w: W(ASTCW), sp: 'xs' };
    },
  ];
  const g61m = [
    (r) => {
      ...
      return { q, a: T(`$${exactTex(fn, t)}$`), w: W(...exW(fn, t)), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`Given $\\${fn}\\,\\theta = ${val}$ and $\\theta$ is in quadrant ${QROMAN[q]}, find $\\theta$ for $0^\\circ \\le \\theta \\le 360^\\circ$.`, `Diberi $\\${fn}\\,\\theta = ${val}$ dan $\\theta$ berada dalam sukuan ${QROMAN[q]}, cari $\\theta$ bagi $0^\\circ \\le \\theta \\le 360^\\circ$.`), a: T(`$\\theta = ${theta}^\\circ$`), w: W(T(`$\\${fn}\\,${ref}^\\circ = ${exactTex(fn, ref)}$, so the reference angle is $${ref}^\\circ$`, `$\\${fn}\\,${ref}^\\circ = ${exactTex(fn, ref)}$, jadi sudut rujukan ialah $${ref}^\\circ$`), T(`In quadrant ${QROMAN[q]}: $\\theta = ${fromRef(ref, q)}$`, `Dalam sukuan ${QROMAN[q]}: $\\theta = ${fromRef(ref, q)}$`)), sp: 'm' };
    },
    (r) => {
      ...
      return { q: T(`A student evaluates $\\${fn}\\,${t}^\\circ = ${shown}$. Is this correct? If not, explain the error and give the correct value.`, `Seorang murid menilai $\\${fn}\\,${t}^\\circ = ${shown}$. Adakah ini betul? Jika tidak, terangkan kesilapan itu dan berikan nilai yang betul.`), a: wrong ? T(`No: the sign is wrong. In quadrant ${QROMAN[quadOf(t)]}, $\\${fn}\\,\\theta$ is ${SGN[fn][quadOf(t) - 1] > 0 ? 'positive' : 'negative'}; correct value $${correct}$.`, `Tidak: tandanya salah. Dalam sukuan ${QROMAN[quadOf(t)]}, $\\${fn}\\,\\theta$ adalah ${SGN[fn][quadOf(t) - 1] > 0 ? 'positif' : 'negatif'}; nilai yang betul $${correct}$.`) : T('Yes, this is correct.', 'Ya, ini betul.'), w: W(...exW(fn, t)), sp: 'm' };
    },
    (r) => {
      ...
      need(opts.length >= 3);
      const letters = 'ABC';
      return { q: T(`Which of the following is the exact value of $\\${fn}\\,${opt}^\\circ$? ${opts.map((o, i) => `(${letters[i]}) $${o}$`).join(', ')}`, `Yang manakah nilai tepat bagi $\\${fn}\\,${opt}^\\circ$? ${opts.map((o, i) => `(${letters[i]}) $${o}$`).join(', ')}`), a: T(`(${letters[opts.indexOf(correct)]}) $${correct}$`), w: W(...exW(fn, opt)), sp: 's' };
    },
    (r) => {
    ...
    (r) => {
      ...
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} has turned through $${t}^\\circ$ from its starting position. Find the exact value of $\\${fn}\\,${t}^\\circ$, showing the reference angle and quadrant used.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} telah berputar melalui $${t}^\\circ$ daripada kedudukan permulaannya. Cari nilai tepat bagi $\\${fn}\\,${t}^\\circ$, tunjukkan sudut rujukan dan sukuan yang digunakan.`), a: T(`$${exactTex(fn, t)}$`), w: W(...exW(fn, t)), sp: 's' };
    },
    (r) => {
      const ctx = r.pick(ANGCTX), q = r.int(2, 4), ref = r.pick([30, 45, 60]);
      const t = q === 2 ? 180 - ref : q === 3 ? 180 + ref : 360 - ref;
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} rotates into quadrant ${QROMAN[q]}, reaching a position whose reference angle is $${ref}^\\circ$. Through what total angle (from $0^\\circ$ to $360^\\circ$) has it turned?`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} berputar ke sukuan ${QROMAN[q]}, mencapai kedudukan yang sudut rujukannya ialah $${ref}^\\circ$. Melalui sudut jumlah berapakah (dari $0^\\circ$ hingga $360^\\circ$) ia telah berputar?`), a: T(`$${t}^\\circ$`), w: W(T(`In quadrant ${QROMAN[q]}: $\\theta = ${fromRef(ref, q)}$`, `Dalam sukuan ${QROMAN[q]}: $\\theta = ${fromRef(ref, q)}$`)), sp: 's' };
    },
    (r) => {
      const [p, q, h] = r.pick(RT3s), sx = r.sign(), sy = r.sign();
      const x = sx * p, y = sy * q;
      const quad = sx > 0 && sy > 0 ? 1 : sx < 0 && sy > 0 ? 2 : sx < 0 && sy < 0 ? 3 : 4;
      return { q: T(`The terminal arm of angle $\\theta$ passes through the point $(${x}, ${y})$. Find $\\sin\\theta$, $\\cos\\theta$ and $\\tan\\theta$.`, `Lengan penamat sudut $\\theta$ melalui titik $(${x}, ${y})$. Cari $\\sin\\theta$, $\\cos\\theta$ dan $\\tan\\theta$.`), a: T(`$r = \\sqrt{(${x})^2 + (${y})^2} = ${h}$; $\\sin\\theta = ${sy < 0 ? '-' : ''}\\dfrac{${q}}{${h}}$, $\\cos\\theta = ${sx < 0 ? '-' : ''}\\dfrac{${p}}{${h}}$, $\\tan\\theta = ${SGN.tan[quad - 1] < 0 ? '-' : ''}\\dfrac{${q}}{${p}}$`, `$r = \\sqrt{(${x})^2 + (${y})^2} = ${h}$; $\\sin\\theta = ${sy < 0 ? '-' : ''}\\dfrac{${q}}{${h}}$, $\\cos\\theta = ${sx < 0 ? '-' : ''}\\dfrac{${p}}{${h}}$, $\\tan\\theta = ${SGN.tan[quad - 1] < 0 ? '-' : ''}\\dfrac{${q}}{${p}}$`), w: W(...tpW(x, y, h), `$\\sin\\theta = \\dfrac{${y}}{${h}}$, $\\cos\\theta = \\dfrac{${x}}{${h}}$, $\\tan\\theta = \\dfrac{${y}}{${x}}$`), sp: 'm' };
    },
    (r) => {
      const ctx = r.pick(ANGCTX), q = r.int(1, 4);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} turns to an angle $\\theta$ in quadrant ${QROMAN[q]}. State which of $\\sin\\theta$, $\\cos\\theta$, $\\tan\\theta$ are positive and which are negative.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} berputar ke sudut $\\theta$ dalam sukuan ${QROMAN[q]}. Nyatakan yang manakah antara $\\sin\\theta$, $\\cos\\theta$, $\\tan\\theta$ positif dan yang manakah negatif.`), a: T(`$\\sin\\theta$ ${SGN.sin[q - 1] > 0 ? '+' : '-'}, $\\cos\\theta$ ${SGN.cos[q - 1] > 0 ? '+' : '-'}, $\\tan\\theta$ ${SGN.tan[q - 1] > 0 ? '+' : '-'}`), w: W(T(`In quadrant ${QROMAN[q]}: $x ${SGN.cos[q - 1] > 0 ? '> 0' : '< 0'}$, $y ${SGN.sin[q - 1] > 0 ? '> 0' : '< 0'}$`, `Dalam sukuan ${QROMAN[q]}: $x ${SGN.cos[q - 1] > 0 ? '> 0' : '< 0'}$, $y ${SGN.sin[q - 1] > 0 ? '> 0' : '< 0'}$`), T('$\\sin\\theta = \\dfrac{y}{r}$, $\\cos\\theta = \\dfrac{x}{r}$, $\\tan\\theta = \\dfrac{y}{x}$ with $r > 0$', '$\\sin\\theta = \\dfrac{y}{r}$, $\\cos\\theta = \\dfrac{x}{r}$, $\\tan\\theta = \\dfrac{y}{x}$ dengan $r > 0$')), sp: 's' };
    },
  ];
  ...
  const g61a = [
    (r) => {
      ...
      return { q: T(`Given $\\${given}\\,\\theta = ${givenTex}$ and $\\theta$ is in quadrant ${QROMAN[q]}, find $${others[0][0]}\\,\\theta$ and $${others[1][0]}\\,\\theta$.`, `Diberi $\\${given}\\,\\theta = ${givenTex}$ dan $\\theta$ berada dalam sukuan ${QROMAN[q]}, cari $${others[0][0]}\\,\\theta$ dan $${others[1][0]}\\,\\theta$.`), a: T(`$${others[0][0]}\\,\\theta = ${others[0][1]}$, $${others[1][0]}\\,\\theta = ${others[1][1]}$`), w: W(T(`Reference triangle: opposite $${p}$, adjacent $${qq}$, hypotenuse $${h}$ ($${p}^2 + ${qq}^2 = ${h}^2$)`, `Segi tiga rujukan: bertentangan $${p}$, bersebelahan $${qq}$, hipotenus $${h}$ ($${p}^2 + ${qq}^2 = ${h}^2$)`), T(`Quadrant ${QROMAN[q]}: positive ratios are ${ASTC[q].en}; the others are negative`, `Sukuan ${QROMAN[q]}: nisbah positif ialah ${ASTC[q].ms}; yang lain negatif`), `$${others[0][0]}\\,\\theta = ${others[0][1]}$, $${others[1][0]}\\,\\theta = ${others[1][1]}$`), sp: 'm' };
    },
    (r) => {
      ...
      return { q: T(`Find all values of $\\theta$ from $0^\\circ$ to $360^\\circ$ for which $\\${fn}\\,\\theta = ${valTex}$.`, `Cari semua nilai $\\theta$ dari $0^\\circ$ hingga $360^\\circ$ bagi $\\${fn}\\,\\theta = ${valTex}$.`), a: T(`$\\theta = ${a1}^\\circ$ or $${a2}^\\circ$`, `$\\theta = ${a1}^\\circ$ atau $${a2}^\\circ$`), w: W(...solW(fn, ref, sign, a1, a2)), sp: 'm' };
    },
    (r) => {
      const RATPOOL = [
        { fn: 'sin', t: r.pick([30, 150]), val: Fr.make(1, 2) }, { fn: 'sin', t: r.pick([210, 330]), val: Fr.make(-1, 2) },
    ...
    (r) => {
      ...
      const idFn = id.lhs.slice(1, 4), idQ = QROMAN.indexOf(id.quad);
      const w = W(T(`For acute $\\theta$, the angle in $${id.lhs}$ lies in quadrant ${id.quad}, with reference angle $\\theta$`, `Bagi $\\theta$ tirus, sudut dalam $${id.lhs}$ berada dalam sukuan ${id.quad}, dengan sudut rujukan $\\theta$`), signLine(idFn, idQ), `$${id.lhs} = ${id.rhs}$`);
      const forms = [
    ...
    (r) => {
      ...
      const givenTex = given === 'sin' ? sinTex : given === 'cos' ? cosTex : tanTex;
      const others = given === 'sin' ? [['\\cos', cosTex], ['\\tan', tanTex]] : given === 'cos' ? [['\\sin', sinTex], ['\\tan', tanTex]] : [['\\sin', sinTex], ['\\cos', cosTex]];
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} has rotated to an angle $\\theta$ in quadrant ${QROMAN[q]}, with $\\${given}\\,\\theta = ${givenTex}$. Find $${others[0][0]}\\,\\theta$ and $${others[1][0]}\\,\\theta$.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} telah berputar ke sudut $\\theta$ dalam sukuan ${QROMAN[q]}, dengan $\\${given}\\,\\theta = ${givenTex}$. Cari $${others[0][0]}\\,\\theta$ dan $${others[1][0]}\\,\\theta$.`), a: T(`$${others[0][0]}\\,\\theta = ${others[0][1]}$, $${others[1][0]}\\,\\theta = ${others[1][1]}$`), w: W(T(`Reference triangle: opposite $${p}$, adjacent $${qq}$, hypotenuse $${h}$ ($${p}^2 + ${qq}^2 = ${h}^2$)`, `Segi tiga rujukan: bertentangan $${p}$, bersebelahan $${qq}$, hipotenus $${h}$ ($${p}^2 + ${qq}^2 = ${h}^2$)`), T(`Quadrant ${QROMAN[q]}: positive ratios are ${ASTC[q].en}; the others are negative`, `Sukuan ${QROMAN[q]}: nisbah positif ialah ${ASTC[q].ms}; yang lain negatif`), `$${others[0][0]}\\,\\theta = ${others[0][1]}$, $${others[1][0]}\\,\\theta = ${others[1][1]}$`), sp: 'm' };
    },
    (r) => {
      ...
      return { q: T(`Given $\\sin\\theta = ${sinVal}$ and $\\cos\\theta ${cosSign < 0 ? '< 0' : '> 0'}$, determine the quadrant of $\\theta$, then find $\\tan\\theta$.`, `Diberi $\\sin\\theta = ${sinVal}$ dan $\\cos\\theta ${cosSign < 0 ? '< 0' : '> 0'}$, tentukan sukuan $\\theta$, kemudian cari $\\tan\\theta$.`), a: T(`$\\sin\\theta > 0$ and $\\cos\\theta ${cosSign < 0 ? '< 0' : '> 0'}$ together place $\\theta$ in quadrant ${QROMAN[quad]}; $\\tan\\theta = ${tanVal}$.`, `$\\sin\\theta > 0$ dan $\\cos\\theta ${cosSign < 0 ? '< 0' : '> 0'}$ bersama meletakkan $\\theta$ dalam sukuan ${QROMAN[quad]}; $\\tan\\theta = ${tanVal}$.`), w: W(T(`$\\sin\\theta > 0$: quadrant I or II; $\\cos\\theta ${cosSign < 0 ? '< 0' : '> 0'}$: quadrant ${QROMAN[quad]}`, `$\\sin\\theta > 0$: sukuan I atau II; $\\cos\\theta ${cosSign < 0 ? '< 0' : '> 0'}$: sukuan ${QROMAN[quad]}`), T(`$\\sin ${ref}^\\circ = ${sinVal}$, so the reference angle is $${ref}^\\circ$ and $\\tan ${ref}^\\circ = ${TAN30[IDX30[ref]]}$`, `$\\sin ${ref}^\\circ = ${sinVal}$, jadi sudut rujukan ialah $${ref}^\\circ$ dan $\\tan ${ref}^\\circ = ${TAN30[IDX30[ref]]}$`), signLine('tan', quad), `$\\tan\\theta = ${tanVal}$`), sp: 'm' };
    },
    (r) => {
      const x = r.sign(), y = r.sign();
      const q = x > 0 && y > 0 ? 1 : x < 0 && y > 0 ? 2 : x < 0 && y < 0 ? 3 : 4;
      return { q: T(`A point $P$ on the unit circle corresponds to angle $\\theta$, so $P = (\\cos\\theta, \\sin\\theta)$. If $P$ has ${x > 0 ? 'a positive' : 'a negative'} $x$-coordinate and ${y > 0 ? 'a positive' : 'a negative'} $y$-coordinate, which quadrant is $\\theta$ in, and what does this tell us about the sign of $\\tan\\theta$?`, `Satu titik $P$ pada bulatan unit sepadan dengan sudut $\\theta$, jadi $P = (\\cos\\theta, \\sin\\theta)$. Jika $P$ mempunyai koordinat-$x$ yang ${x > 0 ? 'positif' : 'negatif'} dan koordinat-$y$ yang ${y > 0 ? 'positif' : 'negatif'}, dalam sukuan manakah $\\theta$ berada, dan apakah maksudnya tentang tanda $\\tan\\theta$?`), a: T(`Quadrant ${QROMAN[q]}; $\\tan\\theta = \\dfrac{\\sin\\theta}{\\cos\\theta}$ is ${SGN.tan[q - 1] > 0 ? 'positive' : 'negative'} there.`, `Sukuan ${QROMAN[q]}; $\\tan\\theta = \\dfrac{\\sin\\theta}{\\cos\\theta}$ adalah ${SGN.tan[q - 1] > 0 ? 'positif' : 'negatif'} di sana.`), w: W(T(`$\\cos\\theta ${x > 0 ? '> 0' : '< 0'}$ and $\\sin\\theta ${y > 0 ? '> 0' : '< 0'}$: quadrant ${QROMAN[q]}`, `$\\cos\\theta ${x > 0 ? '> 0' : '< 0'}$ dan $\\sin\\theta ${y > 0 ? '> 0' : '< 0'}$: sukuan ${QROMAN[q]}`), T(`$\\tan\\theta = \\dfrac{\\sin\\theta}{\\cos\\theta}$: ${x * y > 0 ? 'same signs give a positive quotient' : 'opposite signs give a negative quotient'}`, `$\\tan\\theta = \\dfrac{\\sin\\theta}{\\cos\\theta}$: ${x * y > 0 ? 'tanda sama memberikan hasil bahagi positif' : 'tanda berlawanan memberikan hasil bahagi negatif'}`)), sp: 's' };
    },
    (r) => {
      const q = r.pick([2, 3, 4]);
      ...
      return { q: T(`Without a calculator, determine which is greater: $\\${fn}\\,${t1}^\\circ$ or $\\${fn}\\,${t2}^\\circ$ (both angles are in quadrant ${QROMAN[q]}). Explain using reference angles.`, `Tanpa kalkulator, tentukan yang manakah lebih besar: $\\${fn}\\,${t1}^\\circ$ atau $\\${fn}\\,${t2}^\\circ$ (kedua-dua sudut berada dalam sukuan ${QROMAN[q]}). Terangkan menggunakan sudut rujukan.`), a: T(`$\\${fn}\\,${bigger}^\\circ$ is greater (reference angles $${r1}^\\circ$ and $${r2}^\\circ$; compare using how $\\${fn}$ behaves on $0^\\circ$–$90^\\circ$ together with the sign in quadrant ${QROMAN[q]}).`, `$\\${fn}\\,${bigger}^\\circ$ lebih besar (sudut rujukan $${r1}^\\circ$ dan $${r2}^\\circ$; bandingkan menggunakan cara $\\${fn}$ berkelakuan pada $0^\\circ$–$90^\\circ$ bersama tanda dalam sukuan ${QROMAN[q]}).`), w: W(signLine(fn, q), `$\\${fn}\\,${t1}^\\circ = ${SGN[fn][q - 1] < 0 ? '-' : ''}\\${fn}\\,${r1}^\\circ \\approx ${n(round(v1, 4))}$`, `$\\${fn}\\,${t2}^\\circ = ${SGN[fn][q - 1] < 0 ? '-' : ''}\\${fn}\\,${r2}^\\circ \\approx ${n(round(v2, 4))}$`, T(`On $0^\\circ$ to $90^\\circ$, ${fn === 'cos' ? 'cosine decreases' : `${FNM[fn].en} increases`} as the angle increases; ${SGN[fn][q - 1] < 0 ? 'the minus sign reverses the order' : 'the sign is positive, so the order is kept'}.`, `Pada $0^\\circ$ hingga $90^\\circ$, ${fn === 'cos' ? 'kosinus berkurang' : `${FNM[fn].ms} bertambah`} apabila sudut bertambah; ${SGN[fn][q - 1] < 0 ? 'tanda negatif menterbalikkan susunan' : 'tandanya positif, jadi susunan dikekalkan'}.`)), sp: 'l' };
    },
    (r) => {
      const [p, q, h] = r.pick(RT3s), sx = r.sign(), sy = r.sign();
      const x = sx * p, y = sy * q;
  ...
  const ampW = (a, b) => [T(`Amplitude $= |a| = ${a}$`, `Amplitud $= |a| = ${a}$`), T(`Period $= \\dfrac{360^\\circ}{b} = \\dfrac{360^\\circ}{${b}} = ${360 / b}^\\circ$`, `Tempoh $= \\dfrac{360^\\circ}{b} = \\dfrac{360^\\circ}{${b}} = ${360 / b}^\\circ$`)];
  const cstr = (c) => (c === 0 ? '' : ` ${c > 0 ? '+' : '-'} ${Math.abs(c)}`);
  const mmW = (a, c) => T(`$\\sin$ and $\\cos$ lie between $-1$ and $1$: maximum $= ${a}(1)${cstr(c)} = ${a + c}$, minimum $= ${a}(-1)${cstr(c)} = ${c - a}$`, `$\\sin$ dan $\\cos$ terletak antara $-1$ dengan $1$: maksimum $= ${a}(1)${cstr(c)} = ${a + c}$, minimum $= ${a}(-1)${cstr(c)} = ${c - a}$`);
  const SHAPEW = { sin: T('Starts at $(0^\\circ, 0)$, maximum $1$ at $90^\\circ$, minimum $-1$ at $270^\\circ$: sine.', 'Bermula di $(0^\\circ, 0)$, maksimum $1$ pada $90^\\circ$, minimum $-1$ pada $270^\\circ$: sinus.'), cos: T('Starts at its maximum $(0^\\circ, 1)$, minimum $-1$ at $180^\\circ$: cosine.', 'Bermula pada maksimumnya $(0^\\circ, 1)$, minimum $-1$ pada $180^\\circ$: kosinus.'), tan: T('No maximum or minimum; breaks at $90^\\circ$ and $270^\\circ$ (asymptotes) and repeats every $180^\\circ$: tangent.', 'Tiada maksimum atau minimum; terputus pada $90^\\circ$ dan $270^\\circ$ (asimptot) dan berulang setiap $180^\\circ$: tangen.') };
  const TIDECTX = [
  ...
  const g62e = [
    (r) => {
      ...
      return { q: T('The graph shows one of $y = \\sin x$, $y = \\cos x$ or $y = \\tan x$ for $0^\\circ \\le x \\le 360^\\circ$. Which one is it?', 'Graf menunjukkan salah satu daripada $y = \\sin x$, $y = \\cos x$ atau $y = \\tan x$ bagi $0^\\circ \\le x \\le 360^\\circ$. Yang manakah?'), fig, a: T(`$y = \\${fn} x$`), w: W(SHAPEW[fn]), sp: 's' };
    },
    (r) => {
      const fn = r.pick(['sin', 'cos']);
      return { q: T(`State the maximum and minimum values of $y = \\${fn} x$ and the values of $x$ (from $0^\\circ$ to $360^\\circ$) at which they occur.`, `Nyatakan nilai maksimum dan minimum bagi $y = \\${fn} x$ dan nilai $x$ (dari $0^\\circ$ hingga $360^\\circ$) apabila ia berlaku.`), a: fn === 'sin' ? T('Maximum $1$ at $x = 90^\\circ$; minimum $-1$ at $x = 270^\\circ$', 'Maksimum $1$ pada $x = 90^\\circ$; minimum $-1$ pada $x = 270^\\circ$') : T('Maximum $1$ at $x = 0^\\circ$ and $360^\\circ$; minimum $-1$ at $x = 180^\\circ$', 'Maksimum $1$ pada $x = 0^\\circ$ dan $360^\\circ$; minimum $-1$ pada $x = 180^\\circ$'), w: W(fn === 'sin' ? T('$\\sin x$ is the $y$-coordinate on the unit circle: highest at the top ($90^\\circ$), lowest at the bottom ($270^\\circ$).', '$\\sin x$ ialah koordinat-$y$ pada bulatan unit: tertinggi di atas ($90^\\circ$), terendah di bawah ($270^\\circ$).') : T('$\\cos x$ is the $x$-coordinate on the unit circle: largest at $0^\\circ$ and $360^\\circ$, smallest at $180^\\circ$.', '$\\cos x$ ialah koordinat-$x$ pada bulatan unit: terbesar pada $0^\\circ$ dan $360^\\circ$, terkecil pada $180^\\circ$.'), fn === 'sin' ? '$\\sin 90^\\circ = 1$, $\\sin 270^\\circ = -1$' : '$\\cos 0^\\circ = \\cos 360^\\circ = 1$, $\\cos 180^\\circ = -1$'), sp: 's' };
    },
    (r) => {
      const a = r.int(2, 5), b = r.pick([1, 2, 3]), fn = r.pick(['sin', 'cos']);
      return { q: T(`State the amplitude and the period of $y = ${a} \\${fn} ${b === 1 ? '' : b}x$.`, `Nyatakan amplitud dan tempoh bagi $y = ${a} \\${fn} ${b === 1 ? '' : b}x$.`), a: T(`Amplitude ${a}; period $${360 / b}^\\circ$`, `Amplitud ${a}; tempoh $${360 / b}^\\circ$`), w: W(...ampW(a, b)), sp: 's' };
    },
    (r) => {
      const ctx = r.pick(TIDECTX), a = r.int(2, 5), b = r.pick([1, 2, 3]), fn = r.pick(['sin', 'cos']);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} is modelled by $y = ${a}\\${fn}\\,${b === 1 ? '' : b}x$. State the amplitude and period of this model.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} dimodelkan oleh $y = ${a}\\${fn}\\,${b === 1 ? '' : b}x$. Nyatakan amplitud dan tempoh model ini.`), a: T(`Amplitude ${a}; period $${360 / b}^\\circ$`, `Amplitud ${a}; tempoh $${360 / b}^\\circ$`), w: W(...ampW(a, b)), sp: 's' };
    },
    (r) => {
      const ctx = r.pick(TIDECTX), a = r.int(2, 5), fn = r.pick(['sin', 'cos']);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} is modelled by $y = ${a}\\${fn}\\,x$. State the maximum and minimum values this model predicts.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} dimodelkan oleh $y = ${a}\\${fn}\\,x$. Nyatakan nilai maksimum dan minimum yang diramalkan oleh model ini.`), a: T(`Maximum $${a}$; minimum $${-a}$`, `Maksimum $${a}$; minimum $${-a}$`), w: W(mmW(a, 0)), sp: 'xs' };
    },
    (r) => {
      const b = r.pick([1, 2, 3, 4]);
      return { q: T(`State the period of $y = \\tan ${b === 1 ? '' : b}x$. (Note: tangent has no amplitude or maximum/minimum value.)`, `Nyatakan tempoh bagi $y = \\tan ${b === 1 ? '' : b}x$. (Nota: tangen tiada amplitud atau nilai maksimum/minimum.)`), a: T(`$${180 / b}^\\circ$`), w: W(T(`$\\tan x$ repeats every $180^\\circ$, so the period of $y = \\tan bx$ is $\\dfrac{180^\\circ}{${b}} = ${180 / b}^\\circ$`, `$\\tan x$ berulang setiap $180^\\circ$, jadi tempoh $y = \\tan bx$ ialah $\\dfrac{180^\\circ}{${b}} = ${180 / b}^\\circ$`)), sp: 's' };
    },
    (r) => {
      const fn = r.pick(['sin', 'cos']), x = r.pick([0, 90, 180, 270, 360]);
      const fig = trigFig(fn, 1, 1, 0);
      const y = fn === 'sin' ? Math.sin(rad2(x)) : Math.cos(rad2(x));
      return { q: T(`From the graph of $y = \\${fn} x$, state the value of $y$ when $x = ${x}^\\circ$.`, `Daripada graf $y = \\${fn} x$, nyatakan nilai $y$ apabila $x = ${x}^\\circ$.`), fig, a: T(`$${n(round(y, 4))}$`), w: W(T(`At $x = ${x}^\\circ$ the unit-circle point is $${pt(UPT[x])}$; $\\${fn} x$ is its ${fn === 'sin' ? '$y$' : '$x$'}-coordinate: $${n(round(y, 4))}$`, `Pada $x = ${x}^\\circ$ titik bulatan unit ialah $${pt(UPT[x])}$; $\\${fn} x$ ialah koordinat-${fn === 'sin' ? '$y$' : '$x$'}nya: $${n(round(y, 4))}$`)), sp: 'xs' };
    },
    (r) => {
      const fn = r.pick(['sin', 'cos']), k = r.pick([0, 1, -1]);
      const sols = fn === 'sin' ? (k === 0 ? [0, 180, 360] : k === 1 ? [90] : [270]) : (k === 0 ? [90, 270] : k === 1 ? [0, 360] : [180]);
      return { q: T(`Solve $\\${fn}\\,x = ${k}$ for $0^\\circ \\le x \\le 360^\\circ$.`, `Selesaikan $\\${fn}\\,x = ${k}$ bagi $0^\\circ \\le x \\le 360^\\circ$.`), a: T(`$x = ${sols.map((s) => s + '^\\circ').join('$ or $')}$`, `$x = ${sols.map((s) => s + '^\\circ').join('$ atau $')}$`), w: W(T(`$\\${fn} x$ is the ${fn === 'sin' ? '$y$' : '$x$'}-coordinate on the unit circle; it equals $${k}$ at ${[...new Set(sols.map((s) => pt(UPT[s])))].map((v) => `$${v}$`).join(', ')}`, `$\\${fn} x$ ialah koordinat-${fn === 'sin' ? '$y$' : '$x$'} pada bulatan unit; ia sama dengan $${k}$ pada ${[...new Set(sols.map((s) => pt(UPT[s])))].map((v) => `$${v}$`).join(', ')}`), T(`So $x = ${sols.map((s) => s + '^\\circ').join(', ')}$${sols.length > 1 ? ' in this interval' : ''}`, `Jadi $x = ${sols.map((s) => s + '^\\circ').join(', ')}$${sols.length > 1 ? ' dalam selang ini' : ''}`)), sp: 's' };
    },
    (r) => {
      const st = r.pick([
        [T('The amplitude of $y = \\tan x$ is $1$.', 'Amplitud bagi $y = \\tan x$ ialah $1$.'), false, T('$\\tan x$ has no maximum or minimum (it grows without bound near $90^\\circ$), so it has no amplitude.', '$\\tan x$ tiada maksimum atau minimum (ia bertambah tanpa had berhampiran $90^\\circ$), jadi ia tiada amplitud.')],
        [T('$y = \\tan x$ has no amplitude and no maximum or minimum value.', '$y = \\tan x$ tiada amplitud dan tiada nilai maksimum atau minimum.'), true, T('$\\tan x$ grows without bound near its asymptotes.', '$\\tan x$ bertambah tanpa had berhampiran asimptotnya.')],
        [T('The period of $y = \\sin x$ is $360^\\circ$.', 'Tempoh bagi $y = \\sin x$ ialah $360^\\circ$.'), true, T('The unit-circle point returns to the same place after one full turn, $360^\\circ$.', 'Titik bulatan unit kembali ke tempat yang sama selepas satu pusingan lengkap, $360^\\circ$.')],
        [T('The period of $y = \\tan x$ is $360^\\circ$.', 'Tempoh bagi $y = \\tan x$ ialah $360^\\circ$.'), false, T('$\\tan x$ repeats every $180^\\circ$, e.g. $\\tan 45^\\circ = \\tan 225^\\circ = 1$.', '$\\tan x$ berulang setiap $180^\\circ$, cth. $\\tan 45^\\circ = \\tan 225^\\circ = 1$.')],
        [T('The graph of $y = \\cos x$ passes through $(0^\\circ, 1)$.', 'Graf $y = \\cos x$ melalui $(0^\\circ, 1)$.'), true, '$\\cos 0^\\circ = 1$'],
        [T('The graph of $y = \\sin x$ passes through $(0^\\circ, 1)$.', 'Graf $y = \\sin x$ melalui $(0^\\circ, 1)$.'), false, T('$\\sin 0^\\circ = 0$, so it passes through $(0^\\circ, 0)$.', '$\\sin 0^\\circ = 0$, jadi ia melalui $(0^\\circ, 0)$.')],
        [T('$y = a\\sin bx$ has amplitude $a$ and period $\\dfrac{360^\\circ}{b}$ for $a, b > 0$.', '$y = a\\sin bx$ mempunyai amplitud $a$ dan tempoh $\\dfrac{360^\\circ}{b}$ bagi $a, b > 0$.'), true, T('$a$ stretches the graph vertically; $b$ fits $b$ cycles into $360^\\circ$.', '$a$ meregangkan graf secara menegak; $b$ memuatkan $b$ kitaran dalam $360^\\circ$.')],
        [T('$y = a\\tan bx$ has period $\\dfrac{180^\\circ}{b}$ for $b > 0$.', '$y = a\\tan bx$ mempunyai tempoh $\\dfrac{180^\\circ}{b}$ bagi $b > 0$.'), true, T('$\\tan x$ has period $180^\\circ$, and $b$ fits $b$ cycles into it.', '$\\tan x$ mempunyai tempoh $180^\\circ$, dan $b$ memuatkan $b$ kitaran di dalamnya.')],
        [T('The graph of $y = \\sin x$ is symmetrical about $x = 90^\\circ$ within $0^\\circ \\le x \\le 180^\\circ$.', 'Graf $y = \\sin x$ adalah simetri terhadap $x = 90^\\circ$ dalam $0^\\circ \\le x \\le 180^\\circ$.'), true, T('$\\sin(180^\\circ - x) = \\sin x$, e.g. $\\sin 30^\\circ = \\sin 150^\\circ$.', '$\\sin(180^\\circ - x) = \\sin x$, cth. $\\sin 30^\\circ = \\sin 150^\\circ$.')],
        [T('The graphs of $y = \\sin x$ and $y = \\cos x$ never take the same value in $0^\\circ \\le x \\le 360^\\circ$.', 'Graf $y = \\sin x$ dan $y = \\cos x$ tidak pernah mempunyai nilai yang sama dalam $0^\\circ \\le x \\le 360^\\circ$.'), false, T('They meet at $x = 45^\\circ$ and $x = 225^\\circ$, e.g. $\\sin 45^\\circ = \\cos 45^\\circ = \\dfrac{\\sqrt{2}}{2}$.', 'Kedua-duanya bertemu pada $x = 45^\\circ$ dan $x = 225^\\circ$, cth. $\\sin 45^\\circ = \\cos 45^\\circ = \\dfrac{\\sqrt{2}}{2}$.')],
        [T('The graph of $y = \\tan x$ is a smooth continuous curve for all $x$ from $0^\\circ$ to $360^\\circ$.', 'Graf $y = \\tan x$ ialah lengkung selanjar licin bagi semua $x$ dari $0^\\circ$ hingga $360^\\circ$.'), false, T('It breaks at the asymptotes $x = 90^\\circ$ and $x = 270^\\circ$, where $\\tan x$ is not defined.', 'Ia terputus pada asimptot $x = 90^\\circ$ dan $x = 270^\\circ$, di mana $\\tan x$ tidak ditakrifkan.')],
    ...
    (r) => {
      ...
      const wP = b === 1 ? 180 : 360 * b;
      const wrong2 = T(`Amplitude ${a}, period $${wP}^\\circ$`, `Amplitud ${a}, tempoh $${wP}^\\circ$`);
      const opts = r.shuffle([correct, wrong1, wrong2]);
      const letters = 'ABC';
      return { q: T(`For $y = ${a}\\${fn}\\,${b === 1 ? '' : b}x$, which option correctly states the amplitude and period? ${opts.map((o, i) => `(${letters[i]}) ${o.en}`).join(', ')}`, `Bagi $y = ${a}\\${fn}\\,${b === 1 ? '' : b}x$, pilihan manakah yang menyatakan amplitud dan tempoh dengan betul? ${opts.map((o, i) => `(${letters[i]}) ${o.ms}`).join(', ')}`), a: T(`(${letters[opts.indexOf(correct)]}) ${correct.en}`, `(${letters[opts.indexOf(correct)]}) ${correct.ms}`), w: W(...ampW(a, b)), sp: 's' };
    },
    (r) => {
      const ctx = r.pick(TIDECTX), b = r.pick([1, 2, 3]), fn = r.pick(['sin', 'cos']);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} is modelled by $y = \\${fn}\\,${b === 1 ? '' : b}x$. State the period of this cycle.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} dimodelkan oleh $y = \\${fn}\\,${b === 1 ? '' : b}x$. Nyatakan tempoh kitaran ini.`), a: T(`$${360 / b}^\\circ$`), w: W(T(`Period $= \\dfrac{360^\\circ}{${b}} = ${360 / b}^\\circ$`, `Tempoh $= \\dfrac{360^\\circ}{${b}} = ${360 / b}^\\circ$`)), sp: 'xs' };
    },
  ];
  const g62m = [
    (r) => {
      const a = r.int(2, 4), b = r.pick([1, 2, 3]), c = r.int(-3, 3), fn = r.pick(['sin', 'cos']);
      return { q: T(`State the amplitude, period and midline of $y = ${a} \\${fn}\\,${b === 1 ? '' : b}x${cstr(c)}$.`, `Nyatakan amplitud, tempoh dan garis tengah bagi $y = ${a} \\${fn}\\,${b === 1 ? '' : b}x${cstr(c)}$.`), a: T(`Amplitude ${a}; period $${360 / b}^\\circ$; midline $y = ${c}$; range $[${c - a}, ${c + a}]$`, `Amplitud ${a}; tempoh $${360 / b}^\\circ$; garis tengah $y = ${c}$; julat $[${c - a}, ${c + a}]$`), w: W(...ampW(a, b), T(`The constant term moves the midline to $y = ${c}$`, `Sebutan pemalar menganjak garis tengah ke $y = ${c}$`), `$${c} - ${a} = ${c - a}$, $${c} + ${a} = ${c + a}$`), sp: 's' };
    },
    (r) => {
      const a = r.int(2, 3), b = r.pick([1, 2]), c = r.int(-2, 2), fn = r.pick(['sin', 'cos']);
      const maxX = fn === 'sin' ? 90 / b : 0, minX = fn === 'sin' ? 270 / b : 180 / b;
      return { q: T(`For $y = ${a} \\${fn}\\,${b === 1 ? '' : b}x${cstr(c)}$ over $0^\\circ \\le x \\le 360^\\circ$, state the maximum value and the smallest $x$ at which it occurs, and the minimum value and the smallest $x$ at which it occurs.`, `Bagi $y = ${a} \\${fn}\\,${b === 1 ? '' : b}x${cstr(c)}$ dalam $0^\\circ \\le x \\le 360^\\circ$, nyatakan nilai maksimum dan $x$ terkecil apabila ia berlaku, dan nilai minimum dan $x$ terkecil apabila ia berlaku.`), a: T(`Maximum $${a + c}$ at $x = ${maxX}^\\circ$; minimum $${c - a}$ at $x = ${minX}^\\circ$`, `Maksimum $${a + c}$ pada $x = ${maxX}^\\circ$; minimum $${c - a}$ pada $x = ${minX}^\\circ$`), w: W(mmW(a, c), T(`$\\${fn}\\,${b === 1 ? '' : b}x = 1$ first when $${b === 1 ? '' : b}x = ${fn === 'sin' ? 90 : 0}^\\circ$, so $x = ${maxX}^\\circ$; $\\${fn}\\,${b === 1 ? '' : b}x = -1$ first when $${b === 1 ? '' : b}x = ${fn === 'sin' ? 270 : 180}^\\circ$, so $x = ${minX}^\\circ$`, `$\\${fn}\\,${b === 1 ? '' : b}x = 1$ kali pertama apabila $${b === 1 ? '' : b}x = ${fn === 'sin' ? 90 : 0}^\\circ$, jadi $x = ${maxX}^\\circ$; $\\${fn}\\,${b === 1 ? '' : b}x = -1$ kali pertama apabila $${b === 1 ? '' : b}x = ${fn === 'sin' ? 270 : 180}^\\circ$, jadi $x = ${minX}^\\circ$`)), sp: 'm' };
    },
    (r) => {
      ...
      return { q: T(`Solve $\\${fn}\\,x = ${valTex}$ for $0^\\circ \\le x \\le 360^\\circ$.`, `Selesaikan $\\${fn}\\,x = ${valTex}$ bagi $0^\\circ \\le x \\le 360^\\circ$.`), a: T(`$x = ${s1}^\\circ$ or $${s2}^\\circ$`, `$x = ${s1}^\\circ$ atau $${s2}^\\circ$`), w: W(...solW(fn, ref, sign, s1, s2)), sp: 'm' };
    },
    (r) => {
    ...
    (r) => {
      ...
      const valTex = (sign < 0 ? '-' : '') + table[IDX30[ref]];
      const [s1, s2] = twoSolutions(fn, ref, sign);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} is modelled by $y = \\${fn}\\,x$ for $0^\\circ \\le x \\le 360^\\circ$. Find the value(s) of $x$ at which $y = ${valTex}$.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} dimodelkan oleh $y = \\${fn}\\,x$ bagi $0^\\circ \\le x \\le 360^\\circ$. Cari nilai $x$ apabila $y = ${valTex}$.`), a: T(`$x = ${s1}^\\circ$ or $${s2}^\\circ$`, `$x = ${s1}^\\circ$ atau $${s2}^\\circ$`), w: W(...solW(fn, ref, sign, s1, s2)), sp: 'm' };
    },
    (r) => {
  ...
  const g62a = [
    ...
    (r) => {
      ...
      const table = fn === 'sin' ? SIN30 : COS30;
      const tableNum = fn === 'sin' ? Math.sin(rad2(ref)) : Math.cos(rad2(ref)); // numeric value of table[IDX30[ref]]
      const kNum = a * sign * tableNum + c;
      const [s1, s2] = twoSolutions(fn, ref, sign);
      const lhs = `${a}\\${fn}\\,x${cstr(c)}`, kv = n(round(kNum, 4)), ak = n(round(kNum - c, 4));
      const exact4 = Math.abs(round(sign * tableNum, 4) - sign * tableNum) < 1e-9;
      const w = W(...(c === 0 ? [] : [`$${a}\\${fn}\\,x = ${kv}${cstr(-c)} = ${ak}$`]), `$\\${fn}\\,x = \\dfrac{${ak}}{${a}} ${exact4 ? '=' : '\\approx'} ${n(round(sign * tableNum, 4))} = ${(sign < 0 ? '-' : '') + table[IDX30[ref]]}$`, ...solW(fn, ref, sign, s1, s2));
      return { q: T(`Solve $${lhs} = ${kv}$ for $0^\\circ \\le x \\le 360^\\circ$.`, `Selesaikan $${lhs} = ${kv}$ bagi $0^\\circ \\le x \\le 360^\\circ$.`), a: T(`$\\${fn}\\,x = ${(sign < 0 ? '-' : '') + table[IDX30[ref]]}$; $x = ${s1}^\\circ$ or $${s2}^\\circ$`, `$\\${fn}\\,x = ${(sign < 0 ? '-' : '') + table[IDX30[ref]]}$; $x = ${s1}^\\circ$ atau $${s2}^\\circ$`), w, sp: 'm' };
    },
    (r) => {
      const b = r.pick([1, 2, 3]), fn = r.pick(['sin', 'cos']);
      const count = 4 * b;
      return { q: T(`For $y = \\${fn}\\,${b === 1 ? '' : b}x$ over $0^\\circ \\le x \\le 720^\\circ$, find the number of solutions of $\\${fn}\\,${b === 1 ? '' : b}x = 0.4$.`, `Bagi $y = \\${fn}\\,${b === 1 ? '' : b}x$ dalam $0^\\circ \\le x \\le 720^\\circ$, cari bilangan penyelesaian bagi $\\${fn}\\,${b === 1 ? '' : b}x = 0.4$.`), a: T(`${count} (period $${360 / b}^\\circ$, so $720 \\div ${360 / b} = ${(720 * b) / 360}$ periods in the range; each period gives 2 solutions for $0 < 0.4 < 1$, so $2 \\times ${(720 * b) / 360} = ${count}$)`, `${count} (tempoh $${360 / b}^\\circ$, jadi $720 \\div ${360 / b} = ${(720 * b) / 360}$ tempoh dalam julat itu; setiap tempoh memberi 2 penyelesaian kerana $0 < 0.4 < 1$, jadi $2 \\times ${(720 * b) / 360} = ${count}$)`), w: W(T(`Period $= \\dfrac{360^\\circ}{${b}} = ${360 / b}^\\circ$; number of cycles $= \\dfrac{720}{${360 / b}} = ${2 * b}$`, `Tempoh $= \\dfrac{360^\\circ}{${b}} = ${360 / b}^\\circ$; bilangan kitaran $= \\dfrac{720}{${360 / b}} = ${2 * b}$`), T(`$0 < 0.4 < 1$, so the line $y = 0.4$ cuts each cycle twice: $2 \\times ${2 * b} = ${count}$`, `$0 < 0.4 < 1$, jadi garis $y = 0.4$ memotong setiap kitaran dua kali: $2 \\times ${2 * b} = ${count}$`)), sp: 'm' };
    },
    (r) => {
      const b = r.pick([1, 2, 3]);
      const asymps = [];
      for (let k = 0; ; k++) { const xa = (90 + 180 * k) / b; if (xa > 360) break; asymps.push(xa); }
      return { q: T(`List all the values of $x$ from $0^\\circ$ to $360^\\circ$ at which $y = \\tan\\,${b === 1 ? '' : b}x$ has an asymptote.`, `Senaraikan semua nilai $x$ dari $0^\\circ$ hingga $360^\\circ$ di mana $y = \\tan\\,${b === 1 ? '' : b}x$ mempunyai asimptot.`), a: T(`$x = ${asymps.map((v) => n(v) + '^\\circ').join(', ')}$`), w: W(T(`Asymptotes where $${b === 1 ? '' : b}x = 90^\\circ, 270^\\circ, 450^\\circ, \\ldots$ (tangent not defined)`, `Asimptot apabila $${b === 1 ? '' : b}x = 90^\\circ, 270^\\circ, 450^\\circ, \\ldots$ (tangen tidak ditakrifkan)`), T(`Divide by ${b} and keep $x \\le 360^\\circ$: $x = ${asymps.map((v) => n(v) + '^\\circ').join(', ')}$`, `Bahagi dengan ${b} dan kekalkan $x \\le 360^\\circ$: $x = ${asymps.map((v) => n(v) + '^\\circ').join(', ')}$`)), sp: 'm' };
    },
    (r) => {
      const feats = r.pick([{ a: 3, b: 1, fn: 'sin' }, { a: 3, b: 2, fn: 'cos' }, { a: 4, b: 1, fn: 'cos' }, { a: 2, b: 3, fn: 'sin' }]);
      ...
      const opts = r.shuffle([correct, wrong1, wrong2]);
      const letters = 'ABC';
      return { q: T(`A graph has maximum value $${feats.a}$, minimum value $${-feats.a}$, period $${360 / feats.b}^\\circ$, and passes through $(0^\\circ, ${feats.fn === 'cos' ? feats.a : 0})$. Which equation matches? ${opts.map((o, i) => `(${letters[i]}) ${o.en}`).join(', ')}`, `Satu graf mempunyai nilai maksimum $${feats.a}$, nilai minimum $${-feats.a}$, tempoh $${360 / feats.b}^\\circ$, dan melalui $(0^\\circ, ${feats.fn === 'cos' ? feats.a : 0})$. Persamaan manakah yang sepadan? ${opts.map((o, i) => `(${letters[i]}) ${o.ms}`).join(', ')}`), a: T(`(${letters[opts.indexOf(correct)]}) ${correct.en}`, `(${letters[opts.indexOf(correct)]}) ${correct.ms}`), w: W(T(`Maximum $${feats.a}$: $a = ${feats.a}$; period $${360 / feats.b}^\\circ$: $b = \\dfrac{360^\\circ}{${360 / feats.b}^\\circ} = ${feats.b}$`, `Maksimum $${feats.a}$: $a = ${feats.a}$; tempoh $${360 / feats.b}^\\circ$: $b = \\dfrac{360^\\circ}{${360 / feats.b}^\\circ} = ${feats.b}$`), feats.fn === 'cos' ? T(`At $x = 0^\\circ$, $y = ${feats.a}$ (the maximum): a cosine graph`, `Pada $x = 0^\\circ$, $y = ${feats.a}$ (maksimum): graf kosinus`) : T('At $x = 0^\\circ$, $y = 0$: a sine graph', 'Pada $x = 0^\\circ$, $y = 0$: graf sinus')), sp: 's' };
    },
    (r) => {
      const ctx = r.pick(TIDECTX), b = r.pick([1, 2, 3]), fn = r.pick(['sin', 'cos']);
      const count = 4 * b;
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} is modelled by $y = \\${fn}\\,${b === 1 ? '' : b}x$ over an extended period $0^\\circ \\le x \\le 720^\\circ$. Find the number of times $y = 0.4$.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} dimodelkan oleh $y = \\${fn}\\,${b === 1 ? '' : b}x$ sepanjang tempoh lanjutan $0^\\circ \\le x \\le 720^\\circ$. Cari bilangan kali $y = 0.4$.`), a: T(`${count} (period $${360 / b}^\\circ$, so $720 \\div ${360 / b} = ${(720 * b) / 360}$ periods in the range, each giving 2 solutions)`, `${count} (tempoh $${360 / b}^\\circ$, jadi $720 \\div ${360 / b} = ${(720 * b) / 360}$ tempoh dalam julat itu, setiap satu memberi 2 penyelesaian)`), w: W(T(`Period $= \\dfrac{360^\\circ}{${b}} = ${360 / b}^\\circ$; number of cycles $= \\dfrac{720}{${360 / b}} = ${2 * b}$`, `Tempoh $= \\dfrac{360^\\circ}{${b}} = ${360 / b}^\\circ$; bilangan kitaran $= \\dfrac{720}{${360 / b}} = ${2 * b}$`), T(`$0 < 0.4 < 1$, so $y = 0.4$ is reached twice in each cycle: $2 \\times ${2 * b} = ${count}$`, `$0 < 0.4 < 1$, jadi $y = 0.4$ dicapai dua kali dalam setiap kitaran: $2 \\times ${2 * b} = ${count}$`)), sp: 'm' };
    },
    (r) => {
      const ctx = r.pick(TIDECTX), a = r.int(3, 6), c = r.int(a + 2, a + 10);
      const ref = r.pick([30, 45, 60]);
      const table = COS30;
      const tableNum = Math.cos(rad2(ref)); // numeric value of table[IDX30[ref]]
      const target = round(c - a * tableNum, 2);
      const [s1, s2] = twoSolutions('cos', ref, -1);
      return { q: T(`${ctx.en[0].toUpperCase()}${ctx.en.slice(1)} is modelled by $y = ${a}\\cos\\,x + ${c}$ for $0^\\circ \\le x \\le 360^\\circ$, where $x^\\circ$ represents time. Find the value(s) of $x$ at which $y = ${n(target)}$.`, `${ctx.ms[0].toUpperCase()}${ctx.ms.slice(1)} dimodelkan oleh $y = ${a}\\cos\\,x + ${c}$ bagi $0^\\circ \\le x \\le 360^\\circ$, dengan $x^\\circ$ mewakili masa. Cari nilai $x$ apabila $y = ${n(target)}$.`), a: T(`$\\cos\\,x = -${table[IDX30[ref]]}$; $x = ${s1}^\\circ$ or $${s2}^\\circ$`, `$\\cos\\,x = -${table[IDX30[ref]]}$; $x = ${s1}^\\circ$ atau $${s2}^\\circ$`), w: W(`$${a}\\cos\\,x = ${n(target)} - ${c} = ${n(round(target - c, 2))}$`, `$\\cos\\,x = \\dfrac{${n(round(target - c, 2))}}{${a}} \\approx ${n(round((target - c) / a, 3))} = -${table[IDX30[ref]]}$`, ...solW('cos', ref, -1, s1, s2)), sp: 'l' };
    },
    (r) => ({ q: T('Explain why it does not make sense to ask for the amplitude or the maximum value of $y = \\tan x$.', 'Terangkan mengapa tidak masuk akal untuk bertanya tentang amplitud atau nilai maksimum bagi $y = \\tan x$.'), a: T('Unlike sine and cosine, $\\tan x$ is not bounded: as $x$ approaches $90^\\circ$ (or $270^\\circ$) the value of $\\tan x$ increases (or decreases) without bound, so the graph has no highest or lowest point and no fixed distance from a midline — amplitude and maximum/minimum have no meaning for tangent.', 'Tidak seperti sinus dan kosinus, $\\tan x$ tidak terbatas: apabila $x$ menghampiri $90^\\circ$ (atau $270^\\circ$) nilai $\\tan x$ meningkat (atau menurun) tanpa had, jadi graf itu tiada titik tertinggi atau terendah dan tiada jarak tetap daripada garis tengah — amplitud dan maksimum/minimum tidak bermakna bagi tangen.'), w: W(T('$\\tan x = \\dfrac{\\sin x}{\\cos x}$ and $\\cos x \\to 0$ as $x \\to 90^\\circ$, so $\\tan x$ becomes as large (or as negative) as we like.', '$\\tan x = \\dfrac{\\sin x}{\\cos x}$ dan $\\cos x \\to 0$ apabila $x \\to 90^\\circ$, jadi $\\tan x$ menjadi sebesar (atau senegatif) yang dikehendaki.'), T('No greatest or least value means no amplitude.', 'Tiada nilai terbesar atau terkecil bermakna tiada amplitud.')), sp: 'm' }),
  ];
