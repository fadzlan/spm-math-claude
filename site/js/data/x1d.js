...
(function () {
  ...
  const eVerify = (r) => {
    ...
    return { q: T(`When $x = ${t}$, the left-hand side of $${e.tex}$ equals $\\square$ and the right-hand side equals $\\square$. Fill in the blanks and state whether $x = ${t}$ makes the equation true.`, `Apabila $x = ${t}$, sebelah kiri bagi $${e.tex}$ bersamaan $\\square$ dan sebelah kanan bersamaan $\\square$. Isikan tempat kosong dan nyatakan sama ada $x = ${t}$ menjadikan persamaan itu betul.`), a: T(`LHS $= ${tx(lv)}$, RHS $= ${tx(rv)}$; ${good ? 'true' : 'not true'}`, `LHS $= ${tx(lv)}$, RHS $= ${tx(rv)}$; ${good ? 'betul' : 'tidak betul'}`), w: wkg, sp: 's' };
  };
  const eMissing = (r) => {
  ...
  const eNumEq = (r) => {
    ...
    if (v === 0) return { q: T(`Fill in the box to keep the equality true: $${a} + \\square = ${b} \\times ${c}$.`, `Isikan petak supaya kesamaan itu betul: $${a} + \\square = ${b} \\times ${c}$.`), a: T(`$${b * c - a}$`), w: W(T(`Right-hand side: $${b} \\times ${c} = ${b * c}$`, `Sebelah kanan: $${b} \\times ${c} = ${b * c}$`), T(`$\\square = ${b * c} - ${a} = ${b * c - a}$`)), sp: 'xs' };
    if (v === 1) {
      const off = r.chance() ? 0 : r.pick([-2, -1, 1, 2]), rhs = a * b + c + off;
      return { q: T(`Is the statement $${a} \\times ${b} + ${c} = ${rhs}$ true or false?`, `Adakah pernyataan $${a} \\times ${b} + ${c} = ${rhs}$ betul atau salah?`), a: off === 0 ? T('True', 'Betul') : T('False', 'Salah'), w: W(T(`Multiply before adding: $${a} \\times ${b} + ${c} = ${a * b} + ${c} = ${a * b + c}$`, `Darab sebelum tambah: $${a} \\times ${b} + ${c} = ${a * b} + ${c} = ${a * b + c}$`), off === 0 ? T(`This is the same as the right-hand side, so the statement is true.`, `Ini sama dengan sebelah kanan, maka pernyataan itu betul.`) : T(`The right-hand side is $${rhs}$, not $${a * b + c}$, so the statement is false.`, `Sebelah kanan ialah $${rhs}$, bukan $${a * b + c}$, maka pernyataan itu salah.`)), sp: 'xs' };
    }
    const s = a + b;
    if (v === 2) {
      const o = r.pick([
      ...
      return { q: T(`$${a} + ${b} = ${s}$. Suppose ${d} is ${o[0].en} the left-hand side. What must be done to the right-hand side to keep the equality true?`, `$${a} + ${b} = ${s}$. Andaikan ${d} ${o[0].ms} sebelah kiri. Apakah yang mesti dilakukan pada sebelah kanan supaya kesamaan kekal betul?`), a: T(`${o[3].en} ${d} on the right-hand side too: $${o[4]} = ${s} ${o[1]} ${d}$`, `${o[3].ms} ${d} pada sebelah kanan juga: $${o[4]} = ${s} ${o[1]} ${d}$`), w: W(T('An equality stays true only if the same operation is done to both sides.', 'Kesamaan kekal betul hanya jika operasi yang sama dilakukan pada kedua-dua belah.'), T(`Left: $${o[4]} = ${o[2]}$; right: $${s} ${o[1]} ${d} = ${o[2]}$`, `Kiri: $${o[4]} = ${o[2]}$; kanan: $${s} ${o[1]} ${d} = ${o[2]}$`)), sp: 's' };
    }
    const k = r.int(2, 6);
  ...
  const aTF = (r) => {
    const t = r.pick(TF);
    return { q: T(`True or false? ${t[0].en} Give a reason.`, `Betul atau salah? ${t[0].ms} Berikan satu sebab.`), a: T(`${t[1] ? 'True' : 'False'}. ${t[2].en}`, `${t[1] ? 'Betul' : 'Salah'}. ${t[2].ms}`), w: W(t[2], T(`So the statement is ${t[1] ? 'true' : 'false'}.`, `Maka pernyataan itu ${t[1] ? 'betul' : 'salah'}.`)), sp: 's' };
  };
  const aPairs = (r) => {
  ...
  const eConcept = (r) => {
    ...
    const B = [
      ...
      [T('A linear equation in one variable has', 'Persamaan linear dalam satu pemboleh ubah mempunyai'), T('exactly one solution', 'tepat satu penyelesaian'), [T('exactly two solutions', 'tepat dua penyelesaian'), T('infinitely many solutions', 'penyelesaian yang tidak terhingga banyaknya'), T('no solution ever', 'tiada penyelesaian sama sekali')], T('For example $2x + 3 = 11$ is true only for $x = 4$.', 'Contohnya $2x + 3 = 11$ betul hanya untuk $x = 4$.')],
  ...
  const aTF3 = (r) => { const t = r.pick(TF3); return { q: T(`True or false? ${t[0].en} Give a reason.`, `Betul atau salah? ${t[0].ms} Berikan satu sebab.`), a: T(`${t[1] ? 'True' : 'False'}. ${t[2].en}`, `${t[1] ? 'Betul' : 'Salah'}. ${t[2].ms}`), w: W(t[2], T(`So the statement is ${t[1] ? 'true' : 'false'}.`, `Maka pernyataan itu ${t[1] ? 'betul' : 'salah'}.`)), sp: 's' }; };
  ...
  const aTF4 = (r) => { const t = r.pick(TFG); return { q: T(`True or false? ${t[0].en} Give a reason.`, `Betul atau salah? ${t[0].ms} Berikan satu sebab.`), a: T(`${t[1] ? 'True' : 'False'}. ${t[2].en}`, `${t[1] ? 'Betul' : 'Salah'}. ${t[2].ms}`), w: W(t[2], T(`So the statement is ${t[1] ? 'true' : 'false'}.`, `Maka pernyataan itu ${t[1] ? 'betul' : 'salah'}.`)), sp: 's' }; };
  const aFindPQ = (r) => {
  ...
  const e65TF = (r) => { const t = r.pick(TF5); return { q: T(`True or false? ${t[0].en} Give a reason.`, `Betul atau salah? ${t[0].ms} Berikan satu sebab.`), a: T(`${t[1] ? 'True' : 'False'}. ${t[2].en}`, `${t[1] ? 'Betul' : 'Salah'}. ${t[2].ms}`), w: W(t[2], T(`So the statement is ${t[1] ? 'true' : 'false'}.`, `Maka pernyataan itu ${t[1] ? 'betul' : 'salah'}.`)), sp: 's' }; };
